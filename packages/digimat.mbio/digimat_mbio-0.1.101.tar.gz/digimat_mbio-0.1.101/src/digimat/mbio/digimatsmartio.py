...
class SIOConfigurator(object):
    def __init__(self, host, token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9'):
        self._host=host
        self._token=token
        self._config={}
        self._updated=False
    ...
    def post(self, path, data=None):
        try:
            ...
            r=requests.post(url, headers=headers, payload=data, verify=False, timeout=2.0)
            if r and r.ok:
                data=r.json()
                return data
        except:
    ...
    def setIP(self, ip, mask='255.255.255.0', gw=None):
        if self.connect():
            ...
            try:
                ...
                config={'dhcp': False, 'staticip': ip, 'staticnetmask': mask}
                r=self.post(api, config)
                print(r)
            except:
                pass
...
class SIOScanner(MBIONetworkScanner):
    ...
    def configureHostFromGateway(self, host, gateway):
        c=SIOConfigurator(host)
        if c.connect():
            self.logger.warning("SIO RECONFIGURE %s" % host)
            pass
