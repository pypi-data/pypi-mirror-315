...
class MBIODevice(object):
    ...
    def ping(self):
        if self._pingRegisterIndex is not None:
            self.logger.debug('Modbus probing device %s' % self.key)
            ...
            self.logger.debug('Generic probing device %s' % self.key)
            self.gateway.checkIdleAfterSend()
    ...
    def richstr(self):
        return '[yellow]%s[/yellow]([bold]%s[/bold]=%s/%s, %s)' % (self.__class__.__name__, self.key, self.vendor, self.model, self.richstatestr())
