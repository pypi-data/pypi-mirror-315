...
class ManagerUserPermission(RoleBasedPermission):
    """
    Permission that allows only users with the role 'manager'.
    """

    def has_permission(self, request: Request, view: Any) -> bool:
        """
        Asserts that the user is a manager.
        """
        return self._validate_user_role(request, allowed_roles=["manager"])
...
class AnyLoggedUserPermission(RoleBasedPermission):
    ...
    def has_permission(self, request: Request, view: Any) -> bool:
        ...
        """
        return self._validate_user_role(request, allowed_roles=["industry-seller", "employee", "manager", "owner"])
