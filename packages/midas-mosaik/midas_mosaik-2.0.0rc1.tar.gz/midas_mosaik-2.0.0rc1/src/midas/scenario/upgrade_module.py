...
class UpgradeModule(ABC):
    ...
    def _start_simulator(self):
        ...
        self.scenario.makelists.sim_config[self.sim_params["sim_name"]] = {
            cmd: import_str
        }
        ...
        if not self.scenario.sim_started(self.sim_key):
            ...
            #     sid = getattr(self.scenario.get_sim(self.sim_key),
            # "_sim").sid
            # self.logger.debug(
            #     "Started simulator %s (key: %s).", sid, self.sim_key
            # )
    ...
    def start_model(
        ...
        if not self.scenario.model_started(model_key, self.sim_key):
            ...
            # self.logger.debug(
            #     "Created model %s (key: %s).", entity.full_id, model_key
            # )
        ...
        #     self.logger.info(
        #         "Model %s (key: %s) already started", entity.full_id,
        # model_key
        #     )
        #     return entity.full_id
