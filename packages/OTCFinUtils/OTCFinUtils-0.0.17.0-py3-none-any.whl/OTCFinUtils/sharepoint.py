...
from OTCFinUtils.security import get_flow_token, get_graph_token
...
def load_document(dataverse_url: str, token: str, document_path: str, drive: str) -> BytesIO:
    site_id, drive_id = get_connection_details(dataverse_url, token, drive)
...
def get_connection_details(dataverse_url: str, token: str, drive: str = "account") -> tuple[str, str]:
    ...
    flow_headers = {"Authorization": f"Bearer {flow_token}", "Content-Type": "application/json"}
...
def create_excel_document(
    ...
    access_token = get_graph_token()
    site_id, drive_id = get_connection_details(dataverse, token, drive)
