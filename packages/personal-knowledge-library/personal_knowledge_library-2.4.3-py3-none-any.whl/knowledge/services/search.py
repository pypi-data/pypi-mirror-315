...
class SemanticSearchClient(WacomServiceAPIClient):
    ...
    def retrieve_documents_chunks(self, locale: LocaleCode, uri: str, max_retries: int = 3, backoff_factor: float = 0.1,
                                  auth_key: Optional[str] = None) -> List[VectorDBDocument]:
        """
        ...
        raise handle_error("Failed to retrieve the document.", response, headers=headers,
                           parameters={"locale": locale, "uri": uri})

    def retrieve_labels(self, locale: LocaleCode, uri: str,  max_retries: int = 3, backoff_factor: float = 0.1,
                        auth_key: Optional[str] = None) -> List[VectorDBDocument]:
        """
        ...
        with requests.Session() as session:
            ...
            if response.ok:
                return [VectorDBDocument(elem) for elem in response.json()]
        raise handle_error("Failed to retrieve the labels.", response, headers=headers,
                           parameters={"locale": locale, 'uri': uri})

    def count_documents(self, locale: LocaleCode, concept_type: Optional[str] = None,
    ...
    def document_search(self, query: str, locale: LocaleCode, filters: Optional[Dict[str, Any]] = None,
                        ...
                        auth_key: Optional[str] = None) -> DocumentSearchResponse:
        """
    ...
    def labels_search(self, query: str, locale: LocaleCode, filters: Optional[Dict[str, Any]] = None,
                      max_results: int = 10, max_retries: int = 3, backoff_factor: float = 0.1,
                      auth_key: Optional[str] = None) \
