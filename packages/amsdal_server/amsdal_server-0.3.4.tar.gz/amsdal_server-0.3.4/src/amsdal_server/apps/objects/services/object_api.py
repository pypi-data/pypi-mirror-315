...
class ObjectApi(PermissionsMixin, ObjectDataMixin):
    ...
    async def _async_bulk_create_objects(
        ...
        *,
        ...
        try:
            await model_class.objects.bulk_acreate(object_items)
        except ObjectAlreadyExistsError as e:
            msg = 'Object with the same ID already exists'
            raise ValueError(msg) from e
    ...
    async def bulk_update_objects(
        ...
        *,
        ...
        if AmsdalConfigManager().get_config().async_mode:
            return await cls._async_bulk_update_objects(  # type: ignore[misc]
                user,
                base_url,
                data,
                load_references=load_references,
    ...
    async def bulk_partial_update_objects(
        ...
        *,
        ...
        if AmsdalConfigManager().get_config().async_mode:
            return await cls._async_bulk_partial_update_objects(  # type: ignore[misc]
                user,
                base_url,
                data,
                load_references=load_references,
    ...
    async def bulk_delete_objects(cls, user: BaseUser, data: list[BulkAddressBody]) -> None:
        if AmsdalConfigManager().get_config().async_mode:
            return await cls._async_bulk_delete_objects(user, data)  # type: ignore[misc]
    ...
    def _get_model_class(
        ...
        return class_manager.import_model_class(
            ...
            class_manager.resolve_schema_type(class_item.object_id),  # type: ignore[arg-type]
        )
