...
Op_make_scalar_time_point_ns = fire.Opcode(
    "fireducks.make_scalar.timestamp_ns"
)
...
Op_make_null_scalar_time_point_ns = fire.Opcode(
    "fireducks.make_null_scalar.timestamp_ns"
)
Op_make_null_scalar_timedelta_ns = fire.Opcode(
    "fireducks.make_null_scalar.timedelta_ns"
)
...
Op_rename_index = fire.Opcode("fireducks.rename_index")
...
def take(table, indices, axis, check_boundary, check_negative, ignore_index):
...
def make_null_scalar_time_point_ns():
    return _build_op(Op_make_null_scalar_time_point_ns, [Ty_scalar], []).outs[
        0
    ]


def make_null_scalar_timedelta_ns():
    return _build_op(Op_make_null_scalar_timedelta_ns, [Ty_scalar], []).outs[0]
