...
class VMSizesList:
    ...
    @staticmethod
    def get_vm_specs(resource) -> VMCapabilitiesMap:
        """
    ...
    def get_location_vmsizes(self, azure_api, location) -> tuple[list[str], VMSpecsMap]:
        """
    ...
    def generate_location_to_vmsize_mapping(self) -> tuple[LocationToVMSizeMap, VMSpecsMap]:
        ...
        if location:
            ...
            self.location_to_vmsize_mapping[location], self.vmsize_to_specs_mapping = self.get_location_vmsizes(
                self.azure_api, location)
        else:
