...
from clusterondemand.brightsetup import use_password_hash
...
from clusterondemand.configuration import CFG_NO_ADMIN_EMAIL
...
class ClusterCreate(ClusterCommand):
    ...
    def generate_bright_conf(self):
        ...
        storage_acc = "brightimagesdev" if self.head_node_image.version.endswith(("-dev", "trunk")) else "brightimages"
        ...
        bs = {
            ...
            "azure": {
                ...
                "head_node_ip": config["head_node_ip"] or "0.0.0.0"
            }
        }
        if use_password_hash(self.head_node_image):
            bs["bright"]["password_hash"] = sha512_crypt.hash(config["cluster_password"])
        else:
            bs["bright"]["password"] = config["cluster_password"]
        ...
        if BcmVersion(config["version"]) >= "9.0":
            if self.use_existing_vnet:
                ...
                bs["azure"]["network"] = {
                    "resource_group": self.resource_group,
                    "vnet": f"{self.cluster_name}-vnet",
                    "subnet": f"{self.cluster_name}-vnet-subnet",
                }

        bs["bright"]["license"] = clusterondemand.brightsetup.get_license_dict()

        if config["admin_email"] != CFG_NO_ADMIN_EMAIL:
            bs["bright"]["admin_email"] = config["admin_email"]

        if config["http_proxy"]:
            bs["bright"]["http_proxy"] = config["http_proxy"]
    ...
    def _head_node_image_name(self):
        return f"{self.cluster_name}-os-disk-image"

    def _head_node_image_blob_name(self):
    ...
    def run(self):
        ...
        if BcmVersion(config["version"]) >= "9.0":
            default_vnet_name = f"{self.cluster_name}-vnet"
        else:
        ...
        if config["create_public_ip"] or config["existing_public_ip"]:
            ...
            if config["run_cm_bright_setup"]:
                ...
                wait_for_cluster(config, public_ip, config["version"])
        else:
            public_ip = "N/A"
