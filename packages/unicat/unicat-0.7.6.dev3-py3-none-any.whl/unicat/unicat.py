...
class Unicat:
    ...
    def get_definitions_by_name(self, names):
        if not len(names):
            return {}
        if not self._definitions_by_name.keys() >= set(names):
            self._update_definitions_by_name()
        definitions_by_name = {
            name: UnicatDefinition(self, self._definitions_by_name.get(name))
    ...
    def get_classes_by_name(self, names):
        if not len(names):
            return {}
        if not self._classes_by_name.keys() >= set(names):
            self._update_classes_by_name()
        classes_by_name = {
            name: UnicatClass(self, self._classes_by_name.get(name))
    ...
    def get_fields_by_name(self, names):
        if not len(names):
            return {}
        if not self._fields_by_name.keys() >= set(names):
            self._update_fields_by_name()
        fields_by_name = {
            name: UnicatField(self, self._fields_by_name.get(name))
    ...
    def _get_queries_by_type_name(self, type, names):
        ...
        queries_by_name = {
            name: UnicatQuery(
                self,
                self._queries_by_type_name.get(self._query_type_name_key(type, name)),
            )
    ...
    def get_record_query_by_name(self, name):
        return self._get_query_by_type_name("record", name)

    def get_record_queries_by_name(self, names):
        return self._get_queries_by_type_name("record", names)

    def get_asset_query_by_name(self, name):
        return self._get_query_by_type_name("asset", name)

    def get_asset_queries_by_name(self, names):
    ...
    def walk_record_query(self, language, query, *, limit=None):
        request = {
            ...
            "page.size": min(100, limit if limit else 999),
        }
        result_count = 0
        while True:
