...
@register_command(
    extending=("sampledb", "v2", "servicetask"),
...
def list_tasks(
    ...
    route = "/sampledb/v2/servicetask".format(limit=limit,
        state=state,
        operation=operation,
        user=user,
...
def reindex_resources(
    ...
    """Starts a reindex service task for samples and/or submissions (DEV)
    
    :param str startTimestamp: Timestamp to start reindexing
    :param str endTimestamp: Timestamp to end reindexing
    :param str resource: Types of resources to be reindexed \(default sample\)
    :param str dataCenter: Which data center\(s\) to reindex at \(default all\)
    :param bool reversed: Reindexing order \(default true\)
    :param json: return the response's body as a ``dict`` parsed from json. ``True`` by
      default. If set to false, the raw ``requests.Response`` object will be returned.
    :param verify: path to a certificate bundle or boolean indicating whether SSL
      verification should be performed.
