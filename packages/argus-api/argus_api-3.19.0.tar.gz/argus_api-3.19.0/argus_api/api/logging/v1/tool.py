...
def submit_tooling_log_events_bulk(
    ...
    """Submit tool log events in a bulk operation (INTERNAL)
...
#: **DEPRECATED** : ``submit_log_events_bulk_1`` is an alias for ``submit_tooling_log_events_bulk``. Exists
#: only for backward compatibility - **do not use** - use ``submit_tooling_log_events_bulk`` instead.
submit_log_events_bulk_1 = register_command(
    extending=("logging", "v1", "tool"),
    module=argus_cli_module,
    alias="submit_log_events_bulk_1"
)(deprecated_alias("submit_log_events_bulk_1")(submit_tooling_log_events_bulk))