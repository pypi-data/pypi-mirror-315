        ...
        if checkpoint_path:
            ...
            for key, value in state_dict.items():
                if key.startswith("net."):
                    new_key = key.replace("net.", "model.net.")
                else:
                    new_key = f"model.{key}"
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Inferring paths"):
                ...
                for path, score in zip(paths, scores):
                    results.append({"filename": os.path.basename(path), "score": score})
...
def demo():
    ...
    ub.saves(results_df, "results.parquet")
    print("Inference done, saved results to results.parquet")
