...
def get_dividend_amount(order_book_ids, start_quarter=None, end_quarter=None, date=None, market="cn"):
    ...
    df = df.groupby(["order_book_id", "quarter", "event_procedure"]).apply(_sum_one_quarter)
...
@export_as_api
def get_buy_back(order_book_ids, start_date=None, end_date=None, fields=None, market='cn'):
    """ 获取股票回购数据
    ...
    data = get_client().execute("get_buy_back", order_book_ids, start_date, end_date, fields, market=market)
    if not data:
        return None
    df = pd.DataFrame.from_records(data)
    df.set_index(["order_book_id", "date"], inplace=True)
