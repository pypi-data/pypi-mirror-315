...
from ..core import Condition, EventData, State, Transition, listify
from ..core import Event, Machine, MachineError
...
class AsyncState(State):
    ...
    async def enter(self, event_data):
        ...
        """
        func_names = ", ".join([function for function in self.on_enter])
        if func_names:
            _LOGGER.debug("%sEntering state %s. Processing callbacks: '%s'", event_data.machine.name, self.name, func_names)
...
class AsyncTransition(Transition):
    ...
    async def _eval_conditions(self, event_data):
        ...
        cond_func = ",".join([f"{'~' if not cond.target else ''}{cond.func}" for cond in self.conditions])
        if not all(res):
            _LOGGER.debug("%sTransition condition failed: %s.", event_data.machine.name, cond_func)
            return False
        elif cond_func:
            _LOGGER.debug("%sTransition condition passed: %s.", event_data.machine.name, cond_func)
        return True
    ...
    async def execute(self, event_data):
        ...
        if self.before:
            before_cb = ", ".join([cb.__name__ for cb in self.before])
            _LOGGER.debug("%sExecuted callback before transition. Callbacks: ", event_data.machine.name, before_cb)
        ...
        if self.after:
            after_cb = ", ".join([cb.__name__ for cb in self.after])
            _LOGGER.debug("%sExecuted callback after transition. %s", event_data.machine.name, after_cb)
        return True
    ...
    async def _trigger(self, event_data):
        ...
        try:
            ...
            try:
                ...
                if self.machine.finalize_event:
