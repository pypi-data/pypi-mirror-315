...
class SdkContext:
    ...
    def __init__(self, sdk_mode: SdkMode, **kwargs):
        ...
        grpc_address = kwargs.get("grpc_address", None)
        if grpc_address is None:
            self._init_mgmt_client(**kwargs)
        else:
    ...
    def _init_mgmt_client(self, **kwargs):
        ...
        if self._sdk_mode in [SdkMode.SDK_COMPILE, SdkMode.SDK_SIMULATE]:
            ...
            num_csx = 1
            task_type = ClusterDetails.TaskInfo.TaskType.WRK
        ...
        self._mgmt_client = ClusterManagementClient(**self._mgmt_client_args)
        with ExitStack() as stack:
            stack.enter_context(self._mgmt_client)
...
class SdkClient:
    ...
    def __init__(self, sdk_mode: SdkMode, **kwargs):
        ...
        if os.path.exists("sdk_test_params.json"):
            with open("sdk_test_params.json", "r", encoding="utf8") as fparam:
                ...
                kwargs["resource_mem"] = data["resource_mem"]
                kwargs["resource_cpu"] = data["resource_cpu"]

        self._stub = None

        self._context = SdkContext(sdk_mode, **kwargs)
    ...
    def download_artifact(self, artifact_name: str, out_path: str):
        ...
        response_iterator = self._stub.sdk_download_files(request)
    ...
    def _upload_files(
        ...
        response = self._stub.sdk_upload_files(generate_artifacts_args())
        if response.code in [
            sdk_common_pb2.StatusCode.SDK_RT_INTERNAL_ERROR,
            sdk_common_pb2.StatusCode.SDK_RT_FILE_ERROR,
        ]:
...
class SdkLauncher(SdkClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        response = self.upload_artifact(artifact_path)
        self._check_sdk_response(
            response, "SdkLauncher creation (upload phase)"
        )

        self._artifact_id = get_artifact_id(artifact_path)

    def __del__(self):
        if self._stub is not None:
            response = self._stub.sdk_shutdown(
    ...
    def run(self, *args):
        ...
        response = self._stub.sdk_run_command(generate_command_params())
...
class SdkRuntime(SdkClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        artifact_id = get_artifact_id(artifact_path)

        # then instantiate the SdkRuntime on the worker
        response = self._stub.sdk_runtime_create(
    ...
    def __del__(self):
        if self._stub is not None:
            if self._active:
                LOGGER.error(
                    """SdkRuntime.stop() should be called before
                SdkRuntime is destroyed"""
                )
                self.stop()
            response = self._stub.sdk_shutdown(
                sdk_appliance_pb2.SdkEmptyMessage()
            )
            self._check_sdk_response(response, "SdkRuntime shutdown")
    ...
    def start(self):
        ...
        response = self._stub.sdk_runtime_start(
            sdk_appliance_pb2.SdkEmptyMessage()
        )
    ...
    def stop(self):
        ...
        response = self._stub.sdk_runtime_stop(
            sdk_appliance_pb2.SdkEmptyMessage()
        )
        self._active = False
    ...
    def memcpy_h2d(
        ...
        **kwargs,
        ...
        nonblock = kwargs["nonblock"]
        response = self._stub.sdk_runtime_memcpy_h2d(generate_h2d_requests())
    ...
    def memcpy_d2h(
        ...
        **kwargs,
        ...
        nonblock = kwargs["nonblock"]
        response_iterator = self._stub.sdk_runtime_memcpy_d2h(request)
    ...
    def call(self, name: str, args: List[int], **kwargs) -> Optional[Task]:
        ...
        response = self._stub.sdk_runtime_call(request)
    ...
    def get_id(self, symbol: str) -> int:
        ...
        response = self._stub.sdk_runtime_get_id(request)
        symbol_id = -1
    ...
    def _check_task_status(self, task: Task, wait: bool) -> bool:
        ...
        if task.is_done:
            ...
            if wait:
                response_iterator = self._stub.sdk_runtime_task_wait(request)
                funcname = "SdkRunner.Task.wait()"
            else:
                response_iterator = self._stub.sdk_runtime_is_task_done(request)
                funcname = "SdkRunner.Task.done()"
            offset = 0
            for response in response_iterator:
                if response.HasField("status"):
                    if (
