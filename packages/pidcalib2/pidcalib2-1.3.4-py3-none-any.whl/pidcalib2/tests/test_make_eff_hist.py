...
def test_make_eff_hists_run_3(config, test_path, tmp_path):
    config.update(
        {
            "bin_vars": ["P"],
            "magnet": "down",
            ...
            "pid_cuts": ["DLLK > 4"],
            "sample": "test",
            "samples_file": "src/pidcalib2/tests/test_data/test_samples.json",
            "max_files": 1,
        }
    )
    ...
    make_eff_hists.make_eff_hists(config)
    eff_histo = pd.read_pickle(
        tmp_path / "test_output/effhists-test-down-Pi-DLLK>4-P.pkl"
    )

    eff_histo_reference = pd.read_pickle(
        test_path / "test_data/effhists-test-down-Pi-DLLK>4-P.pkl"
    )
    assert eff_histo == eff_histo_reference
...
def test_decode_arguments(capsys, test_path, tmp_path):
    ...
    with pytest.raises(SystemExit):
        make_eff_hists.decode_arguments(["--list", "aliases"])
    captured = capsys.readouterr().out
    ...
    with pytest.raises(KeyError):
        make_eff_hists.decode_arguments(["--list", "nonsense"])

    with pytest.raises(SystemExit):
        make_eff_hists.decode_arguments(["--list", "configs"])
    captured = capsys.readouterr().out

    test_string = "Sample                | Magnet | Particle\n"
    test_string += "----------------------|--------|----------\n"
    assert captured.startswith(test_string)
