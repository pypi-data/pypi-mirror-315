...
def kaldi_rspecifier_arg_type(string: str) -> str:
...
def kaldi_wspecifier_arg_type(string: str) -> str:
...
def kaldi_rxfilename_arg_type(string: str) -> str:
...
def kaldi_wxfilename_arg_type(string: str) -> str:
    """argument type to make sure string is a valid extended readable file"""
...
def kaldi_dtype_arg_type(string: str) -> kaldi_io_enums.KaldiDataType:
...
def kaldi_bool_arg_type(string: str) -> bool:
...
def parse_kaldi_config_file(file_path: str, allow_space=True) -> List[str]:
...
class KaldiParser(argparse.ArgumentParser):
    ...
    def print_usage(self, file: Optional[TextIO] = None):
        if file is None:
            file = sys.stderr
