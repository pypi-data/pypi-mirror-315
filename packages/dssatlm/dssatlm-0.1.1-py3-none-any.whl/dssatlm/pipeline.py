...
class DSSATAnyLMPipeline:
    def __init__(
        ...
        self.wandb_params = self.set_up_wandb(wandb_params)
    ...
    def answer_query(self, farmer_input_query):
        ...
        try:
            ...
            _ = self.interpret_simulation_results_for_farmer(
    ...
    def setup_api_keys(self):
        for api_key in API_KEYS_REQUIRED:
            if api_key not in os.environ:
                print(f"{api_key} is required but not found in the environment variables. Please provide it below...")
                os.environ[api_key] = getpass()

        all_present = all([os.environ[api_key] for api_key in API_KEYS_REQUIRED])
        if not all_present:
            raise ValueError("All required API keys are not provided.")
