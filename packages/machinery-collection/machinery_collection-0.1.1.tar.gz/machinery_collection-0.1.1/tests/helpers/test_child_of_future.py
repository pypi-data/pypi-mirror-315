...
class TestChildOfFuture:
    async def test_takes_in_an_original_future(self, final_future):
    ...
    async def test_ensure_future_returns_the_ResettableFuture_as_is(self, final_future):
        with hp.ChildOfFuture(final_future) as fut:
            assert asyncio.ensure_future(fut) is fut

    class TestContextManager:
        async def test_is_cancelled_if_not_resolved_after_block(self, final_future):
        ...
        async def test_is_resolved_if_given_result_in_block(self, final_future):
            assert len(final_future._callbacks) == 1
        ...
        async def test_is_resolved_if_given_exception_in_block(self, final_future):
            assert len(final_future._callbacks) == 1
        ...
        async def test_is_resolved_if_cancelled_in_block(self, final_future):
            assert len(final_future._callbacks) == 1
    ...
    class TestOriginalResolving:
        async def test_cancels_the_fut_if_the_final_future_cancels(self, final_future):
            with hp.ChildOfFuture(final_future) as fut:
                assert len(fut._callbacks) == 2
                assert len(final_future._callbacks) == 2
        ...
        async def test_passes_on_exception_if_the_final_future_has_exception(self, final_future):
            error = ValueError("HI")
        ...
        async def test_cancels_the_fut_if_the_final_future_gets_a_result(self, final_future):
            with hp.ChildOfFuture(final_future) as fut:
                assert len(fut._callbacks) == 2
                assert len(final_future._callbacks) == 2
    ...
    class TestDoneCallbacks:
        async def test_fires_done_callbacks_on_cancel(self, final_future):
            called = []
        ...
        async def test_fires_done_callbacks_on_exception(self, final_future):
            called = []
        ...
        async def test_fires_done_callbacks_on_result(self, final_future):
            called = []
    ...
    class TestResolvingTheFuture:
        async def test_can_have_a_result(self, final_future):
            with hp.ChildOfFuture(final_future) as fut:
                fut.set_result(True)
                assert fut.done()
                await asyncio.sleep(0)
        ...
        async def test_can_have_an_exception(self, final_future):
            error = ValueError("NOPE")
        ...
        async def test_can_be_cancelled(self, final_future):
            with hp.ChildOfFuture(final_future) as fut:
                fut.cancel()
                assert fut.done()
                assert fut.cancelled()
