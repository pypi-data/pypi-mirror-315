...
from redbookweb.exception import (
    DataFetchError,
    DownloadError,
    ErrorEnum,
    IPBlockError,
...
class XhsClient:
    def __init__(
        ...
        self.oss = oss  # 这里存储 OSSManager 实例
    ...
    def get_self_info_from_creator(self):
        ...
        headers = {"referer": "https://creator.xiaohongshu.com/creator/home"}
        return self.get(uri, is_creator=True, headers=headers)

    def get_user_by_keyword(
        self,
        keyword: str,
        page: int = 1,
        page_size: int = 20,
        ...
        while has_more:
            ...
            for note_id in note_ids:
                try:
                    ...
                    if (
                        ErrorEnum.NOTE_ABNORMAL.value.msg in e.__repr__()
    ...
    def login_from_creator(self):
        ...
        headers = {"referer": "https://creator.xiaohongshu.com/login"}
    ...
    def get_creator_note_list(self, tab: int = 0, page: int = 0):
        ...
        headers = {"Referer": "https://creator.xiaohongshu.com/new/note-manager"}
    ...
    def create_complete_multipart_upload(
        self, file_id: str, token: str, upload_id: str, parts: list
    ):
    ...
    def upload_file_with_slice(self, file_id: str, token: str, file_path: str):
        headers = {"X-Cos-Security-Token": token}
        ...
        with open(file_path, "rb") as f:
            ...
            while True:
                ...
                parts.append({"PartNumber": part_num, "ETag": res.headers["Etag"]})
                part_num += 1
    ...
    def create_image_note(
        ...
        for keyword in topics:
            topic_info_dtos = self.get_suggest_topic(keyword)
            if len(topic_info_dtos) > 0:
                topic_info = topic_info_dtos[0]
                    ...
                    topic_list.append(
                        {
                            "id": topic_id,
                            "name": topic_name,
                            "link": f"https://www.xiaohongshu.com/page/topics/{topic_id}?naviHidden=yes",
                            "type": "topic",
                        }
                    )
        # 下载URL中的文件
        ...
        if len(urls) > 0:
            if self.oss is None:
                raise DownloadError(f"OSS Is Not Set")
            try:
                download_list = self.oss.download_urls(urls, "images")
            except DownloadError as e:
                raise e
        # 处理本地文件和下载的文件
        all_files = files + download_list
        images = []
        for file_path in all_files:
            image_type = get_mime_type(file_path)  # 获取文件的 MIME 类型
            image_id, token = self.get_upload_files_permit("image")
            self.upload_file(image_id, token, file_path)
            images.append(
                {
                    "file_id": image_id,
                    "metadata": {"source": -1},
        ...
        if self.oss:
            self.oss.delete_files(download_list)  # 删除下载的文件
