...
# Unit standardization?
# Value form standardization?
# Value transformation? E.g. TRYpros.value_trans_latlon(...)
```
...
print("Unique Values:", spec_form.OrigValueStr.unique())
spec_form[["OriglName", "Comment", "UnitName"]].drop_duplicates()
```
```
# More questions like the following.
# * What do raw values look like? - Examine 
...
# Value form standardization?
# Value transformation?
def value_trans_year(data_year):
    '''
    ...
    data_year["data"] = data_year["data"].assign(
        StdValue = data_year["data"].apply(
            lambda row: TRYpros.extract_year(row)
        , axis=1)
    )
    return data_year
```
