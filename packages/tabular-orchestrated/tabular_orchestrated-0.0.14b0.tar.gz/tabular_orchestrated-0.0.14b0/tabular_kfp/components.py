...
@component(
    ...
    packages_to_install=[f"tabular-orchestrated[mljar]=={version('tabular-orchestrated')}"]
)
def mljartraining(
    ...
    mljar_automl_params: Dict = {'total_time_limit': 43200, 'algorithms': ['Linear', 'Random Forest', 'Extra Trees', 'LightGBM', 'Xgboost', 'CatBoost'], 'train_ensemble': True, 'eval_metric': 'auto', 'validation_strategy': {'validation_type': 'kfold', 'k_folds': 5, 'shuffle': True, 'stratify': True}, 'explain_level': 2}
):
...
@component(
    base_image="python:3.11",
    packages_to_install=[f"tabular-orchestrated[spliter]=={version('tabular-orchestrated')}"]
)
def datasplitter(
    dataset: Input[Dataset] = None,
