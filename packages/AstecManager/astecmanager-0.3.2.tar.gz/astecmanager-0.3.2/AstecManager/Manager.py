        ...
        if self.omero_result:  # if upload to omero , connect
            ...
            for i in range(0, len(astec_dir)):
                ...
                self.omero_dataset_name.append(
                    astec_dir[i].replace("POST/", "").replace("SEG/", "").replace("FUSE/", "").replace("INTRAREG/","").replace(
                        '"', '').replace("'", "").replace("/","_"))
...
class Manager:
    ...
    def generate_shift_to_boundingbox(self,parameters):
        ...
        image_files = [f for f in os.listdir(post_path) if
                       os.path.isfile(os.path.join(post_path, f)) and f.endswith(".nii")]
    ...
    def load_data_for_lineage(self,lineage1, target_generation, info_name="float_boundingbox_to_cell_ratio"):
        ...
        for cell in names_lineage_1:
            ...
            if target_generation == gen and cell in ratio_by_cell_lineage_1:
    ...
    def plot_found_ratios(self,parameters):
        ...
        for target_generation in target_generations:
            ...
            for i in range(0, len(lineage_list)):
                ...
                plt.plot(timepointlist, value, '-', alpha=0.5, label=lineage_list[i].split("/POST/")[1].split("/")[0])
                ...
                for name_key in ratios_history_by_name_lineage_list:
                    ...
                    for lineage in ratios_history_by_name_lineage_list[name_key]:
                        ...
                        plt.plot(timepointlist, value, '-', alpha=0.5,
                                 label=lineage_list[i].split("/POST/")[1].split("/")[0])
                    plt.legend()
                    plt.tight_layout()
                    plt.savefig(image)
                    plt.clf()
