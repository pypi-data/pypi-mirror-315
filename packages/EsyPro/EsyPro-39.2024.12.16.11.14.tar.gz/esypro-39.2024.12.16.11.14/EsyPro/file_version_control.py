...
class ScriptResultManager:
    ...
    #region static properties
    project_path = None
    def __init__(self, author: str, locals:dict, project_path: MyPath=None, result_path: MyPath=None):       
        ...
        self.version = len(self.result_path.get_files(self.__sub_result_folder_name))        
    ...
    def init_once(self, project_path, result_path):
        if ScriptResultManager.project_path is None:
            ...
            try:
                import torch
                def my_repr(tensor, d=1):
                    return f'Tensor{tuple(tensor.shape)}-{tensor.device.__str__()}|{str(tensor.dtype)[6:]}:{tensor.cpu().detach().numpy()}'
                torch.Tensor.__repr__ = my_repr
            except:
                pass
