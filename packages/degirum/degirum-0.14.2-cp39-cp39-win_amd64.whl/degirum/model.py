...
class Model(ABC):
    ...
    def input_shape(self) -> List[List[int]]:
        ...
        ret = []
        for ii in range(len(self._model_parameters.InputShape)):
            if (
                self._model_parameters.InputShape_defined(ii)
                and self._model_parameters.InputShape[ii]
            ):
                ret.append(copy.copy(self._model_parameters.InputShape[ii]))
            else:
                ret.append(
                    [
                        self._model_parameters.InputN[ii],
                        self._model_parameters.InputH[ii],
                        self._model_parameters.InputW[ii],
                        self._model_parameters.InputC[ii],
                    ]
                )
        return ret
    ...
    def input_shape(self, val: List[List[int]]):
        shape = self._model_parameters.InputShape
        N = self._model_parameters.InputN
        H = self._model_parameters.InputH
        W = self._model_parameters.InputW
        C = self._model_parameters.InputC
        ...
        for ii in range(len(self._model_parameters.InputShape)):
            if (
                ...
                if len(val[ii]) != 4:
                    raise DegirumException(
                        f"Tensor shape for input #{ii} must be a list of 4 integers, not {val[ii]}"
                    )
                N[ii] = val[ii][0]
                H[ii] = val[ii][1]
                W[ii] = val[ii][2]
        ...
        self._model_parameters.InputShape = shape
        self._model_parameters.InputN = N
        self._model_parameters.InputH = H
        self._model_parameters.InputW = W
        self._model_parameters.InputC = C
    ...
    def output_class_set(self) -> set:
        ...
        """
        return self._output_class_set

    @output_class_set.setter
    def output_class_set(self, val: set):
        if not isinstance(val, set):
            raise DegirumException(
                f"output_class_set must be a set, not `{type(val).__name__}`"
            )
        if val:
    ...
    def _predict_impl(self, sources):
        ...
        saved_exception: Optional[Exception] = None
        try:
