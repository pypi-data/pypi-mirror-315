...
from Adlib.api import EnumBanco, EnumProcesso, putRequestFunction, EnumStatus
...
def importarArquivos(virtaus, enumBanco: EnumBanco, codigoBanco: int, nomeBanco: str, substring: str, formatoArquivo: str, usuarioWindows: str):
    ...
    try:
        putRequestFunction(EnumStatus.IMPORTANDO, EnumProcesso.INTEGRACAO, enumBanco)
        ...
        if qntArquivos == 0:
            putRequestFunction(EnumStatus.SEM_ARQUIVOS, EnumProcesso.INTEGRACAO, enumBanco)
        ...
        for i, arquivo in enumerate(arquivosFiltrados, start=1):
            ...
            try:
                ...
                print(mensagem)
                mensagemTelegam(token, chatId, mensagem)
                putRequestFunction(EnumStatus.LIGADO, EnumProcesso.INTEGRACAO, enumBanco)
        ...
        print(erro)
        print('Não deu certo')
        putRequestFunction(EnumStatus.ERRO, EnumProcesso.INTEGRACAO, enumBanco)
