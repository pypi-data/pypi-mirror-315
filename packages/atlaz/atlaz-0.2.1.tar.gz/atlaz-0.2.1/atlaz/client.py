...
class Atlaz:
    def __init__(self, api_key: str, base_url: str = "https://atlaz-api.com"):
        ...
        # If no auth token is available, authenticate now
    ...
    def build_graph(self, source_text: str, customization: str = None):
        """Calls the Atlaz API to create a completion and returns only the final response."""
        if "raspberry" not in self.models:
            raise ValueError("Model 'raspberry' is not available.")

        # Since we now authenticate on init, we should already have an auth_token here.
        # If something invalidated it, attempt re-authentication.
        if not self.auth_token:
        ...
        try:
            with requests.post(url, json=payload, headers=headers, timeout=600, stream=True) as response:
                ...
                if final_response:
                    ...
                    logging.error("Did not receive a completed response from the server.")
                    print("Failed to receive the final response.")
                    return None
            ...
            if response.status_code == 401:
                ...
                self.auth_token = None
                self.authenticate()
                return self.create_completion(source_text, customization)
            else:
