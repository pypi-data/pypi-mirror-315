...
class AbsenceRequestModelViewSet(EmployeeViewMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = qs.annotate(
            department=F("employee__position__id"),
        ).select_related(
            "type",
            "employee",
        )
    ...
    def increaseday(self, request, pk=None):
        ...
        if absence_request.type.is_extensible and (number_days := int(request.POST.get("number_days", 1))):
            absence_request.period = TimestamptzRange(
                lower=absence_request.period.lower,
                upper=(absence_request.period.upper + BDay(number_days)).to_pydatetime(),
            )
