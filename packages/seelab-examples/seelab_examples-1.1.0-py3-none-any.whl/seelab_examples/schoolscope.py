...
from .layouts import ui_schoolscope_layout
...
class Expt(QtWidgets.QWidget, ui_schoolscope_layout.Ui_Form):
    TIMER = 2
    loopCounter = 0
    AWGmin = 1
    AWGmax = 5000
    ...
    def __init__(self, device=None):
        ...
        self.sensorList = []
        self.setupUi(self)
        penCols2  = ['#990', '#090', '#900','magenta','cyan']     #pqtgraph pen colors
        htmlcols2 = ['#990', '#090', '#900','magenta','cyan']
        self.resultCols = penCols2
        self.traceCols = [pg.mkPen(p, width=2) for p in penCols2]
        self.htmlColors = htmlcols2
        self.plot_widget = pg.PlotWidget(self.pwinview)
        ...
        self.plot_widget.proxy = pg.SignalProxy(self.plot_widget.scene().sigMouseMoved, rateLimit=60, slot=self.updateTV)
        self.plot_widget.showGrid(x=True, y=True)  # with grid
        ...
        for k in range(self.MAXRES):  # pg textItem to show the Results
            ...
            self.plot_widget.addItem(self.resLabs[k])


        vLine = pg.InfiniteLine(angle=90, movable=False, pen='r')
        self.plot_widget.addItem(vLine, ignoreBounds=True)
        self.plot_widget.vLine = vLine
        self.plot_widget.vLine.setPos(-1)
        ...
        self.plot_widget.setXRange(0, self.tbvals[self.TBval] * 10)
        self.plot_widget.setYRange(-16, 16)
        self.plot_widget.hideButtons()  # Do not show the 'A' button of pg
        ...
        for ch in range(self.MAXCHAN):  # initialize the pg trace widgets
            self.traceWidget[ch] = self.plot_widget.plot([0, 0], [0, 0], pen=self.traceCols[ch])
        self.diffTraceW = self.plot_widget.plot([0, 0], [0, 0], pen=self.traceCols[-1])
        ...
        self.fitSelCB = [self.A1FitCombo, self.A2FitCombo, self.A3FitCombo, self.MICFitCombo]
        self.fitSelLabels = [self.A1Fit, self.A2Fit, self.A3Fit, self.MICFit]
    ...
    def showVoltagesAtCursor(self, xval):
        for k in range(self.MAXRES):
            self.plot_widget.removeItem(self.resLabs[k])
        t = self.timeData[0]
        index = 0
        ...
        self.plot_widget.addItem(self.resLabs[0])
        ...
        for k in range(self.MAXCHAN):
            if self.chanStatus[k] == 1:
                ...
                self.plot_widget.addItem(self.resLabs[k + 1])
    ...
    def updateTV(self, evt):
        ...
        if self.plot_widget.sceneBoundingRect().contains(pos):
            ...
            xval = mousePoint.x()
            self.plot_widget.vLine.setPos(mousePoint.x())
    ...
    def update(self):
        ...
        for a in self.sensorList:
            if a[0].isVisible():
                if a[0].isPaused == 0 or (a[0].currentPage != None and a[
        ...
        if self.Cross.isChecked():
            self.showVoltagesAtCursor(self.plot_widget.vLine.x())
        else:
            for k in range(self.MAXRES):
                try:
                    self.plot_widget.removeItem(self.resLabs[k])
        ...
        if self.loopCounter % 5 == 0:
            for ch in ['A1','A2','A3']:
                if self.voltMeterCB[ch].isChecked() == True:
                    try:
                        v = self.p.get_voltage(ch)  # Voltmeter functions
                    except:
                        self.comerr()
                    self.voltMeterCB[ch].update_value(v)
                else:
    ...
    def showRange(self, ch):
        ...
        self.plot_widget.removeItem(self.scaleLabs[ch])
        ...
        self.plot_widget.addItem(self.scaleLabs[ch])
    ...
    def select_channel(self, ch):
        if self.chanSelCB[ch].isChecked() == True:
            ...
            self.plot_widget.removeItem(self.traceWidget[ch])
        self.showRange(ch)
