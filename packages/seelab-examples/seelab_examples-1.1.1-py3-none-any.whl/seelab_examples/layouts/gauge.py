...
class Gauge(QtWidgets.QWidget):
	...
	def __init__(self, parent=None, name=''):
		...
		self.decimals = True
		self.checked= True
		self.mouseReleaseFunction = None
		...
		self.set_DisplayTitleColor(70, 30, 50, 255)
	...
	def setChecked(self, st):
		self.checked= st
		if st:
			self.set_NeedleColor()
			self.set_enable_filled_Polygon()
		else:
			self.set_NeedleColor(255,0,0,30)
			self.set_enable_filled_Polygon(False)

	def toggleChecked(self,evt):
	...
	def create_values_text(self):
		...
		# angle_distance = (float(self.scale_angle_size) / float(self.scala_main_count))
		# for i in range(self.scala_main_count + 1):
		if self.decimals: text = str('%.2f'%(self.realvalue))
		else: text = str('%d'%(self.realvalue))
		w = fm.width(text) + 1
		h = fm.height()
	...
	def create_title_text(self):
		...
		y = text_radius * math.sin(math.radians(angle))
		# print(w, h, x, y, text)
		text = [x - int(w/2), int(-self.widget_diameter/3) , int(w), int(h)*2, QtCore.Qt.AlignCenter, text]
		painter.drawText(int(text[0]), int(text[1]), int(text[2]), int(text[3]), text[4], text[5])
		# painter.restore()
