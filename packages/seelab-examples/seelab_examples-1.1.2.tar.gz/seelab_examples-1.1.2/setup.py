...
def copy_dir(dir_path):
    base_dir = os.path.join('seelab_examples', dir_path)
...
def make_rules():
	...
	print('installing rules for accessing SEELab3 hardware...', sys.argv)
...
setuptools.setup(
    ...
    url="https://github.com/csparkresearch/seelab3",
    packages=setuptools.find_packages(exclude=['contrib', 'tests']),
    package_data = {
        '': ['*.c','*.qss','*.rules','*.conf', '*.json', '*.jpg','*.jpeg','*.png','*.svg']+[f for f in copy_dir('icons')]+[f for f in copy_dir('online')],
    },
    classifiers=[
    ...
    entry_points={
    'console_scripts': [
        'seelab_examples=seelab_examples:main',
    		],
	},
    ...
    project_urls={  # Optional
        ...
        'Read The Docs': 'https://eyes17lib.readthedocs.io',
        'Buy Hardware': 'https://csparkresearch.in/seelab3',
    },
)
