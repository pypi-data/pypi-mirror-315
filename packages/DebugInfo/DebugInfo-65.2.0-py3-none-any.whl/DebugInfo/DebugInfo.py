...
class 搜索接口类:
    ...
    def 搜索(self,
        ...
        if not posixLocate搜索字串.startswith('^'):
            posixLocate搜索字串 = f'^{posixLocate搜索字串}'
        if not posixLocate搜索字串.endswith('$'):
            posixLocate搜索字串 = f'{posixLocate搜索字串}$'

        if 参考路径全名:
            # 如果有参考路径，则将参考路径体现到 搜索字串中
            posixLocate搜索字串 = f'{参考路径全名}{路径分隔符}{posixLocate搜索字串.lstrip("^")}'

        # 把关键字中的 * 统一替换成 .*, 以支持正则处理
        posixLocate搜索字串 = posixLocate搜索字串.replace('*', '.*')
        ...
        if self.在posix系统中 and posixLocate搜索字串:
            locate指令: str
            locate参数字: str = f'-er{"" if 大小写敏感 else "i"}{"w" if 路径分隔符 in posixLocate搜索字串 else "b"}'
            if 指定命令参数:
                locate指令 = f'locate {指定命令参数.lstrip("locate ")}'
            elif 搜文档:
                # 如果被要求搜文档，则处理搜文档逻辑，不再关注是否要求搜路径
                locate指令 = f'''locate {locate参数字} "{posixLocate搜索字串}" | xargs -I {{}} sh -c 'if [ -f "{{}}" ]; then echo {{}}; fi' '''
            else:
                locate指令 = f'''locate {locate参数字} "{posixLocate搜索字串}" | xargs -I {{}} sh -c 'if [ -d "{{}}" ]; then echo {{}}; fi' '''
