...
class SlicesTab(HorizonTab):
    ...
    def __init__(
        ...
        *,
        ...
        self._change_slice_x = partial(
            self._change_slice_value, selected_slice=self._slice_x
        )
        self._adjust_slice_x = partial(
            self._change_slice_value, selected_slice=self._slice_x, sync_slice=True
        )
        ...
        self._change_slice_visibility_x = partial(
            self._update_slice_visibility, selected_slice=self._slice_x, actor_idx=0
        )
        ...
        self._change_slice_y = partial(
            self._change_slice_value, selected_slice=self._slice_y
        )
        self._adjust_slice_y = partial(
            self._change_slice_value, selected_slice=self._slice_y, sync_slice=True
        )
        ...
        self._change_slice_visibility_y = partial(
            self._update_slice_visibility, selected_slice=self._slice_y, actor_idx=1
        )
        ...
        self._change_slice_z = partial(
            self._change_slice_value, selected_slice=self._slice_z
        )
        self._adjust_slice_z = partial(
            self._change_slice_value, selected_slice=self._slice_z, sync_slice=True
        )
        ...
        self._change_slice_visibility_z = partial(
            self._update_slice_visibility, selected_slice=self._slice_z, actor_idx=2
        )
        ...
        if not self._visualizer.rgb:
            ...
            if len(self._visualizer.data_shape) == 4:
                self._adjust_volume = partial(self._change_volume, sync_vol=True)
    ...
    def _register_visualize_partials(self):
        self._visualize_slice_x = partial(
            ...
            y1=0,
            y2=self._visualizer.data_shape[1] - 1,
            z1=0,
            z2=self._visualizer.data_shape[2] - 1,
        )
        self._visualize_slice_y = partial(
            self._visualizer.slice_actors[1].display_extent,
            x1=0,
            x2=self._visualizer.data_shape[0] - 1,
            z1=0,
            z2=self._visualizer.data_shape[2] - 1,
        )
    ...
    def _change_slice_value(self, slider, selected_slice, *, sync_slice=False):
        ...
        self._visualize_slice_x(
            x1=self._slice_x.selected_value, x2=self._slice_x.selected_value
        )
        self._visualize_slice_y(
            y1=self._slice_y.selected_value, y2=self._slice_y.selected_value
        )
        self._visualize_slice_z(
    ...
    def _update_colormap(self):
        if self._colormap_switcher.selected_value[1] == "dist":
            ...
            rgb = colormap.create_colormap(
                ...
                name=self._colormap_switcher.selected_value[1],
                auto=True,
            )
        num_lut = rgb.shape[0]
        ...
        lut.SetRange(
            self._intensities.selected_value[0], self._intensities.selected_value[1]
        )
    ...
    def build(self, tab_id):
        ...
        if not self._visualizer.rgb:
            x_pos = 0.52
            self._intensities_label.position = (x_pos, 0.85)
            self._colormap_switcher_label.position = (x_pos, 0.56)

            x_pos = 0.60
            self._intensities.position = (x_pos, 0.85)
            ...
            if len(self._visualizer.data_shape) == 4:
                x_pos = 0.52
                self._volume_label.position = (x_pos, 0.15)
