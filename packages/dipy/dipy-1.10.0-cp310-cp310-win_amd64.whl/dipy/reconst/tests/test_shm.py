...
class TestQballModel:
    ...
    def test_single_voxel_fit(self):
        ...
        with warnings.catch_warnings():
            ...
            model = self.model(
                gtab, sh_order_max=4, min_signal=1e-5, assume_normed=True
            )
        ...
        with warnings.catch_warnings():
            ...
            model = self.model(
                gtab, sh_order_max=4, min_signal=1e-5, assume_normed=False
            )
...
def test_anisotropic_power():
    for n_coeffs in [6, 15, 28, 45, 66, 91]:
        for norm_factor in [0.0005, 0.00001]:
            ...
            analytic = np.log(len(range(2, max_order + 2, 2))) - np.log(norm_factor)
