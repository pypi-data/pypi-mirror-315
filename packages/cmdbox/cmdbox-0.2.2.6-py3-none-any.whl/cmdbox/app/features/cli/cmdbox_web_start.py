...
class WebStart(Feature):
    ...
    def get_option(self):
        ...
        return dict(
            ...
            choise=[
                ...
                dict(opt="ssl_cert", type="file", default=None, required=False, multi=False, hide=True, choise=None,
                     discription_ja="SSLサーバー証明書ファイルを指定します。",
                     discription_en="Specify the SSL server certificate file."),
                dict(opt="ssl_key", type="file", default=None, required=False, multi=False, hide=True, choise=None,
                     discription_ja="SSLサーバー秘密鍵ファイルを指定します。",
                     discription_en="Specify the SSL server private key file."),
                dict(opt="ssl_keypass", type="str", default=None, required=False, multi=False, hide=True, choise=None,
    ...
    def apprun(self, logger:logging.Logger, args:argparse.Namespace, tm:float) -> Tuple[int, Dict[str, Any], Any]:
        ...
        try:
            ...
            msg = {"warn":f"Web server start error. {e}"}
            common.print_format(msg, args.format, tm, args.output_json, args.output_json_append)
            return 1, msg, w
