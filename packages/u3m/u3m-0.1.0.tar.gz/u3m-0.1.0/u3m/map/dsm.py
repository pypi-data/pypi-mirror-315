...
from rs_tools import save_map, geo2proj
import os
import laspy
import argparse
...
def generate_dsm(las_fn, outdir, usfeet, current_epsg, target_epsg, target_resolution = 0.5, lonlat=False, out_format='GTiff'):
    ...
    if dsm_id in dsm_list:
        ...
        INTERP_DSM_FRST = interpolate.griddata((x1, y1), newarr.ravel(),
                                (xx, yy),
                                    method='nearest')
        ...
        INTERP_DSM_LAST = interpolate.griddata((x1, y1), newarr.ravel(),
                                (xx, yy),
                                    method='nearest')
...
if __name__ == '__main__':     
    args = get_args()
    generate_dsm(las_fn=args.fn, outdir=args.outdir, usfeet=args.usfeet, current_epsg = args.current_epsg, target_epsg=args.target_epsg, target_resolution=args.target_resolution, lonlat=args.lonlat)