...
import glob
from rs_tools import save_map, LightImage
import os
import argparse
from scipy import ndimage
from u3m.utils.utils import get_intersect_image
import tqdm
from osgeo import gdal

def generate_water_map(outdir, usfeet, target_epsg, resol_ = 0.5, wdwsz_ = 9, sigma_ = 2, denom_ = 2, buffer_ = 5, bigwater_ = 5000, onlybig = True, elevation = False, out_format='GTiff'):
    ...
    for occ_fn,dsm_fn in tqdm.tqdm(zip(occ_list,dsm_list)):
        ...
        fid = '_'.join(os.path.basename(occ_fn).split('_')[1:])[:-4]
...
def refine_water_map(outdir, usfeet, target_epsg, MINSIZE=1000, LEVEL=0.1, out_format='GTiff'):
...
if __name__ == '__main__': 
    ...
    args = get_args()
    generate_water_map(args.fn, args.outdir, args.usfeet, args.SRC)