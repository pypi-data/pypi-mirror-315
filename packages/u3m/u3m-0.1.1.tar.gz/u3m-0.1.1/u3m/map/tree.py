...
def generate_tree_map(wsdir,outdir,usfeet,target_epsg,OHT=5,OPTION1=False,OPTION2=True,out_format='GTiff'):
    ...
    ortho_list = glob.glob(os.path.join(wsdir,'ortho','*.tif'))
    ...
    for bldg_fn in tqdm.tqdm(bldg_list):
        ...
        if totalwater.shape != New_Building.shape:
            New_Building = np.array(Image.fromarray(New_Building).resize(totalwater.shape[::-1]))
        if totalwater.shape != Roughness_Map.shape:
            Roughness_Map = np.array(Image.fromarray(Roughness_Map).resize(totalwater.shape[::-1]))
        if totalwater.shape != NDHM_F.shape:
            NDHM_F = np.array(Image.fromarray(NDHM_F).resize(totalwater.shape[::-1]))
        if totalwater.shape != NDHM_L.shape:
            NDHM_L = np.array(Image.fromarray(NDHM_L).resize(totalwater.shape[::-1]))
        if totalwater.shape != NDVI.shape:
            NDVI = np.array(Image.fromarray(NDVI).resize(totalwater.shape[::-1]))
        ...
        if FILLHOLE_BY_MEDIAN:
            TREE_1D = ndimage.median_filter(TREE_1D,3)
        ...
        if FILLHOLE_BY_MEDIAN:
            TREE_3D = ndimage.median_filter(TREE_3D,3)
        ...
        if FILLHOLE_BY_MEDIAN:
            TREE_5D = ndimage.median_filter(TREE_5D,3)
        ...
        if FILLHOLE_BY_MEDIAN:
            TREE_7D = ndimage.median_filter(TREE_7D,3)
