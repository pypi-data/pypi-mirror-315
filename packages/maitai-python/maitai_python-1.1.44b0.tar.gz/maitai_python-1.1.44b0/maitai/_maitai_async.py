...
from maitai_gen.chat import ChatCompletionParams, ClientParams, EvaluationContentType
        ...
        *,
        ...
        if server_side_inference:
            ...
            if self._client_params is not None:
                request_dict["client_params"] = self._client_params.to_dict()

            try:
                timing.time_pre_request = time.time()
                response = await maitai.Inference.infer_async(
                ...
                if stream:
                    ...
                    return _process_inference_stream_async(
                        response,
                        retry_args,
                        self.create,
                        timing,
                ...
                async for resp in response:
                    ...
                    return MaitaiCompletion(
                        resp.chat_completion_response, response_format=response_format
                    )
        ...
        if not server_side_inference:
            ...
            if stream:
                return _process_async_openai_stream(
                    session_id,
                    reference_id,
                    intent,
                ...
                return MaitaiCompletion(
                    maitai_completion, response_format=response_format
                )
