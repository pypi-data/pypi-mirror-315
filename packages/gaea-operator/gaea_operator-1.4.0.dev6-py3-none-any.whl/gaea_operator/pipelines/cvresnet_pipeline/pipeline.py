...
NAME_TO_IMAGES = {
    BaseTrain.name():
        [
            Image(kind=Accelerator.NVIDIA,
                  name='iregistry.baidu-int.com/windmill-public/train/cvresnet:20241213_1734073206623')
        ],
    BaseEval.name():
        [
            Image(kind=Accelerator.NVIDIA,
    ...
    BaseTransform.name():
        [
            Image(kind=Accelerator.NVIDIA,
                  name="iregistry.baidu-int.com/windmill-public/transform:1.3.9.1"),
    ...
    def __call__(self,
        ...
        train_params = {"skip": self.train_skip,
                        ...
                        "advanced_parameters": '{"epoch":"1",'
                                               ...
                                               '"networkArchitecture":"图像分类多任务模型-高精版"}'}
...
class Eval(BaseEval):
    ...
    def __init__(self, eval_skip: int = -1, 
        ...
        """
            初始化ContainerStep类的实例。
        
        Args:
            eval_skip (int, optional): 跳过多少个步骤进行评估，默认为-1，表示不跳过任何步骤。 Default: -1.
            pre_nodes (Dict[str, ContainerStep], optional): 前置节点字典，包含每个前置节点的名称和对应的ContainerStep实例。 Default: None.
