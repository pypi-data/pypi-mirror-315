#!/usr/bin/env python3
"""PyTorch Training & Finetuning Script

This script is for training and finetuning models on image classification datasets.

This script was started from an version 1.0.9 of the PyTorch Image Models (timm) library
...
def build_default_parser():
    ...
    group.add_argument(
        "--fast-norm", default=False, action="store_true", help="enable experimental fast-norm"
    )
    ...
    group.add_argument(
        "--decay-epochs", type=float, default=90, metavar="N", help="epoch interval to decay LR"
    )
    group.add_argument(
        "--warmup-epochs",
        type=int,
    ...
    group.add_argument(
        "--bce-pos-weight", type=float, default=None, help="Positive weighting for BCE loss."
    )
    group.add_argument(
        "--reprob", type=float, default=0.0, metavar="PCT", help="Random erase prob (default: 0.)"
    )
    group.add_argument(
    ...
    group.add_argument(
        "--mixup", type=float, default=0.0, help="mixup alpha, mixup enabled if > 0. (default: 0.)"
    )
    group.add_argument(
        "--cutmix",
        type=float,
    ...
    ################################################################################################
    ###                             TrivialAugment and RandAugment                               ###
    ################################################################################################
    group.add_argument(
        "--ta",
        action="store_true",
        default=False,
        help="Use TrivialAugment (default: False)",
    )
    group.add_argument(
        "--ra",
        action="store_true",
        default=False,
        help="Use RandAugment (default: False)",
    )
    group.add_argument(
        "--ra-num-ops",
        type=int,
        default=2,
        metavar="N",
        help="RandAugment number of ops (default: 2)",
    )
    group.add_argument(
        "--ra-magnitude",
        type=int,
    ...
    group.add_argument(
        ...
        default=31,
        metavar="N",
        help="RandAugment magnitude bins (default: 31)",
    )
    # group.add_argument(
    #     "--ra-interpolation",
    #     type=str,
    #     default="nearest",
    #     help='RandAugment interpolation (default: "nearest")',
    # )  TODO
    ...
    group.add_argument(
        "--sync-bn", action="store_true", help="Enable NVIDIA Apex or Torch synchronized BatchNorm."
    )
    group.add_argument(
        "--dist-bn",
        type=str,
    ...
    group.add_argument(
        "--split-bn", action="store_true", help="Enable separate BN layers per augmentation split."
    )
    ...
    group.add_argument(
        "--no-prefetcher", action="store_true", default=False, help="disable fast prefetcher"
    )
    group.add_argument(
        "--output",
        default="",
    ...
    group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable linear probing (default: False)",
    )
    group.add_argument(
        "--pct-to-freeze",
        type=float,
        default=0.0,
        metavar="PCT",
        help="Percentage of layers to freeze (default: 0.0)",
    )
    group.add_argument(
        "--stoch-norm",
        action="store_true",
        default=False,
        help="Enable stochastic normalization (default: False)",
    )
    group.add_argument(
        "--bss-reg",
        type=float,
    ...
    group.add_argument(
        ...
        default=0.0,
        metavar="WEIGHT",
        help="Co-tuning regularization weight (default: 0.0)",
    )
    group.add_argument(
        "--delta-reg",
        type=float,
        default=0.0,
        metavar="WEIGHT",
        help="Delta regularization weight (default: 0.0)",
    )
    group.add_argument(
        "--sp-reg",
        type=float,
        default=0.0,
        metavar="WEIGHT",
        help="SP (Starting Point) regularization weight (default: 0.0)",
    )
    return parser
...
def main():
    ...
    source_model.to(device=device)
    if args.channels_last:
        model.to(memory_format=torch.channels_last)  # type: ignore
        source_model.to(memory_format=torch.channels_last)  # type: ignore
    ...
    if args.model_ema:
        ...
        model_ema = utils.ModelEmaV3(
            ...
            device="cpu" if args.model_ema_force_cpu else None,  # type: ignore
        )
        if args.resume:
    ...
    if args.torchcompile:
        ...
        model = torch.compile(model, backend=args.torchcompile)  # type: ignore
    ...
    if mixup_active:
        ...
        if args.prefetcher:
            assert (
                not num_aug_splits
            )  # collate conflict (need to support de-interleaving in collate mixup)
    ...
    lr_scheduler, num_epochs = create_scheduler_v2(
        optimizer,
        **scheduler_kwargs(args, decreasing_metric=decreasing_metric),  # type: ignore
    ...
    if best_metric is not None:
        results["best"] = results["all"][best_epoch - start_epoch]  # type: ignore
...
def validate(
    model, loader, loss_fn, args, device=torch.device("cuda"), amp_autocast=suppress, log_suffix=""
):
