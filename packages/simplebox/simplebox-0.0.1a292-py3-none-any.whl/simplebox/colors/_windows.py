...
def _color(*objects, sep=' ', end="\n", flush: bool = False, file=sys.stdout, cf=ColorForeground.WHITE,
           cb=ColorBackground.BRIGHT_BLACK, style=None):
    ...
    if isinstance(cb, (tuple, list)) and len(cb) == 3:
        cf = ColorBackground.BRIGHT_BLACK
    ...
    cb_attribute = cb.value if cb is not None else ColorBackground.BRIGHT_BLACK.value
...
def _dye(string, cf=ColorForeground.WHITE, cb=ColorBackground.BLACK, style=None):
    warnings.warn("Older Windows cmd.exe not support front dye color, but can output color front.", NotSupportWarning)
    return string
