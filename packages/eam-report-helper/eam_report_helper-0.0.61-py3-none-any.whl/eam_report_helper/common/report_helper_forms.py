        ...
        for k, v in distribution['data'].items():
            if ('value' in v) or ('values' in v):
                ...
                try:
                    ...
                    print(f"IndexError: {v}")
                    try:
                        label = f"{v['value']['first']}"
                    except KeyError:
                        # print(f"KeyError: {v['value']}")
                        label = ""
    ...
    def get_sub_forms(self, distributions: list, users: list) -> list:
        ...
        for sub_form in sub_form_ref_list_filtered:
            ...
            sub_form['matched_elements'] = dict({
                ...
                # 'Assign Supervisor': sub_form['matched_elements']['Assign Supervisor']
            })
    ...
    def format_cell_lists(self, rows: list) -> list:
        ...
        for row in rows:
            for key, value in row.items():
                ...
                if key == 'Select Supervisor':
                    if 'first' and 'last' in value:
                        row[key] = f"{value['first']} {value['last']}"
                    else:
                        row[key] = ''
                    # change key to 'Supervisor' and set value to 1
                    # row[key] =
