...
def Factory(settings, Model):
    ...
    |                                     |            | "echo_level":  0                |
    +-------------------------------------+------------+---------------------------------+
    | "model_part_input_settings"         | Parameters | "prefix": "/ModelData"          |
    +-------------------------------------+------------+---------------------------------+
...
def CreateCoreSettings(user_settings, model):
    ...
    if model[model_part_name].IsDistributed():
        model_part_input_type = "partitioned_model_part_input"
        core_settings[0]["io_settings"]["io_type"] = "parallel_hdf5_file_io"
    else:
        model_part_input_type = "model_part_input"
        core_settings[0]["io_settings"]["io_type"] = "serial_hdf5_file_io"
    core_settings[0]["list_of_operations"] = [
        CreateOperationSettings(model_part_input_type,
                                user_settings["model_part_input_settings"]),
