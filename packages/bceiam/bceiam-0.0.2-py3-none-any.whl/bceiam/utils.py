...
def convert_to_standard_string(input_string):
    ...
    #    return input_string.encode(bceiam.DEFAULT_ENCODING)
        ...
        def _decorated(*args, **kwds):
            for i, v in enumerate(args):
                if f.__code__.co_varnames[i] in types:
                    if v is None:
                        raise ValueError(
                            'arg "%s" should not be None' % (f.__code__.co_varnames[i])
                        )
...
def parse_host_port(endpoint, default_protocol):
    ...
    :type: bceiam.protocol.HTTP or bceiam.protocol.HTTPS
    ...
    try:
        ...
        parse_result = urlparse(
            endpoint, compat.convert_to_bytes(default_protocol.name)
        )
    except Exception as e:
        raise ValueError(
            "Invalid endpoint:%s, error:%s" % (endpoint, compat.convert_to_string(e))
        )

    if parse_result.scheme == compat.convert_to_bytes(bceiam.protocol.HTTP.name):
        protocol = bceiam.protocol.HTTP
        port = bceiam.protocol.HTTP.default_port
    elif parse_result.scheme == compat.convert_to_bytes(bceiam.protocol.HTTPS.name):
        protocol = bceiam.protocol.HTTPS
        port = bceiam.protocol.HTTPS.default_port
    else:
    ...
    if host.lower().startswith(HTTP_PROTOCOL_HEAD):
        ...
        if len(host_split) == 2:
            return (
                host_split[1]
                .lower()
                .startswith(compat.convert_to_bytes(bucket_name.lower()))
            )
        return False
