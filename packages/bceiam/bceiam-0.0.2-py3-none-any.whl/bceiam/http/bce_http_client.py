...
import bceiam
from bceiam import compat
from bceiam import utils
from bceiam.bce_response import BceResponse
from bceiam.exception import BceHttpClientError
from bceiam.exception import BceClientError
from bceiam.http import http_headers
...
def _get_connection(protocol, host, port, connection_timeout_in_millis):
    """
    :param protocol
    :type protocol: bceiam.protocol.Protocol
    :param endpoint
    :type endpoint: str
    ...
    if protocol.name == bceiam.protocol.HTTP.name:
        return http.client.HTTPConnection(
            host=host, port=port, timeout=connection_timeout_in_millis / 1000
        )
    elif protocol.name == bceiam.protocol.HTTPS.name:
...
def send_request(
    ...
    :param config
    :type config: bceiam.BceClientConfiguration
    ...
    :param request:
    :type request: bceiam.internal.InternalRequest

    :return:
    :rtype: bceiam.BceResponse
    """
    _logger.debug(
        b"%s request start: %s %s, %s, %s", http_method, path, headers, params, body
    )
    headers = headers or {}

    user_agent = "bce-idaas-sdk-python/%s/%s/%s" % (
        compat.convert_to_string(bceiam.SDK_VERSION),
        sys.version,
        sys.platform,
    )
    ...
    if isinstance(body, str):
        body = body.encode(bceiam.DEFAULT_ENCODING)
    if not body:
        ...
        raise ValueError("No {} is specified.".format(http_headers.CONTENT_LENGTH))
    ...
    if need_sign_for_sts:
        headers[http_headers.AUTHORIZATION] = sign_function(
            config.bce_credentials, http_method, path, headers, params
        )
    ...
    while True:
        ...
        try:
            ...
            conn = _get_connection(
                protocol, host, port, config.connection_timeout_in_mills
            )

            _logger.debug(
                "request args:method=%s, uri=%s, headers=%s,params=%s, body=%s",
                http_method,
                uri,
                headers,
                params,
            ...
            if config.retry_policy.should_retry(e, retries_attempted):
                delay_in_millis = (
                    config.retry_policy.get_delay_before_next_retry_in_millis(
                        e, retries_attempted
                    )
                )
