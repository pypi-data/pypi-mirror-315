...
def parse_json(http_response, response):
    ...
    :type response: bceiam.BceResponse
    ...
    if body:
        ...
        # Check if the body is a JSON array (List)
        if body.startswith("[") and body.endswith("]"):
            # Parse the JSON array into a Python List
            response.__dict__["result"] = json.loads(body)
        else:
...
def parse_error(http_response, response):
    ...
    :raise bceiam.exception.BceClientError: if http status code is NOT 2xx
    """
    ...
    if bse is None:
        raise BceServerError(
            http_response.reason, request_id=response.metadata.bce_request_id
        )
