...
def _get_canonical_headers(headers, headers_to_sign=None):
    ...
    if headers_to_sign is None or len(headers_to_sign) == 0:
        headers_to_sign = set([b"host", b"content-type"])
    result = []
    signed_headers = []
    for k in headers:
        k_lower = k.strip().lower()
        value = utils.convert_to_standard_string(headers[k]).strip()
        if k_lower.startswith(http_headers.BCE_PREFIX) or k_lower in headers_to_sign:
            str_tmp = b"%s:%s" % (
                utils.normalize_string(k_lower),
...
def sign(
    ...
    sign_key = hmac.new(
        credentials.secret_access_key, sign_key_info, hashlib.sha256
    ).hexdigest()

    canonical_uri = path
    canonical_querystring = utils.get_canonical_querystring(params, True)

    canonical_headers, signed_headers = _get_canonical_headers(headers, headers_to_sign)
    string_to_sign = (b"\n").join(
        [http_method, canonical_uri, canonical_querystring, canonical_headers]
    )
    sign_result = hmac.new(
        compat.convert_to_bytes(sign_key), string_to_sign, hashlib.sha256
    ).hexdigest()
    # convert to bytes
    sign_result = compat.convert_to_bytes(sign_result)

    if signed_headers:
        result = b"%s/%s/%s" % (sign_key_info, (b";").join(signed_headers), sign_result)
    else:
        result = b"%s//%s" % (sign_key_info, sign_result)

    _logger.debug(
        "sign_key=[%s] sign_string=[%d bytes][ %s ]"
