        ...
        if isinstance(key, frozenset):
            ...
            for k in list(key):
                if k in self.linkages:
                    ...
                    if len(unmatched_keys) > 0:
                        raise ValueError(
                            f"Attempting to set linked placeholders for {key}, "
                            + f"but {k} is already linked to {unmatched_keys}."
                        )
            ...
            for k in list(key):
                if k in self.mapping:
                    del self.mapping[k]
                if k in self.linkages:
                    if self.linkages[k] in self.mapping:
            ...
            if is_singular(value):
                ...
                if nvalue == 0:
                    raise ValueError(
                        f"Can not overwrite placeholder {key} "
                        + f"with new values as it is linked to: {self.linkages[key]}"
                    )
                elif nvalue == 1:
                    ...
                    for skey in old_values:
                        old_values[skey] = tuple(old_values[skey][i] for i in idx)
            else:
                if all(v in old_values[key] for v in value):
                    idx = []
                    for new_v in value:
                        for i, old_v in enumerate(old_values[key]):
                            if old_v == new_v and i not in idx:
                                idx.append(i)
                    for skey in old_values:
                        old_values[skey] = tuple(old_values[skey][i] for i in idx)
                else:
...
class Template:
    ...
    def get_all_placeholders(
        ...
        for to_link in link:
            res[tuple(to_link)] = list(zip(*sorted(
                {tuple(Placeholders(match).get(key, None) for key in to_link) for match in matches}
            )))
        if return_matches:
            return (res, matches)
        return res

    def all_matches(self, placeholders: Placeholders, keys_to_fill: Collection[str]=None) -> List[Dict[str, Any]]:
        ...
        for match in self._parts.all_matches():
            if not all(
    ...
    def rich_line(self, all_keys):
        ...
        return (
            unique_part
            + " ("
            + ", ".join("[cyan]" + key + "[/cyan]" for key in keys)
            + ")"
        )
