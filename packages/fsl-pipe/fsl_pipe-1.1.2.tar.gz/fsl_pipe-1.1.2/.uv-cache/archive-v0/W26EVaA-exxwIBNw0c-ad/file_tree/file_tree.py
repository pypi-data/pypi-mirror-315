...
from .template import Placeholders, Template, is_singular, DuplicateTemplate
...
class FileTree:
    ...
    @classmethod
    def empty(
        cls, top_level: Union[str, Template] = ".", return_path=False
    ) -> "FileTree":
        ...
        if isinstance(found_tree, Path):
            ...
            with parse_tree.extra_tree_dirs([found_tree.parent]):
                return cls.from_string(
                    text, top_level, return_path=return_path, **placeholders
                )
        elif isinstance(found_tree, str):
            return cls.from_string(
                found_tree, top_level, return_path=return_path, **placeholders
            )
        elif isinstance(found_tree, FileTree):
            new_tree = cls.empty(top_level, return_path)
    ...
    def copy(
        ...
        """
        new_tree = type(self)(
            dict(self._templates), Placeholders(self.placeholders), self.return_path
        )
        return new_tree
    ...
    def get_template(self, key: str, error_duplicate=True) -> Template:
        ...
        try:
            ...
            if error_duplicate and isinstance(value, DuplicateTemplate):
                ...
                raise ValueError(f"There are multiple templates matching key '{key}': {templates}")
            return value
        except KeyError:
            pass
    ...
    def override(self, new_filetree: "FileTree", required: Collection[str]=[], optional: Collection[str]=[]):
        ...
        old_duplicate_keys = self.template_keys(skip_duplicates=False).difference(self.template_keys(skip_duplicates=True))
        duplicates = [key for key in all_keys if key in old_duplicate_keys]
        if len(duplicates) > 0:
            raise ValueError("Some of the keys to be replaced in the original FileTree are duplicates: %s", ", ".join(duplicates))
        ...
        if len(undefined) > 0:
            raise ValueError("Some required keys are missing from the input FileTree: %s", ", ".join(undefined))

        new_duplicate_keys = new_available_keys.difference(new_filetree.template_keys(skip_duplicates=True))
        duplicates = [key for key in all_keys if key in new_duplicate_keys]
        if len(duplicates) > 0:
            raise ValueError("Some of the keys to be used in the input FileTree are duplicates: %s", ", ".join(duplicates))

        res_filetree = self.copy()
        res_filetree.placeholders.update(new_filetree.placeholders)
        for key in all_keys:
            if key in new_available_keys:
                res_filetree._add_actual_template(new_filetree.get_template(key), key, overwrite=True)
        return res_filetree

    @property
    def _iter_templates(self, ) -> Dict[Template, Set[str]]:
    ...
    def template_keys(self, only_leaves=False, skip_duplicates=True):
        ...
        parents = {t.parent for t in self._iter_templates.keys() if t.parent is not None}
        return {
            key for key in keys if self.get_template(key) not in parents
        }
    ...
    def filter_templates(
        ...
        new_filetree = FileTree(
            {}, self.placeholders.copy(), return_path=self.return_path
        )
    ...
    def update_glob(
        self, template_key: Union[str, Sequence[str]], inplace=False, 
        ...
        if link is None:
            ...
            link = [link]
        link_as_frozenset = [frozenset(l) for l in link]
    ...
    def get_mult(
        self, key: Union[str, Sequence[str]], filter=False, make_dir=False
    ...
    def get_mult_glob(
        ...
        if isinstance(key, str):
            ...
            updates, matches = template.get_all_placeholders(self.placeholders, return_matches=True)
            ...
            return xarray.merge(
                [self.get_mult_glob(k) for k in key],
                join="outer",
    ...
    def fill(self, keep_optionals=True) -> "FileTree":
        ...
        """
        new_tree = FileTree({}, self.placeholders.split()[1], self.return_path)
    ...
    def iter_vars(
        ...
        for sub_placeholders in self.placeholders.iter_over(placeholders):
            yield FileTree(self._templates, sub_placeholders, self.return_path)
    ...
    def to_string(self, indentation=4) -> str:
        ...
        for t in top_level:
            if t not in already_done:
                lines.append(t.as_multi_line(self._iter_templates, indentation=indentation))
                already_done.add(t)
        return "\n\n".join(lines)
    ...
    def _generate_rich_report(self):
        ...
        for key, value in self.placeholders.items():
            ...
            if isinstance(key, frozenset):
                linked_vars.append(key)
                for linked_key, linked_value in value.items():
                    multi_var_table.add_row(
                        linked_key, ", ".join(str(v) for v in linked_value)
                    )
            elif np.array(value).ndim == 1:
                multi_var_table.add_row(key, ", ".join(str(v) for v in value))
            else:
                single_var_table.add_row(key, value)
        if single_var_table.row_count > 0:
            yield single_var_table
        if multi_var_table.row_count > 0:
        ...
        def add_children(t: Template, tree: Optional[Tree]):
            for child in t.children(self._iter_templates.keys()):
