...
pub struct CreateEmbeddingRequest {
    ...
    /// [List models](https://platform.TupleleapAI.com/docs/api-reference/models/list)
    /// API to see all of your available models, or see our
    /// [Model overview](https://platform.TupleleapAI.com/docs/models/overview)
    /// for descriptions of them.
    pub model: String,

    ///  Input text to embed, encoded as a string or array of tokens. To embed multiple inputs in a single request, pass an array of strings or array of token arrays. The input must not exceed the max input tokens for the model (8192 tokens for `text-embedding-ada-002`), cannot be an empty string, and any array must be 2048 dimensions or less. [Example Python code](https://cookbook.TupleleapAI.com/examples/how_to_count_tokens_with_tiktoken) for counting tokens.
    ...
    /// A unique identifier representing your end-user, which will help TupleleapAI
    ///  to monitor and detect abuse. [Learn more](https://platform.TupleleapAI.com/docs/usage-policies/end-user-ids).
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct Embedding {
    ...
    /// depends on the model as listed in the [embedding guide](https://platform.TupleleapAI.com/docs/guides/embeddings).
...
impl From<Base64EmbeddingVector> for Vec<f32> {
    fn from(value: Base64EmbeddingVector) -> Self {
        let bytes = general_purpose::STANDARD
            .decode(value.0)
            .expect("TupleleapAI base64 encoding to be valid");
