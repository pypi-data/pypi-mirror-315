...
class TestSQLAFeatures(tb.SQLATestCase):
    ...
    def test_sqla_multiprops_01(self):
        val = self.sess.query(self.sm.MultiProp)\
                  .filter_by(name='got one').one()
        self.assertEqual(
            {t.target for t in val.tags},
            {'solo tag'},
        )

        val = self.sess.query(self.sm.MultiProp)\
                  .filter_by(name='got many').one()
        self.assertEqual(
            {t.target for t in val.tags},
            {'one', 'two', 'three'},
        )

    def test_sqla_multiprops_02(self):
        val = self.sess.query(self.sm.MultiProp)\
                  .filter_by(name='got one').one()
        self.assertEqual(
            {t.target for t in val.tags},
        ...
        # create and add a few more tags
        self.sess.add(
            self.sm.MultiProp_tags_prop(source=val, target='hello'))
        self.sess.add(
            self.sm.MultiProp_tags_prop(source=val, target='world'))
        self.sess.flush()
        ...
        self.assertEqual(
            {t.target for t in val.tags},
            {'solo tag', 'hello', 'world'},
        )

    def test_sqla_multiprops_03(self):
        val = self.sess.query(self.sm.MultiProp)\
                  .filter_by(name='got one').one()
        self.assertEqual(
            {t.target for t in val.tags},
        ...
        # create and add a few more tags (don't specify source explicitly)
        val.tags.append(
            self.sm.MultiProp_tags_prop(target='hello'))
        val.tags.append(
            self.sm.MultiProp_tags_prop(target='world'))
        self.sess.flush()
    ...
    def test_sqla_multiprops_04(self):
        val = self.sess.query(self.sm.MultiProp)\
                  .filter_by(name='got many').one()
        self.assertEqual(
            {t.target for t in val.tags},
