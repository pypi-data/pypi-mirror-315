...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    if not os.path.isfile(file_path): # if the lst.txt file exist
        SharedMethods.print_message(f"ERROR: Default list file {filename} does not exist. "
                                    "Please go back to VISION and click run first. This should auto generate the required file.","31")
...
def main_menu(simname, filename, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    if option == "1":
        summary_check_option(simname,branch_list,branch_line_list,branch_list_t,sp_list,trans_list,errorwarning_list)

    if option == "2":
        sp_connect = connection_info_built(simname,branch_line_list,sp_list,option_select,branch_list,branch_list_t)

    if option == "3":
        plot_connection(simname,branch_list,branch_line_list,branch_list_t,sp_list,trans_list,errorwarning_list)
...
#===========================Option 3: Plotting =======================================
# option 3 to plot things
def plot_connection(simname,branch_list,branch_line_list,branch_list_t,sp_list,trans_list,errorwarning_list):
    ...
    # # Compute the layout for each connected component and draw
    # for component in connected_components:
    #     subgraph = G.subgraph(component)
    #     node_colors = ['red' if node in sp_node else 'skyblue' for node in subgraph.nodes()]
    #     pos = nx.spring_layout(subgraph)
    #     nx.draw(subgraph, pos, node_size=25, node_color=node_colors, with_labels=True, font_size=6, edge_color='gray')
    ...
    plt.savefig(filename, format="png", dpi=360)  # dpi sets the resolution (dots per inch)
