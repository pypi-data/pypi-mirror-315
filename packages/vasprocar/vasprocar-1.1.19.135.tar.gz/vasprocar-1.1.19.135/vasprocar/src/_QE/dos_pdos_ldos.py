...
if (test1 == 'Calling' and test2 == 'projwave_nc'): passo = 4
if (test1 == 'Calling' and test2 == 'projwave'):    passo = 15

for ii in range(passo):
...
for i in range(1,(n_wfc+1)):
    ...
    r_ion = str(VTemp[7])
    wfc   = str(VTemp[11])
    if (str(VTemp[13]) == 'l'): n_l = str(VTemp[15])
    if (str(VTemp[16]) == 'l'): n_l = str(VTemp[18])
    if (SO == 2):
       if (str(VTemp[13]) == 'j'): n_j = float(VTemp[15])
       if (str(VTemp[16]) == 'j'): n_j = float(VTemp[18])
    #----------------------------------
    if (n_l == '0'): n_l = 's'
    if (n_l == '1'): n_l = 'p'
    if (n_l == '2'): n_l = 'd'
    ...
    # print(f'{number} {filpdos} {n_ion} {r_ion} {wfc} {n_l} {n_j}')
    
    if (SO == 1):
       name_file[i] = filpdos + '.pdos_atm#' + n_ion + '(' + r_ion + ')_wfc#' + wfc + '(' + n_l + ')'
          
    if (SO == 2):
       name_file[i] = filpdos + '.pdos_atm#' + n_ion + '(' + r_ion + ')_wfc#' + wfc + '(' + n_l + '_j' + str(n_j) + ')'
...
for i in range(1,(len(new_name)+1)):
    ...
    name = name.replace('(', ' ( ')  
    name = name.replace(')', ' ) ') 
    name = name.replace('_atm', ' _atm ')
    name = name.replace('_j', ' _j ')
    name = name.split()
    ion[i] = int(name[3])
    orb[i] = str(name[11])
...
pdos_F     = [[0]*(NEDOS+1) for i in range(ni+1)]   # pdos_F[ni][NEDOS]
pdos_P_tot = [0]*(NEDOS+1)                          # pdos_P_tot[NEDOS]
pdos_D_tot = [0]*(NEDOS+1)                          # pdos_D_tot[NEDOS]
pdos_F_tot = [0]*(NEDOS+1)                          # pdos_F_tot[NEDOS]
...
for i in range(1,(len(new_name)+1)):
    ...
    print (f'({i}/{len(new_name)}) {new_name[i-1]}')
    ...
    for j in range (1,(NEDOS+1)):
        for t_orb in range(1,(n_orb+1)):
            if (esc == 0 or (esc == 1 and temp_sn == "sim")):
               ...
               if (t_orb == 4): pdos_F[t_ion][j] = pdos_F[t_ion][j] + pdos[t_orb][t_ion][j]  
               #---------------------------------------------------------------------------
        ...
        pdos_D_tot[j] = pdos_D_tot[j] + pdos_D[t_ion][j]
        pdos_F_tot[j] = pdos_F_tot[j] + pdos_F[t_ion][j]
...
for k in range (1,(NEDOS+1)):
    ...
    dos_pdos_ldos.write(f' {pdos_tot[1][k]} {pdos_P_tot[k]} {pdos_D_tot[k]} {pdos_F_tot[k]}')
