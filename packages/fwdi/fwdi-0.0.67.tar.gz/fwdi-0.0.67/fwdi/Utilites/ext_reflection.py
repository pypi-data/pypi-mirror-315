...
from ..Utilites.global_setting_service import GlobalSettingService, TypeLoggingLevel
...
class ExtReflection():
    ...
    @staticmethod
    def __get_default_v0(lst_sign:list[inspect.Parameter], name_key:str) -> Any:
    ...
    def get_inst_method_info_v2(fn:Callable[..., Any], args:tuple, kwargs:dict)->dict:
            ...
            fn_datas['method_type'] = ExtReflection.get_type_method(fn, fn_datas['signature_has_self'])
            fn_datas['_arg_has_self_'] = ExtReflection.fn_have_self(args, inspect._findclass(fn))

            if fn_datas['_arg_has_self_'] and fn_datas['signature_has_self']:
                fn_datas['method_signature'] = fn_datas['method_signature'][1:]
            elif fn_datas['_arg_has_self_'] and not fn_datas['signature_has_self']:
                fn_datas['origin_args'] = args[1:]
            elif not fn_datas['_arg_has_self_']:
                raise Exception(f'Error signature have self but no self in args! :: {fn.__module__}:{fn}')
    ...
    @staticmethod
    def get_function_info_v2_v0(fn:Callable[..., Any], args:tuple, kwargs:dict)->dict:
            fn_datas:dict = {}
            fn_args:list[dict] = []
            ...
            if len(kwargs) > 0:
                if arg_name in kwargs:
                    ...
                    if ResolveProviderFWDI.contains(arg_type):
                        ...
                        raise Exception(f"Code::1000:Error type:{arg_type} not found in Dependency Collection and no in kwargs.")
            else:
                if ResolveProviderFWDI.contains(arg_type):
                    new_kwargs_params[arg_name] = ResolveProviderFWDI.get_service(arg_type)
            ...
            try:
                ...
                match GlobalSettingService.log_lvl:
                    case TypeLoggingLevel.INFO:
                        ExtReflection.__log__(f"{ex}", 'error')
                    case TypeLoggingLevel.DEBUG:
                        ExtReflection.__log__(f"error exec :{func.__name__} Error:{ex}", 'error')
                    case TypeLoggingLevel.ALL:
                        ExtReflection.__log__(f"sync exec :{func.__module__}::{func.__name__}, args={args}, kwargs:{kwargs}", 'debug')
    ...
    def _inject_(func: _C)->_C:
        ...
        @wraps(func, updated=())
        def __inst_sync_v0(*args, **kwargs)->_C:
            if not ResolveProviderFWDI.is_init():
                return func(*args, **kwargs)
            ...
            if 'is_inject' not in kwargs:
                ...
                match method_info['method_type']:
                    ...
                    case TypeMethod.Static:
                        if method_info['signature_has_self']:
                            new_args = ExtReflection.__instance_gen_new_args(method_info)
                        else:
