...
class DecoratedTask:
    ...
    def remove(self):
        self._task.todotxt.tasks.remove(self._task)
...
class TaskDataModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, config):
        ...
        self.sources = []
        self._tasks = []
        self.sort_order_txt = self.config.get(common.SETTING_GROUP_GENERAL,
    ...
    def removeTask(self, index):
        start = index.row()
        self.beginRemoveRows(self.createIndex(start, 0), 0, 0)
        self._tasks.remove(index.data(TASK_ROLE))
        del index
        self.endRemoveRows()
        self.dataChanged.emit(self.createIndex(start, 0), self.createIndex(start+1, 0))
...
class TaskDescriptionDelegate(QtWidgets.QAbstractItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        baseline = int(middle + task.context.baseline_offset)
...
class MainWindow(QMainWindow):
    def __init__(self, config, pidfile, sources):
        ...
        self.trash_file = self.config.path(common.SETTING_GROUP_GENERAL,
                                           common.SETTING_TRASHFILE,
                                           common.DEFAULT_TRASHFILE);
        self.delete_is = self.config.get(common.SETTING_GROUP_GENERAL,
        ...
        self.actionCopy = QtWidgets.QAction(tr("Cop&y"), self)
        ...
        self.actionIncreasePrio = QtWidgets.QAction(tr("&Increase priority"), self)
        self.actionDecreasePrio = QtWidgets.QAction(tr("De&crease priority"), self)
        self.actionRemovePrio = QtWidgets.QAction(tr("Remove priority"), self)
        self.actionSetPriorityA = QtWidgets.QAction(tr("A"), self)
        self.actionSetPriorityB = QtWidgets.QAction(tr("B"), self)
        self.actionSetPriorityC = QtWidgets.QAction(tr("C"), self)
        self.actionSetPriorityD = QtWidgets.QAction(tr("D"), self)
        self.actionToggleDarkStyle = None
        self.actionDeleteTask = None
        ...
        if self.delete_is != common.DELETE_OPTION_DISABLED:
            self.actionDeleteTask = QtWidgets.QAction(tr("Delete task"), self)
    ...
    def construct(self):
        ...
        if self.actionDeleteTask is not None:
            self.editMenu.addSeparator()
            self.editMenu.addAction(self.actionDeleteTask)
        self.editMenu.addSeparator()
        self.editMenu.addAction(self.actionDelegate)
        self.editMenu.addSeparator()
        priomenu = self.editMenu.addMenu(tr("Set priority"))
        priomenu.addAction(self.actionSetPriorityA)
        priomenu.addAction(self.actionSetPriorityB)
        priomenu.addAction(self.actionSetPriorityC)
        priomenu.addAction(self.actionSetPriorityD)
        self.editMenu.addAction(self.actionIncreasePrio)
        self.editMenu.addAction(self.actionDecreasePrio)
        self.editMenu.addAction(self.actionRemovePrio)
        self.editMenu.addSeparator()
        ...
        keymapping = [
                ...
                (common.SETTING_GK_COPY, self.actionCopy, Qt.WindowShortcut),
                ...
                (common.SETTING_GK_INC_PRIO, self.actionIncreasePrio, Qt.WindowShortcut),
                (common.SETTING_GK_DEC_PRIO, self.actionDecreasePrio, Qt.WindowShortcut),
                (common.SETTING_GK_REM_PRIO, self.actionRemovePrio, Qt.WindowShortcut),
                (common.SETTING_GK_SET_PRIOA, self.actionSetPriorityA, Qt.WindowShortcut),
                (common.SETTING_GK_SET_PRIOB, self.actionSetPriorityB, Qt.WindowShortcut),
                (common.SETTING_GK_SET_PRIOC, self.actionSetPriorityC, Qt.WindowShortcut),
                (common.SETTING_GK_SET_PRIOD, self.actionSetPriorityD, Qt.WindowShortcut),
                ]
        if self.actionDeleteTask is not None:
            keymapping.append((common.SETTING_GK_DELETE_TASK,
                               self.actionDeleteTask,
                               Qt.WindowShortcut))
    ...
    def update_search(self, text):
        new_sort_order = self.config.get(common.SETTING_GROUP_GENERAL,
                                         common.SETTING_SORT_ORDER)
...
class Program:
    def __init__(self, config, sources, pidfile):
        ...
        self.clear_contexts = [context for context in config.list(common.SETTING_GROUP_GENERAL,
                               ...
                               if len(context) > 0]
        self.reuse_recurring = self.config.bool(common.SETTING_GROUP_GENERAL,
        ...
        self.window.actionCopy.triggered.connect(self.do_copy_to_clipboard)
        ...
        self.window.actionIncreasePrio.triggered.connect(self.do_increase_prio)
        self.window.actionDecreasePrio.triggered.connect(self.do_decrease_prio)
        self.window.actionRemovePrio.triggered.connect(self.do_remove_prio)
        self.window.actionSetPriorityA.triggered.connect(lambda: self.do_set_prio('A'))
        self.window.actionSetPriorityB.triggered.connect(lambda: self.do_set_prio('B'))
        self.window.actionSetPriorityC.triggered.connect(lambda: self.do_set_prio('C'))
        self.window.actionSetPriorityD.triggered.connect(lambda: self.do_set_prio('D'))
        self.window.taskList.activated.connect(self.do_edit)

        if self.window.actionDeleteTask is not None:
            self.window.actionDeleteTask.triggered.connect(self.do_delete_task)
    ...
    def do_copy_to_clipboard(self):
        ...
        if task is None:
            return
        QtWidgets.QApplication.instance().clipboard().setText(str(task.data(TASK_ROLE)))
    ...
    def do_increase_prio(self):
        ...
        if taskindex is not None:
            task = taskindex.data(TASK_ROLE)
            if utils.increase_priority(task):
                task.todotxt.save(safe=self.safe_save)
                self.window.taskModel.setData(taskindex, task)

    def do_decrease_prio(self):
        taskindex = self.window.taskList.get_selected_index()
        if taskindex is None:
            return
        ...
        if taskindex is not None:
            task = taskindex.data(TASK_ROLE)
            if utils.decrease_priority(task):
                task.todotxt.save(safe=self.safe_save)
                self.window.taskModel.setData(taskindex, task)

    def do_remove_prio(self):
        taskindex = self.window.taskList.get_selected_index()
        if taskindex is None:
            return
    ...
    def do_toggle_done(self):
        ...
        if taskindex is not None:
            task = taskindex.data(TASK_ROLE)
            utils.toggle_done(task, self.clear_contexts, self.reuse_recurring, self.safe_save)
            task.todotxt.save(safe=self.safe_save)
            self.window.taskModel.setData(taskindex, task)
    ...
    def do_delete_task(self):
        ...
        if self.window.delete_is == common.DELETE_OPTION_DISABLED:
            return

        elif self.window.delete_is == common.DELETE_OPTION_PERMANENT:
            taskindex = self.window.taskModel.ensure_up_to_date(taskindex)
            if taskindex is not None:
                task = taskindex.data(TASK_ROLE)
                task.remove()
                task.todotxt.save(safe=self.safe_save)
                self.window.taskModel.removeTask(taskindex)

        elif self.window.delete_is == common.DELETE_OPTION_TRASH:
            # first save the task in the trash
            trashfile = TodoTxt(self.window.trash_file)
            trashfile.parse()
            trashfile.tasks.append(taskindex.data(TASK_ROLE)._task)
            trashfile.save()
...
def run_qtui(args, config):
    ...
    p = Program(config, utils.open_sources(args, config), fullpath)
