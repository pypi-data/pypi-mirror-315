...
def pre_model(X, y, VS, model_name, bootstrap_times=5):
    ...
    models = {
        ...
        'Ridge': MultiOutputRegressor(Ridge()),
        'SVR': MultiOutputRegressor(SVR()),
        'GaussianProcess': GaussianProcessRegressor(alpha=0.1)
    }
    ...
    if isinstance(best_model, GaussianProcessRegressor):
        # If the model is Gaussian Process
        y_pred, y_std = best_model.fit(X,y).predict(VS, return_std=True)
        vs_mean = y_pred[0]
