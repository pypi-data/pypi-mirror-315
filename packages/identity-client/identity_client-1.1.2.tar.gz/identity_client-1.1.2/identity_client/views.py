...
def login(request):
    ...
    try:
        ...
        message = (
            f"Token request failed. Status: {e.response.status_code}, "
            f"Auth Redirect URL: {auth_redirect_url}"
            f"Reason: {e.response.reason}, Error Details: {e.response.text[:500]}..."
        )
    ...
    try:
        ...
        message = (
            f"Introspection failed. Status: {e.response.status_code}, "
            f"Reason: {e.response.reason}, Error Details: {e.response.text[:500]}..."
        )
        return HttpResponse(f"Unauthorized: {message}", status=401)
