...
def retrieve_logFile_or_fallback_to_db(logId):
    """Gets the log body for the provided logId from file (if exists) otherwise from the Org"""
    logRecord,fullBody,filepath = retrieve_fullBody_from_File_Id(logId)
...
def worker_read_logBody_from_org(q):
    while True:
        try:
            Id = q.get()
            retrieve_logFile_or_fallback_to_db(Id)
    ...
    try:
        ...
        while (True):
            ...
            if len(logRecords) > 0:
                ...
                #Filter out records already processed
                ...
                if pc.get('parseSmallFiles')==True:
                ...
                for id in ids_not_processed:
                    pc['downloadBody_queue'].put( (id) )

                if len(logIds)>0:
                    parse_apexlogs_tail(logIds=logIds,pc=pc,raiseKeyBoardInterrupt=True,raise_no_log=False)
...
def parse_apexlogs_tail(pc,logIds=None,raiseKeyBoardInterrupt=False,printProgress=False,threads=10,raise_no_log=True):
    ...
    for num,item in enumerate(items):
        ...
        try:
            ...
            if filepath == None:
                continue
            pc['logRecord'],pc['body'],pc['filepath'] = retrieve_fullBody_from_File(filepath) #retrieve_logFile_or_fallback_to_db(pc['logId'])
...
def set_apexlog_body_in_pc(pc):
    ...
    if file.exists(filename):
        ...
        pc['logRecord'],pc['body'],pc['filepath'] = retrieve_logFile_or_fallback_to_db(pc['logId'])
    if pc['body'] == None :   utils.raiseException(errorCode='NO_LOG',error=f'The requested log <{pc["logId"]}> cannot be found. ')
    if len(pc['body'])==0:    utils.raiseException(errorCode='NO_LOG',error=f'The body for the requested log <{pc["logId"]}> is empty. ')
