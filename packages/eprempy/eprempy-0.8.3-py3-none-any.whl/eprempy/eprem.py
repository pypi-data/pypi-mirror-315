...
class Dataset:
    ...
    @property
    def times(self) -> physical.Coordinates:
        """This dataset's time coordinates."""
        if self._times is None:
            self._times = self.streams[0].times
        return self._times

    @property
    def shells(self) -> physical.Points:
        """This dataset's shell indices."""
        if self._shells is None:
            self._shells = self.streams[0].shells
        return self._shells

    @property
    def species(self) -> physical.Symbols:
        """This dataset's species symbols."""
        if self._species is None:
            self._species = self.streams[0].species
        return self._species

    @property
    def energies(self) -> physical.Coordinates:
        """This dataset's energy coordinates."""
        if self._energies is None:
            self._energies = self.streams[0].energies
        return self._energies

    @property
    def mus(self) -> physical.Coordinates:
        """This dataset's mu coordinates."""
        if self._mus is None:
            self._mus = self.streams[0].mus
        return self._mus
    ...
    @property
    def config(self):
        """An interface to this dataset's raw configuration file."""
        return self._config
