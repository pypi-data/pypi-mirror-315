...
from .rotary_pos_emdedding import RotaryEmbedding
...
class GPTModel(LanguageModule):
    ...
    def __init__(
        ...
        if self.position_embedding_type == 'rope':
            ...
            self.rotary_pos_emb = RotaryEmbedding(rotary_dim // 2, True, None, dtype=torch.float16)
    ...
    def forward(
        ...
        if self.position_embedding_type == 'rope':
            ...
            rotary_pos_emb = rotary_pos_emb.view(rotary_seq_len, -1, 32, 2)
            # 推理好像只支持一个批次，训练时会有多个批次，所以需要在批次维度上扩展一下。
            batch_size = input_ids.shape[0]
            rotary_pos_emb = rotary_pos_emb.expand(-1, batch_size, -1, -1)
