...
class Attention(MegatronModule, ABC):
    ...
    def forward(
        ...
        if self.position_encoding_2d:
            # print("*********************run 2d endocer**********************")
            # print("2d encoder: ", position_ids.shape)
            q1, q2 = query.chunk(2, dim=(query.ndim - 1))
            k1, k2 = key.chunk(2, dim=(key.ndim - 1))
            ...
            q2, k2 = apply_rotary_pos_emb_index(q2, k2, cos, sin,
                                                block_position_ids)
            query = torch.concat([q1, q2], dim=(q1.ndim - 1))
            key = torch.concat([k1, k2], dim=(k1.ndim - 1))
        # else:
        #     # if rotary_pos_emb is not None: 
        #     #     q_pos_emb, k_pos_emb = rotary_pos_emb
        ...
        #     query, key = apply_rotary_pos_emb_index(
