...
def generate_tokens_probs_and_return_on_first_stage(
    ...
    MASK, gMASK = tokenizer._tokenizer.mask_token_id, tokenizer._tokenizer.gmask_token_id
    mask_positions, use_gmasks = [], []
    seqs = tokens[:, :min_prompt_length].tolist()
    for seq in seqs:
    ...
    with torch.no_grad():
        attention_mask, position_ids = _build_attention_mask_and_position_ids(
            tokens[:, :min_prompt_length], mask_positions, use_gmasks )
...
def _build_attention_mask_and_position_ids(tokens,mask_positions, use_gmasks=None):
    ...
    attention_mask = get_masks(input_ids=tokens, device=tokens.device)
