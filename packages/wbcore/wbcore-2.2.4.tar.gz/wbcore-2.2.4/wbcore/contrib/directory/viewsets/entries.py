...
class PersonModelViewSet(EntryModelViewSet):
...
class BankRepresentationViewSet(CompanyRepresentationViewSet):
    serializer_class = BankRepresentationSerializer

    def get_queryset(self):
        return super().get_queryset().filter(id__in=Company.bank_objects.values("id"))
