...
class Audio:
    ...
    @classmethod
    def create_silence(cls, duration: float, sample_rate: float):
    ...
    def export(self, output: BinaryIO, audio_format: str):
...
class Modifier:
    ...
    @classmethod
    def from_str(cls, string: str):  # pylint: disable=unused-argument
...
class PitchChangeModifier(Modifier):
    ...
    def __init__(self, pitch_parameter: float):
...
class AcceleratorModifier(Modifier):
    ...
    def __init__(self, speed_multiplier: float, count: int):
        self.speed_multiplier = speed_multiplier
        self.count = count

    # "hello|a2.0+100"
    # Where 2.0 is speed multiplier and 100 is the count
    @classmethod
    def from_str(cls, string: str):
        ...
        speed_multiplier = float(components[0])
        count = int(components[1])

        return AcceleratorModifier(speed_multiplier=speed_multiplier, count=count)

    def as_str(self) -> str:
        return f"{self.IDENTIFIER}{self.speed_multiplier}+{self.count}"
    ...
    def modify_audio(self, audio: Audio) -> Audio:
        ...
        Returns:
            Audio: modified audio segment
        """
        segment = copy.deepcopy(audio)
        speed_multiplier = self.speed_multiplier
        for _ in range(self.count):
            speed_modifier = SpeedChangeModifier(speed_parameter=speed_multiplier)
            modified_audio = copy.deepcopy(audio)
            if speed_multiplier != 1.0:
                modified_audio = speed_modifier.modify_audio(audio=modified_audio)
...
class Voice:
    ...
    def _extract_modifiers(words: List[Word]) -> List[Word]:
        ...
        for word in words:
            ...
            modifiers: dict[
                str, Union[SpeedChangeModifier, NoOpModifier1, NoOpModifier2]
            ] = {}
