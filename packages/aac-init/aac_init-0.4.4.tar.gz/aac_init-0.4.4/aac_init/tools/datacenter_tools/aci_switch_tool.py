...
class AciSwitchTool:
    ...
    def __init__(
        ...
        self.logger = setup_logger(f"aciswitchtool_{self.hostname}.log")
        self.switch_telnet = {
    ...
    def _telnet_validate_aci_switch(self, telnet_connection):
        ...
        try:
            ...
            if "loader >" in switch_prompt:
                ...
                if "aci" in version_output or "nxos" in version_output:
                    self.aci_switch_workflow = 1
                    self.logger.debug(
                        "ACI/NXOS image identified. Switch validated successfully."
                    )
                    return True
    ...
    def _telnet_set_loader(self, telnet_connection, delay_factor=60):
        ...
        try:
            switch_prompt = telnet_connection.find_prompt(delay_factor=60)
            if "loader >" in switch_prompt:
