...
class Restack:
    ...
    def __init__(self, options: Optional[CloudConnectionOptions] = None):
        ...
        log_with_context("DEBUG", "Restack instance initialized", options=str(options))
    ...
    async def create_service(
        ...
        try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    try:
                        async with session.post(
                            ...
                            log_with_context(
                                "DEBUG", "Workflows APIregistered successfully", status_code=response.status, response_text=response_text
                            )
                    except Exception as e:
                        log_with_context(
            ...
            if (
                ...
                log_with_context(
                    "ERROR",
                    "Failed to start service: Functions in workflow steps need to be imported with import_functions(). See docs at https://docs.restack.io/libraries/python/workflows",
                )
            else:
    ...
    async def get_workflow_memory(
        self, workflow_id: str, run_id: str, memory_name: str
    ) -> Any:
        handle = await self.get_workflow_handle(workflow_id, run_id)
        try:
            return await handle.query(memory_name)
        except Exception as e:
            log_with_context("ERROR", "Failed to get workflow memory", error=str(e))
            raise e
