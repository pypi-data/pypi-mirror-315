...
@click.option("--previous-item", required=False, help="The previous item of the schedule item playlist schedule.")
...
def create_schedule_item_playlist_schedule(ctx, schedule_id, days, duration_time_code, end_time_code, playlist_schedule, previous_item, time_code):
    ...
    try:
        ...
        click.echo({"error": f"Error creating schedule item playlist schedule: {e}"})
        sys.exit(1)