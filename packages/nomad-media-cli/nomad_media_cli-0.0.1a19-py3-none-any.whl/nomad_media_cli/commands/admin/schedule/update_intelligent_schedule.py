...
@click.option("--name", required=False, help="The name of the intelligent schedule.")
@click.option("--thumbnail-asset", required=False, help="The thumbnail asset of the intelligent schedule in JSON dict format.")
@click.option("--time-zone-id", required=False, help="The time zone ID of the intelligent schedule.")
@click.pass_context
def update_intelligent_schedule(ctx, schedule_id, default_video_asset, name, thumbnail_asset, time_zone_id):
    ...
    try:
        ...
        click.echo({"error": f"Error updating intelligent schedule: {e}"})
        sys.exit(1)