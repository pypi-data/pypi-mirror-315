...
@click.option("--object-key", help="Object-key only of the Asset (file or folder) to delete teh asset ad break for. This option assumes the default bucket that was previously set with the `set-bucket` command.")
...
def delete_asset_ad_break(ctx, id, url, object_key, ad_break_id):
    ...
    try:
        ...
        click.echo({"error": f"Error deleting asset ad break: {e}"})
        sys.exit(1)