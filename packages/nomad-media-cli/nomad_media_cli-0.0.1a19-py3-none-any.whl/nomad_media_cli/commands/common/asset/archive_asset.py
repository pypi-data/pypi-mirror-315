...
@click.command()
@click.option("--id", help="The ID of the asset to be archived.")
@click.option("--url", help="The Nomad URL of the Asset (file or folder) to be archived (bucket::object-key).")
@click.option("--object-key", help="Object-key only of the Asset (file or folder) to be archived. This option assumes the default bucket that was previously set with the `set-bucket` command.")
@click.pass_context
def archive_asset(ctx, id, url, object_key):
    ...
    try:
        ...
        click.echo("Asset archived successfully.")
        click.echo(json.dumps(result, indent=4))

    except Exception as e:
        click.echo({"error": f"Error archiving asset: {e}"})
        sys.exit(1)