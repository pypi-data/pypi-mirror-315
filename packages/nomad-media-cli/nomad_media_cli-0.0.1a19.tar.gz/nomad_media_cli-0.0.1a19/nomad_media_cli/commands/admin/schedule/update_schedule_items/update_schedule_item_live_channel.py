...
@click.option("--days", required=False, help="The days of the schedule item live channel in JSON list format.")
...
@click.option("--end-time-code", required=False, help="The end time code of the schedule item live channel. Format: hh:mm:ss;ff.")
@click.option("--live-channel", required=False, help="The live channel of the schedule item live channel in JSON dict format.")
@click.option("--time-code", required=False, help="The time code of the schedule item live channel. Format: hh:mm:ss;ff.")
@click.pass_context
def update_schedule_item_live_channel(ctx, schedule_id, item_id, days, duration_time_code, end_time_code, live_channel, time_code):
    ...
    try:
        ...
        click.echo({"error": f"Error updating schedule item live channel: {e}"})
        sys.exit(1)