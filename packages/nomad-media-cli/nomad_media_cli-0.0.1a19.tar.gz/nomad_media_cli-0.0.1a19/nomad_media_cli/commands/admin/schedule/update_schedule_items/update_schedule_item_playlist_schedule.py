...
@click.option("--days", required=False, help="The days of the schedule item playlist schedule in JSON list format.")
...
@click.option("--end-time-code", required=False, help="The end time code of the schedule item playlist schedule. Format: hh:mm:ss;ff.")
@click.option("--playlist-schedule", required=False, help="The playlist schedule of the schedule item playlist schedule in JSON dict format.")
@click.option("--time-code", required=False, help="The time code of the schedule item playlist schedule. Format: hh:mm:ss;ff.")
@click.pass_context
def update_schedule_item_playlist_schedule(ctx, schedule_id, item_id, days, duration_time_code, end_time_code, playlist_schedule, time_code):
    ...
    try:
        result = nomad_sdk.update_schedule_item_playlist_schedule(
            ...
            duration_time_code,
            end_time_code,
            json.loads(playlist_schedule) if playlist_schedule else None,
            time_code
        )
        ...
        click.echo({"error": f"Error updating schedule item playlist schedule: {e}"})
        sys.exit(1)