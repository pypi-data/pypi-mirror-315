...
@click.command()
@click.option("--id", required=True, help="The ID of the asset of the annotation.")
@click.option("--url", help="The Nomad URL of the Asset (file or folder) to delete the annotation for (bucket::object-key).")
@click.option("--object-key", help="Object-key only of the Asset (file or folder) to delete the annotation for. This option assumes the default bucket that was previously set with the `set-bucket` command.")
...
def delete_annotation(ctx, id, url, object_key, annotation_id):
    ...
    try:
        ...
        click.echo("Annotation deleted successfully.")
        click.echo(json.dumps(result, indent=4))

    except Exception as e:
        click.echo({"error": f"Error deleting annotation: {e}"})
        sys.exit(1)