...
try:
    ...
    GET_OCR_FORM_RPC = False
except ModuleNotFoundError:
    GET_OCR_FORM_RPC = True
...
class OCRBase:
    ...
    def _pdocr_client(
        ...
        if GET_OCR_FORM_RPC:
            put_handle = open(os.path.expanduser(screen_path), "rb")
            ...
            from paddleocr.paddleocr import logger as log
            from pylinuxauto.ocr.server import paddle_ocr

            from pylinuxauto.utils import TempLogLevel

            with TempLogLevel(log, logging.INFO):
                return paddle_ocr(screen_path, lang)
    ...
    def ocr(
        ...
        for _ in range(max_match_number):
            ...
            start_x = start_y = 0
            if bbox is not None:
                start_x = bbox.get("start_x")
                start_y = bbox.get("start_y")
                if start_x is None or start_y is None:
                    raise ValueError("缺失 start_x 或 start_y 坐标")

                w = bbox.get("w")
                h = bbox.get("h")
                end_x = bbox.get("end_x")
                end_y = bbox.get("end_y")
                return_one: Union[bool, None] = bbox.get("return_one", True)
