...
def run_polaris_convergence(
    ...
    iterations_fn=lambda config: config.iterations(),
    ...
    for current_iteration in iterations:
        ...
        exe = config.polaris_exe if not current_iteration.is_cristal else config.cristal_exe
...
def check_model_pre_run(config):
    ...
    supply_checker = SupplyChecker(config.data_dir / config.supply_file())
    _ = supply_checker.has_critical_errors(fail_on_errors=False)
    _ = DemandChecker(config.data_dir / config.demand_file()).has_critical_errors(fail_on_errors=False)
    _ = GlobalChecker(config).has_critical_errors(fail_on_errors=False)
    # Logs consistency check errors
    supply_checker.consistency_tests()
    if not len(supply_checker.errors):
        logging.info(f"The Supply file has no additional consistency issues")
    else:
        logging.warning(f"The Supply file has {len(supply_checker.errors)} consistency issues")
        for error in supply_checker.errors:
            logging.error(error)
