...
class ResultKPIs:
    ...
    def __init__(self, inputs: PolarisInputs, cache_dir: Path, population_scale_factor: float):
    ...
    def from_args(
        ...
        return cls(files, cache_dir, population_scale_factor)
    ...
    def cache_all_available_metrics(self, verbose=True, metrics_to_cache=None, skip_cache=False):
        ...
        for m in metrics_to_cache:
            ...
            try:
                ...
                logging.info(f"               {memory_info.rss / (1024 ** 2):.2f} MB")
            except Exception:
    ...
    def metric_activity_distances(self):
        return self._slow_fast(sql_dir / "travel_time.template.sql", "ttime_By_ACT_Average")
    ...
    def network_gaps_by_x(self, x):
        ...
        for t in h5_results.timesteps:
            ...
            df["hour"] = (df["entering_time"] / 3600 / 1000).astype(int)
    ...
    def _get_target_dir(self):
        ...
        if not targets_dir.exists():
            logging.warning("No calibration targets found")
            return None
        return targets_dir

    def _get_simulated_and_target_dict(self, target_type: str):
    ...
    def metric_rmse_vs_observed(self):
        ...
        rmse_boardings = calculate_rmse(simulated, target)
    ...
    def metric_calibration_act_gen(self):
        columns = ["pertype", "acttype", "simulated", "target"]
    ...
    def metric_calibration_destination(self):
        columns = ["acttype", "target", "simulated"]
        if self._get_target_dir() is None:
            return
        ...
        keys = set.intersection(set(target.keys()), set(simulated.keys()))
        return pd.DataFrame([(key, target[key], simulated[key]) for key in keys], columns=columns)
    ...
    def metric_trip_length_distribution(self):
        ...
        bins = int(max(30, np.floor(self.__trip_data.shape[0] / 1000)))
        df = self.__trip_data[self.__trip_data.travel_distance > 0]

        y, x = np.histogram(df.travel_distance.to_numpy() / 1000, bins=bins)
    ...
    def metric_activity_start_time_distributions(self):
        ...
        # ~ 1000 trips per bin on average worked well for both Bloomington and Austin
        bins = int(max(30, np.floor(self.__trip_data.shape[0] / 1000)))
        y, x = np.histogram(self.__trip_data.tstart.to_numpy() / 1000, bins=bins)
    ...
    def __get_trip_data(self):
        ...
        am = TripMetrics(supply_file=self.inputs.supply_db, demand_file=self.inputs.demand_db)
        self.__trip_data = am.data

    def m_etric_traffic_cumulative_gap(self):
