...
def clean_db_for_abm(input_files: PolarisInputs):
    sql_file = sql_dir / "clean_db_for_abm.sql"
    logging.info(f"  Cleaning database for ABM ")
    logging.info(f"     running {sql_file} ")
    logging.info(f"          on {input_files.demand_db}")
...
def clean_db_for_dta(input_files: PolarisInputs):  # pragma : no cover
    sql_file = sql_dir / "clean_db_for_dta.sql"
...
def clean_db_for_cristal(input_files: PolarisInputs):
    sql_file = sql_dir / "clean_db_for_cristal.sql"
    logging.info(f"  Cleaning database for cristal ")
    logging.info(f"     running {sql_file} ")
    logging.info(f"          on {input_files.demand_db}")
    with commit_and_close((input_files.demand_db)) as conn:
        run_sql_file(sql_file, conn)
...
def default_start_of_loop_fn(
    ...
    if current_iteration.is_dta:
        ...
        clean_db_for_cristal(PolarisInputs.from_dir(config.data_dir, config.db_name))
    else:
...
def precache_kpis(current_iteration: ConvergenceIteration, skip_cache: bool = False):
    ...
    ResultKPIs.from_iteration(current_iteration).cache_all_available_metrics(skip_cache=skip_cache)
