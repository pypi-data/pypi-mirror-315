...
SELECT
    ...
    scaling_factor*count(*) as board_count
FROM
...
CREATE TABLE individual_boardings as
SELECT
    pml.object_id,
    pml."value_act_wait_count" as "index",
    ta.agency as agency,
    transit_mode_fn as "mode",
    tr.route as route,
    ts.stop as stop,
    cast(cast(cast(pml.value_act_arrival_time + pml.value_act_bus_wait_time + pml.value_act_rail_wait_time + pml.value_act_comm_rail_wait_time as real)/1800 as int) as real)/2 as HH
FROM
    a.transit_agencies ta,
    a.transit_routes tr,
    a.transit_patterns tp,
    ...
    path_multimodal_links pml
where
    pml.value_act_wait_count > 0 AND
    (pml.value_act_bus_wait_time + pml.value_act_rail_wait_time + pml.value_act_comm_rail_wait_time) > 0 AND
    ta.agency_id = tr.agency_id AND
    tr.route_id = tp.route_id AND
    tp.pattern_id = tt.pattern_id and
    tt.trip_id = pml.value_transit_vehicle_trip AND
    tl.transit_link = pml.value_link AND
    tl.from_node = ts.stop_id
;

DROP TABLE IF EXISTS individual_transfers;
CREATE TABLE individual_transfers as
SELECT
    i1.object_id as object_id,
    i1."index" as index1,
    i1.agency as agency1,
    i1.mode as mode1,
    i1.stop as stop1,
    i1.HH as HH1,
    i2."index" as index2,
    i2.agency as agency2,
    i2.mode as mode2,
    i2.stop as stop2,
    i2.HH as HH2
FROM
    individual_boardings i1,
    individual_boardings i2
WHERE
    i1."index" + 1 = i2."index" and
    i1.object_id = i2.object_id
;
...
SELECT
    ...
    Est_Wait_Count,
    Est_Transfer_Pen,
    scaling_factor*count(*) as trip_count
FROM
    "Path_Multimodal"
Where mode <> 8
GROUP BY
