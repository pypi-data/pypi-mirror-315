...
The :func:`~dataclasses.dataclass` fields that have a "default" value are turned into optional arguments, while the non default fields will
be positional arguments.

The script can then be used in the same way as used with :mod:`~argparse`::
...
It is possible to pass arguments in code, in the same way as the original :meth:`~argparse.ArgumentParser.parse_args` method::
...
script or program, using the :func:`~dataparsers.parse_known` function for this. It works much like :func:`~dataparsers.parse` except that it does not
...
All functions :func:`~dataparsers.parse`,  :func:`~dataparsers.make_parser` and :func:`~dataparsers.parse_known` accepts a `parser=...` keyword argument to modify an
existing parser::
...
because the name of the class attribute is determined by the :func:`~dataclasses.dataclass` field name. So, it is not allowed to pass the
:argument_link:`dest<dest>` parameter.

The parameter :argument_link:`type<type>` is one of the :meth:`~argparse.ArgumentParser.add_argument` parameters that is inferred from the :func:`~dataclasses.dataclass` field properties
when not present.
...
However, the parameter :argument_link:`name_or_flags<name-or-flags>` **must be passed only with flags** (i.e., starting with `-` or `--`). That's because
doesn't make sense to pass a simple not flag name, since the simple name normally determines the class attribute's name,
which is already defined by the :func:`~dataclasses.dataclass` field name.

### Automatic flag creation

One situation where the :argument_link:`default<default>` keyword argument does not automatically makes the argument optional (i.e., creating a
`--` flag) is when the parameter :argument_link:`nargs<nargs>` is set equal to `?` or `*`. That's because this setting also allows that
positional arguments may use a :argument_link:`default<default>` value in the original :meth:`~argparse.ArgumentParser.add_argument` method. So, the flags must be passed
...
:func:`~dataclasses.dataclass` field name (as shown in the example of the "Aliases" section). To prevent that from happening, pass
`make_flag=False`::
...
:argument_link:`action<action>` parameter of the original :meth:`~argparse.ArgumentParser.add_argument` method. If the boolean field is created with no default value, the
flag is still automatically created and the default value of the parameter is set to `False` (this default value can be
modified by the keyword argument :argument_link:`default_bool<default-bool>` of the :func:`~dataparsers.dataparser` decorator - see "Default for booleans")::
...
group will be created later, by the method :meth:`~argparse.ArgumentParser.add_argument_group`, which is used just to separate the arguments in
...
the :func:`~dataparsers.dataparser` decorator, which allows to define options for the :class:`~argparse.ArgumentParser` object.

#### Mutual exclusion

The :argument_link:`mutually_exclusive_group_id<mutually-exclusive-group-id>` defines the name (or the ID) of the mutually exclusive argument group in which the
argument may be included. The identified group will be created later, by the method :meth:`~argparse.ArgumentParser.add_mutually_exclusive_group`,
which is used in :mod:`~argparse` to create mutually exclusive arguments::
...
With that, :mod:`~argparse` will make sure that only one of the arguments in the mutually exclusive group was present on the
command line::
...
Note:
    ...
    :func:`~dataclasses.dataclass` field names, regardless of the value of :argument_link:`make_flag<make-flag>`.
...
### Argument groups using :data:`~typing.ClassVar` (v2.1+)

Two new additional keyword arguments were introduced in v2.1 with functionality analogue to the previous parameters.

The :argument_link:`group<group>` and :argument_link:`mutually_exclusive_group<mutually-exclusive-group>` keyword arguments also accepts a predefined :data:`~typing.ClassVar`, that can be
initialized using 2 new functions: :func:`~dataparsers.group` and :func:`~dataparsers.mutually_exclusive_group`::
...
Using :data:`~typing.ClassVar` names makes it even more easier to prevent typos when identifying groups inside the class. Moreover:
...
command-line arguments. However, there is the original :mod:`~argparse`'s :meth:`~argparse.ArgumentParser.set_defaults` method that allows some additional
attributes to be determined without any inspection of the command line to be added. This functionality can be reproduced
with the :func:`~dataparsers.default` function::
...
See the :func:`~dataparsers.subparser` method in section "Subparsers" for examples.
...
:argument_link:`default_bool<default-bool>` (which is defaults to `False`, i.e., `action="store_true"`)::
...
To define subparsers (or [sub commands](https://docs.python.org/3/library/argparse.html#sub-commands)) use a :data:`~typing.ClassVar`
and initialize it with the function :func:`~dataparsers.subparser`. This function accepts all parameters of the original :meth:`~argparse.ArgumentParser.add_parser`
method, except for `name`: the name of the subparser will receive the :func:`~dataclasses.dataclass` field name.

To add an argument to the created subparser (instead of the main parser), use the :argument_link:`subparser<subparser>` keyword argument of the
:func:`~dataparsers.arg` function and assign to it the previously created field::
...
it with the function :func:`~dataparsers.subparsers`. This function accepts all parameters of the original :meth:`~argparse.ArgumentParser.add_subparsers` method
(except for :argument_link:`dest<dest>`, which automatically receives the :func:`~dataclasses.dataclass` field name)::
...
One additional keyword argument of the function :func:`~dataparsers.subparser` (i.e., beyond those of the the original :meth:`~argparse.ArgumentParser.add_parser`
method) is the :argument_link:`defaults<defaults>` dictionary, which reproduce the functionality of the original :meth:`~argparse.ArgumentParser.set_defaults` method (or the
"main parser-level" :func:`~dataparsers.default` function) for the created subparsers.

One caveat of using this functionality is that the function requires  the dictionary keys to be defined previously as a
main parser-level default field, with the :func:`~dataparsers.default` function::
...
Parser-level defaults with subparsers defaults are the original :mod:`~argparse`'s
...
Like in the original :mod:`~argparse` module, there are 2 possible ways to parse to subparsers: (1) Using parser-level
defaults and (2) using the subparser name.

#### 1. Using parser-level defaults

Parser-level defaults are the original effective way of handling sub-commands, combining the use of the :func:`~dataparsers.subparser`
function with the :argument_link:`defaults<defaults>` keyword argument dictionary, so that each subparser knows which Python function it should
execute. For example::
...
This way, you can let :func:`~dataparsers.parse` do the job of calling the appropriate function after argument parsing is complete.
According to the :mod:`~argparse` documentation, [associating functions with actions like this is typically the easiest way to
...
the :func:`~dataparsers.subparsers` function will work::
...
def arg(
    *name_or_flags: str,
    ...
    This function accepts all parameters of the original :meth:`~argparse.ArgumentParser.add_argument` method (except for :argument_link:`dest<dest>`). Three additional
    ...
    - `name_or_flags` (`str`)
        ...
        name even when :argument_link:`name_or_flags<name-or-flags>` is not given. See the :argument_link:`make_flag<make-flag>` argument for details.


    .. _group:

    - `group` (`Field[Any] | str | int | None`, optional). Defaults to `None`.
        A previously defined :data:`~typing.ClassVar` field name using the function :func:`~dataparsers.group`, or the :argument_link:`title<title>` (or a simple id integer) of
        the argument group in which the argument may be added.

        This is the best way to use the functionality of the method :meth:`~argparse.ArgumentParser.add_argument_group` of the standard :class:`~argparse.ArgumentParser`
        class::
        ...
        By default, :class:`~argparse.ArgumentParser` groups command-line arguments into "positional arguments" and "options" when displaying
        help messages. When there is a better conceptual grouping of arguments than this default one, appropriate groups can
        be created using the :meth:`~argparse.ArgumentParser.add_argument_group` method, that accepts :argument_link:`title<title>` and :argument_link:`description<description>` parameters, which can be
        used to customize the help display.

        To define the :argument_link:`title<title>` and :argument_link:`description<description>` of the argument group, see the :func:`~dataparsers.group` function used to define the
        :data:`~typing.ClassVar`. When a string is passed to the :argument_link:`group<group>` keyword argument, it is associated to the group :argument_link:`title<title>`.


    .. _mutually-exclusive-group:

    - `mutually_exclusive_group` (`Field[Any] | str | int | None`, optional). Defaults to `None`.
        A previously defined :data:`~typing.ClassVar` field name using the function :func:`~dataparsers.mutually_exclusive_group`, or a string or a simple
        ...
        This is the best way to use the functionality of the method :meth:`~argparse.ArgumentParser.add_mutually_exclusive_group` of the standard
        ...
        mutually exclusive argument group, see the :func:`~dataparsers.mutually_exclusive_group` function used to define the :data:`~typing.ClassVar`.
    ...
    - `group_title` (`str | int | None`, optional). Defaults to `None`.
        **Note**:
            ...
            better way to define argument groups is using the :argument_link:`group<group>` keyword argument.

        The :argument_link:`title<title>` (or a simple id integer) of the argument group in which the argument may be added.

        This is quick way to use the functionality of the method :meth:`~argparse.ArgumentParser.add_argument_group` of the standard :class:`~argparse.ArgumentParser`
        class.
    ...
    - `mutually_exclusive_group_id` (`str | int | None`, optional). Defaults to `None`.
        **Note**:
            This argument is kept to maintain compatibility with version prior to v2.1, and may be removed in the future. A
            better way to define mutually exclusive argument groups is using the :argument_link:`mutually_exclusive_group<mutually-exclusive-group>` keyword
            argument.
        ...
        This is a way to use the functionality of the method :meth:`~argparse.ArgumentParser.add_mutually_exclusive_group` of the standard
    ...
    - `make_flag` (`bool | None`, optional). Defaults to `None`.
        ...
        In general, the :argument_link:`default<default>` keyword argument automatically makes the argument optional (i.e., creates a `--`
        flag), but there are some situation when that doesn't happen, e.g., when the parameter :argument_link:`nargs<nargs>` is passed and
        ...
        when :argument_link:`name_or_flags<name-or-flags>` is not given:

        - If :argument_link:`default<default>` value is given (with :argument_link:`nargs<nargs>` not equal to `?` or `*`)::
    ...
    Parameters from the original :meth:`~argparse.ArgumentParser.add_argument` method
    ----------------------------------------------------------------------------------
    ...
    - `action` (`Literal["store", "store_const", "store_true", "store_false", "append", "append_const", "count", "help", "version", "extend"] | type[Action]`, optional). Defaults to `"store"`.
        ...
        :class:`~argparse.ArgumentParser` objects associate command-line arguments with actions. These actions can do just about
        anything with the command-line arguments associated with them, though most actions simply add an attribute
        to the object returned by :meth:`~argparse.ArgumentParser.parse_args`. The :argument_link:`action<action>` keyword argument specifies how the command-line
        ...
        - `"append_const"`:
            This stores a list, and appends the value specified by the :argument_link:`const<const>` keyword argument to the list;
            note that the :argument_link:`const<const>` keyword argument defaults to `None`. The `"append_const"` action is typically
        ...
        - `"help"`:
            ...
            default a help action is automatically added to the parser. See :class:`~argparse.ArgumentParser` for details of how
        ...
        - `"version"`:
            This expects a `version=` keyword argument in the :meth:`~argparse.ArgumentParser.add_argument` call, and prints version
        ...
        - `"extend"`:
            ...
            the same interface. The `BooleanOptionalAction` is available in :mod:`~argparse` and adds support for
    ...
    - `nargs` (`int | Literal["?", "*", "+"]`, optional). Defaults to `None`.
        ...
        :class:`~argparse.ArgumentParser` objects usually associate a single command-line argument with a single action to be taken.
        ...
        - `"?"`:
            ...
            command-line argument. In this case the value from :argument_link:`const<const>` will be produced. Some examples to
            illustrate this::
    ...
    - `const` (`Any | None`, optional). Defaults to `None`.
        A constant value required by some :argument_link:`action<action>` and :argument_link:`nargs<nargs>` selections.
    ...
    - `default` (`Any`, optional). Defaults to `None`.
        ...
        All optional arguments and some positional arguments may be omitted at the command line. The :argument_link:`default<default>`
        keyword argument of :meth:`~argparse.ArgumentParser.add_argument`, whose value defaults to `None`, specifies what value should be used if
        the command-line argument is not present. For optional arguments, the :argument_link:`default<default>` value is used when the
        ...
        If the :argument_link:`default<default>` value is a string, the parser parses the value as if it were a command-line argument. In
        particular, the parser applies any :argument_link:`type<type>` conversion argument, if provided, before setting the attribute on
        ...
        For positional arguments with :argument_link:`nargs<nargs>` equal to `?` or `*`, the default value is used when no command-line
        ...
        **Note**:
            ...
            :argument_link:`name_or_flags<name-or-flags>` argument or explicit passing `make_flag=True`.
    ...
    - `type` (`Callable[[str], T] | FileType | None`, optional). Defaults to `None`.
        **Note**:
        ...
        command-line string should instead be interpreted as another type, such as a `float` or `int`. The :argument_link:`type<type>`
        keyword for :meth:`~argparse.ArgumentParser.add_argument` allows any necessary type-checking and type conversions to be performed.

        If the :argument_link:`type<type>` keyword is used with the :argument_link:`default<default>` keyword, the type converter is only applied if the default
        is a string.

        The argument to :argument_link:`type<type>` can be any callable that accepts a single string. If the function raises
        ...
        given by the :argument_link:`type<type>` keyword. A `JSONDecodeError` would not be well formatted and a `FileNotFoundError`
        exception would not be handled at all.

        Even `FileType` has its limitations for use with the :argument_link:`type<type>` keyword. If one argument uses `FileType` and
        ...
        For type checkers that simply check against a fixed set of values, consider using the :argument_link:`choices<choices>` keyword
        instead.
    ...
    - `choices` (`Iterable[T] | None`, optional). Defaults to `None`.
        ...
        passing a sequence object as the choices keyword argument to :meth:`~argparse.ArgumentParser.add_argument`. When the command line is
        ...
        Note that inclusion in the choices sequence is checked after any :argument_link:`type<type>` conversions have been performed, so
        the type of the objects in the choices sequence should match the :argument_link:`type<type>` specified::
        ...
        Formatted choices override the default :argument_link:`metavar<metavar>` which is normally derived from :argument_link:`dest<dest>`. This is usually what
        you want because the user never sees the dest parameter. If this display isn't desirable (perhaps because
        there are many choices), just specify an explicit :argument_link:`metavar<metavar>`.
    ...
    - `required` (`bool | None`, optional). Defaults to `None`.
        ...
        In general, the :mod:`~argparse` module assumes that flags like `-f` and `--bar` indicate optional arguments,
        which can always be omitted at the command line. To make an option required, `True` can be specified for the
        `required=` keyword argument to :meth:`~argparse.ArgumentParser.add_argument`::
        ...
        As the example shows, if an option is marked as :argument_link:`required<required>`, :meth:`~argparse.ArgumentParser.parse_args` will report an error if that
    ...
    - `help` (`str | None`, optional). Defaults to `None`.
        ...
        The :argument_link:`help<help>` value is a string containing a brief description of the argument. When a user requests help
        (usually by using `-h` or `--help` at the command line), these :argument_link:`help<help>` descriptions will be displayed with
        each argument::
        ...
        or the argument :argument_link:`default<default>`. The available specifiers include the program name, `%(prog)s` and most keyword
        arguments to :meth:`~argparse.ArgumentParser.add_argument`, e.g. `%(default)s`, `%(type)s`, etc.::
        ...
        :mod:`~argparse` supports silencing the help entry for certain options, by setting the :argument_link:`help<help>` value to
        `argparse.SUPPRESS`::
    ...
    - `metavar` (`str | tuple[str, ...] | None`, optional). Defaults to `None`.
        ...
        When :class:`~argparse.ArgumentParser` generates help messages, it needs some way to refer to each expected argument. By
        default, ArgumentParser objects use the :argument_link:`dest<dest>` value as the "name" of each object. By default, for
        positional argument actions, the :argument_link:`dest<dest>` value is used directly, and for optional argument actions, the
        :argument_link:`dest<dest>` value is uppercased. So, a single positional argument with `dest='bar'` will be referred to as `bar`.
        ...
        An alternative name can be specified with :argument_link:`metavar<metavar>`::
        ...
        Note that :argument_link:`metavar<metavar>` only changes the displayed name - the name of the attribute on the :meth:`~argparse.ArgumentParser.parse_args` object
        is still determined by the :argument_link:`dest<dest>` value.

        Different values of :argument_link:`nargs<nargs>` may cause the metavar to be used multiple times. Providing a tuple to :argument_link:`metavar<metavar>`
    ...
    - `dest` (`str | None`, optional). Defaults to `None`.
        **Note**:
        ...
        Most :class:`~argparse.ArgumentParser` actions add some value as an attribute of the object returned by :meth:`~argparse.ArgumentParser.parse_args`. The
        name of this attribute is determined by the :argument_link:`dest<dest>` keyword argument of :meth:`~argparse.ArgumentParser.add_argument`. For positional
        argument actions, :argument_link:`dest<dest>` is normally supplied as the first argument to :meth:`~argparse.ArgumentParser.add_argument`::
        ...
        For optional argument actions, the value of :argument_link:`dest<dest>` is normally inferred from the option strings.
        :class:`~argparse.ArgumentParser` generates the value of :argument_link:`dest<dest>` by taking the first long option string and stripping away the
        initial `--` string. If no long option strings were supplied, :argument_link:`dest<dest>` will be derived from the first short
        ...
        :argument_link:`dest<dest>` allows a custom attribute name to be provided::
    ...
    -------
    `default@arg | Field`:
        A :func:`~dataclasses.dataclass` field with given :argument_link:`default<default>` value of the field and `metadata` dictionary filled with argument parameters.
    """
    ...

def group(title: str | None = None, description: str | None = None) -> Any:
    """Helper function to create :func:`~dataclasses.dataclass` class variables (:data:`~typing.ClassVar`) storing specification about argument groups, used
    later in the method :meth:`~argparse.ArgumentParser.add_argument_group`.

    This function accepts the parameters of the original :meth:`~argparse.ArgumentParser.add_argument_group` method, i.e., :argument_link:`title<title>` and :argument_link:`description<description>`, and
    must be used to define a :data:`~typing.ClassVar` in the class scope.
    ...
    -------
    `Field`:
        A :func:`~dataclasses.dataclass` field with `metadata` dictionary filled with argument group parameters, which must be assigned to a
        :data:`~typing.ClassVar` field.
    """
    ...

def mutually_exclusive_group(*, required: bool = False) -> Any:
    """Helper function to create :func:`~dataclasses.dataclass` class variables (:data:`~typing.ClassVar`) storing specification about mutually exclusive
    argument groups, used later in the method :meth:`~argparse.ArgumentParser.add_mutually_exclusive_group`.

    This function accepts the parameters of the original :meth:`~argparse.ArgumentParser.add_mutually_exclusive_group` method, i.e., :argument_link:`required<required>`, and must be
    used to define a :data:`~typing.ClassVar` in the class scope.
    ...
    -------
    `Field`:
        A :func:`~dataclasses.dataclass` field with `metadata` dictionary filled with mutually exclusive argument group parameters, which must be
        assigned to a :data:`~typing.ClassVar` field.
    """
    ...
...
def subparsers(
    *,
    ...
    """Helper function to create a :func:`~dataclasses.dataclass` field storing specification about a subparser group, used later in the method
    :meth:`~argparse.ArgumentParser.add_subparsers`. This function accepts all parameters of the original :meth:`~argparse.ArgumentParser.add_subparsers` method (except for :argument_link:`dest<dest>`).
    ...
    - `dest` (`str | None`, optional). Defaults to `None`.
        ...
        **Note**: The parameter :argument_link:`dest<dest>` is described here just for documentation. It will raise an error if it is passed to the
        :func:`~dataparsers.subparsers` function, because it is not necessary: the :argument_link:`dest<dest>` keyword argument of the :meth:`~argparse.ArgumentParser.add_subparsers`
        method is taken from the dataclass field name.
    ...
    -------
    `Field[str]`:
        A :func:`~dataclasses.dataclass` field with `metadata` dictionary filled with subparser group parameters.
    """
    ...
...
def subparser(
    *,
    ...
    - `defaults` (`dict[str, Any] | None = None`, optional). Defaults to `None`.
        ...
        **The dictionary keys must be defined previously** with the :func:`~dataparsers.default` function.
    ...
    - `help` (`str`, optional).
        A help message for each subparser command can be given by supplying this argument o :meth:`~argparse.ArgumentParser.add_parser` as
        below::
    ...
    ---------------------------------------------------------------------------
        See the :func:`~dataparsers.dataparser` decorator parameters.

    Returns
    -------
    `Field`:
        A :func:`~dataclasses.dataclass` field with a default values assigned as a instance of a read-only `SubParser` class storing information
        about the subparser, which must be assigned to a :data:`~typing.ClassVar` field.
    """
    ...

def default(default: T | None = None) -> T:
    """Helper function to create a :func:`~dataclasses.dataclass` field storing a parser-level default, used later in the method :meth:`~argparse.ArgumentParser.set_defaults`.

    It allows some additional attributes to be stored without any inspection of the command line to be added.

    **Note**:
        This function must be used prior to pass a `dict` value to the :argument_link:`defaults<defaults>` keyword argument in the function
        :func:`~dataparsers.subparser`.
    ...
    -------
    `Field`:
        A :func:`~dataclasses.dataclass` field with the default attribute value stored in it.
    """
    ...
...
def make_parser(cls: type, *, parser: ArgumentParser | None = None) -> ArgumentParser:
    ...
    Parameters
    ----------
    - `cls` (:argument_link:`type<type>`)
        A :func:`~dataclasses.dataclass` according to which argument parser is created.
    ...
    :class:`~argparse.ArgumentParser`:
        The new :class:`~argparse.ArgumentParser` object or the existing parser with added arguments.
    """
    ...
...
def parse(cls: type[Class], args: Sequence[str] | None = None, *, parser: ArgumentParser | None = None) -> Class:
    ...
    Accepts classes decorated with :func:`~dataclasses.dataclass`.

    Parameters
    ----------
    - `cls` (`type[Class]`)
        A :func:`~dataclasses.dataclass` used as object to take the attributes to parse the command-line arguments.

    - `args` (`Sequence[str] | None`, optional). Defaults to `None`.
        List of strings to parse. The default is taken from `sys.argv`, like the original :meth:`~argparse.ArgumentParser.parse_args` method.
    ...
    -------
    `Class`:
        The populated :func:`~dataclasses.dataclass` with argument values.
    """
    ...
...
def parse_known(
    ...
    Same as :func:`~dataparsers.parse` except that it  it does not produce an error when extra arguments are present. Instead, it returns a two
    ...
    - `args` (`Sequence[str] | None`, optional). Defaults to `None`.
        List of strings to parse. The default is taken from `sys.argv`, like the original :meth:`~argparse.ArgumentParser.parse_known_args` method.
...
def dataparser(
    *,
    ...
    supplied, namely :argument_link:`groups_descriptions<groups-descriptions>`, :argument_link:`required_mutually_exclusive_groups<required-mutually-exclusive-groups>`, :argument_link:`default_bool<default-bool>` and :argument_link:`help_formatter<help-formatter>`.

    Parameters
    ----------

    .. _groups-descriptions:
    ...
    - `default_bool` (`bool`, optional). Defaults to `False`.
        The default boolean value used in in boolean fields when there is no :argument_link:`default<default>` value passed.
    ...
    - `help_formatter` (`Callable[[str], str] | None`, optional). Defaults to `None`.
        ...
        :argument_link:`formatter_class<formatter-class>` parameter passed to the :class:`~argparse.ArgumentParser` constructor is assumed to be
        `RawDescriptionHelpFormatter`.

    Parameters from the original :class:`~argparse.ArgumentParser` class
    ---------------------------------------------------------------------
    ...
    - `prog` (`str | None`, optional). Defaults to `None`.
        ...
        By default, :class:`~argparse.ArgumentParser` objects use `sys.argv[0]` to determine how to display the name of the program
    ...
    - `usage` (`str | None`, optional). Defaults to `None`.
        ...
        By default, :class:`~argparse.ArgumentParser` calculates the usage message from the arguments it contains::
    ...
    - `description` (`str | None`, optional). Defaults to `None`.
        ...
        Most calls to the :class:`~argparse.ArgumentParser` constructor will use the `description=` keyword argument. This argument
        ...
        behavior, see the :argument_link:`formatter_class<formatter-class>` argument.
    ...
    - `epilog` (`str | None`, optional). Defaults to `None`.
        ...
        Such text can be specified using the `epilog=` argument to :class:`~argparse.ArgumentParser`::
        ...
        As with the :argument_link:`description<description>` argument, the `epilog=` text is by default line-wrapped, but this behavior can be
        adjusted with the :argument_link:`formatter_class<formatter-class>` argument to :class:`~argparse.ArgumentParser`.


    .. _parents:

    - `parents` (`Sequence[ArgumentParser]`, optional). Defaults to `[]`.
        A list of :class:`~argparse.ArgumentParser` objects whose arguments should also be included.
        ...
        :class:`~argparse.ArgumentParser` can be used. The `parents=` argument takes a list of :class:`~argparse.ArgumentParser` objects, collects all
        the positional and optional actions from them, and adds these actions to the :class:`~argparse.ArgumentParser` object being
        constructed::
        ...
        Note that most parent parsers will specify `add_help=False`. Otherwise, the :class:`~argparse.ArgumentParser` will see two
    ...
    - `formatter_class` (`_FormatterClass`, optional)
        ...
        :class:`~argparse.ArgumentParser` objects allow the help formatting to be customized by specifying an alternate formatting
        ...
        displayed. By default, :class:`~argparse.ArgumentParser` objects line-wrap the :argument_link:`description<description>` and :argument_link:`epilog<epilog>` texts in
        ...
        Passing `RawDescriptionHelpFormatter` as `formatter_class=` indicates that :argument_link:`description<description>` and :argument_link:`epilog<epilog>` are
        ...
        `MetavarTypeHelpFormatter` uses the name of the :argument_link:`type<type>` argument for each argument as the display name for
        its values (rather than using the :argument_link:`dest<dest>` as the regular formatter does)::
    ...
    - `fromfile_prefix_chars` (`str | None`, optional). Defaults to `None`.
        ...
        is given to the :class:`~argparse.ArgumentParser` constructor, then arguments that start with any of the specified characters
        ...
        :class:`~argparse.ArgumentParser` uses filesystem encoding and error handler to read the file containing arguments.

        The `fromfile_prefix_chars=` argument defaults to `None`, meaning that arguments will never be treated as
        file references.

        Changed in version 3.12: :class:`~argparse.ArgumentParser` changed encoding and errors to read arguments files from default
    ...
    - `conflict_handler` (`str`, optional). Defaults to `"error"`.
        ...
        :class:`~argparse.ArgumentParser` objects do not allow two actions with the same option string. By default, :class:`~argparse.ArgumentParser`
        ...
        Sometimes (e.g. when using :argument_link:`parents<parents>`) it may be useful to simply override any older arguments with the same
        option string. To get this behavior, the value `'resolve'` can be supplied to the `conflict_handler=`
        argument of :class:`~argparse.ArgumentParser`::
        ...
        Note that :class:`~argparse.ArgumentParser` objects only remove an action if all of its option strings are overridden. So, in
    ...
    - `add_help` (`bool`, optional). Defaults to `True`.
        ...
        `False` as the `add_help=` argument to :class:`~argparse.ArgumentParser`::
        ...
        character in :argument_link:`prefix_chars<prefix-chars>` is used to prefix the help options::
    ...
    - `allow_abbrev` (`bool`, optional). Defaults to `True`.
        Normally, when you pass an argument list to the :meth:`~argparse.ArgumentParser.parse_args` method of an :class:`~argparse.ArgumentParser`, it recognizes
        abbreviations of long options.

        This feature can be disabled by setting :argument_link:`allow_abbrev<allow-abbrev>` to `False`::
    ...
    - `exit_on_error` (`bool`, optional). Defaults to `True`.
        Normally, when you pass an invalid argument list to the :meth:`~argparse.ArgumentParser.parse_args` method of an :class:`~argparse.ArgumentParser`, it
        will exit with error info.

        If the user would like to catch errors manually, the feature can be enabled by setting :argument_link:`exit_on_error<exit-on-error>` to
        `False`::
    ...
    -------
    `Callable[[type[Class]], type[Class]]`:
        The decorator used to wrap around :func:`~dataclasses.dataclass` decorator passing parameters to the :class:`~argparse.ArgumentParser` constructor. When it
        is used with no parameters, just returns the class decorated with :func:`~dataclasses.dataclass`.
    """
    ...
...
def write_help(
    ...
    This is supplied as an option to use in the :argument_link:`help_formatter<help-formatter>` argument.
