...
class PeakDetectionModule:
    ...
    def plot_plateaus(self, ax, t, N, density_info, column_name, fit_result=None):
        ...
        if fit_result != None:
            ...
            ax.plot(t_fit, N_fit, label="Fitted Curve", color="orange",alpha=0.5)
            popt = fit_result['params']
            ax.vlines((popt[-1]+t_fit[0]), np.min(N_fit), np.max(N_fit), color='orange', linestyle='dashed', label='Cut Line',alpha=0.5,linewidth=1)
