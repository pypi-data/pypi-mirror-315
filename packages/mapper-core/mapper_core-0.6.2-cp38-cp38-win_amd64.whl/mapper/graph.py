...
from mapper.nerve import partition_vec_to_cover, weighted_partition_graph_edge_list_csr
...
class CSRGraph:
    ...
    def from_edge_list(
        ...
        edge_attrs = {key: arr[sorted_edges] for key, arr in edge_attrs.items()}
    ...
    def as_edge_list_graph(self) -> EdgeListGraph:
        edge_list = get_edge_list_masked_csr(
            self._neighbors,
            self._neighborhood_boundaries,
            zeros_read_only(self._neighbors.shape, dtype=np.int32),
            full_read_only(1, self._neighbors.shape, dtype=np.bool_),
            1,
        )
        edge_mtx = np.array(edge_list, dtype=np.float32)

        # TODO: attributes
        return EdgeListGraph(edge_mtx, self.n_nodes)
    ...
    def to_msgpack(self) -> bytes:
        ...
        # TODO: support for serializing complex node_attrs
        # (specifically, arrays of arrays)
        neighbors_bytes = np_to_bytes(self._neighbors)
        neighborhood_boundaries_bytes = np_to_bytes(self._neighborhood_boundaries)
        mask_bytes = np_to_bytes(self.mask)
        node_attrs_dict = {
            attr: np_to_bytes(arr) for attr, arr in self.node_attrs.items()
        }
        edge_attrs_dict = {
            attr: np_to_bytes(arr) for attr, arr in self.edge_attrs.items()
        }
        data = {
            "obj_type": "CSRGraph",
            "version": self._serialization_version,
            "neighbors": neighbors_bytes,
            "neighborhood_boundaries": neighborhood_boundaries_bytes,
    ...
    def from_msgpack(cls, data: bytes):
        ...
        if data_dict["obj_type"] != "CSRGraph":
            raise SerializationError(
                f"Incorrect serialized object type {data_dict['obj_type']}."
            )
        if data_dict["version"] != cls._serialization_version:
            raise SerializationError(
                f"Incorrect serialization version {data_dict['version']}. "
                f"Expected {cls._serialization_version}."
            )
        neighbors = np_from_bytes(data_dict["neighbors"])
        neighborhood_boundaries = np_from_bytes(data_dict["neighborhood_boundaries"])
        node_attrs = {
            attr: np_from_bytes(d) for attr, d in data_dict["node_attrs"].items()
        }
        edge_attrs = {
            attr: np_from_bytes(d) for attr, d in data_dict["edge_attrs"].items()
        }
        mask = np_from_bytes(data_dict["mask"])
