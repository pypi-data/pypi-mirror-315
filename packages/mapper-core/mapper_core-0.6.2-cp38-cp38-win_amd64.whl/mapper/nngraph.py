...
def to_symmetric_csr(
    ...
    csr_neighborhood_boundaries_arr = np.empty(N + 1, dtype=np.int32)
    csr_neighborhood_boundaries = cython.declare(
        int32_t[::1], csr_neighborhood_boundaries_arr
    )
    csr_neighborhood_boundaries[0] = 0
    acc: int32_t = 0
    for i in range(1, N + 1):
        acc += csr_n_neighbors[i - 1]
        csr_neighborhood_boundaries[i] = acc

    edge_count: int32_t = csr_neighborhood_boundaries[N]
    ...
    for i in range(N):
        for jdx in range(n_neighbors[i]):
            ...
            for idx in range(n_neighbors[j]):
                ...
                csr_j_start = csr_neighborhood_boundaries[j]
        ...
        csr_i_start += csr_n_neighbors[i]

    csr_jdx: cython.Py_ssize_t
    for i in range(N):
        for csr_jdx in range(
            csr_neighborhood_boundaries[i], csr_neighborhood_boundaries[i + 1]
        ):
    ...
    return (
        csr_neighbors_arr,
        csr_distances_arr,
        csr_neighborhood_boundaries_arr,
        csr_edge_ranks_arr,
    )
...
def rerank_nbhds(
    ...
    for i in range(N):
        for j in range(n_neighbors[i]):
            rerank_distances[i, j] = distances[i, index_rank[i, j]]
            if rerank_distances[i, j] == INFINITY:
                rerank_n_neighbors[i] = j
                break
            rerank_neighbors[i, j] = neighbors[i, index_rank[i, j]]
