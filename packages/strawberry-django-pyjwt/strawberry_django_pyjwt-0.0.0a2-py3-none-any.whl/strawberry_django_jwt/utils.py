...
async def create_user_token(user: User) -> object_types.TokenDataType:
    ...
    token_object = object_types.TokenDataType(
        payload=token, token=jwt_settings.JWT_ENCODE_HANDLER(token)
    )
    if jwt_settings.JWT_ALLOW_REFRESH:
        token_object.refresh_expires_in = token.exp - int(datetime.now().timestamp())
    if jwt_settings.JWT_LONG_RUNNING_REFRESH_TOKEN:
        refresh_token = (  # type: ignore
            (await sync_to_async(create_refresh_token)(user))
            if asyncio.get_event_loop().is_running()
            else create_refresh_token(user)
        )
        token_object.refresh_expires_in = (
            refresh_token.created.timestamp()
