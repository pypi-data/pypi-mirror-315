...
class KeepAliveRefreshMixin(JSONWebTokenMixin):
    ...
    def _refresh(self, info: Info, token: Optional[str]) -> TokenDataType:
        ...
        result = TokenDataType(
            payload=payload, token=token, refresh_expires_in=refresh_expires_in
        )
    ...
    async def refresh_async(
        self, info: Info, token: Optional[str] = None
    ) -> TokenDataType:
        return await sync_to_async(KeepAliveRefreshMixin._refresh)(
            self, info=info, token=token
        )
...
class RefreshTokenMixin(JSONWebTokenMixin):
    ...
    def _refresh(
        ...
        if hasattr(context, "jwt_cookie"):
            ...
            new_refresh_token = (
                refresh_token_lazy_async if _is_async else refresh_token_lazy
            )(
                old_refresh_token.user,
                old_refresh_token,
            )
    ...
    def refresh(
        self, info: Info, refresh_token: Optional[str] = None
    ) -> RefreshedTokenType:
    ...
    async def refresh_async(
        ...
        return await sync_to_async(RefreshTokenMixin._refresh)(
            self, info=info, refresh_token=refresh_token, _is_async=True
        )
...
class RefreshMixin(base_class, JSONWebTokenMixin):  # type: ignore
    ...
    refresh = strawberry.mutation(
        base_class.refresh,
        # extensions=mutation_extensions
    )


class RelayRefreshMixin(base_class, JSONWebTokenMixin):  # type: ignore
    """Relay version of RefreshMixin"""

    refresh = strawberry.mutation(
        base_class.refresh,
        extensions=[DynamicInputMutationExtension()],
    )
...
class AsyncRefreshMixin(base_class, JSONWebTokenMixin):  # type: ignore
    ...
    refresh = strawberry.mutation(
        base_class.refresh_async,
        # extensions=mutation_extensions
    )


class AsyncRelayRefreshMixin(base_class, JSONWebTokenMixin):  # type: ignore
    """Async version of RelayRefreshMixin"""
