...
class EloMatchmaker(BaseMatchmaker):
    def __init__(self, n_closest=3, k_factor=32, order_matters=False, meta_file="meta/elo.json"):
        super().__init__()
    ...
    def save_meta_file(self):
        """Save the current ELO data to the meta file."""
        try:
            with open(self.meta_file, "w") as file:
                json.dump(
                    {bot: data["elo"] for bot, data in self.bot_data.items()}, file, indent=4
                )
        except Exception as e:
            print(f"Error saving meta file {self.meta_file}: {e}")
