...
from cs410_arena.matchmaker.round_robin import RoundRobinMatchmaker
...
class GoArena:
    ...
    def run_match(self, black_bot_url: str, white_bot_url: str) -> dict:
        ...
        move_times = {black_bot_url: [], white_bot_url: []}
        ...
        while not state.is_terminal():
            ...
            if move == -1 or elapsed_time > remaining_time[current_bot_url]:
                ...
                return {
                    ...
                    'disqualified': current_bot_url
                }

            # Record move time
            move_times[current_bot_url].append(elapsed_time)
        ...
        avg_move_times = {bot: sum(times) / len(times) if times else 0.0 for bot, times in move_times.items()}
        return {
            'winner': 'black' if result[0] > 0 else 'white',
            'score': str(result),
            'moves': state.move_number(),
            'avg_move_times': avg_move_times
        }
    ...
    def run_tournament(self) -> List[dict]:
        ...
        bot_stats = {bot: {'wins': 0, 'losses': 0, 'disqualified': 0, 'total_score': 0.0, 'matches': 0, 'avg_move_time': []} for bot in bot_names}

        matchmaker = RoundRobinMatchmaker(order_matters=True)
        ...
        for black_bot, white_bot in matchups:
            ...
            if 'disqualified' in result:
                bot_stats[result['disqualified']]['disqualified'] += 1
            else:
                ...
                for bot in [black_bot, white_bot]:
                    bot_stats[bot]['avg_move_time'].append(result['avg_move_times'].get(self.bot_urls[bot], 0.0))
        ...
        for bot, stats in bot_stats.items():
            ...
            avg_move_time = sum(stats['avg_move_time']) / len(stats['avg_move_time']) if stats['avg_move_time'] else 0.0
            logging.info(f"Bot {bot}: Wins: {stats['wins']}, Losses: {stats['losses']}, Disqualified: {stats['disqualified']}, Average Score: {avg_score}, Average Move Time: {avg_move_time:.2f}s")
