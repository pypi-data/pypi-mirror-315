...
avg field - prompt for a numeric field total an average for code/barcode
...
{Fore.light_green}DOE:{Fore.light_magenta}{log.DayLogDate}{Style.reset}'''
	...
	def avg_field(self):
		h='DayLog@Avg Field'
		integer_fields=[i.name for i in DayLog.__table__.columns if str(i.type).lower() == "integer"]
		ct=len(integer_fields)
		for num,i in enumerate(integer_fields):
			msg=f'''{num}/{num+1}/{ct} - {i}'''
			print(msg)
		while True:
			try:
				...
				with Session(ENGINE) as session:
					...
					for i in split_totals:
						...
						if i in all_months:
							print(f"{Fore.light_magenta}{i}{Fore.light_red} {round(n.mean(),2)} | Avg BS Cst: ${round(b.mean(),2)} | Avg Cst: ${round(p.mean(),2)}{Style.reset}")
						elif i in all_days:
							print(f"{Fore.light_yellow}{i}{Fore.light_green} {round(n.mean(),2)} | Avg BS Cst: ${round(b.mean(),2)} | Avg Cst: ${round(p.mean(),2)}{Style.reset}")
				break
			except Exception as e:
				print(e)
				break
