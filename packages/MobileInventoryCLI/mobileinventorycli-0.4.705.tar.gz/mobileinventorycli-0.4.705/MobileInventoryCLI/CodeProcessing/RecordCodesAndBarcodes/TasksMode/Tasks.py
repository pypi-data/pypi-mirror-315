...
from MobileInventoryCLI.CodeProcessing.RecordCodesAndBarcodes.GeoTools import *
from MobileInventoryCLI.CodeProcessing.RecordCodesAndBarcodes.ExportUtility import *
                        ...
                        def mkT(text,code):
                            try:
                                if text not in ['',]:
                                    ...
                                    if text.lower() in ['r','+r','-r']:
                                        #value,text,suffix
                                        return float(processQtyRe(code,text)),text,''
                                    if text.lower() in [':-)','ipcv','invert pallet count value','invert_pallet_count_value']:
                                        if mkTextStore:
                                            ...
                                            return value,text,''
                                    if text.lower() in [':-)','iscv','invert shelf count value','invert_shelf_count_value']:
                                        if mkTextStore:
                                            ...
                                            return value,text,''
                                    if text.lower() in [';-)','ilcv','invert load count value','invert_load_count_value']:
                                        if mkTextStore:
                                            ...
                                            return value,text,''
                                    if text.lower() in [':-p','iccv','invert case count value','invert_case_count_value']:
                                        if mkTextStore:
                                            ...
                                            return value,text,''
                                    if text.lower() in [':-d','ipcv','invert pallet count value','invert_pallet_count_value']:
                                        if mkTextStore:
                                            print(f"{Fore.black}{Back.grey_84}Type What you see on the pallet... for the Location Below{Style.reset}")
                                            value=mkTextStore.PalletCount-float(processQtyRe(code,text))
                                            print(f"Inverted PalletCount is {Fore.light_steel_blue}{value}{Style.reset}")
                                            return value,text,''
                                    if text.lower() in [':-|','iscvc','invert shelf count value cases','invert_shelf_count_value_cases']:
                                        if mkTextStore:
                                            ...
                                            print(f"Inverted ShelfCount is {Fore.light_steel_blue}{value}{Style.reset}")
                                            return value,text,''
                                    if text.lower() in [':-*','ilcvc','invert load count value cases','invert_load_count_value_cases']:
                                        if mkTextStore:
                                            if mkTextStore.CaseCount in [None,] or mkTextStore.CaseCount < 1:
                                                print(f"{Fore.orange_red_1}There is an issue this Entry's {Fore.cyan}CaseCount{Fore.orange_red_1} -> {mkTextStore}/n{Fore.green_yellow}Setting it to 1!{Style.reset}")
                                                mkTextStore.CaseCount=1
                                            ...
                                            print(f"Inverted Load Count is {Fore.light_steel_blue}{value}{Style.reset}")
                                            return value,text,''
                                    if text.lower() in [':O','iccvc','invert case count value cases','invert_case_count_value_cases']:
                                        if mkTextStore:
                                            if mkTextStore.CaseCount in [None,] or mkTextStore.CaseCount < 1:
                                                print(f"{Fore.orange_red_1}There is an issue this Entry's {Fore.cyan}CaseCount{Fore.orange_red_1} -> {mkTextStore}/n{Fore.green_yellow}Setting it to 1!{Style.reset}")
                                                mkTextStore.CaseCount=1
...
""")
        ...
        with Session(ENGINE) as session:
            ...
            while True:
                ...
                if cmd in [None,'d']:
                    ...
                    run=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Previous Menu or Run?",helpText=f'''{actions}''')
                    pr=[None,]
                    ...
                    if run in pr:
                        ...
                        for num,c in enumerate(callables):
                            ...
                            try:
                                ...
                                nextcmd=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Next Cmd in Chain? h to see chain",helpText=f"{callables}",data="boolean")
                                n=[]
    ...
    def __init__(self,engine,parent,init_only=False,root_modes={}):
        ...
        self.options["ExportTables"]={
                    ...
                    'desc':f'export selected tables to selected XLSX (Excel)',
                    'exec':ExportTable,
                    }
        count+=1
