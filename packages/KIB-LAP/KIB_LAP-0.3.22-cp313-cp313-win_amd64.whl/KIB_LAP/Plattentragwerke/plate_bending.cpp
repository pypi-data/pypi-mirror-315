...
std::vector<std::vector<double>> assemble_stiffness_matrix(
    ...
    double E, double I_S, double L_S, double x_stiffener,
    double I_T, double L_T, double y_stiffener)
{
        ...
        for (int n = 1; n <= reihen; ++n)
        {
            for (int p = 1; p <= reihen; ++p)
            {
                for (int q = 1; q <= reihen; ++q)
                {
                    ...
                    double stiffener_long = integrate_longitudinal_stiffener_energy_cpp(m, n, p, q,
                                                                                        a, b, E, I_S,
                                                                                        L_S, x_stiffener);
                    double stiffener_trans = integrate_transverse_stiffener_energy_cpp(m, n, p, q,
                                                                                       a, b, E, I_T,
                                                                                       L_T, y_stiffener);
                    value += stiffener_long + stiffener_trans;
...
PYBIND11_MODULE(plate_bending_cpp, m)
{
    ...
    m.def("assemble_stiffness_matrix", &assemble_stiffness_matrix,
          ...
          py::arg("E"), py::arg("I_S"), py::arg("L_S"), py::arg("x_stiffener"),
          py::arg("I_T"), py::arg("L_T"), py::arg("y_stiffener"));
}
