...
if __name__ == "__main__":
    db = WkSqlite3(DB)
    # db.set_table(TABLE).create_table({"id": "INTEGER PRIMARY KEY AUTOINCREMENT", "name": "TEXT", "age": "INTEGER"}, delete_if_exists=False)
    # db.set_table(TABLE2).create_table({"id": "INTEGER PRIMARY KEY AUTOINCREMENT", "name": "TEXT", "age": "INTEGER", "sex": "INTEGER"}, delete_if_exists=True)
    ...
    # print(db.select_all())
    # res = db.set_table(TABLE).get_column_names()
    # print(res)
    # exit()
    # db.set_table("users")
    # for i in range(10):
    ...
    """ db.execute_many("INSERT INTO test_table(`key`, sno) VALUES(?, ?)", [[1, "test"], [2, "test2"]])
    db.execute_many("UPDATE test_table SET `key`=? WHERE sno=?", [[10, "test"], [30, "test2"]]) """
    # db.set_table(TABLE)
