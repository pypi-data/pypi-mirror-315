...
# @Filepath : WkSqlite.py
# @Brief    : WkSqlite3封装类
# Copyright 2024 WANGKANG, All Rights Reserved.

""" 
项目地址：https://gitee.com/purify_wang/wk-sqlite3.git
"""
...
class WkSqlite3:
    def __init__(
        self,
        database,
        time_interval=60,  # 默认一分钟进行一次连接检查
        check_same_thread=False,  # 默认关闭单线程检查
        **kwargs,
    ):
        ...
        self.conn: sqlite3.Connection = self.connect_db()
    ...
    def __get_query_params(self, obj: dict | list):
        if isinstance(obj, dict):
            return " AND ".join([f"`{column_name}` {'=' if obj[column_name] is not None else 'is'} ?" for column_name in obj.keys()])
        elif isinstance(obj, list):
            return " AND ".join([f"`{column_name}` {'=' if column_name is not None else 'is'} ?" for column_name in obj])

    def __get_set_params(self, obj: dict):
        return ", ".join([f"`{column_name}` {'=' if obj[column_name] is not None else 'is'} ?" for column_name in obj.keys()])
    ...
    def execute_many(self, sql, values_list):
        ...
        try:
            ...
            self.conn.rollback()
            self.__print_info(cursor, sys._getframe().f_code.co_name, sql=sql, values=values_list, success=False, error_msg=str(e))
            return -1
