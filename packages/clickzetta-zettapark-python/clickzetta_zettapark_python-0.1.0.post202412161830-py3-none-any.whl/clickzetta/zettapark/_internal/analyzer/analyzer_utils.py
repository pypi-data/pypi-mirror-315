...
def function_expression(
    name: str, children: List[str], is_distinct: bool, parse_local_name: bool = False
) -> str:
    return (
        (quote_if_needed(name) if parse_local_name else name)
        + LEFT_PARENTHESIS
...
def schema_cast_seq(schema: List[Attribute]) -> List[str]:
    ...
    for index, attr in enumerate(schema):
        name = (
            ...
            + str(index + 1)
            + DOUBLE_COLON
            + convert_sp_to_cz_type(attr.datatype)
        )
...
def file_operation_statement(
    ...
    if is_volume_uri(stage_location):
        return volume_file_operation_statement(
...
def select_from_path_with_format_statement(
    ...
    vol = _parse_volume_uri(path)
    if not vol:
        raise ValueError(f"Invalid volume location: {path}")
    schema_string = attribute_to_schema_string(schema)
    return (
        f"SELECT {(STAR if not project else COMMA.join(project))} "
        f"FROM {vol.volume_identifier} "
        f"({schema_string}) "
        f"USING {format_name} "
        f"{to_table_options_clause(file_format_options)} "
        f"{vol.path_filter_clause(pattern)}"
    )
...
def cast_expression(child: str, datatype: DataType, try_: bool = False) -> str:
    return (
        ...
        + child
        + AS
        + convert_sp_to_cz_type(datatype)
        + RIGHT_PARENTHESIS
    )
...
def copy_into_table(
    ...
    *,
    ...
    volume_uri = _parse_volume_uri(file_path)
    if not volume_uri:
        raise ValueError(f"Invalid volume location: {file_path}")

    # TODO(guantao.gao) update after we support schema infer
    if not user_schema:
        raise ValueError("user_schema must be specified")

    volume_filter = ""
    if not volume_uri.path.endswith("/"):
        if files is None:
            files = [volume_uri.path]
        else:
            files.append(volume_uri.path)
        volume_filter = "FILES ( " + ", ".join(f"'{f}'" for f in files) + " )"
    else:
        if not files:
            volume_filter = f"SUBDIRECTORY '{volume_uri.path}'"
            if pattern:
                volume_filter += f" REGEXP '{pattern}'"
        elif volume_uri.path == "/":
            volume_filter = "FILES ( " + ", ".join(f"'{f}'" for f in files) + " )"
        else:
            raise ValueError("Cannot specify files when using a directory")

    from_source = (
        f"{volume_uri.volume_identifier} "
        f"{('( ' + attribute_to_schema_string(user_schema._to_attributes()) + ' )') if user_schema else ''} "
        f"USING {file_format_type} {to_table_options_clause(format_type_options)} "
        f"{volume_filter}"
    )
    if transformations:
        from_source = f"( SELECT {COMMA.join(transformations)} FROM {from_source} )"
    column_names_str = ("( " + COMMA.join(column_names) + " )") if column_names else ""
    return (
        f"COPY INTO {table_name} {column_names_str} "
        f"FROM {from_source} "
        f"FILE_FORMAT = ( {to_options_clause(format_type_options)} ) "
