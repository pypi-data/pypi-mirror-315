...
from clickzetta.zettapark._adaptor import ClickzettaConnection as Connection
...
from clickzetta.zettapark._internal.analyzer.snowflake_plan import SnowflakePlanBuilder
...
class Session:
    ...
    The Zettapark library uses `the Snowflake Connector for Python <https://docs.snowflake.com/en/user-guide/python-connector.html>`_
...
"""
    ...
    def add_packages(
        ...
        To use Python packages that are not available in Snowflake, refer to :meth:`~clickzetta.zettapark.Session.custom_package_usage_config`.
    ...
    def replicate_local_environment(
        ...
        You can also find examples in :class:`~clickzetta.zettapark.udf.UDFRegistration`. See details of `third-party Python packages in Snowflake <https://docs.snowflake.com/en/developer-guide/udf/python/udf-python-packages.html>`_.
        ...
        that are not available in Snowflake, refer to :meth:`~clickzetta.zettapark.Session.custom_package_usage_config`.
        ...
        Example::
            ...
            >>> session.replicate_local_environment(ignore_packages={"snowflake-zettapark-python", "snowflake-connector-python", "urllib3", "tzdata", "numpy"}, relax=True)
            >>> @udf
            ... def get_package_name_udf() -> list:
            ...     return [numpy.__name__, pandas.__name__]
            >>> session.sql(f"select {get_package_name_udf.name}()").to_df("col1").show()
                    ...
                    if is_in_stored_procedure():  # pragma: no cover
                        errors.append(
                            RuntimeError(
                                ...
                                f"https://docs.snowflake.com/en/developer-guide/zettapark/python/creating-udfs.html#using-third-party-packages-from-anaconda-in-a-udf."
                            )
                        )
                        continue
                    if (
    ...
    def _upload_unsupported_packages(
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"https://docs.snowflake.com/en/developer-guide/zettapark/python/creating-udfs.html#using"
    ...
    def query_tag(self, tag: str) -> None:
        # not supported in clickzetta lakehouse
        # if tag:
    ...
    def _get_remote_query_tag(self) -> None:
        ...
        # not supported in clickzetta lakehouse; return local value as it is
        return self._query_tag
    ...
    def write_pandas(
        ...
        **kwargs: Dict[str, Any],
        ...
        try:
            ...
            if "use_logical_type" not in signature.parameters:
    ...
    def create_dataframe(
        ...
        if installed_pandas and isinstance(data, pandas.DataFrame):
            ...
            try:
                ...
                sf_database = self._conn._lower_case_parameters("workspace")

            try:
                sf_schema = self._conn._get_current_parameter("schema", quoted=False)
            except AttributeError:
                sf_database = self._conn._lower_case_parameters("schema")
        ...
        for field, name in zip(new_schema.fields, names):
            if isinstance(field.datatype, DecimalType):
                ...
                # # to_timestamp_ltz and to_timestamp_tz don not support
            ...
            # do not support VariantType & GeographyType & GeometryType
    ...
    def telemetry_enabled(self) -> bool:
        """Controls whether or not the Zettapark client sends usage telemetry to Snowflake.
            ...
            for arg in args:
                if isinstance(arg, Column):
                    ...
                    if isinstance(expr, Cast):
                        arg_types.append(convert_sp_to_cz_type(expr.to))
                    else:
                        arg_types.append(convert_sp_to_cz_type(expr.datatype))
                else:
                    arg_types.append(convert_sp_to_cz_type(infer_type(arg)))
    ...
    def call(
        ...
        *args: Any,
        ...
        Example::
            ...
            >>>
            >>> session.add_packages('snowflake-zettapark-python')
            >>>
