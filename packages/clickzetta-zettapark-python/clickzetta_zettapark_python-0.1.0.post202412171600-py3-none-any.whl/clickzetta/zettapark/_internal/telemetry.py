...
from clickzetta.zettapark._connector import ClickzettaConnection
...
class TelemetryField(Enum):
    ...
    # Types of telemetry
    TYPE_PERFORMANCE_DATA = "zettapark_performance_data"
    TYPE_FUNCTION_USAGE = "zettapark_function_usage"
    TYPE_SESSION_CREATED = "zettapark_session_created"
    TYPE_SQL_SIMPLIFIER_ENABLED = "zettapark_sql_simplifier_enabled"
    TYPE_ERROR = "zettapark_error"
    ...
    KEY_CREATED_BY_ZETTAPARK = "created_by_zettapark"
...
def df_collect_api_telemetry(func):
    @functools.wraps(func)
    def wrap(*args, **kwargs):
        with args[0]._session.query_history():
...
def dfw_collect_api_telemetry(func):
    @functools.wraps(func)
    def wrap(*args, **kwargs):
        with args[0]._dataframe._session.query_history():
            result = func(*args, **kwargs)
...
class TelemetryClient:
    def __init__(self, conn: ClickzettaConnection) -> None:
    ...
    def _create_basic_telemetry_data(self, telemetry_type: str) -> Dict[str, Any]:
        message = {
            TelemetryField.KEY_SOURCE.value: self.source,
            ...
            TelemetryField.KEY_TYPE.value: telemetry_type,
        }
        return message

    @safe_telemetry
    def send_session_created_telemetry(self, created_by_zettapark: bool):
        message = {
            **self._create_basic_telemetry_data(
            ...
            TelemetryField.KEY_DATA.value: {
                ...
                TelemetryField.KEY_CREATED_BY_ZETTAPARK.value: TelemetryData.TRUE
                if created_by_zettapark
    ...
    def send_upload_file_perf_telemetry(
        ...
        message = {
            **self._create_basic_telemetry_data(
            ...
            TelemetryField.KEY_DATA.value: {
                TelemetryField.KEY_SFQIDS.value: [sfqid],
