...
from clickzetta.connector.v0.utils import strip_leading_comment
from clickzetta.zettapark._internal.analyzer.table_function import (
    TableFunctionExpression,
    TableFunctionJoin,
    TableFunctionRelation,
)
from clickzetta.zettapark._internal.analyzer.window_expression import WindowExpression
...
if TYPE_CHECKING:
    from clickzetta.zettapark._internal.analyzer.analyzer import (
        Analyzer,
    )  # pragma: no cover
...
from clickzetta.zettapark._internal.analyzer.analyzer_utils import (
    placeholder_values_statement,
    result_scan_statement,
    schema_value_statement,
    wrap_select_if_sql_show_statement,
)
from clickzetta.zettapark._internal.analyzer.binary_expression import And
...
from clickzetta.zettapark._internal.analyzer.query_plan import Query, QueryPlan
...
class Selectable(LogicalPlan, ABC):
    """The parent abstract class of a DataFrame's logical plan. It can be converted to and from a QueryPlan."""
    ...
    def api_calls(self, value: Optional[List[Dict[str, Any]]]) -> None:
        ...
        if self._query_plan:
            ...
            self._query_plan.api_calls = value

    @property
    def query_plan(self):
        """Convert to a QueryPlan"""
        if self._query_plan is None:
            ...
            self._query_plan.api_calls = self.api_calls
...
class SelectableEntity(Selectable):
    """Query from a table, view, or any other ClickZetta objects.
...
class SelectSQL(Selectable):
    ...
    def __init__(
        ...
        *,
        ...
        sql = strip_leading_comment(sql)
        is_select = is_sql_select_statement(sql)
        is_show = is_sql_show_statement(sql)
        if not is_select and not is_show and convert_to_select:
...
class SelectValuesSQL(SelectSQL):
    """Query from a SQL. Like select * from values (?, ?)"""
    ...
    def __init__(
        ...
        *,
        ...
        for match in matches:
            ...
            count = match.count("?")
            data.append(params[start_step : start_step + count])
            start_step = start_step + count
        values = placeholder_values_statement(columns, data)
        for match, value in zip(matches, values):
            sql = sql.replace(match, value, 1)
        super().__init__(sql, convert_to_select=convert_to_select, analyzer=analyzer)


class SelectQueryPlan(Selectable):
    """Wrap a QueryPlan to a subclass of Selectable."""

    def __init__(self, query_plan: LogicalPlan, *, analyzer: "Analyzer") -> None:
        super().__init__(analyzer)
        self._query_plan: QueryPlan = (
            query_plan
            if isinstance(query_plan, QueryPlan)
            else analyzer.resolve(query_plan)
        )
        self.expr_to_alias.update(self._query_plan.expr_to_alias)
        self.df_aliased_col_name_to_real_col_name.update(
            self._query_plan.df_aliased_col_name_to_real_col_name
        )

        self.pre_actions = self._query_plan.queries[:-1]
        self.post_actions = self._query_plan.post_actions
        self._api_calls = self._query_plan.api_calls
        self._query_params = []
        for query in self._query_plan.queries:
            if query.params:
    ...
    @property
    def sql_query(self) -> str:
        return self._query_plan.queries[-1].sql

    @property
    def schema_query(self) -> str:
        return self.query_plan.schema_query
...
class SelectStatement(Selectable):
    ...
    def sql_query(self) -> str:
        ...
        where_clause = (
            ...
            else clickzetta.zettapark._internal.utils.EMPTY_STRING
        )
        order_by_clause = (
    ...
    def select(self, cols: List[Expression]) -> "SelectStatement":
        ...
        if (
            ...
            new._query_plan = (
                ...
                # They shouldn't share the same query_plan.
                # Setting it to None so the new._query_plan will be created later.
            )
        ...
        if new_column_states is None:
            ...
            and self.query_plan.session
            and not self.query_plan.session.conf.get(
        ...
        *selectables: Union[
        ...
        if (
            ...
            if operator == last_operator:
                ...
                # In ClickZetta SQL, intersect has higher precedence than other set operators.
...
class SelectTableFunction(Selectable):
    ...
    def __init__(
        ...
        *,
        ...
        self._query_plan: QueryPlan
        if other_plan:
            self._query_plan = analyzer.resolve(
                TableFunctionJoin(other_plan, func_expr, left_cols, right_cols)
            )
        else:
            self._query_plan = analyzer.resolve(TableFunctionRelation(func_expr))
        self.pre_actions = self._query_plan.queries[:-1]
        self.post_actions = self._query_plan.post_actions
        self._api_calls = self._query_plan.api_calls
    ...
    @property
    def schema_query(self) -> str:
        return self._query_plan.schema_query

    @property
    def query_params(self) -> Optional[Sequence[Any]]:
        return self.query_plan.queries[-1].params
...
class SetStatement(Selectable):
    ...
    @property
    def sql_query(self) -> str:
        sql = f"({wrap_select_if_sql_show_statement(self.set_operands[0].selectable.sql_query)})"
        for i in range(1, len(self.set_operands)):
            sql = (
                f"{sql}{self.set_operands[i].operator}"
                f"({wrap_select_if_sql_show_statement(self.set_operands[i].selectable.sql_query)})"
            )
        return sql
    ...
    def schema_query(self) -> str:
        ...
        Refer to https://doc.clickzetta.com/
        """
        attributes = self.set_operands[0].selectable.query_plan.attributes
        sql = f"({schema_value_statement(attributes)})"
        for i in range(1, len(self.set_operands)):
            attributes = self.set_operands[i].selectable.query_plan.attributes
...
def parse_column_name(
    ...
    if isinstance(column, Expression):
        if isinstance(column, Attribute):
            ...
            # ClickZetta SQL removes the spaces in the returned column names.
        ...
        for dc in dependent_columns:
            ...
            if dc_state:
                if dc_state.change_state == ColumnChangeState.CHANGED_EXP:
                    ...
                    # is used in a clause, the sql doesn't work in ClickZetta.
...
def derive_column_states_from_subquery(
    ...
    for c in cols:
        if isinstance(c, UnresolvedAlias) and isinstance(c.child, Star):
            if c.child.expressions:
                ...
                if c.child.df_alias not in from_.df_aliased_col_name_to_real_col_name:
                    raise ZettaparkClientExceptionMessages.DF_ALIAS_NOT_RECOGNIZED(
                        c.child.df_alias
                    )
        ...
        if c_name is None:
            return None
        quoted_c_name = clickzetta.zettapark._internal.utils.quote_name(c_name)
