...
from clickzetta.zettapark._internal.analyzer.binary_plan_node import Join, SetOperation
...
from clickzetta.zettapark._internal.analyzer.grouping_set import (
    GroupingSet,
...
from clickzetta.zettapark._internal.analyzer.table_merge_expression import (
...
from clickzetta.zettapark.types import _NumericType
...
class Analyzer:
    def __init__(self, session: "clickzetta.zettapark.session.Session") -> None:
        ...
        self.plan_builder = QueryPlanBuilder(self.session)
    ...
    def analyze(
        ...
        if isinstance(expr, Collate):
            collation_spec = (
                expr.collation_spec.lower() if parse_local_name else expr.collation_spec
            )
        ...
        if isinstance(expr, Literal):
            if expr.identity is not None and expr.identity:
                return expr.value
        ...
        if isinstance(expr, FunctionExpression):
            ...
            func_name = expr.name.lower() if parse_local_name else expr.name
            children = [
                self.to_sql_avoid_offset(c, df_aliased_col_name_to_real_col_name)
                for c in expr.children
            ]
            if func_name == "datediff":
                first_arg = expr.children[0]
                assert isinstance(first_arg, Literal)
                children[0] = str(first_arg.value)
            return function_expression(
                func_name, children, expr.is_distinct, parse_local_name
            )
        ...
        if isinstance(expr, UDF):
            ...
            func_name = expr.udf_name.lower() if parse_local_name else expr.udf_name
            return function_expression(
                func_name,
                [
                    self.analyze(
        ...
        if isinstance(expr, RankRelatedFunctionExpression):
            return rank_related_function_expression(
                (quote_if_needed(expr.sql) if parse_local_name else expr.sql),
                self.analyze(
                    expr.expr, df_aliased_col_name_to_real_col_name, parse_local_name
                ),
                expr.offset,
        ...
        raise ZettaparkClientExceptionMessages.PLAN_INVALID_TYPE(
            str(expr)
        )  # pragma: no cover
    ...
    def unary_expression_extractor(
        ...
        elif isinstance(expr, MathUnaryExpression):
            return math_unary_expression(
                self.analyze(
                    expr.child, df_aliased_col_name_to_real_col_name, parse_local_name
                ),
                expr.sql_operator,
    ...
    def resolve(self, logical_plan: LogicalPlan) -> QueryPlan:
        self.subquery_plans = []
        self.generated_alias_maps = {}
    ...
    def do_resolve(self, logical_plan: LogicalPlan) -> QueryPlan:
        resolved_children = {}
    ...
    def do_resolve_with_resolved_children(
        ...
        resolved_children: Dict[LogicalPlan, QueryPlan],
        df_aliased_col_name_to_real_col_name: DefaultDict[str, Dict[str, str]],
    ) -> QueryPlan:
        if isinstance(logical_plan, QueryPlan):
            return logical_plan
        ...
        if isinstance(logical_plan, Range):
            # schema of Range. Since this corresponds to the column "id"
        ...
        if isinstance(logical_plan, Values):
        ...
        if isinstance(logical_plan, CreateTable):
        ...
        if isinstance(logical_plan, Pivot):
            if (
                ...
                # Currently pivot creates a group by from all columns outside of
        ...
        if isinstance(logical_plan, CreateViewCommand):
            if isinstance(logical_plan.view_type, PersistedView):
                ...
                raise ZettaparkClientExceptionMessages.PLAN_ANALYZER_UNSUPPORTED_VIEW_TYPE(
        ...
        if isinstance(logical_plan, CopyIntoTableNode):
            ...
            if format_name is not None:
                format_type_options["TYPE"] = format_name
        ...
        raise TypeError(
            f"Cannot resolve type logical_plan of {type(logical_plan).__name__} to a QueryPlan"
        )
    ...
    def create_select_query_plan(self, *args, **kwargs):
        return SelectQueryPlan(*args, **kwargs)
