...
# "%?" is for table stage
STAGE_NAME_PATTERN = f"(%?{ID_PATTERN})"
...
# Anonymous stored procedures: https://doc.clickzetta.com/
ANONYMOUS_CALL_WITH_PATTERN = re.compile(
...
def is_sql_select_statement(sql: str) -> bool:
    return (
        SELECT_SQL_PREFIX_PATTERN.match(sql) is not None
        and ANONYMOUS_CALL_WITH_PATTERN.match(sql) is None
    )
...
def get_stage_file_prefix_length(stage_location: str) -> int:
    ...
    for i, c in enumerate(normalized):
        if c == '"':
            ...
            res = re.findall(STAGE_NAME_PATTERN, full_stage_name)
            if not res:
                break
            stage_name = res[-1][0]
