...
from clickzetta.zettapark._connector import pandas as pd
from clickzetta.zettapark._internal.analyzer.analyzer_utils import (
    quote_name_without_upper_casing,
)
from clickzetta.zettapark._internal.type_utils import infer_type
...
if TYPE_CHECKING:
    from clickzetta.zettapark import DataFrame, Session
...
def _extract_schema_and_data_from_pandas_df(
    ...
    when running against clickzetta, infer_schema (https://doc.clickzetta.com/
    ...
    # as pandas is optional to zettapark-python
    ...
    for row_idx in range(data.shape[0]):
        for col_idx in range(data.shape[1]):
            ...
            if isinstance(plain_data[row_idx][col_idx], (float, numpy.float_)):
                ...
                # which can not be inferred by zettapark python, we cast to built-in float type
                if math.isnan(plain_data[row_idx][col_idx]):
                    # math.nan in a pandas DataFrame is treated as None
                ...
                if isinstance(data.dtypes[col_idx], pd.DatetimeTZDtype):
                    # this is to align with the current behavior that it
                    ...
                    # zettapark will convert it to microseconds
                ...
                # keeps the unit of nanoarrow seconds
                ...
                # returns the ordinal of a period object
                ...
                if isinstance(data_type, (MapType, ArrayType)):
                    # converts python dict/array to variant
                ...
                if isinstance(data_type, DecimalType):
                    ...
                    if precision > 38:
                        raise ZettaparkClientException(
    ...
    for idx, pandas_type in enumerate(data.dtypes):
        if isinstance(pandas_type, pd.IntervalDtype):
            ...
            isinstance(pandas_type, pd.Float64Dtype)
            or pandas_type.type == numpy.float64
        ):
            data_type = DoubleType()
        elif (
            isinstance(pandas_type, pd.Float32Dtype)
            or pandas_type.type == numpy.float32
        ):
            data_type = FloatType()
        elif (
            ...
            data_type = LongType()
        elif isinstance(pandas_type, pd.Int32Dtype) or pandas_type.type == numpy.int32:
            ...
            isinstance(pandas_type, pd.StringDtype)
            or pandas_type.type == numpy.object_
            or pandas_type.type == numpy.string_
        ):
            data_type = inferred_type_dict.get(idx, StringType())
        else:
            data_type = inferred_type_dict.get(idx, StringType(length=65535))
        # zettapark write_pandas will ignore the nullability of pd dataframe and set nullable to True
