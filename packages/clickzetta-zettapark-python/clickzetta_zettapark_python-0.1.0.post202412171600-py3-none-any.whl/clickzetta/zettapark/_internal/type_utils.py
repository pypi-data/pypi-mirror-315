...
def convert_metadata_to_sp_type(metadata: ResultMetadata) -> DataType:
    # column_type_name =
    column_type_name = metadata.type_code
    if column_type_name == "VECTOR":
        raise NotImplementedError("Vectors are not supported yet")
    else:
        return convert_sf_to_sp_type(
            column_type_name,
            0 if metadata.precision is None else int(metadata.precision),
            0 if metadata.scale is None else int(metadata.scale),
            0 if metadata.internal_size is None else int(metadata.internal_size),
        )
...
def convert_sf_to_sp_type(
    ...
    """Convert the logical type to the Zettapark type."""
    if column_type_name == "ARRAY":
        return ArrayType(StringType())
    if column_type_name == "VARIANT":
        return VariantType()
    if column_type_name == "OBJECT" or column_type_name == "MAP":
    ...
    if column_type_name == "BOOLEAN":
        return BooleanType()
    if column_type_name == "INT8":
        return ByteType()
    if column_type_name == "INT16":
        return ShortType()
    if column_type_name == "BINARY":
        return BinaryType()
    if column_type_name in ["TEXT", "VARCHAR", "CHAR"]:
        if internal_size > 0:
    ...
    if (column_type_name == "FIXED" or column_type_name == "NUMBER") and scale == 0:
        return LongType()
    if column_type_name == "INT32":
        return IntegerType()
    if column_type_name == "INT64":
        return LongType()
    if column_type_name == "STRING":
        return StringType()
    if column_type_name == "FLOAT32":
        return FloatType()
    if column_type_name == "FLOAT64":
        return DoubleType()
    if column_type_name == "VOID":
        return NullType()
...
def convert_sp_to_cz_type(datatype: DataType) -> str:
    ...
    if isinstance(datatype, ArrayType):
        element_type = convert_sp_to_cz_type(datatype.element_type)
        return f"ARRAY<{element_type}>"
    if isinstance(datatype, MapType):
        key_type = convert_sp_to_cz_type(datatype.key_type)
        value_type = convert_sp_to_cz_type(datatype.value_type)
...
# Add additional mappings to match clickzetta db data types
...
ARRAY_RE = re.compile(r"^\s*(array)\s*<\s*(.*)\s*>\s*$")
...
def type_string_to_type_object(type_str: str) -> DataType:
    ...
    array_match = ARRAY_RE.match(type_str)
    if array_match:
        return ArrayType(type_string_to_type_object(array_match.group(2)))
