...
ZETTAPARK_PACKAGE_NAME: str = "clickzetta-zettapark-python"
...
def identify_supported_packages(
    ...
    """
    Detects which `packages` are present in the Zettapark Anaconda channel using the `valid_packages` mapping.
    ...
    for package in packages:
        ...
        if package_name in valid_packages:
            ...
            if (
                ...
                _logger.info(
                    f"Package {package_name}{version_text} is available! The package will not be uploaded."
                )
            ...
            # Native packages should be anaconda dependencies, even if the requested version is not available.
                ...
                if package_name not in package_dict:
                    _logger.warning(
                        f"Package {package_name}{version_text} contains native code, switching to latest available version instead"
                    )
...
def add_zettapark_package(
    package_dict: Dict[str, str], valid_packages: Dict[str, List[str]]
) -> None:
    """
    Adds the Zettapark Python package to package dictionary, if not present. We either choose the version available in
    ...
    Raises:
        pkg_resources.DistributionNotFound: If the Zettapark Python Package is not installed in the local environment.
    """
    if ZETTAPARK_PACKAGE_NAME not in package_dict:
        package_dict[ZETTAPARK_PACKAGE_NAME] = ZETTAPARK_PACKAGE_NAME
        try:
            package_client_version = pkg_resources.get_distribution(
                ZETTAPARK_PACKAGE_NAME
            ).version
            if package_client_version in valid_packages[ZETTAPARK_PACKAGE_NAME]:
                package_dict[
                    ZETTAPARK_PACKAGE_NAME
                ] = f"{ZETTAPARK_PACKAGE_NAME}=={package_client_version}"
            else:
                _logger.warning(
                    f"The version of package '{ZETTAPARK_PACKAGE_NAME}=={package_client_version}' in the local environment is "
                    f"{package_client_version}, which is not available. Your UDF might not work when "
                    f"the package version is different between the server and your local environment."
                )
        except pkg_resources.DistributionNotFound:
            _logger.warning(
                f"Package '{ZETTAPARK_PACKAGE_NAME}' is not installed in the local environment. "
