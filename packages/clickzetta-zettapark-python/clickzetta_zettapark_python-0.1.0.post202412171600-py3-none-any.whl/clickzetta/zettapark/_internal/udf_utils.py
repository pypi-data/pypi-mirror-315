...
from clickzetta.zettapark._internal import code_generation, type_utils
from clickzetta.zettapark._internal.analyzer.datatype_mapper import to_sql
...
from clickzetta.zettapark.types import DataType, StructField, StructType
...
def get_types_from_type_hints(
    ...
    for key, python_type in python_types_dict.items():
        ...
        if object_type == TempObjectType.PROCEDURE and index == 0:
            if python_type != clickzetta.zettapark.Session and python_type not in [
                "Session",
                "clickzetta.zettapark.Session",
            ]:
                raise TypeError(
...
def extract_return_input_types(
    ...
    if (
        ...
        if num_args != len(input_types_from_type_hints):
            raise TypeError(
                f'{"" if object_type == TempObjectType.FUNCTION else "Excluding session argument in stored procedure, "}'
    ...
    if isinstance(res_return_type, PandasSeriesType):
        if len(res_input_types) == 0:
            ...
            return (
                ...
                res_input_types[0].get_database_col_datatypes(),
            )
...
def process_registration_inputs(
    session: "clickzetta.zettapark.Session",
    object_type: TempObjectType,
    func: Union[Callable, Tuple[str, str]],
    return_type: Optional[DataType],
    input_types: Optional[List[DataType]],
...
def cleanup_failed_permanent_registration(
    ...
    if stage_location and upload_file_stage_location:
        try:
            logger.debug(
                "Removing Zettapark uploaded file: %s",
                upload_file_stage_location,
            )
            session._run_query(f"REMOVE {upload_file_stage_location}")
            logger.info(
                "Finished removing Zettapark uploaded file: %s",
                upload_file_stage_location,
            )
...
def generate_python_code(
    ...
    # which are from zettapark modules and will not work on the server side
...
def create_python_udf_or_sp(
    ...
    if isinstance(return_type, StructType):
        return_sql = f'RETURNS TABLE ({",".join(f"{field.name} {convert_sp_to_cz_type(field.datatype)}" for field in return_type.fields)})'
    elif installed_pandas and isinstance(return_type, PandasDataFrameType):
        return_sql = f'RETURNS TABLE ({",".join(f"{name} {convert_sp_to_cz_type(datatype)}" for name, datatype in zip(return_type.col_names, return_type.col_types))})'
    else:
        return_sql = f"RETURNS {convert_sp_to_cz_type(return_type)}"
    input_sql_types = [convert_sp_to_cz_type(arg.datatype) for arg in input_args]
    sql_func_args = ",".join(
...
def generate_call_python_sp_sql(
    session: "clickzetta.zettapark.Session", sproc_name: str, *args: Any
) -> str:
    sql_args = []
    for arg in args:
        if isinstance(arg, clickzetta.zettapark.Column):
