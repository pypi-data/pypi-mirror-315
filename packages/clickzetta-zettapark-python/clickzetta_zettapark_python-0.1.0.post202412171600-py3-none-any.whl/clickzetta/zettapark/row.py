...
def _canonicalize_field(value: str) -> str:
    from clickzetta.zettapark._internal.utils import unquote_name

    return unquote_name(value.strip())
...
class Row(tuple):
    ...
    class _RowBuilder:
        ...
        def set_case_sensitive(self, case_sensitive: bool) -> "Row._RowBuilder":
            if case_sensitive:
                raise ValueError("case_sensitive=True is not supported yet")
            self._case_sensitive = case_sensitive
            return self

        def _canonicalize_field(self, value: str) -> str:
            from clickzetta.zettapark._internal.utils import unquote_name

            return unquote_name(value)
        ...
        def to_row(self) -> "Row":
            if self._named_values and not self._case_sensitive:
                ...
                #     )
                self._named_values = {
                    _canonicalize_field(k): v for k, v in self._named_values.items()
                }
            ...
            if self._values and not self._case_sensitive:
                ...
                #         "Case insensitive fields is not supported in presence of quoted columns"
                #     )
                self._values = [_canonicalize_field(value) for value in self._values]
    ...
    def __new__(cls, *values: Any, **named_values: Any):
        ...
        # DB can return duplicate column names, for instance, "select a, a from a_table."
        ...
        row.__dict__["_case_sensitive"] = False
        return row
    ...
    def __getattr__(self, item):
        ...
        if self._named_values and item in self._named_values:
            return self._named_values[item]
        if (
            self._named_values and f"`{item}`" in self._named_values
        ):  # FIXME remove this workaround
            return self._named_values[f"`{item}`"]
        elif self._fields and self._check_if_having_duplicates():
            try:
                index = self._fields.index(item)  # may throw ValueError
    ...
    def __setattr__(self, key, value):
        ...
        if value is not None:
            if not self._case_sensitive:
                value = tuple(_canonicalize_field(val) for val in value)
        ...
        if self._named_values:
            ...
            for input_key, input_value in kwargs.items():
                if isinstance(input_key, str):
                    input_key = _canonicalize_field(input_key)
    ...
    def _populate_named_values_from_fields(self) -> None:
        ...
        if (
            ...
            self.__dict__["_named_values"] = {
                _canonicalize_field(k): v for k, v in zip(self._fields, self)
            }
