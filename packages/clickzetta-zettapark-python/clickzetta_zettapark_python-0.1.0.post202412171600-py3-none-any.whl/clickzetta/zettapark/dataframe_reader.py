...
from clickzetta.zettapark.column import METADATA_COLUMN_TYPES, Column, _to_col_if_str
...
from clickzetta.zettapark.types import StructType, VariantType
...
class DataFrameReader:
    """Provides methods to load data in various supported formats from a Clickzetta
    stage to a :class:`DataFrame`. The paths provided to the DataFrameReader must refer
    to ClickZetta stages.
    ...
    Example 11:
        Querying metadata for staged files:
            >>> from clickzetta.zettapark.column import METADATA_FILENAME, METADATA_FILE_ROW_NUMBER
    ...
    def __init__(self, session: "clickzetta.zettapark.session.Session") -> None:
        ...
        self._infer_schema_transformations: Optional[
            List["clickzetta.zettapark.column.Column"]
        ] = None
    ...
    def csv(self, path: str) -> DataFrame:
        ...
        Returns:
            a :class:`DataFrame` that is set up to load data from the specified CSV file(s) in a ClickZetta stage.
        """
        ...
        if not self._user_schema:
            if not self._infer_schema:
                raise ZettaparkClientExceptionMessages.DF_MUST_PROVIDE_SCHEMA_FOR_READING_FILE()
    ...
    def json(self, path: str) -> DataFrame:
        ...
        Returns:
            a :class:`DataFrame` that is set up to load data from the specified JSON file(s) in a ClickZetta stage.
        """
    ...
    def avro(self, path: str) -> DataFrame:
        ...
        Returns:
            a :class:`DataFrame` that is set up to load data from the specified AVRO file(s) in a ClickZetta stage.
        """
    ...
    def parquet(self, path: str) -> DataFrame:
        ...
        Returns:
            a :class:`DataFrame` that is set up to load data from the specified PARQUET file(s) in a ClickZetta stage.
        """
    ...
    def orc(self, path: str) -> DataFrame:
        ...
        Returns:
            a :class:`DataFrame` that is set up to load data from the specified ORC file(s) in a ClickZetta stage.
        """
    ...
    def xml(self, path: str) -> DataFrame:
        ...
        Returns:
            a :class:`DataFrame` that is set up to load data from the specified XML file(s) in a ClickZetta stage.
        """
    ...
    def _infer_schema_for_file_format(
        ...
        try:
            ...
            new_schema = []
            schema_to_cast = []
            transformations: List["clickzetta.zettapark.column.Column"] = []
