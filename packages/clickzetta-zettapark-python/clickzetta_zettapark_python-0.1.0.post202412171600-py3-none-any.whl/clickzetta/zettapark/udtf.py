...
"""User-defined table functions (UDTFs) in Zettapark. Please see `Python UDTF <https://doc.clickzetta.com/>`_ for details.
...
Note:
    A vectorized UDTF must be called with :meth:`~clickzetta.zettapark.Window.partition_by` to build the partitions.

Refer to :class:`~clickzetta.zettapark.udtf.UDTFRegistration` for details and sample code on how to create regular and vectorized UDTFs using Zettapark Python API.
"""
...
from clickzetta.zettapark._internal.utils import TempObjectType, validate_object_name
from clickzetta.zettapark.table_function import TableFunctionCall
from clickzetta.zettapark.types import DataType, PandasDataFrameType, StructType
...
class UserDefinedTableFunction:
    ...
    :func:`~clickzetta.zettapark.functions.udtf`, :meth:`UDTFRegistration.register` or
    ...
    :class:`~clickzetta.zettapark.table_function.TableFunctionCall` instance. The input type can be
    a column name as a :class:`str`, or a :class:`~clickzetta.zettapark.Column` object.

    See Also:
        - :class:`UDTFRegistration`
        - :func:`~clickzetta.zettapark.functions.udtf`
    """
...
class UDTFRegistration:
    """
    Provides methods to register classes as UDTFs in the ClickZetta database.
    For more information about ClickZetta Python UDTFs, see `Python UDTFs <https://doc.clickzetta.com/>`__.

    :attr:`session.udtf <clickzetta.zettapark.Session.udtf>` returns an object of this class.
    ...
    Registering a UDTF is like registering a scalar UDF, you can use :meth:`register` or :func:`clickzetta.zettapark.functions.udtf`
    ...
    Another way is to use :meth:`register_from_file`. Refer to module :class:`clickzetta.zettapark.udtf.UDTFRegistration` for when to use them.

    To query a registered UDTF is the same as to query other table functions.
    Refer to :meth:`~clickzetta.zettapark.Session.table_function` and :meth:`~clickzetta.zettapark.DataFrame.join_table_function`.
    ...
    Example 11
        ...
        ``PandasDataFrameType`` with column names as ``output_schema``.
            ...
            >>> from clickzetta.zettapark.types import PandasDataFrameType, IntegerType, StringType, FloatType
            >>> class multiply:
            ...     def __init__(self):
            ...         self.multiplier = 10
    ...
    Example 12
        Creating a vectorized UDTF by specifying ``PandasDataFrame`` with nested types as type hints.
            ...
            >>> from clickzetta.zettapark.types import PandasDataFrame
            >>> class multiply:
            ...     def __init__(self):
            ...         self.multiplier = 10
    ...
    Example 13
        Creating a vectorized UDTF by specifying a ``pandas.DataFrame`` as type hints and a ``StructType`` with type information and column names as ``output_schema``.
            ...
            >>> import pandas as pd
            >>> from clickzetta.zettapark.types import IntegerType, StringType, FloatType, StructType, StructField
            >>> class multiply:
            ...     def __init__(self):
            ...         self.multiplier = 10
    ...
    See Also:
        ...
        - :meth:`~clickzetta.zettapark.Session.table_function`
        - :meth:`~clickzetta.zettapark.DataFrame.join_table_function`
    """

    def __init__(self, session: "clickzetta.zettapark.Session") -> None:
    ...
    def register(
        ...
        *,
        ...
        they are for :func:`~clickzetta.zettapark.functions.udtf`, but :meth:`register`
    ...
    def register_from_file(
        ...
        *,
        ...
        """
        Registers a Python class as a ClickZetta Python UDTF from a Python or zip file,
        ...
        Args:
            ...
            name: A string or list of strings that specify the name or fully-qualified
                ...
                command or via :func:`~clickzetta.zettapark.functions.call_udtf`.
                If it is not provided, a name will be automatically generated for the UDTF.
                A name must be specified when ``is_permanent`` is ``True``.
            is_permanent: Whether to create a permanent UDTF. The default is ``False``.
                If it is ``True``, a valid ``stage_location`` must be provided.
            ...
            packages: A list of packages that only apply to this UDTF. These UDTF-level packages
                will override the session-level packages added by
                :meth:`~clickzetta.zettapark.Session.add_packages` and
                :meth:`~clickzetta.zettapark.Session.add_requirements`. To use Python packages that are not
                available in ClickZetta, refer to :meth:`~clickzetta.zettapark.Session.custom_package_usage_config`.
            replace: Whether to replace a UDTF that already was registered. The default is ``False``.
                If it is ``False``, attempting to register a UDTF with a name that already exists
                results in a ``ZettaparkSQLException`` exception being thrown. If it is ``True``,
                an existing UDTF with the same name is overwritten.
