...
"""User-defined functions (UDFs) in Zettapark. Please see `Python UDFs <https://doc.clickzetta.com/>`_ for details.
Furthermore, there is vectorized UDF (Please see `Python UDF Batch API <https://doc.clickzetta.com/>`__ for details). Compared to the default row-by-row processing pattern of a normal UDF, which sometimes is
...
Refer to :class:`~clickzetta.zettapark.udf.UDFRegistration` for sample code on how to create and use regular and vectorized UDF's using Zettapark Python API.
"""
...
class UserDefinedFunction:
    ...
    :func:`~clickzetta.zettapark.functions.udf`, :meth:`UDFRegistration.register` or
    ...
    :class:`~clickzetta.zettapark.Column` expressions. The input type can be
    a column name as a :class:`str`, or a :class:`~clickzetta.zettapark.Column` object.

    See Also:
        - :class:`UDFRegistration`
        - :func:`~clickzetta.zettapark.functions.udf`
    """
    ...
    def _create_udf_expression(self, exprs: List[Expression]) -> UDF:
...
class UDFRegistration:
    """
    Provides methods to register lambdas and functions as UDFs in the ClickZetta database.
    For more information about ClickZetta Python UDFs, see `Python UDFs <https://doc.clickzetta.com/>`__.

    :attr:`session.udf <clickzetta.zettapark.Session.udf>` returns an object of this class.
    ...
    which you can also use in :class:`~clickzetta.zettapark.Column` expressions.

    There are two ways to register a UDF with Zettapark:

        - Use :func:`~clickzetta.zettapark.functions.udf` or :meth:`register`. By pointing to a
          ...
          function on the ClickZetta server during UDF creation. During the serialization, the
          ...
          If it's under 8K, it will be added to the `UDF in-line code <https://doc.clickzetta.com/>`__.
          ...
          :meth:`~clickzetta.zettapark.Session.add_import` and :meth:`~clickzetta.zettapark.Session.add_packages`
          ...
          the function from elsewhere. However, the limitations of this approach are:
            ...
            * If the runtime function references some very large global variables (e.g., a machine
              ...
              local environment first, add it to the UDF using  :meth:`~clickzetta.zettapark.Session.add_import`,
        ...
        - Use :meth:`register_from_file`. By pointing to a `Python file` or a `zip file containing
          ...
          the Python runtime on the ClickZetta server during UDF creation. Then this function will be
    ...
    For a vectorized UDF:
        You can use :func:`~clickzetta.zettapark.functions.udf`, :meth:`register` or
        :func:`~clickzetta.zettapark.functions.pandas_udf` to create a vectorized UDF by providing
    ...
    ClickZetta supports the following data types for the parameters for a UDF:
    ...
    ``datetime.time``                              :class:`~clickzetta.zettapark.types.TimeType`             TIME
    ...
    ``dict``                                       :class:`~clickzetta.zettapark.types.GeographyType`        GEOGRAPHY
    ``pandas.Series``                              :class:`~clickzetta.zettapark.types.PandasSeriesType`     No SQL type
    ``pandas.DataFrame``                           :class:`~clickzetta.zettapark.types.PandasDataFrameType`  No SQL type
    =============================================  ======================================================= ============
    ...
    Note:
        ...
        :class:`~clickzetta.zettapark.types.GeographyType` (:attr:`~clickzetta.zettapark.types.Geography`)
        by a UDF will be represented as a `GeoJSON <https://datatracker.ietf.org/doc/html/rfc7946>`_
        string.

        3. :class:`~clickzetta.zettapark.types.PandasSeriesType` and
        :class:`~clickzetta.zettapark.types.PandasDataFrameType` are used when creating a pandas
        (vectorized) UDF, so they are not mapped to any SQL types. ``element_type`` in
        :class:`~clickzetta.zettapark.types.PandasSeriesType` and ``col_types`` in
        :class:`~clickzetta.zettapark.types.PandasDataFrameType` indicate the SQL types
        in a pandas Series and a pandas DataFrame.

        4. To annotate the ClickZetta specific Timestamp type (TIMESTAMP_NTZ, TIMESTAMP_LTZ and
        TIMESTAMP_TZ), use :class:`~clickzetta.zettapark.types.Timestamp` with
        :class:`~clickzetta.zettapark.types.NTZ`, :class:`~clickzetta.zettapark.types.LTZ`,
        :class:`~clickzetta.zettapark.types.TZ` (e.g., ``Timestamp[NTZ]``).
    ...
    Example 8
        Use cache to read a file once from a stage in a UDF::
            ...
            ...     import_dir = sys._xoptions.get("clickzetta_import_directory")
            ...     if import_dir:
            ...
            ... )
            >>>
            >>> df = session.create_dataframe(["clickzetta", "python"], schema=["a"])
            >>> df.select(concat_file_content_with_str_udf("a")).to_df("col1").collect()
            [Row(COL1='zettapark-clickzetta'), Row(COL1='zettapark-python')]
        ...
        are not working when registering UDFs using Zettapark, due to the limitation of cloudpickle.

    Example 9
        Create a vectorized UDF from a lambda with a max batch size and apply it to a dataframe::

            >>> from clickzetta.zettapark.functions import udf
            >>> from clickzetta.zettapark.types import IntegerType, PandasSeriesType, PandasDataFrameType
    ...
    Example 10
        Create a vectorized UDF with type hints and apply it to a dataframe::
            ...
            >>> from clickzetta.zettapark.functions import udf
            >>> from clickzetta.zettapark.types import PandasSeries, PandasDataFrame
            >>> @udf
    ...
    Example 11
        Create a vectorized UDF with original ``pandas`` types and Zettapark types and apply it to a dataframe::

            >>> # `pandas_udf` is an alias of `udf`, but it can only be used to create a vectorized UDF
            >>> from clickzetta.zettapark.functions import pandas_udf
            >>> from clickzetta.zettapark.types import IntegerType
            >>> import pandas as pd
            >>> df = session.create_dataframe([[1, 2], [3, 4]]).to_df("a", "b")
    ...
    def describe(
        ...
        """
        Returns a :class:`~clickzetta.zettapark.DataFrame` that describes the properties of a UDF.

        Args:
            udf_obj: A :class:`UserDefinedFunction` returned by
                :func:`~clickzetta.zettapark.functions.udf` or :meth:`register`.
        """
        func_args = [convert_sp_to_cz_type(t) for t in udf_obj._input_types]
    ...
    def register(
        ...
        **kwargs,
        ...
        they are for :func:`~clickzetta.zettapark.functions.udf`, but :meth:`register`
        cannot be used as a decorator. See examples in
        :class:`~clickzetta.zettapark.udf.UDFRegistration` and notes in
        :func:`~clickzetta.zettapark.functions.udf`.
        ...
        Args:
            ...
            name: A string or list of strings that specify the name or fully-qualified
                ...
                command or via :func:`~clickzetta.zettapark.functions.call_udf`.
    ...
    def register_from_file(
        ...
        *,
        ...
        """
        Registers a Python function as a ClickZetta Python UDF from a Python or zip file,
