...
from clickzetta.zettapark._internal.analyzer.binary_plan_node import create_join_type
from clickzetta.zettapark._internal.analyzer.query_plan_node import UnresolvedRelation
...
from clickzetta.zettapark._internal.telemetry import add_api_call, set_api_call_source
from clickzetta.zettapark._internal.type_utils import ColumnOrLiteral
from clickzetta.zettapark.column import Column
from clickzetta.zettapark.dataframe import DataFrame, _disambiguate
...
class WhenMatchedClause:
    ...
    def update(self, assignments: Dict[str, ColumnOrLiteral]) -> "WhenMatchedClause":
        ...
        Example::
            ...
            >>> from clickzetta.zettapark.functions import when_matched, lit
        ...
        """
        if self._clause:
            raise ZettaparkClientExceptionMessages.MERGE_TABLE_ACTION_ALREADY_SPECIFIED(
                "update"
    ...
    def delete(self):
        ...
        Example::
            ...
            >>> from clickzetta.zettapark.functions import when_matched
            >>> target_df = session.create_dataframe([(10, "old"), (10, "too_old"), (11, "old")], schema=["key", "value"])
            >>> target_df.write.save_as_table("my_table", mode="overwrite", table_type="temporary")
            >>> target = session.table("my_table")
            >>> source = session.create_dataframe([(10, "new")], schema=["key", "value"])
...
class WhenNotMatchedClause:
    ...
    def insert(
        ...
        Examples::
            ...
            >>> from clickzetta.zettapark.functions import when_not_matched
...
class Table(DataFrame):
    ...
    with the name of the table in ClickZetta. See examples in :meth:`Session.table`.
    """
        ...
        *,
        ...
        Args:
            ...
            sampling_method: Specifies the sampling method to use:
                ...
                Default is ``None``. Then the ClickZetta database will use "ROW" by default.
    ...
    def update(
        ...
        *,
        statement_params: Optional[Dict[str, str]] = None,
        block: bool = True,
    ) -> Union[UpdateResult, "clickzetta.zettapark.AsyncJob"]:
        """
        ...
        result = new_df._internal_collect_with_tag(
            ...
            block=block,
            data_type=clickzetta.zettapark.async_job._AsyncResultType.UPDATE,
        )
    ...
    def delete(
        ...
        *,
        statement_params: Optional[Dict[str, str]] = None,
        block: bool = True,
    ) -> Union[DeleteResult, "clickzetta.zettapark.AsyncJob"]:
        """
        ...
        result = new_df._internal_collect_with_tag(
            statement_params=statement_params,
            block=block,
            data_type=clickzetta.zettapark.async_job._AsyncResultType.DELETE,
        )
    ...
    def merge(
        ...
        *,
        statement_params: Optional[Dict[str, str]] = None,
        block: bool = True,
    ) -> Union[MergeResult, "clickzetta.zettapark.AsyncJob"]:
        """
        ...
        See `MERGE <https://doc.clickzetta.com/>`_
        for details.
        ...
        Example::
            ...
            >>> from clickzetta.zettapark.functions import when_matched, when_not_matched
            >>> from clickzetta.zettapark.types import IntegerType, StringType, StructField, StructType
            >>> schema = StructType([StructField("key", IntegerType()), StructField("value", StringType())])
            >>> target_df = session.create_dataframe([(10, "old"), (10, "too_old"), (11, "old")], schema=schema)
            >>> target_df.write.save_as_table("my_table", mode="overwrite", table_type="temporary")
        ...
        result = new_df._internal_collect_with_tag(
            statement_params=statement_params,
            block=block,
            data_type=clickzetta.zettapark.async_job._AsyncResultType.MERGE,
        )
        if not block:
    ...
    def drop_table(self) -> None:
        """Drops the table from the ClickZetta database.
        ...
        table in the ClickZetta database no longer exists.
        """
        self._session.sql(
