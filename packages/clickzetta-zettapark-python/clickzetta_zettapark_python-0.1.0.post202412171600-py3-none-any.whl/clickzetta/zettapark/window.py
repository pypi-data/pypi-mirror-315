...
"""Window frames in Zettapark."""

import sys
from typing import List, Tuple, Union

import clickzetta.zettapark
from clickzetta.zettapark._internal.analyzer.expression import Expression, Literal
...
class Window:
    ...
    `Window functions <https://doc.clickzetta.com/>`_ for reference.

    Examples::

        >>> from clickzetta.zettapark.functions import col, avg
    ...
    def partition_by(
        *cols: Union[
        ...
        Args:
            cols: A column, as :class:`str`, :class:`~clickzetta.zettapark.column.Column`
                or a list of those.
        """
...
class WindowSpec:
    ...
    def partition_by(
        ...
        *cols: Union[
        ...
        partition_spec = [
            e._expression
            if isinstance(e, clickzetta.zettapark.column.Column)
            else clickzetta.zettapark.column.Column(e)._expression
            for e in exprs
        ]
        ...
        *cols: Union[
        ...
        for e in exprs:
            if isinstance(e, str):
                order_spec.append(
                    SortOrder(
                        clickzetta.zettapark.column.Column(e)._expression, Ascending()
                    )
                )
            elif isinstance(e, clickzetta.zettapark.column.Column):
    ...
    ) -> "clickzetta.zettapark.column.Column":
        spec = WindowSpecDefinition(self.partition_spec, self.order_spec, self.frame)
        return clickzetta.zettapark.column.Column(WindowExpression(aggregate, spec))
