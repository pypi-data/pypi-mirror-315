...
from clickzetta.zettapark._internal.utils import experimental
from clickzetta.zettapark.column import Column, _to_col_if_str
...
class DataFrameAnalyticsFunctions:
    ...
    def __init__(self, df: "clickzetta.zettapark.DataFrame") -> None:
        self._df = df
    ...
    def _get_sliding_interval_start(
        ...
        if unit == "mm":
            ...
            months_since_base = months_between(base_date, time_col)
    ...
    def _perform_window_aggregations(
        self,
        base_df: "clickzetta.zettapark.dataframe.DataFrame",
        input_df: "clickzetta.zettapark.dataframe.DataFrame",
    ...
    def moving_agg(
        ...
        Args:
            aggs: A dictionary where keys are column names and values are lists of the desired aggregation functions.
                Supported aggregation are listed here https://doc.clickzetta.com/
        ...
        Returns:
            A Zettapark DataFrame with additional columns corresponding to each specified moving aggregation.

        Raises:
            ValueError: If an unsupported value is specified in arguments.
            TypeError: If an unsupported type is specified in arguments.
            ZettaparkSQLException: If an unsupported aggregration is specified.
        ...
        for column, agg_funcs in aggs.items():
            for window_size in window_sizes:
                for agg_func in agg_funcs:
                    ...
                    # Apply the user-specified aggregation function directly. ClickZetta will handle any errors for invalid functions.
    ...
    def cumulative_agg(
        ...
        Returns:
            A ClickZetta DataFrame with additional columns corresponding to each specified cumulative aggregation.
    ...
    def compute_lag(
        ...
        Returns:
            A ClickZetta DataFrame with additional columns corresponding to each specified lag period.
    ...
    def compute_lead(
        ...
        Returns:
            A ClickZetta DataFrame with additional columns corresponding to each specified lead period.
    ...
    def time_series_agg(
        ...
        Returns:
            A ClickZetta DataFrame with additional columns corresponding to each specified time window aggregation.
        ...
        for window in windows:
            ...
            for column in right_df.column_names():
                if column not in group_by:
                    right_df = right_df.with_column_renamed(column, f"{column}B")

            self_joined_df = left_df.join(
                right_df, left_df[group_by] == right_df[group_by], how="leftouter"
            )

            # cast unit to dateadd unit.
            if window_unit == "s":
                dateadd_unit = "SECOND"
            elif window_unit == "m":
                dateadd_unit = "MINUTE"
                ...
                raise ValueError(
                    f"Unsupported unit '{window_unit}'. Supported units are '{SUPPORTED_TIME_UNITS}"
                )

            window_frame = dateadd(
                dateadd_unit, lit(window_duration), f"{sliding_point_col}"
            )
