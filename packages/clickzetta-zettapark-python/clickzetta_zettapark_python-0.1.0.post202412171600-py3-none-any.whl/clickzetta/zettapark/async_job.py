...
from clickzetta.zettapark._internal.analyzer.query_plan import Query
...
class AsyncJob:
    """
    Provides a way to track an asynchronous query in ClickZetta. A :class:`DataFrame` object can be
    ...
    Note:
        - If a dataframe is associated with multiple queries:
            ...
            + if you use :meth:`Session.create_dataframe` to create a dataframe from a large amount
              ...
              into ClickZetta synchronously, and only fetching data from ClickZetta again will be
              performed asynchronously.
            + otherwise, multiple queries will be wrapped into a
              `Anonymous Block <https://doc.clickzetta.com/>`_
        ...
        - Temporary objects (e.g., tables) might be created when evaluating dataframes and they will
          ...
          :meth:`result`.
        - This feature is currently not supported in ClickZetta Python stored procedures.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._cursor = session._conn.execute_and_notify_query_listener(query)
        self._df = self._session.sql(self.query)

    @property
    def query(self) -> Optional[str]:
        """
        The SQL text of of the executed query. Returns ``None`` if it cannot be retrieved from ClickZetta.
        """
        if not self._can_query_be_retrieved:
            ...
            #         )
            #     except ZettaparkSQLException as ex:
    ...
    def to_df(self) -> "clickzetta.zettapark.dataframe.DataFrame":
        """
    ...
    def _table_result(
        ...
        "clickzetta.zettapark.MergeResult",
        "clickzetta.zettapark.UpdateResult",
        "clickzetta.zettapark.DeleteResult",
    ]:
        if result_type == _AsyncResultType.UPDATE:
            return clickzetta.zettapark.UpdateResult(
                int(result_data[0][0]), int(result_data[0][1])
            )
        elif result_type == _AsyncResultType.DELETE:
            return clickzetta.zettapark.DeleteResult(int(result_data[0][0]))
        else:
            idx = 0
            ...
            return clickzetta.zettapark.MergeResult(
    ...
    def result(
        ...
        if self._num_statements is not None:
            ...
            # scan to have ClickZetta read the result and return Arrow format.
