...
from clickzetta.zettapark._connector import installed_pandas
...
from clickzetta.zettapark._internal.analyzer.unary_plan_node import (
...
from clickzetta.zettapark._internal.telemetry import (
    add_api_call,
    adjust_api_subcalls,
    df_api_usage,
...
from clickzetta.zettapark.column import Column, _to_col_if_sql_expr, _to_col_if_str
...
from clickzetta.zettapark.exceptions import ZettaparkDataframeException
...
from clickzetta.zettapark.types import StringType, StructType, _NumericType
...
def _alias_if_needed(
    ...
    col = df.col(c)
    unquoted_col_name = unquote_name(c)
    if c in common_col_names:
        if suffix:
            return col.alias(quote_name(f"{unquoted_col_name}{suffix}"))
        return col.alias(quote_name(f"{prefix}{unquoted_col_name}"))
    else:
        return col.alias(quote_name(f"{unquoted_col_name}"))
...
class DataFrame:
    ...
    **Creating a DataFrame**
    ...
    Example 1
        Creating a DataFrame by reading a table in ClickZetta::
    ...
    **Performing operations on a DataFrame**
    ...
    - **Actions** cause the DataFrame to be evaluated. When you call a method that performs an action, Zettapark sends the SQL query for the DataFrame to the server for evaluation.
    ...
    **Performing an action on a DataFrame**
    ...
    def __init__(
        ...
        self._reader: Optional["clickzetta.zettapark.DataFrameReader"] = None
    ...
    def _internal_collect_with_tag_no_telemetry(
        ...
        *,
        ...
        # When executing a DataFrame in any method of zettapark (either public or private),
    ...
    def __getitem__(self, item: Union[str, Column, List, Tuple, int]):
        if isinstance(item, str):
            ...
            return self.__getitem__(self.column_names()[item])
        else:
            raise TypeError(f"Unexpected item type: {type(item)}")

    def __getattr__(self, name: str):
        # ClickZetta DB ignores cases when there is no quotes.
        if quote_name(name.lower()) not in [c.lower() for c in self.column_names()]:
    ...
    def columns(self) -> List[str]:
        ...
        return self.column_names(False)

    def column_names(self, quoted: bool = True) -> List[str]:
        return (
            self.schema.names
            if quoted
            else [unquote_name(c) for c in self.schema.names]
        )
    ...
    def select(
        ...
        *cols: Union[
        ...
        Example 5::
            ...
            >>> from clickzetta.zettapark.functions import table_function
    ...
    def drop(
        ...
        *cols: Union[ColumnOrName, Iterable[ColumnOrName]],
        ...
        Raises:
            :class:`ZettaparkClientException`: if the resulting :class:`DataFrame`
        ...
        for c in exprs:
            if isinstance(c, str):
                ...
                raise ZettaparkClientExceptionMessages.DF_CANNOT_DROP_COLUMN_NAME(
                    str(c)
                )
        ...
        if not keep_col_names:
            raise ZettaparkClientExceptionMessages.DF_CANNOT_DROP_ALL_COLUMNS()
        else:
    ...
    def agg(
        ...
        *exprs: Union[Column, Tuple[ColumnOrName, str], Dict[str, str]],
        ...
        Examples::
            ...
            >>> from clickzetta.zettapark.functions import col, stddev, stddev_pop
        ...
        Note:
            The name of the aggregate function to compute must be a valid ClickZetta `aggregate function
    ...
    def rollup(
        self,
        *cols: Union[ColumnOrName, Iterable[ColumnOrName]],
    ) -> "clickzetta.zettapark.RelationalGroupedDataFrame":
        """Performs a SQL
        `GROUP BY ROLLUP <https://doc.clickzetta.com/>`_.
        on the DataFrame.
        ...
        return clickzetta.zettapark.RelationalGroupedDataFrame(
            self,
            rollup_exprs,
            clickzetta.zettapark.relational_grouped_dataframe._RollupType(),
        )
    ...
    def group_by(
        ...
        *cols: Union[ColumnOrName, Iterable[ColumnOrName]],
        ...
        Examples:
            ...
            >>> from clickzetta.zettapark.functions import col, lit, sum as sum_, max as max_
        ...
        return clickzetta.zettapark.RelationalGroupedDataFrame(
            self,
            grouping_exprs,
            clickzetta.zettapark.relational_grouped_dataframe._GroupByType(),
        )
    ...
    def group_by_grouping_sets(
        self,
        *grouping_sets: Union[
            "clickzetta.zettapark.GroupingSets",
            Iterable["clickzetta.zettapark.GroupingSets"],
        ],
    ) -> "clickzetta.zettapark.RelationalGroupedDataFrame":
        """Performs a SQL
        `GROUP BY GROUPING SETS <https://doc.clickzetta.com/>`_.
        on the DataFrame.
    ...
    def cube(
        ...
        *cols: Union[ColumnOrName, Iterable[ColumnOrName]],
        ...
        """Performs a SQL
        `GROUP BY CUBE <https://doc.clickzetta.com/>`_.
        on the DataFrame.
        ...
        return clickzetta.zettapark.RelationalGroupedDataFrame(
            self,
            cube_exprs,
            clickzetta.zettapark.relational_grouped_dataframe._CubeType(),
        )
    ...
    def drop_duplicates(self, *subset: Union[str, Iterable[str]]) -> "DataFrame":
        ...
        output_cols = [self.col(col_name) for col_name in self.column_names()]
        rownum = row_number().over(
            clickzetta.zettapark.Window.partition_by(*filter_cols).order_by(
                *filter_cols
            )
        )
    ...
    def pivot(
        ...
        return clickzetta.zettapark.RelationalGroupedDataFrame(
            self,
            [],
            clickzetta.zettapark.relational_grouped_dataframe._PivotType(
                pc[0], value_exprs
            ),
        )
        ...
        if self._select_statement:
            return self._with_plan(
                self._select_statement.set_operator(
                    ...
                    or SelectQueryPlan(other._plan, analyzer=self._session._analyzer),
                    operator=SET_UNION,
                )
            )
    ...
    def _union_by_name_internal(
        ...
        try:
            ...
            raise ZettaparkClientExceptionMessages.DF_CANNOT_RESOLVE_COLUMN_NAME_AMONG(
                ", ".join(missing_lattrs),
    ...
    def join(
        ...
        **kwargs,
        ...
        See Also:
            - Usage notes for asof join: https://doc.clickzetta.com/
        ...
        if isinstance(right, DataFrame):
            if self is right or self._plan is right._plan:
                raise ZettaparkClientExceptionMessages.DF_SELF_JOIN_NOT_SUPPORTED()
    ...
    def with_column(
        ...
        Example 2::
            ...
            >>> from clickzetta.zettapark.functions import udtf
    ...
    def write(self) -> DataFrameWriter:
        ...
        a ClickZetta database or a stage location
    ...
    def copy_into_table(
        ...
        **copy_options: Any,
        ...
        """Executes a `COPY INTO <table> <https://doc.clickzetta.com/>`__ command to load data from files in a stage location into a specified table.

        It returns the load result described in `OUTPUT section of the COPY INTO <table> command <https://doc.clickzetta.com/>`__.
        ...
        Example::
            ...
            >>> from clickzetta.zettapark.types import StringType, StructField, StringType
            >>> from clickzetta.zettapark.functions import length
        ...
        The arguments of this function match the optional parameters of the `COPY INTO <table> <https://doc.clickzetta.com/>`__.
        ...
        if not self._reader or not self._reader._file_path:
            raise ZettaparkDataframeException(
    ...
    def _show_string(
        ...
        **kwargs,
        ...
        header = []
        for field in meta:
            name = quote_name(field.name) if header_quoted else unquote_name(field.name)
    ...
    def create_or_replace_view(
        ...
        *,
        ...
        ``name`` must be a valid `identifier <https://doc.clickzetta.com/>`_.
    ...
    def _do_create_or_replace_view(self, view_name: str, view_type: ViewType, **kwargs):
        ...
        full_view_name = self._session.get_fully_qualified_name_if_possible(names)
        self._session._register_temp_object(TempObjectType.VIEW, full_view_name)
        return self._session._conn.execute(
    ...
    @property
    def session(self) -> "clickzetta.zettapark.Session":
        """
        Returns a :class:`clickzetta.zettapark.Session` object that provides access to the session the current DataFrame is relying on.
        """
            ...
            for c, t in numerical_string_col_type_dict.items():
                ...
                if isinstance(t, StringType):
                    if name in ["mean", "stddev"]:
                        agg_cols.append(cast(func(lit(0)), StringType()).as_(c))
                    else:
                        agg_cols.append(cast(func(c), StringType()).as_(c))
                else:
    ...
    def with_column_renamed(self, existing: ColumnOrName, new: str) -> "DataFrame":
        ...
        if not to_be_renamed:
            ...
            raise ZettaparkClientExceptionMessages.DF_CANNOT_RENAME_COLUMN_BECAUSE_MULTIPLE_EXIST(
    ...
    def cache_result(
        ...
        temp_table_name = self._session.get_fully_qualified_name_if_possible(
            f"{random_name_for_temp_object(TempObjectType.TABLE)}_cache"
        )
        self._session._register_temp_object(TempObjectType.TABLE, temp_table_name)
        ...
        *,
        ...
        if not weights:
            ...
            cached_df = self.with_column(
                temp_column_name, abs_(random(seed) * _ONE_MILLION) % _ONE_MILLION
    ...
    def explain(self) -> None:
        ...
        `EXPLAIN <https://doc.clickzetta.com/>`_ command.
        """
...
{output_queries}"""
    ...
    def _resolve(self, col_name: str) -> Union[Expression, NamedExpression]:
        ...
        cols = list(
            filter(
                lambda attr: quote_name(attr.name) == normalized_col_name,
                self._output,
            )
        )
        if len(cols) == 1:
            ...
            raise ZettaparkClientExceptionMessages.DF_CANNOT_RESOLVE_COLUMN_NAME(
                col_name
            )
