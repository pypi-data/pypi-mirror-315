...
from clickzetta.zettapark import functions
...
from clickzetta.zettapark._internal.analyzer.unary_plan_node import Aggregate, Pivot
from clickzetta.zettapark._internal.error_message import (
    ZettaparkClientExceptionMessages,
)
from clickzetta.zettapark._internal.telemetry import relational_group_df_api_usage
from clickzetta.zettapark._internal.type_utils import ColumnOrName, LiteralType
from clickzetta.zettapark._internal.utils import parse_positional_args_to_list
...
def _alias(expr: Expression) -> NamedExpression:
    ...
    elif isinstance(expr, (NamedExpression, UDF)):
        return expr
    else:
        return Alias(expr, expr.column_name)
...
class RelationalGroupedDataFrame:
    ...
    See also:
        :meth:`clickzetta.zettapark.DataFrame.agg`.
    """
    ...
    def _to_df(self, agg_exprs: List[Expression]) -> DataFrame:
        ...
        if isinstance(self._group_type, _GroupByType):
            ...
            if len(agg_exprs) != 1:
                raise ZettaparkClientExceptionMessages.DF_PIVOT_ONLY_SUPPORT_ONE_AGG_EXPR()
            group_plan = Pivot(
                self._grouping_exprs,
        ...
        if self._df._select_statement:
            group_plan = self._df._session._analyzer.create_select_statement(
                from_=self._df._session._analyzer.create_select_query_plan(
    ...
    def apply_in_pandas(
        ...
        **kwargs,
        ...
        specifying the keyword argument `packages=["pandas]` in this call or calling :meth:`~clickzetta.zettapark.Session.add_packages` beforehand.
        ...
        Args:
            func: A Python native function that accepts a single input argument - a ``pandas.DataFrame``
                ...
                a vectorized UDTF.
            output_schema: A :class:`~clickzetta.zettapark.types.StructType` instance that represents the
            ...
            kwargs: Additional arguments to register the vectorized UDTF. See
                :meth:`~clickzetta.zettapark.udtf.UDTFRegistration.register` for all options.

        Examples::
            Call ``apply_in_pandas`` using temporary UDTF:

                >>> import pandas as pd
                >>> from clickzetta.zettapark.types import StructType, StructField, StringType, FloatType
            ...
            Call ``apply_in_pandas`` using permanent UDTF with replacing original UDTF:
                ...
                >>> from clickzetta.zettapark.types import IntegerType, DoubleType
        ...
        See Also:
            ...
            - :func:`~clickzetta.zettapark.functions.pandas_udtf`
        """
        ...
        return self._df.select(
            _apply_in_pandas_udtf(*self._df.column_names()).over(
    ...
    @relational_group_df_api_usage
    def bitand(self, *cols: ColumnOrName) -> DataFrame:
        """Return the bitand for the specified numeric columns."""
        return self._non_empty_argument_function("bitand", *cols)

    @relational_group_df_api_usage
    def bitor(self, *cols: ColumnOrName) -> DataFrame:
        """Return the bitand for the specified numeric columns."""
        return self._non_empty_argument_function("bitor", *cols)

    @relational_group_df_api_usage
    def bitxor(self, *cols: ColumnOrName) -> DataFrame:
        """Return the bitand for the specified numeric columns."""
        return self._non_empty_argument_function("bitxor", *cols)
