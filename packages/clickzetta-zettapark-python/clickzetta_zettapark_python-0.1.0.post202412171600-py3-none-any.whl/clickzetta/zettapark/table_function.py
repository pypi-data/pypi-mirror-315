...
from clickzetta.zettapark._internal.analyzer.sort_expression import Ascending, SortOrder
...
from clickzetta.zettapark._internal.utils import quote_name, validate_object_name
from clickzetta.zettapark.column import Column, _to_col_if_str
from clickzetta.zettapark.types import ArrayType, MapType

from ._internal.analyzer.query_plan import QueryPlan
...
class TableFunctionCall:
    ...
    Instead, use :func:`~clickzetta.zettapark.function.call_table_function`, which will create an instance of this class.
    Or use :func:`~clickzetta.zettapark.function.table_function` to create a ``Callable`` object and call it to create an
    ...
    def over(
        ...
        *,
        ...
        Before rows are passed to table functions, the rows can be grouped into partitions. Partitioning has two main benefits:
          ...
          - Partitioning allows ClickZetta to divide up the workload to improve parallelization and thus performance.
          - Partitioning allows ClickZetta to process all rows with a common characteristic as a group. You can return results that are based on all rows in the group, not just on individual rows.

        Refer to `table functions and partitions <https://doc.clickzetta.com/>`__ for more information.
        ...
        If this function isn't called at all, the ClickZetta database will use implicit partitioning.
        """
...
def _get_cols_after_join_table(
    ...
    def get_column_names_from_plan(plan: QueryPlan) -> List[str]:
...
def _get_cols_after_explode_join(
    func: _ExplodeFunctionCall, plan: QueryPlan
) -> Tuple[List, List]:
