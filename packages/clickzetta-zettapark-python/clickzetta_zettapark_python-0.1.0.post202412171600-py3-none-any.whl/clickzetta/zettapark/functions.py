...
from clickzetta.zettapark.stored_procedure import StoredProcedure
...
from clickzetta.zettapark.udaf import UserDefinedAggregateFunction
from clickzetta.zettapark.udf import UserDefinedFunction
from clickzetta.zettapark.udtf import UserDefinedTableFunction
...
def flatten(
    ...
    See Also:
        - :func:`explode`
        - `Flatten <https://doc.clickzetta.com/>`_
    """
    col = _to_col_if_str(col, "flatten")
    func_call = clickzetta.zettapark.table_function.TableFunctionCall(
        "flatten",
        input=col,
        path=lit(path),
        outer=lit(outer),
...
def collate(e: Column, collation_spec: str) -> Column:
    ...
    For details, see the ClickZetta documentation on
...
def months_between(date1: ColumnOrName, date2: ColumnOrName) -> Column:
    ...
    return builtin("datediff")("month", c1, c2)
...
def to_geography(e: ColumnOrName) -> Column:
    ...
    For all supported formats confer https://doc.clickzetta.com/
    """
...
def arrays_to_object(
    ...
    See Also:
        - https://doc.clickzetta.com/ for information on Objects
    """
...
def daydiff(col1: ColumnOrName, col2: ColumnOrName) -> Column:
    ...
    Example::
        >>> from clickzetta.zettapark.functions import daydiff, to_date
...
def is_boolean(col: ColumnOrName) -> Column:
    ...
    Examples::
        ...
        >>> from clickzetta.zettapark.types import StructField, VariantType
...
def is_real(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import to_variant, is_real
        >>> df = session.create_dataframe([[1.2, "X"]], schema=["a", "b"])
...
def is_null_value(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import to_variant, is_null_value
...
def is_object(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import to_variant, is_object
...
def is_time(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> import datetime
        >>> from clickzetta.zettapark.functions import to_variant, is_time
...
def is_timestamp_ltz(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import to_variant, is_timestamp_ltz
...
def is_timestamp_ntz(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import to_variant, is_timestamp_ntz
        >>> df = session.sql("select to_timestamp_ntz('2017-02-24 12:00:00.456') as timestamp_ntz1, "
        ...                  "to_timestamp_ltz('2017-02-24 13:00:00.123 +01:00') as timestamp_ltz1, "
        ...                  "to_timestamp_tz('2017-02-24 13:00:00.123 +01:00') as timestamp_tz1")
...
def is_timestamp_tz(col: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import to_variant, is_timestamp_tz
        >>> df = session.sql("select to_timestamp_ntz('2017-02-24 12:00:00.456') as timestamp_ntz1, "
        ...                  "to_timestamp_ltz('2017-02-24 13:00:00.123 +01:00') as timestamp_ltz1, "
        ...                  "to_timestamp_tz('2017-02-24 13:00:00.123 +01:00') as timestamp_tz1")
...
def typeof(col: ColumnOrName) -> Column:
    ...
    For columns where all rows share the same type, the result of `typeof` is the underlying ClickZetta column type.
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.types import VariantType, StructType, StructField
...
def json_extract_path_text(col: ColumnOrName, path: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import json_extract_path_text, to_variant
...
def array_contains(variant: ColumnOrName, array: ColumnOrName) -> Column:
    ...
    return builtin("array_contains")(a, v)
...
def array_position(variant: ColumnOrName, array: ColumnOrName) -> Column:
    ...
    return builtin("array_position")(a, v)
...
def object_agg(key: ColumnOrName, value: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.types import StructType, StructField, VariantType, StringType
        >>> df = session.create_dataframe(
        ...     [["name", "Joe"], ["zip", "98004"]],
...
def as_char(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import as_char, to_variant
...
def as_varchar(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import as_varchar, to_variant
        >>> df = session.sql("select 'some string' as char")
        >>> df.char_v = to_variant(df.char)
        >>> df.select(df.char_v.as_("char")).collect() == df.select(df.char).collect()
        False
...
def as_real(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.types import VariantType, StructType, StructField, DoubleType
...
def as_time(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import as_time, to_variant
...
def as_timestamp_ltz(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import as_timestamp_ltz, to_variant
...
def as_timestamp_ntz(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import as_timestamp_ntz, to_variant
...
def as_timestamp_tz(variant: ColumnOrName) -> Column:
    ...
    Example::
        ...
        >>> from clickzetta.zettapark.functions import as_timestamp_tz, to_variant
...
def to_json(e: ColumnOrName) -> Column:
    ...
    Example::
        >>> from clickzetta.zettapark.types import VariantType, StructField, StructType
        >>> from clickzetta.zettapark import Row
        >>> schema = StructType([StructField("a", VariantType())])
...
def udf(
    ...
    *,
    ...
    """Registers a Python function as a ClickZetta Python UDF and returns the UDF.
    ...
    in :class:`~clickzetta.zettapark.udf.UDFRegistration`.
    ...
    Args:
        ...
        name: A string or list of strings that specify the name or fully-qualified
            ...
            the UDF in ClickZetta, which allows you to call this UDF in a SQL
        ...
        imports: A list of imports that only apply to this UDF. You can use a string to
            ...
            argument in :meth:`~clickzetta.zettapark.Session.add_import`). These UDF-level imports
            will override the session-level imports added by
            :meth:`~clickzetta.zettapark.Session.add_import`. Note that an empty list means
        ...
        packages: A list of packages that only apply to this UDF. These UDF-level packages
            ...
            :meth:`~clickzetta.zettapark.Session.add_requirements`. Note that an empty list means
            no package for this UDF, and ``None`` or not specifying this parameter means using
            session-level packages. To use Python packages that are not available in ClickZetta,
            refer to :meth:`~clickzetta.zettapark.Session.custom_package_usage_config`.
        ...
        max_batch_size: The maximum number of rows per input pandas DataFrame or pandas Series
            ...
            guarantee that ClickZetta will encode batches with the specified number of rows. It will
        ...
        secure: Whether the created UDF is secure. For more information about secure functions,
            see `Secure UDFs <https://doc.clickzetta.com/>`_.
    ...
    Returns:
        A UDF function that can be called with :class:`~clickzetta.zettapark.Column` expressions.
    ...
    Note:
        ...
        :class:`~clickzetta.zettapark.udf.UDFRegistration`.
            ...
            - You can use use :attr:`~clickzetta.zettapark.types.Variant` to
              annotate a variant, and use :attr:`~clickzetta.zettapark.types.Geography`
              or :attr:`~clickzetta.zettapark.types.Geometry` to annotate geospatial
              types when defining a UDF.

            - You can use use :attr:`~clickzetta.zettapark.types.PandasSeries` to annotate
              a pandas Series, and use :attr:`~clickzetta.zettapark.types.PandasDataFrame`
        ...
        (:func:`session.get_session_stage() <clickzetta.zettapark.Session.get_session_stage>`).
        ...
        automatically, with the latest version on the ClickZetta server. If you don't want to
        use this version, you can overwrite it by adding `pandas` with specific version
        requirement using ``package`` argument or :meth:`~clickzetta.zettapark.Session.add_packages`.
...
def udtf(
    ...
    *,
    ...
    """Registers a Python class as a ClickZetta Python UDTF and returns the UDTF.
    ...
    in :class:`~clickzetta.zettapark.udtf.UDTFRegistration`.

    Args:
        handler: A Python class used for creating the UDTF.
        output_schema: A list of column names, or a :class:`~clickzetta.zettapark.types.StructType` instance that represents the table function's columns, or a ``PandasDataFrameType`` instance for vectorized UDTF.
        ...
        name: A string or list of strings that specify the name or fully-qualified
            object identifier (database name, schema name, and function name) for
            the UDTF in ClickZetta, which allows you to call this UDTF in a SQL
        ...
        imports: A list of imports that only apply to this UDTF. You can use a string to
            ...
            :meth:`~clickzetta.zettapark.Session.add_import`) in this list, or a tuple of two
            strings to represent a file path and an import path (similar to the ``import_path``
            argument in :meth:`~clickzetta.zettapark.Session.add_import`). These UDTF-level imports
            will override the session-level imports added by
        ...
        packages: A list of packages that only apply to this UDTF. These UDTF-level packages
            will override the session-level packages added by
            :meth:`~clickzetta.zettapark.Session.add_packages` and
            :meth:`~clickzetta.zettapark.Session.add_requirements`. To use Python packages that are not available
            in ClickZetta, refer to :meth:`~clickzetta.zettapark.Session.custom_package_usage_config`.
    ...
    Returns:
        A UDTF function that can be called with :class:`~clickzetta.zettapark.Column` expressions.

    Note:
        1. When type hints are provided and are complete for a function,
        ``return_type`` and ``input_types`` are optional and will be ignored.
        See details of supported data types for UDTFs in
        :class:`~clickzetta.zettapark.udtf.UDTFRegistration`.
...
def udaf(
    ...
    *,
    ...
    """Registers a Python class as a ClickZetta Python UDAF and returns the UDAF.
    ...
    in :class:`~clickzetta.zettapark.udaf.UDAFRegistration`.
    ...
    Args:
        ...
        name: A string or list of strings that specify the name or fully-qualified
            object identifier (database name, schema name, and function name) for
            the UDAF in ClickZetta, which allows you to call this UDAF in a SQL
        ...
        imports: A list of imports that only apply to this UDAF. You can use a string to
            ...
            :meth:`~clickzetta.zettapark.Session.add_import`) in this list, or a tuple of two
            strings to represent a file path and an import path (similar to the ``import_path``
            argument in :meth:`~clickzetta.zettapark.Session.add_import`). These UDAF-level imports
            will override the session-level imports added by
            :meth:`~clickzetta.zettapark.Session.add_import`. Note that an empty list means
    ...
    Returns:
        A UDAF function that can be called with :class:`~clickzetta.zettapark.Column` expressions.

    Note:
        1. When type hints are provided and are complete for a function,
        ``return_type`` and ``input_types`` are optional and will be ignored.
        See details of supported data types for UDAFs in
        :class:`~clickzetta.zettapark.udaf.UDAFRegistration`.
...
def pandas_udf(
    ...
    *,
    ...
    See examples in :class:`~clickzetta.zettapark.udf.UDFRegistration`.

    See Also:
        - :func:`udf`
        - :meth:`UDFRegistration.register() <clickzetta.zettapark.udf.UDFRegistration.register>`

    Example::

        >>> from clickzetta.zettapark.types import PandasSeriesType, PandasDataFrameType, IntegerType
...
def pandas_udtf(
    ...
    *,
    ...
    See examples in :class:`~clickzetta.zettapark.udtf.UDTFRegistration`.

    See Also:
        - :func:`udtf`
        - :meth:`UDTFRegistration.register() <clickzetta.zettapark.udf.UDTFRegistration.register>`
    ...
    You can use :func:`~clickzetta.zettapark.functions.udtf`, :meth:`register` or
    :func:`~clickzetta.zettapark.functions.pandas_udtf` to create a vectorized UDTF by providing
    ...
    Note: A vectorized UDTF must be called with `~clickzetta.zettapark.Window.partition_by` to build the partitions.
...
def sproc(
    ...
    *,
    ...
    replace: bool = False,
    if_not_exists: bool = False,
    session: Optional["clickzetta.zettapark.Session"] = None,
    parallel: int = 4,
    statement_params: Optional[Dict[str, str]] = None,
    ...
    **kwargs,
    ...
    in :class:`~clickzetta.zettapark.stored_procedure.StoredProcedureRegistration`.

    Note that the first parameter of your function should be a zettapark Session. Also, you need to add
    `clickzetta-zettapark-python` package to your session before trying to create a stored procedure.
    ...
    Args:
        ...
        imports: A list of imports that only apply to this stored procedure. You can use a string to
            ...
            :meth:`~clickzetta.zettapark.Session.add_import`) in this list, or a tuple of two
            strings to represent a file path and an import path (similar to the ``import_path``
            argument in :meth:`~clickzetta.zettapark.Session.add_import`). These stored-proc-level imports
            will override the session-level imports added by
            :meth:`~clickzetta.zettapark.Session.add_import`.
        packages: A list of packages that only apply to this stored procedure. These stored-proc-level packages
            will override the session-level packages added by
            :meth:`~clickzetta.zettapark.Session.add_packages` and
            :meth:`~clickzetta.zettapark.Session.add_requirements`. To use Python packages that are not available in
            ClickZetta, refer to :meth:`~clickzetta.zettapark.Session.custom_package_usage_config`.
        ...
        execute_as: What permissions should the procedure have while executing. This
            supports caller, or owner for now. See `owner and caller rights <https://doc.clickzetta.com/>`_
    ...
    Note:
        ...
        :class:`~clickzetta.zettapark.stored_procedure.StoredProcedureRegistration`.
            ...
            - You can use :attr:`~clickzetta.zettapark.types.Variant` to
              annotate a variant, and use :attr:`~clickzetta.zettapark.types.Geography`
              or :attr:`~clickzetta.zettapark.types.Geometry` to annotate geospatial
        ...
        use one of the following ways:
            ...
            - (Recommended) Describe the return type using :attr:`~clickzetta.zettapark.types.StructType`
              and :attr:`~clickzetta.zettapark.types.StructField`. Set ``return_type =
            ...
            - When using type hints, the return type of function can be set as
              :class:`~clickzetta.zettapark.dataframe.DataFrame`. This registers a
    ...
    Example::
        >>> from clickzetta.zettapark.types import IntegerType
        >>> @sproc(return_type=IntegerType(), input_types=[IntegerType(), IntegerType()], packages=["clickzetta-zettapark-python"])
