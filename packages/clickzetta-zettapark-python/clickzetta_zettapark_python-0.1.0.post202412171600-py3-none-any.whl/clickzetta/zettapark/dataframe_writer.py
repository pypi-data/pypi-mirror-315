...
import clickzetta.zettapark  # for forward references of type hints
...
from clickzetta.zettapark._internal.type_utils import ColumnOrName, ColumnOrSqlExpr
...
from clickzetta.zettapark.functions import sql_expr
...
class DataFrameWriter:
    ...
    def __init__(self, dataframe: "clickzetta.zettapark.dataframe.DataFrame") -> None:
    ...
    def save_as_table(
        ...
        *,
        ...
        """Writes the data to the specified table in a Clickzetta database.
        ...
        column_names = (
            self._dataframe.column_names() if column_order.lower() == "name" else None
        )
        clustering_exprs = (
            [
        ...
        query_plan = session._analyzer.resolve(create_table_logic_plan)
    ...
    def copy_into_location(
        ...
        **copy_options: Optional[str],
        ...
        """Executes a `COPY INTO <location> <https://doc.clickzetta.com/>`__ to unload data from a ``DataFrame`` into one or more files in a stage or external stage.
        ...
        Args:
            ...
            format_type_options: Depending on the ``file_format_type`` specified, you can include more format specific options. Use the options documented in the `Format Type Options <https://doc.clickzetta.com/>`__.
            header: Specifies whether to include the table column headings in the output files.
            statement_params: Dictionary of statement level parameters to be set while executing this action.
            copy_options: The kwargs that are used to specify the copy options. Use the options documented in the `Copy Options <https://doc.clickzetta.com/>`__.
            block: A bool value indicating whether this function will wait until the result is available.
                When it is ``False``, this function executes the underlying queries of the dataframe
                asynchronously and returns an :class:`AsyncJob`.
        ...
        Example::
            ...
            3
            >>> # the following code snippet just verifies the file content and is actually irrelevant to Zettapark
    ...
    copy_into_volume = copy_into_location
