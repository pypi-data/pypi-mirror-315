...
class OracledbExtractor:
    def __init__(self, config, settings):
        ...
        self.driver_name = 'oracle:thin'
        self.driver_jdbc = 'oracle.jdbc.OracleDriver'
        self.settings.driver_name = self.driver_name
        self.jdbc_url = f'jdbc:{self.driver_name}:{self.username}/{self.password}@//{self.host}:{self.port}/{self.database}?oracle.jdbc.defaultNChar=true'
    ...
    def extract(self):
        ...
        logger = Logger(__file__)
        logger.info({'message': 'Extracting data from Oracle DB...'})
        t = self.table
        try:
            target_name = t['target_name']
