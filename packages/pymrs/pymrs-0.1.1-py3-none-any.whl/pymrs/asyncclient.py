...
class AsyncMRSClient:
    ...
    Attributes:
        hostname (str): The base URL of the MRS server.
    ...
    def __init__(
        ...
        if password is not None:
            ...
            logging.info(f"Kadme token: {self.kadme_token}, username: {self.username}")
        else:
            self._ticket = ticket
            self.client.headers.update({"ticket": f"{ticket}"})
            logging.info(
                f"Kadme token: {self.kadme_token}, ticket: {self._ticket}, username: {self.username}"
            )
    ...
    async def _authenticate(self, oauth_token=None) -> str:
        ...
        if hasattr(self, "password"):
            ...
            url = f"{self.hostname}/whereoil-rest-server/security/auth/login.json"
            logging.info(f"POST: {url}")
            logging.info(f"JSON: {payload}")
            logging.info(f"Headers:{self.client.headers}")
            if oauth_token != None:
            ...
            logging.info(f"Retrieved ticket: {ticket}")
            self.client.headers.update({"ticket": f"{ticket}"})
            return ticket
        else:
            return ""
    ...
    async def validate_ticket(self):
        ...
        if hasattr(self, "password"):
            ...
            if self._ticket == "":
                ...
                url = f"{self.hostname}/whereoil-rest-server/security/auth/validateticket.json"
    ...
    async def close(self):
        ...
        logout_url = f"{self.hostname}/whereoil-rest-server/security/auth/logout.json"
        try:
    ...
    async def _handle_error(self, response: httpx.Response):
        ...
        if isinstance(error_data.get("error"), str):
            raise MRSClientError(error_data["status"], error_data["error"])
        else:
            raise MRSClientError(
    ...
    async def request(
        ...
        # client = AsyncMRSClient(host, kadme_token, username, password)
        # await client._autheticate()
        # r = await client.request("POST", "/whereoil-rest-server/security/auth/login.json", "{"userName": username, "password": password}")
        # await client.client.aclose() #close session after usage
        # ```
        ...
        url: str = f"{self.hostname}/whereoil-rest-server{endpoint}"
