...
class BookkeepingManagerHandlerMixin:
    ...
    def _getFilesWithMetadata(cls, in_dict):
        ...
        method = cls.bkkDB.getFilesWithMetadata
        force_kwargs = {}
        if parameters := in_dict.get("OnlyParameters", None):
            force_kwargs["parameters"] = parameters
        if seed_md5 := in_dict.get("SampleSeedMD5", None):
            force_kwargs["seed_md5"] = seed_md5
        if sample_max := in_dict.get("SampleMax", None):
            force_kwargs["sample_max"] = sample_max

        if force_kwargs:
            # Only the new implementation supports this
            method = partial(cls.bkkDB._newdb.getFilesWithMetadata, **force_kwargs)
        else:
    ...
    def export_getFilesSummary(self, in_dict):
        ...
        retVal = self.bkkDB.getFilesSummary(
    ...
    #############################################################################
    types_getInputOutputFilesForProd = [int, int]
    ...
    #################
    types_getOutputDescendantsForProd = [int, int]
    ...
    def export_getProductionProducedEvents(cls, prodid):
        ...
        cls.log.debug("Retrieving the number of processed event for production", prodid)
