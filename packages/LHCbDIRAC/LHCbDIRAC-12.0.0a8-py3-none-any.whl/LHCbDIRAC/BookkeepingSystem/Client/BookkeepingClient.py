...
class BookkeepingClient(Client):
    ...
    def getFilesWithMetadata(in_dict):
        ...
        in_dict["MethodName"] = "getFilesWithMetadata"
        params = JEncoder.dumps(in_dict)
        with tempfile.NamedTemporaryFile() as fileName:
            retVal = bkk.receiveFile(fileName.name, params)
            if not retVal["OK"]:
                return retVal
            value = JEncoder.load(open(fileName.name))
        return value
    ...
    def getFiles(self, in_dict):
        ...
        with tempfile.NamedTemporaryFile() as fileName:
            ...
            if not retVal["OK"]:
                return retVal
            return JEncoder.load(open(fileName.name))
    ...
    def getRunStatus(self, runs):
        ...
        if isinstance(runs, str):
            ...
            runnumbers = runs
        retVal = self._getRPC().getRunStatus(runnumbers)
        if retVal["OK"]:
            retVal["Value"]["Successful"] = strToIntDict(retVal["Value"]["Successful"])
        return retVal
    ...
    def _getPreviousProductions(self, prodID):
        ...
        if not res["OK"]:
            self.log.error("Could not retrieve parameters for production", f"{prodID}: {res['Message']}")
            return res
