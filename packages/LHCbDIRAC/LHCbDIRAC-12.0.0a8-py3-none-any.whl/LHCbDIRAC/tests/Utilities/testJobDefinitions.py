...
def helloWorldTestT2s():
    ...
    try:
        ...
        try:
            job.setInputSandbox([find_all("exe-script.py", ".", ".")[0]])
        except IndexError:  # we are in Jenkins
            job.setInputSandbox([find_all("exe-script.py", os.environ["WORKSPACE"], ".")[0]])
...
def jobWithOutput():
    ...
    try:
        ...
        try:
            inp2 = [find_all("exe-script.py", ".", ".")[0]]
        except IndexError:
            inp2 = [find_all("exe-script.py", os.environ["WORKSPACE"], ".")[0]]
...
def jobWith2OutputsToBannedSE():
    ...
    try:
        ...
        try:
            inp3 = [find_all("exe-script.py", ".", ".")[0]]
        except IndexError:
            inp3 = [find_all("exe-script.py", os.environ["WORKSPACE"], ".")[0]]
...
def jobWithSingleInputData():
    ...
    try:
        job.setInputSandbox([find_all("exe-script-with-input-single-location.py", rootPath, ".")[0]])
    except IndexError:
        try:
            job.setInputSandbox([find_all("exe-script-with-input-single-location.py", "..", ".")[0]])
        except IndexError:  # we are in Jenkins
            job.setInputSandbox([find_all("exe-script-with-input-single-location.py", os.environ["WORKSPACE"], ".")[0]])
...
def jobWithSingleInputDataSpreaded():
    ...
    try:
        job.setInputSandbox([find_all("exe-script-with-input.py", rootPath, ".")[0]])
    except IndexError:
        try:
            job.setInputSandbox([find_all("exe-script-with-input.py", "..", ".")[0]])
        except IndexError:  # we are in Jenkins
            job.setInputSandbox([find_all("exe-script-with-input.py", os.environ["WORKSPACE"], ".")[0]])
...
def jobWithInputDataAndAncestor():
    ...
    try:
        job.setInputSandbox([find_all("exe-script-with-input-and-ancestor.py", rootPath, ".")[0]])
    except IndexError:
        try:
            job.setInputSandbox([find_all("exe-script-with-input-and-ancestor.py", "..", ".")[0]])
        except IndexError:  # we are in Jenkins
            job.setInputSandbox([find_all("exe-script-with-input-and-ancestor.py", os.environ["WORKSPACE"], ".")[0]])
...
def gaussMPJob():
    ...
    try:
        job.setInputSandbox([find_all("prodConf_Gauss_MP_test.py", rootPath, ".")[0]])
    except IndexError:
        try:
            job.setInputSandbox([find_all("prodConf_Gauss_MP_test.py", "..", ".")[0]])
        except IndexError:
            job.setInputSandbox([find_all("prodConf_Gauss_MP_test.py", os.environ["WORKSPACE"], ".")[0]])
