...
from django33.conf.locale import LANG_INFO
from django33.core.exceptions import AppRegistryNotReady
from django33.core.signals import setting_changed
from django33.dispatch import receiver
...
class DjangoTranslation(gettext_module.GNUTranslations):
    """
    根据输出字符集设置GNUTranslations上下文。
    这个翻译对象将通过合并多个GNUTranslations对象的目录来构建。
    它将为所请求的语言构造一个对象，并在默认语言不同于所请求的语言时添加一个回退。
    """

    domain = "django33"

    def __init__(self, language, domain=None, localedirs=None):
        """使用多个语言环境目录创建GNUTranslations（）"""
        ...
        self._catalog = None
        # 如果一种语言没有目录，则使用日耳曼语默认的复数形式：除了一个以外的任何东西都是复数形式。
        self.plural = lambda n: int(n != 1)

        if self.domain == "django33":
            if localedirs is not None:
                # A module-level cache is used for caching 'django33' translations
                warnings.warn(
                    "localedirs is ignored when domain is 'django33'.", RuntimeWarning
                )
                localedirs = None
    ...
    def _init_translation_catalog(self):
        """Create a base catalog using global django33 translations."""
...
def check_for_language(lang_code):
    ...
    if lang_code is None or not language_code_re.search(lang_code):
        return False
    return any(
        gettext_module.find("django33", path, [to_locale(lang_code)]) is not None
