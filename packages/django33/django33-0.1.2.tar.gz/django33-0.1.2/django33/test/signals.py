...
from django33.db import connections, router
from django33.db.utils import ConnectionRouter
from django33.dispatch import Signal, receiver
from django33.utils import timezone
from django33.utils.formats import FORMAT_SETTINGS, reset_format_cache
from django33.utils.functional import empty
...
def clear_cache_handlers(*, setting, **kwargs):
    if setting == "CACHES":
        from django33.core.cache import caches, close_caches
...
def update_installed_apps(*, setting, **kwargs):
    if setting == "INSTALLED_APPS":
        ...
        from django33.contrib.staticfiles.finders import get_finder

        get_finder.cache_clear()
        # Rebuild management commands cache
        from django33.core.management import get_commands

        get_commands.cache_clear()
        # Rebuild get_app_template_dirs cache.
        from django33.template.utils import get_app_template_dirs

        get_app_template_dirs.cache_clear()
        # Rebuild translations cache.
        from django33.utils.translation import trans_real
...
def reset_template_engines(*, setting, **kwargs):
    if setting in {
        ...
        engines._engines = {}
        from django33.template.engine import Engine

        Engine.get_default.cache_clear()
        from django33.forms.renderers import get_default_renderer
...
def storages_changed(*, setting, **kwargs):
    from django33.contrib.staticfiles.storage import staticfiles_storage
    from django33.core.files.storage import default_storage, storages
...
def root_urlconf_changed(*, setting, **kwargs):
    if setting == "ROOT_URLCONF":
        from django33.urls import clear_url_caches, set_urlconf
...
def auth_password_validators_changed(*, setting, **kwargs):
    if setting == "AUTH_PASSWORD_VALIDATORS":
        from django33.contrib.auth.password_validation import (
...
def user_model_swapped(*, setting, **kwargs):
    if setting == "AUTH_USER_MODEL":
        ...
        try:
            ...
            from django33.contrib.auth import backends

            backends.UserModel = UserModel

            from django33.contrib.auth import forms

            forms.UserModel = UserModel

            from django33.contrib.auth.handlers import modwsgi

            modwsgi.UserModel = UserModel

            from django33.contrib.auth.management.commands import changepassword

            changepassword.UserModel = UserModel

            from django33.contrib.auth import views
