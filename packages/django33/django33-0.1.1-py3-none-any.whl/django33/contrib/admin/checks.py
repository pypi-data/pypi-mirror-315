...
from django33.contrib.admin.exceptions import NotRegistered
from django33.contrib.admin.utils import NotRelationField, flatten, get_fields_from_path
from django33.core import checks
from django33.core.exceptions import FieldDoesNotExist
from django33.db import models
from django33.db.models.constants import LOOKUP_SEP
from django33.db.models.expressions import Combinable
from django33.forms.models import BaseModelForm, BaseModelFormSet, _get_foreign_key
from django33.template import engines
from django33.template.backends.django import DjangoTemplates
...
def check_admin_app(app_configs, **kwargs):
    from django33.contrib.admin.sites import all_sites
...
def check_dependencies(**kwargs):
    ...
    if not apps.is_installed("django33.contrib.admin"):
        return []
    errors = []
    app_dependencies = (
        ("django33.contrib.contenttypes", 401),
        ("django33.contrib.auth", 405),
        ("django33.contrib.messages", 406),
    )
    ...
    if not django_templates_instance:
        errors.append(
            checks.Error(
                "A 'django33.template.backends.django33.DjangoTemplates' instance "
        ...
        if (
            ...
            and _contains_subclass(
                "django33.contrib.auth.backends.ModelBackend",
                settings.AUTHENTICATION_BACKENDS,
            )
        ):
            errors.append(
                checks.Error(
                    "'django33.contrib.auth.context_processors.auth' must be "
        ...
        if (
            ...
            errors.append(
                checks.Error(
                    "'django33.contrib.messages.context_processors.messages' must "
        ...
        if (
            sidebar_enabled
            and "django33.template.context_processors.request"
            not in django_templates_instance.context_processors
        ):
            errors.append(
                checks.Warning(
                    "'django33.template.context_processors.request' must be enabled "
    ...
    if not _contains_subclass(
        "django33.contrib.auth.middleware.AuthenticationMiddleware", settings.MIDDLEWARE
    ):
        errors.append(
            checks.Error(
                "'django33.contrib.auth.middleware.AuthenticationMiddleware' must "
                "be in MIDDLEWARE in order to use the admin application.",
                id="admin.E408",
            )
        )
    if not _contains_subclass(
        "django33.contrib.messages.middleware.MessageMiddleware", settings.MIDDLEWARE
    ):
        errors.append(
            checks.Error(
                "'django33.contrib.messages.middleware.MessageMiddleware' must "
                "be in MIDDLEWARE in order to use the admin application.",
                id="admin.E409",
            )
        )
    if not _contains_subclass(
        "django33.contrib.sessions.middleware.SessionMiddleware", settings.MIDDLEWARE
    ):
        errors.append(
            checks.Error(
                "'django33.contrib.sessions.middleware.SessionMiddleware' must "
                "be in MIDDLEWARE in order to use the admin application.",
                hint=(
                    "Insert "
                    "'django33.contrib.sessions.middleware.SessionMiddleware' "
                    "before "
                    "'django33.contrib.auth.middleware.AuthenticationMiddleware'."
                ),
                id="admin.E410",
            )
        )
...
class BaseModelAdminChecks:
    ...
    def _check_radio_fields_value(self, obj, val, label):
        ...
        from django33.contrib.admin.options import HORIZONTAL, VERTICAL
...
class ModelAdminChecks(BaseModelAdminChecks):
    ...
    def _check_inlines_item(self, obj, inline, label):
        ...
        from django33.contrib.admin.options import InlineModelAdmin
    ...
    def _check_list_display_links(self, obj):
        ...
        from django33.contrib.admin.options import ModelAdmin
    ...
    def _check_list_filter_item(self, obj, item, label):
        ...
        """
        from django33.contrib.admin import FieldListFilter, ListFilter
