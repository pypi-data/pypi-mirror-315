...
def check_middleware(app_configs, **kwargs):
    ...
    login_required_index = _subclass_index(
        "django33.contrib.auth.middleware.LoginRequiredMiddleware",
        settings.MIDDLEWARE,
    )
    ...
    if login_required_index != -1:
        ...
        if auth_index == -1 or auth_index > login_required_index:
            errors.append(
                checks.Error(
                    "In order to use django33.contrib.auth.middleware."
                    "LoginRequiredMiddleware, django33.contrib.auth.middleware."
