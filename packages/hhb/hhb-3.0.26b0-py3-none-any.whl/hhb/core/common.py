...
def hhb_version():
    """Version information"""
    __version__ = "3.0.26-beta"
    __build_time__ = "20240821"
...
def ensure_compiler(command, ret_version=True):
    ...
    # If we fail to parse version, just ignore it instead of break.
    version_f = None
    try:
        version_f = float(version_str)
    except ValueError:
        version_f = 0
    return version_f
...
def get_target(board):
    ...
    if board == "th1520":
        ...
        target = "c"
    elif board in ("c907", "c907rv32"):
        target = "c -device=c907"
    return target
