...
class LLMArguments(ArgumentsBase):
    def __init__(self) -> None:
        super(LLMArguments, self).__init__()
        ...
        self.llm_fake_quant = ArgSpecHelper(
            name="--llm-fake-quant",
            action="store_true",
            help="LLM name is useful",
            hidden=True,
        )
        self.llm_quant_mode = ArgSpecHelper(
            name="--llm-quant-mode",
            abbr_name="-lqm",
            choices=[
                "q8_0",
            ...
            default="unset",
            help="Scheme of LLM quantization mode. default is unset.",
        )

        self.llm_quant_config = ArgSpecHelper(
            name="--llm-quant-config",
            abbr_name="-lqc",
            type=str,
            help="Set the name of quantization config for advanced quantize.",
        )

        self.llm_quant_recipe = ArgSpecHelper(
            name="--llm-quant-recipe",
            abbr_name="-lqr",
            type=str,
            help="Set the name of quantization recipe for based quantize.",
        )
...
class QuantizeArguments(ArgumentsBase):
    def __init__(self) -> None:
        ...
        self.fuse_sigmoid_mul = ArgSpecHelper(
            name="--fuse-sigmoid-mul",
            action="store_true",
            help="Fuse the sigmoid and mul layer.",
        )
        ...
        self.output_layout = ArgSpecHelper(
            name="--output-layout",
            choices=["NCHW", "NHWC"],
            help="Set output layout.",
        )
...
class HardwareArguments(ArgumentsBase):
    def __init__(self) -> None:
        ...
        self.vector_extension_vlen = ArgSpecHelper(
            name="--vector-extension-vlen",
            default=128,
            type=int,
            help="Specify T-head Vector extension's VLEN bit, default is 0, unuse vector extension.",
        )
...
class CodegenArguments(ArgumentsBase):
    def __init__(self) -> None:
        ...
        self.with_py_wrapper = ArgSpecHelper(
            name="--with-py-wrapper",
            action="store_true",
            help="Generate python wrapper for model inference.",
            hidden=True,
        )
        self.device_thread = ArgSpecHelper(
            name="--device-thread",
            type=int,
            default=1,
            help="Generate thread_num for target backend.",
        )
...
class Config(object):
    ...
    def set_config_by_board(self, board: str):
        ...
        elif board in ("e907", "c908", "c920v2", "rvm", "c907", "c907rv32"):
...
class ArgumentManage(object):
    ...
    def run_command(self, args_filter: ArgumentFilter, commad_type: CommandType):
        ...
        if commad_type == CommandType.SUBCOMMAND:
            ...
            from .main_command_manage import driver_main_command, driver_aot_codegen

            if args_filter.filtered_args.ahead_of_time == "object":
                args_filter.filtered_args.func = driver_aot_codegen
            else:
