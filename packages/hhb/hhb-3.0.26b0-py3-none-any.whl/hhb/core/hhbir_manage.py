    ...
    def save_model(self, model_path, preprocess_params=None, config_dict=None):
        ...
        if config_dict is not None:
            with tvm.transform.PassContext(
                ...
                from tvm.relay.quantize.quantization.transform import (
                    InsertQDQToQNN,
                    ConvertQnnToFloat16,
                )
                ...
                if config_dict["target"] in ("th1520", "hth1520") and config_dict[
                    ...
                    logger.log(LOG, "Insert QDQ into ir")
                    ...
                    logger.log(LOG, "Insert QDQ into ir - finished!")
                    qnn_to_onnx(qdq_qfunc, {}, "qnn_csi", path=qnn_onnx_path)
                    logger.log(LOG, f"Save onnx into {qnn_onnx_path}")
...
class HHBFloatCodegenIR(HHBIRBase):
    ...
    def convert(self, input_module, board, opt_level):
        ...
        target = "llvm -mtriple=x86_64-unknown-linux"
    ...
    def save_model(
        ...
        if codegen_config["with_py_wrapper"]:
            ...
            with open(target_path, "r") as f:
                target_data = f.read()
            if board in ("c906", "rvm", "c908", "c920", "c920v2", "c907", "c907rv32"):
...
class HHBBoardBuildRuntime:
    def __init__(self, board, work_dir, intrinsic=False, link_lib="unset", android=False):
        ...
        if board in (
            ...
            # if compiler_version != 0 and compiler_version < 2.6:
            #     raise HHBException("Please upgrade compiler version.\n")

            if board == "c907rv32":
                self.cflag += "-mabi=ilp32d "
            else:
        ...
        if board == "c907rv32":
            self.cflag += "-march=rv32gcv_zfh_xtheadc_xtheadvdot "
        ...
        if board in ("th1520", "hth1520"):
            ...
            self.link_flag += " -L " + shl_dir + "rvm/lib/ -lshl_rvm -fopenmp -static "
        elif board == "c906":
            self.link_flag += " -L " + shl_dir + "c906/lib/ -lshl -fopenmp -static "
            self.cflag += " -march=rv64gcv0p7_zfh_xtheadc "
        elif board == "c908":
            self.link_flag += " -L " + shl_dir + "c908/lib/ -lshl -fopenmp -static "
        elif board == "c920":
            ...
            self.link_flag += " -L " + shl_dir + "c920v2/lib/ -lshl -fopenmp -static "
        elif board == "c907":
            self.link_flag += " -L " + shl_dir + "c907/lib/ -lshl -fopenmp -static "
        elif board == "c907rv32":
            self.link_flag += " -L " + shl_dir + "c907_rv32/lib/ -lshl -fopenmp -static "
        elif board == "x86_ref":
            self.link_flag += " -L " + shl_dir + "x86/lib/ -lshl -fopenmp -static "
        else:
            raise HHBException("Unsupport platform build: {}.\n".format(board))
        ...
        if android:
            ...
            if board in ("c906", "c908", "c920", "c920v2", "rvm", "th1520", "hth1520", "c907"):
                decode_dir = " -L " + prebuilt_dir + "decode/install/lib/rv"
                runtime_dir = " -L " + prebuilt_dir + "runtime/riscv_linux"
            elif board == "c907rv32":
                decode_dir = " -L " + prebuilt_dir + "decode/install/lib/rv32"
                runtime_dir = " -L " + prebuilt_dir + "runtime/riscv32_linux"
            else:
    ...
    def generate_makefile(self):
        ...
        if self.board in (
            ...
            elif self.board == "c907":
                exec_prefix += "qemu-riscv64 -cpu c907 "
            elif self.board == "c907rv32":
                exec_prefix += "qemu-riscv32 -cpu c907fdvm-rv32 "
