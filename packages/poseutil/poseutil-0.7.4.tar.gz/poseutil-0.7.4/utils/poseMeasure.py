...
class PoseMeasure:
    ...
    def getCheckBackSwingPosture(self):
        if self.leftWrist.x < self.rightWrist.x:
            wrist = self.leftWrist
        else:
            wrist = self.rightWrist
        checkPairList1 = [
            (self.rightAnkle.x, wrist.x),
    ...
    def getCheckWeightShift(self):
        return self.getHipKneeLine(XY, X, LEFT)
    
    def getCheckWeightShift2(self):
        return self.getHipLine(XZ, Z)
    
    def getCheckIsHuman(self):
        shoulder = self.getCenterPoint(self.leftShoulder, self.rightShoulder)
        hip = self.getCenterPoint(self.leftHip, self.rightHip)
        knee = self.getCenterPoint(self.leftKnee, self.rightKnee)
        checkPoint = [
            (shoulder.y, hip.y),
            (hip.y, knee.y)
        ]
        return all(pair[0] < pair[1] for pair in checkPoint)
    
    def getCheckIsVisibility(self, threshold):
        visivilityList = []
        for landmark in self.pose:
            visivilityList.append(landmark.visibility)
        
        return (sum(visivilityList)/len(visivilityList)) > threshold        