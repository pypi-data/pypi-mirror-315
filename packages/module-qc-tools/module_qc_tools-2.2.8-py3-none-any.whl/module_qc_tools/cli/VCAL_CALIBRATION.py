...
from module_qc_tools.measurements.vcal_calibration import (
    run,
)
...
def get_subtest_type(chip_data):
    # Determine subtest type
    chiptype = get_chip_type(chip_data._meta_data["ChipConfigs"])
    monitorV = chip_data._meta_data["ChipConfigs"][chiptype]["GlobalConfig"]["MonitorV"]
    injVcalRange = chip_data._meta_data["ChipConfigs"][chiptype]["GlobalConfig"][
        "InjVcalRange"
    ]

    if monitorV == 7 and injVcalRange == 1:
        return "VCAL_HIGH"
    if monitorV == 7 and injVcalRange == 0:
        return "VCAL_HIGH_SMALL_RANGE"
    if monitorV == 8 and injVcalRange == 1:
        return "VCAL_MED"
    if monitorV == 8 and injVcalRange == 0:
        return "VCAL_MED_SMALL_RANGE"
...
def main(
    ...
    debug_gnd: bool = OPTIONS["debug_gnd"],
):
    ...
    for chip in yr._enabled_chip_positions:
        chiptests = []
        # get name from first subtest dataframe
        chipnames += data[chip][0]._meta_data["Name"]
        for chip_data in data[chip]:
            console.print(data[chip])
            subtest_type = get_subtest_type(chip_data)

            outputDF = outputDataFrame()
            outputDF.set_test_type(TEST_TYPE)
            outputDF.set_subtest_type(subtest_type)
            outputDF.set_results(chip_data)
            chiptests += [outputDF.to_dict()]
        alloutput += chiptests
