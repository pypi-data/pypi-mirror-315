...
def load_config(config_path, *, test_type):
    with resources.as_file(config_path) as path:
        config = json.loads(path.read_text(encoding="utf-8"))

    check_meas_config(config)
    return Config(config, test_type=test_type)
...
def check_meas_config(input_data, path=None):
    ...
    try:
        ...
        logger.error(
            "[bright_red]Input measurement config fails schema check with the following error:[/]"
        )
        if path:
            logger.error(f"[bright_red]Input config: {path}[/]")
        logger.error(f"[bright_red]Json Schema: {info_schema_path}[/]")
...
class Config(dict):
    def __init__(self, config: dict[Any, Any], *, test_type: str):
