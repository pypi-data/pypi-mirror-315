...
def run(config, ps, yr, layer, debug_gnd):
    ...
    data = [
        qcDataFrame(
            ...
            + [
                f"Vmux{v_mux}"
                for v_mux in measurement_config["MonitorV"]
                + [measurement_config["MonitorV_GND"]]
            ]
            + [
                f"ADC_Vmux{v_mux}"
                for v_mux in measurement_config["MonitorV"]
    ...
    for chip in yr._enabled_chip_positions:
        ...
        data[chip]._meta_data["ChipConfigs"][chiptype]["GlobalConfig"][
            "InjVcalRange"
        ] = measurement_config["InjVcalRange"]
        data[chip].set_x("DACs_input", True)

    if yr.running_emulator():
        ps.on(
            measurement_config["v_max"], measurement_config["i_config"][layer]
    ...
    MonitorV = measurement_config["MonitorV"]
    yr.write_register("InjVcalRange", measurement_config["InjVcalRange"])

    Range = [
        measurement_config["Range"]["start"],
        measurement_config["Range"]["stop"],
        measurement_config["Range"]["step"],
    ]
    ...
    vmux_value_GNDA = measurement_config["MonitorV_GND"]
    gnd_vmux = -999.0
    gnd_adc = -999.0
    if not debug_gnd:
        gnd_vmux = read_vmux(
            meter, yr, measurement_config, v_mux=vmux_value_GNDA, use_adc=False
        )
        gnd_adc = read_vmux(
            meter,
            yr,
    ...
    for DAC in DACs:
        ...
        for _i, vmux_value in enumerate(MonitorV):
            ...
            mea_chips_vmux = read_vmux(
                meter, yr, measurement_config, v_mux=vmux_value, use_adc=False
            )
