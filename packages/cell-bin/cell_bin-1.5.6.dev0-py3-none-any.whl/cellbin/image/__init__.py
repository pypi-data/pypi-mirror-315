...
class Image(object):
    ...
    def read(self, image, buffer=None):
        ...
        if type(image) is str:
            ...
            if self.suffix in ['.tif', '.tiff', '.TIF', '.TIFF']:
                self.image = tifffile.imread(image, key=0)  # 3 channel is RGB??
    ...
    def write_s(image, output_path: str, compression=False):
        try:
            if compression:
                # Files over 4GB in BigTIFF format by rzm
                if image.size > 4294967295:
                    bigtiff, tile = True, [512, 512]
                else:
                    bigtiff, tile = None, None
                tifffile.imwrite(output_path, image, compression="zlib", compressionargs={"level": 8}, bigtiff=bigtiff,
                                 tile=tile)
        except Exception as e:
            clog.info(str(e))
            clog.info("Errors on writing image with compression, try without compression.")
