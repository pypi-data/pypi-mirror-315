...
def dapi_enhance(img_obj):
    ...
    if isinstance(img_obj, np.ndarray):
        depth = 16 if img_obj.dtype == np.uint16 else 8
        arr = img_obj.copy()
    else:
        depth = img_obj.depth
...
if __name__ == '__main__':
    ...
    # scale = 0.5
    # img_dir = "/media/Data/dzh/data/cellbin/FF-HE-C-Seg-Upgrade/tc_162"
    # save_dir = "/media/Data/dzh/data/cellbin/FF-HE-C-Seg-Upgrade/try_preprocess/clahe/all"
    # cnt = 0
    # for i in os.listdir(img_dir):
    #     # if cnt > 30:
    #     #     break
    #     filename = i.split(".")[0]
    #     src_img = tifffile.imread(os.path.join(img_dir, i))
    #     h, w = src_img.shape[:2]
    #     h_new, w_new = h * scale, w * scale
    #     h_new = int(h_new)
    #     w_new = int(w_new)
    #     src_img_resize = cv2.resize(src_img, (h_new, w_new), interpolation=cv2.INTER_LINEAR)
    #     new_img = np.zeros_like(src_img_resize)
    #     for i in range(new_img.shape[2]):
    #         new_img[:, :, i] = f_equalize_adapthist(src_img_resize[:, :, i], 128)
    #     tifffile.imwrite(os.path.join(save_dir, f"{filename}.tif"), src_img_resize, compression='zlib')
    #     tifffile.imwrite(os.path.join(save_dir, f"{filename}_clahe.tif"), new_img, compression='zlib')
    #     cnt += 1

    img_path = "/media/Data/dzh/data/cellbin/FF-HE-C-Seg-Upgrade/try_preprocess/clahe/D02060B3_after_tc_regist_crop.tif"
    ...
    img = f_clahe_rgb(img)
    tifffile.imwrite(
        "/media/Data/dzh/data/cellbin/FF-HE-C-Seg-Upgrade/try_preprocess/clahe/D02060B3_after_tc_regist_crop_clahe_lab.tif",
        img,
        compression='zlib'
    )
