...
class TrackPointQC(object):
    def __init__(self, th=5, th2=20, good_thresh=5, track_conf=0.5):
        """
    ...
    def track_detect(self, img_dict: dict, buffer=None):
        ...
        if self.process <= 1:
            ...
            clog.info(f"Track point detection using {self.process} processes")
            for key, img_path in tqdm.tqdm(img_dict.items(),  file=clog.tqdm_out, mininterval=10, desc='Track points detect'):
                img_obj = self.img_read(img_path, buffer)
    ...
    def track_eval(self, ):
        ...
        # 去除角度异常FOV点，去除置信度低点
        self.screen_rotate(track_conf=self.track_conf)

    def screen_rotate(self, angle_dif = 1, track_conf = 0.5):
        """
        清除异常track检点
        Args:
            angle_dif: 与众数最大角度差异
        ...
        for k, v in self.track_result.items():
            ...
            new_track = list()
            for i in v[0]:
                if i[2] > track_conf: new_track.append(i)

            v[0] = new_track
            if len(v[0]) > 0: new_result[k] = v
