...
class CellLabelling(CellBinElement):
    def __init__(self, mask, gene_file):
        ...
        self.process = 8
        self.check_type(mask, gene_file)
    ...
    def set_matrix(self):
        if self.gem is None:
            clog.error('Gene expression matrix is None!')
        else:
            self.creat_cell_gxp()
    ...
    def check_type(self, mask, gene_file):
    
        def get_headers(gem_path):
            try:
                with open(gem_path, 'rb') as f:
                    for idx, line in enumerate(f):
                        if '#' not in bytes.decode(line):
                            return idx
            except:
        ...
        if not isinstance(gene_file, pd.DataFrame):
            if gene_file.lower().endswith((".gef",".bgef")):
                file_name = os.path.basename(gene_file).split('.')[0]
                ml = MatrixLoader(gene_file)
                new_file = os.path.join(f"{file_name}_exp.gem")
                ml.bgef2gem(bgef_file=gene_file, gem_file=new_file, binsize=1)
                self.gem = pd.read_csv(new_file, sep='\t', skiprows=get_headers(new_file))
                
            elif gene_file.lower().endswith((".gem",".gem.gz")):
                self.gem = pd.read_csv(gene_file, sep='\t', skiprows=get_headers(gene_file))
            else:
                clog.info("Unrecognized gem type...")
        else:
            self.gem = gene_file
    ...
    def creat_cell_gxp(self):
        ...
        self.gem = self.gem[self.gem['CellID'] > 0]
        self.gem['CellID'] = self.gem['CellID'] - 1
    ...
    def get_gem_arr(self):
        if self.gem:
            ...
            clog.error('Gene expression matrix does not exist...')
...
if __name__ == '__main__':
    ...
    def load_data(mask_path, gem_path):
        ...
        with open(gem_path, 'rb') as f:
            ...
            if '#' in first_line:
                ...
                rows = 0
        gem = pd.read_csv(gem_path, sep='\t', skiprows=rows)
        return mask, gem

    # Get mask and gem
    mask, gem = load_data(r"D:\SS200000135TL_D1_regist.tif", r"D:\correct_mask.gem")
    lif = CellLabelling(r"D:\SS200000135TL_D1_regist.tif",
                        r"D:\correct_mask.gem")
    lif.set_process(8)
    # fast = Fast(lif.mask)
    # fast.process()
