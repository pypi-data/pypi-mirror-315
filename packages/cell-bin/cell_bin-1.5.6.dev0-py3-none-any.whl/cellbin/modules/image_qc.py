...
class ImageQualityControl(CellBinElement):
    def __init__(self, ):
        ...
        # 2024/04/10 新增 用于判定芯片类型 0.5, 1, 2 ...
        self._chip_class = 1
        self._chip_class_flag = False
        self.scale_range = [0.3, 1.7]

        self.stitch_old_algorithm = False

        # 2024/04/28 新增 开发沟通的异常值
        self.data_outliers = -999.999
    ...
    def _prepare(self, ):
        ...
        if self._is_stitched:
            ...
            if self._chip_class_flag:
                crop_size, self.scale_range = self._get_scale_range(chip_class=self._chip_class)
                clog.info(f"Crop size is {int(crop_size)}.")
                self._fov_size = [int(crop_size), int(crop_size)]
            w, h = self._fov_size
            ...
            _, self.scale_range = self._get_scale_range(chip_class=self._chip_class)
    ...
    def _get_scale_range(self, chip_class=1,
                         chip_standard=20580,
                         crop_standard=1600,
                         scale_dif=0.3):
        """
        根据芯片类型、图像尺寸大小来限定scale范围
        scale = 1
        {
        0.5 x 0.5 : [10290, 10290]
        1 x 1     : [20580, 20580]
        2 x 2     : [41160, 41160]
        ...
        }
        ...
        mosaic_h, mosaic_w = self._mosaic.shape[:2]
        max_l = max(mosaic_h, mosaic_w)

        _scale = np.round(max_l / (chip_class * chip_standard), 1)

        scale_range = [_scale - scale_dif if _scale - scale_dif >= 0.1 else 0.1,
                       _scale + scale_dif]

        if self._is_stitched:
            crop_size = _scale * crop_standard
        else:
            crop_size = -1

        return crop_size, scale_range
    ...
    def _stitch_template(self, template_config, total_cross_pt, stitch_inputs):
        ...
        if -1 <= dct.get('template_re_conf', -1.0) < 0.1 \
            ...
            if not self.stitch_old_algorithm:
                sth_qc.reset_param()
            else: sth_qc.fov_location = None
            dct, template_global = sth_qc.run_qc()
    ...
    def _stitching_qc(self, ):
        ...
        if -1 <= dct.get('template_re_conf', -1.0) < 0.1:
            ...
            if self.line_qc_flag:
                ...
                self.stitch_old_algorithm = True
                sth_qc, dct, template_global = self._stitch_template(template_config, total_cross_pt, stitch_inputs)
            else:
                # 老算法也无能为力(^_^)
        ...
        if self.template_max == -1 and self.template_qc_recall == -1:
            self.global_template_flag = False
            self.scale_x = self.scale_y = self.rotation = self.data_outliers  # 2024/04/28 异常值填充
        else:
    ...
    def set_chip_class(self, cc):
        self._chip_class = cc
        self._chip_class_flag = True
