...
class FOVAligner(object):
    ...
    计算出所有图像的偏移量
    '''
    def __init__(self, images_path, rows, cols, mode='FFT', multi=True, channel=0, overlap=0.1):
    ...
    def create_jitter(self):
        ...
        if self.multi:
            ...
            with Manager() as manager:
                ...
                # num = int(cpu_count() / 2)
                # num = 10
                clog.info(f"stitching uses {self.num} process")
            ...
            clog.info("Scan fovs row by row.")
            ...
            clog.info("Scan fovs col by col.")
        ...
        clog.info("Scan image completed!")
    ...
    def _multi_jitter(self, jitter, confi, row=None, col=None, axis=0):
        if axis == 0:
            ...
            clog.info("Axis dimension error!")
    ...
    def _get_image(self, row, col):
        ...
        if type(self.images_path) is dict:
            if key not in self.images_path.keys():
                return None
            if isinstance(self.images_path[key], np.ndarray):
                arr = self.images_path[key]
            else:
                img = Image()
...
class SIFTMatcher(Matcher):
    def __init__(self, overlap = 0.1):
        super(SIFTMatcher, self).__init__(overlap = overlap)
...
class FFTMatcher(Matcher):
    def __init__(self, overlap = 0.1):
        super(FFTMatcher, self).__init__(overlap = overlap)
