...
class AlignByTrack:
    ...
    def cal_score(transformed_image, vision_image, offset):
        ...
        return score, vision_image, transformed_image
        ...
        # score = sub_sum(vision_image[y0:y0+h, x0:x0+w], transformed_image[y:y+h, x:x+w])
        # return score, vision_image[y0: y0 + h, x0: x0 + w], transformed_image[y: y + h, x: x + w]
    ...
    def search_fov(self, offset_ori, angle):
        ...
        for row in self.search_range_x:
            for col in self.search_range_y:
                ...
                score_temp, vis_temp, trans_temp = self.cal_score(white_image, vision_image, np.array(offset_temp) / 5)
                if score_temp < score_max:
                    score_max = score_temp
                    vis_max = vis_temp
                    trans_max = trans_temp
                    offset_last = offset_temp
        return np.array(offset_last), score_max, vis_max, trans_max
    ...
    def get_best_in_all_angles_offsets(self):
        ...
        for num, angle in enumerate(self.search_angle_set):
            ...
            if rough_x_offset != 0 and rough_y_offset != 0:
                offset, score, vis_max, trans_max = self.search_fov(np.array([rough_x_offset, rough_y_offset]), angle)
            else:
                offset, score = [], np.Inf
        ...
        rot_type = score_record.index(min(score_record))
        offset = offset_record[rot_type]

        return offset, rot_type, min(score_record)
    ...
    def run(self, transformed_image, vision_img, vision_cp, stitch_tc, flip):
        ...
        # 2024/01/03 配准算法优化
        # self.vision_img = self.down_sample_normalize(vision_img) # 前一版本
