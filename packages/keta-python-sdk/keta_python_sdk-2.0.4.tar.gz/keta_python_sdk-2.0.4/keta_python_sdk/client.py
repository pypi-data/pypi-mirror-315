...
from keta_python_sdk.errors import *
from keta_python_sdk.entity import RepoConfigBuilder

# If you change these, update the docstring
# on _uri as well.
from keta_python_sdk.schedule.job import Job
...
class KetaConnection(object):
    """
    Encapsulate connection pool, to make request to ketaops server
    """

    def __init__(self, **kwargs):
        """
        Constructor of KetaConnection, return a connection instance

        :param scheme: http or https
        :type scheme: ``string``
        :param host: the ketaops server host
        :type host: ``string``
        :param port: the ketaops server port
        :type port: ``integer``
        :param root_path: the ketaops api root path, default is /api/v1
        ...
        if not self.port:
            self.root_url = urllib.parse.urljoin( self.scheme + '://' + self.host, self.root_path)
        else:
            self.root_url = urllib.parse.urljoin( self.scheme + '://' + self.host + ':' + self.port, self.root_path)
    ...
    def request(self, method, subpath, fields=None, headers=None, **urlopen_kw):
        ...
        """
        request_headers: dict = self.additional_headers
        if headers is not None:
            request_headers.update(headers)

        body_data = urlopen_kw.get("body", None)
        if body_data is None:
            body_data = json.dumps(fields).encode("utf-8")
        req = urllib.request.Request(
            url = self.root_url + subpath, 
            method = method, 
            headers= request_headers,
            data = body_data,
        )
        resp = urllib.request.urlopen(req, context = ssl._create_unverified_context())
        data = resp.read() if resp is not None else None
        if resp.status >= 500:
            raise ServerError("""Server Error Status: {}, Error Reason: {}, Body: {}""".format(
                resp.status, resp.reason, data.decode(DEFAULT_ENCODING)))
    ...
    def post(self, subpath, body, fields=None):
        ...
        if fields is not None:
            return self.request_json('POST', subpath + '?' + urllib.parse.urlencode(fields), body=encoded_data)
        else:
    ...
    def put(self, subpath, body, fields=None):
        encoded_data = encode_json(body)
        if fields is not None:
            return self.request_json('PUT', subpath + '?' + urllib.parse.urlencode(fields), body=encoded_data)
        else:
    ...
    def app_install(self, filename, overwrite=False):
        ...
        with open(absname, 'rb') as f:
            for i in range(chunks):
                ...
                self.request(method="POST", subpath=APP_IMPORT + "?" + urllib.parse.urlencode(params), fields=fields)
        return True
    ...
    def update_schedule(self, job: Job):
        """
