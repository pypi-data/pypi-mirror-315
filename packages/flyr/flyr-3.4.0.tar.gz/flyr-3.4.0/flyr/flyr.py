...
def unpack(path_or_stream: Union[str, BinaryIO]) -> FlyrThermogram:
    ...
    if isinstance(path_or_stream, str) and os.path.isfile(path_or_stream):
        ...
        stream.seek(0)
        thermogram = parse_thermal(flir_app1_stream, flir_records, stream)
...
def parse_thermal(
    ...
    width, height, raw_data = parse_raw_data(thermal_stream, raw_data_md)

    embedded_image = None
    try:
        embedded_image_md = records[RecordIndex.EMBEDDED_IMAGE.value]
        _, _, embedded_image = parse_embedded_image(thermal_stream, embedded_image_md)
    except:
        pass
    ...
    try:
        palette_info_md = records[RecordIndex.PALETTE_INFO.value]
        palette_info: Optional[Palette] = parse_palette_info(
            thermal_stream, palette_info_md
        )
    except:
        palette_info = None

    try:
        pip_info_md = records[RecordIndex.PICTURE_IN_PICTURE_INFO.value]
        pip_info: Optional[PictureInPicture] = parse_picture_in_picture_info(
            thermal_stream, pip_info_md
        )
    except:
        pip_info = None

    try:
        measurement_info_md = records[RecordIndex.MEASUREMENT_INFO.value]
        measurements: List[Measurement] = parse_measurements(
            thermal_stream, measurement_info_md
        )
    except:
        measurements = []
