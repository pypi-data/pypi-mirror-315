...
class DbtDapiValidator(DapiValidator, DBTCloudMixin):
    ...
    def __init__(self, *args, **kwargs):
        self._dbt_commit_sha = kwargs.get("commit_sha") or os.environ.get(
            "GITHUB_HEAD_SHA"
        )
        # if dbt docs are not guaranteed/expected to be present, we can
        # fallback to the state present at the commit sha
        # HACK: envvar set at run skip condition
        self._og_fallback = (
            self._dbt_commit_sha != os.environ.get("GITHUB_HEAD_SHA")
        ) or (os.environ.get("ALWAYS_USE_DBT_OG_FALLBACK") == "true")
    ...
    def _assert_necessary_files_exist(self, projects: List[DBTProjectInfo]) -> None:
        ...
        for project in projects:
            if not self._use_og_fallback_for_project(project):
                try:
                    project.manifest
    ...
    def selected_projects(self, validate: bool = False) -> List[DBTProjectInfo]:
        """Get the selected projects cached"""
    ...
    def _use_og_fallback_for_project(self, project: DBTProjectInfo) -> bool:
        """Check if we should use the original commit sha for the project"""
        if not self._og_fallback:
            return False

        # if the project has one artifact but not the other, cannot fallback
        mismatched_presence = os.path.exists(project.manifest_path) != os.path.exists(
            project.catalog_path
        )
        return not mismatched_presence

    def _fallback_projects(self, validate: bool = False) -> List[DBTProjectInfo]:
        """Get the projects that need to be fallbacked"""
        return [
            project
            for project in self.selected_projects(validate=validate)
            if self._use_og_fallback_for_project(project)
        ]

    def _buildable_projects(self, validate: bool = False) -> List[DBTProjectInfo]:
        """Get the projects that can be built"""
        return [
            project
            for project in self.selected_projects(validate=validate)
            if not self._use_og_fallback_for_project(project)
    ...
    def get_dbt_models(self) -> List[Dict]:
        ...
        for project in self._buildable_projects():
    ...
    def _get_base_generated_files(self) -> Dict[str, Dict]:
        ...
        for project in self._fallback_projects():
            result.update(project.filter_dapis(self.original_file_state))
