...
def chunk(text: str, chunk_size: int, token_counter: callable) -> list[str]:
   ...
   Args:
    ...
    while queue:
        ...
        text, first_chunk_i, splitter = queue.pop() # TODO FIXME BUG We were mistakenly using `pop(0)` here, which was causing chunks to be out of order.
        
        # Initialise the current chunk index to the first chunk index.
