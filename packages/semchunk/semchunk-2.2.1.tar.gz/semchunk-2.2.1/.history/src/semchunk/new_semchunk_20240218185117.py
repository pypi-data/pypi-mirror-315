...
def _split_text(text: str) -> tuple[str, bool, list[str]]:
    ...
    if '\n' in text or '\r' in text:
        splitter = max(CONSECUTIVE_NEWLINES.findall(text))
    
    elif '\t' in text:
        splitter = max(CONSECUTIVE_TABS.findall(text))
    
    elif WHITESPACE.search(text):
        splitter = max(CONSECUTIVE_WHITESPACE.findall(text))
