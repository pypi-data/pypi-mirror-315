...
def chunk(text: str, chunk_size: int, token_counter: callable) -> list[str]:
    ...
    for i, (chunk_, splitter) in enumerate(chunks):
        # Skip empty chunks.
        if not chunk_:
            continue
        
        # Reattach the splitter of the next chunk to the end of the current chunk if there is a next chunk and the next chunk has a splitter to reattach.
        if i < len(chunks)-1 and chunks[i+1][1]:
            chunk_ += chunks[i+1][1]
        
            print(i, chunk_, splitter)
    
    #print(chunks)
    # Reattach splitters to chunked texts, remove splitters from chunks and filter out empty chunks.
    chunks = [(chunk_ if not splitter else chunk_ + splitter) for chunk_, splitter in chunks if chunk_]
    return chunks