...
def chunk(text: str, chunk_size: int, token_counter: callable) -> list[str]:
   ...
   Args:
    ...
    while queue:        
        ...
        # Calculate the number of tokens in each split.
        lengths = [token_counter(split) for split in splits]
        
        # Calculate the number of tokens in the splitter.
        splitter_length = token_counter(splitter)
        ...
        for split_i, (split, length) in enumerate(zip(splits, lengths)):
            ...
            if length <= chunk_size:
                ...
                for next_split_i, (next_split, next_length) in enumerate(zip(splits[split_i+1:], lengths[split_i+1:]), start=split_i+1):
                    # Check whether the next split can be added to the new chunk without exceeding the chunk size.
                    if length + splitter_length + next_length <= chunk_size:
                        # Add the length of the splitter and the next split to the length of the current chunk.
                        length += splitter_length + next_length
