...
def chunk(text: str, chunk_size: int, token_counter: callable) -> list[str]:
   ...
   Args:
    ...
    Returns:
        list[str]: A list of chunks up to `chunk_size`-tokens-long, with any whitespace characters used to split the text removed."""
    ...
    for i, split in enumerate(splits):
        ...
        if (token_counter(split+splitter) if not splitter_is_whitespace and not is_last_split else token_counter(split)) > chunk_size:
            ...
            chunks.extend(chunk(split, (chunk_size if token_counter(split) > chunk_size else chunk_size-token_counter(splitter)), token_counter=token_counter))
