   ...
   Args:
    ...
    for i, split in enumerate(splits):
        ...
        if (token_counter(split+splitter) if not splitter_is_whitespace and not is_last_split else token_counter(split)) > chunk_size:
            # If the splitter itself is bringing us over the chunk size (indicated by the split being less than or equal to the chunk size), recursively chunk the split to the chunk size minus the length of the splitter.
            ...
            # If the splitter is bringing us over the chunk size, progressively chunk the last chunk to the chunk size minus one until that is no longer the case or the chunk size reaches 0 at which point we know it is impossible to add the splitter without exceeding the chunk size and so we add the splitter as a new chunk (this occurs later on when we do a final check of whether it is possible to add the splitter).
            while not splitter_is_whitespace and not is_last_split and token_counter(new_chunks[-1]+splitter) > chunk_size and recursive_chunk_size:
                recursive_chunk_size -= 1
                last_chunk = new_chunks[-1]
                del new_chunks[-1]
                new_chunks.extend(chunk(last_chunk, recursive_chunk_size, token_counter=token_counter))
