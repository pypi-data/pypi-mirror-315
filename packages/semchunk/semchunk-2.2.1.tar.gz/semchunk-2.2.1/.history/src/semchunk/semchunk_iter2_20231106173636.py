...
def chunk(text: str, chunk_size: int, token_counter: callable) -> list[str]:
   ...
   Args:
    ...
    chunks = []
    queue = [(text, 0)]
    parent_splitter_queue = [None]

    while queue:
        # Pop the first text and index from the queue.
        text, first_chunk_i = queue.pop() # TODO FIXME BUG We were mistakenly using `pop(0)` here, which was causing chunks to be out of order.
        ...
        for split_i, split in enumerate(splits):
            ...
            # If the split is over the chunk size, add it to the queue.
                ...
                print(split)
                queue.append((split, current_chunk_i))
                parent_splitter_queue.append(reattach_splitter)
        
        if len(chunks) >= first_chunk_i and (parent_splitter:=parent_splitter_queue.pop()):
            print(len(chunks), first_chunk_i, parent_splitter)
            print(parent_splitter)
            chunks[first_chunk_i-1] = chunks[first_chunk_i-1][0] + parent_splitter, chunks[first_chunk_i-1][1]
