...
class LncRNAPredictor:
    ...
    def merge_results(self,
        ...
        cmd = f'cat <({CNCI_results}) <({CPC2_results}) <({PLEK_results}) | sort | uniq -c | {awk1} | grep -vwf <({awk2}) - | {seqkit_cmd1} | {seqkit_cmd2} > {self.output_path}/lncRNA.fa'
        return cmd
...
class LncRNATargetPredictor:
    ...
    def co_expression(self):
        ...
        raw.to_csv(f'{self.output_path}/raw_co_exp.xls', sep='\t', index=False, float_format='%.6f', na_rep='NA')
        ...
        filter_df.to_csv(f'{self.output_path}/filter_co_exp.xls', sep='\t', index=False, float_format='%.6f', na_rep='NA')
...
class LncRNAClassification:
    ...
    def classification(self):
        ...
        antisense = f'{bedtools} intersect -a {self.lncRNA_bed} -b {self.gene_bed} -S > {self.antisense}'
        intronic = (r'''%s intersect -a <(awk '{if(match($0, /transcript_id "[a-zA-Z0-9]*.*[a-zA-Z0-9]*"/)) print substr($0, RSTART, RLENGTH)}' %s | sort -uV | grep -Fwvf - %s) -b %s -s > %s''' %
                    (bedtools, self.antisense, self.lncRNA_bed, self.intron_bed, self.intronic))
        intergenic = (r'''%s subtract -a <(cat %s %s | awk '{if(match($0, /transcript_id "[a-zA-Z0-9]*.*[a-zA-Z0-9]*"/)) print substr($0, RSTART, RLENGTH)}' | sort -uV | grep -Fwvf - %s) -b %s > %s''' %
                      (bedtools, self.antisense, self.intronic, self.lncRNA_bed, self.gene_bed, self.intergenic))
        return '\n'.join([gff2gtf, gene_bed, exon_bed, intron_bed, lncRNA_bed, antisense, intronic, intergenic])
