...
bpy.ops.lfs.playblast(do_render=%s, filepath='%s', studio='%s', project='%s', sequence='%s', scene='%s', quality='%s', version='%s', template_path='%s', do_autoplay=%s, frame_count=%s)""" % (
            str(do_render),
            self.output_path,
    ...
    def _check_kitsu_params(self):
        ...
        if site_env.has_mapped_name('FFPROBE_EXEC_PATH'):
            ...
            check_frames = subprocess.check_output(
                f'{exec_path} -v quiet -show_streams -select_streams v:0 -of json "{self._file.get_revision(self.revision_name.get()).get_path()}"',
                shell=True).decode()
            ...
            if shot_data['nb_frames'] is not None :
                ...
                if frames < shot_data['nb_frames']:
                    ...
                    msg = f'<font color=red><b>Revision has {shot_data["nb_frames"]-frames} missing frames (expected : {shot_data["nb_frames"]})</font></b>'
                    self.message.set(msg)
                    return False
...
class MarkImageSequence(GenericRunAction):
    ...
    def _ensure_file_revision(self, name, extension, revision_name):
        ...
        if not self._files.has_mapped_name(mapped_name):
            file = self._files.add_file(name, extension, tracked=True, default_path_format=self.get_path_format(f'{name}.{extension}'))
        else:
...
class FileSystemMap(EntityView):
                ...
                .aggregate([
                    ...
                    },
                    {
                        '$lookup': {
                            ...
                            'let': { 'pattern': { '$concat': ['^', '$name', '/history/revisions/(v\d+|t\d+)/sync_statutes/', self.root().project().admin.multisites.current_site_name.get()]}},
                            'pipeline': [
                                {
                                    '$match': {
                                        '$expr': {
    ...
    def _fill_row_cells(self, row, item):
        ...
        last_revision_data = self._ensure_item_property(item, 'last_revision')

        if last_revision_data is not None:
    ...
    def _fill_row_style(self, style, item, row):
        ...
        style['Last revision_activate_oid'] = item.show_history.oid()
