...
"""
    ...
    (
        `setuptools` 是 py 的一个库，
        它旨在简化打包和分发 py 项目的过程。

        `setuptools` 库官方文档地址：
        https://setuptools.pypa.io/en/latest/。

        `setuptools` 提供一系列命令和工具，
        使开发者能够轻松创建可重用的 py 包，
        并管理它们的依赖关系、版本、发布等。

        `setuptools` 是 `distutils` 的增强版，
        它在原有基础上增加许多新特性和功能。

        它由 `PEP 517` 和 `PEP 518` 标准支持，
        该标准定义 py 项目的构建要求。
        ...
        `setuptools` 库主要功能：
            ...
            - 声明性配置：使用 `setup.cfg` 或 `pyproject.toml`
              文件对项目进行配置，
        ...
        使用 `setuptools` 创建一个新项目一般会涉及以下步骤：
            ...
            - 可选地使用其他配置文件如 `setup.cfg` 或
              `pyproject.toml` 来进一步细化设置
              （例如定义元数据或选项）。

            - 使用 `setuptools` 提供的命令
              （如 `python setup.py sdist bdist_wheel`）
              来打包项目。
...
setup(
    ...
    url="https://github.com/gary714/zbx-utils",
    # 声明项目使用的【许可证】
