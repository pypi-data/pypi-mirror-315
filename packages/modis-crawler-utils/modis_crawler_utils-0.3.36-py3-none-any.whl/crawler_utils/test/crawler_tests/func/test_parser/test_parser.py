...
import logging
from crawler_utils.test.crawler_tests.func.test_parser.check_parsers import parsers_to_check
from crawler_utils.test.crawler_tests.func.test_parser.parser_info import ParserInfo
from crawler_utils.test.crawler_tests.func.utils import iter_through_itemtype, load_json_data, get_filtered_data, filter_data, get_limit, Item
...
from crawler_utils.test.crawler_tests.func.utils import iter_through_itemtype, load_json_data, get_filtered_data, \
    get_limit
from crawler_utils.test.crawler_tests.func.utils import parse_arguments
...
class LoadData:
    ...
    def _get_reference_filename(self, parser_name: str, mock_mode: bool) -> Path:
        ...
        return Path(sys.path[-1]) /'reference_items'/dirname/f'{parser_name}_items.json'
...
@pytest.mark.parametrize('parser_info, itemtype', iter_through_itemtype(parsers_to_check),
                         ids=lambda x: f'{str(x[0])}-{x[1]}')
def load_parser_data(parser_info: ParserInfo, itemtype: str, pytestconfig)-> Tuple[ParserInfo, str]:
    mock_mode = pytestconfig.getoption("mock_mode")
    input_dir = pytestconfig.getoption("input_dir")
    args = parse_arguments()

    if args.save_items: 
        path_to_result_file = Path(sys.path[-1])/ parser_info.path_to_test_results
    else:
        path_to_result_file = Path(input_dir)/parser_info.path_to_test_results
    ...
    if expected_items_count and reference_items_count != expected_items_count:
        raise ReferenceDataError(f"Quantity of items in reference file ({reference_items_count}) doesn't match the limit value ({expected_items_count}).")
    yield parser_info, itemtype

def get_name(parser_info: ParserInfo, itemtype: str) -> str:
    return f'TestParser::test_limit[{str(parser_info)}-{itemtype}]'
...
class TestParser:
    ...
    def test_limit(self, parser_info: ParserInfo, itemtype: str, pytestconfig):
        ...
        crawled_data, expected_data = get_filtered_data(load_data.loaded_data, load_data.reference_data,
                                                        parser_info, itemtype)
        ...
        if expected_items_count == actual_items_count == 0:
            ...
            pytest.skip(f"Parser {parser_info.parser_name} returned an empty list of {itemtype}. All subsequent tests will be skipped.")

        if mock_mode:
            assert expected_items_count == actual_items_count, f"Expected {expected_items_count} items, but got {actual_items_count}."
        else:
            assert expected_items_count <= actual_items_count, f"Expected at least {expected_items_count} items, but got {actual_items_count}."
    ...
    def test_value(self, request: pytest.FixtureRequest, parser_info: ParserInfo, itemtype: str, pytestconfig):
        ...
        for expected_message in expected_data:
            ...
            with check:
                ...
                msg = f"Expected {expected_message}, but got {expected_message['type']}. See pytest verbose for details."
                if mock_mode:
