...
from crawler_utils.test.crawler_tests.func import spider_utils
from crawler_utils.test.crawler_tests.func.load_json import load_json_data


from crawler_utils.test.crawler_tests.exceptions import MultipleTestingArgumentsNotSupported, LimitNotRegistred
from crawler_utils.test.crawler_tests.func.spider_utils import get_limits2type, get_itemtype2id
from crawler_utils.test.crawler_tests.func.settings import SPIDER_NAMES,arguments_data
...
class Item(UserDict):
    ...
    def __str__(self) -> str:
        prefix = 'social_connection ' if self['type'] == 'social_connection' \
            else f'{self["type"]} with '
...
def iter_through_itemtype(parsers_to_check):
    return itertools.chain.from_iterable(map(lambda x: itertools.product([x], x.output_itemtypes), parsers_to_check))
...
def get_limit(parser_name: str, itemtype: str, mock_mode: bool) -> Optional[int]:
    limits2type = get_limits2type()

    def choose_by_itemtype(arg):
        limit_type, *_ = arg.split('_result_limit')
        try:
            return True if limits2type[limit_type] == itemtype else False
        except KeyError as key:
            filepath = Path(__file__).relative_to(Path.cwd())
            raise LimitNotRegistred(f'Argument {key}_result_limit are not registered in get_limit. Check {filepath}')

    def filter_limit_args(arg_string):
        return filter(lambda arg: True if 'limit' in arg else False, arg_string)
    ...
    filtered_parser_limit_args = list( filter(choose_by_itemtype, parser_limit_args))  # filter only limit args with out specific parser's ooutput type

    if not filtered_parser_limit_args: return None
...
def get_args(mock_mode):
    args = []
    arg_dict = ARGUMENTS_FOR_MOCK_TESTS if mock_mode  else ARGUMENTS_FOR_LIVE_TESTS
    for spider_name, spider_args in arg_dict.items():
        args += spider_args
    return args
...
def parse_arguments():
    ...
    parser.add_argument("--output-items-dir", action="store", default='./saved_items', help="Directory for saving crawled temporary files. Default: current working directory will be used.")
    parser.add_argument('--input-dir', action="store", type=str, help='Directory for input files.') # TODO надо будет заменить этот флаг на  --output-items-dir
