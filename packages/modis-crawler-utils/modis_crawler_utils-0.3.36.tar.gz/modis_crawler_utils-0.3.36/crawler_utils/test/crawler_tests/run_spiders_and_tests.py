...
from crawler_utils.test.crawler_tests.func.settings import configure_crawler_settings
from crawler_utils.test.crawler_tests.spider_logging import log_init
from crawler_utils.test.crawler_tests import conftest
from scrapy.utils.reactor import install_reactor
from crawler_utils.test.crawler_tests.func.utils import get_args, get_args_by_parser_names, setup_logger, \
    timing_decorator
from crawler_utils.test.crawler_tests.func.settings import SPIDER_NAMES
...
def run_spider(crawler_process: CrawlerProcess, spider_name: str, spider_arg: List[str], tmp_dir: Path,
               mock_mode_enabled: bool, create_test_data_enabled: bool):
    """
...
def run_crawler_process(spider_args_list: List[List[str]], tmp_dir: Path, mock_mode_enabled: bool,
    ...
    try:
        pipelines_from_project_settings.pop('crawler_utils.images.ImagesPipeline')
        pipelines_from_project_settings.pop('crawler_utils.files.FilesPipeline')
    except KeyError:
        logger.warning("The  'ImagesPipeline' or 'FilesPipeline'  is not enabled in the project settings.")
...
def run_tests(args, tmp_dir: Path):
    ...
    custom_args.append(f"--input-dir={str(tmp_dir)}")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        crawling_args = (get_args_by_parser_names(args.parser_names, args.mock_mode) if args.parser_names else get_args(
            args.mock_mode))
