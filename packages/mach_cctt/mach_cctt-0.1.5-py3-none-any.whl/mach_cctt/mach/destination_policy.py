...
from mach_client import AssetServer, Chain, SupportedChain, Token
...
class CheapChainFixedSymbolPolicy(RandomChainFixedSymbolPolicy):
    ...
    def __init__(self, client: MachClient, symbol: str):
...
class SingleChainPolicy(TokenIteratorPolicy):
    ...
    def __init__(self, client: AssetServer, chains: AbstractSet[Chain]):
        chain_tokens = [list(tokens) for chain, tokens in client.tokens.items() if chain in chains]
