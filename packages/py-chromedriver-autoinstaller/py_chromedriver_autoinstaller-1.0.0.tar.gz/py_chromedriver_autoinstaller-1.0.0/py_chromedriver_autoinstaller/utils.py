...
def check_version(binary, required_version):
    try:
        version = subprocess.check_output([binary, "-v"], shell=True)
...
def get_driver_url(path: Optional[AnyStr] = None, no_ssl: bool = False):
    """Returns the download URL for Chromedriver instead of downloading."""
    ...
    if not os.path.isfile(chromedriver_filepath) or not check_version(
        ...
        return get_chromedriver_url(chromedriver_version=chromedriver_version, download_options=download_options, no_ssl=no_ssl)
