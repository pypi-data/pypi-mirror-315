...
class SemiData:
    """
    SemiData is like Data class include any type of data ('cross', 'time') as a table: column's labels are 'variable' and row's labels are 'index'.\n
    default type is 'cross', and TimeSeries is a child of Data that develop mathods on 'time' Data.\n
    But by this diffrence, Data.values must have same index, but in SemiData miss indexes are nan value.\n
    Example:\n
    * Data(values={
        'v1':{1:5,2:-3,3:math.nan},
        'v2':{1:15,2:math.nan,3:5},
    })\n
    * SemiData(values={
        'v1':{1:5,2:-3},
        'v2':{1:15,3:5},
    })\n
    * SemiData.fix_index() -> Data\n
    """
    pass
...
class Data:
    ...
    def summerize(self)->Data:
        ...
        for var in self.variables():
            if (count:=Sample(self).stats.count(var))>0:
                res[var]['count'] = count
                for i in self.index():
                    if not is_nan(self.values[var][i]):
                        res[var]['start_index'] = i
                        break
                for i in self.index()[::-1]:
                    if not is_nan(self.values[var][i]):
                        res[var]['end_index'] = i
                        break
                if self.dtype(var)=='numeric':
                    
                    res[var]['average'] = Sample(self).stats.average(var)
                    res[var]['std'] = Sample(self).stats.std(var)
                    res[var]['min'] = Sample(self).stats.min(var)
                    res[var]['max'] = Sample(self).stats.max(var)
        return Data(values=res).transpose()
    ...
    def fix_index(self, print_progress:bool=False, indent:int=0, subindent:int=5)->None:
        '''
        If the index of the variables are not the same, it makes them the same.\n
        Whenever there is a need to create an index for a variable, 'nan' records the value of that variable.\n
        '''
        if print_progress:
            print(' '*indent+'fixing index')
            print(' '*(indent+subindent)+'make total index')
        index = set()
        for var in self.variables():
            index.update(self.values[var].keys())
        if print_progress:
            print(' '*(indent+subindent)+'making "nan" for new indexs of variables.')
        st, pre, n = time.perf_counter(), 0, len(self.variables())
        for j, var in enumerate(self.variables()):
            for i in index.difference(self.values[var].keys()):
                self.values[var][i] = math.nan
            if print_progress:
                pre = progress(st, j, n, pre, var, indent=indent+2*subindent)
                
    def set_index(self, variable:str|list, duplicate_sum_vars:list=[], index_name:str='',
        ...
        a = Data(values={'v1':{'1':'ali', '2':'ali', '3':'hosein'}})\n
        print(a)\n
        a.set_index('v1',duplicate_sum_vars=['v2'])\n
        print(a)\n
        duplicate_sum_vars: if there are some duplicate in new index for this list of variables, calculate 'sum' of duplicated values, for other variable, calculate last value.\n
        '''
        if print_progress:
        ...
        if not isinstance(variable, list):
            variable = [variable]
        if len(variable) != len(old_index):
            new_index = []
            for i in old_index:
                j = ''
                for v in variable:
                    try:
                        j += f'_{self.values[v][i]}'
            ...
            if new_index != []:
                ...
                raise ValueError(f"Error! len of var (={len(variable)}) is not equal to len of data index (={len(old_index)}).")

        if print_progress:
            if len(variable) == 1:
                print(variable[0])
            elif len(variable) <= 5:
                print(', '.join([str(v) for v in variable]))
            else:
                print(variable[0],'...',variable[-1])
        if duplicate_sum_vars == []:
            self.values = {v:{variable[i]:val
                            for i, val in enumerate(self.values[v].values())}
                            for v in self.variables()}
        else:
            for v in self.variables():
                if v in duplicate_sum_vars:
                    vals = {}
                    for i, val in enumerate(self.values[v].values()):
                        if is_numeric(val):
                            try:
                    ...
                    vals = {variable[i]:val
                            for i, val in enumerate(self.values[v].values())}
                self.values[v] = vals
        if not index_name in ['', 'index']:
            for v in self.values:
                new_index = self.values[v].keys()
                break
            self.values.update({index_name:{i_new:old_index[i] for i,i_new in enumerate(new_index)}})
    ...
    def select_index(self,index:list|str|int|float,
        ...
        if not isinstance(index, list):
            ...
            res_dict = {}
            for var in self.variables():
                res_dict[var] = {i:self.values[var][i] for i in index}
            return Data(type=self.type, values=res_dict)
        else:
    ...
    def counts(self, variables:str|list=[])->int|list:
        """counts of not nans of all variables in a list."""
        if variables == []:
            variables = self.variables()
    ...
    def line_plot(self, vars:list[str], title:str='', xticks_rotation:int=0, show:bool=True, print_progress:bool=False, indent:int=0):
        '''
        A line graph plots one or more variables.\n
        ...
        ax.set_xticks(ax.get_xticks()[::interval], self.index(), rotation=xticks_rotation)
        plt.tight_layout()
        if show:
            plt.show()
        return plt

    def bar_plot(self, vars:list[str], title:str='', xticks_rotation:int=0, show:bool=True, print_progress:bool=False, indent:int=0):
        '''
        A bar graph plots one or more variables.\n
        matplotlib must installed.\n
        pip install matplotlib.
        '''
        if print_progress:
        ...
        plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.2)
        legs = []
        width = 10/(len(vars)*len(self.index())*2)
        x = list(range(len(self.index())))
        for j, var in enumerate(vars):
            rects = ax.bar([i+width*j for i in range(len(self.index()))], list(self.values[var].values()), width, label=var)
            ax.bar_label(rects, padding=3)
            legs.append(var)
        ax.legend(legs)
        ax.set_xticks([xi + width*(len(vars)-1)/2 for xi in x], self.index(), rotation=xticks_rotation)
        if show:
            plt.show()
        return plt
    ...
    def read_excel(cls, path_file:str, sheet:str|int=0, data_type:str='cross', na:any='', index:str='index',
        ...
        pip install openpyxl xlrd\n
        \n
        first_row = -1 -> first_row and first_col find automaticly.
        '''
        if print_progress:
            file_name = path_file.split('//')[-1]
            print(' '*indent+f"reading data from {file_name}")
        ...
        try:
            ...
            if first_row == -1:
                max_vars = (-1,[])
                for i in range(20):
                    rows = list(ws.iter_rows(i))
                    var_names = [cell.value for cell in rows[0]] if rows != [] else []
                    if len(var_names)-var_names.count(None)>len(max_vars[1])-max_vars[1].count(None):
                        max_vars = (i, var_names)
                first_row = max_vars[0]
                first_col = 0
                for var in max_vars[1]:
                    if var == None:
                        first_col += 1
                    else:
                        break
            ...
            if first_row == -1:
                max_vars = (-1,[])
                for i in range(20):
                    var_names = [ws.cell_value(i, col) for col in range(0, ws.ncols)]
                    if len(var_names)-var_names.count('')>len(max_vars[1])-max_vars[1].count(''):
                        max_vars = (i, var_names)
                first_row = max_vars[0]
                first_col = 0
                for var in max_vars[1]:
    ...
    def to_excel(self,path_file:str, sheet:str='sheet1', na:str='',replace_file:bool=False, replace_sheet:bool=False,
                 first_row:int=0, first_col:int=0, 
                 print_progress:bool=False, indent:int=0, subindent:int=5)->None:
        '''
        ...
        for index in self.index():
            for j,var in enumerate(self.variables()):
                ws.cell(1+first_row,j+2+first_col).value = var
        ws.cell(1+first_row,1+first_col).value = 'index'
        for i,index in enumerate(self.index()):
            for j,var in enumerate(self.variables()):
                ws.cell(i+2+first_row,1+first_col).value = index
        for i,index in enumerate(indexs:=self.index()):
            for j,var in enumerate(self.variables()):
                if is_nan(val:=self.values[var][index]):
                    ws.cell(i+2+first_row,j+2+first_col).value = na
                else:
                    ws.cell(i+2+first_row,j+2+first_col).value = val
            if print_progress:
...
class TimeSeries(Data):
    ...
    def complete_dates(self, print_progress:bool=False, indent:int=0, subindent:int=5):
        ...
        if self.dates != []:
            ...
            if not is_standard:
                self.set_index(var=self.dates, print_progress=print_progress, indent=indent+subindent)
            self.dates.sort()
    ...
    def set_index(self, var: str | list, duplicate_sum_vars:list=[], index_name:str='',
                  print_progress:bool=False, indent:int=0) -> None:
        '''
        setting index base on a variable or list of index out of data
        '''
        super().set_index(var, duplicate_sum_vars, index_name, print_progress, indent)
    ...
    def to_lag(self, lags:list[int], variables:list[str]=[], add_to_data:bool=True,
        ...
        """
        if not isinstance(lags,list) and isinstance(lags,Iterable):
            lags = list(lags)
        if print_progress:
            if len(lags)==1:
        ...
        if 0 in lags:
            lagged_values = {f'l0.{var}':self.values[var] for var in variables}
            if add_to_data:
                for var in variables:
                    self.values[f'l0.{var}'] = self.values[var]
            lags.remove(0)
        lagged_values = {f'l{lag}.{var}':{} for var in variables for lag in lags}
        if add_to_data:
            for var in variables:
                for lag in lags:
                    self.values[f'l{lag}.{var}'] = {}
            ...
            for t, date in enumerate(all_dates[:lag-1:-1]):
                for var in variables:
                    if (lagged_date:=all_dates[len(all_dates)-t-lag-1]) in self.dates:
                        lagged_values[f'l{lag}.{var}'][date] = self.values[var][lagged_date]
                    else:
                        lagged_values[f'l{lag}.{var}'][date] = math.nan
                    if add_to_data:
                        self.values[f'l{lag}.{var}'][date] = lagged_values[f'l{lag}.{var}'][date]
            if print_progress:
        ...
        for lag in lags:
            for date in self.dates[:lag]:
                for var in variables:
                    lagged_values[f'l{lag}.{var}'][date] = math.nan
                    if add_to_data:
                        self.values[f'l{lag}.{var}'][date] = math.nan
        if add_to_data:
            for var in self.variables():
...
if __name__=='__main__':
    os.system('cls')
    data = Data(values={
        'var1':{'1':542,'2':6325,'3':653,'4':2513},
        'var2':{'1':5,'2':3,'3':math.nan,'4':13},
        'var3':{'1':math.nan,'2':'s','3':'q','4':math.nan},
        'var4':{'1':'a','2':25,'3':'-','4':'33'},
    })
    print(data.summerize())
