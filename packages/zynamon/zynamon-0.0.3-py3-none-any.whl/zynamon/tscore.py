...
This class provides a generic type to handle samples of data associated w/ distinct instants in
time, but w/ potentially irregular frequency of appearance. It can thus describe both:
    (i) equidistant sampled data (e.g. audio)
    (ii) event-based data (e.g. warnings in a log file)
Aiming for a most flexible framework, the basic time & value arrays are contained in a
'pandas.DataFrame' that is amended by other "meta" information and tools.
"""
__version__ = 2.1
...
from zdev.validio import valid_str_name


# EXPORTED DATA
TS_ISO_FORMAT = '%Y-%m-%d %H:%M:%S.%f'
TS_SAVE_FORMATS = ('pk', 'h5', 'json')
...
_PRINT_HORIZONTAL = True
_ISO_TIMEZONE = False   # TODO: add support for timezones? (see also explanation at very bottom)
...
_TIME_TYPE = {
    ...
    int: 'stamp_ns',
    str: 'iso',
    dt.datetime: 'obj', pd.Timestamp: 'obj'
    }
_VALUE_TYPE = {
...
#   - "times" (covering various precisions)
# Further differentiation has to be made w.r.t. to operating regime, i.e. if applied to
#   - format 'settings' (SET)
...
# _RX_DAT_DATE_DASH_Y2L_MNUM = re.compile(r'\d{2}/\d{2}/\d{2}')           # 'dd/mm/yy' (same as Y2F!!)
# (DAT) -> times
...
class TimeSeries():
    ...
    def __init__(self, name, samples=None, tags=None):
        ...
        # public attributes
        self.df = pd.DataFrame(columns=('t','x')) # joint container for time & value arrays
        self.name = name        # identifier for the time-series
        self.num_batches = 0    # number of batches (if progressive construction)

        ###
        ## TODO: replace num batchs by batches = [(start, stop), ] for each batch as index!!!
        ##          --> num bacthes = len(batches)
        ...
        # add initial samples (if any)
        if (samples is not None):
            self.samples_add(samples)
    ...
    def __add__(self, other):
        """ Operation +. """
        from zynamon.tsutils import ts_relate
        return ts_relate(self, other, '+', res=None, mode='avg')
    ...
    def clear(self, keep_history=False):
        ...
        Returns:
            --
        """
        self.df = pd.DataFrame(columns=('t','x'))
        self.num_batches = 0
        self.time = None
        if (not keep_history):
            self.history = []
    ...
    def samples_add(self, new_samples, analyse=True, incognito=False):
        ...
        Args:
            new_samples (dict or list): Object containing new samples for the object. These may
                ...
                + dict w/ required keys 't' & 'x'
                + zip of 2-tuples where each item represents [time, value]
                + list/tuple/np.ndarray / items representing [time, value]
        ...
        if (is_timeseries(new_samples)):
            mode = 'ts'
        elif ((type(new_samples) is pd.DataFrame) and
              (set(['t','x']).issubset(new_samples.columns))):
            mode = 'df'
        ...
        # ingest new samples by preparing dataframe (acc. to mode)
        if (mode == 'ts'):
            df_new = new_samples.df
        elif (mode == 'df'):
            df_new = new_samples[['t','x']]
        elif (mode == 'dict'):
            if (not set(['t','x']).issubset(new_samples.keys())):
                raise KeyError("Missing time/value keys 't'/'x' in dict")
            if (len(new_samples['t']) != len(new_samples['x'])):
                raise ValueError('Inconsistent number of time/value items')
            df_new = pd.DataFrame(data={'t': new_samples['t'], 'x': new_samples['x']})
        elif (mode in ('zip','list')):
            unzipped = list(zip(*new_samples))
            df_new = pd.DataFrame(data={'t': unzipped[0], 'x': unzipped[1]})
        else: # mode == 'list-flat'
            time_unknown = np.nan * np.ones_like(new_samples)
            df_new = pd.DataFrame(data={'t': time_unknown, 'x': new_samples})

        #
        # TODO: include hysteresis here...
        #

        # ensure all batches have same time format
        if (self.df.empty): # initial batch (get spec from data)
            self.time = TimeSpec(df_new.t)
        else: # auto-convert (if required)
            t_new = ts_convert_time(df_new.t, self.get_timespec_str())
    ...
    def samples_crop(self, selected, incognito=False, inplace=True, addon='_cropped'):
        ...
        range is found at all, 'selected' is either in the "future" or in "past".

        For simplicity and intuitive use, a direct indexing is provided as well. That is, if
        'selected' contains integers less than 'TS_INDEX_LIMIT', these are interpreted as normal
        indices of the time array rather than 'stamp_ns' defined by 'ts_timespec()'.
        ...
        Args:
            selected (2-tuple or scalar): Frame definition by interval or scalars in *any form*
                supported by 'ts_timespec()'. Note that intervals '[A,B]' are *inclusive* (i.e.
                both 'df.iloc[A]' and 'df.iloc[B]' will be present) whereas scalars are resolved
                into the range '[len(df)-selected:]', i.e. cropping is done from the end.
            incognito (bool, optional): Switch to avoid history tracking. Defaults to 'False'.
        ...
        orig_spec = self.get_timespec_str()
        arr_t = ts_convert_time(self.df.t, 'stamp')
        ...
        # (2) indexing by any "timespec"
        else:
            if (mode == 'scalar'): # i.e. interval "from the end"
                t_sel = ts_convert_time(selected, 'stamp')
                condition = (arr_t >= t_sel)
            else: # mode == 'interval'
                tmp = TimeSpec(selected[0])
                selected[1] = tmp.force_spec(selected[1]) # supports "spec-mixed" intervals!
                t_sel = ts_convert_time(selected, 'stamp')
    ...
    def samples_unique(self, array=None, keep='first', causalise=False,
        ...
        Args:
            array (str, optional): Switch to select either 't' or 'x' to check for duplicates.
                Otherwise, only samples where both time & value are identical are counted as
                duplicates. Defaults to 'None' (i.e. check both).
    ...
    def time_align(self, res=1.0, shift='bwd', recon='avg', allow_expand=True,
        ...
        Args:
            ...
            recon (str, optional): Reconstruction / assignment method for time-series values.
                Defaults to 'mean'. Available options:
        ...
        if (allow_expand):
            ...
            df_aligned = pd.DataFrame({'t': np.array(t_new), 'x': x_new})
    ...
    def time_analyse(self):
        """ Assess consistency of temporal sampling found in data samples. """

        # ensure "usable" format

        #
        # WHAT IS THE ISSUE??? --> self.time not even init??????????????
        #



        arr_t = ts_convert_time(self.df.t, float)
        orig_spec = self.get_timespec_str()

        # create dict for advanced statistics (if not yet existing)
        if (self.time.stat is None):
            self.time.stat = {
                'quality': None,      # general quality of sampling rate & quality
        ...
        td = arr_t.diff()
        if (len(td)):
            idx = (td != 0.0) # exclude all "0.0" entries (i.e. duplicate time instants!)
            self.time.Ts = round(td[idx].mean(), 9)
        else:
            self.time.Ts = -1
            self.time.stat['quality'] = '[ not enough samples (yet) ]'
            return self.time.Ts

        # additional statistics
        self.time.stat['Ts_max'] = round(td[idx].max(), 9)
        self.time.stat['Ts_min'] = round(td[idx].min(), 9) # Note: Robustify due to division below!
        self.time.stat['Ts_sigma'] = round(td[idx].std(), 9)
        ...
        else: # self.time.stat['Ts_sigma_norm'] # i.e. > 10%
    ...
    def time_causalise(self, incognito=False, inplace=True, addon='_causal'):
        ...
        """
        if (inplace):
            self.df.sort_values('t', ascending=True, inplace=True)
            self.time_analyse()
            if (not incognito):
                self.history.append(('@time_causalise', None))
            return
        else:
            tmp = self.df.sort_values('t', ascending=True)
            ts_out = self.clone(self.name+addon)
            ts_out.clear(keep_history=True)
    ...
    def time_convert(self, target, scaling=None, offset=None, incognito=False):
        """ Converts time array to 'target' w/ optional 'scaling' and/or 'offset' (inplace).

        Notes:
            (1) Modifications (scaling / offset) are only available for 'stamp'-type inputs!
            (2) For more information on 'TimeSeries' time formats, refer to 'ts_timespec()'.

        Args:
            target (scalar): Desired time specification.
            scaling (float, optional): Scaling factor for all times. Defaults to 'None'.
        ...
        Returns:
            --
        """
        self.df.t = ts_convert_time(self.df.t, target, scaling, offset)
        self.time = TimeSpec(target)
        if (scaling is not None):
            self.time.scale *= scaling
        if (offset is not None):
            self.time.shift += offset
        if (not incognito):
            self.history.append(('@time_convert', f"'{target}', scaling={scaling}, offset={offset}"))
        return
    ...
    def values_purge(self, mode='patch_local', params={'N': 3}, outliers=None,
        ...
        Args:
            mode (str, optional): Correction mode for outliers (if any). Defaults to
                ...
                + 'patch_local'  -> patch outlier values by local mean around position
                + 'patch_global' -> patch outliers by global mean
            params (dict, optional): Parameters for correction (if required at all). Defaults
                to {'N': 3} where the individual entries refer to:
                + 'N' (int): (One-sided) width of local window to calculate mean value
        ...
        if (outliers is None):
            outliers, _ = ts_find_outliers(self)
        ...
        if (len(outliers)):
            ...
            if (mode == 'remove'):
                for item in outliers:
                    df_new.iloc[item[0]] = {'t': np.nan, 'x': np.nan}
                df_new.dropna(inplace=True)

            elif (mode == 'patch_local'):
                for item in outliers:
                    df_new.x[item[0]] = local_val(self.df.x, item[0], params['N'], mode='mean')

            elif (mode == 'patch_global'):
                global_patch = self.df.x.mean()
                for item in outliers:
                    df_new.x[item[0]] = global_patch
    ...
    def to_numpy(self, type_t=None, type_x=None):
        """ Converts both 't' and 'x' columns to NumPy arrays (for processing-heavy tasks ;).

        Args:
            type_t (type or str, optional): Convert time array to any desired type supported by
                'ts_timespec()'. Defaults to 'None' (i.e. no conversion).
        ...
        if (type_t is not None):
            arr_t = ts_convert_time(arr_t, type_t)
        #
        # TODO: conversion for values???
        #
        return arr_t, arr_x, types


    def to_pandas(self):
        """ Converts time-series to plain 'pd.core.frame.DataFrame' type (e.g. for new objects).

        Returns:
            df_out (:obj:): Plain pandas 'DataFrame' object containing only columns 't' and 'x'.
        """
        df_out = self.df.copy()
        return df_out
    ...
    def export_to_file(self, fname=None, overwrite=False, combine=False, keep_history=False,
        ...
        Moreover it needs to be emphasized that the function 'zynamon.tsutils.ts_write_file()'
    ...
    def export_to_pk(self, fname=None, overwrite=False, combine=False, keep_history=False):
    ...
    def export_to_hdf5(self, fname=None, overwrite=False, combine=False, keep_history=False):
    ...
    def export_to_json(self, fname=None, overwrite=False, combine=False, keep_history=False):
    ...
    def get_timespec_str(self):
        """ Returns effective time-spec of object (e.g. for enforcing it on another object). """
        if (self.time.fmt != ''):
    ...
    def get_time_range(self):
        ...
        if (self.time.type == 'stamp'):
            ...
            tmp = ts_convert_time([self[0].t, self[-1].t], 'stamp')
            return (tmp[1] - tmp[0])


    def get_type_t(self, check_array=False):
        """ Returns type of DataFrame column 't'.
        ...
        Returns:
            type_t (str): Time type found as an element of 'set(_TIME_TYPE.values())', i.e.
                'stamp_ns'|'stamp'|'iso'|'obj'.
        """

        # get type of 1st time instant
        type_t, _, __ = ts_timespec(self.df.t.iloc[0])
        ...
        if (not check_array):
            ...
            for item in self.df.t.iloc[1:]:
                chk_type, _, __ = ts_timespec(item)
                if (chk_type != type_t):
    ...
    def get_type_x(self, check_array=False):
        """ Returns type of DataFrame column 'x'.
    ...
    def is_type_t(self, type_chk, check_array=False):
        """ Validates that DataFrame column 't' contains (only) values matching 'type_chk'.

        Args:
            type_chk (str): Type of values to be checked for in 'self.df.t' w/ options acc. to
        ...
        if (type(type_chk) is type):
            type_chk = _TIME_TYPE[type_chk]

        # get existing type & compare w/ check
        the_type = self.get_type_t(check_array)
        if (the_type == type_chk):
            return True
        else:
            return False


    def is_type_x(self, type_chk, check_array=False):
        """ Validates that DataFrame column 'x' contains values matching 'type_chk'.

        Args:
            type_chk (str): Type of values to be checked for in 'self.df.x' w/ available
...
class TimeSpec():
    ...
    def __init__(self, obj=None):
        """ Initializes a time-spec instance (i.e. an empty one or for 'obj').
        ...
        'iso'     |str        |'2022-10-20 19:02:44:776' => String in default '_TS_ISO_FORMAT'
        'obj'     |dt.datetime|:obj:                     => Object 'dt.datetime' (w/ several parts)

        ...or...
        detailed formats e.g. |'%y/%b/%d %H:%M'          => arbitrary string formats
        ----------------------------------------------------------------------------------------
        """

        # general attributes
        self.orig_type = None   # Python type of original input (e.g. 'pd.Series' or 'list')
        self.type = None        # defined  type [str] ('stamp'|'stamp_ns'|'iso'|'obj')
        ...
        self.Ts = None      # sampling interval [s] (will typically be averaged)
        ...
        # apply proper mapping on provided 'obj' (if already provided)
        if (obj is not None):
            if ((type(obj) in (list, np.ndarray, pd.Series)) or
                ((type(obj) is str) and re.search(S_NUMBER, obj)) or
                type(obj) in _TIME_TYPE.keys()):
                self.from_data(obj)
            else: # ((obj in _TIME_TYPE.keys()) or (obj in list(set(_TIME_TYPE.values()))))
                self.from_param(obj, check_known=True)

        return


    def __eq__(self, other):
        """ Checks equality of own TimeSpec with other's. """
        return (self.type == other.type) and (self.fmt == other.fmt)


    def from_param(self, setting, check_known=True):
        """ Sets time-spec acc. to a 'setting' either by data type or strings.

        Arg:
            setting (type or str): Time specification to be set w/ possible ways of definition
                by type|str|formatted string (see examples below).
            check_known (bool, optional): Switch to check if the provided format matches *any*
                known pattern, only applicable for 'iso'. Defaults to 'True'.

        Returns:
            --

        Note: If no specific format is given for string type/'iso', 'tscore.TS_ISO_FORMAT' is
        used as default!

        Examples:
            (type)     'setting':   int       |float  |str  |dt.datetime
            (string)   'setting':   'stamp_ns'|'stamp'|'iso'|'obj'
            (detailed) 'setting':   '%Y-%m-%d %H:%M:%S.%f' (acc. to ISO 8601 / RFC3339)
        """
        self.orig_type = type(setting)

        # determine time-spec "type"
        if (type(setting) is type):
            self.type = _TIME_TYPE[setting]
        elif (setting in set(_TIME_TYPE.values())):
            self.type = setting
        elif (type(setting) is str):
            self.type = 'iso'
        else:
            raise ValueError(f"Unknown time-spec input {setting}")

        # determine time-spec "format"?
        if (self.type == 'iso'):
            if ((setting is str) or (setting == 'iso')):
                self.fmt = TS_ISO_FORMAT
            else:
                if (check_known):
                    # try to match w/ known DATE patterns...
                    if ((_RX_SET_DATE_HYPH_Y4F_MNUM.match(setting) is not None) or
                        (_RX_SET_DATE_HYPH_Y2F_MNUM.match(setting) is not None) or
                        (_RX_SET_DATE_HYPH_Y4F_MNAME.match(setting) is not None) or
                        (_RX_SET_DATE_HYPH_Y2F_MNAME.match(setting) is not None) or
                        (_RX_SET_DATE_DASH_Y4F_MNUM.match(setting) is not None) or
                        (_RX_SET_DATE_DASH_Y2F_MNUM.match(setting) is not None) or
                        (_RX_SET_DATE_DASH_Y4F_MNAME.match(setting) is not None) or
                        (_RX_SET_DATE_DASH_Y2F_MNAME.match(setting) is not None)): # -> date ok
                        # ...then try to match w/ known TIME pattern / precision
                        if ((_RX_SET_TIME_USEC_DOT.search(setting) is not None) or
                            (_RX_SET_TIME_USEC_COMMA.search(setting) is not None) or
                            (_RX_SET_TIME_SEC.search(setting) is not None) or
                            (_RX_SET_TIME_MIN.search(setting) is not None)): # -> time ok
                            self.fmt = setting
                        else:
                            raise ValueError("Unknown time pattern in '{setting}'")
                    else:
                        raise ValueError("Unknown date pattern in '{setting}'")
                else:
                    self.fmt = setting # i.e. apply w/o checking
        else:
            self.fmt = '' # TODO: or set expected print precisions like '%f' or '%d'?

        return


    def from_data(self, t_in):
        """ Sets time-spec acc. to time data in 't_in' (i.e. scalar or array).

        Args:
            t_in (scalar or array): Data sample or array from which to derive the time-spec. In
                case of arrays, only the 1st element will be analysed since a "homogeneous"
        ...
        Examples:
            (scalar) 't_in':    '2022-10-20 19:02'
            (array)  't_in':    [1657186852.027276, 1657188744.037128, ... ]
        """
        self.orig_type = type(t_in)
        ...
        if (self.orig_type in (list, np.ndarray)):
            ...
            if (self.orig_type is str):
                ...
                t_chk = t_in
        # Note: Even if input data *is* a string, try to map it to a number (int? / float?) if
        # possible. This is done for more robust importing of CSV-data where the items read
        # line-by-line may often be surrounded by (superfluous!) '...' or "..." ;)
        ...
        if (type(t_chk) in (int, np.int64)): # Note: 'np.int32' can't hold 'stamp_ns' precision!
            ...
            self.type = 'stamp'
        elif (type(t_chk) in (dt.date, dt.datetime, pd.Timestamp)):
              self.type = 'obj'
        else:
            self.type = 'iso'
            # Note: This assumes ISO strings, as otherwise the above conversion (int? / float?)
            # would have succeeded in recovering plain numbers...

        # determine time-spec "format"
        if (self.type == 'iso'):
            # try to match w/ known DATE patterns...
            if (_RX_DAT_DATE_HYPH_Y4F_MNUM.match(t_chk) is not None):
                fmt_date = '%Y-%m-%d'
            elif (_RX_DAT_DATE_HYPH_Y2F_MNUM.match(t_chk) is not None):
                fmt_date = '%y-%m-%d'
            elif (_RX_DAT_DATE_HYPH_Y4F_MNAME.match(t_chk) is not None):
                fmt_date = '%Y-%b-%d'
            elif (_RX_DAT_DATE_HYPH_Y2F_MNAME.match(t_chk) is not None):
                fmt_date = '%y-%b-%d'
            elif (_RX_DAT_DATE_DASH_Y4F_MNUM.match(t_chk) is not None):
                fmt_date = '%Y/%m/%d'
            elif (_RX_DAT_DATE_DASH_Y2F_MNUM.match(t_chk) is not None):
                fmt_date = '%y/%m/%d'
            elif (_RX_DAT_DATE_DASH_Y4L_MNUM.match(t_chk) is not None):
                fmt_date = '%d/%m/%Y'
            # elif (_RX_DAT_DATE_DASH_Y2L_MNUM.match(t_chk) is not None):
                # fmt_date = '%d/%m/%y'
            # Note: Two-year versions with "first" and "last" cannot be differentiated, i.e.
            # '_RX_DAT_DATE_DASH_Y2F_MNUM' may actually be '_RX_DAT_DATE_DASH_Y2L_MNUM'! :(
            else:
                fmt_date = ''
            # ...then try to match w/ known time pattern / precision
            if (_RX_DAT_TIME_USEC_DOT.search(t_chk) is not None):
                fmt_time = '%H:%M:%S.%f'
            elif (_RX_DAT_TIME_USEC_COMMA.search(t_chk) is not None):
                fmt_time = '%H:%M:%S,%f'
            elif (_RX_DAT_TIME_SEC.search(t_chk) is not None):
                fmt_time = '%H:%M:%S'
            elif (_RX_DAT_TIME_MIN.search(t_chk) is not None):
                fmt_time = '%H:%M'
            else:
                fmt_time = ''
            if (not ((fmt_date == '') and (fmt_time == ''))): # i.e. at least one match
                if ((fmt_date != '') and (fmt_time != '')):
                    sep = ' '
                else:
                    sep = ''
                self.fmt = fmt_date + sep + fmt_time
                # Note: Detection will *only fail* if neither date nor time cannot be mapped!
                # In this way, both "date-only" ('2022-10-08') or "time-only" ('12:07:32')
                # formats are allowed as well and can safely be ingested...
        else:
            self.fmt = ''   # TODO: or set expected print precisions like '%f' or '%d'?
    ...
    def force_spec(self, ts):
        """ Force same time-spec onto (other) time-series 'ts'.

        ### TODO: include sclaing and offset?

        Args:
            ts (:obj:): Time-series object ("slave").

        Returns:
            ts_mod (:obj:): Output time-series object w/ converted in time representation (if
                required).
        """

        if (self.time == ts.time):
            return ts
        else: # i.e. time-specs are different
            if (self.time.fmt != ''):
                return ts_convert_time(ts, self.time.fmt)
            else:
                return ts_convert_time(ts, self.time.type)
...
def _ts_from_h5group(ts_item):
    ...
    # recreate core data (arrays & additional time info)
...
def is_timeseries(obj, accept_type=True):
    ...
    if (isinstance(obj, TimeSeries)):
        return True
    elif (accept_type and (str(obj).find('TimeSeries') >= 0)):
        return True
    else:
        return False
...
# class TimeParams(self):
#     """ Parameters describing the time array of 'TimeSeries' objects. """
...
# class TimeSpec(self):
#     """ Time specifications to be used in context of 'TimeSeries' objects. """
...
def ts_convert_time(t_in, target, scaling=None, offset=None):
    """ Converts 't_in' to the given 'target' (w/ optional 'offset' and 'scaling').
    ...
    apply. Modifications on a time array may be enforced by using the 'scaling' (multiplicative)
    and 'offset' (additive) options.

    Notes:
        (i) Modifications (scaling / offset) are only available for 'stamp'-type inputs!
        (ii) For more information on 'TimeSeries' time formats, refer to 'ts_timespec()'.
    ...
    Args:
        ...
        target (scalar): Desired time specification.
        scaling (float, optional): Scaling factor for stretching (> 1.0) or squeezing (< 1.0)
        ...
        offset (scalar, optional): Additional offset that can be given in *any format* supported
            by 'ts_timespec()' and which shall be applied to all array elements before
            conversion (but *after* scaling). Defaults to 'None'.

    Returns:
        t_out (list): Array of properly converted time values.
    """
    ...
    if ((src.type == 'stamp') or (src.type == 'stamp_ns')):
        if (scaling is not None):
            try:
                ...
                raise ValueError(f"Unknown scaling '{scaling}' specified!")
        if (offset is not None):
            try:
                off = ts_convert_time(offset, 'stamp')
            except:
                raise ValueError(f"Unknown offset '{offset}' specified!")
    # Note: For any other source type, these settings (if any) are ignored!

    # laziness checks (i.e. try to avoid work if actually nothing to do ;)
    if (src.type == tgt.type):
        if ((tgt.type in ('stamp', 'stamp_ns')) and (off == 0.0) and (fac == 1.0)):
            ...
            pass #TODO: any other "skip conditions"?

    # init conversion (i.e. use temporary lists for consistent processing)
    t_out = []
    if (scalar_input):
        t_in = [t_in]
    ...
    if (src.type == 'stamp'):
        for n, tn in enumerate(t_in):
            ...
            if (tgt.type == 'stamp'):
                ...
                obj = dt.datetime.utcfromtimestamp(tmp)
                t_out.append( obj.strftime(tgt.fmt) )
            elif (tgt.type == 'obj'):
                t_out.append( dt.datetime.utcfromtimestamp(tmp) )
        ...
        for n, tn in enumerate(t_in):
            tmp = dt.datetime.strptime(tn, src.fmt)
            if (tgt.type == 'stamp'):
                t_out.append( tmp.replace(tzinfo=dt.timezone.utc).timestamp() )
            elif (tgt.type == 'stamp_ns'):
                t_out.append( int(1e9*tmp.replace(tzinfo=dt.timezone.utc).timestamp()) )
            elif (tgt.type == 'iso'):
        ...
        for n, tn in enumerate(t_in):
            ...
            if (tgt.type == 'stamp'):
                ...
                t_out.append( tmp.strftime(tgt.fmt) )
            # elif (tgt.type == 'obj'):
            #     t_out.append( tmp ) # Note: Already handeled by above "skip" condition!

    # return output (matching to input type/shape)
    if (src.orig_type is pd.Series):
        return pd.Series(t_out) # Note: This will convert 'obj' type to 'pd.Timestamp'!
...
def ts_find_outliers(ts, mode='local_mean', params={'N': 10, 'excess': 0.5}):
    """ Detects possible outliers in 'ts' acc. to 'mode' and detection 'params'.

    Args:
        ts (:obj:): 'TimeSeries' object for which to identify outliers.
        mode (str, optional): Detection mode for outliers. Defaults to 'local_mean'.
            Available options:
            + 'local_mean'  -> compare w/ non-causal sliding window of 2*N+1 centered
            + 'avg'         -> compare w/ smoothed version by FIR-filtering of length N
        params (dict, optional): Parameters for tuning the detection. Defaults to
            {'N': 10, 'excess': 0.5} where the individual entries refer to:
            + 'N' (int): Depending on the 'find' mode this either specifies the
                         (one-sided) width (if comaring with the local mean) or the
                         FIR filter length (if averaging)
            + 'excess' (float): Excess factor specifying how much a value has to exceed or
                                fall short w.r.t. its surrounding to be declared as outlier.
                                (e.g. for 0.5 samples need to either exceed by 50% or have
                                only half the value, see examples below)
    ...
    Example: If comparison is done w.r.t. value 'cmp[n] = 3.0' (regardless of 'find' mode)
             and if 'excess' = 1.0, i.e. outliers are required to be 100% larger or smaller:
    ...
    # create comparator
    cmp = ts.clone(ts.name+'_cmp_'+mode)
    if (mode == 'local_mean'): # non-causal filtering!
        tmp = np.zeros(len(ts))
        for k in range(len(ts)):
            tmp[k] = local_val(ts.df.x, k, params['N'], mode='mean')
        cmp.values_set(tmp)
    elif (mode == 'avg'): # phase-shifted!
        cmp.values_filter('FIR_LP_MA', {'N': params['N']})
...
def fix_prec(x, p):
    """ Returns float 'x' with a fixed precision 'p' (i.e. number of digits after '.').

    Args:
        x (float): Floating-point number
        p (int): Desired precision w.r.t. number of digits after/to the right of '.'.
...
def local_val(x, k, width, mode='mean'):
    ...
    Args:
        ...
        mode (str, optional): Mode of filtering. Defaults to 'mean'. Available options are
...
if __name__ == "__main__":
    print("This is the 'zynamon.tscore' module.")
    print("See 'help(zynamon.tscore)' for proper usage.")
    print("")
    print("Usage example:")
    print(">>> ts = TimeSeries('MyTest')")
...
################################################################################################
#
# Differences in DEFAULT BEHAVIOUR between Pandas and TimeSeries:
#   pandas.core.frame.DataFrame     -> creates new object (per default)
#   zynamon.tscore.TimeSeries       -> works inplace (per default)
#
# Overview on TimeSeries' methods:
#
# clear             = (fixed) inplace
...
# # set/define timezon (in which you’re in, i.e. CET=+1, CEST=+2)
# mytz = dt.timezone(dt.timedelta(hours=+2))

# # show timezone info
# mytz.tzname(dt.datetime.now())
...
# #
# #
# # HOW-TO: include timezone information? (only conversion just before display??? )
# #         ISO 8601 format: '2022-09-09T12:45:23.757239' --> using the 'T'
# #
# #   import pytz
