...
__date__ = '2023 @ Erlangen, Germany (Siemens Energy Global GmbH & Co. KG)'
...
from zynamon.tscore import ts_convert_time
...
def x_read_spectrum(fname):
    ...
    with open(fname, mode='r') as tf:
        ...
        for n, item in enumerate(fields):
            fields[n] = item.strip() # clean whitespaces (incl. newline)
        # spectrum['time'] = fields[0] # 1st column = time reference (assigned directly)
        spectrum['time'] = ts_convert_time(fields[0], str) 
...
def x_write_spectrum(fname, spectrum, orig_header=None, spread=0.1):
    """ Writes 'spectrum' to 'fname' where the FFT is "fake" as a regular time-series. 
    ...
    with open(fname, mode='wt') as tf:
        ...
        T0 = ts_convert_time(spectrum['time'], int) 
    ...
    time_span = (ts_convert_time(T0, str), ts_convert_time(T0+int(len(spectrum['freq']))*Tsp, str))    
...
#     """
#     files_converted = []    
#     try: # ROBUSTIFICATION since interruptions may occur (e.g. external USB drive is removed?)   
...
#             mod_file = x_convert_spectrum(the_file)
#             files_converted.append(mod_file)
...
#         # summary
#         if (not verbose):
#             print(f"    ==> Found & converted {N} files")        
             
#     except:
#         print("    ==> !!!! INTERRUPTION !!!! (external storage?)")
#         print("    ==> Remaining files will be CONVERTED IN NEXT RUN!")
#         msg = 1
    
#     finally:
#         return msg, files_converted
