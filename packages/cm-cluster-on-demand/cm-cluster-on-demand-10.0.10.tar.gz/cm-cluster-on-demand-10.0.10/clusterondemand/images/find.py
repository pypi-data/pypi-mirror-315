...
class CODImage:
    ...
    bcm_edge_head_node_private_key: str = ""
    bcm_edge_head_node_public_key: str = ""
    ...
    def get_properties(self) -> dict[str, str]:
        ...
        return {
            ...
            "bcm_api_hash": str(self.bcm_api_hash),
...
class ImageSource:
    ...
    def find_images(self, latest: bool | None = None, latest_invert: bool = False,
        ...
        if config is not None:
            if config["all_revisions"]:
                if config["latest"]:
                    log.debug("--all-revisions flag detected, ignoring configuration of 'latest' option")
                latest = None
                latest_invert = False
        ...
        images = (
            image for image in images
            if image.arch == arch or (self.uuids and image.uuid in self.uuids) or
    ...
    @classmethod
    def _pick_image_by_ids(cls, image_ids: Sequence[str], image_type: str, cloud_type: str) -> CODImage:
...
findimages_ns.add_parameter(
    "arch",
    default="x86_64",
    help="(default: x86_64) show only images of this CPU architecture",
)
findimages_ns.add_parameter(
    "distro",
