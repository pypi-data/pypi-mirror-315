...
def run_invoked_command(
    ...
    try:
        ...
        _log_user_reportable_exception(e, configuration)
...
def _check_for_unknown_config_parameters(configuration: ConfigurationView, system_config_files: list[str],
    ...
    if parameters is None or check_all_commands:
        ...
        parameters = chain(aws_parameters, azure_parameters, openstack_parameters, oci_parameters)
...
def _print_error_stack_on_failure(e: Exception, configuration: ConfigurationView | dict[str, Any]) -> None:
...
def _log_user_reportable_exception(exception: Exception, config: ConfigurationView) -> None:
    _print_error_stack_on_failure(exception, config)
