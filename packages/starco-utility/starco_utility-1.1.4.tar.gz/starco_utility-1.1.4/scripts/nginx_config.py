...
from datetime import datetime  # اضافه کردن import صحیح
import logging

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename='/var/log/nginx_config.log'
)


class SecurityManager:
    def __init__(self):
        self.modsec_config = "/etc/nginx/modsec"

    def configure_waf(self):
        subprocess.run(["apt", "install", "libmodsecurity3",
                       "nginx-module-modsecurity"], check=True)
        # ModSecurity configuration

    def setup_rate_limiting(self, rate="10r/s"):
        return f"""
    ...
    def block_malicious_ips(self):
        blacklist = input(
            "Enter IP addresses to block (comma-separated): ").split(",")
        return "\n".join([f"deny {ip.strip()};" for ip in blacklist])


class CacheManager:
    def configure_cache(self, location="/var/cache/nginx", size="10g"):
        return f"""
        proxy_cache_path {location} levels=1:2 keys_zone=my_cache:10m max_size={size} 
...
class SSLManager:
    def install_certbot(self):
        try:
            # First ensure required repos
            subprocess.run(
                ["apt", "install", "software-properties-common", "-y"], check=True)
            subprocess.run(
                ["add-apt-repository", "universe", "-y"], check=True)
            subprocess.run(
                ["add-apt-repository", "ppa:certbot/certbot", "-y"], check=True)
            subprocess.run(["apt", "update"], check=True)

            # Now install certbot
            subprocess.run(["apt", "install", "certbot",
                           "python3-certbot-nginx", "-y"], check=True)
            return True
        except subprocess.CalledProcessError as e:
            logging.error(f"Failed to install certbot: {e}")
            return False
...
class BackupManager:
    ...
    def create_backup(self):
        ...
        backup_path = f"{self.backup_dir}/nginx_config_{timestamp}.tar.gz"
        subprocess.run(["tar", "czf", backup_path, "/etc/nginx"], check=True)
        return backup_path
...
class NginxConfigManager:
    ...
    def create_config(self):
        ...
        if use_ssl:
            ...
            ssl_cert = input(
                "Enter path to SSL certificate (default: /etc/nginx/ssl/domain.crt): ") or f"/etc/nginx/ssl/{domain}.crt"
            ssl_key = input(
                "Enter path to SSL key (default: /etc/nginx/ssl/domain.key): ") or f"/etc/nginx/ssl/{domain}.key"
            redirect_ssl = input(
                "Redirect HTTP to HTTPS? (y/n): ").lower() == 'y'

        # Root Directory Settings
        root_dir = input(
            "Enter root directory (default: /var/www/html): ") or "/var/www/html"
        ...
        custom_index = input(
            f"Enter index files (default: {default_index}): ") or default_index
        ...
        while input("\nAdd custom location block? (y/n): ").lower() == 'y':
            ...
            proxy_pass = input(
                "Enter proxy_pass URL (if any, e.g., http://localhost:3000): ")
        ...
        while True:
            ...
            try:
                if choice == "1":
                    ...
                    subprocess.run(
                        ["systemctl", "enable", "nginx"], check=True)
                    print("Nginx enabled - will start on boot")
                elif choice == "6":
                    subprocess.run(
                        ["systemctl", "disable", "nginx"], check=True)
    ...
    def create_load_balancer(self):
        ...
        if use_ssl:
            ssl_port = input("Enter HTTPS port (default 443): ") or "443"
            ssl_cert = input(
                "Enter path to SSL certificate: ") or f"/etc/nginx/ssl/{domain}.crt"
            ssl_key = input(
                "Enter path to SSL key: ") or f"/etc/nginx/ssl/{domain}.key"
            redirect_ssl = input(
                "Redirect HTTP to HTTPS? (y/n): ").lower() == 'y'
        ...
        # Session Persistence
        sticky_sessions = input(
            "Enable sticky sessions? (y/n): ").lower() == 'y'

        # Health Check Settings
        health_check = input("Configure health checks? (y/n): ").lower() == 'y'
        if health_check:
            health_path = input(
                "Health check path (default /health): ") or "/health"
            check_interval = input(
                "Check interval in seconds (default 5): ") or "5"
    ...
    def configure_static_files(self, location):
        config = f"""
        location /static/ {{
            root {location};
            expires 30d;
    ...
    def cleanup(self):
        try:
            ...
            logging.info("Nginx configuration reloaded successfully")
        except Exception as e:
            pass
...
def main():
    ...
    while True:
        ...
        print("7. Remove site")
        print("8. Configure SSL/Let's Encrypt")
        print("9. Security Settings")
        print("10. Cache Settings")
        print("11. Static Files Config")
        print("12. Monitoring Setup")
        print("13. Create Backup")
        print("14. Test Configuration")
        print("15. Exit")
        ...
        if choice == "8":
            ...
            manager.ssl.install_certbot()
            manager.ssl.request_certificate(domain)
        elif choice == "9":
            manager.security.configure_waf()
        elif choice == "10":
            manager.cache.configure_cache()
        elif choice == "11":
            location = input("Enter static files location: ")
            manager.configure_static_files(location)
        elif choice == "12":
            manager.monitoring.setup_prometheus_metrics()
        elif choice == "13":
            backup_path = manager.backup.create_backup()
