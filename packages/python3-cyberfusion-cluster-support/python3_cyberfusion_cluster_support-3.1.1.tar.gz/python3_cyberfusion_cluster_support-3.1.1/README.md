...
Tests re-use [cassettes](https://lore.cyberfusion.nl/technical/cassettes/).

Added or changed a test? To create or re-create cassettes, run pytest with the following arguments:

* `--record-mode=rewrite`
* `--core-api-path=...` (Core API resets automatically after every test)
* `--cluster-api-server-url=...` (by default: `http://localhost:8080`)
* `--cluster-api-username=...` (by default: `admin`)
* `--cluster-api-password=...` (by default: `8KQ7LyVNrY2PtywgDnYKcGMP`)
* `--cluster-api-api-key=...` (by default: `1|LXbq3jzQpSYRNA3SWeM3BPpF`)
