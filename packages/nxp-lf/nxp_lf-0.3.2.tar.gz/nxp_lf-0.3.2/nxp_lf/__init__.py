...
class DbModel:
    ...
    def __init__(self, build_plan):
        logging.info("LF history db script version: 0.3.0")
    ...
    def log_into_db(self, result):
        ...
        for per_result in result.result_body:
            ...
            sql = "INSERT INTO lf_history (build_plan,build_number,scope,platform,testcase,result) VALUES ('{}', {}, '{}', '{}', '{}', {}) ON CONFLICT (build_number,platform,scope,testcase) DO UPDATE SET result={}".format(
                *(result.result_header + per_result + per_result[-1:])
            )
            self.cur.execute(sql)
...
if __name__ == "__main__":
    ...
    result_body = []
    result_body.append(("I.MX8MM", "CASE0", 1))
