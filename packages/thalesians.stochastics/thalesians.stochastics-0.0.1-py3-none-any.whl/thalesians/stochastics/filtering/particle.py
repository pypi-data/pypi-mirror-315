...
from thalesians.stochastics.distrs import EmpiricalDistr
...
class ParticleFilter(objects.Named):
    ...
    class ParticleObservable(filtering.Observable):
        def __init__(self, stochastic_filter, name, obs_model, observed_processes, *args, **kwargs):
            super().__init__(stochastic_filter, name)
    ...
    def _weight(self, observation):
        ...
        if npu.is_vectorized(self._weighting_func):
            ...
            for i in range(self._particle_count):
                self._current_particle_idx = i
                self._unnormalized_weights[i] = npu.to_scalar(self._weighting_func(observation, self._prior_particles[i,:], self))
    ...
    def prior_particles(self):
        return npu.immutable_copy_of(self._prior_particles)
    
    @property
    def resampled_particles(self):
        return npu.immutable_copy_of(self._resampled_particles)

    @property
    def unnormalized_weights(self):
        return npu.immutable_copy_of(self._unnormalized_weights)

    @property
    def weights(self):
        return npu.immutable_copy_of(self._weights)
