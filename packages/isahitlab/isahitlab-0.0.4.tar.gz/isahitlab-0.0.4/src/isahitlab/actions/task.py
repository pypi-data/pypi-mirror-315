import warnings
from typing import List, Optional, Iterable, Dict, cast, Union, Generator, Any
...
class TaskActions(BaseAction):
    ...
    def tasks(
        ...
        filters = TaskFilters(
            ...
            batch_id_in = cast(List[BatchId], batch_id_in) if batch_id_in else None,
        ...
        operation_gen = operation.run(project_id=project_id, filters=filters, disable_progress_bar=disable_progress_bar or iterate, compatibility_mode=compatibility_mode)
    ...
    def create_tasks(
        ...
        projects = list(GetProjectsOperation(self.http_client).run(filters=ProjectFilters(id_in=[project_id]), disable_progress_bar=disable_progress_bar))
