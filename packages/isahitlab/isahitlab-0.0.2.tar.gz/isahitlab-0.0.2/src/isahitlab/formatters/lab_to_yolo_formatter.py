...
from isahitlab.helpers.labels import extract_main_labels, flatten_available_labels_used_in_main_lists
    ...
    **image1.jpg.txt**
    ...
    def __init__(self, project_configuration : Dict, options : Dict = {}) -> None :
        self._init_configuration(project_configuration, options, {
            "compatible_types" : ["form.tool-iat-rectangle", "iat-rectangle"]
        })

        # Check if is sequence
        if project_configuration.get("metadata", {}).get("toolOptions", {}).get("sequence", False):
