from OpenDrive.modules.perception.trained_models.objects_detection.get_object_detection import get_obj_detection_image_TESTING
...
def consuming_video_real_time(path_input_video,name_window):
    ...
    while captureFrame.isOpened():
        ...
        if ret == True:    
            ...
            image_processed = get_obj_detection_image_TESTING(frame_reduce)
...
def consuming_video_real_time_jumpFrame(path_input_video, name_window):
    ...
    while captureFrame.isOpened():
        ...
        # Procesamiento
        frame_reduce = resize_image_CV(frame, output_size=(720,720))
        image_processed = get_obj_detection_image_TESTING(frame_reduce)
        image_processed_sings = get_sign_detection_image(image_processed)
...
# thread2 = threading.Thread(target=consuming_video_real_time_jumpFrame, args=('OpenDrive/utils/video_processing/input_videos/simulador3-Trasero.mp4', 'Trasero'))
