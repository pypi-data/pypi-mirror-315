...
from caerp_base.models.types import (
    JsonEncodedList,
)
        ...
        if self.status in ("planned", "running"):
            if datetime.now() - self.updated_at > TIMEOUT:
                ...
                if hasattr(self, "error_messages"):
                    self.error_messages = [
                        "Cette tâche a été automatiquement annulée car elle "
                        "n'a pas pu être traitée. Veulliez contacter un "
                        "administrateur en lui fournissant l'identifiant de "
                        "tâche suivant : {0}".format(self.id)
                    ]
                result = False
        return result
...
class CsvImportJob(Job):
    ...
    def todict(self):
        ...
        res["has_unhandled_datas"] = self.is_not_void_str(self.unhandled_datas_csv)
        return res
...
class FileGenerationJob(Job):
    ...
    # Wether to force or not the download of the result file (content-disposition: Attachment)
    force_download = Column(
        Boolean(),
        default=True,
    )
    ...
    def todict(self):
        ...
        res["error_messages"] = self.error_messages
        res["force_download"] = self.force_download
        return res


class BulkFileGenerationJob(Job):
    """
    A single job (re)generating several caerp.models.files.File
    """
