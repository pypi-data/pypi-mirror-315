...
class V1CreateCameraRequest(BaseModel):
    """
    ...
    pixels_x: StrictInt = Field(..., alias="pixelsX")
    pixels_y: StrictInt = Field(..., alias="pixelsY")
    ...
    sensor_xmm: Union[StrictFloat, StrictInt] = Field(..., alias="sensorXmm")
    sensor_ymm: Union[StrictFloat, StrictInt] = Field(..., alias="sensorYmm")
    unity_gain: StrictInt = Field(..., alias="unityGain", description="the gain level where e-/ADU = 1")
    optimal_nighttime_gain: StrictInt = Field(..., alias="optimalNighttimeGain")
    optimal_nighttime_gain_offset: StrictInt = Field(..., alias="optimalNighttimeGainOffset")
    optimal_nighttime_gain_mode: StrictInt = Field(..., alias="optimalNighttimeGainMode")
    supports_video_mode: StrictBool = Field(..., alias="supportsVideoMode", description="whether the camera supports video mode")
    adc_bit_depth: StrictInt = Field(..., alias="adcBitDepth")
    quantum_efficiency: Union[StrictFloat, StrictInt] = Field(..., alias="quantumEfficiency")
    gain_curves: Optional[conlist(V1GainCurve)] = Field(None, alias="gainCurves", description="gain curves for the camera")
    setup_actions: Optional[conlist(V1SetupAction)] = Field(None, alias="setupActions")
    is_color: StrictBool = Field(..., alias="isColor", description="whether the camera is monochrome or color (OSC)")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> V1CreateCameraRequest:
        """Create an instance of V1CreateCameraRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> V1CreateCameraRequest:
        """Create an instance of V1CreateCameraRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return V1CreateCameraRequest.parse_obj(obj)

        _obj = V1CreateCameraRequest.parse_obj({
            "model": obj.get("model"),
