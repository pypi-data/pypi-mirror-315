...
def echo(*data: str):
    ...
    print(const.EMPTY.join(data), end=const.EMPTY, flush=True)
...
class History:
    ...
    def __getitem__(self, pos: int):
        """Return entry at pos"""
        if pos < 1:
            raise HistoryError("Not available")
        #
        if pos == len(self):
            return const.EMPTY
        #
        try:
            ...
            raise HistoryError("Not available") from error
        #
        return found_entry
...
class LineBuffer:
    ...
    def redraw(self):
        ...
        rpos = len(self._buffer) - self._pos
        if rpos:
            echo(const.BS * rpos)
        #
    ...
    def insert_string(self, value: str) -> None:
        ...
        if not value.isprintable():
            raise ValueError("Can only insert fully printable strings")
        #
    ...
    def insert(self, character: str) -> None:
        """Insert _character at the cursor position"""
        if len(character) != 1:
            self.insert_string(character)
            return
        #
        if self._pos == len(self):
            self._pos += 1
