...
class ASA(UMnetNapalm, NetworkDriver):
    ...
    def get_config(
        ...
        if retrieve in ["all", "running"]:
            configs["running"] = self._send_command(
                "show running-config", flatten_result=True
            )
        if retrieve in ["all", "startup"]:
            configs["startup"] = self._send_command(
                "show startup-config", flatten_result=True
            )
    ...
    def get_ip_interfaces(self, context: str = "all") -> List[models.IPInterfaceDict]:
        ...
        sh_interface = self._send_command("show interface", context=context)
        ...
        for context, raw_output in sh_interface.items():
            output = textfsm_extractor(self, "sh_interface", raw_output)
            for row in output:
                results.append(
                    {
                        "ip_address": f'{row["ip"]}/{row["netmask"]}',
    ...
    def get_active_routes(self, context: str = "all") -> List[models.RouteDict]:
        ...
        for context, raw_output in sh_route.items():
            ...
            for row in output:
                ...
                if row["protocol"] not in ASA_PROTOCOLS:
                    raise UMnetNapalmError(f"Unknown protocol '{row['protocol']}'")

                protocol = ASA_PROTOCOLS[row["protocol"]]
