...
from bceidaas.bce_response import BceResponse
...
def _get_connection(protocol, host, port, connection_timeout_in_millis):
    """
    :param protocol
    :type protocol: bceidaas.protocol.Protocol
    :param endpoint
    :type endpoint: str
    ...
    if protocol.name == bceidaas.protocol.HTTP.name:
        return http.client.HTTPConnection(
            host=host, port=port, timeout=connection_timeout_in_millis / 1000
        )
    elif protocol.name == bceidaas.protocol.HTTPS.name:
...
def send_request(
    ...
    :param config
    :type config: bceidaas.BceClientConfiguration
    ...
    :param request:
    :type request: bceidaas.internal.InternalRequest

    :return:
    :rtype: bceidaas.BceResponse
    """
    _logger.debug(
    ...
    user_agent = "bce-idaas-sdk-python/%s/%s/%s" % (
        compat.convert_to_string(bceidaas.SDK_VERSION),
        sys.version,
        sys.platform,
    )
    ...
    if isinstance(body, str):
        body = body.encode(bceidaas.DEFAULT_ENCODING)
    if not body:
