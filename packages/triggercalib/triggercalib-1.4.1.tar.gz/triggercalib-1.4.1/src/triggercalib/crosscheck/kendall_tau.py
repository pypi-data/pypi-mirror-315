...
class KendallTau:
    ...
    def __init__(
        ...
        _signal_rdf = R.RDataFrame(*split_paths(signal_sample))
        _background_rdf = R.RDataFrame(*split_paths(background_sample))
        ...
        _, _, signal_pvalue = kendall_tau(
            _signal_df[self.discriminating_var], _signal_df[self.control_var]
        )
        _, _, background_pvalue = kendall_tau(
