...
class Plot:
    ...
    def _zfit_plot(self):
        ...
        data = self.data.value(self.observable).numpy()
        ...
        data_yvals, data_edges = np.histogram(data, bins=bin_edges)
        ...
        if plot_kwargs["ylog"]:
            plot_ax.set_ylim(
                max(0.5, component_min, min(data_yvals) / 10), 4 * max(pdf_yvals)
            )
