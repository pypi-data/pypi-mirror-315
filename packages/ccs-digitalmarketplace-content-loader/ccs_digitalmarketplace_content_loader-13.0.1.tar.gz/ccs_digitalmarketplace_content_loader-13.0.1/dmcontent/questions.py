...
from .utils import TemplateField, drop_followups, drop_dependent_follow_ups, get_option_value
...
class Multiquestion(Question):
    ...
    def filter(
        ...
        multi_question = super(Multiquestion, self).filter(
            context,
            dynamic=dynamic,
...
class DynamicList(Multiquestion):
    ...
    def filter(
        ...
        if not dynamic:
            return super(DynamicList, self).filter(
                context,
                dynamic=dynamic,
                inplace_allowed=inplace_allowed,
                ignore_depends_on_follow_up=ignore_depends_on_follow_up
...
class ListMultiquestion(Multiquestion):
    ...
    def summary(self, service_data, inplace_allowed: bool = False) -> "ListMultiquestionSummary":
...
class ListMultiquestionSummary(MultiquestionSummary, ListMultiquestion):
    def __init__(self, question, service_data):
