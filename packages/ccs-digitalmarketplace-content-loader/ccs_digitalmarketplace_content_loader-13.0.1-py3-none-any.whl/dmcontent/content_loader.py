...
from .questions import Question, ContentQuestion, Multiquestion, ListMultiquestion
from .messages import ContentMessage
from .metadata import ContentMetadata
from .utils import TemplateField, template_all, drop_followups, drop_dependent_follow_ups, LazyDict
...
class ContentManifest(object):
    ...
    def get_dependent_follow_up_fields(self):
        return list({
            form_field for section in self.sections for form_field in section.get_dependent_follow_up_fields()
        })
...
class ContentSection(object):
    ...
    def create(cls, section):
        if isinstance(section, ContentSection):
            ...
            return ContentSection(
                ...
                removable=section.get('removable'),
    ...
    def __init__(
        ...
        self.removable = removable if removable is not None else edit_questions
    ...
    def summary(self, service_data, inplace_allowed: bool = False) -> "ContentSection":
    ...
    def get_data(self, form_data):
        ...
        section_data = drop_dependent_follow_ups(self, section_data)
    ...
    def get_error_messages(self, errors: dict, question_descriptor_from: str = "label") -> OrderedDict:
        ...
        for question in self.questions:
            if isinstance(question, Multiquestion) and not isinstance(question, ListMultiquestion):
                questions = question.get_questions_flat()
            else:
                questions = [question]

            for sub_question in questions:
                errors_map.update(sub_question.get_error_messages(
                    errors,
    ...
    def get_dependent_follow_up_fields(self):
        """
        Return a list of field names that questions in this section have dependants on
        """
        return list({
            form_field for question in self.questions for form_field in question.get('dependent_follow_up', {}).keys()
        })
