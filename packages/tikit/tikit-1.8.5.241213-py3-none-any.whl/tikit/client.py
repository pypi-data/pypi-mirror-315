...
ALL_SUPPORT_TRAINING_MODES = ["PS_WORKER", "DDP", "MPI", "HOROVOD","SPARK"]
...
class Client(object):
    ...
    def describe_resource_groups(self, offset=0, limit=20, search_word="", tag_filters=None):
        """获取资源组列表
        ...
        try:
            ...
            if req.TrainingMode == "SPARK":
                if worker_resource.InstanceNum != 1:
                    raise TencentCloudSDKException(message='SPARK task only support one instance')
    ...
    def _convert_data_config(self, input_data_item) -> models.DataConfig:
        ...
        elif input_data_item.DataSource == "CFS" or input_data_item.DataSource == "CFSTurbo":
            assert input_data_item.CfsId
            storage_type = self._get_storage_type(input_data_item.CfsId)
            if  storage_type== "turbo":
                data_config.CFSTurboSource = models.CFSTurboSource()
                data_config.CFSTurboSource.Id = input_data_item.CfsId
                data_config.CFSTurboSource.Path = input_data_item.CfsPath 
                data_config.DataSourceType = "CFSTurbo"
            elif storage_type == "cfs":
    ...
    def _convert_notebook_data_config(self, input_data_item) -> models.DataConfig:
        ...
        data_config.DataSourceType = input_data_item.DataSource
        data_config.MappingPath = input_data_item.TargetPath
        if input_data_item.DataSource == "CFS" or input_data_item.DataSource == "CFS_TURBO":
            assert input_data_item.CfsId
            storage_type = self._get_storage_type(input_data_item.CfsId)
            if storage_type == "cfs":
                data_config.CFSSource = models.CFSConfig()
                ...
                data_config.CFSSource.Path = input_data_item.CfsPath
                data_config.DataSourceType = "CFS"
            elif storage_type == "turbo":
                data_config.CFSSource = models.CFSConfig()
                data_config.CFSSource.Id = input_data_item.CfsId
                data_config.CFSSource.Path = input_data_item.CfsPath
                data_config.DataSourceType = "CFS_TURBO"
    ...
    def stop_notebook(self, notebook_id):
        ...
        try:
            if len(notebook_id.strip()) == 0:
                raise TencentCloudSDKException(code="InvalidParameterValue", message='notebook id is empty')
