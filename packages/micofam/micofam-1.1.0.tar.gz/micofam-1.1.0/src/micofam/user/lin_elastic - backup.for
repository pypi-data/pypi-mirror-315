...
C*** CALCULATE ENERGY DENSITY AND DAMAGE EVOLUTION ***C
      IF ((TIME(1).GT.((2 * N + 1) * 0.01 - 0.005)).AND.(TIME(1).LT.
     & ((2 * N + 1) * 0.01 + 0.005))) THEN 
C
      dU = 2.0 * ((1.0 - 2.0 * nu) / (6.0 * (1.0 - D)  
     & * E)) * (S11 + S22 + S33) ** 2.0
      N0 = C1 * dU ** C2
      STATEV(1) = N0
C
...
C     & ((1.0 - D ** p) * E) * (S12 ** 2.0 +  S13 ** 2.0 + S23 ** 2.0)
     ...
     & * E)) * (S11 + S22 + S33) ** 2.0
      ...
      ENDIF
      ENDIF
      IF ((TIME(1).GT.((2 * N + 2) * 0.01 - 0.005)).AND.(TIME(1).LT.
     & ((2 * N + 2) * 0.01 + 0.005))) THEN
      D = STATEV(3) + dD
      N = STATEV(4) + 1
      IF (D.GT.1.0) THEN
      D = 0.99999
...
C*** STIFFNESS OF DAMAGED MODEL ***C
      ...
      cfull(1,1) = (1.0 - D) * (1.0 - nu) * E / atemp
      cfull(2,2) = (1.0 - D) * (1.0 - nu) * E / atemp
      cfull(3,3) = (1.0 - D) * (1.0 - nu) * E / atemp
      cfull(1,2) = (1.0 - D) * nu * E / atemp
      cfull(2,1) = (1.0 - D) * nu * E / atemp
      cfull(1,3) = (1.0 - D) * nu * E / atemp
      cfull(3,1) = (1.0 - D) * nu * E / atemp
      cfull(2,3) = (1.0 - D) * nu * E / atemp
      cfull(3,2) = (1.0 - D) * nu * E / atemp
      cfull(4,4) = (1.0 - D) * E / (2.0 * (1.0 + nu))
      cfull(5,5) = (1.0 - D) * E / (2.0 * (1.0 + nu))
      cfull(6,6) = (1.0 - D) * E / (2.0 * (1.0 + nu))
C
C*** SET STATE VARIABLES ***C 
      ...
      STATEV(8) = ELSTAT
C      IF (NOEL.EQ.1) THEN
C      CALL URDFIL(LSTOP = 0, LOVRWRT = 0, KSTEP, KINC, DTIME, TIME)
C      END IF
C
...
C      IF (JRCD.NE.0) KEY = JRRAY(1,2)
C      IF  (KEY.EQ.3) PRINT ARRAY
C      END DO
CC
C      RETURN
C      END