...
version_conf_path = os.path.join(script_dir, 'Version.conf')
server_conf_path = os.path.join(script_dir, 'Server.conf')
blockip_conf_path = os.path.join(script_dir, 'blacklist.conf')

configs = json.load(open(version_conf_path))[os.path.basename(sys.argv[0]).split('.')[0]]
...
class Main():
    ...
    def __init__(self,args):
        ...
        if self.__check_parameters(args):
            ...
            self.__DEVICE_TYPE = {}
            self.DEV_BLACKLIST = set(json.load(open(blockip_conf_path))['DEVICE']['black-list'])
            self.HACK_BLACKLIST = set(json.load(open(blockip_conf_path))['HACKER']['black-list'])
            for i in range(1, int(self._CONFIG['MAX_DEV'])+1):
    ...
    def __run(self):
        ...
        device_thread.start()
        logger.log("System", f" [INFO] Device access module loading completed --- Max_devices [{self._CONFIG['MAX_DEV']}]...")
        hacker_thread.start()
    ...
    def __check_parameters(self,args):
        try:
            self._CONFIG=json.load(open(server_conf_path))
            ...
            self.__MAXDEV = int(self._CONFIG['MAX_DEV'])
            self.__KEYH = args.KEYH
            self.__KEYV = args.KEYV
            if self._CONFIG['DINGTALK']['TOKEN'] and self._CONFIG['DINGTALK']['SECRET']:
                self.DINGTALK = DingTalk(False)
                self.DINGTALK.set_token(self._CONFIG['DINGTALK']['TOKEN'])
                self.DINGTALK.set_secret(self._CONFIG['DINGTALK']['SECRET'])
    ...
    def __send_heartbeat(self):
        while True:
            for dev_id in range(1, int(self._CONFIG['MAX_DEV'])+1):
                client_socket = self.__DEVICE_IDS[dev_id]
                if client_socket:
                    try:
                        data=self._CONFIG['HEART']
                        if self._CONFIG['DEV_ENCRYPTION_Server'][self.__DEVICE_TYPE[dev_id]]:
            ...
            time.sleep(int(self._CONFIG['HEART_TIMEOUT']))
    ...
    def __control_device(self,device_id, client_socket, command):
        ...
        try:
                ...
                if device_socket:
                    if self._CONFIG['DEV_ENCRYPTION_Server'][self.__DEVICE_TYPE[device_id]] :
                    ...
                    #     pass
                        # if respon:
                        #     if self._CONFIG[self.__DEVICE_TYPE[device_id]]['DEV_RUN_RECV'] == str(respon):
                        ...
                        #     elif  self._CONFIG[self.__DEVICE_TYPE[device_id]]['DEV_STOP_RECV'] == respon:
                        #         self.__DEVICE_STATE[device_id] = 0
    ...
    def __construct_device_payload(self,stat,task):
        return f'{stat}:{task}'
    ...
    def __server_hacker(self):
        ...
        def handle_client(client_socket, client_address):
            ...
            client_socket.settimeout(int(self._CONFIG['HACK_AUTH_TIMEOUT']))
            try:
                while True: # 这里开始获取数据包
                    ...
                    try:
                        ...
                        task = int(received_string[16:19], 16)
                        sig = received_string[20:]
                        ...
                        if result and htimestamp == result and current_time - htimestamp <= int(self._CONFIG['HACK_PACK_TIMEOUT']):
                            ...
                            if state == 1 or state == 2:
                                payload = self.__construct_device_payload(task,state)
                                if device_id == 0:
                                    ...
                                    if len(device_ids_true_id) > 0:
                                        for id in device_ids_true_id:
                                            try:
                                                self.__control_device(id, client_socket,payload)
                                            except Exception as e:
                                    ...
                                    self.__control_device(device_id, client_socket, payload)
                            elif state == 3:
    ...
    def __server_device(self):
        ...
        def handle_client(client_socket, client_address):
            ...
            client_socket.settimeout(int(self._CONFIG['DEV_AUTH_TIMEOUT']))
            try:
                ...
                if data['TYPE'] not in self._CONFIG['DEV_TYPE']:
