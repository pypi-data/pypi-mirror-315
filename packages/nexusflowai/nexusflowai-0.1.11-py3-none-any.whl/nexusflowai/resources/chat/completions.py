...
from nexusflowai.types import NexusflowAIChatCompletion, NexusflowAIChatCompletionChunk
...
__all__ = ["Completions", "AsyncCompletions", "ChatCompletionsInputValidator"]
...
class Completions(SyncAPIResource):
    ...
    def create(
        ...
        *,
    ...
    ) -> NexusflowAIChatCompletion | Stream[NexusflowAIChatCompletionChunk]:
        """
        ...
        res = self._post(
            ...
            cast_to=NexusflowAIChatCompletion,
            stream=stream or False,
            stream_cls=Stream[NexusflowAIChatCompletionChunk],
        )

        if not isinstance(res, Stream):
            maybe_print_hints(res)

            for choice in res.choices:
                parsed = choice.message.parsed
                choice.message.parsed = maybe_parse_into_pydantic(
...
class AsyncCompletions(AsyncAPIResource):
    ...
    async def create(
        ...
        *,
        messages: List[ChatCompletionMessageParam],
        model: Union[str, ALLOWED_MODELS],
        frequency_penalty: Optional[float] | NotGiven = NOT_GIVEN,
        function_call: completion_create_params.FunctionCall | NotGiven = NOT_GIVEN,
        functions: List[completion_create_params.Function] | NotGiven = NOT_GIVEN,
        logit_bias: Optional[Dict[str, int]] | NotGiven = NOT_GIVEN,
        ...
        extra_body: Body | None = None,
        timeout: float | httpx.Timeout | None | NotGiven = NOT_GIVEN,
    ) -> NexusflowAIChatCompletion | AsyncStream[NexusflowAIChatCompletionChunk]:
        """
        Creates a model response for the given chat conversation.
        ...
        res = await self._post(
            ...
            cast_to=NexusflowAIChatCompletion,
            stream=stream or False,
            stream_cls=AsyncStream[NexusflowAIChatCompletionChunk],
        )
    ...
    def batch_create(
        self,
        batch: List[ChatCompletionCreateParams],
        num_calls_in_parallel: Optional[int] = None,
    ) -> List[Union[NexusflowAIChatCompletion | Exception]]:
        """
        Will run the batched messages in an async manner
        """
        return batch_call(self.create, batch, num_calls_in_parallel)

    def batch_run_messages(
        self,
        batch_messages: List[List[ChatCompletionMessage]],
        params: ChatCompletionCreateParams,
        num_calls_in_parallel: Optional[int] = None,
    ) -> List[Union[NexusflowAIChatCompletion | Exception]]:
        """
        This function takes in a list of messgaes of type ChatCompletionMessage, appends other parameters (like modelname, temperature, etc) to it and creates a list of ChatCompletionCreateParams objects. These objects are then passed to the batch_create function to be run asynchronously.

        Args:
        batch_messages: A list of messages of type ChatCompletionMessage
        params: A ChatCompletionCreateParams object which is essentially a TypedDict. The key - value pairs should correspond to the input parameters - parameter types of the AsynCompletions.create function.

        Returns:
        A list of NexusflowAIChatCompletion objects.
        """

        if params.get("stream", False):
            raise ValueError("Batch create does not support streaming requests!")

        batched_messages: List[ChatCompletionCreateParams] = [
            {**params, "messages": chat_completion_message}
            for chat_completion_message in batch_messages
        ]

        return self.batch_create(batched_messages, num_calls_in_parallel)
