...
from databricks.feature_store.online_lookup_client import (
    ...
    should_use_brickstore_http_gateway,
...
from databricks.ml_features_common.protos.feature_store_serving_pb2 import DataType
...
class _FeatureStoreModelWrapper:
    ...
    def _is_model_eligible_for_batch_lookup(
        ...
        ) or should_use_brickstore_http_gateway(online_feature_tables)
    ...
    def _predict(
        ...
        self._latency_debug("validation")
        ...
        self._latency_debug("predict")

        self._latency_debug_print()
        return prediction
    ...
    def _latency_debug_init(self):
        """
        Initializing the latency debugger. An entry with tag "start" is added to the debug log.
        This is a no-op if performance debugging is not enabled by the env var.
        """
    ...
    def _latency_debug(self, tag):
        """Add an entry to performance debugging log."""
        if self._latency_debugger:
            self._latency_debugger.append((tag, self._current_time_ms()))

    def _latency_debug_print(self):
        """
        If performance debugging is enabled, print the time elapsed from the init of the debugger in
        milliseconds. Format:
        Latency Debugger: [(TAG1, t1), (TAG2, t2), ...]
        """
        if self._latency_debugger:
            # the first entry was added by _latency_debug_init
            start = self._latency_debugger[0][1]
            latencies = [(tag, t - start) for (tag, t) in self._latency_debugger]
            _logger.warning(f"Latency Debugger: {latencies}")
    ...
    def _augment_with_materialized_features(
        ...
        *,
        ...
        self._latency_debug("lk_partial")
    ...
    @staticmethod
    def _validate_lookup_key_datatype(
        df: pd.DataFrame,
        ...
        for pk, lookup_name in zip(primary_keys, lookup_key):
            if pk.data_type == DataType.STRING and not pd.api.types.is_string_dtype(
                dtypes_dict[lookup_name]
            ):
                raise ValueError(
                    f"Provided primary keys {pk.name} is not of type string"
                )

    def _get_primary_key_list(
        self,
        lookup_key: LookupKeyType,
        primary_keys: List[PrimaryKeyDetails],
        df: pd.DataFrame,
        ...
        self._validate_lookup_key_datatype(lookup_key_columns, primary_keys, lookup_key)
