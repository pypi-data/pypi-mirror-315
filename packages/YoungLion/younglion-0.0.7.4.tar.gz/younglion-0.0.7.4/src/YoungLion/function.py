...
class File:
    ...
    def latex_to_image(self, latex_formula: str, output_file: Optional[str] = None) -> bytes:
        """
        Converts a LaTeX formula to an image using MathText from Matplotlib.
        ...
        """
        # Ensure the formula is wrapped with math mode symbols only if necessary
        if not (latex_formula.startswith("$") and latex_formula.endswith("$")):
            latex_formula = f"${latex_formula}$"

        # Create a figure
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.text(0.5, 0.5, latex_formula, fontsize=20, ha='center', va='center', usetex=False)
        ax.axis('off')
