...
class Metabase(object):
    ...
    # Main 1
    def query(self, url, format='json', filter=None, filter_chunk_size=2000, preview_endpoint=False):
        '''
        ...
        :param preview_endpoint: Use it when the default API endpoint does not work with the filter in bulk. Remember that It is suitable for unique object queries.
        ...
        if preview_endpoint and format != 'json':
            raise ValueError('Preview endpoint only support for JSON format.')

        if preview_endpoint and filter_chunk_size > 2000:
            raise ValueError('Preview endpoint response only 2000 rows. filter_chunk_size must be less than or equal to 2000.')

        result = asyncio.run(self.handle_urls(urls=url, format=format.lower(), filters=filter, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint))
    ...
    # Async for URL query
    async def handle_urls(self, urls, format='json', filters=None, filter_chunk_size=2000, preview_endpoint=False):
        '''
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit_per_host=self.limit_per_host, verify_ssl=False), timeout=aiohttp.ClientTimeout(total=self.timeout)) as session:

            # 1 URL 1 filter
            if not isinstance(urls, list) and not isinstance(filters, list):
                url_type = define_url(url=urls)
                if url_type == 'sql':
                    return await self.SQL.query_url(session=session, url=urls, format=format, filters=filters, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint)
                elif url_type == 'card':
                    return await self.Card.query_card(session=session, url=urls, format=format, filters=filters, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint)
                elif url_type == 'dataset':
                    return await self.Dataset.query_dataset(session=session, url=urls, format=format, filters=filters, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint)
            ...
            # Make sure URL list and Filter list are the same length.
                ...
                for url, f in zip(urls, filters):
                    ...
                    if url_type == 'sql':
                        task = asyncio.create_task(self.SQL.query_url(session=session, url=url, format=format, filters=f, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint))
                        task.url = url
                        task.filter = f
                        tasks.append(task)
                    elif url_type == 'card':
                        task = asyncio.create_task(self.Card.query_card(session=session, url=url, format=format, filters=f, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint))
                        task.url = url
                        task.filter = f
                        tasks.append(task)
                    elif url_type == 'dataset':
                        task = asyncio.create_task(self.Dataset.query_dataset(session=session, url=url, format=format, filters=f, filter_chunk_size=filter_chunk_size, preview_endpoint=preview_endpoint))
                        task.url = url
                        task.filter = f
                        tasks.append(task)
    ...
    async def export(self, session, url, form_data=None, json_data=None, format='json', column_sort=None):
        '''
        ...
        if form_data:
            ...
            headers = {'Content-Type': 'application/json', 'X-Metabase-Session': self.metabase_session}
            # print(json_data)
        ...
        async def handler():
            ...
            try:
                response = await session.post(url, headers=headers, data=form_data, json=json_data)
                ...
                self.print_if_verbose(f'Query {query_number} got an error: {e}')
                raise e
