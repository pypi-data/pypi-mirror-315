...
from httpx import Client as httpxClient, HTTPStatusError
...
def record(client: httpxClient, deployment_id: str, resource: Resource | Ref, state: str):
    ...
    res = client.request(
        "post",
        "/api/api/customentities/~deployment",
        json={
            "displayName": resource.id,
            "description": "...",
            "identifiers": [
                {
                    "identifierScope": "deployment",
...
def fetch_log_entries(client: httpxClient, params: Dict):
    ...
    while True:
        ...
        res = client.get("/api/api/customentities/~deployment", params=pp).json()
...
def entry_to_logline(entry: Dict):
    indexed = {
        "identifiers": {
            i["identifierType"]: i["identifierValue"]
...
def list_deployments(client: httpxClient) -> List[str]:
    log_lines = [
        entry_to_logline(e)
        for e in fetch_log_entries(client, {})
    ]
...
def get_resource(client: httpxClient, deployment_id: str, resource_id: str) -> List[LogLine]:
    ...
    log_lines = [
        entry_to_logline(e)
        for e in fetch_log_entries(client, {"filter": list_filter})
    ]
    return log_lines
...
def print_tree(index: Dict[str, LogLine], deps: DepList, start_at: str, marker: str) -> None:
    ...
    def helper(resource_id, depth):
        if depth == 0:
            print(resource_id,
                  f"({index[resource_id].resource_type})"
                  if index.get(resource_id, None) else "Unknown Type")
        else:
            print(leader * depth, marker if depth > 0 else "", resource_id,
                  f"({index[resource_id].resource_type})"
                  if index.get(resource_id, None) else "Unknown Type")
...
def remove(client: httpxClient, deployment_id: str, resource_id: str) -> str:
    ...
    res = client.delete(
        f"/api/api/customentities/~deployment/resource/{identifier_value}",
        params={"identifierScope": "deployment"},
    )
...
def identifier_create(client: httpxClient) -> Optional[Dict]:
    try:
        ...
        print("Created identifier", res.json())
        return res.json()
...
def entity_def_get(client: httpxClient) -> Optional[Dict]:
    try:
        res = client.get("/api/api/customentitytypes/~deployment")
        return res.json()
    except HTTPStatusError as e:
        if (
...
def entity_def_create(client: httpxClient) -> Dict:
    body = {"entityTypeName": "deployment"} | entity_definition
    res = client.request("post", "/api/api/customentitytypes", json=body)
    return res.json()


def entity_def_update(client: httpxClient) -> Dict:
    res = client.request("put", "/api/api/customentitytypes/~deployment", json=entity_definition)
    return res.json()
