...
from .BackoffHandler import BackoffHandler
...
def start_query(client: httpx.Client, sql: str) -> str:
    return client.put(
        ...
        content=sql,
        headers={"Content-type": "text/plain"}).json()["executionId"]
...
def wait_for_background(client, execution_id) -> bool:
    ...
    match status:
        ...
        case TaskStatus.CREATED | TaskStatus.WAITING_FOR_ACTIVATION | TaskStatus.WAITING_TO_RUN \
...
class ViewResource(BaseModel, Resource):
    ...
    def delete(client, old_state):
        ...
        for i in range(1, ViewResource.Registration.tries + 1):
            if ViewResource.registration_version(
                    client=client,
                    view_name=old_state.provider,
                    backoff_handler=backoff) is None:
                break
            else:
