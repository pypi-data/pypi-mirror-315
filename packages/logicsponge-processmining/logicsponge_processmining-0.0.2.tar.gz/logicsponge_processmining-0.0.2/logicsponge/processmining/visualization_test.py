...
from logicsponge.processmining.algorithms_and_structures import BaseStructure, FrequencyPrefixTree, NGram
from logicsponge.processmining.streaming import IteratorStreamer
...
def generate_graph_step(pm: BaseStructure):
    """Pipeline step to generate Cytoscape elements from the pm object."""
    elements = []
...
class Update(ls.FunctionTerm):
    def __init__(self, *args, model: BaseStructure, **kwargs):
    ...
    def run(self, ds_view: ls.DataStreamView):
        ds_view.next()
        item = ds_view[-1]
        self.model.update(item["case_id"],item["action"])


class GenerateGraph(ls.FunctionTerm):
    def __init__(self, *args, model: BaseStructure, **kwargs):
        super().__init__(*args, **kwargs)
        self.model = model
    ...
    def run(self, ds_view: ls.DataStreamView):
        ds_view.next()
        generate_graph_step(self.model)
...
sponge = (
    streamer
    * ls.KeyFilter(keys=["case_id", "action"])  # Ensure only relevant keys are passed
    * Update(model=pm)  # Update the pm object
    * GenerateGraph(model=pm)  # Generate Cytoscape elements
    * ls.Print()  # Output the elements for visualization
)
