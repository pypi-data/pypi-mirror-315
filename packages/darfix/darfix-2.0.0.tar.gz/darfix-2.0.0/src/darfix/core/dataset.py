...
from typing import Literal, Sequence, Tuple, Optional, Generator, Union, Dict
...
class ImageDataset:
    ...
    :param isH5: True if the data is containined into HDF5 file
    ...
    def __init__(
        ...
        self.moments_dims = {}
        self.state_of_operations = _StateOfOperations()
        self._dir = _dir
    ...
    def find_dimensions(self, kind: int, tolerance=1e-9) -> None:
        """
        ...
        all_values = numpy.asarray(
            [self.get_metadata_values(kind=kind, key=key) for key in keys]
        )

        dimensions = []
        dataset_size = len(self.data.metadata)
        # For every key that has more than one different value, create a new Dimension.
        for key, values in zip(keys, all_values):
            ...
            if unique_counts[0] == dataset_size:
                continue
            try:
                dimension = Dimension(kind, key, tolerance=tolerance)
                dimension.set_unique_values(numpy.array(unique_values, dtype=float))
                _logger.info(
                ...
                # of the reshaping.
                dimension.changing_value = self.__compute_changing_value(values)
                dimensions.append(dimension)
            except ValueError:
                _logger.warning("Couldn't convert to float")

        for dimension in sorted(
            dimensions, key=lambda x: x.changing_value, reverse=True
        ):
    ...
    def get_metadata_values(
        self, kind: int, key, indices=None, dimension=None
    ) -> numpy.ndarray:
    ...
    def __compute_changing_value(self, values, changing_value=1):
        ...
        if len(numpy.unique(values)) > 1:
            return self.__compute_changing_value(
        ...
        if not self._dimensions_values or indices is not None:
            for dimension in self.__dims.values():
                self._dimensions_values[dimension.name] = self.get_metadata_values(
                    kind=dimension.kind, key=dimension.name, indices=indices
                )
        return self._dimensions_values

    def compute_mosaicity_colorkey(
        self, dimensions=None, scale=100, indices=None, third_motor=None
    ):
        """
        ...
        if dimensions is None:
            dimensions = (0, 1)
        # only work with 2 dimensions
        assert len(dimensions) == 2, "Only works with two motors"
        ...
        for axis in dimensions:
            dim = self.__dims.get(axis)
            assert dim is not None
            dims_list.append(numpy.linspace(-1, 1, dim.size * scale))
        ...
        sqr = numpy.sqrt(
            numpy.power(mesh[0], 2) + numpy.power(mesh[1], 2)
        ) / numpy.sqrt(2)
        hsv_key = numpy.stack(
            (
                normalized_data,
                sqr,
                numpy.ones((len(dims_list[1]), len(dims_list[0]))),
            ),
            axis=2,
        )
    ...
    def apply_shift_along_dimension(
        ...
        dataset = self
        for value in range(self.dims.get(dimension[0]).size):
            data, rindices = self.get_data(
                indices=indices, dimension=[dimension[0], value], return_indices=True
            )
            frames = numpy.arange(
                self.get_data(indices=indices, dimension=[dimension[0], value]).shape[0]
            )
            dataset = dataset.apply_shift(
                numpy.outer(shift[value], frames),
                [dimension[0], value],
                shift_approach,
                indices,
                callback,
    ...
    def compute_transformation(
        ...
        if not self.dims:
            raise ValueError(
                "The transformation matrix needs defined dimensions to be computed. Run `find_dimensions` before `compute_transformation`."
            )
        ...
        def get_dataset(name) -> float | numpy.array | None:
            dataset = self.get_metadata_values(POSITIONER_METADATA, name)
    ...
    def project_data(self, dimension: Sequence[int], indices=None, _dir=None):
        ...
        if not self.dims:
            raise ValueError(
                "Data projection needs to have defined dimensions. Run `find_dimensions` before `project_data`."
            )
    ...
    def compute_rsm(
        ...
        diffry = self.get_metadata_values(POSITIONER_METADATA, "diffry")
...
class Data(numpy.ndarray):
    ...
    def __new__(cls, urls, metadata, in_memory=True, data=None):
        ...
        if in_memory:
            ...
            if data is None:
                ...
                if urls.size == 0:
                    ...
                    img = utils.get_data(next(urls.flat))
                    data = numpy.empty(url_shape + img.shape, img.dtype)
                    for url, *url_idx in zip(urls.flat, *url_idxs):
                        data[tuple(url_idx)] = utils.get_data(url)
            obj = data.view(cls)
        else:
        ...
        obj.state_of_operations = _StateOfOperations()
...
def extract_metadata_values(
    ...
    for data in metadata:
        ...
        try:
            value = data.get_value(kind=kind, name=key)
        except KeyError:
