...
from simgen.generation_utils import (calculate_path_length,
...
from simgen.utils import (get_hydromace_calculator, get_mace_config,
                           get_mace_similarity_calculator,
                           get_system_torch_device_str, setup_logger,
                           time_function)
...
class GenerationServer:
    ...
    def generate(self, run_params: dict, common_data: CommonData) -> dict:
        ...
        json_atoms = [atoms_to_json(x) for x in trajectories]
        return {"atoms": json_atoms}
    ...
    def relax(self, run_params: dict, common_data: CommonData) -> dict:
        ...
        json_atoms = [atoms_to_json(x) for x in relaxed_atoms]
        return {"atoms": json_atoms}
    ...
    def _hydrogenate_by_bond_lengths(self, atoms, moldiff_calc, max_steps: int) -> dict:
        ...
        if not found:
            ...
            logging.info("Done")
            json_atoms = [hydrogenated_atoms, relaxed]
            json_atoms = [atoms_to_json(x) for x in json_atoms]
            return {"atoms": json_atoms}
...
if __name__ == "__main__":
    ...
    mol = build.molecule("C6H6")
    json_mol = atoms_to_json(mol)
    params = {"atoms_per_angstrom": 1.5, "strength": 5}
    name = "generate"
    data = {"name": name, "params": params, "json_mol": json_mol}
    print(json.dumps(data))
    resp = requests.get("http://localhost:8000/", data=json.dumps(data))
    print(resp)
    print(resp.json())
