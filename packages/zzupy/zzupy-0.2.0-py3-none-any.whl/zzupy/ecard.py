...
class eCard:
    ...
    def recharge_electricity(self, room: str,paypasswd: str) -> tuple:
        ...
        response = httpx.post('https://ecard.v.zzu.edu.cn/server/auth/getEncrypt', cookies=cookies, headers=headers)
        pay_id=json.loads(response.text)["resultData"]["id"]
        encrypted_public_key=json.loads(response.text)["resultData"]["publicKey"]
        # 解密被加密的公钥
        public_key=sm4_decrypt_ecb(base64.b64decode(encrypted_public_key), bytes.fromhex("773638372d392b33435f48266a655f35"))
        # 请求体明文
        json_data = {
        ...
        data = {
            "id": pay_id,
            "params": (encrypted_params.hex())[2:]
        }
        response = httpx.post('https://ecard.v.zzu.edu.cn/server/utilities/pay', cookies=cookies, headers=headers, json=data)
        return json.loads(response.text)["success"],json.loads(response.text)["message"]
    ...
    def get_balance(self) -> float:
        ...
        headers = {
            'User-Agent': self._parent._DeviceParams["userAgentPrecursor"] + "uni-app Html5Plus/1.0 (Immersed/38.666668)",
            'Connection': 'Keep-Alive',
        ...
        response = httpx.get('https://info.s.zzu.edu.cn/portal-api/v1/thrid-adapter/get-person-info-card-list',headers=headers)
    ...
    def get_remaining_power(self, room: str) -> float:
        ...
        response = httpx.post('https://ecard.v.zzu.edu.cn/server/utilities/account', cookies=cookies,headers=headers, json=data)
