...
from .setup import VerboseLevel, logger
...
class CosmosClient:
    ...
    def _make_request(
        ...
        if self.verbose >= VerboseLevel.DEBUG:
            ...
            self._log(f"Request Files: {files}", VerboseLevel.DEBUG)
    ...
    def translate_file_request(
        ...
        output_language: str,
        input_language: str | None = None,
        return_type: str | FileTranslationReturnType = FileTranslationReturnType.RAW_TEXT,
    ) -> dict[str, Any] | None:
    ...
    def files_parse_request(
        self: "CosmosClient",
        filepath: Path,
        extract_type: str | ParserExtractType = ParserExtractType.CHUNKS,
        k_min: int | None = None,
        k_max: int | None = None,
        ...
        data = {
            "extract_type": extract_type if isinstance(extract_type, str) else extract_type.value,
            "k_min": k_min,
            "k_max": k_max,
            "overlap": overlap,
    ...
    def files_index_create_request(
        ...
        files_upfiles = [("files", (filepath.name, filepath.open("rb"))) for filepath in files_paths]
    ...
    def files_index_ask_request(
        ...
        if active_files_hashes:
            data["active_files_hashes"] = json.dumps(active_files_hashes)
