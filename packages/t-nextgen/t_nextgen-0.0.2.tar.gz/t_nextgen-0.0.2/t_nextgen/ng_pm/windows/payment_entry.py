...
class PaymentEntryWindow(NextGenWindow):
    ...
    def __init__(self, app_path: str, textract: Optional[Textract]):
    ...
    def set_all_service_lines_to_appeal(self) -> None:
        ...
        Args:
            note (str): Note to be set in the transaction notes.
        """
        # Look at set_encounter_to_appeal_function
        logger.info("Setting Encounter to Appeal...")
    ...
    def is_line_voided(
        self, row: ListItemWrapper, column_name: str = "Rendering", image_path: Path = Path("C:\\temp\\")
    ) -> bool:
    ...
    def is_text_strikethrough(self, element: Any, image_path: Path) -> bool:
        ...
        element_name_with_date = f"{element.window_text()}_{datetime.now().strftime('%H_%M_%S')}.png"
    ...
    def click_open_button(self) -> None:
        ...
        open_button = self.desktop_app.dialog.child_window(title="_cmdAction_1", control_type="Button")
    ...
    def insert_text_in_ln_item_rsns(self, text: str, row: ListItemWrapper) -> None:
        ...
        if self.is_payer_a_patient():
            logger.info("The payer is Patient. Ln Item Rsns field is disabled")
            return
    ...
    def select_status_in_service_row(self, pane: WindowSpecification, status: str, status_key: str = "") -> None:
        ...
        if status_key:
            ...
            combo_box = pane.child_window(control_type="ComboBox")
            combo_box.child_window(auto_id="mcbo_EmbeddableTextBox", control_type="Edit").set_text(status)
            pane.click_input()
    ...
    def click_recalc_button(self, click_recalc_button: bool = True) -> None:
        ...
        if click_recalc_button:
            ...
            logger.warning("Skipping recalc button click to prevent changes.")
    ...
    def click_save_button(self, click_save_button: bool = True) -> None:
        ...
        if click_save_button:
            ...
            logger.warning("Clicking cancel in payment entry to prevent changes.")
            self.click_cancel()

    def click_cancel(self) -> None:
        """Clicks the cancel button on the payment entry window."""
        self.desktop_app.click_button_by_element_name(element_name="_cmdAction_3")
    ...
    def verify_and_handle_modal(self, expected_text: str) -> None:
        ...
        """
        success, modal_text = self.desktop_app.close_modal()

        if success and modal_text.upper() == expected_text.upper():
    ...
    def get_transaction_notes_text(self) -> str:
        ...
        """
        transaction_note_input = self.desktop_app.dialog.child_window(
            auto_id="txtTransactionNotes", control_type="Edit"
        )
    ...
    def click_new_button(self) -> None:
        """Click New Button."""
        self.desktop_app.click_button_by_element_name(element_name="_cmdAction_0")
    ...
    def enter_text_in_enc_clm_field(self, text: str) -> None:
        ...
        with contextlib.suppress(_ctypes.COMError):
            ...
            container.set_focus()
            container.child_window(title="_txtActiveSearch_103", control_type="Edit").set_text(text)
            send_keys("{ENTER}")
    ...
    def enter_text_in_tracking_field(self, text: str) -> None:
        ...
        with contextlib.suppress(_ctypes.COMError):
            container = self.desktop_app.dialog.child_window(auto_id="vseContainer", control_type="Text")
            container.set_focus()
            container.child_window(title="_txtDataEntry_105", control_type="Edit").set_text(text)
            send_keys("{ENTER}")

    def click_close_nextgen_alerts(self) -> None:
        """Click close button on NextGen Alerts."""
        alert = self.desktop_app.dialog.child_window(title="NextGen Alerts", control_type="Window")
        if alert.exists(timeout=5, retry_interval=1):
            alert.set_focus()
            alert.child_window(title="Close", control_type="Button").click()

    def click_yes_encounter_in_history_status_pop_up(self) -> None:
        """Click yes button in the encounter in history status pop-up."""
        if self.desktop_app.dialog.child_window(
            ...
            control_type="Text",
        ).exists(timeout=1, retry_interval=1):
            self.desktop_app.dialog.child_window(title="Yes", control_type="Button").click_input()
    ...
    def click_shield_icon(self) -> None:
        """Click shield icon in pyament entry window."""
    ...
    def select_payer_in_drop_down(self, payer: str) -> None:
        ...
        for option in options:
            ...
            if payer.lower() in value.lower():
                combo_box.child_window(control_type="Edit", found_index=0).set_text(value)
                return
    ...
    def skipping_payer_aarp_pop_up_exist(self) -> bool:
        ...
        """
        if self.desktop_app.dialog.child_window(auto_id="65535", control_type="Text").exists(
    ...
    def set_text_in_deduct_field(self, row: ListItemWrapper, deduct_amt: Decimal) -> None:
        ...
        for item in items:
            ...
            if "deduct" in window_text.lower():
                ...
                self.check_if_skipping_payer_aarp_pop_up_exist()
                return
