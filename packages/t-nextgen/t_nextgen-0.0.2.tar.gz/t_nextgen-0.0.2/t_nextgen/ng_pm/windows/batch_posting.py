...
class BatchPostingWindow(NextGenWindow):
    ...
    def window(self):
        ...
        return self.desktop_app.child_window(auto_id="NGEPMBatchLookup", control_type="Window").wait("visible", 5)

    @retry(tries=3, delay=2)
    def click_batch_icon_from_bar(self, practice_name: str) -> None:
        """Clicks the Batch Posting button from the menu bar."""
        try:
            batch_icon = self.desktop_app.child_window(auto_id="cmdToolPosting")
    ...
    def click_batch_from_window_menu_if_exist(self) -> bool:
        ...
        """
        windows_menu = self.desktop_app.child_window(title="Window", control_type="MenuItem")
    ...
    def _deal_with_no_batch_found_popup(self) -> None | NoMatchingBatchDescriptionException:
        modal = self.desktop_app.child_window(title="NextGen", control_type="Window")
    ...
    def run_batch_lookup(self, trn: str, amount: Decimal, fill_amount: bool = False) -> None | ListItemWrapper:
        ...
        """
        logger.info(f"Running batch lookup: trn={trn}, amount={amount}")
        self._click_menu_icon("a")
        ...
        data_items = self._get_data_items(tree_view, "Description")
    ...
    def _click_menu_icon(self, key: str) -> None:
    ...
    def _get_batch_window(self) -> WindowSpecification:
        ...
        batch_window = self.desktop_app.child_window(title="BatchPosting", control_type="Window")
        return batch_window
        ...
        for d in dicts:
            if "Description" in d and description in d["Description"]:
                ...
                if amt_extracted is None:
                    logger.info("Unable to determine amount value from batch description")
                elif amt == Decimal(amt_extracted):
                    result.append(d)
        return result

    def _extract_amt_from_description(self, description: str) -> str:
    ...
    def check_boxes_in_batch_posting(self, current_phase: str) -> None:
        ...
        checkboxes = []
        checkboxes.append(
            self.desktop_app.child_window(auto_id=checkbox_ids[0], control_type="CheckBox").wait("visible", 10)
        )
        for checkbox_id in checkbox_ids:
            checkboxes.append(self.desktop_app.child_window(auto_id=checkbox_id, control_type="CheckBox"))
    ...
    def set_focus_in_batch_window(self):
        ...
        window = self.desktop_app.child_window(title="BatchPosting", control_type="Window")
    ...
    def click_ok_quick_batch(self) -> None:
        ...
        ok_button = self.desktop_app.child_window(title="cmdOK", control_type="Button")
        with contextlib.suppress(_ctypes.COMError):
            ok_button.click()
    ...
    def _maximize_batch_window(self) -> None:
        """This method maximizes the batch window."""
        try:
            if not self.window.was_maximized():
                self.window.maximize()
            time.sleep(2)
        except Exception as e:
            logger.warning(f"Could not maximize the batch window: {str(e)}")
    ...
    def get_batch_row(self, trn: str, amount: str) -> None | ListItemWrapper:
        ...
        """
        div = self.desktop_app.child_window(auto_id="Data Area", control_type="Custom", found_index=0)
        tree_view = div.child_window(auto_id="ColScrollRegion: 0, RowScrollRegion: 0")
    ...
    def close_batch_look_up_window(self) -> None:
        ...
        batch_lookup_window = self.desktop_app.child_window(title="Batch Lookup", control_type="Window")
    ...
    def enter_quick_batch_name(self, name: str) -> None:
        ...
        """
        quick_batch_window = self.desktop_app.child_window(auto_id="NGEPMQuickBatch", control_type="Window")
    ...
    def press_l_to_open_ledger_in_dropdown(self) -> None:
        ...
        send_keys("l")
        self.desktop_app.child_window(title="BatchLedger", control_type="Window").wait("visible", 10)
