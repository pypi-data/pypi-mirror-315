...
class EncounterInsuranceWindow(NextGenWindow):
    ...
    def window(self):
        ...
        return self.desktop_app.child_window(title="frmEncounterPayer", control_type="Window")
    ...
    def click_right_arrow_in_encounter_insurance_selection_window(self) -> None:
        ...
        self.desktop_app.child_window(title="cmdAdd", control_type="Button").click_input()

    def click_ok_in_encounter_insurance_selection_window(self) -> None:
        """Click ok in Encounter Insurance Selection Window."""
        self.desktop_app.child_window(title="cmdOk", control_type="Button").click_input()
    ...
    def click_ok_encounter_insurance_selection_window_pop_up(self) -> None:
        ...
        if self.desktop_app.child_window(title="OK", control_type="Button").exists(timeout=2):
            with contextlib.suppress(_ctypes.COMError):
                self.desktop_app.child_window(title="OK", control_type="Button").click_input()
    ...
    def get_insurance_maintenance_window(self) -> WindowSpecification:
        ...
        """
        insurance_window = self.desktop_app.child_window(title="frmAMIns", control_type="Window")
    ...
    def enter_co_pay(self, insurance_window: WindowSpecification, co_pay_amount: int) -> None:
        ...
        """
        with contextlib.suppress(_ctypes.COMError):
            insurance_window.child_window(title="txtEncounterCopayAmt", control_type="Edit").set_text(co_pay_amount)
    ...
    def click_close_nextgen_alerts(self) -> None:
        ...
        alert = self.desktop_app.child_window(title="Edits", control_type="Window")
        if alert.exists(timeout=2, retry_interval=1):
            alert.set_focus()
            with contextlib.suppress(_ctypes.COMError):
    ...
    def attach_unattached_payer(self, listing: InsuranceListing) -> None:
        ...
        if insurance_window:
            logger.info("Insurance maintenance window opened")
    ...
    def click_close_nextgen_alerts_encounter_insurance_selection_window(self) -> None:
        """Click close button on NextGen Alerts."""
        alert = self.desktop_app.child_window(
