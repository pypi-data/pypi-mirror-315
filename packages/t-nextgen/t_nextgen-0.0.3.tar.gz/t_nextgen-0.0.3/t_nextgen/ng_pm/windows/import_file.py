...
class ImportFileWindow(NextGenWindow):
    ...
    def click_era_posting_from_processes_menu(self, processes_menu: WindowSpecification) -> None:
        """Click on the ERA Posting option in the Processes menu.

        Args:
            processes_menu (WindowSpecification): Processes Menu control
    ...
    def click_cancel_import_window(self, import_window: WindowSpecification) -> None:
        """Click on cancel button in Import Window.

        Args:
            import_window (WindowSpecification): Import Window Control
        """
        with contextlib.suppress(_ctypes.COMError):
            import_window.child_window(title="cmdCancel", auto_id="cmdCancel", control_type="Button").click()

    def select_directory_import(self, import_window: WindowSpecification) -> None:
        ...
        """
        with contextlib.suppress(_ctypes.COMError):
            import_window.child_window(
                title="optFolderSearch", auto_id="optFolderSearch", control_type="RadioButton"
            ).click()

    def set_835_directory_path(self, folder_835_path: str, import_window: WindowSpecification) -> None:
        ...
        """
        with contextlib.suppress(_ctypes.COMError):
            import_window.child_window(title="txtFolderName", auto_id="txtFolderName", control_type="Edit").set_text(
                folder_835_path
            )

    def click_find_button(self, import_window: WindowSpecification) -> None:
        ...
        """
        with contextlib.suppress(_ctypes.COMError):
            import_window.child_window(title="cmdFind", auto_id="cmdFind", control_type="Button").click()

    def get_edi_pane(self, import_window: WindowSpecification) -> WindowSpecification:
        ...
        """
        pane = import_window.child_window(auto_id="frmFolderResults", control_type="Group").child_window(
    ...
    def select_edi_row_checkboxes(self, edi_pane: WindowSpecification, check_box_count: int) -> None:
        """Select all the EDIs as visible in ERA processing window in NextGen App.

        Args:
            edi_pane (WindowSpecification): EDI Pane control
            check_box_count (int): number of EDIs (checkboxes) to be selected in sequence
        """
    ...
    def import_edi_files(self, import_window: WindowSpecification) -> None:
        """Click on import button in the ERA processing window in NextGen App.

        Args:
            import_window (WindowSpecification): Import Window control
        """
        data = import_window.child_window(title="cmdImport", auto_id="cmdImport", control_type="Button")
        with contextlib.suppress(_ctypes.COMError):
            data.click()
    ...
    def wait_for_edi_import_to_finish(self) -> None:
        ...
        while self.import_edi_modal_exist() and datetime.now() < end_time:
            ...
            try:
                self.app = self.desktop_app.connect_to_logged_in_screen()
    ...
    def convert_edi_files_to_excel(self, excel_file_path: str) -> None:
        """Converts EDI files to CSV format by interacting with the application's excel export functionality.
    ...
    def set_excel_export_path(self, export_path: str) -> None:
        ...
        with contextlib.suppress(_ctypes.COMError):
            closest_window.child_window(control_type="Edit").set_text(export_path)
    ...
    def click_export_to_excel_icon(self) -> None:
        ...
        if excel_button.exists(timeout=5):
            with contextlib.suppress(_ctypes.COMError):
                ...
                logger.info("Excel button clicked successfully.")

            closest_window = self.desktop_app.dialog.child_window(
                auto_id="ReportViewer", control_type="Window"
            ).child_window(auto_id="FileDialog", control_type="Custom")
            closest_window.wait("ready", timeout=3)
            logger.info("FileDialog window is ready.")
        else:
            logger.warning("Excel button not found within timeout.")
    ...
    def import_edi_modal_exist(self) -> bool:
        ...
        """
        with contextlib.suppress((_ctypes.COMError, TimeoutError, RuntimeError)):
            return not self.desktop_app.dialog.child_window(
                title="ERA Import Posting - Enhanced Report Mode", auto_id="ReportViewer", control_type="Window"
            ).exists(timeout=5, retry_interval=1)
            # Return True if a timeouterror or RuntimeError happens to prevent break the import
        return True
    ...
    def patient_has_duplicated_import(self) -> bool:
        ...
        with contextlib.suppress(_ctypes.COMError):
            ...
            ).exists(timeout=3, retry_interval=1)
        return False

    def _handle_duplicate_import(self, import_window: WindowSpecification) -> None:
        ...
        with contextlib.suppress(_ctypes.COMError):
            ...
            import_window.child_window(title="cmdCancel", control_type="Button").click_input()
