...
class YGC():  
    ...
    async def create_message(self, dic: dict, needref = True):
        try:
            ...
            channel = discord.TextChannel(state=self.bot._get_state(), data={"id": channel_id, "name": dic["channelName"], "type": 0,"position": 0,"permission_overwrites": [],"nsfw": False,"parent_id": None})
        try:
