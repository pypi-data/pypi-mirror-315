...
def db_migrate_data(errors: list[str] | None,
    ...
    The parameter *skip_rows* is used to offset the retrieval of tuples. If it is set to -1,
    ...
    The parameters *batch_size_in* and *limit_rows* are used to limit the maximum number of tuples
    ...
    :param skip_rows: number of tuples to skip in source table (defaults to none)
    :param limit_rows: maximum number of tuples to migrate (defaults to no limit)
    ...
    if curr_source_conn and curr_target_conn:
        ...
        if orderby_clause:
            ...
            if skip_rows or batch_size_in:
                sel_stmt += " OFFSET @offset"
            if limit_rows or batch_size_in:
        ...
        if not op_errors:
            ...
            try:
                ...
                if skip_rows:
                    curr_stmt = curr_stmt.replace("@offset", str(skip_rows))
                else:
                    # no offset in initial query
                    curr_stmt = curr_stmt.replace(" OFFSET @offset", "")
                curr_stmt = curr_stmt.replace("@limit", str(min(limit_rows, batch_size_in)))
                ...
                while rows_in:
                    ...
                    if batch_size_in and \
                            (limit_rows == 0 or limit_rows > result):
                        curr_limit: int = batch_size_in if limit_rows == 0 else limit_rows - result
                        curr_stmt = sel_stmt.replace("@offset", str(skip_rows + result))\
...
def db_migrate_lobs(errors: list[str] | None,
    ...
    The parameters *batch_size* and *limit_rows* are used to limit the maximum number of tuples
    ...
    if curr_source_conn and curr_target_conn:
        ...
        try:
            ...
            if skip_rows:
                ...
                # no offset in initial query
                curr_stmt = curr_stmt.replace(" OFFSET @offset", "")
            curr_stmt = curr_stmt.replace("@limit", str(min(limit_rows, batch_size)))
            ...
            while next_rs:
                ...
                if next_rs and batch_size and \
                        (limit_rows == 0 or limit_rows > result):
                    curr_limit: int = batch_size if limit_rows == 0 else limit_rows - result
                    curr_stmt = sel_stmt.replace("@offset", str(skip_rows + result))\
    ...
    if curr_conn:
        ...
        try:
            ...
            while next_rs:
                ...
                # adjust the new offset and limit
                if next_rs and batch_size and \
                        (limit_rows == 0 or limit_rows > lob_count):
                    curr_limit: int = batch_size if limit_rows == 0 else limit_rows - lob_count
                    curr_stmt = sel_stmt.replace("@offset", str(skip_rows + lob_count))\
                                        .replace("@limit", str(min(batch_size, curr_limit)))
                else:
                    # signal end of migration
                    next_rs = False
