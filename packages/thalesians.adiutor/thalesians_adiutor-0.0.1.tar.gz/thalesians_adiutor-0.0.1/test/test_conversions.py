...
import thalesians.adiutor.time_consts as time_consts
...
class TestConversions(unittest.TestCase):
    ...
    def test_to_python_time(self):
        ...
        python_time = dt.time(14, 19, 31, 357000)
        microseconds = 14 * time_consts.MICROSECONDS_PER_HOUR +\
                19 * time_consts.MICROSECONDS_PER_MINUTE +\
                31 * time_consts.MICROSECONDS_PER_SECOND +\
                357000
        ...
        python_times = [dt.time(13, 19, 31, 357000), dt.time(14, 19, 31, 357000)]
        microseconds1 = 13 * time_consts.MICROSECONDS_PER_HOUR +\
                19 * time_consts.MICROSECONDS_PER_MINUTE +\
                31 * time_consts.MICROSECONDS_PER_SECOND +\
                357000
        microseconds2 = 14 * time_consts.MICROSECONDS_PER_HOUR +\
                19 * time_consts.MICROSECONDS_PER_MINUTE +\
                31 * time_consts.MICROSECONDS_PER_SECOND +\
                357000
    ...
    def test_to_python_int(self):
        self.assertEqual(conv.to_python_int(3), 3)
        with self.assertRaises(ValueError):
            conv.to_python_int(3.5)
        self.assertEqual(conv.to_python_int(3.5, allow_floats=True), 3)
        self.assertEqual(conv.to_python_int('3'), 3)
        self.assertEqual(conv.to_python_int('   3  '), 3)
        with self.assertRaises(ValueError):
            conv.to_python_int(None)
        self.assertEqual(conv.to_python_int(None, allow_none=True), None)
        self.assertEqual(conv.to_python_int([3.5, '   3  '], allow_floats=True), [3, 3])
    
    def test_to_python_float(self):
        self.assertEqual(conv.to_python_float(3.5), 3.5)
        with self.assertRaises(ValueError):
            conv.to_python_float(3)
        self.assertEqual(conv.to_python_float(3, allow_ints=True), 3.)
        self.assertEqual(conv.to_python_float('3.5'), 3.5)
        self.assertEqual(conv.to_python_float('   3.5  '), 3.5)
        with self.assertRaises(ValueError):
            conv.to_python_float(None)
        self.assertEqual(conv.to_python_float(None, allow_none=True), None)
        self.assertEqual(conv.to_python_float([3, '   3.5  '], allow_ints=True), [3., 3.5])
