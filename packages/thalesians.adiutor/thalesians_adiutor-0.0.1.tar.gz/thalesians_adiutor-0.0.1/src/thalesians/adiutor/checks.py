...
import thalesians.adiutor.utils as our_utils
...
def check(arg, message='Check failed', level=1):
    ...
    level : int
        The level of this check. The check will only be carried out if `config.MIN_CHECK_LEVEL` is less than or
    ...
    if level < our_config.MIN_CHECK_LEVEL or arg:
        return arg
    else:
...
def is_some_numpy_int(arg, allow_none=False):
    ...
    """
    import numpy as np
    return is_instance(arg, (np.intp, np.int8, np.int16, np.int32, np.int64), allow_none)
...
def is_some_numpy_uint(arg, allow_none=False):
    ...
    """
    import numpy as np
    return is_instance(arg, (np.uint, np.uintp, np.uint8, np.uint16, np.uint32, np.uint64), allow_none)
...
def is_iterable_over_instances(arg, types, allow_none=False, allow_empty=False):
    ...
    if is_iterable(arg):
        objs, iterable = our_utils.peek(iter(arg))
