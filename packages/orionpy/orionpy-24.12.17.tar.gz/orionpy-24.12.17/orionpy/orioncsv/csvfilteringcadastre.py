...
class CSVFilteringCadastre(orioncsv.OrionCSV):
        ...
        for dictionary in profiles_dict:
            ...
            if "rules" in dictionary.keys():
                ...
                if self.right not in temp_dict.keys():
                    temp_dict[
                        self.right
                    ] = "public"  # if there is no "right" defined, public
    ...
    def _csv_process(self, csv_path, data_list):
        ...
        """
        csvfile = open(csv_path, "w", encoding="utf-8-sig", newline="")
        ...
        for dico in data_list:
            ...
            if scope != set():
                ...
                scope = scope[:-1]
            writer.writerow(
                {self.group: group, self.right: role, self.filter_values: scope}
            )
        csvfile.close()
    ...
    def read_and_apply(self, csv_path):
        ...
        fdu_filter = self.orion.filters.get_with_id(
            cadastre_resource.associated_filter_id
        )
        ...
        for row in csv_reader:
            ...
            if group is None:
                print(
                    ...
                    ' ( group : "',
                    row[self.group_i],
                    '" ) of the CSV will not be used, due '
                    "to an error. Please check the group name in the CSV or check if the group is define on aOB",
                )
                continue
            filter_values = list(
                filter(lambda value: value != "", row[self.filter_values_i].split(","))
            )  # make a list of filter values
            self._set_filter_values(
                group, fdu_filter, filter_values, field, row[self.right_i]
            )
            if not cadastre_resource.update_right_from_string(group, row[self.right_i]):
                ...
                continue
            self._activate_or_deactivate_filter(
                row, group, filter_values, cadastre_resource
            )

    def _activate_or_deactivate_filter(
        self, row, group, filter_values, cadastre_resource
    ):
