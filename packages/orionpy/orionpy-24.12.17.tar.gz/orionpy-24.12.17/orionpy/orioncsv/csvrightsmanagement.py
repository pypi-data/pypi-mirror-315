...
class CSVRightsManagement(OrionCSV):
    ...
    def generate(self, csv_path, service, group_list=None):
        ...
        if group_list is None or not group_list:
            ...
            if not cfg_global.is_federated:
                ...
                #     return
                groups_ids = self.orion.services_gis_mgr.get_groups_id_shared(
                    service=service
                )
        ...
        try:
            with open(csv_path, "w+", encoding="utf-8-sig", newline="") as csv_file:
                ...
                for layer in service.layers.all():
                    ...
                    for field in layer.fields.all():
                        print(
                            '\t\tGetting rights for field "{}"...'.format(
                                field.get_name()
                            )
                        )
            ...
            print(
                "Done ! Csv file containing rights on resource was generated at",
                csv_path,
            )
        except IOError as e:
            print(e)
    ...
    def _write_resource_information(self, writer, resource, group_list):
        ...
        """
        row_dict = {
            self.type_head: resource.get_type(),
        ...
        for group in group_list:
            if group is not None:
                ...
                if resource.has_inherited_right(group):
                    row_dict[group.get_name()] = "{} ({})".format(
                        self.inherited_val, right
                    )
                else:
        ...
        try:
            with open(csv_path, "r", encoding="utf-8-sig") as csv_file:
                ...
                for row in reader:
                    ...
                    if (
                        handle_sharing
                        and not sharing_checked
                        and dic_resource[Service.get_type()] is not None
                    ):
                        self._handle_sharing(dic_resource[Service.get_type()], groups)
                        sharing_checked = (
                            True  # avoid calling this method a million time
                        )
                    if resource is None:
                        continue
    ...
    def _handle_sharing(self, service, groups):
        ...
        for group in groups:
            if group is not None and not self.orion.services_gis_mgr.is_shared_with(
                service, group.get_id()
            ):
    ...
    def _get_correct_resource(self, row, resource_d):
        ...
        if row[self.type_i] == Service.get_type():
            resource_d[Service.get_type()] = self.orion.services.get_in_managed(
                row[self.url_i]
            )
            return resource_d[Service.get_type()]
        elif (
            row[self.type_i] == Layer.get_type()
            and resource_d[Service.get_type()] is not None
        ):
            # Get layer's id from REST url.
            layer_id = row[self.url_i].split("/")[-1]
            resource_d[Layer.get_type()] = resource_d[
                Service.get_type()
            ].layers.get_with_id(layer_id)
            return resource_d[Layer.get_type()]
        elif (
            row[self.type_i] == Table.get_type()
            and resource_d[Service.get_type()] is not None
        ):
            resource_d[Layer.get_type()] = None  # for field's right handling
            table_id = row[self.url_i].split("/")[-1]
            resource_d[Table.get_type()] = resource_d[
                Service.get_type()
            ].tables.get_with_id(table_id)
