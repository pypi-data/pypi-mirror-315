...
class RequestManager(metaclass=_Singleton):
    ...
    def get(self, url, params=None, keep_base_parameters=True):
        ...
        try:
            ...
            print("HTTP error : ", he)
            raise RequestError(he)
        return req

    def post(self, url, data=None, keep_base_parameters=True, json=None):
        ...
        try:
            req = requests.post(url, data = data, json = json, verify = self.verify)
            if req.status_code != requests.codes.ok:
                req.raise_for_status()
        except requests.exceptions.ConnectionError as ce:
            print("Connection error :", ce)
            print("url :", url)
            raise RequestError(ce)
        except requests.exceptions.HTTPError as he:
            print('HTTP error : ', he)
            exit(-1)
