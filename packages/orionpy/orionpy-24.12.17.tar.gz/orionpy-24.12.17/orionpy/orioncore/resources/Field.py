...
class Field(Resource):
    ...
    def __init__(self, description, service_url, layer_id, capa, is_managed=False, user_filters = []):
        capa = capa.split(",")
        super().__init__(
            service_url + "/" + str(layer_id) + "/" + str(description["name"]),
            capabilities=capa,
            parent_access=service_url + "/" + str(layer_id),
    ...
    def _can_change_rights(self, group, right_to_apply):
        ...
        if right_to_apply == RightLevel.WRITE:
            ...
            if parent_rights["rights"][0]["action"] != "write":
                print(
                    "To give edit right on a field, containing layer needs to have it as well"
                )
                return False
        return True
    ...
    def __str__(self):
        return "Field {}; alias {}; layer {}; service {}".format(
            self.name, self.alias, self.layerId, self.parentUrl
        )
