...
class Resource(metaclass=abc.ABCMeta):
    ...
    def __init__(self, access_url, capabilities="", parent_access="", is_managed=False, user_filters=[]):
    ...
    def activate_fdu_filter(self, group, filt):
        ...
        if not group.has_defined_filter_values(filt):
            print(
                "WARNING : you should define filtering values for this group and this filter"
            )
    ...
    def _activate_filter(self, current_rights, filt, group):
        ...
        config_url = self._url_builder.resource_configuration_url(
            group.get_id(), self.access_url
        )
        ...
        print(
            "Filter",
            filt.name,
            "successfully applied on resource",
            self.access_url,
            "for group",
            group.get_name(),
        )
    ...
    def _deactivate_filter(self, filt, group):
        ...
        print(
            "Filter",
            filt.name,
            "successfully deactivated on resource",
            self.access_url,
            "for",
            group.get_name(),
        )
    ...
    #Apply user filters to resource
    def apply_user_filters(self, filter_ids):
        resource_apply_user_filters_url = self._url_builder.resource_apply_user_filters_url(resource_path = self.access_url)
        req = self._request_mgr.get_in_python(resource_apply_user_filters_url, { "value" : json.dumps({"filteredDimensions" : filter_ids})})
        return req
    ...
    def _get_right_structure(self, group):
        ...
        """
        resource_rights_url = self._url_builder.resource_rights_url(
            profile_id=group.profile_id, resource_path=self.access_url
        )
    ...
    def force_right(self, group, new_right):
        ...
        if (
            cfg_global.is_federated
            and not self.is_managed()
            and self.get_type() == "SERVICE"
        ):
            self.enable()

        if not self._can_modify(
            group, check_inheritance=False
        ) or not self._can_change_rights(group, new_right):
    ...
    def update_right(self, group, new_right):
        ...
        if not self._can_modify(group) or not self._can_change_rights(group, new_right):
            return
        # Applies the new configuration
    ...
    def _apply_new_right(self, group, new_right, disable_inheritance=False):
        ...
        :return:
        """
        current_rights = self._prepare_rights_structure(
            self._get_right_structure(group), new_right, disable_inheritance
        )
        self._request_mgr.post(
            self._url_builder.resource_configuration_url(
                group.profile_id, self.access_url
            ),
    ...
    def _prepare_rights_structure(
        self, current_rights, right_level, disable_inheritance=False
    ):
        ...
        if current_rights["rights"]:  # if rights already has fields
            ...
            if right_level == RightLevel.READ or right_level == RightLevel.WRITE:
                rights_dic = {"filteredDimensions": [], "action": right_level.value}
    ...
    def user_filters(self):
        """Check if resource's  user filters by aOB"""
    ...
    def _can_modify(self, group, check_inheritance=True):
        ...
        if not self.is_managed():
            print(
                "You must enable {rs} to handle rights first : "
                "Use Service::enable()".format(rs=self.get_name())
            )
            return False
        if not self.is_shared_with(group):  # TODO doesn't work yet
            print(
                "{rs} must be shared with the group {gr} "
                "to change anything".format(rs=self.get_name(), gr=group.get_name())
            )
            return False
        if check_inheritance and self.has_inherited_right(group):
            print(
                "You must disable inheritance on {rs} for group {gr} to change anything : "
                "Use the disable_inheritance(group) method".format(
                    rs=self.get_name(), gr=group.get_name()
                )
            )
            return False
        return True
    ...
    def _can_change_rights(self, group, right_to_apply):
        ...
        """
        if (
            right_to_apply == RightLevel.WRITE
            and self.capa
            and "Editing" not in self.capa
        ):
            print(
                "Impossible to add editing write "
                "for group {gr} on {rs}".format(gr=group.get_name(), rs=self.get_name())
            )
            return False
    ...
    def _can_activate_filter(self, group_rights, filt, group):
        ...
        if not group_rights["rights"]:
            print(
                "The resource has no READ nor WRITE right = no filter to activate."
    ...
    def _change_inheritance(self, group, new_inheritance):
        ...
        if not group_rights[
            "isInherited"
        ]:  # if disable inheritance, need to prepare structure...
        ...
        print(
            "Inheritance {inh} for {gr} on {rs}".format(
                inh=inherit_str, gr=group.get_name(), rs=self.get_name()
            )
        )
