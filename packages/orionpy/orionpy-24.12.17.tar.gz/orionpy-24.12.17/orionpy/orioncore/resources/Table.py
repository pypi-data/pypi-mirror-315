...
class Table(Resource):
    def __init__(self, description, parent_service_url, is_managed=False, user_filters=[]):
        super().__init__(
    ...
    def _can_activate_FDU(self, group_rights, filt, group):
        ...
        for attr in filt.get_fields():
            if self.fields.get(attr) is None:
                print(
                    "In order to apply filter on a table, its attributes has to be in fields"
                )
                return False
        return True
    ...
    def __str__(self):
        return "Table {}; name {}; for Service {}".format(
            self.id, self.name, self.parent_access
        )
