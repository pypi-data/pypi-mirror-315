...
class UrlBuilder(metaclass=_Singleton):
    ...
    CALCULATE_PERMISSIONS_FROM_WEBMAP_ITEM = "/__calculatePermissionsFromWebmapItem"
    ...
    def resource_rights_url(self, profile_id, resource_path):
        ...
        """
        return (
            self.right_management_profiles_url(profile_id)
            + "/SERVICES/"
            + resource_path
        )
    ...
    def resource_resolved_permissions_url(self, group_id, resource_path):
        ...
        """
        url = (
            self._main_group_url()
            + "/{gId}/authorizedResources/SERVICES/{path}".format(
                gId=group_id, path=resource_path
            )
        )
        return url
    ...
    def resource_apply_user_filters_url(self, resource_path):
        """
        :param resource_path: access to the resource
        :return: Url to manage a resource
        :Example: https://$main_service_url$/$resourcePath$/__applyUserFilter
        """
        return self.main_services_url() + "/" + resource_path + "/__applyUserFilter"
    ...
    def resource_sharing_url(self, resource_id, owner="admin_aob"):
        ...
        url = (
            self.base_url
            + "/{port}/sharing/rest/content/users/{own}/items/{rId}".format(
                port=self.portal, own=owner, rId=resource_id
            )
        )
        return url
    ...
    def right_management_profiles_url(self, profile_id):
        ...
        """
        return (
            self._api_header_url()
            + "/rightmanagement/profiles/{pId}/aobrights".format(pId=profile_id)
        )
    ...
    def subject_list_url(self, subject_type):
        ...
        """
        url = self.base_url + "/{port}/sharing/rest/community/{subject_type}".format(
            port=self.portal, subject_type=subject_type
        )
        return url
    ...
    def orion_self_url(self):
        """
        :return: url to get data about connected user
        :Example: https://front.arcopole.fr/Orion/orion/self
        """
        return self._base_orion_url() + "/self"
    ...
    def main_imc_url(self):
        """
        :return: Main service handling url
        :Example: https://front.arcopole.fr/Orion/orion/admin/tree/object/IMC
        """
        return self._api_header_url() + "/object/IMC"

    def imc_children_url(self):
        """
        :return: url to get all IMC defined in orion.
        :Example: https://front.arcopole.fr/Orion/orion/admin/tree/object/IMC/__children
        """
        return self.main_imc_url() + self.CHILDREN

    def imc_definition_url(self, resource_name):
        """
        :param resource_access: relative url to the resource
        :return: url for resource definition
        :Example: https://front.arcopole.fr/Orion/orion/admin/tree/object/IMC/__children
        """
        # Layer :  + "/" + layerId
        return self.main_imc_url() + "/" + resource_name

    def execute_imc_url(self):
        """
        ...
        """
        return self._base_orion_url() + "/imc/imc"
    ...
    def calculate_permissions_from_webmap_item_url(self):
        """
        :return: Url to calculate permissions from a webmap item
        :Example: https://front.arcopole.fr/Orion/orion/admin/tree/rightmanagement/__calculatePermissionsFromWebmapItem

        """
        return self._main_rm_url() + self.CALCULATE_PERMISSIONS_FROM_WEBMAP_ITEM

    def get_admin_node(self, nodePath):
        return self._api_header_url() + "/" + nodePath
    
    def list_children_node(self, nodePath):
        return self.get_admin_node(nodePath) + "/__children"
    
    def list_commands_node(self, nodePath):
        return self.get_admin_node(nodePath) + "/__commands"
    
    def call_command_node(self, nodePath, commandName):
        return self.get_admin_node(nodePath) + "/__" + commandName
