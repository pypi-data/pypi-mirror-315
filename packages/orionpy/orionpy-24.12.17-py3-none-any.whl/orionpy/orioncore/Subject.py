...
class Subject:
    ...
    def set_filter_values(self, fdu_filter, labels=None, add_all_values=False):
        ...
        try:
            if not isinstance(fdu_filter, _FDUFilter):
                raise FilteringValuesError(
                    "[ERROR] Can only define filtering values on a FDU"
                )
            # Check that the good arguments are given
            if not add_all_values:
                if labels is None:
                    raise FilteringValuesError(
                        "[ERROR] If add_all False, labels must be provided."
                    )
                elif not labels:
                    raise FilteringValuesError(
                        '[ERROR] "labels" can\'t be an empty list'
                    )
    ...
    def add_filtering_values(self, fdu_filter, labels):
        ...
        f_index = self._get_filter_index(
            fdu_filter.get_id(), subject_info[self.perimeters_key]
        )
        ...
        filter_values = subject_info[self.perimeters_key][f_index][
            self.filter_val_key
        ].split(",")
    ...
    def _get_filter_index(self, filter_id, filtering_values):
        ...
        return next(
            (
                index
                for (index, filt) in enumerate(filtering_values)
                if filt[self.filter_id_key] == filter_id
            ),
            None,
        )
    ...
    def _apply_configuration(self, config):
        ...
        """
        self._request_mgr.get(
            self._url_builder.subject_configure_url(self.subject_type, self.id),
            params={"value": json.dumps(config)},
        )
        print(
            'Successfully applied change on configuration for subject "{}" !'.format(
                self.title
            )
        )
    ...
    def _subject_information_minus_filter(self, filter_to_set):
        ...
        self._remove_current_filter_value(
            subject_information[self.perimeters_key], filter_to_set.id
        )
    ...
    def _build_perimeters(self, filter_concerned, filtering_values, field=""):
        ...
        for filtering_value in filtering_values:
            ...
            elif field and filter_concerned.is_field_value_defined(
                field, filtering_value
            ):
                approved_values += filter_concerned.get_labels_for(
                    field, filtering_value
                )
            else:
                print(
                    "Value",
                    filtering_value,
                    "not defined for this filter. It will not be set",
                )
        return {
            self.filter_id_key: filter_concerned.id,
    ...
    def get_subject_informations(self):
        subject_information_url = self._url_builder.subject_information_url(
            self.subject_type, self.id
        )
    ...
    def print_defined_filtering_values(self):
        ...
        for val in subject_information[self.perimeters_key]:
            filtering_str.append(
                "\tOn filter {}, "
                "value(s) defined is/are : {}\n".format(
                    val[self.filter_id_key], val[self.filter_val_key]
                )
            )
