...
class Items:
    ...
    def data_export(self, itemId, export_file):
        ...
        try:
            file = open(export_file, "w", encoding="utf-8-sig")
            file.write(data)
            file.close()
        except Exception:
    ...
    def add_item_from_file(self, owner, title, json_path, snippet=None):
        """
        ...
        print(json_data)
        return self.add_item(owner, title, json_data, snippet)
    ...
    def update_item(self, itemId, title, text, snippet=None):
        ...
        url = self.url_manager.update_item_feature_url(
            self.type, self._get_owner_from_id(itemId), itemId
        )
        data = {
            "title": title,
            "snippet": snippet,
    ...
    def delete(self, itemId):
        ...
        :return: result of the query (a boolean for "success" and the id of the item)
        """
        url = self.url_manager.delete_feature_url(
            self.type, self._get_owner_from_id(itemId), itemId
        )
        req = self.request.post(url).json()
        self._verify(
            req,
            'Item with id: "' + str(itemId) + '" was deleted successfully',
            'WARNING : Item with id: "'
            + str(itemId)
    ...
    def reassign(self, itemId, targetUsername):
        ...
        :return: result of the query (a boolean for "success" and the id of the item)
        """
        url = self.url_manager.reassign_feature_url(
            self.type, self._get_owner_from_id(itemId), itemId
        )
        ...
        self._verify(
            ...
            'Item with id: "'
            + str(itemId)
            + '" was reassigned successfully\nNew owner : '
            + str(targetUsername),
            'WARNING : Item with id: "'
            + str(itemId)
            + '" was NOT reassigned successfully',
        )
        return req
