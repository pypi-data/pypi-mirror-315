...
class FilterFactory:
    ...
    def try_to_build_sql(name, where_clause, filter_dic):
        ...
        """
        if (
            filter_dic is not None
            and "whereClause" in filter_dic
            and filter_dic["whereClause"]
        ):
            return _SQLFilter(
                filter_dic["name"].strip(),
                filter_dic["whereClause"].strip(),
                filter_dic,
            )
        if name is not None and where_clause is not None:
            if (
                name.strip() and where_clause.strip()
            ):  # name and whereClause aren't empty
    ...
    def try_to_build_fdu(name, attributes, properties, filtering_values, filter_dic):
        ...
        if filter_dic is not None and "attributeName" in filter_dic:
            return _FDUFilter(
                filter_dic["name"],
                filter_dic["attributeName"],
                filter_dic["properties"],
                filter_dic["partitions"],
                filter_dic,
            )
        if name is not None and attributes is not None and filtering_values is not None:
            if name.strip() and attributes and filtering_values:
                return _FDUFilter(
                    name.strip(), attributes, properties, filtering_values
                )
            else:
                raise BuildFilterError(
        ...
        try:
            if filter_type is not None:
                if filter_type != "sql" and filter_type != "fdu":
                    raise BuildFilterError(
                        "[ERROR Filter] invalid type_filt.",
                        filter_type,
                        "must be 'fdu' or 'sql'",
                    )
                if filter_type == "sql":
                    return FilterFactory.try_to_build_sql(
                        name, where_clause, filter_dic
                    )
                if filter_type == "fdu":
                    return FilterFactory.try_to_build_fdu(
                        name, attributes, properties, filtering_values, filter_dic
                    )
            else:
                new_filter = FilterFactory.try_to_build_sql(
                    name, where_clause, filter_dic
                )
                if new_filter is None:
                    new_filter = FilterFactory.try_to_build_fdu(
                        name, attributes, properties, filtering_values, filter_dic
                    )
                    if new_filter is None:
                        raise BuildFilterError(
...
class Filter:
    # TODO private class ?
    def __init__(self, name=None, properties=None, filter_dic=None, filter_id=None):
    ...
    def get_properties(self):
        """ Returns filter's properties """
        return self.properties
    
    def get_propertie_value(self, property_name):
        """ Returns filter's propertie value """
        if self.properties is None:
            return None
        return self.properties[property_name]

    def has_propertie(self, property_name):
        """ Returns if filter has propertie """
        if self.properties is None:
            return False
...
class _SQLFilter(Filter):
    ...
    def __init__(self, name="", whereClause="", properties=None, filter_dic=None, filter_id=None):
        super(_SQLFilter, self).__init__(
            name=name, properties = properties, filter_dic=filter_dic, filter_id=filter_id
        )
        if filter_dic is not None:
    ...
    def __str__(self):
        filter_str = [
            'SQL Filter "{name}"; whereClause : {wc}'.format(
                name=self.name, wc=self.whereClause
            )
        ]
        if self.id is not None:
...
class _FDUFilter(Filter):
    ...
    def __init__(
        ...
        """
        super(_FDUFilter, self).__init__(
            name=name, properties=properties, filter_dic=filter_dic, filter_id=filter_id
        )
    ...
    def __str__(self):
        ...
        """
        filter_str = [
            'FDU Filter "{}"; attribute name(s): {};'.format(
