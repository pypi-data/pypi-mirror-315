...
from .utils.methylation_performance_metrics import MethylationPerformanceMetrics
...
INTERVAL_MIN = -0.05
INTERVAL_MAX = 1.15
SCALE_FACTOR = INTERVAL_MAX - INTERVAL_MIN
weight = np.arange(FINAL_CH) #.cuda()

def approx(probs):
    return np.sum(weight*probs)/FINAL_CH*SCALE_FACTOR+INTERVAL_MIN


def convert_back(vec):
    conv = np.zeros((len(vec), 275))
    for rix, row in enumerate(vec):
        for cix, ix in enumerate(range(0, len(row), 12)):
            conv[rix, cix] = approx(row[ix:ix+12])
    #conv = torch.Tensor(conv).cuda()
    return conv
...
class TrainMethylationBinLeg(object):
    """
    ...
    def __init__(self,
        ...
        self.reg_criterion = nn.KLDivLoss(reduction='none')
        #self.reg_criterion = nn.BCEWithLogitsLoss()
        print(self.reg_criterion)
    ...
    def _init_train(self, scheduler_kwargs):
        ...
        if self._use_scheduler:
            ...
            self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
                ...
                max_lr=0.1,
                div_factor=25.,
                steps_per_epoch=self.nth_step_report_stats,
                epochs=self.max_steps // self.nth_step_report_stats,
                pct_start=0.3,
    ...
    def _init_validate(self):
        ...
        self._validation_metrics = MethylationPerformanceMetrics(
    ...
    def _init_test(self):
        ...
        self._test_metrics = MethylationPerformanceMetrics(
            self.sampler.get_feature_from_index,
            report_gt_feature_n_positives=self._report_gt_feature_n_positives,
            metrics=self._metrics)
    ...
    def train(self):
        ...
        #logpreds, reg_pred = self.model(inputs.transpose(1, 2))
        reg_pred = self.model(inputs.transpose(1, 2))

        rpred_rs = F.log_softmax(reg_pred.reshape((len(reg_pred) * 275, 12)), dim=1)
        targets_rs = targets.reshape((len(targets) * 275, 12))

        #mask = torch.isnan(targets)
        mask = torch.isnan(targets_rs)
        reg_loss = self.reg_criterion(
            #logpreds[~mask],
            #reg_pred[~mask],
            #targets[~mask],
            rpred_rs,
            targets_rs)[~mask].mean()
        self.optimizer.zero_grad()
        reg_loss.backward()
        self.optimizer.step()

        self._train_loss.append(reg_loss.item())
        t_f = time()
    ...
    def _evaluate_on_data(self, data_in_batches):
        ...
        batch_losses = []
        all_predictions = []
        for (seqs, targets) in data_in_batches:
            ...
            with torch.no_grad():
                #logpreds, reg_pred = self.model(seqs.transpose(1, 2))
                reg_pred = self.model(seqs.transpose(1, 2))
        ...
        return loss_breakdown, all_predictions
    ...
    def validate(self):
        ...
        """
        lossdict, val_pred_tgts = self._evaluate_on_data(
            self._validation_data)
        val_pred_tgts = convert_back(val_pred_tgts)
        all_val_tgts = convert_back(self._all_validation_targets)
    ...
    def evaluate(self):
        ...
        losdict, test_pred_tgts = self._evaluate_on_data(
            self._test_data)
