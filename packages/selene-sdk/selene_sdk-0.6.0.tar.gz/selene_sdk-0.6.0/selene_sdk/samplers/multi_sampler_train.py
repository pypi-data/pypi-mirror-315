...
class MultiSampler(Sampler):
    ...
    Parameters
    ----------
    train_sampler : selene_sdk.samplers.dataloader.DataLoader
        Load your training data as a `DataLoader`
    validate_sampler : DataLoader
        The validation dataset
    features : list(str)
        The list of features the model should predict
    test_sampler : None or DataLoader, optional
    ...
    def __init__(self,
        ...
        self._dataloaders = {
            "train": train_sampler_lt,
            "validate": validate_sampler_lt,
        }

        self._iterators = {
            "train": [iter(d) for d in self._dataloaders["train"]],
            "validate": [iter(d) for d in self._dataloaders["validate"]],
        }

        self._index_to_feature = {i: f for (i, f) in enumerate(features)}

        if test_sampler_lt is not None:
            self.modes.append("test")
            self._dataloaders["test"] = test_sampler_lt
            self._iterators["test"] = [iter(d) for d in self._dataloaders['test']]
    ...
    def _set_batch_size(self, batch_size, mode=None):
        ...
        if self._dataloaders[mode]:
            ...
            for d in self._dataloaders[mode]:
                if d.batch_sampler:
                    if d.batch_sampler.batch_size != batch_size:
                        d.batch_sampler.batch_size = batch_size
                        batch_size_matched = False
                else:
                    if d.batch_size != batch_size:
            ...
            if not batch_size_matched:
                ...
                self._iterators[mode] = [iter(d) for d in self._dataloaders[mode]]
    ...
    def get_data_and_targets(self, batch_size, n_samples=None, mode=None):
        ...
        n_samples : int or None, optional
            ...
            If a Dataloader is specified, will set `n_samples` to 32000
        ...
        output = self.sample(batch_size=remainder, mode=mode)
        data_and_targets.append(output)
        targets_mat.append(output[1])
    ...
    def get_validation_set(self, batch_size, n_samples=None):
        ...
        n_samples : int, optional
            ...
            retrieve. If `n_samples` is None, if a Dataloader is specified,
            will set `n_samples` to 32000.
