...
class TrainMethylationUNet(object):
    """
    ...
    def __init__(self,
        ...
        self.reg_criterion = F.mse_loss
        #self.reg_criterion = nn.BCELoss(reduction='none')
    ...
    def train(self):
        ...
        #print('TRAINING')
        #print(reg_pred)
        #print(self.reg_criterion(reg_pred, targets))
        #print(torch.nanmean(self.reg_criterion(reg_pred, targets), dim=-1))
        #print(torch.nanmean(torch.nanmean(self.reg_criterion(reg_pred, targets), dim=-1), dim=-1))

        mask = torch.isnan(targets)
        #reg_loss = torch.nanmean(
        #    torch.nanmean(
        #        torch.nanmean(self.reg_criterion(reg_pred, targets), dim=-1),
        #        dim=-1),
        #    dim=-1)
        #print(reg_loss)
        #print()
        reg_loss = self.reg_criterion(
            reg_pred[~mask], targets[~mask])
    ...
    def validate(self):
        ...
        pearson, _ = pearsonr(
            val_pred_tgts[~np.isnan(self._all_validation_targets)],
            self._all_validation_targets[~np.isnan(self._all_validation_targets)])
        spearman, _ = spearmanr(
            val_pred_tgts[~np.isnan(self._all_validation_targets)],
            self._all_validation_targets[~np.isnan(self._all_validation_targets)])

        reg_valid_scores = {'pearsonr': pearson, 'spearmanr': spearman}
