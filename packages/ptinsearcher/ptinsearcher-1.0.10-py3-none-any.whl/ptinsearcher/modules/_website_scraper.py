...
class WebsiteScraper:
    def __init__(self, args, things_to_extract: dict, url_list_len: int, ptjsonlib: object):
    ...
    def handle_network_error(self):
        """End or keep going when exception occurs"""
        if self.url_list_len > 1:
            ptprinthelper.ptprint("Server not responding", "ERROR", not self.use_json)
        else:
            self.ptjsonlib.end_error("Server not responding", self.use_json)

    def get_response(self, url: str):
        """Retrieves response from <url>"""
        ptprinthelper.ptprint(f"Request URL.................: {url}", "TITLE", not self.use_json, colortext=True, end=" ")           #, newline_above=True if len(self.url_list) > 1 and index != 0 else False)
        try:
            ...
            ptprinthelper.ptprint(f"[{history[0].status_code}]", condition=not self.use_json)
            response.encoding = response.apparent_encoding
            if self.stop_on_redirect(response, history=history):
                return
            ptprinthelper.ptprint(f"Content-Type................: {response.headers.get('content-type', 'None').split(';')[0]}", "TITLE", not self.use_json)
            return response
        except requests.exceptions.RequestException:
    ...
    def stop_on_redirect(self, response, history):
        """Stop on redirect if not --redirects"""
        if response.is_redirect and not self.args.redirects:
            if response.headers.get("location"):
                ptprinthelper.ptprint(f"Location....................: {response.headers.get('location')}", "TITLE", not self.use_json)
        ...
        if self.args.redirects and len(history) > 1:
            ptprinthelper.ptprint(f"Redirect URL................: {history[-1].url} [{history[-1].status_code}]", "TITLE", not self.use_json)
    ...
    def _scrape_response(self, response) -> dict:
            ...
            result_data = {"url": response.url, "metadata": None, "emails": None, "phone_numbers": None, "ip_addresses": None, "abs_urls": None, "internal_urls": None, "internal_urls_with_parameters": None, "external_urls": None, "insecure_sources": None, "subdomains": None, "forms": None, "comments": None}
            content_type = response.headers.get("content-type")
            PAGE_CONTENT = urllib.parse.unquote(urllib.parse.unquote(html.unescape(response.text)))

            if len(response.text) < 500:  # Adjust length threshold as needed
                print(response.text)

            # Find Metadata
            if self.extract_types["metadata"]:
                result_data["metadata"] = metadata.MetadataExtractor().get_metadata(response=response)
                self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("metadata", None, None, properties={"metadata": result_data["metadata"]}))
            ...
            if self.extract_types["emails"]:
                ...
                self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("emails", None, None, properties={"emails": result_data["emails"]}))

            # Find Page Comments
            if self.extract_types["comments"]:
                result_data["comments"] = comments.find_comments(PAGE_CONTENT)
                self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("comments", None, None, properties={"comments": result_data["comments"]}))

            # Find Phone numbers
            if self.extract_types["phone_numbers"]:
                result_data["phone_numbers"] = phone_numbers.find_phone_numbers(PAGE_CONTENT)
                self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("phone_numbers", None, None, properties={"phone_numbers": result_data["phone_numbers"]}))

            # Find IP Addresses
            if self.extract_types["ip_addresses"]:
                result_data["ip_addresses"] = ip_addresses.find_ip_addresses(PAGE_CONTENT)
                self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("ip_addresses", None, None, properties={"ip_addresses": result_data["ip_addresses"]}))
            ...
            if self.extract_types["forms"]:
                ...
                if soup:
                    ...
                    self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("form", None, None, properties={"forms": result_data["forms"]}))
            ...
            if any([self.extract_types["external_urls"], self.extract_types["subdomains"], self.extract_types["internal_urls"], self.extract_types["internal_urls_with_parameters"], self.extract_types["insecure_sources"]]):
                ...
                if self.extract_types["external_urls"] or self.extract_types["insecure_sources"]:
                    result_data['external_urls'] = urls.find_urls_in_response(PAGE_CONTENT, r'(href=|src=)[\'"](.+?)[\'"]', response.url, "external", without_parameters=self.args.without_parameters, abs_urls=result_data["abs_urls"])
                    if self.extract_types["external_urls"]:
                        self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("external_urls", None, None, properties={"external_urls": result_data["external_urls"]}))

                # Find Subdomains | Filters through absolute_urls
                if self.extract_types["subdomains"]:
                    result_data['subdomains'] = urls.find_urls_in_response(PAGE_CONTENT, r'(href=|src=)[\'"](.+?)[\'"]', response.url, "subdomain", without_parameters=self.args.without_parameters, abs_urls=result_data["abs_urls"])
                    self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("subdomains", None, None, properties={"subdomains": result_data["subdomains"]}))
                ...
                if self.extract_types["internal_urls"] or self.extract_types["internal_urls_with_parameters"] or self.extract_types["insecure_sources"]:
                    result_data["internal_urls"] = urls.find_urls_in_response(PAGE_CONTENT, r'(href=|src=)[\'"](.+?)[\'"]', response.url, "internal", without_parameters=self.args.without_parameters, abs_urls = result_data["abs_urls"])

                    # Add to self.ptjsonlib
                    if self.extract_types["internal_urls"]:
                        self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("internal_urls", None, None, properties={"internal_urls": result_data["internal_urls"]}))
                    ...
                    if self.extract_types["internal_urls_with_parameters"]:
                        result_data["internal_urls_with_parameters"] = sorted(urls._find_internal_parameters(result_data["internal_urls"], group_parameters=self.args.group_parameters), key=lambda k: k['url'])
                        self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("internal_urls_with_parameters", None, None, properties={"internal_urls_with_parameters": result_data["internal_urls_with_parameters"]}))
                        if not self.extract_types["internal_urls"]:
                            result_data["internal_urls"] = None

                if self.extract_types["insecure_sources"]:
                    result_data["insecure_sources"] = []
                    ...
                    self.ptjsonlib.add_node(self.ptjsonlib.create_node_object("insecure_sources", None, None, properties={"insecure_sources": result_data["insecure_sources"]}))
                    if not self.extract_types["internal_urls"]:
                        result_data["internal_urls"] = None
