...
from clusterondemand.inbound_traffic_rule import RESERVED_NETWORK_ACL_RULE_NUM, InboundTrafficRule, RuleType
...
def _create_head_node(vpc, hostname, public_subnet, head_node_image, head_node_sg_id, node_sg_id):
    ...
    if config["existing_subnet_id"]:
        ...
        cloud_init_script += \
            ...
            "/cm/local/apps/cm-setup/lib/python3.9/site-packages/cmsetup/plugins/brightsetup/stages.py\n"
...
def _create_network_acls(vpc, public_subnet, private_subnet, ec2_client):
    ...
    def _allow_all_outbound_and_all_vpc_traffic(vpc, network_acl):
    ...
    _allow_all_outbound_and_all_vpc_traffic(vpc, public_network_acl)
    ...
    _allow_all_outbound_and_all_vpc_traffic(vpc, private_network_acl)
    ...
    LOWEST_PORT = 1024
    HIGHEST_PORT = 65535
    log.info(f"ALLOW rule for inbound TCP port range {LOWEST_PORT}-{HIGHEST_PORT} in '{private_network_acl_name}'")
    private_network_acl.create_entry(
        CidrBlock="0.0.0.0/0",
        Egress=False,
        PortRange={
