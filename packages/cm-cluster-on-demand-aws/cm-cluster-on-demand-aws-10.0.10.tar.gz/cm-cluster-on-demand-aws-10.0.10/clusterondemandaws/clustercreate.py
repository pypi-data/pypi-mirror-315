...
config_ns.add_enumeration_parameter(
    ...
    help=("One or several inbound network acl rules for the cluster's head node in one of the following format: "
          ...
          f"but {inbound_traffic_rule.RESERVED_NETWORK_ACL_RULE_NUM} is reserved for communication inside VPC. "
          "Examples: '11.0.0.0/24,20-23:TCP,ALLOW,100' '443:TCP,ALLOW,200' 'ICMP,ALLOW,500' "
          "'10.0.0.0/16,ALL,ALLOW,500'. "
    ...
    default=[
        inbound_traffic_rule.InboundNetworkACLRule(rule) for rule in
        [
            "22:tcp,ALLOW,100",
            "80:tcp,ALLOW,200",
            "443:tcp,ALLOW,300",
            "8081:tcp,ALLOW,400",
            "32768-65535:tcp,ALLOW,500",
            "icmp,ALLOW,600"
        ]
    ]
)
...
class ClusterCreate(ClusterCommandBase):
    ...
    def _generate_cluster_names(cls, names, cluster_count):
        ...
        if conflicting_names:
            raise UserReportableException(f"Cluster(s) with conflicting name(s) already exists: "
        ...
        for cluster in self.requested:
            try:
                ...
                if e.response["Error"]["Code"] == "VpcLimitExceeded":
                    ...
                    raise UserReportableException(str(e.response["Error"]))
...
def wait_for_cloud_init(clusters):
    ...
    while time.time() < wait_until and clusters:
        ...
        for cluster in clusters[:]:
            ...
            ip_to_check = None
            if config["head_node_assign_public_ip"]:
                ip_to_check = cluster.primary_head_node.public_ip_address
            else:
                ip_to_check = cluster.primary_head_node.private_ip_address
