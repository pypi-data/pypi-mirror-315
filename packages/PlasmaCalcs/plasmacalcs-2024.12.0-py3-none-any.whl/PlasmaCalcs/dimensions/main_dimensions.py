...
from ..tools import (
    format_docstring,
    simple_property, simple_setdefault_property, alias_child,
    product,
...
class MainDimensionsHaver(DimensionHaver):
    ...
    def behavior_attrs(self):
        ...
        here, returns ['slices', 'multi_slices', 'maindims_means'], plus any behavior_attrs from super().
        (Note that if self.slicing==False, self.slices will be adjusted appropriately,
            so there's no need for behavior to also check self.slicing.)
        '''
        return ['slices', 'multi_slices', 'maindims_means'] + list(getattr(super(), 'behavior_attrs', []))
    ...
    def _slice_maindims_numpy(self, array):
    ...
    def slice_maindims(self, array, **kw_xarray_isel):
        ...
        return xarray_isel(array, to_slice, **kw_xarray_isel)
