...
from ...dimensions import XYZ, YZ_FROM_X
...
class VectorArithmeticLoader(QuantityLoader):
    ...
    # {A}_perp_{B}
    @known_pattern(r'(.+)_perp_(.+)', deps=[0,1], ignores_dims=['component'])
    def get_perp(self, var, *, _match=None, _val0=None, _val1=None, **_known_vals):
        ...
        if Bval is None:
            Bval = self(B) if A != B else Aval  # [EFF] if A == B, only calculate once.
        return self.take_perp_to(Bval, Aval)

    # {A}_par_{B}
    @known_pattern(r'(.+)_par_(.+)', deps=[0,1], ignores_dims=['component'])
    def get_parallel(self, var, *, _match=None, _val0=None, _val1=None, **_known_vals):
        ...
        if Bval is None:
            Bval = self(B) if A != B else Aval  # [EFF] if A == B, only calculate once.
        return self.take_parallel_to(Bval, Aval)
    ...
    def get_parmod(self, var, *, _match=None, _val0=None, _val1=None, **_known_vals):
        ...
        '''
        A, _mod_or_mag, B = _match.groups()
        # [EFF] could use self(f'mod_({A}_par_{B})'), however that calculates A dot Bhat,
        #       then multiplies by Bhat, then takes the magnitude.
        # It's equivalent (and less expensive) to just calculate & return A dot Bhat, here.
