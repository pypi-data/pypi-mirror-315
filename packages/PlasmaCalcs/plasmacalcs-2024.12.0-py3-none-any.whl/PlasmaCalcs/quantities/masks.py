...
from ..tools import (
    format_docstring,
    simple_property, simple_setdefaultvia_property, alias_child,
    UNSET,
    xarray_ensure_dims,
...
class MaskLoader(QuantityLoader):
    ...
    def behavior_attrs(self):
        ...
        here, returns ['mask', 'masking', 'output_mask'], plus any behavior_attrs from super().
        '''
        return ['mask', 'masking', 'output_mask'] + getattr(super(), 'behavior_attrs', [])
    ...
    mask = simple_property('_mask', default=None,
        doc='''None or xarray.DataArray of bools with same dims as self.maindims.
        if self.masking, apply mask to results (with self.maindims dims) at top-level (call_depth=1)
        ...
        applying mask means calling self.apply_mask(result);
            masking = True or 'stacked' --> xarray_mask(result, stack=True); i.e., drops all masked points.
                        (and, result will have '_mask_stack' dimension instead of maindims dimensions)
    ...
    masking = simple_property('_masking', default='stacked',
        doc='''how to apply self.mask to results (with self.maindims dims) at top level, if mask exists.
    ...
    def set_mask(self, mask):
        ...
        Also does some checks:
            ...
            - set(mask.dims) == set(self.maindims)
                - promoting nondim coords to dims if necessary and possible.
        Also may alter mask slightly:
            - discard non-dimension coords other than self.maindims.
        '''
        if mask is None:
            self._mask = None
        else:
            ...
            mask = xarray_ensure_dims(mask, self.maindims, promote_dims_if_needed=True)
            if set(mask.dims) != set(self.maindims):
                raise InputError(f'mask dims {mask.dims} must match self.maindims {self.maindims}')
    ...
    def _call_postprocess_toplevel(self, result, *, var, name=UNSET, item=UNSET):
        ...
        if self.mask is not None and self.masking != False:
            ...
            if all(d in result_dims for d in self.maindims):
    ...
    def apply_mask(self, arr, masking=UNSET):
        ...
        arr must have self.maindims dims.
