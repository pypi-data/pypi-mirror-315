"""
File Purpose: equation of state (EOS) and radiation tables for Bifrost
...
from ...tools import (
    alias, simple_setdefaultvia_property,
    format_docstring,
    UNSET, NO_VALUE,
    using_attrs,
...
class erTable():
    ...
    coords_are_sorted = simple_setdefaultvia_property('_coords_are_sorted', 'can_assume_sorted',
...
class erTableManager(dict):
    '''manages tables for Bifrost. Dict of {quantity (str): erTable}'''
...
@format_docstring(**_paramdocs_ertable)
class BifrostTabInputManager(DictlikeFromKeysAndGetitem, UnitsHaver):
    ...
    u: {u}
        note: if this TabInputManager is associated with a BifrostCalculator,
    ...
    def __init__(self, filename, *, u=None, ionize_ev='physical', **kw_super):
        ...
        self.init_elements()  # makes self.elements
    ...
    def init_elements(self):
        '''initialize self.elements.'''
        self.elements = self.make_element_list()

    def make_element_list(self, *, ionize_ev=None):
        '''return ElementList based on params in self, and ionize_ev (from self if None).'''
