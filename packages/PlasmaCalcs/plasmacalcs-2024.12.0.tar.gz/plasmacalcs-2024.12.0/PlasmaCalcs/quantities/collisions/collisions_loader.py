...
from ...errors import QuantInfoMissingError, CollisionsModeError, InputError
...
class CollisionsLoader(CollisionsCrossSectionsLoader):
    ...
    COLLISIONS_MODE_OPTIONS = {
        'best':
            ...
            For charge-neutral, if table doesn't exist, collisions_cross_section will give 0.''',
        'table':
    ...
    collisions_mode = simple_property('_collisions_mode', default='best',
    ...
    def behavior_attrs(self):
        ...
        here, returns ['collisions_mode'], plus any behavior_attrs from super().
        '''
        return ['collisions_mode'] + list(getattr(super(), 'behavior_attrs', []))
    ...
    @known_var(load_across_dims=['fluid', 'jfluid'],
               deps=[lambda ql, var, groups: ql._collision_types_deps()])
    def get_nusj(self):
        '''collision frequency.
        ...
        '''
        # [TODO][EFF] here, getting single collision frequency, iterating through pairs,
        #   but maybe it would be better to load them all at once, using xarray framework?
        #   or, since it's tricky due to the different collision types,
        #   maybe it would be better to group all same-type collisions together,
        #   thus doing e.g. all nusj_maxwell calcs at the same time?
        #   Probably only worthwhile to explore further if collisions are a computational bottleneck.
    ...
    def collision_type(self, fluid=UNSET, jfluid=UNSET):
        '''return the type of collision between fluid and jfluid.
        UNSET --> use self.fluid or self.jfluid, respectively.
        fails if fluid or jfluid represent a list of fluids, instead of a single value.

        result depends on fluids and on self.collisions_mode.
        returns '0' if fluid and jfluid are the same fluid,
            else 'coulomb', 'maxwell', or 'fromtable', inferred from collisions_mode if possible;
            else raise CollisionsModeError if collisions_mode not recognized,
                i.e. not in ('best', 'table', 'maxwell', 'neutral_only', 'table_only').
        '''
        using = dict()
        if fluid is not UNSET: using['fluid'] = fluid
        if jfluid is not UNSET: using['jfluid'] = jfluid
        with self.using(**using):
            if self.fluid_is_iterable() or self.jfluid_is_iterable():
                raise AssertionError('collision_type assumes single-valued fluid & jfluid.')
            ...
            if mode not in ('best', 'table', 'maxwell', 'neutral_only', 'table_only'):
                errmsg = f'{type(self).__name__}.collision_type not implemented when collisions_mode = {mode!r}'
                raise CollisionsModeError(errmsg)
            if fs.q != 0 and fj.q != 0:  # coulomb collisions
                ...
                if mode in ('table', 'table_only'):
                    ...
                    return 'maxwell'
                else:  # mode in ('best', 'neutral_only')
                    table_exists = ((fs, fj) in self.collisions_cross_mapping)
                    if table_exists:
                        return 'fromtable'
                    else:
                        return 'maxwell'
        assert False, "should only reach this line if typo in code above"

    def collision_types(self, fluid=UNSET, jfluid=UNSET):
        '''return the set of all collision types between fluid and jfluid.
        UNSET --> use self.fluid or self.jfluid, respectively.
        here, fluid and/or jfluid may be iterable, unlike in self.collision_type.

        result depends on fluids and on self.collisions_mode.
        returns set of values from '0', 'coulomb', 'maxwell', 'fromtable'.
        '''
        using = dict()
        if fluid is not UNSET: using['fluid'] = fluid
        if jfluid is not UNSET: using['jfluid'] = jfluid
        result = set()
        with self.using(**using):
            for _fs in self.iter_fluid():
                for _fj in self.iter_jfluid():
                    result.add(self.collision_type())
        return result

    def _collision_types_deps(self=UNSET):
        '''returns the set of variables which are used to calculate nusj,
        based on self.fluid, self.jfluid, and self.collisions_mode.
        '''
        if self is UNSET:
            errmsg = ('Cannot determine deps for var="nusj" when called as classmethod. '
                      'nusj depends on the present values of self.fluid, self.jfluid, and self.collisions_mode.')
            raise InputError(errmsg)
        coltypes = self.collision_types()
        return {self.COLLISION_TYPE_TO_VAR[ct] for ct in coltypes}
    ...
    def get_collision_type(self):
        ...
        '''
        return xr.DataArray(self.collision_type())
    ...
    @known_var(deps=['T', 'm', 'n', 'm_sj', 'T_sj', 'delta_sj'])
