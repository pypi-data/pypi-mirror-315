...
class Test(unittest.TestCase):
    ...
    def _read_write_ram(self, cls, ignore=None, precision=1e-10):
        """Read and write"""
        with cls() as th:
            th.write_timestep(1.0)
            for i, system in enumerate(self.system):
    ...
    def test_ram(self):
        self._read_write_ram(trj.TrajectoryRam)
        self._append(trj.TrajectoryRam, path=None)

    def test_ram_full(self):
        self._read_write_ram(trj.ram.TrajectoryRamFull)
        self._append(trj.ram.TrajectoryRamFull, path=None)

    def test_ram_from_data(self):
        # Get positions and species as numpy array
        pos, spe, box = [], [], []
        with trj.TrajectoryXYZ('data/ka_N150_rho1.200.xyz') as th:
            for s in th:
                pos.append(s.dump('position'))
                spe.append(s.dump('particle.species'))
                box.append(s.dump('box'))
            steps = th.steps
        # Store trajectory in memory from data
        data = {'pos': pos, 'spe': spe, 'box': box}
        th = trj.TrajectoryRam(data)
        th.steps = steps
        for i in range(len(th)):
            self.assertTrue(numpy.all(th[i].dump('pos') == pos[i]))
            self.assertTrue(numpy.all(th[i].dump('spe') == spe[i]))
            self.assertTrue(numpy.all(th[i].dump('box') == box[i]))

    def test_ram_light(self):
        self._read_write_ram(trj.ram.TrajectoryRamLight)
        self._append(trj.ram.TrajectoryRamLight, path=None)

    def test_ram_light_custom(self):
        # Test custom attributes
        with trj.ram.TrajectoryRamLight() as th:
            for i, system in enumerate(self.system):
                for p in system.particle:
                    p.custom_field = numpy.ones(3)
                th.write(system, i)
            for system in th:
                x = system.view('particle.custom_field')
...
""")
        ...
        with trj.lammps.TrajectoryLAMMPS(self.inpfile) as th:
        ...
        with trj.lammps.TrajectoryLAMMPS(self.inpfile, 'w') as th:
            th.write(system)

    def test_lammps_style(self):
        with trj.lammps.TrajectoryLAMMPS(self.inpfile, 'w') as th:
            th.write(self.system[0])
        for p in self.system[0].particle:
            p.charge = 1
        with trj.lammps.TrajectoryLAMMPS(self.inpfile, 'w') as th:
            th.write(self.system[0])
        # print(open('/tmp/test_trajectory.inp').read())
...
""")
    ...
    def test_lammps_triclinic(self):
        import sys
        with open(self.inpfile, 'w') as fh:
            fh.write("""\
ITEM: TIMESTEP
...
1 1 12.6334 21.0623 23.5327 1
2 1 17.2724 17.2329 21.5826 1
""")
        with trj.lammps.TrajectoryLAMMPS(self.inpfile) as th:
            self.assertAlmostEqual(list(th[0].cell.side), [19, 19, 19])
            self.assertEqual(sorted(th.variables), ['particle.charge',
                                                    'particle.position',
                                                    'particle.species'])
    ...
    def test_split(self):
        with trj.TrajectoryXYZ(os.path.join(self.inpdir, 'test.xyz'), 'w') as th:
            th.write(self.system[0])
            th.write(self.system[0])
            fname = th.filename
        from atooms.trajectory.utils import split, field
        with trj.TrajectoryXYZ(fname) as th:
            x = field(th, th, 'position', 0)
            split(th, archive=True)
        import glob
        self.assertTrue(os.path.exists(fname + '.tar.gz'))

    def test_slice_twice(self):
        from atooms.trajectory import Sliced
        import glob
        with trj.TrajectoryRUMD(os.path.join(self.inpdir, 'trajectory0.xyz'), 'w') as th:
            th.timestep = 0.001
            th.write(self.system[0], 10)
            th.write(self.system[0], 20)
        with trj.TrajectoryRUMD(os.path.join(self.inpdir, 'trajectory1.xyz'), 'w') as th:
            th.timestep = 0.001
            th.write(self.system[0], 30)
            th.write(self.system[0], 40)
        with trj.SuperTrajectoryRUMD(self.inpdir, basename="trajectory*") as th:
            with Sliced(th, slice(None, None, 1)) as ts:
                for s in ts:
                    pass
            with Sliced(th, slice(3, None, 1)) as ts:
                for s in ts:
                    pass

    def test_array_formatting(self):
        variables = ['position', 'species']
        def _list(x):
            try:
                return [_ for _ in x]
            except:
        ...
        for p in self.system[0].particle:
            #a = replace(']', '').strip()
            a = [_list(getattr(p, variable)) for variable in variables]
            #print(type(a[0][0]), type(a[1]))
            #print(list(chain(a)))
