...
__all__ = ['SimulationEnd', 'WallTimeLimit', 'SimulationKill',
           ...
           'target_shell_stop', 'target_python_stop', 'stuck']
...
def write_thermo(sim, fields=None, fmt=None, precision=6, functions=None):
    ...
    _db_func = {
        ...
        'cm velocity': lambda x: x.system.center_of_mass('velocity'),
        'cm velocity norm': lambda x: numpy.sum(x.system.center_of_mass('velocity')**2)**0.5,
    }
...
def store(sim, what, data=None, db=None):
    """
    ...
    if db is not None:
        import warnings
        warnings.warn('db argument to store is deprecated, use data instead', DeprecationWarning)
        data = db
...
def target(sim, attribute, value):
    ...
    simulation or a function receiving sim as first argument.
...
class stuck:
    ...
    """Stop the simulation if the system gets stuck in configuration space"""
    ...
    def __call__(self, sim):
        if sim.current_step == 0:
            return
        current = sim.rmsd        
        deriv = (current - self._last_value) / (sim.current_step - self._last_step)
        self._derivatives.append(deriv)
        self._values.append(current)
        self._steps.append(sim.current_step)
        if len(self._derivatives) < self.estimates:
            return

        n = self.estimates // 2
        deriv = numpy.mean(self._derivatives[:n])
        # print(sim.current_step, current, (deriv - std) * (sim.current_step - self._last_step))
        # print(numpy.array(self._values[-n:]))
        actual = numpy.array(self._values[-n:])
        estimate = self._values[n] + deriv * (numpy.array(self._steps[-n:]) - self._steps[-n])
        if numpy.all(actual < estimate):
            raise SimulationEnd('system got stuck')

        self._last_value = current
        self._last_step = sim.current_step
        self._derivatives = self._derivatives[-self.estimates:]
        self._values = self._values[-self.estimates:]
        self._steps = self._steps[-self.estimates:]
...
class Speedometer:
    ...
    def __init__(self, log=True, path=None):
        self._init = False
        self._log = log
        self._path = path
    ...
    def __call__(self, sim):
        if self._path is None and sim.output_path is not None:
            self._path = sim.output_path + '.speed.yaml'
        if not self._init:
            ...
            for observer in sim._observer:
                ...
                if c is self:
                    continue
                name = getattr(c, '__name__', 'unknown').lower()
                if 'target' in name:
        ...
        try:
            ...
            if self._log:
                ...
                with open(self._path, 'w') as fh:
                    eta = d_eta.strftime('%Y-%m-%d %H.%M')
                    fh.write(f"""
completed: {int(frac * 100)}%
ETA: {eta}
T/SP: {sim.wall_time(per_step=True, per_particle=True)}
""")
