...
class Particle(object):
    ...
    def __init__(self, position=None, velocity=None, species='A',
        ...
        if velocity is None:
            self.velocity = numpy.zeros_like(self.position)
        else:
    ...
    def nearest_image_of(self, particle, cell, folded=False):
        """
        Return a copy of the nearest image of `particle` from `self` in
        the given `cell`.
        ...
        cell.
        """
        rij = particle.position - self.position
        if folded:
            rij = _periodic_vector(rij, cell.side)
        else:
            rij = _periodic_vector_unfolded(rij, cell.side)
...
def distinct_species(particles):
    ...
    try:
        return list(sorted({p.species for p in particles}))
    except TypeError:
        # This is (most likely) an unhashable numpy array
        # We accept both int and str species labels
        dtype = particles[0].species.dtype
        # We cast species as strings
        species = list(sorted({str(p.species) for p in particles}))
        # Then restore their original types
        return [numpy.array(_, dtype=dtype) for _ in species]
