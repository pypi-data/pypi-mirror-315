...
from .utils import gopen, _PossiblyZippedFile
...
class TrajectoryXYZ(TrajectoryBase):
    ...
    def __init__(self, filename, mode='r', alias=None, fields=None):
        ...
        self._file = _PossiblyZippedFile(self.filename, self.mode)
...
""" % key, namespace)
    ...
    def write_system(self, system, step):
        ...
        # Attempts to bring numpy 2 compatibility, unfortuanately they
        # are significantly slower than the current code
        # numpy.set_printoptions(precision=6, floatmode='fixed')
        # numpy.set_printoptions(formatter={'float': lambda x: f'{x:.6f}'})
        for i, p in enumerate(system.particle):
            p._index = i
            p._step = step
            #self._file.write(fmt.format(p).replace('[', '').replace(']', ''))
