...
def group_quantile(df: pandas.DataFrame | polars.DataFrame, 
    ...
    """
    Compute quantiles for specified variables in a DataFrame, 
        optionally grouping by certain columns.
    ...
    Parameters:
        ...
        vr (str or list[str], optional): Variable(s) to compute quantiles for. 
                                            Default is None.
        q (int, optional): Number of quantiles to compute. Default is None.
        quantiles (dict[str, Union[int, list[float]]], optional): Custom quantiles 
                                                                    to compute for 
                                                                    each variable.
                                                                    Keys are variable 
                                                                    names, values are 
        ...
        no_merge (bool, optional): If True, returns the DataFrame without merging. 
                                    Default is False.

    Returns:
        pandas.DataFrame | polars.DataFrame: DataFrame containing quantiles 
                                                for specified variables, optionally 
...
def quantile_ts(df: pandas.DataFrame, 
                vars: dict[str, list[float]] | list[str], 
                qtiles: list[float] | int = 2,
                date_col: str = 'date'
                ) -> pandas.DataFrame:
    """
    Calculates time-series quantiles for specified variables grouped by a date column.

    Parameters:
        df (pandas.DataFrame): The input DataFrame containing the time-series data.
        vars (dict[str, list[float]] | list[str]): Specifies the variables to compute quantiles for. 
            - If a dictionary, keys are column names and values are lists of quantiles (e.g., [0.25, 0.5, 0.75]).
            - If a list, all variables in the list will use the same quantiles specified by `qtiles`.
        qtiles (list[float] | int, optional): Specifies the quantiles to calculate.
            - If an integer, splits the data into equal-sized quantiles (e.g., 2 for median split, 4 for quartiles).
            - If a list, specifies the quantile thresholds explicitly (e.g., [0.25, 0.5, 0.75]).
            Defaults to 2 (median split).
        date_col (str, optional): The name of the column used for grouping the data by date. 
            Defaults to 'date'.

    Returns:
        pandas.DataFrame: A DataFrame containing the computed quantiles for the specified variables,
        with columns named in the format `{variable}_{quantile}` (e.g., `var_25%`, `var_50%`).

    Notes:
        - When `vars` is a list, all variables will use the same quantile settings from `qtiles`.
        - The function groups the data by `date_col` and computes the quantiles for each group.
        - If multiple variables are specified, the results are merged into a single DataFrame.
        - Quantile labels in the result use the percentage format (e.g., '25%' for the 0.25 quantile).
        - The resulting DataFrame is reset to include a flat index.

    Raises:
        TypeError: If `df` is not a DataFrame
        TypeError: If `vars` is not dict[str, list[float]] or list[str]
        TypeError: If `qtiles` is not list[float] | int
    """

    if not isinstance(df, pandas.DataFrame):
        raise TypeError(f'df: expected type pandas.DataFrame, got {type(df).__name__!r}')
    
    if not (isinstance(vars, dict) or isinstance(vars, list)):
        raise TypeError(f'vars: expected type dict[str, list[float]] or list[str], got {type(vars).__name__!r}')
    
    if not (isinstance(qtiles, list) or isinstance(qtiles, int)):
        raise TypeError(f'vars: expected type list[float] or int, got {type(qtiles).__name__!r}')

    dict_in: dict = {}
    if isinstance(vars, dict):
        dict_in = vars
    else:
        if isinstance(qtiles, int):
            tiles: list = [i / qtiles for i in range(1, qtiles)]
            for var in vars:
                dict_in[var] = tiles
        elif isinstance(qtiles, list):
            for var in vars:
                dict_in[var] = qtiles
        else:
            raise TypeError(f'qtiles: expected type dict[str, int | list[float]] | list[str], got {type(qtiles).__name__!r}')

    res: list = []
    for var, qtiles in dict_in.items():
        temp: pandas.DataFrame | pandas.Series = df.groupby(
            by = date_col
        )[var].describe(percentiles = qtiles)
        ptiles: list = [f'{int(100 * q)}%' for q in qtiles]
        temp = temp[ptiles]
        temp = temp.add_prefix(f'{var}_')
        res.append(temp)
    fin: pandas.DataFrame = functools.reduce(lambda x, y: pandas.merge(x, y, on = date_col), res)
    fin = fin.reset_index()
    return fin
...
def sort_portfolios(df: pandas.DataFrame,
    ...
    df = df[needed_cols]
    df = df.sort_values(by = [date_col, id_col])
    df = df.dropna()
        
    # cast date column to datetime64[ns]
    try: 
        df[date_col] = pandas.to_datetime(df[date_col], format = 'ISO8601')
    except Exception as e:
        print(e)
        raise TypeError('unable to cast `date_col` to datetime with format ISO8601.')
        
    # when to sort
    df['sort_month'] = df[date_col].dt.month    
    if(rebalance_freq == 'A'):
        rebalance_df = df[df.sort_month == sort_month]
    else:
        rebalance_df = df
    ...
    if isinstance(breakpoints, dict):
        breakpoints_df = quantile_ts(df, vars = breakpoints)
    ...
    for rank_col in rank_cols:
        if('--fail' in rebalance_df[rank_col].unique()):
            if(not suppress):
                print(_config.Messages.STOCKS_NOT_SORTABLE.format(
