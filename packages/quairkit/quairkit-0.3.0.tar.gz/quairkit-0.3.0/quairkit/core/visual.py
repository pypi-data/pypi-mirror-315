...
from .utils.linalg import _partial_trace_discontiguous
...
def plot_state_in_bloch_sphere(
    ...
    assert (
        type(state) == list
        or type(state) == torch.Tensor
        or type(state) == np.ndarray
        or type(state) == State
    ), 'the type of "state" must be "list" or "torch.Tensor" or "np.ndarray" or "State".'
    if type(state) == State:
        state = [state.density_matrix]
    if type(state) == torch.Tensor or type(state) == np.ndarray:
        state = [state]
    state_len = len(state)
    assert state_len >= 1, '"state" is NULL.'
    for i in range(state_len):
        assert type(state[i]) == torch.Tensor or type(state[i]) == np.ndarray or type(state[i]) == State, \
            'the type of "state[i]" should be "torch.Tensor" or "numpy.ndarray" or "State".'
    ...
    for i in range(state_len):
        if type(state[i]) == torch.Tensor:
            state[i] = state[i].detach().numpy()
        if type(state[i]) == State:
    ...
    for i in range(state_len):
        ...
        bloch_vector_tmp = bloch_vector_tmp.detach().numpy()
...
def plot_multi_qubits_state_in_bloch_sphere(
    ...
    assert (
        ...
        or type(state) == np.ndarray
        or type(state) == State
    ), 'the type of "state" must be "torch.Tensor" or "np.ndarray" or "State".'
    assert type(set_color) == str, 'the type of "set_color" should be "str".'
    ...
    if state.shape[0] >= 2 and state.numel == state.shape[0]:
        state_vector = state
        state = np.outer(state_vector, np.conj(state_vector))

    # multi qubits state decompose
    if state.shape[0] > 2:
        rho = torch.tensor(state)
        tmp_s = []
        for q in which_qubits:
            tmp_s.append(_partial_trace_discontiguous(rho, [q]))
        state = tmp_s
    else:
        state = [state]
    state_len = len(state)
...
def plot_rotation_in_bloch_sphere(
        init_state: State,
        rotating_angle: List[torch.Tensor],
        show_arrow: Optional[bool] = False,
        save_gif: Optional[bool] = False,
        filename: Optional[str] = None,
    ...
    assert (
        type(init_state) == torch.Tensor
        or type(init_state) == np.ndarray
        or type(init_state) == State
    ), 'the type of input data should be "torch.Tensor" or "numpy.ndarray" or "State".'
    assert (
    ...
    ), 'the rotating_angle must include [theta=torch.Tensor, phi=torch.Tensor, lam=torch.Tensor].'
    for i in range(3):
        assert (
            type(rotating_angle[i]) == torch.Tensor or type(rotating_angle[i]) == float
        ), 'the rotating_angle must include [theta=torch.Tensor, phi=torch.Tensor, lam=torch.Tensor].'
    ...
    for i in range(len(rotating_angle)):
        if type(rotating_angle[i]) == torch.Tensor:
            rotating_angle[i] = float(rotating_angle[i].detach().numpy()[0])
    ...
    if type(init_state) == torch.Tensor:
        init_state = init_state.detach().numpy()

    # Convert state_vector to density_matrix
    if init_state.numel == 2:
    ...
    def u_gate_matrix(params):
        ...
        if (isinstance(theta, torch.Tensor) and
                isinstance(phi, torch.Tensor) and
                isinstance(lam, torch.Tensor)):
            re_a = torch.cos(theta / 2)
            re_b = - torch.cos(lam) * torch.sin(theta / 2)
            re_c = torch.cos(phi) * torch.sin(theta / 2)
            re_d = torch.cos(phi + lam) * torch.cos(theta / 2)
            im_a = torch.zeros([1], 'float64')
            im_b = - torch.sin(lam) * torch.sin(theta / 2)
            im_c = torch.sin(phi) * torch.sin(theta / 2)
            im_d = torch.sin(phi + lam) * torch.cos(theta / 2)

            re = torch.reshape(torch.cat([re_a, re_b, re_c, re_d]), [2, 2])
            im = torch.reshape(torch.cat([im_a, im_b, im_c, im_d]), [2, 2])

            return re + im * torch.tensor([1j])
    ...
    def rotating_operation(rotating_angle_each):
        ...
        if type(gate_matrix) == torch.Tensor:
            gate_matrix = gate_matrix.detach().numpy()
...
def plot_density_matrix_graph(density_matrix: State, size: Optional[float] = 0.3) -> None:
    ...
    """
    if not isinstance(
        density_matrix, (np.ndarray, torch.Tensor, State)
    ):
        msg = f'Expected density_matrix to be np.ndarray or torch.Tensor or State, but got {type(density_matrix)}'
        raise TypeError(msg)
    if isinstance(density_matrix, State):
...
def __density_matrix_convert_to_bloch_vector(
    ...
    assert (
        ...
        or type(density_matrix) == State
    ), 'the type of "state" must be "np.ndarray" or "State".'
    if isinstance(density_matrix, State):
        density_matrix = density_matrix.numpy()
    ...
    return torch.tensor(bloch_vector)
