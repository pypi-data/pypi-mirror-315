...
@_alias({"num_systems": "num_qubits"})
def random_projector(num_systems: int,
                     system_dim: Union[List[int], int] = 2) -> torch.Tensor:
    r"""randomly generate a :math:`d \times d` rank-1 orthogonal projector
    ...
    vec = torch.randn([n, 1], dtype=float_dtype) + 1j * torch.randn(
        [n, 1], dtype=float_dtype
    )
    mat = vec @ _dagger(vec)
...
def random_channel(
    ...
    .. code-block:: python
        ...
        channel_stinespring = random_channel(num_systems=1, target="stinespring",system_dim=2,size=1,rank=1)
