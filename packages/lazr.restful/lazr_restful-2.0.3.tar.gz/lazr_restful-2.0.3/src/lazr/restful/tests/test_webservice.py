...
from zope.component import eventtesting, getGlobalSiteManager, getUtility
...
from lazr.restful._resource import BatchingResourceMixin, CollectionResource
...
def get_operation_factory(model_interface, name):
    ...
    request_interface = getUtility(IWebServiceVersion, name="2.0")
    return getGlobalSiteManager().adapters.lookup(
        (model_interface, request_interface), IResourceGETOperation, name=name
    )
...
class EntryTestCase(WebServiceTestCase):
    ...
    class StubWebsiteRequest:
        """A request to the website, as opposed to the web service."""

    class StubWebsiteURL(StubAbsoluteURL):
        """A web-centric implementation of the stub URL."""
    ...
    @contextmanager
    def entry_resource(self, entry_implementation, *implementation_args):
    ...
    def register_html_field_renderer(
        self, entry_interface, field_interface, render_function, name=""
    ):
    ...
    def _register_url_adapter(self, entry_interface):
        ...
        getGlobalSiteManager().registerAdapter(
            StubAbsoluteURL,
            [entry_interface, IWebServiceClientRequest],
            IAbsoluteURL,
        )
    ...
    def _register_website_url_space(self, entry_interface):
        ...
        def web_service_request_to_website_request(service_request):
            ...
            return self.StubWebsiteRequest()
...
class IHasOneField(Interface):
    ...
    a_field = exported(TextLine(title="A field."))
...
class IHasTwoFields(Interface):
    ...
    a_field = exported(TextLine(title="A field."))
    another_field = exported(TextLine(title="Another field."))
...
class TestEntryWebLink(EntryTestCase):
    ...
    def test_entry_includes_web_link_when_available(self):
        ...
        # 'web_link'.
        with self.entry_resource(HasOneField, "") as resource:
            representation = resource.toDataForJSON()
            self.assertEqual(representation["self_link"], StubAbsoluteURL.URL)
            self.assertEqual(
                representation["web_link"], self.StubWebsiteURL.URL
            )
    ...
    def test_wadl_includes_web_link_when_available(self):
        ...
        representation = [
            rep
            for rep in doc.findall("%srepresentation" % self.WADL_NS)
            if rep.get("id") == "has_one_field-full"
        ][0]
        param = [
            param
            for param in representation.findall("%sparam" % self.WADL_NS)
...
class TestSuppressWebLink(EntryTestCase):
    ...
    def test_entry_omits_web_link_when_suppressed(self):
        ...
        with self.entry_resource(HasNoWebLink, "") as resource:
            representation = resource.toDataForJSON()
            self.assertEqual(representation["self_link"], StubAbsoluteURL.URL)
            self.assertFalse("web_link" in representation)
...
class IHasFieldExportedAsDifferentName(Interface):
    ...
    a_field = exported(TextLine(title="A field."), exported_as="field")
...
class TestEntryWrite(EntryTestCase):
    ...
    def test_applyChanges_rejects_nonexistent_web_link(self):
        ...
        # that references it.
        with self.entry_resource(HasOneField, "") as resource:
            errors = resource.applyChanges({"web_link": "some_value"})
            self.assertEqual(
                errors,
    ...
    def test_applyChanges_rejects_changed_web_link(self):
        ...
        with self.entry_resource(HasOneField, "") as resource:
            errors = resource.applyChanges({"web_link": "some_value"})
            self.assertEqual(
                errors, "web_link: You tried to modify a read-only attribute."
            )
    ...
    def test_applyChanges_accepts_unchanged_web_link(self):
        ...
        with self.entry_resource(HasOneField, "") as resource:
            ...
            representation = json.loads(
                resource.applyChanges({"web_link": existing_web_link}).decode()
            )
    ...
    def test_applyChanges_returns_representation_on_empty_changeset(self):
        ...
        with self.entry_resource(HasOneField, "") as resource:
            ...
            representation = json.loads(resource.applyChanges({}).decode())
    ...
    def test_applyChanges_returns_modified_representation_on_change(self):
        ...
        with self.entry_resource(
            ...
            self.assertEqual(
                "initial value", resource.toDataForJSON()["field"]
            )
            # This returns the changed value.
            representation = json.loads(
                resource.applyChanges(dict(field="new value")).decode()
            )
            self.assertEqual("new value", representation["field"])
...
class TestEntryWriteForRestrictedField(EntryTestCase):
    ...
    def test_applyChanges_binds_to_resource_context(self):
        ...
        with self.entry_resource(HasRestrictedField, "") as resource:
            entry = resource.entry
            entry.schema["a_field"].restrict_to_interface = IHasRestrictedField
            self.assertEqual(entry.a_field, "")
            resource.applyChanges({"a_field": "a_value"})
...
class HTMLRepresentationTest(EntryTestCase):
    ...
    def setUp(self):
        super().setUp()
        self._register_url_adapter(IHasOneField)
        self.unicode_message = "Hello from a \N{SNOWMAN}"
        self.utf8_message = self.unicode_message.encode()

    def test_entry_html_representation_is_utf8(self):
        with self.entry_resource(
            HasOneField, self.unicode_message
        ) as resource:
    ...
    def test_field_html_representation_is_utf8(self):
        with self.entry_field_resource(
            IHasOneField, HasOneField, "a_field", self.unicode_message
        ) as resource:
            html = resource.do_GET()
...
class JSONPlusHTMLRepresentationTest(EntryTestCase):
    ...
    def register_html_field_renderer(self, name=""):
        ...
        super().register_html_field_renderer(
    ...
    def test_normal_json_representation_omits_lp_html(self):
        ...
        self.register_html_field_renderer()
        with self.resource() as resource:
            json_representation = json.loads(resource.do_GET().decode())
            self.assertFalse("lp_html" in json_representation)
    ...
    def test_field_specific_html_renderer_shows_up_in_lp_html(self):
        ...
        with self.resource() as resource:
            json_representation = json.loads(resource.do_GET().decode())
            html = json_representation["lp_html"]
            self.assertEqual(
    ...
    def test_acceptchanges_ignores_lp_html_for_json_plus_html_type(self):
        ...
        with self.resource() as resource:
            ...
            json_representation = json.loads(
                str(resource._representation(json_plus_xhtml))
            )
            resource.applyChanges(json_representation, json_plus_xhtml)
    ...
    def test_acceptchanges_does_not_ignore_lp_html_for_bare_json_type(self):
        ...
        with self.resource() as resource:
            json_representation = json.loads(
                str(resource._representation(resource.JSON_PLUS_XHTML_TYPE))
            )
            resource.applyChanges(json_representation, resource.JSON_TYPE)
...
class UnicodeChoice(EnumeratedType):
    ...
    UNICODE = Item("Uni\u00e7ode", "Uni\u00e7ode choice")
...
class ICanBeSetToUnicodeValue(Interface):
    ...
    a_field = exported(
        Choice(
            ...
            title="A value that might be ASCII or Unicode.",
            required=False,
            default=None,
        )
    )
...
class WadlAPITestCase(WebServiceTestCase):
    ...
    def test_wadl_field_type(self):
        ...
        self.assertEqual(test_tales("field/wadl:type", field=TextLine()), None)
        self.assertEqual(
            test_tales("field/wadl:type", field=Date()), "xsd:date"
        )
        self.assertEqual(
            test_tales("field/wadl:type", field=Datetime()), "xsd:dateTime"
        )
    ...
    def test_wadl_entry_doc(self):
        ...
        doclines = test_tales("entry/wadl_entry:doc", entry=entry).splitlines()
        self.assertEqual(
            [
                '<wadl:doc xmlns="http://www.w3.org/1999/xhtml">',
                "<p>A simple, reusable entry interface for use in tests.</p>",
                "<p>The entry publishes one field and one named operation.</p>",  # noqa: E501
                "",
                "</wadl:doc>",
            ],
            doclines,
    ...
    def test_empty_wadl_entry_doc(self):
        ...
        self.assertEqual(None, test_tales("entry/wadl_entry:doc", entry=entry))
    ...
    def test_field_wadl_doc(self):
        ...
        doclines = test_tales("field/wadl:doc", field=field).splitlines()
        self.assertEqual(
            [
                '<wadl:doc xmlns="http://www.w3.org/1999/xhtml">',
                "<p>A &quot;field&quot;</p>",
    ...
    def test_wadl_operation_doc(self):
        ...
        doclines = test_tales(
            "operation/wadl_operation:doc", operation=operation
        ).splitlines()
    ...
    def test_wadl_operation_with_scopes_doc(self):
        ...
        self.assertEqual(
            [
                ...
                '<p>Scopes: <tt class="rst-docutils literal"><span class="pre">'  # noqa: E501
...
def make_entry(name):
    ...
    fields = {
        "%s_field" % letter: exported(TextLine(title="Field %s" % letter))
...
class DuplicateSingularNameTestCase(DuplicateNameTestCase):
    ...
    def test_duplicate_singular(self):
        self.doDuplicateTest(
            "Both IDuplicate and IGenericEntry expose the "
...
class NotificationsProviderTest(EntryTestCase):
    ...
    class StubWebsiteRequestWithNotifications:
        """A request to the website, as opposed to the web service."""

        @property
        def notifications(self):
            return [
                Notification(logging.INFO, "Informational"),
    ...
    def _register_notification_adapter(self):
        ...
        def web_service_request_to_notification_request(service_request):
            """Create a corresponding request to the website."""
            return self.StubWebsiteRequestWithNotifications()
    ...
    def test_response_notifications(self):
        with self.resource() as resource:
            resource.request.publication.callObject(resource.request, resource)
...
class EventTestCase(EntryTestCase):
    ...
    def test_event_fired_when_changeset_is_not_empty(self):
        ...
        with self.entry_resource(HasOneField, "") as resource:
            resource.applyChanges({"a_field": "Some value"})
...
class MalformedRequest(EntryTestCase):
    ...
    def test_multiple_named_operations_generate_error_on_GET(self):
        ...
        self.assertEqual(resource.request.response.getStatus(), 400)
        self.assertEqual(result, "Expected a single operation: ['foo', 'bar']")
