...
def _response(islog_info, islog_err):
    def __wrapper__(func):
        def __inner__(ins: BigTangerine, *args, **kwargs):
            if kwargs.get("resp").status_code == 200:
                ...
                if islog_err: httpd_err_log(kwargs.get("resp"))
...
def get_mapping(*, uri: str, json_clazz=None, islog_info=False, islog_err=False):
    """
    ...
    def __wrapper__(func):
        ...
        @_url(uri=uri)
        @_get
        @_response(islog_info=islog_info, islog_err=islog_err)
...
def post_mapping(*, uri: str, json_clazz=None, islog_info=False, islog_err=False):
    """
    使用范围：BigTangerine 或其 子类对象
...
def put_mapping(*, uri: str, json_clazz=None, islog_info=False, islog_err=False):
    """
    使用范围：BigTangerine 或其 子类对象
...
def upload_mapping(*, uri: str, json_clazz=None, islog_info=False, islog_err=False):
    """
    使用范围：BigTangerine 或其 子类对象
...
def delete_mapping(*, uri: str, json_clazz=None, islog_info=False, islog_err=False):
    """
    使用范围：BigTangerine 或其 子类对象
