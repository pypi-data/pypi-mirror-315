...
_default_graph = {
    0: {
        "nodes": [
            {"points": [0, 1], "x": 0.5, "y": 0.5, "size": 5},
            {"points": [2], "x": -0.5, "y": -0.5, "size": 5},
        ],
        "edges": [{"s": 0, "t": 1}],
    }
}
...
class Landscape(DOMWidget):
    ...
    @validate("graph")
    def _valid_graph_params(self, proposal):
        # TODO: does this work?
    ...
    @validate("node_sizes")
    def _valid_node_sizes(self, proposal):
        node_sizes: Dict[int, List[float]] = proposal["value"]
        for level, sizes in node_sizes.items():
            if level not in self.graph:
                raise TraitError(f"level {level} not in graph")
            if len(sizes) != len(self.graph[level]["nodes"]):
                raise TraitError(
                    f"node_sizes for level {level} must have length n_nodes"
                )
        return node_sizes
    ...
    def _valid_node_labels(self, proposal):
        ...
        for level, labels in proposal["value"].values():
            if level not in self.graph:
                raise TraitError(f"level {level} not in graph")
