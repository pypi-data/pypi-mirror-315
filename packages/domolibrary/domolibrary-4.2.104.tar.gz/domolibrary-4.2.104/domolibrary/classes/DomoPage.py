...
async def _from_adminsummary(cls, page_obj, auth: dmda.DomoAuth, session : httpx.AsyncClient = None, debug_api : bool = False):
    ...
    if dd.cards and len(dd.cards) > 0:
        pg.cards = await dmce.gather_with_concurrency(
            n=60,
            *[dmc.DomoCard.get_from_id(id=card.id, auth=auth, session = session, debug_api = debug_api) for card in dd.cards],
        )
...
async def _from_content_stacks_v3(
    ...
    if dd.page.owners and len(dd.page.owners) > 0:
        pg.owners = await pg._get_domo_owners_from_dd(dd.page.owners,debug_api = debug_api, session = session, debug_num_stacks_to_drop = debug_num_stacks_to_drop)
...
async def get_by_id(
    ...
    pg = await cls._from_content_stacks_v3(page_obj=res.response, auth=auth, session = session, debug_api = debug_api, debug_num_stacks_to_drop = debug_num_stacks_to_drop)
...
async def get_datasets(
    ...
    self.domo_datasets =  await dmce.gather_with_concurrency(
        n=60,
        *[
            dmds.DomoDataset.get_from_id(dataset_id=ds.get("dataSourceId"), auth=self.auth)
            for card in res.response.get("cards")
