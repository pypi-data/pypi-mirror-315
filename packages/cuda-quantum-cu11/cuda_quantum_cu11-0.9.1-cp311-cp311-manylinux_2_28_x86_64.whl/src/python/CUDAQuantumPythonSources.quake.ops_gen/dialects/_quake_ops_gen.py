...
class ExpPauliOp(_ods_ir.OpView):
  ...
  def __init__(self, parameter, qubits, *, pauli=None, constantPauli=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(parameter))
    operands.append(_get_op_result_or_value(qubits))
    if pauli is not None: operands.append(_get_op_result_or_value(pauli))
    _ods_context = _ods_get_default_loc_context(loc)
    if constantPauli is not None: attributes["constantPauli"] = (constantPauli if (
        issubclass(type(constantPauli), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('StrAttr')) else
          _ods_ir.AttrBuilder.get('StrAttr')(constantPauli, context=_ods_context))
    _ods_successors = None
    super().__init__(self.build_generic(
      attributes=attributes, results=results, operands=operands,
      successors=_ods_successors, regions=regions, loc=loc, ip=ip))
  ...
  @builtins.property
  def constantPauli(self):
    if "constantPauli" not in self.operation.attributes:
      return None
    return _ods_ir.StringAttr(self.operation.attributes["constantPauli"])

  @constantPauli.setter
  def constantPauli(self, value):
    if value is not None:
      self.operation.attributes["constantPauli"] = value
    elif "constantPauli" in self.operation.attributes:
      del self.operation.attributes["constantPauli"]

  @constantPauli.deleter
  def constantPauli(self):
    del self.operation.attributes["constantPauli"]
