...
class Token():
    ...
    @classmethod
    def identifier_for_string(cls, string: str) -> Token:
...
class Lexeme():
    ...
    def __init__(self, text: str, token: Token) -> None:
        self.text = text
        self.token = token
    ...
    def __str__(self) -> str:  # pragma: no cover
        return f"<Lexeme {self.token.name}: {repr(self.text)}>"
