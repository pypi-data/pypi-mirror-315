...
class ParameterTestCase(unittest.TestCase):
    ...
    def assertRaisesTypeError(self, attr, actual, expected):
        ...
        with self.assertRaisesRegex(ConfigConstructError, expected_message):
            yield
    ...
    def test_constructor_raises_if_name_is_not_valid_identifier(self):
        with self.assertRaisesRegex(ConfigConstructError, r"1.name \(= '1'\) must match regex"):
            self.subject_factory(name="1")

    def test_constructor_raises_if_default_is_not_instance_of_type(self):
        with self.assertRaisesRegex(ConfigConstructError, "some_parameter.default .* is not of type int"):
    ...
    def test_constructor_raises_if_key_is_not_a_valid_identifier(self):
        with self.assertRaisesRegex(ConfigConstructError, r"some_parameter.key \(= '1'\) does not match regex"):
            self.subject_factory("some_parameter", key="1")
    ...
    def test_constructor_raises_if_parser_is_not_a_function_that_takes_a_single_argument(self):
        ...
        with self.assertRaisesRegex(ConfigConstructError, "some_parameter.parser .* must have arity of 1"):
    ...
    def test_constructor_raises_if_serializer_is_not_a_function_that_takes_a_single_argument(self):
        ...
        with self.assertRaisesRegex(ConfigConstructError, "some_parameter.serializer .* must have arity of 1"):
    ...
    def test_constructor_raises_if_validation_is_not_a_function_that_takes_a_two_argument(self):
        ...
        with self.assertRaisesRegex(ConfigConstructError, "some_parameter.validation .* must have arity of 2"):
    ...
    def test_copy_creates_a_near_identical_copy(self):
        ...
        self.assertEqual(
            {k: v for (k, v) in subject.__dict__.items() if "parent" != k},
            {k: v for (k, v) in copy.copy(subject).__dict__.items() if "parent" != k}
        )
    ...
    def test_constructor_raises_if_default_function_takes_wrong_number_of_arguments(self):
        ...
        with self.assertRaisesRegex(ConfigConstructError, "some_parameter.default .* must have arity of 2"):
