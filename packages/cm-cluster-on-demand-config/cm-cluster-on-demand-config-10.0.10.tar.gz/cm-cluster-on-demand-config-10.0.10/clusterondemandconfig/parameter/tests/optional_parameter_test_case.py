...
class OptionalParameterTestCase(ParameterTestCase):
    ...
    def test_constructor_raises_if_env_does_not_match_regexp(self):
        with self.assertRaisesRegex(ConfigConstructError, r"some_parameter.env \(= 'env'\) does not match regex"):
    ...
    def test_constructor_raises_if_flag_is_not_a_valid_flag(self):
        with self.assertRaisesRegex(ConfigConstructError, "some_parameter.flag __foo-bar does not match regex "):
