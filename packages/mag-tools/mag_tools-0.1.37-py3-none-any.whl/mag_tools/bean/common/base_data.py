...
class BaseData:
    __text_format: TextFormat
    __data_formats: Optional[dict]

    def __init__(self):
        self.__text_format = TextFormat(number_per_line=4, justify_type=JustifyType.LEFT, at_header='',
                                        decimal_places=4,
                                      decimal_places_of_zero=1)
        self.__data_formats = None

    def set_pad_length(self, pad_lengths: dict[str, int]):
        for arg_name, value in pad_lengths.items():
            data_format = self.__get_data_format(arg_name)
            if data_format is None:
                data_format = self.__text_format.get_data_format_by_value(vars(self).get(arg_name))
            data_format.pad_length = value

    def set_justify_type(self, justify_type: JustifyType):
        self.__text_format.justify_type = justify_type
        for data_format in self.__data_formats:
            data_format.justify_type = justify_type

    def set_decimal_places(self, decimal_places):
        self.__text_format.decimal_places = decimal_places
        for data_format in self.__data_formats:
            data_format.decimal_places = decimal_places

    def set_decimal_places_of_zero(self, decimal_places_of_zero):
        self.__text_format.decimal_places_of_zero = decimal_places_of_zero
        for data_format in self.__data_formats:
            data_format.decimal_places_of_zero = decimal_places_of_zero

    def set_number_per_line(self, number_per_line):
        self.__text_format.number_per_line = number_per_line

    def set_at_header(self, at_header):
        self.__text_format.at_header = at_header

    def set_scientific(self, scientific):
        self.__text_format.scientific = scientific
    ...
    def __get_data_format(self, arg_name: str) -> DataFormat:
        if self.__data_formats is None:
            self.__data_formats = {}
            for name, value in vars(self).items():
                if name not in ['text_format', 'data_formats']:
                    self.__data_formats[name] = self.__text_format.get_data_format_by_value(value)

        return self.__data_formats[arg_name]
...
if __name__ == '__main__':
    ...
    data.set_pad_length({'name': 20, 'age': 8, 'height': 10})
    print(data.get_text(['name', 'age', 'height'], ','))
