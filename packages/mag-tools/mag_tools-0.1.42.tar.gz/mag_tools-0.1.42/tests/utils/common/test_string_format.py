...
class TestStringFormat(unittest.TestCase):
    def test_pad_string(self):
        # 测试右对齐
        self.assertEqual(StringFormat.pad_value("test", 10, JustifyType.RIGHT), "      test")
        # 测试居中对齐
        ...
        # 测试字符串长度大于目标长度
        self.assertEqual(StringFormat.pad_value("test", 2, JustifyType.RIGHT), "test")

    def test_pad_strings(self):
        # 测试多个字符串右对齐
        strings = ["test1", "test2", "test3"]
        expected_output = "     test1     test2     test3"
        self.assertEqual(expected_output, StringFormat.pad_values(strings,  10, JustifyType.RIGHT))
        # 测试多个字符串居中对齐
        expected_output = " test1   test2   test3  "
