...
class Token[ChainType: Chain](Asset[ChainType]):
    ...
    async def register(
        ...
        if token := Token.lookup_cache[client.chain].get(address):
            return token
                ...
                match client:
                    ...
                    case _:
                        raise NotImplementedError(
                            f"Unimplemented chain: {client.chain}"
                        )
        ...
        Token.lookup_cache[token.chain].update(
    ...
    @staticmethod
    def lookup_symbol(chain: ChainType, symbol: str) -> Token[ChainType]:
        return Token.lookup_cache[chain][symbol]

    @staticmethod
    def try_lookup_symbol(chain: ChainType, symbol: str) -> Optional[Token[ChainType]]:
        return Token.lookup_cache[chain].get(symbol)

    @staticmethod
    def lookup_address(chain: ChainType, address: str) -> Token[ChainType]:
        return Token.lookup_cache[chain][address]

    @staticmethod
    def try_lookup_address(
        chain: ChainType, address: str
    ) -> Optional[Token[ChainType]]:
        return Token.lookup_cache[chain].get(address)

    @staticmethod
    def from_str(string: str) -> Token[ChainType]:
        chain, symbol = string.split("-")
        return Token.lookup_symbol(Chain.from_str(chain), symbol)
    ...
    async def transfer_checked(
        ...
        assert await self.get_balance(sender.downcast()) >= amount, "Insufficient balance"
