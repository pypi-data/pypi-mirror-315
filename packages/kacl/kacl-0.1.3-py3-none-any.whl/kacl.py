...
def run(command: str) -> str:
    ...
    return subprocess.check_output(command, shell=True).decode("utf-8").strip()  # noqa: S602
...
def git_commits(start: str, end: str) -> list[str]:
    """Gets commits between two refs."""
    range = f"{start + '..' if start else ''}{end or git_last_sha()}"
    output = run(f"git log --pretty=format:%s {range}")
...
def add_pr_links(url: str, message: str) -> str:
    """Links PRs in a commit message."""
    return re.sub(r"(?<!\[)#(\d+)", rf"[#\1]({url}/pull/\1)", message)
...
class Release:
    ...
    def format(self, url: str) -> str:
        ...
        output.append("")
        for category, messages in sorted(self.entries.items()):
            if messages:
                output.append(f"### {category}")
                output.append("")
                output.extend(
                    add_pr_links(url, m) if url else m for m in messages
                )
                output.append("")
        return "\n".join(output)
...
def classify_commits(commits: list[str]) -> Entries:
    """Categorizes commits into changelog sections."""
    entries: Entries = {}

    def append(category: str, message: str) -> None:
        entries.setdefault(category, []).append(f"- {message}")

    for commit in commits:
        if starts_with_any(commit, IGNORED_PREFIXES):
            continue
        for category, prefixes in PREFIXES.items():
            if starts_with_any(commit, prefixes):
