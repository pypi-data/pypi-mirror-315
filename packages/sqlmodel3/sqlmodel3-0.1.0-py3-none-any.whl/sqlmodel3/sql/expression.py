...
from sqlalchemy3.orm import InstrumentedAttribute, Mapped
from sqlalchemy3.sql._typing import (
    _ColumnExpressionArgument,
    _ColumnExpressionOrLiteralArgument,
    _ColumnExpressionOrStrLabelArgument,
)
from sqlalchemy3.sql.elements import (
    BinaryExpression,
    Case,
    Cast,
    CollectionAggregate,
...
from sqlalchemy3.sql.type_api import TypeEngine
...
def all_(expr: Union[_ColumnExpressionArgument[_T], _T]) -> CollectionAggregate[bool]:
    return sqlalchemy3.all_(expr)  # type: ignore[arg-type]
...
def and_(
    ...
    *clauses: Union[_ColumnExpressionArgument[bool], bool],
    ...
    return sqlalchemy3.and_(initial_clause, *clauses)  # type: ignore[arg-type]


def any_(expr: Union[_ColumnExpressionArgument[_T], _T]) -> CollectionAggregate[bool]:
    return sqlalchemy3.any_(expr)  # type: ignore[arg-type]


def asc(
    column: Union[_ColumnExpressionOrStrLabelArgument[_T], _T],
) -> UnaryExpression[_T]:
    return sqlalchemy3.asc(column)  # type: ignore[arg-type]


def collate(
    expression: Union[_ColumnExpressionArgument[str], str], collation: str
) -> BinaryExpression[str]:
    return sqlalchemy3.collate(expression, collation)  # type: ignore[arg-type]
...
def between(
    ...
    return sqlalchemy3.between(expr, lower_bound, upper_bound, symmetric=symmetric)


def not_(clause: Union[_ColumnExpressionArgument[_T], _T]) -> ColumnElement[_T]:
    return sqlalchemy3.not_(clause)  # type: ignore[arg-type]
...
def case(
    *whens: Union[
    ...
    return sqlalchemy3.case(*whens, value=value, else_=else_)  # type: ignore[arg-type]
...
def cast(
    ...
    return sqlalchemy3.cast(expression, type_)
...
def try_cast(
    ...
    return sqlalchemy3.try_cast(expression, type_)


def desc(
    column: Union[_ColumnExpressionOrStrLabelArgument[_T], _T],
) -> UnaryExpression[_T]:
    return sqlalchemy3.desc(column)  # type: ignore[arg-type]


def distinct(expr: Union[_ColumnExpressionArgument[_T], _T]) -> UnaryExpression[_T]:
    return sqlalchemy3.distinct(expr)  # type: ignore[arg-type]


def bitwise_not(expr: Union[_ColumnExpressionArgument[_T], _T]) -> UnaryExpression[_T]:
    return sqlalchemy3.bitwise_not(expr)  # type: ignore[arg-type]


def extract(field: str, expr: Union[_ColumnExpressionArgument[Any], Any]) -> Extract:
    return sqlalchemy3.extract(field, expr)


def funcfilter(
    func: FunctionElement[_T], *criterion: Union[_ColumnExpressionArgument[bool], bool]
) -> FunctionFilter[_T]:
    return sqlalchemy3.funcfilter(func, *criterion)  # type: ignore[arg-type]
...
def label(
    ...
    return sqlalchemy3.label(name, element, type_=type_)  # type: ignore[arg-type]


def nulls_first(
    column: Union[_ColumnExpressionArgument[_T], _T],
) -> UnaryExpression[_T]:
    return sqlalchemy3.nulls_first(column)  # type: ignore[arg-type]


def nulls_last(column: Union[_ColumnExpressionArgument[_T], _T]) -> UnaryExpression[_T]:
    return sqlalchemy3.nulls_last(column)  # type: ignore[arg-type]
...
def or_(
    ...
    *clauses: Union[_ColumnExpressionArgument[bool], bool],
    ...
    return sqlalchemy3.or_(initial_clause, *clauses)  # type: ignore[arg-type]
...
def tuple_(
    *clauses: Union[_ColumnExpressionArgument[Any], Any],
    ...
    return sqlalchemy3.tuple_(*clauses, types=types)  # type: ignore[return-value]
...
def type_coerce(
    ...
    return sqlalchemy3.type_coerce(expression, type_)


def within_group(
    element: FunctionElement[_T], *order_by: Union[_ColumnExpressionArgument[Any], Any]
) -> WithinGroup[_T]:
    return sqlalchemy3.within_group(element, *order_by)
