...
class Document:
    ...
    def __str__(self) -> str:
        ...
        links_str = f", links: {self.links}" if self.links else ""
        desc_str = f", description: {self.description}" if self.description else ""
        return f"Document(title: {self.title}, repository: {self.repository}{desc_str}{links_str}, content: {content_preview})"
...
class DocumentationFileExplorer:
    def __init__(
        ...
        valid_file_types: set[str] = {"md", "mdx"},
        # TODO Need a better story for passing in directories to skip on a per-repo basis, setting up per-repo configs in general.
        skip_dirs: set[str] = {"docs-old", "prompt-library", "admin-api"},
    ):
        self.root_paths = {name: Path(path) for name, path in root_paths.items()}
        self.documents: dict[DocumentKey, Document] = {}
        self.skip_dirs = skip_dirs
        self.valid_file_types = valid_file_types


    def _document_key(self, file_path: Path | str, repository: str) -> DocumentKey:
        return DocumentKey(path=str(file_path), repository=repository)

    def get(self, repository: str, path: str) -> Document | None:
        key = self._document_key(repository=repository, file_path=path)
        return self.documents.get(key, None)

    async def crawl_documents(self):
        """
        Asynchronously crawl through directories starting from each root_path and collect
        documents with valid file extensions. Stores results in self.documents with
        paths (without extension) as keys and file contents as values.
        """
        tasks = []
        for repo_name, root_path in self.root_paths.items():
            # Create tasks for processing each file
            for file_path in root_path.rglob("*"):
                tasks.append(self._parse_document(file_path, repo_name, root_path))
    ...
    def _extract_title_and_description(self, frontmatter: dict, content: str) -> tuple[str | None, str | None, str]:
        ...
        if not description:
            description, remaining_content = self._extract_description(remaining_content)

        return title, description, remaining_content
    ...
    # TODO: consider removing tag extraction
    ...
    async def _parse_document(self, file_path: Path, repository: str, root_path: Path):
        ...
        try:
            ...
            key = self._document_key(file_path=relative_path.with_suffix(''), repository=repository)

            # Extract document components
            frontmatter, main_content = self._extract_frontmatter(content)
            title, description, final_content = self._extract_title_and_description(frontmatter, main_content)
