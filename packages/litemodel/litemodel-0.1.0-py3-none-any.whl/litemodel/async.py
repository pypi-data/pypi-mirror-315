...
class Model:
    ...
    async def find_by(cls, field_name: str, value: SQL_TYPE):
        # TODO: update this to take **kwargs and updated the find by template to deal with multiple inputs
        template = Template(FIND_BY_TEMPLATE)
        sql_statement = template.render({"table": cls._name, "field": field_name})
        result = await sql_select(sql_statement, (value,))
        return cls(**dict(next(iter(result))))
