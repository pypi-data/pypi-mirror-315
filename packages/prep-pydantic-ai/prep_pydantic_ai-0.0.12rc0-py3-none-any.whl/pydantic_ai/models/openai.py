...
class OpenAIAgentModel(AgentModel):
    ...
    def _map_message(message: Message) -> chat.ChatCompletionMessageParam:
        ...
        if message.role == 'system':
            # SystemPrompt ->
            return chat.ChatCompletionSystemMessageParam(role='system', content=message.content, name=message.name)
        elif message.role == 'user':
            # UserPrompt ->
            return chat.ChatCompletionUserMessageParam(role='user', content=message.content, name=message.name)
            ...
            if message.tool_name is None:
                return chat.ChatCompletionUserMessageParam(role='user', content=message.model_response(), name=message.name)
            else:
                return chat.ChatCompletionToolMessageParam(
                    role='tool',
                    tool_call_id=_guard_tool_id(message),
            ...
            # ModelTextResponse ->
            return chat.ChatCompletionAssistantMessageParam(role='assistant', content=message.content, name=message.name)
