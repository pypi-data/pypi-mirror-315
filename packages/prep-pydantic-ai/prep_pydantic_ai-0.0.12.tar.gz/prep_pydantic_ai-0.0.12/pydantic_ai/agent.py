...
class Agent(Generic[AgentDeps, ResultData]):
        ...
        with _logfire.span(
            ...
            while True:
                ...
                with _logfire.span('model request', run_step=run_step) as model_req_span:
                    ...
                    if hasattr(model_response, 'name') and not model_response.name:
                        model_response.name = self.name
    ...
    async def _prepare_messages(
        ...
        messages.append(_messages.UserPrompt(user_prompt, name=self.name))
    ...
    async def _init_messages(self, deps: AgentDeps) -> list[_messages.Message]:
        ...
        messages: list[_messages.Message] = [_messages.SystemPrompt(p, name=self.name) for p in self._system_prompts]
