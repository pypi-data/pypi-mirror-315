...
\setcounter{page}{1}    % otherwise it's 2
    ...
    A hybrid application, originally named Skyalmanac, was developed using Ephem to calculate 'events' and Skyfield for the rest. This was indeed much faster at the cost of poorer 'event time' data. It took a while to find a better solution: multiprocessing, which was built into SFalmanac. This now could compare to the execution times in Pyalmanac but with improved data.\\[12pt]
    New functionality was added to SFalmanac, e.g. lunar phase as a graphic; Lunar Distance tables and charts. The original Skyalmanac is deprecated and has now been replaced with the latest SFalmanac code, so Skyalmanac and SFalmanac are now identical apart from the name.
