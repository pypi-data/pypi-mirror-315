...
from dektools.str import slugify, replace
...
class TextNode(Node):
    def _get_args(self):
        ...
        return text, wrap, font, color

    def update(self, content):
        self.content[0]['text'] = content

    def render(self, canvas: 'PsdCanvas', draw: ImageDraw, image: Image):
        text, wrap, font, color = self._get_args()
        if wrap:
...
class PsdCanvas:
    ...
    @classmethod
    def load(cls, file, short: int | None = 15, suffix: str = '-'):
...
def get_psd_names(psd, short, suffix):
    ...
    for name, lid in sorted(((slugify(layer.name, True), layer.layer_id) for layer in psd), key=lambda x: x[0]):
        _short = len(name) if short is None else min(short, len(name))
        while True:
            _name = name[:_short] + max(0, _short - len(name)) * suffix
