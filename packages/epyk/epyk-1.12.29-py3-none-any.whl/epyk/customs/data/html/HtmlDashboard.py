...
class Pivots(Html.Html):
    ...
    def __str__(self):
        ...
        if self.sub_rows is not None:
            self.container.add([
                self.page.ui.text(
                    "Rows <i style='font-size:%s'>(unique field)</i>" % self.page.body.style.globals.font.normal(
                        -3)).css(
                    {"display": "block"}), self.rows,
                self.page.ui.text(
                    "Sub Rows <i style='font-size:%s'>(unique field)</i>" % self.page.body.style.globals.font.normal(
                        -3)).css(
                    {"display": "block"}),
                self.sub_rows])
        else:
