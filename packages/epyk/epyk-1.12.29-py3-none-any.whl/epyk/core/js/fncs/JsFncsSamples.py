...
class Samples:
    ...
    def scale_linear(self, count_: int = None, attrs: dict = None):
        ...
        self.page._props['js'].setdefault('functions', {})["scaleLinear"] = {
            'content': '''var cfg = config || {}; var count = cfg.count || 12; var labels = [];
for (let i = 0; i < count; ++i) {labels.push(i.toString());} return labels ''',
            'pmt': ["config"]}
        attrs = attrs or {}
...
for (i = 0; i < count; ++i) {
  ...
  data.push(Math.round(dfactor * value) / dfactor);
    ...
    def points(self, count_: int = None, min_: int = 0, max_: int = 1, attrs: dict = None):
        ...
        if max_ is not None:
            attrs["max"] = max_
        return JsUtils.jsWrap('''(function(config){var records = []; var count = config.count || 12; var labels = [];
for (let i = 0; i < count; ++i) {records.push({x: sampleRand(config), y: sampleRand(config)})};
return records})(%s)''' % JsUtils.jsConvertData(attrs, None))
