...
class Calendar(Html.Html):
    ...
    def task(self, name: str, start: str, capacity: Union[List[float], float], end: Optional[str] = None,
        ...
        :param end: Optional. The task end date format YYYY-MM-DD
        ...
        for i, day in enumerate(self.val):
            if 'number' in day:
                ...
                tasks = "<div>%s</div>" % "".join([
                                                      "<div style='width:100%%;height:20px;display:block;vertical-align:middle'><div style='background:%(color)s;width:100%%;height:%(capacity)s%%;display:inline-block' title='%(name)s: %(capacity)s%%'></div></div>" % t
                                                      for t in day.get("tasks", [])])
                ...
                if day.get("today", False):
                    row.append(
                        ...
                        "".join(tooltip), cell_style, self.page.theme.success.light, numer_day, tasks))
                else:
                    row.append(
