...
class CellComponent(JsPackage):
    ...
    def getRow(self):
        ...
        Related Pages:
          ...
          http://tabulator.info/docs/4.4/components
        """
        return RowComponent(selector="%s.getRow()" % self.toStr(), set_var=False, component=self.component,
                            page=self.page)
    ...
    def restoreOldValue(self):
        ...
        Related Pages:
          ...
          http://tabulator.info/docs/4.4/components
        """
        return JsObjects.JsString.JsString("%s.restoreOldValue()" % self.toStr(), page=self.page,
    ...
    def setValue(self, text, flag: bool = True, dataflows: List[dict] = None):
        """
        ...
        """
        text = JsUtils.dataFlows(text, dataflows, self.page)
    ...
    Related Pages:
      ...
      http://tabulator.info/docs/4.4/components
    """
        return JsObjects.JsBoolean.JsBoolean("%s.checkHeight()" % self.toStr(), page=self.page,
                                             component=self.component)

    def edit(self, flag: bool = True):
        """
        ...
        """
        flag = JsUtils.jsConvertData(flag, None)
        return JsObjects.JsObject.JsObject("%s.edit(%s)" % (self.toStr(), flag), page=self.page,
                                           component=self.component)
...
class ColumnComponent(JsPackage):
    ...
    def getDefinition(self):
        ...
        return JsObjects.JsObject.JsObject("%s.getDefinition()" % self.toStr(), page=self.page,
                                           component=self.component)
    ...
    def getVisibility(self):
        ...
        """
        return JsObjects.JsBoolean.JsBoolean("%s.getVisibility()" % self.toStr(), page=self.page,
                                             component=self.component)
...
class ColumnComponents(JsPackage):
    ...
    def forEach(self, js_funcs: types.JS_FUNCS_TYPES, profile: types.PROFILE_TYPE = None):
        ...
        return self.fnc_closure(
            "forEach(function(rec){%s})" % JsUtils.jsConvertFncs(js_funcs, toStr=True, profile=profile))
    ...
    def rename(self, field: str = None, title: str = None, columns=None):
        ...
        if columns is not None:
            ...
            return JsObjects.JsVoid(
                "var colMaps = %s; %s.forEach(function(rec){if(colMaps[rec.getField()]){rec._column.contentElement.innerText = colMaps[rec.getField()]}})" % (
                columns, self._selector))

        title = JsUtils.jsConvertData(title, None)
        field = JsUtils.jsConvertData(field, None)
        return JsObjects.JsVoid(
            "%s.forEach(function(rec){if(rec.getField() == %s){rec._column.contentElement.innerText = %s}})" % (
            self._selector, field, title))


class RowComponent(JsPackage):
    lib_selector = "row"

    def update(self, data: Union[primitives.JsDataModel, dict], dataflows: List[dict] = None):
        """
    ...
    def normalizeHeight(self):
        ...
        it may sometimes be necessary to recalculate the height of all the cells in the row to make sure they remain aligned
        """
        return JsObjects.JsObject.JsObject("%s.normalizeHeight()" % self.toStr(), page=self.page,
                                           component=self.component)
    ...
    def getTreeChildren(self):
        return JsObjects.JsArray.JsArray("%s.getTreeChildren()" % self._selector, page=self.page,
                                         component=self.component)

    def addTreeChild(self, data, dataflows: List[dict] = None):
        """
        ...
        return JsObjects.JsArray.JsArray(
            "%s.addTreeChild(%s)" % (self._selector, data), page=self.page, component=self.component)

    def addTreeChildren(self, data, dataflows: List[dict] = None):
        """
        ...
        return JsObjects.JsArray.JsArray("%s.forEach(function(rec){%s.addTreeChild(rec)})" % (
            data, self._selector), page=self.page, component=self.component)
...
class TabRowContextMenu(JsPackage):
    ...
    def add(self, name: str, url: str, icon: str = None):
        ...
        if icon is not None:
            return JsObjects.JsVoid(
                "%s.options.rowContextMenu.push({label: '<i class=\"%s\" style=\"margin-right:5px\"></i>%s', action: function(e, row){var data = {row: row.getData(), label: '%s'}; %s('%s', data)} })" % (
                self.toStr(), icon, name, name, js_service, url))

        return JsObjects.JsVoid(
            "%s.options.rowContextMenu.push({label: '%s', action: function(e, row){var data = {row: row.getData(), label: '%s'}; %s('%s', data)} })" % (
            self.toStr(), name, name, js_service, url))
...
class Tabulator(JsPackage):
    ...
    def alert(self, text: str):
        """
        To show an alert, call the alert function on the table. Passing the message into the first argument of the
        function.
        ...
        if rows is not None or values is not None:
            ...
            if rows is None and rows_fields:
                ...
                rows = JsObjects.JsObjects.get(
                    "(function(d){var results = []; d.forEach(function(rec){if(typeof rec === 'string'){rec = {'field': rec, 'title': rec}}; results.push( Object.assign(%s, rec))}); return results})(%s)" % (
                    JsUtils.jsConvertData(self.component.options.get({}, "rows_def"), None), rows or []))
                values = JsObjects.JsObjects.get(
                    "(function(d){var results = []; d.forEach(function(rec){if(typeof rec === 'string'){rec = {'field': rec, 'title': rec}}; results.push( Object.assign(%s, rec))}); return results})(%s)" % (
                    JsUtils.jsConvertData(self.component.options.get({}, "columns_def"), None), values or []))
    ...
    def values(self, data: types.JS_DATA_TYPES, columns=None, options=None, dataflows: List[dict] = None):
        """
        ...
        if columns is None:
            ...
            return self.component.build(data, dataflows=dataflows)

        # Change the columns and replace them with the new ones
        return JsUtils.jsConvertFncs(
            [self.columns(values=columns, options=options), self.component.build(data, dataflows=dataflows)],
            toStr=True)
    ...
    def getColumns(self):
        ...
        """
        columns = ColumnComponents(self.component, selector="%s.getColumns()" % self.varId, set_var=False,
                                   page=self.page)
        self.fnc(columns)
        return columns

    def addColumn(self, data: types.JS_DATA_TYPES, before=False, position=""):
        """
    ...
    def addColumns(self, data: types.JS_DATA_TYPES, before=False, position="", options=None):
        """
    ...
    def deleteColumn(self, data: types.JS_DATA_TYPES):
        """
    ...
    def setSort(self, data: types.JS_DATA_TYPES):
        """
...
%(varId)s.setColumns(%(htmlCode)s_columns)''' % {
    ...
    def setData(self, data, dataflows: List[dict] = None):
        """

        :param data:
        :param dataflows: Chain of data transformations
        """
        return JsObjects.JsVoid("%s.setData(%s)" % (self.varId, JsUtils.dataFlows(data, dataflows, self.page)))

    def setDataFromArray(self, data, header=None, formatters=None, dataflows: List[dict] = None):
        """
...
dataContemt.slice(1).forEach(function(v){var row = {}; dataContemt[0].forEach(function(c, i){row[c] = v[i]})
   if (formatters != null){for (var key in formatters){ row[key] = formatters[key](row)}}
   resultContent.push(row)}); %(varId)s.setData(resultContent)''' % {"varId": self.varId, "formatters": formatters, "data": data}

    def replaceData(self, data: types.JS_DATA_TYPES = None, dataflows: List[dict] = None):
        """
        ...
        """
        if data is None:
            return JsObjects.JsObject.JsObject("%s.replaceData()" % self.varId, component=self.component,
                                               page=self.page)

        return JsObjects.JsObject.JsObject(
            "%s.replaceData(%s)" % (self.varId, JsUtils.dataFlows(data, dataflows, self.page)),
            component=self.component, page=self.page)
