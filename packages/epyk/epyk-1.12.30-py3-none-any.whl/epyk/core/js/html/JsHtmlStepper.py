...
shape.setAttribute('stroke', options.backgrounds[step.status]); shape.setAttribute('stroke-width', 1);
shape.setAttribute('cx', width / options.circle_factor); shape.setAttribute('cy', height / options.circle_factor);
...
class Step(JsNodeDom.JsDoms):
    ...
    def colors(self, colors: Union[list, str, dict], status: str = 'bespoke', clear_gradient: bool = True):
        ...
        :param status: Optional. The status code for the step item
        :param clear_gradient: Optional. Specify if the color need a gradient
        """
...
var gradient = document.createElementNS(%(svgns)s, 'linearGradient'); var gradient_colors = %(colors)s;
for (var i = 0, length = gradient_colors.length; i < length; i++) {
    var stop = document.createElementNS(%(svgns)s, 'stop'); stop.setAttribute('offset', gradient_colors[i].offset);
    stop.setAttribute('stop-color', gradient_colors[i].color); gradient.appendChild(stop)}
...
''' % {"colors": colors, 'comp': self.varName, 'svgns': JsUtils.jsConvertData(Defaults.SVGNS, None),
    ...
    def shape(self, shape: str, status: str = 'success', step: Union[primitives.JsDataModel, dict] = None,
        ...
        :param shape: The shape alias of the step
        :param status: Optional. The status of the step
        :param step: Optional.
        :param options: The stepper options to be changed
        """
        step = {"status": status} if step is None else JsUtils.jsConvertData(step, None)
        return JsObjects.JsObjects.get('''
var svgns = '%(svgns)s'; %(comp)s.querySelector('svg').remove(); %(shape)s(%(comp)s, %(options)s, %(step)s)
...
''' % {'comp': self.varName, 'options': self.component.options.config_js(options), 'step': step})
    ...
    def rectangle(self, status: str = 'success', step: Union[primitives.JsDataModel, dict] = None,
                  options: dict = None):
        """
        Hide all the panels in the drawer component.
...
''' % {'comp': self.varName, 'options': self.component.options.config_js(options), 'step': step})
    ...
    def label(self, value: Union[str, primitives.JsDataModel]):
        ...
        :param value: The text to be added
        """
    ...
    def text(self, text: Union[str, primitives.JsDataModel], x: float = None, y: float = None,
        ...
        :param x: Optional. The x position
        :param y: Optional. The y position
...
if (textNodes.length == 0){
  ...
  newText.setAttributeNS(null, 'x', %(x)s); newText.setAttributeNS(null, 'y', %(y)s); var css = %(css)s;
  ...
  textNodes[0].innerHTML = %(text)s;
    ...
    def blink(self, color: str = "#FFDEB3", border: str = "#FF9200"):
        ...
        :param color: Optional. The hexadecimal color code
        :param border: Optional. The hexadecimal color code
        """
        return JsObjects.JsObjects.get('''
    ...
    def click(self, js_funcs: Union[list, str], profile: Optional[Union[bool, dict]] = None, source_event: str = None,
        ...
        return JsObjects.JsObjects.get(
            '%(src)s.style.cursor = "pointer"; %(src)s.addEventListener("click", function(){%(fncs)s})' % {
                "src": source_event or self.varName,
                "fncs": JsUtils.jsConvertFncs(js_funcs, toStr=True, profile=profile)})
...
class Stepper(JsHtml.JsHtmlRich):
    ...
    def _get_index(self) -> str:
        ...
        return self.page.properties.js.add_constructor('StepByName',
                                                       "function StepByName(doms, label){var index_svg = -1; doms.childNodes.forEach(function(dom, i){if(dom.querySelector('[name=label]').innerHTML == label){ index_svg = i}}); return index_svg}")
    ...
    def __getitem__(self, i):
        return Step(self, component=self.component, js_code=self.querySelectorAll("div[name=svg_holder]")[i],
                    page=self.page)
...
if(%(StepByName)s(%(holder)s, %(label)s) == -1){
  ...
  var div = document.createElement("DIV"); div.setAttribute('title', %(tooltip)s);
  ...
  div.appendChild(span); li.appendChild(div); div.id = "svg_" + %(holder)s.childElementCount;
