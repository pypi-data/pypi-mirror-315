...
function ekPieECharts(data, options){
    ...
    if (data && data.length > 0){
        ...
        yDefs.forEach(function(series, i){
            let dataSet = {data: [], type: options._ek.chart.type, name: series, color: options._ek.colors};
            labels.forEach(function(x){
                if (temp[series][x] == undefined){dataSet.data.push({value: null, name: x })}
                else {dataSet.data.push({value: temp[series][x], name: x })}
            });
