...
class Select(Html.Html):
    ...
    def style(self) -> GrpClsList.ClassSelect:
        """A property to the CSS style of the DOM component. Each component will have default CSS style but they can be
        overridden."""
    ...
    def js(self) -> JsSelect.JSelect:
        ...
        `snapappointments <https://developer.snapappointments.com/bootstrap-select/methods/>`_
        :return: A Javascript Dom object
        """
        if self._js is None:
    ...
    _js__builder__ = '''
var selectObj = %s; selectObj.empty(); const attrs = ['icon', 'content']; var selections = [];
...
selectObj.val(selections).change()''' % JsQuery.decorate_var("htmlObj", convert_var=False)
    ...
    def ajax(self, url: str, js_data: types.JS_FUNCS_TYPES = "function (){return {q: '{{{q}}}'}}", is_json: bool = True,
        ...
        `truckingsim <https://github.com/truckingsim/Ajax-Bootstrap-Select>`_
