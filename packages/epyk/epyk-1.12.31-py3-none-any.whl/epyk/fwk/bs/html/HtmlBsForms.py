...
class BsCheck(Component):
    ...
    str_repr = '''<div class="{container_class}">
<input type="{type}" value="" {attrs}><label class="form-check-label" for="{for}">{text}</label></div>'''
...
class BsSelect(Component):
    ...
    str_repr = '''<select {attrs}>{sub_items}</select>'''
    dyn_repr = '''{sub_item}'''

    @property
    def parsers(self) -> SelectionBox:
    ...
    def add_option(self, value: str, label: str, selected: bool = False, options: dict = None) -> Option:
...
class BsDataList(Component):
    ...
    _option_cls = OptBsForms.Check
    init_selected = None
    str_repr = '''<input {attrs}><datalist id="{htmlCode}Options">{sub_items}</datalist>'''
    dyn_repr = '''{sub_item}'''
...
class BsRange(Component):
    ...
    str_repr = '''<label for="customRange2" class="form-label">Example range</label>
...
class BsSFloatingLabel(Component):
    ...
    str_repr = '''<div class="form-floating"><input {attrs}><label for="{for}">{label}</label></div>'''
    ...
    def write_values(self) -> dict:
        ...
        return {"label": self._vals['label'], "for": self.html_code}
...
class BsInputGroup(Component):
    ...
    str_repr = '''<div class="input-group mb-3">
...
class BsSFloatingSelect(BsSFloatingLabel):
    ...
    str_repr = '''<div class="form-floating"><select {attrs}>{sub_items}</select>
<label for="floatingSelect">{label}</label></div>'''
