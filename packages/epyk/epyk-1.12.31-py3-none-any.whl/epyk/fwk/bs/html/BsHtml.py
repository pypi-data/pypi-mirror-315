...
class BsComposite(html.HtmlTextComp.Composite):
    ...
    def _get_comp_map(self) -> dict:
        ...
        if self.extended_map is None:
            ...
            self.extended_map.update({'close': self.page.web.bs.buttons.close})
...
class Section(html.Html.Html):
    ...
    def __str__(self):
        cols = "".join(
            [htmlObj.html() if hasattr(htmlObj, 'html') else str(htmlObj) for i, htmlObj in enumerate(self.val)])
...
class BsModals(html.Html.Html):
    ...
    def __str__(self):
        return '<div %s>%s</div>' % (
          self.get_attrs(css_class_names=self.style.get_classes()), self.dialog.html())
