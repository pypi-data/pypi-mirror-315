...
''' % {"lightGrey": page.theme.greys[1], "fontSize": page.body.style.globals.font.normal(),
       "color": page.theme.notch(), "black": page.theme.dark_or_white(False),
...
let listData = %s; var dummy = document.createElement("textarea"); document.body.appendChild(dummy);
dummy.value = JSON.stringify(listData); dummy.select( document.execCommand("copy");
document.body.removeChild(dummy)''' % self.all.toStr())
...
%s.childNodes.forEach( function(dom, k){  
  ...
  if (item != null){ 
    item.checked = true; dom.querySelector('[data-valid]').setAttribute("data-valid", true);
}})''' % self.varName)
...
if(itemOptions.delete){
  var close = document.createElement("i"); close.setAttribute("class", itemOptions.delete_icon);
  ...
  li.lastChild.style.display = 'inline-block'; li.appendChild(close); const cls = %(cls)s;
    ...
    def append(self, items: list, css_attrs: dict = None, css_cls: str = None, options: dict = None):
        ...
        return JsObjects.JsVoid(
            "%s.forEach(function(newItem){%s})" % (items, self.add(
                JsUtils.jsWrap("newItem"), css_attrs=css_attrs, css_cls=css_cls, before=False,
...
if(enumTags != ''){
  ...
  enumTags.forEach(function(val){
    ...
    if (css != null){
      for (const [key, value] of Object.entries(css)) { item.style[key] = value }
    }
    const cls = %(cls)s;
...
if(window.context_menu_source.getAttribute("name")){ data["source"] = window.context_menu_source.innerHTML } 
...
''']
        ...
        return JsObjects.JsVoid('''
%s.lastChild.addEventListener("contextmenu", function(event){ window.context_menu_source = this; %s});
...
(function(dom){var index = -1; var children = dom.childNodes; var count = 0; 
  for(child in children){if((typeof children[child] === 'object') && (children[child].querySelector('span[name=chip_value]') != null) && children[child].querySelector('span[name=chip_value]').textContent == %(text)s){
    ...
    def values(self, category: Union[str, primitives.JsDataModel] = None):
        if category is None:
            return JsObjects.JsArray.JsArray.get(
                "(function(dom){var content = []; dom.childNodes.forEach(function(rec){content.push(rec.querySelector('span[name=chip_value]').textContent)}); return content})(%s)" % self.querySelector(
                    "div[name=panel]"))
