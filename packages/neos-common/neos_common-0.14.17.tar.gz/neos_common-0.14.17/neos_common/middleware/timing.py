...
def add_timing_middleware(
    ...
    async def timing_middleware(request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        with _TimingStats(metric_name, record=record, exclude=exclude, trace_id=trace_id) as timer:
...
def get_trace_id(request: Request) -> str | None:
    ...
    if timer is not None and isinstance(timer, _TimingStats):
        return timer.trace_id
...
def record_timing(request: Request, note: str | None = None) -> None:
    ...
    if timer is not None and isinstance(timer, _TimingStats):
        ...
        msg = "No timer, or invalid timer, present on request. Ensure timing middleware is added to the app."
...
class _TimingStats:
    ...
    def start(self) -> None:
        ...
        if not self.silent:
            prefix = "TIMING" if self.trace_id is None else f"TIMING ({self.trace_id})"
            message = f"{prefix}: Start: {self.name}"
            self.record(message)
    ...
    def __enter__(self) -> _TimingStats:  # noqa: PYI034
        self.start()
        return self
    ...
    def emit(self, note: str | None = None) -> None:
        ...
        if not self.silent:
            ...
            prefix = "TIMING" if self.trace_id is None else f"TIMING ({self.trace_id})"
            message = f"{prefix}: Wall: {wall_ms:6.1f}ms | CPU: {cpu_ms:6.1f}ms | {self.name}"
            if note is not None:
                message += f" ({note})"
            self.record(message)
