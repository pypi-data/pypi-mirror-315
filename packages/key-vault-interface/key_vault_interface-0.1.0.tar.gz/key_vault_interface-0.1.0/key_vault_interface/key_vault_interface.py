...
class KeyVaultInterface:
    ...
    def __init__(
        ...
        client_secret = os.getenv("KEY_VAULT_CLIENT_SECRET")
        if not client_secret:
            raise ValueError(
                "KEY_VAULT_CLIENT_SECRET environment variable is required but not set."
            )

        # Remove the client_secret from the environment for security
        try:
            os.environ.pop("KEY_VAULT_CLIENT_SECRET")
            log.info("Removed KEY_VAULT_CLIENT_SECRET from runtime environment")
        except Exception as e:
            log.warning(f"Failed to clean KEY_VAULT_CLIENT_SECRET: {e}")
