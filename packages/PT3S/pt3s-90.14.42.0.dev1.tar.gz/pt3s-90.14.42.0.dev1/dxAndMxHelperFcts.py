...
class dxWithMx():
    ...
    def _V3_AGSNVEC(self,V3_AGSN):
        ...
        try: 
            ...
            #logger.debug("Creating rename dictionary for columns.")
            ...
            '''
            # Iterate over the columns of dfAGSNVec again to rename them
            for col in dfAGSNVec.columns:
                # Check if the column is a tuple and contains one of the unique timestamps
                if isinstance(col, tuple) and col[2] in unique_timestamps:
                    # Determine the new column name based on the position of the timestamp in unique_timestamps
                    new_col_name = f"{col[1]}_{unique_timestamps.index(col[2]) + 1}"
                    rename_dict[col] = new_col_name
                    logger.debug(f"Column {col} will be renamed to {new_col_name}.")
            
            # Rename the columns in dfAGSNVec
            dfAGSNVec.rename(columns=rename_dict, inplace=True)
            '''
