...
# Description :
"""
import bceiam.middleware.auth.const as auth
from bceiam.middleware.auth.iam_session import (
    IAMSessionMiddleware,
    iam_session_handler,
)
from bceiam.middleware.auth.idaas_session import (
    IDaasSessionMiddleware,
    idaas_session_handler,
)
from bceiam.middleware.auth.utils import convert_cookie_to_dict
...
def get_authenticate_dependency(config: Config):
    ...
    async def authenticate(request: Request):
        ...
        context.set_var_value(GLOBAL_AUTH_INFO_KEY, result)
