...
def genai_vision_back(detected_text, model):
    result = model.generate_content(
        [detected_text,"\n\n", "give me date of issue(in dd/mm/yy format), expiry date (in dd/mm/yy format) and national number from {detected_text}, please give me output as just dictionary - issuing_date, expiry_date, national_number"]
    )
    return  result.text
...
def syr_passport_extraction_back(passport_text_back, api_key):
    ...
    try:
        ...
        try:
            ...
            passport_back_data = {'issuing_date': '', 'expiry_date': '', 'national_number': ''}
            try:
