...
class Document:
    ...
    def replace_node(self, xpath: str = '', xml_node: str | etree._Element | DataFrame = None, root_name: str = None,
        ...
        if p is not None:
            ...
            if isinstance(node_to_add, DataFrame): node_to_add = etree.fromstring(node_to_add.to_xml(
                root_name=xpath.split('/')[-1] if root_name is None else root_name, row_name=row_name, index=False,
    ...
    def add_previous(self, xpath: 'str' = '', xml_node: 'str|etree._Element|DataFrame' = None, root_name: str = None,
        ...
        if p is not None:
            node_to_add = xml_node
            if isinstance(node_to_add, str): node_to_add = etree.fromstring(node_to_add)
            if isinstance(node_to_add,  DataFrame): node_to_add = node_to_add.to_xml(
                root_name=xpath.split('/')[-1] if root_name is None else root_name, row_name=row_name, index=False,
                xml_declaration=False)
    ...
    def publish(self, is_revision: bool = True) -> 'ServiceResponse':
        """
        Pubblica il documento sul servet tidy4.
