...
class Adapter:
    ...
    def login(self, user: str, password: str, workspace: str, name_application: str, type: str = 'NORMAL',
        ...
        sr=self.service.Login(input_type(user, as_user, password, workspace, name_application, type, 1, 1))
        if sr is not None and (sr.code=='0' or sr.code=='1'): self.default_id_session=sr.xml
        return sr,sr.xml

    def logout(self, id_session: str = None, background: bool = False) -> 'ServiceResponse':
        """
    ...
    def nameClasses(self, id_session: str = None, background: bool = False) -> 'str[] , ServiceResponse':
        ...
        list = []
        for n in Adapter.__to_element__(sr.xml):
            list.append(n.text)
        return list, sr
...
adapter.login('federico', 'Gen2024!', 'mathmarket', 'excel')
