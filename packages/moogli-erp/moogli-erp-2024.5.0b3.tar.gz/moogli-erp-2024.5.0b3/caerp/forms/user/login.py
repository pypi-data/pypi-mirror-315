...
def get_groups(request, account_type=None):
    ...
    query = query.order_by(Group.default_for_account_type.desc(), Group.label)
...
def get_group_label(request, group: Group) -> str:
    result = group.label
    tooltip = ""
    tooltip = ",".join([a.label for a in group.access_rights])
    icon = build_icon_str(request, "question-circle")
    result += f'&nbsp;<span class="icon" title="{tooltip}">{icon}</span>'
    return result
...
def get_group_widget(request, groups):
    ...
    return deform.widget.CheckboxChoiceWidget(
        values=[(group.name, get_group_label(request, group)) for group in groups],
    )
...
def set_widgets(schema, account_type: str = None):
    ...
    :param str account_type: The type of account (entrepreneur or equipe_appui)
    """
