        ...
        <div class='collapsible separate_top content_vertical_double_padding'>
            <h2 class="collapse_title">
                <a href="javascript:void(0);" 
                    ...
                    aria-label="Afficher le détail des droits du rôle ${group.label}">
                    % if not group.editable:
                        <small title="Ce rôle n’est pas modifiable">${api.icon('lock')}<span class="screen-reader-text">Ce rôle n’est pas modifiable</span></small>
                    % endif
                    ${group.label}
