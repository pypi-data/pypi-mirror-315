...
def _create_aliases_parameter_argument(aliases):
    aliases_param = _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":aliases", "")
    for alias in aliases:
        alias_param = _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":alias", "")

        alias_param.parameters.append(
            _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":alias:name", alias.name))
        alias_param.parameters.append(
            _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":alias:queryFormat", alias.queryFormat))
        if len(alias.arguments) > 0:
            alias_param.parameters.append(
                _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":alias:argumentCount", str(len(alias.arguments))))
        alias_param.parameters.append(
            _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":alias:xPathVersion", alias.xPathVersion))

        aliases_param.parameters.append(alias_param)
    return aliases_param
...
class ProtocolSocket(object):
    ...
    def quote(self, uri):
        params = [
            _create_argument_param(BuiltinProcedure.Args.COMMAND_NAME_ARGUMENT, BuiltinProcedure.Commands.QUOTE),
            _create_argument_param(BuiltinProcedure.Args.ARGUMENT, str(uri))
        ]
        return self.test_case_procedure(BuiltinProcedure.PYTHON_COMMAND, parameters=params)

    def query31_command(self, structured_data, xpath, aliases):
        params = [
            _create_argument_param(BuiltinProcedure.Args.COMMAND_NAME_ARGUMENT, BuiltinProcedure.Commands.QUERY),
            _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":xpath", xpath),
            _create_argument_param(BuiltinProcedure.Args.ARGUMENT + ":structuredData", structured_data),
            _create_aliases_parameter_argument(aliases)
        ]
        return self.test_case_procedure(BuiltinProcedure.PYTHON_COMMAND, parameters=params)
