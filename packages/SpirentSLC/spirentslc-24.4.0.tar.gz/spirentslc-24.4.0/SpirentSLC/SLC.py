...
def get_instance():
    """Return current instance of of SpirentSLC"""
...
class _SLC(object):
    ...
    def __init__(self, isLocal, host, port, agent_path, itar_path, license_server):
        ...
        if isLocal:
            ...
            executable = agent_path / 'velocity-agent.bat'
            if not executable.exists():
                executable = agent_path / 'velocity-agent'
            
            if not executable.exists():
                raise SLCError("Can't find agent executable at " + str(agent_path))

            self.agent_args = [str(executable)]
            ...
            self.agent_log_file.write('Starting agent with arguments:\n' + ' '.join(final_args) + '\n\n\n')
        ...
        self._sessions = dict()
        self._projects: dict[str, Project] = dict()
        self._sync_projects()
    ...
    def open(self, projectName) -> Project:
        """Opens a project.
    ...
    def quote(self, uri):
        message = self._protosock.quote(uri)
        return SessionActionResponse(message)

    def query31_command(self,structured_data, xpath, aliases):
        message = self._protosock.query31_command(structured_data, xpath, aliases)
        return SessionActionResponse(message)
