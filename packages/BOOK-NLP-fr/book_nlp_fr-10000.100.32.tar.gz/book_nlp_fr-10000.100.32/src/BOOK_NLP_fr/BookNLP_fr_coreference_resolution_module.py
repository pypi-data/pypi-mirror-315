...
from BOOK_NLP_fr import load_tokenizer_and_embedding_model, get_embedding_tensor_from_tokens_df
from BOOK_NLP_fr import load_tokens_df, load_entities_df
...
def get_tokens_embeddings_tensor_dict(tokens_embeddings_tensor_dict_path, model_name, files_directory,
    ...
    if not os.path.exists(tokens_embeddings_tensor_dict_path):
        ...
        for file_name in tqdm(tokens_files, desc="Generating tokens embeddings"):
            ...
            tokens_embedding_tensor = get_embedding_tensor_from_tokens_df(tokens_df, tokenizer, embedding_model,
...
def initialize_mention_pairs_df(entities_df,
    ...
    N = len(mentions_prop_list)  # Number of mentions

    # Initialize the max distance matrix with default distance for proper nouns
    max_distances_matrix = np.full((N, N), proper_common_nouns_antecedent_max_distance)

    # Update the matrix for low-information nouns
    for A in range(N):
        if ('NOM' in mentions_prop_list[A]) and (mentions_len_list[A] == 1):
            max_distances_matrix[A, :] = low_information_noun_max_distance
            max_distances_matrix[:, A] = low_information_noun_max_distance

    # Update the matrix for pronouns
    for A in range(N):
    ...
    # Compute the mention distance matrix
...
def add_mentions_infos(mention_pairs_df, entities_df,
                       features=['mention_len', 'start_token_ID_within_sentence']):
    """
    Add individual mention information to mention pairs.

    Args:
        mention_pairs_df (pd.DataFrame): DataFrame containing mention pair indices with columns "A" and "B".
        entities_df (pd.DataFrame): DataFrame containing information about individual mentions.
        features (list of str, optional): List of mention-level features to include (default: ['mention_len', 'start_token_ID_within_sentence']).

    Returns:
        pd.DataFrame: Enriched mention_pairs_df with columns corresponding to the selected features
        prefixed with 'A_' or 'B_' for each mention in the pair.
    """
    # Filter columns in entities_df based on requested features
    columns = ['mention_len', 'start_token_ID_within_sentence']
    columns = [column for column in columns if column in features]

    # Merge mention information for both mentions in the pair (A and B)
    for mention_polarity in ['A', 'B']:
        mention_pairs_df = pd.merge(mention_pairs_df,
                                    entities_df[columns].add_prefix(f"{mention_polarity}_"),
                                    left_on=mention_polarity,
                                    right_index=True  # Use the index of entities_df for matching
                                    )
    return mention_pairs_df


def get_mention_pairs_distance_features(mention_pairs_df, entities_df,
                                        features=['mention_ID_delta', 'start_token_delta', 'end_token_delta',
                                                  'paragraph_ID_delta', 'sentence_ID_delta',
                                                  'out_to_in_nested_level_delta']):
    """
    Calculate distance-based features for mention pairs.

    Args:
        mention_pairs_df (pd.DataFrame): DataFrame containing mention pair indices with columns "A" and "B".
        entities_df (pd.DataFrame): DataFrame containing mention-level information.
    ...
    if 'paragraph_ID_delta' in features:
        mention_pairs_df['paragraph_ID_delta'] = abs(
            mention_pairs_df['A_paragraph_ID'] - mention_pairs_df['B_paragraph_ID'])
    if 'sentence_ID_delta' in features:
        mention_pairs_df['sentence_ID_delta'] = abs(
            mention_pairs_df['A_sentence_ID'] - mention_pairs_df['B_sentence_ID'])
    if 'out_to_in_nested_level_delta' in features:
        mention_pairs_df['out_to_in_nested_level_delta'] = abs(
            (mention_pairs_df['A_out_to_in_nested_level'] - mention_pairs_df['B_out_to_in_nested_level']))

    # Drop intermediate columns to clean up the DataFrame
    intermediate_columns = [f"{mention_polarity}_{col}" for mention_polarity in ["A", "B"] for col in columns]
    mention_pairs_df.drop(intermediate_columns, axis=1, inplace=True)
...
def get_text_and_syntactic_match_features(mention_pairs_df, entities_df,
                                          features=['shared_token_ratio', 'text_match', 'head_text_match',
                                                    'syntactic_head_match', 'cat_match']):
    """
    ...
    # Define required columns for the features
    columns = ['text', 'head_word', 'head_syntactic_head_ID', 'cat']
    ...
    if 'text_match' in features:
        mention_pairs_df['text_match'] = (mention_pairs_df['A_text'].str.lower() == mention_pairs_df[
            'B_text'].str.lower()) * 1
    if 'head_text_match' in features:
        mention_pairs_df['head_text_match'] = (mention_pairs_df['A_head_word'].str.lower() == mention_pairs_df[
            'B_head_word'].str.lower()) * 1
    if 'syntactic_head_match' in features:
        mention_pairs_df['syntactic_head_match'] = (mention_pairs_df['A_head_syntactic_head_ID'] == mention_pairs_df[
            'B_head_syntactic_head_ID']) * 1
    if 'cat_match' in features:
        mention_pairs_df['cat_match'] = (mention_pairs_df['A_cat'] == mention_pairs_df['B_cat']) * 1
...
def get_one_hot_encoded_features(mention_pairs_df, entities_df,
                                 features=['prop', 'head_dependency_relation', 'gender', 'number',
                                           'grammatical_person']):
    """
    ...
    # Merge mention-level information into mention_pairs_df
    for mention_polarity in ["A", "B"]:
        mention_pairs_df = mention_pairs_df.merge(
    ...
    # Clean up: Drop original categorical columns
    mention_pairs_df.drop([f"{mention_polarity}_{feature}" for feature in features for mention_polarity in ["A", "B"]],
                          axis=1, inplace=True)
...
def convert_mention_pairs_df_to_features_array(mention_pairs_df):
    ...
    """
    # Identify and sort feature columns (excluding the identifiers "A" and "B")
    features_columns = sorted([column for column in mention_pairs_df.columns if column not in ["A", "B"]])
    # Extract and reorder the DataFrame to include only feature columns
    mention_pairs_df = mention_pairs_df[features_columns]

    # Convert the DataFrame to a NumPy array with specified data type
    features_array = mention_pairs_df.values.astype(np.float32)
    return features_array


def generate_mention_pairs_features_array(mention_pairs_df, CAT_entities_df,
    ...
    """
    # print(mention_pairs_df.columns)
    mention_pairs_df = add_mentions_infos(mention_pairs_df, CAT_entities_df, features=features)
    # print(mention_pairs_df.columns)
    mention_pairs_df = get_mention_pairs_distance_features(mention_pairs_df, CAT_entities_df, features=features)
    # print(mention_pairs_df.columns)
    mention_pairs_df = get_text_and_syntactic_match_features(mention_pairs_df, CAT_entities_df, features=features)
    # print(mention_pairs_df.columns)
    mention_pairs_df = get_one_hot_encoded_features(mention_pairs_df, CAT_entities_df, features=features)
    # print(mention_pairs_df.columns)
...
def get_mention_pairs_gold_labels(mention_pairs_df, entities_df):
    """
    Extracts gold labels for mention pairs based on their COREF values.

    Args:
        mention_pairs_df (pd.DataFrame): DataFrame containing mention pairs with their indices ('A' and 'B').
        entities_df (pd.DataFrame): DataFrame containing entity information, including the 'COREF' column.

    Returns:
        np.ndarray: A NumPy array of binary labels (1 if mentions are coreferent, 0 otherwise).
    """
    # Create a mapping from entity indices to their COREF group
    coref_dict = entities_df['COREF'].to_dict()

    # Map the COREF groups to mentions A and B in the mention pairs
    mention_pairs_df['A_COREF'] = mention_pairs_df['A'].map(coref_dict)
    mention_pairs_df['B_COREF'] = mention_pairs_df['B'].map(coref_dict)

    # Compute gold labels (1 if coreferent, 0 otherwise)
...
class MentionPairsDataset(Dataset):
    def __init__(self, generator_model_data):
        ...
        self.per_mentions_embeddings = torch.tensor(generator_model_data['overall_mentions_embeddings_tensor'],
                                                    dtype=torch.float32)
        self.features = torch.tensor(generator_model_data['overall_features_tensor'], dtype=torch.float32)
        self.labels = torch.tensor(generator_model_data['overall_labels_tensor'], dtype=torch.float32)
...
# Generate coreference metrics dfs
def initialize_gold_coreference_matrix_from_entities_df(entities_df):
    ...
    Parameters:
        entities_df (pd.DataFrame): DataFrame containing a 'COREF' column, where each value represents
                                    the coreference group ID of the corresponding mention.

    Returns:
        np.ndarray: A gold coreference matrix where:
    ...
    """
    # Extract the 'COREF' column as a NumPy array for efficient processing
    COREF_array = np.array(entities_df['COREF'])

    # Create a matrix where each element (i, j) is True if COREF[i] == COREF[j], otherwise False
    gold_coreference_matrix = np.equal.outer(COREF_array, COREF_array).astype(int)

    # Convert boolean values (True/False) into integers (1/-1) for consistency
    gold_coreference_matrix = np.where(gold_coreference_matrix, 1, -1)

    return gold_coreference_matrix


def generate_coreference_matrix_with_cache(entities_df, mention_pairs_df, verbose=0):
    """
    ...
    matrix = np.zeros((N, N), dtype=int)  # Initialize the coreference matrix with zeros
    ...
    for A, B, prediction in mention_pairs_list:
        if matrix[A, B] == 0:  # Process only unprocessed pairs
            ...
            AB_concat_ids_array = np.array(list(AB_concat_ids))  # Convert to NumPy array for indexing

            # Create submatrix for the A and B clusters
            A_coref_ids_list = np.array(list(A_coref_ids))
            B_coref_ids_list = np.array(list(B_coref_ids))
            ...
            if empty_matrix:
                if prediction == 1:
                    matrix[np.ix_(AB_concat_ids_array, AB_concat_ids_array)] = 1  # Propagate coreference
                ...
                if prediction == -1:
                    ...
                    print('Illegal Coreference', [A, B])  # Log error if prediction contradicts existing coreference
            elif coreference_pairs and not non_coreference_pairs:
                if prediction == 1:
                    matrix[np.ix_(AB_concat_ids_array, AB_concat_ids_array)] = 1  # Merge coreference clusters
                    for idx in AB_concat_ids:
                        coref_cache[idx] = AB_concat_ids  # Update cache
                elif verbose:
                    print('Illegal Breaking Coreference', [A, B])  # Log error if prediction breaks existing coreference
...
def get_coref_metrics_for_token_window(start_token_id=0,
    ...
    predicted_coreference_matrix = generate_coreference_matrix_with_cache(valid_CAT_entities_df,
                                                                          valid_processed_mention_pairs.copy(),
                                                                          verbose=0)
...
def evaluate_coreference_model(trained_coreference_model, coreference_resolution_training_dict, test_file,
    ...
    all_processed_mention_pairs = postprocess_mentions_pairs(predicted_mention_pairs,
    ...
    tokens_span_step = 1000
    window_ranges = [500] + [n * tokens_span_step for n in range(1, int(tokens_count / tokens_span_step) + 1)] + [
        tokens_count]
    ...
    for window_range in tqdm(window_ranges, desc="Evaluating Coreference Resolution for different windows",
        ...
        for window in tqdm(boundaries, leave=False):
...
## INTRODUCTION:
This model, developed as part of the [BookNLP-fr project](https://github.com/lattice-8094/fr-litbank), is a **coreference resolution model** built on top of [{foundation_model.split("/")[-1]}](https://huggingface.co/{foundation_model}) embeddings. It is trained to link mentions of the same entity across a text, focusing on literary works in French.
...
def load_coreference_resolution_model(
        model_path="AntoineBourgois/BookNLP-fr_coreference-resolution_camembert-large_PER", force_download=False):
    """
...
def generate_clusters(matrix):
    """
    Generate clusters from the predicted coreference matrix.
...
def perform_coreference(entities_df=None,
    ...
    mention_pairs_post_process = coreference_resolution_model["mention_pairs_post_process"]
    all_processed_mention_pairs = postprocess_mentions_pairs(predicted_mention_pairs,
                                                             CAT_entities_df=CAT_entities_df,
                                                             mention_pairs_post_process=mention_pairs_post_process)

    # Step 7: Generate the coreference matrix from processed mention pairs
    predicted_coreference_matrix = generate_coreference_matrix_with_cache(CAT_entities_df, all_processed_mention_pairs,
                                                                          verbose=0)
