...
class Node(NodeParent):
    # TODO: These prefixes should be read from schema-ikml.txt file
    ...
    def keys(self):
        ...
        for n in self.children:
            if n.tag_name and n.tag_name[0] == ".":
    ...
    def generate_id(self):
        ...
        if self.parent != None and self.parent.props.get("id", None):
            if "rel_id" in self.props:
                self.props["id"] = (
                    self.parent.props["id"]
                    + "."
                    + self.tag_prefix.get(self.tag_name, "")
                    + str(self.props["rel_id"])
                )
    ...
    # TODO: implement max_depth for to_txt
