...
class MongoCRUDBase(Generic[ModelType]):
    ...
    def first(self, m: MongoClient, *, query: Optional[QueryBase | dict] = None, project: Optional[dict] = None, **kwargs) -> \
            Optional[ModelType]:
        doc = self.first_doc(m, query=query, project=project, **kwargs)
        return self.model(**doc) if doc is not None else None

    def first_doc(self, m: MongoClient, *, query: Optional[QueryBase | dict] = None, project: Optional[dict] = None,
    ...
    def get_multi(
            self, m: MongoClient, *, query: Optional[QueryBase | dict] = None
    ) -> List[ModelType]:
        ...
        if query is not None and query.offset is not None:
            assert isinstance(query.offset, int), f"[Db] Expected query offset to be type int, but got {query.offset}"
            cursor = cursor.skip( query.offset)
        if query is not None and query.limit is not None:
            assert isinstance(query.offset, int), f"[Db] Expected query limit to be type int, but got {query.limit}"
    ...
    def delete_multi(
        ...
        if query is None:
            ...
            if isinstance(query, dict):
                query = QueryBase(**query)
            query_dict: dict = query.model_dump(exclude_none=True, exclude_unset=True, exclude_defaults=True)
        assert "filters" in query_dict and query_dict[
            "filters"] is not None, "[Db] You must set, and only set query.filters to delete_multi in mongodb"
