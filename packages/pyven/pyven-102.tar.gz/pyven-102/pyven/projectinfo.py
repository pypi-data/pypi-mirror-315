    ...
    def published(cls, reqstrs):
        ...
        for r in cls.parselines(reqstrs):
            try:
                ...
                with urlopen(Request(f"https://pypi.org/simple/{quote(r.namepart, safe = '')}/", method = 'HEAD')):
                    pass
                yield r
...
class ProjectInfo:
    ...
    def seek(cls, realdir):
        path = Seek.seek(realdir, cls.projectaridname)
        if path is None:
    ...
    def seekany(cls, realdir):
        try:
            ...
            pass
        setuppath = Seek.seek(realdir, 'setup.py')
        if setuppath is not None:
            log.info('Use setuptools mode.')
            return _setuptoolsinfo(setuppath)
        log.info('Use uninstallable mode.')
        projectdir = os.path.dirname(Seek.seek(realdir, '.git'))
        ...
        try:
            info.allrequires.extend((Path(projectdir) / 'requirements.txt').read_text().splitlines())
    ...
    def nextversion(self): # XXX: Deduce from tags instead?
        ...
        try:
            with urllib.request.urlopen(f"https://pypi.org/simple/{self.config.name}/") as f:
    ...
    def descriptionandurl(self):
        ...
        while True:
            try:
                with urllib.request.urlopen(f"https://api.github.com/repos/{urlpath}") as f:
    ...
    def mainmodules(self):
        ...
        with TemporaryDirectory() as tempdir:
            ...
            for line in subprocess.check_output([f"python{next(iter(self.config.pyversions))}", scriptpath, self.projectdir] + paths).splitlines():
    ...
    def devversion(self):
        ...
        return f"{(max(releases) if releases else 0) + 1}.dev0"
