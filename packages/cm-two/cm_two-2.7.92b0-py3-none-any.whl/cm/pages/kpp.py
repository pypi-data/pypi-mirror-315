...
class KPP(SuperPage):
    ...
    def set_arriving_status(
        ...
        if text.lower() == "ожидается проезд авто":
            ...
            if self.orupState and self.operator.current == "KPP":
                self.destroy_orup()
            if self.blockImgDrawn and self.operator.current == "KPP":
                self.destroyBlockImg()
    ...
    def double_click_record(self, event):
        ...
        if opened:
            ...
            if not self.notif_anim_in_progress:
                threading.Thread(
                    target=self.create_notification_animation).start()
