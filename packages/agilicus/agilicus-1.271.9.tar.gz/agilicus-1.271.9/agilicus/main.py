...
@click.option("--tag", type=str, required=True, help="the tag name of a message")
@click.option("--org-id", default=None)
@click.pass_context
...
def vnc_pw_valid(ctx, param, value):
    if value and len(value) > 256:
        raise click.BadParameter(
            "password length must be less than or equal to 256 chars."
        )
...
@click.pass_context
def update_point_of_presence(ctx, point_of_presence_id, **kwargs):
    result = regions.update_point_of_presence(ctx, pop_id=point_of_presence_id, **kwargs)
    output_entry(ctx, result.to_dict())
