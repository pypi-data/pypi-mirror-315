...
def url_to_filename(url: str, url_param_as_filename: str = "") -> str:
    ...
    Args:
        ...
        url_param_as_filename: Extract a URL parameter value as the filename.
            If None (default), uses the path.

    Returns:
        The generated filename. Returns "index.html" if no suitable filename is
        found.
    """
...
async def fetch_url_by_aiohttp(
    ...
    try:
        async with session.get(url) as resp:
            ...
            filename = Path(config.output_dir, filename)
