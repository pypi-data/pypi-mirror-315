...
class makeYAML:
    ...
    def extraColliders(self,mech_rxn,colliders,refCol,filter=False):
        ...
        if filter:
            extra_keys = {col['name'] for col in extras}
            filtered_colliders = [col for col in colliders if col['name'] not in extra_keys]
            return filtered_colliders, extras
        else:
    ...
    def zippedMech(self, data):
        ...
        for mech_rxn in data['mech']['reactions']:
            ...
            # rxn is specifically covered either in defaults or user input
                ...
                cols = self.extraColliders(mech_rxn,colliders,refCol)
                newRxn['colliders'] = [colliderM] + cols[0] + cols[1]
                ...
                cols = self.extraColliders(mech_rxn,colliders,refCol,filter=True)
                newRxn['colliders'] = [colliderM] + cols[0] + cols[1]
                newData['reactions'].append(newRxn)
