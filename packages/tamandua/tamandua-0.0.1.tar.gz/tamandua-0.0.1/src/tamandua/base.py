...
class Scraper:
    ...
    def __init__(
        ...
        with shelve.open(self.state_filepath) as db:
    ...
    def deep_process_url(self, url: str) -> Results:
        ...
        response = requests.get(
            url, timeout=30, headers={'User-Agent': self.user_agent}
        )
        ...
        if response.headers['Content-Type'] in self.store_content_types:
            ...
            # Analisa URLs na página.
            for link in re.findall(r'href="(.+?)"', response.text):
                # Resolve links relativos.
                # link = urljoin(url, link).replace('&amp;', '&')

                # Processa apenas páginas abaixo da raíz e que já não tenha sido processada.
                if link not in self.downloaded and self.process_filter(
                    link, current_url=url
                ):
                    sub_results = self.deep_process_url(link)

                    results.sublinks += 1 + sub_results.sublinks
                    results.stored += sub_results.stored
