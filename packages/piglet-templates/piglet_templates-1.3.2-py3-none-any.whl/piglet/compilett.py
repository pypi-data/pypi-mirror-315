...
class Statement(parse.ParseItem):
    ...
    end_tag_pos: Optional[Position] = None
    children: list["Statement"] = dataclasses.field(default_factory=list)
    ws_before: Optional[str] = None
    ws_after: Optional[str] = None
...
@dataclasses.dataclass
class Container(parse.ParseItem):
    children: list[parse.ParseItem] = dataclasses.field(default_factory=list)
...
def _compile_node(node):
    ...
    if isinstance(node, Container):
        root = im.ContainerNode(children=[], pos=0)
