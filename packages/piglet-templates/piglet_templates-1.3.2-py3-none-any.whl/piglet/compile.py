...
def compile_to_source(node, filename="<string>"):
    ...
    """
    return unparse(compile_to_ast(node, filename))
...
class Compiler:
    ...
    def compile_textnode(self, srcnode, body):
        expr = Expr(value=Yield(Constant(srcnode.content)))
        ...
        assign = Assign(
            ...
            value=Call(
                ...
                args=[
                    ...
                    Constant(srcnode.href),
                ],
                keywords=[],
            ),
        )
        assign = set_pos(assign, srcnode.pos)
    ...
    def compile_translationnode(self, srcnode, body):
        translated = Call(
            func=LoadName("_"),
            args=[Constant(srcnode.get_msgstr())],
            keywords=[],
        )
                        ...
                        Assign(
                            targets=[
                                Subscript(
                                    ...
                                    slice=Index(value=Constant(name)),
                                    ctx=Store(),
                                )
                            ],
                            value=Call(
                                func=Attribute(
                                    value=Constant(""), attr="join", ctx=Load()
                                ),
                                args=[LoadName(ACC)],
                                keywords=[],
                            ),
    ...
    def compile_call(self, srcnode, body):
        ...
        leftovers = im.ContainerNode(children=[], pos=0)
        for item in srcnode.children:
    ...
    def compile_filternode(self, srcnode, body):
        ...
        joined = Call(
            func=Attribute(value=Constant(""), attr="join"),
            args=[LoadName(ACC)],
            keywords=[],
        )
        body.append(
    ...
    def collect_output(self, body):
        ...
        body.append(
            Assign(targets=[StoreName(acc)], value=List(elts=[], ctx=Load()), lineno=0)
        )
        body.append(
            Assign(
    ...
    for f, names in func_names.items():
        ...
        for n in sorted(names):
            ...
            value = Call(
                ...
                args=[Constant(n), default],
                keywords=[],
            )

            a = Assign(targets=[Name(id=n, ctx=Store())], value=value, lineno=0)
...
def _ensure_all_functions_yield(module):
    ...
    for f in functions:
        if not functions[f]:
            f.body.append(Expr(Yield(Constant(""))))
...
def _interpolated_str_to_ast_value(source):
    ...
    return Call(
        func=Attribute(Constant(""), "join", Load()),
        args=[items],
        keywords=[],
    )
