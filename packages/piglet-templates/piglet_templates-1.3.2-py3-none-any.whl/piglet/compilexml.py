...
class ElsePlaceholder(im.ContainerNode):
    """
...
class Directive:
    ...
    default: Union[str, object, None] = REQUIRED
    ...
    extra_attrs: list[tuple[str, bool]] = dataclasses.field(default_factory=list)
    ...
    def qattr(self):
        return f"{self.ns}:{self.attr}"

    def qualified_names_and_aliases(self):
        """
        Return a set of qualified tag name and aliases for this directive
        """
        return {f"{self.ns}:{n}" for n in self.names_and_aliases()}

    def names_and_aliases(self):
        """
        Return a set of tag name and aliases for this directive
        """
        return chain([self.name] if self.name else [], self.aliases)
    ...
    def parse(self, el):
        ...
        An element matches if:

        - Its tag name matches the directive's name or an alias
          (eg ``<py:if test="" ...>``)

        - It contains an attribute whose name matches the directive's name or
          an alias (eg ``<div py:if="foo" ...>``)
        """
        matched = False
        data = {}
        pos = el.pos

        qattr = self.q(self.attr)
        qtagnames = set(self.qualified_names_and_aliases())

        # Element name matches the directive or one if its aliases
        #  (eg <py:if test="…">)
        if el.qname in qtagnames:
            matched = True
            remaining = Fragment(
            ...
            matched_attr = None
            if qtagnames:
                # Element name/alias matches an attribute (eg <div py:if="…">)
                matched_attr = next((a for a in qtagnames if a in el.attrs), None)

            elif qattr in el.attrs:
                # Name is empty, but attr still matches. This is the case where it
                # doesn't make sense for the directive to match an entire element  (eg
                # py:strip, py:attrs)
                matched_attr = qattr

            if matched_attr:
                matched = True
                pos = el.attrs[matched_attr].value_pos
                remaining = copy(el)
                remaining.attrs = copy(el.attrs)
                if self.empty:
                    remaining.attrs.pop(matched_attr)
                else:
                    data = {self.attr: remaining.attrs.pop(matched_attr).value}
...
directives = [
    Directive(name="comment", attr="comment", empty=True, factory=im.Comment),
    Directive(name="with", attr="vars", factory=im.WithNode),
    Directive(name="def", attr="function", factory=im.DefNode),
    Directive(
        name="extends",
        attr="href",
        extra_attrs=[("ignore-missing", False)],
        factory=im.ExtendsNode,
    ),
    Directive(name="include", attr="href", factory=im.IncludeNode),
    Directive(name="block", attr="name", inner=True, factory=im.BlockNode),
    Directive(
        name="import",
        attr="href",
        extra_attrs=[("alias", True), ("ignore-missing", False)],
        factory=im.ImportNode,
    ),
    Directive(name="when", attr="test", factory=im.WhenNode),
    Directive(
        name="case",
        attr="value",
        factory=lambda value: im.WhenNode(test=value),
    ),
    Directive(
        name="otherwise",
        attr="otherwise",
        empty=True,
        factory=im.OtherwiseNode,
    ),
    Directive(name="else", attr="else", empty=True, factory=ElsePlaceholder),
    Directive(name="for", attr="each", factory=im.ForNode),
    Directive(name="if", attr="test", factory=im.IfNode),
    Directive(
        name="switch",
        attr="test",
        default="True",
        factory=im.ChooseNode,
    ),
    Directive(
        name="choose",
        attr="test",
    ...
    Directive(name="call", attr="function", inner=True, factory=im.Call),
    Directive(name="keyword", attr="name", factory=im.CallKeyword),
    Directive(
        name="content",
        attr="value",
        inner=True,
        factory=im.InterpolateNode,
    ),
    Directive(name="replace", attr="value", factory=im.InterpolateNode),
    Directive(name="filter", attr="function", inner=True, factory=im.FilterNode),
    PyAttrsDirective(name=None, attr="attrs", factory=None),
    PyStripDirective(name=None, attr="strip", factory=None),
...
def _compile_node(node):
    ...
    if isinstance(node, Fragment):
        ...
        if node.target == "python":
            ...
            return im.TextNode(content=node.source, pos=node.pos)
        ...
        c = "<!--" + node.content + "-->"
        n = im.TextNode(content=c, pos=node.pos)
        ...
        return im.TextNode(content=node.source, pos=node.pos, autoescape=False)
...
def _compile_text(node):
    ...
    for i in items:
        if isinstance(i, str):
            container.append(im.TextNode(pos=pos, content=i))
            pos = pos.advance(i)
...
def _compile_open_tag(node, tagname_expr=None, extra_attrs=None, strip_condition=None):
    ...
    wn.append(im.TextNode(content="<", pos=node.pos))
    if tagname_expr:
        wn.append(im.InterpolateNode(value=tagname_expr, pos=node.end_pos))
    else:
        wn.append(im.TextNode(content=node.qname, pos=node.pos))
    wn.append(im.TextNode(content=node.space, pos=node.pos))
    if extra_attrs:
    ...
    if extra_attrs:
        ...
        if_ = im.IfNode(
            ...
            pos=node.end_pos,
            children=[
                im.TextNode(content=" ", pos=node.end_pos),
...
def _compile_attr(a, container):
    ...
    """
    intro_text = im.TextNode(
        content=f"{a.name}{a.space1}={a.space2}{a.quote}", pos=a.pos
    )
    outro_text = im.TextNode(
        content=f"{a.quote}{a.space3}", pos=a.value_pos.advance(a.value)
    )
    ...
    if (
        ...
        for i in items:
            if isinstance(i, str):
                container.append(im.TextNode(content=i, pos=pos))
                pos = pos.advance(i)
            else:
...
def _compile_close_tag(node, tagname_expr=None, strip_condition=None):
    if not node.close_tag or (isinstance(node, Fragment) and not tagname_expr):
        return im.TextNode(content="", pos=node.close_tag_pos)

    if tagname_expr:
        closetag = im.ContainerNode(
            children=[
                im.TextNode(content="</", pos=node.close_tag_pos),
                im.InterpolateNode(value=tagname_expr, pos=node.close_tag_pos),
                im.TextNode(content=">", pos=node.close_tag_pos),
            ]
        )
    else:
        closetag = im.TextNode(content=node.close_tag, pos=node.close_tag_pos)
    if strip_condition:
        if strip_condition in {"True", "1"}:
