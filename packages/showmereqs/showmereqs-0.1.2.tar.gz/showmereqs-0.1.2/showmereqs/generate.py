...
def generate_reqs(package_infos: list[PackageInfo], **kwargs):
    detail = kwargs["no_detail"]
    ...
    # sorted_row_txts = full_info, no_version, no_package_name, no_info
    sorted_row_txts = sort_row_txts(row_txts)
    header_txts = [
        "package with version",
        "package without version",
        "package without package name",
        "package without version and package name",
    ]
    prefix_txts = ["", "", "# ", "# "]
    with open(Path(kwargs["outdir"]) / "requirements.txt", "w") as f:
        f.write("# generated by showmereqs\n")
        f.write(f"# python version: {sys.version}\n\n")

        for i, row_txts in enumerate(sorted_row_txts):
