...
from nanorpc.utils.nanolib_helpers import *
from enum import Enum


class NanoWallet:
    def __init__(self, url, username=None, password=None):
        self.rpc = NanoRpcTyped(url=url, username=username, password=password)
    ...
    async def network_difficulty(self):
        if not self.difficulty:
            result = await self.rpc.active_difficulty()
            self.difficulty = result.get("network_current", "fffffff800000000")
    ...
    async def change(self, private_key, representative, process=True):
        ...
        block = nl_create_block_from_key(
            private_key, frontier, current_balance, link, work, representative, await self.network_difficulty())
    ...
    async def send(self, private_key, amount_to_send, destination):
        ...
        block = nl_create_block_from_key(
            private_key, frontier, final_balance, destination, work, representative, await self.network_difficulty())
        response = await self.process(block)
        return response
    ...
    async def receive_block(self, private_key, incoming_amount, send_hash):
        ...
        block = nl_create_block_from_key(
            private_key, frontier, final_balance, send_hash, work, representative, await self.network_difficulty())
        response = await self.process(block)
        return response
    ...
    async def receive_blocks_many(self, private_key, threshold=0.000001, threshold_type=AmountType.NANO):
        result = []
        account = nl_get_account_from_key(private_key)
        threshold_raw = nl_nano_to_raw(
            threshold) if threshold_type == "nano" else threshold

        blocks = await self.receivable(account, threshold=threshold_raw, source=True)
    ...
    async def account_info(self, account, representative=None, weight=None, receivable=None, pending=None, include_confirmed=None):
        result: dict
        result = await self.rpc.account_info(account, representative=representative, weight=weight, receivable=receivable, pending=pending, include_confirmed=include_confirmed)
        result.setdefault("work_hash", result.get(
            "frontier", nl_get_account_public_key(account)))
