...
class Auth:
    ...
    def __create_user_context_graph(self, exit=False):
        url = "https://graph.microsoft.com/v1.0/users/" + self.upn + "?$select=id,userPrincipalName,onPremisesSamAccountName,displayName,lastPasswordChangeDateTime&$expand=transitiveMemberOf"
        ...
        if response.status_code == 200:
            ...
            self.graph_context = Format.CYAN + " Name: " + Format.END + response_parse["displayName"] + "\n"
            self.graph_context += Format.CYAN + " UserId: " + Format.END + response_parse["id"] + "\n"
            self.graph_context += Format.CYAN + " UserPrincipalName: " + Format.END + str(response_parse["userPrincipalName"]) + "\n"
            if "onPremisesSamAccountName" in response_parse:
                self.graph_context += Format.CYAN + " OnPremisesSamAccountName: " + Format.END + str(response_parse["onPremisesSamAccountName"]) + "\n"
            if "lastPasswordChangeDateTime" in response_parse:
                self.graph_context += Format.CYAN + " LastPasswordChange: " + Format.END + str(response_parse["lastPasswordChangeDateTime"]) + "\n"
            if self.msgraphdata is not None:
                self.graph_context += Format.CYAN + " ObjectId (MSGraph): " + Format.END + str(self.msgraphdata["id"]) + "\n"
            groups_count = 0
            roles_count = 0
            assigned_roles = ""
            for transitivememberof in response_parse["transitiveMemberOf"]:
                if transitivememberof["@odata.type"] == "#microsoft.graph.group":
                    ...
                    if transitivememberof["roleTemplateId"] in Permissions.INTERESTING_DIRECTORY_ROLE:
                        assigned_roles += "  " + Permissions.INTERESTING_DIRECTORY_ROLE[transitivememberof["roleTemplateId"]] + "\n"
                    else:
                        assigned_roles += "  " + transitivememberof["displayName"] + "\n"
            self.graph_context += Format.CYAN + " Roles: " + Format.END + str(roles_count) + "\n"
            if assigned_roles:
                self.graph_context += assigned_roles
            self.graph_context += Format.CYAN + " Groups: " + Format.END + str(groups_count) + "\n"
    ...
    def __create_context_arm(self, exit=False):
        ...
        if response.status_code == 200:
            ...
            self.arm_context = Format.CYAN + " Subscription: " + Format.END + str(subscription_count)
        elif response.status_code == 401:
            print(Format.BOLD_START + Format.GREEN + "\n[*] Access token expired, requesting a new one..." + Format.END)
    ...
    def __create_client_context_graph(self, exit=False):
        url = "https://graph.microsoft.com/v1.0/servicePrincipals(appId='{" + self.clientid + "}')?$expand=appRoleAssignments"
        ...
        if response_serviceprincipal.status_code == 200:
            ...
            self.graph_context = Format.CYAN + " Name: " + Format.END + response_serviceprincipal_parse["appDisplayName"] + "\n"
            self.graph_context += Format.CYAN + " ObjectId (ServicePrincipal): " + Format.END + response_serviceprincipal_parse["id"] + "\n"
            if self.msgraphdata is not None:
                self.graph_context += Format.CYAN + " ObjectId (MSGraph): " + Format.END + str(self.msgraphdata["id"]) + "\n"
            self.graph_context += Format.CYAN + " Permissions: " + Format.END + "\n"
            url = "https://graph.microsoft.com/v1.0/oauth2PermissionGrants/?$filter=clientId eq '" + response_serviceprincipal_parse["id"] + "'"
                    ...
                    for approle in response_serviceprincipal_parse["appRoleAssignments"]:
                        if approle["appRoleId"] in msgraph_roles:
                            if approle["appRoleId"] in Permissions.INTERESTING_APP_PERMISSIONS:
                                self.graph_context += Format.PURPLE + "  [Application] " + Format.END + Permissions.INTERESTING_APP_PERMISSIONS[approle["appRoleId"]] + "\n"
                            else:
                                self.graph_context += Format.PURPLE + "  [Application] " + Format.END + msgraph_roles[approle["appRoleId"]] + "\n"
    ...
    def __create_accesstoken_context(self):
        ...
        if "upn" in jwt_payload.keys() :
            ...
            print(Format.CYAN + " AppName: " + Format.END + str(jwt_payload["app_displayname"] if "app_displayname" in jwt_payload.keys() else None))
        elif "app_displayname" in jwt_payload.keys():
            print('\n' + Format.BOLD_START + Format.YELLOW + jwt_payload["app_displayname"] + Format.END )
        print(Format.CYAN + " AppId: " + Format.END + str(jwt_payload["appid"] if "appid" in jwt_payload.keys() else None))
        print(Format.CYAN + " Resource (aud): " + Format.END + jwt_payload["aud"])
        print(Format.CYAN + " Expiration Date: " + Format.END + str(datetime.datetime.fromtimestamp(jwt_payload["exp"]).strftime('%A, %d %B %Y %H:%M:%S')))
        if "roles" in jwt_payload:
            if jwt_payload["roles"] is not None:
                print(Format.CYAN + " Permissions: " + Format.END + ", ".join(jwt_payload["roles"]) if "roles" in jwt_payload else None)
        if self.msgraphdata is not None:
            print(Format.CYAN + " ObjectId (MSGraph): " + Format.END + str(self.msgraphdata["id"]))
