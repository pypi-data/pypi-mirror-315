...
parser_auth.add_argument('-c', '--client-id', dest='clientid', action='store', help='client ID (Default for user authentication: Microsoft PowerShell client ID)\n', required=False)
parser_auth.add_argument('-a', '--access-token', dest='accesstoken', action='store', help='graph access token\n', required=False)
parser_auth.add_argument('-r', '--refresh-token', dest='refreshtoken', action='store', help='refresh token\n', required=False)
parser_auth.add_argument('--user-agent', dest='useragent', action='store', default='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36', help='user-agent header (Default: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36")\n', required=False)
...
########### GET PARSE ###########
#################################
parser_get = subparsers.add_parser('get', help='[context, tokens, privesc, users, groups, devices, roles, applications, service-principals, policies, domains, mfa, updatable-groups, files, sites, messages, chats, subscriptions, vms, vm-perms, rbac, vaults, vault-secrets, vault-keys]\n')
...
parser_get_privesc.add_argument('--onlygroups', action='store_true', help='get only the updatable groups\n', required=False)
...
parser_get_updatable_group = subparsers_get.add_parser('updatable-groups', help='print the groups that can be updated by the current user\n')
parser_get_updatable_group.add_argument('--search', type=str, help='group name or search term\n', required=False)
parser_get_updatable_group.add_argument('--all', action='store_true', help='dump all group data to a json\n', required=False)
parser_get_updatable_group.add_argument('--getupdatablegroups', type=bool, default=True, help=argparse.SUPPRESS, required=False)
...
parser_get_serviceprincipal = subparsers_get.add_parser('service-principals', help='print service principals\n')
...
parser_get_vm_perms = subparsers_get.add_parser('vm-perms', help='print virtual machine permissions\n')
...
parser_get_vaultsecrets = subparsers_get.add_parser('vault-secrets', help='print the secrets associated with a vault\n')
...
parser_get_vaultkeys = subparsers_get.add_parser('vault-keys', help='print the keys associated with a vault\n')
...
########### ADD PARSE ###########
#################################
parser_add = subparsers.add_parser('add', help='[app-secret, service-principal-secret, application, service-principal, role-member, group-member, app-role-assigment, user, guest, group, new-password, message, chat-message, identity, mfa-phone, exclude-user-policy, vm-command]\n')
subparsers_add = parser_add.add_subparsers(dest='option',required=True)

parser_add_appsecret = subparsers_add.add_parser('app-secret', help='add a password to an existing application\n')
...
parser_add_serviceprincipalsecret = subparsers_add.add_parser('service-principal-secret', help='add a password to an existing service service\n')
...
parser_add_serviceprincipal = subparsers_add.add_parser('service-principal', help='creates a new service principal associated with an application\n')
...
parser_add_rolemember = subparsers_add.add_parser('role-member', help='assigns a user to a role\n')
...
parser_add_groupmember = subparsers_add.add_parser('group-member', help='assigns a user to a group\n')
...
parser_add_approleassigment = subparsers_add.add_parser('app-role-assigment', help='add and grant permissions to an application\n')
...
parser_add_newpassword = subparsers_add.add_parser('new-password', help='resets a user\'s password to a new one\n')
...
parser_add_chatmessage = subparsers_add.add_parser('chat-message', help='send a new chat message\n')
...
parser_add_mfa_phone = subparsers_add.add_parser('mfa-phone', help='add phone authentication method to a user\n')
...
parser_add_excludeuserpolicy = subparsers_add.add_parser('exclude-user-policy', help='exclude a user from a policy \n')
...
parser_add_vmCommand = subparsers_add.add_parser('vm-command', help='executes commands on virtual machine\n')
...
########### DEL PARSE ###########
#################################
parser_del = subparsers.add_parser('del', help='[user, group, application, service-principal, app-secret, service-principal-secret, message, chat-message, role-member, group-member, mfa-phone, app-role-assigment, identity, exclude-user-policy]\n')
...
parser_del_serviceprincipal = subparsers_del.add_parser('service-principal', help='remove a service principal associated with an application\n')
...
parser_del_appsecret = subparsers_del.add_parser('app-secret', help='remove a password from an existing app\n')
...
parser_del_serviceprincipalsecret = subparsers_del.add_parser('service-principal-secret', help='remove a password from an existing service principal\n')
...
parser_del_chatmessage = subparsers_del.add_parser('chat-message', help='delete a chat message\n')
...
parser_del_rolemember = subparsers_del.add_parser('role-member', help='remove a user from a role\n')
...
parser_del_groupmember = subparsers_del.add_parser('group-member', help='remove a user from a group\n')
...
parser_del_approleassigment = subparsers_del.add_parser('app-role-assigment', help='remove permissions from an application\n')
...
parser_del_mfa_phone = subparsers_del.add_parser('mfa-phone', help='remove phone authentication method from a user\n')
...
parser_del_excludeuserpolicy = subparsers_del.add_parser('exclude-user-policy', help='removes the excluded user from a policy \n')
    ...
    def do_get(self,opts):
        ...
        if "getprivesc" in opts:
            ...
            if opts.onlygroups:
                ...
                groups.get_groups(updatable=True, privesc=True)
            ...
            groups.get_groups()
        elif "getupdatablegroups" in opts:
            groups = Groups(self.auth, self.request, opts.search, opts.all)
            groups.get_groups(updatable=True) 
