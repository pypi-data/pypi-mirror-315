...
class BaseParameter(BaseModel, Generic[T]):
    ...
    def get_type(cls) -> str:
        ...
        _type = str(cls.__annotations__['type'])
        _match = re.match(r".*Literal\['([^']+)'.*", _type)
        if _match is None:
            raise ValueError(f'Invalid type annotation: {_type}')
...
class ProjectManager(BaseModel):
    ...
    def parse_project_dir(cls, project_dir: Path | str) -> ProjectManager:
        ...
        return cls.parse_yaml_file(project_path / Constants.PROJECT_FILE_NAME)
    ...
    def parse_yaml_file(cls, file_path: Path | str) -> ProjectManager:
        ...
        return cls(**project_config, file_path=file_path)
...
class SnapshotPackager(BaseModel):
    ...
    parameters: Dict[str, Optional[Union[str, int, float, bool]]]
    ...
    def package(self) -> bytes:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            # We still need to include the _internal.py file
            zf.writestr('_internal.py', f'_user_params = {self.parameters!r}')
            self.included_files.append('_internal.py')
    ...
    def _can_include(self, project_dir: Path, file_path: Path) -> bool:
        ...
        if len(rel_path.parts) == 1:
            ...
            if str(rel_path) == 'requirements.txt':
                return True
            if str(rel_path) == Constants.PROJECT_FILE_NAME:
                return True
