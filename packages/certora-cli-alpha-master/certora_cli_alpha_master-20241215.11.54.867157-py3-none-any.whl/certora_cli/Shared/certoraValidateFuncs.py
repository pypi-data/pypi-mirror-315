...
def validate_solc_evm_version_map(args: Dict[str, str]) -> None:
    ...
    if len(value_set) == 1:
        validation_logger.warning(f"All EVM versions --solc_evm_version_map are the same."
                                  f" --solc_evm_version {value_set.pop()} can be used instead")
...
def validate_solc_optimize_map(args: Dict[str, str]) -> None:
    ...
    if len(value_set) == 1:
        validation_logger.warning(f"All contracts are optimized for the same number of runs in --solc_optimize_map."
                                  f" --solc_optimize {value_set.pop()} can be used instead")
