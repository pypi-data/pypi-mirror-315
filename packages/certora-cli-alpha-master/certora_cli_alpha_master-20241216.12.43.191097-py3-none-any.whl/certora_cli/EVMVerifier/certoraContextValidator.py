...
def _normalize_maps(context: CertoraContext, map_attr_name: str, map_attr: Dict) -> None:
    ...
    for (key, value) in map_attr.copy().items():
        ...
        # normalized form
        if not attr_value:  # key in file paths but no mapping yet
...
def check_map_attributes(context: CertoraContext) -> None:
    map_attr_prefixes = (
        Attrs.EvmAttributes.SOLC.get_conf_key(),
        Attrs.EvmAttributes.SOLC_OPTIMIZE.get_conf_key(),
        Attrs.EvmAttributes.SOLC_EVM_VERSION.get_conf_key()
    )
    for attr_prefix in map_attr_prefixes:
        map_attr_name = f"{attr_prefix}_map"
        map_attr = getattr(context, map_attr_name, None)
        if not map_attr:
            continue
        base_attr = getattr(context, attr_prefix, None)
        if base_attr is not None and map_attr is not None:
