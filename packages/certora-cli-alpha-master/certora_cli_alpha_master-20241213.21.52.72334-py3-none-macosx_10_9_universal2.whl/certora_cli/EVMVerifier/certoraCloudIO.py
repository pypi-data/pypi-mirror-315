...
class CloudVerification:
    ...
    def print_output_links(self) -> None:
        prefix = "Manage your jobs at"
        if Ctx.is_supported_server(self.context):
            if Ctx.is_staging(self.context):
                print(f"{prefix} {Util.SupportedServers.STAGING.value}")
            else:
                print(f"{prefix} {Util.SupportedServers.PRODUCTION.value}")
        else:
            print(f"{prefix} {Util.SupportedServers.PRODUCTION.value}")
