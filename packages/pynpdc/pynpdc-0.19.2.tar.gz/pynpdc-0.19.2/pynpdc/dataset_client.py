...
from .utils import guard_dir, guard_path
from .models import (
    Attachment,
...
class DatasetClient:
    ...
    def get_datasets(
        self, chunk_size: int = DEFAULT_CHUNK_SIZE, **query: Unpack[DatasetQuery]
    ) -> DatasetCollection:
        """
    ...
    def update_dataset(self, dataset_id: uuid.UUID, content: Content) -> Dataset:
        """
    ...
    def add_attachments(
        ...
        return [AttachmentCreationInfo(item) for item in response.json()]
        ...
        **query: Unpack[AttachmentZIPQuery],
        ...
        Args:
            ...
            query (dict):
                ...
                See
                https://beta.data.npolar.no/-/docs/dataset/#/attachment/get_dataset__datasetID__attachment__blob
                for details
    ...
    def add_record(
        ...
        Raises:
            APIException: If HTTP status code > 207. Mostly this will be an
                authentification or authorisation issue.
        """
        return self.add_records(dataset_id, [record])
