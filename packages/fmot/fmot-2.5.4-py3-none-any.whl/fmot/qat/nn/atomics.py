...
class GMACv2(AtomicModule):
    def __init__(
        ...
        if scalar_multipliers is not None:
            ...
            logger.debug(
                f"GMACv2: scalar_multipliers={self.scalar_multipliers}\norig={self.scalar_multipliers_orig}"
            )
            quant_snr = 10 * torch.log10(
                self.scalar_multipliers_orig.pow(2).sum()
                / (self.scalar_multipliers_orig - self.scalar_multipliers).pow(2).sum()
            )
            logger.debug(f"GMACv2 scalar multiplier qSNR: {quant_snr}")
        else:
...
class TemporalConv2d(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.empty(d_band_out, d_band_in, kernel_band, kernel_t)
        )
