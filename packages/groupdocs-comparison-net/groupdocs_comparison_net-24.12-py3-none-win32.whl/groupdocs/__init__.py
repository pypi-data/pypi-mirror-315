...
class GroupDocsModuleLoader:
    groupdocs_native_lib_dir = os.path.dirname(__file__)
    groupdocs_py_modules = os.path.join(groupdocs_native_lib_dir, "lib")
    ...
    def _load_native_module(self, native_name):
        for ext in _platform_suffixes:
            lib_file = os.path.join(
                self.groupdocs_native_lib_dir, native_name + ext)
    ...
    def _register_additional_classes(self, module, sub_lib):
        folder = os.path.join(GroupDocsModuleLoader.groupdocs_py_modules, sub_lib)
        if os.path.isdir(folder):
            foreach_file(folder, im.SOURCE_SUFFIXES, lambda name, path,
                         module=module: GroupDocsModuleLoader._import_file(module, name, path))

        return


class GroupDocsModuleFinder:
    def find_module(self, fullname, path=None):
        if fullname.startswith(__package__):
            return GroupDocsModuleLoader()
        return None


__path__ = []

os.environ["NETCORE_WRAPPER_NETCORE_DIR_groupdocs"] = os.path.join(
    GroupDocsModuleLoader.groupdocs_native_lib_dir, "netcore")
os.environ["NETCORE_WRAPPER_ASSEMBLY_DIR_groupdocs"] = os.path.join(
    GroupDocsModuleLoader.groupdocs_native_lib_dir, "assemblies")


os.environ["WRAPPER_MODULE_DIR_groupdocs"] = GroupDocsModuleLoader.groupdocs_native_lib_dir
...
# to support 'from GroupDocs import *' syntax
__all__ = []
foreach_file(GroupDocsModuleLoader.groupdocs_native_lib_dir,
             _platform_suffixes, lambda name, path: __all__.append(name))

sys.meta_path.append(GroupDocsModuleFinder())
