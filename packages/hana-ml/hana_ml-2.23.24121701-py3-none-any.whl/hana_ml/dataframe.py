...
class ConnectionContext(object):
    ...
    def to_sqlalchemy(self, **kwargs):
        ...
        if self.userkey is None:
            ...
            user = conn_config[2]
            password = conn_config[3]
            return create_engine('hana://{}:{}@{}:{}/'.format(user, password, url, port), **kwargs)
        return create_engine('hana://userkey={}'.format(self.userkey), **kwargs)
    ...
    def __getattr__(self, col):
        if isinstance(col, str):
            if col in self.columns:
                col_df = Column(self.connection_context, self.select(col).select_statement)
                col_df._set_name(col)
                col_df._set_base_select_statement(self.select_statement)
                return col_df
        raise TypeError(
            '__getattr__ argument not understood: {!r}'.format(col))
...
def _get_dtype(dtype, allow_bigint, object_type_as_bin, primary_key=False, is_spark=False):
    if is_spark:
        if ('long' in dtype.lower()) and allow_bigint:
            return 'BIGINT'
        if 'int' in dtype.lower() or 'bool' in dtype.lower():
            return 'INT'
        if 'float' in dtype.lower() or 'double' in dtype.lower():
            return 'DOUBLE'
        if 'timestamp' in dtype.lower():
            return 'TIMESTAMP'
        if 'byte' in dtype.lower() and object_type_as_bin:
            return 'BLOB'
        if primary_key and 'str' in dtype.lower():
            return 'NVARCHAR(255)'
        return 'VARCHAR(5000)'
    ...
    if 'datetime' in dtype.lower():
        return 'TIMESTAMP'
    if 'object' in dtype.lower() and object_type_as_bin:
        return 'BLOB'
    if primary_key and 'str' in dtype.lower():
        return 'NVARCHAR(255)'
    return 'VARCHAR(5000)'
    ...
    if table_structure:#if table_structure is provided and non-empty
        for col in pandas_df.columns:
            if col in table_structure:
                if 'real_vector' in table_structure[col].lower():
                    real_vector_cols.append(col)
        pandas_df = _memoryview2str(pandas_df, real_vector_cols)
