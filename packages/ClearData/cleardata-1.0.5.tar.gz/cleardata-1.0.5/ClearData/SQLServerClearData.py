			...
			match parameter.SqlDbType:
				case SqlDbType.Bit:
					if parameter.Direction == ParameterDirection.Input:
						_ParametersHelpers.AddBitInput(sqlCommand, parameter.Name, parameter.Value)
					elif parameter.Direction == ParameterDirection.Output:
						_ParametersHelpers.AddBitOutput(sqlCommand, parameter.Name)
				case SqlDbType.Int:
					if parameter.Direction == ParameterDirection.Input:
						_ParametersHelpers.AddIntInput(sqlCommand, parameter.Name, parameter.Value)
					elif parameter.Direction == ParameterDirection.Output:
						_ParametersHelpers.AddIntOutput(sqlCommand, parameter.Name)
				case SqlDbType.BigInt:
					if parameter.Direction == ParameterDirection.Input:
						_ParametersHelpers.AddBigIntInput(sqlCommand, parameter.Name, parameter.Value)
					elif parameter.Direction == ParameterDirection.Output:
						_ParametersHelpers.AddBigIntOutput(sqlCommand, parameter.Name)
				case SqlDbType.NVarChar:
					if parameter.Direction == ParameterDirection.Input:
						_ParametersHelpers.AddNVarCharInput(sqlCommand, parameter.Name, parameter.Size, parameter.Value)
					elif parameter.Direction == ParameterDirection.Output:
						_ParametersHelpers.AddNVarCharOutput(sqlCommand, parameter.Name, parameter.Size)
				case SqlDbType.VarChar:
					if parameter.Direction == ParameterDirection.Input:
						_ParametersHelpers.AddVarCharInput(sqlCommand, parameter.Name, parameter.Size, parameter.Value)
					elif parameter.Direction == ParameterDirection.Output:
						_ParametersHelpers.AddVarCharOutput(sqlCommand, parameter.Name, parameter.Size)
				case SqlDbType.DateTime2:
					if parameter.Direction == ParameterDirection.Input:
						_ParametersHelpers.AddDateTime2Input(sqlCommand, parameter.Name, parameter.Size, parameter.Value)
					elif parameter.Direction == ParameterDirection.Output:
						_ParametersHelpers.AddDateTime2Output(sqlCommand, parameter.Name, parameter.Size)
				case _:
...
class SQLServerStoredProcedures(IStoredProcedures):
	...
	def Execute(self, schema:str, name:str, parameters:list[any] = None):
		...
		try:
			...
			if (parameters is not None):
				_ParametersHelpers.AddParameters(sqlCommand, parameters)
	...
	def ExecuteJSONNamed(self,
		...
		try:
			...
			if (inputParameterName is not None):
				_ParametersHelpers.AddNVarCharInput(sqlCommand, inputParameterName, -1, inputValue)
			if (outputParameterName is not None):
				_ParametersHelpers.AddNVarCharOutput(sqlCommand, outputParameterName, -1)
			sqlCommand.ExecuteNonQuery()
			if (outputParameterName is not None):
				outputParameter = _ParametersHelpers.GetParameter(sqlCommand, outputParameterName)
				if (outputParameter):
	...
	def ExecuteJSON(self,
		...
		try:
			...
			sqlCommand.CommandType = CommandType.StoredProcedure
			sqlCommand.CommandTimeout = 0
			_ParametersHelpers.AddNVarCharInput(sqlCommand, "Input", -1, inputValue)
			_ParametersHelpers.AddNVarCharOutput(sqlCommand, "Output", -1)
			sqlCommand.ExecuteNonQuery()
			outputParameter = _ParametersHelpers.GetParameter(sqlCommand, "Output")
			if (outputParameter):
				if (outputParameter.Value):
					if (type(outputParameter.Value) == str):
						returnValue = str(outputParameter.Value)
...
class SQLServerBulkData(IBulkData):
	...
	def GetDataTable(self, schema:str = None, tableOrView:str = None, query:str = None, options:dict = None):
		returnValue = DataTable()
		exception = None
		sqlConnection = None
		sqlCommand = None
		commandText:str = None
		if (schema is not None and tableOrView is not None):
			commandText = f"SELECT * FROM [{schema}].[{tableOrView}]"
		elif (query is not None):
			commandText = query
		if (commandText is None):
			raise ValueError("Either both Schema and Table/View must be provided, or Query must be provided.")
		try:
			sqlConnection = SqlClient.SqlConnection(self.ConnectionString)
			sqlConnection.Open()
