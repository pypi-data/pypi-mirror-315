...
class APK:
    def __init__(self):
        ...
        # DexFile 初始化
        self._is_dex_files_parsed = False
        self._dex_files: list = []
        """dex文件列表"""

        # dex
        self._is_dex_parsed = False
        self._dex_strings: list = []
        """DEX字符串"""
        self._dex_hex_strings: list = []
        """DEX16进制字符串"""
        self._opcodes: list = []
        """opcode列表"""
        self._strings_refx: dict = {}
        """字符串引用表"""
        self._classes: list = []
        """类列表"""
        self._methods: list = []
        """方法列表"""
        self._methods_refx: dict = {}
        """方法引用表"""
        ...
        # resources.arsc
        self._is_arsc_parsed = False
        self._arsc: ARSCParser | None = None
        """资源文件解析器"""
        self._app_icons: list = []
        """图标列表"""

        # AndroidManifest.xml
        self._is_manifest_parsed = False
        self._manifest: str = ""
        """清单文件"""
        ...
        self._app_name: str = ""
        """应用名"""
        self._application_icon_addr: str = ""
        """应用图标地址"""
        self._main_activities: list = []
        """主活动列表"""

    # ------------------------- 读取APK文件 ----------------------------
    @classmethod
    def from_file(cls, path: str) -> "APK":
        cls._apk_path = path
        return cls.from_io(path)

    @classmethod
    def from_bytes(cls, _bytes) -> "APK":
        return cls.from_io(io.BytesIO(_bytes))

    @classmethod
    def from_io(cls, _io) -> "APK":
    ...
    def get_manifest(self):
        if not self._is_manifest_parsed:
            self._init_manifest()
        return self._manifest

    def _init_manifest(self):
        self._is_manifest_parsed = True
        ...
        try:
            if ANDROID_MANIFEST in self.afile.namelist():
                ...
                try:
                    ...
                    if buff is None:
                        return
                    self._manifest = buff.decode("UTF-8")
                except Exception:
                    traceback.print_exc()
                    return
        except Exception:
        ...
        # fix manifest
        self._manifest = re.sub(
            r'\s:(="[\w]*?\.[\.\w]*")', r" android:name\1", self._manifest
        )

        soup = BeautifulSoup(self._manifest, "lxml-xml")
        ...
        def find_activities(tag):
            for item in soup.select(tag):
                ...
                assert isinstance(self._main_activities, list)
                self._main_activities.append(name)

                addr = item.get("android:icon", "")
                if isinstance(addr, str):
                    self._activities_icon_addrs.append(addr.lower().replace("@", "0x"))
                else:
                    raise TypeError("addr: expected str, got %s" % type(addr))
    ...
    def _init_dex_strings(self):
        ...
        self._dex_strings = sorted(list(str_set))
        self._dex_hex_strings = sorted(list(hex_str_set))

    def get_dex_hex_strings(self):
        if self._dex_hex_strings == []:
            self._init_dex_strings()
    ...
    def _init_arsc(self):
        self._is_arsc_parsed = True
        ARSC_NAME = "resources.arsc"
        try:
            if ARSC_NAME in self.afile.namelist():
                data = self.afile.read(ARSC_NAME)
                self._arsc = ARSCParser(data)
                self.package = self._arsc.get_packages_names()[0]
        except Exception as e:
            print(self._apk_path)
            print(e)

    def get_arsc(self):
        if not self._is_arsc_parsed:
            self._init_arsc()
        return self._arsc
    ...
    def _init_app_icons(self):
        ...
        try:
            soup = BeautifulSoup(
                self._arsc.get_public_resources(self._package_name), "lxml-xml"
            )
    ...
    def _init_app_name(self):
        ...
        try:
            soup = BeautifulSoup(
                self._arsc.get_string_resources(self._package_name), "lxml-xml"
            )
