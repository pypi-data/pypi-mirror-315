...
import { B as Block, S as Static, W as BlockTitle, a0 as Clear, M as Music, ap as Video, K as File$1, aq as Paperclip, ar as Send, as as Square } from './2-BHQ5_Fzx.js';
import { U as Upload } from './ModifyUpload-GcZjrOvZ.js';
import { I as Image$1 } from './Image-DFqHtuJN.js';
export { default as BaseExample } from './Example18-BdkBf4jd.js';
...
const MultimodalTextbox = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="${["full-container svelte-cgv3eu", dragging ? "dragging" : ""].join(" ").trim()}" role="group" aria-label="Multimedia input field"${add_attribute("this", full_container, 0)}> <label${add_classes((container ? "container" : "").trim())}>${validate_component(BlockTitle, "BlockTitle").$$render($$result, { root, show_label, info }, {}, {
      default: () => {
        return `${escape(label)}`;
      }
    })} ${value.files.length > 0 || uploading ? `<div class="thumbnails scroll-hide svelte-cgv3eu" aria-label="Uploaded files" data-testid="container_el" style="${"display: " + escape(value.files.length > 0 || uploading ? "flex" : "none", true) + ";"}">${each(value.files, (file, index) => {
      return `<span role="listitem" aria-label="File thumbnail"><button class="thumbnail-item thumbnail-small svelte-cgv3eu"><button class="${["delete-button svelte-cgv3eu", disabled ? "disabled" : ""].join(" ").trim()}">${validate_component(Clear, "Clear").$$render($$result, {}, {}, {})}</button> ${file.mime_type && file.mime_type.includes("image") ? `${validate_component(Image$1, "Image").$$render(
        $$result,
        {
          src: file.url,
          title: null,
    ...
    })} ${uploading ? `<div class="loader svelte-cgv3eu" role="status" aria-label="Uploading"></div>` : ``}</div>` : ``} <div class="input-container svelte-cgv3eu">${!disabled && !(file_count === "single" && value.files.length > 0) ? `${validate_component(Upload, "Upload").$$render(
      $$result,
      {
        file_count,
        filetype: file_types,
      ...
      {}
    )} <button data-testid="upload-button" class="upload-button svelte-cgv3eu">${validate_component(Paperclip, "Paperclip").$$render($$result, {}, {}, {})}</button>` : ``} <textarea data-testid="textbox" class="${["scroll-hide svelte-cgv3eu", !show_label ? "no-label" : ""].join(" ").trim()}"${add_attribute("dir", rtl ? "rtl" : "ltr", 0)}${add_attribute("placeholder", placeholder, 0)}${add_attribute("rows", lines, 0)} ${disabled ? "disabled" : ""} ${autofocus ? "autofocus" : ""}${add_attribute("style", text_align ? "text-align: " + text_align : "", 0)}${add_attribute("this", el, 0)}>${escape(value.text || "")}</textarea> ${submit_btn ? `<button class="${["submit-button svelte-cgv3eu", submit_btn !== true ? "padded-button" : ""].join(" ").trim()}">${submit_btn === true ? `${validate_component(Send, "Send").$$render($$result, {}, {}, {})}` : `${escape(submit_btn)}`}</button>` : ``} ${stop_btn ? `<button class="${["stop-button svelte-cgv3eu", stop_btn !== true ? "padded-button" : ""].join(" ").trim()}">${stop_btn === true ? `${validate_component(Square, "Square").$$render($$result, { fill: "none", stroke_width: 2.5 }, {}, {})}` : `${escape(stop_btn)}`}</button>` : ``}</div></label> </div>`;
  } while (!$$settled);
  return $$rendered;
});
...
//# sourceMappingURL=Index38-z3gaffCs.js.map
