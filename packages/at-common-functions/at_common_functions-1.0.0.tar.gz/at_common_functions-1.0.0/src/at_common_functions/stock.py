...
async def stock_get_quotation(symbol: str) -> Dict:
    ...
    return quotations[0].__dict__

async def stock_get_candlesticks(symbol: str, type: str, limit: int) -> List[Dict]:
    storage = get_storage()
    ...
    return [c.__dict__ for c in candlesticks]

async def stock_get_indicators(symbol: str, type: str, limit: int) -> List[Dict]:
    storage = get_storage()
...
async def stock_get_financials(symbol: str, period: str, statement: str, limit: int) -> List[Dict]:
    storage = get_storage()
    ...
    return [statement.__dict__ for statement in statements]