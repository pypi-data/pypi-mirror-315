...
def test_help(invoke) -> None:
    result = invoke(["--help"])
    assert result.exit_code == 0
    assert all([cmd in result.output for cmd in ("auth", "config", "repo", "--version", "--config", "--help")]), result.output
...
def test_repo(invoke, isolated_org_name, helpers, kind=None) -> None:
    ...
    assert f"Repo {org_name}/{_o_origin_name} not found" in result.stderr
