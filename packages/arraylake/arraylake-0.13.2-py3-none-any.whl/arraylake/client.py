...
class AsyncClient:
    ...
    async def get_repo(
    ...
    ) -> repo_v1.AsyncRepo | IcechunkStore:  # noqa
        """Get a repo by name
        ...
        if repo_model.kind == RepoKind.V1:
            ...
            return await icechunk_store_from_repo_model(
                repo_model=repo_model, prefix=None, credentials=credentials, mode="r+" if checkout else "r"
            )
    ...
    async def create_repo(
        ...
        if kind == RepoKind.V1:
            ...
            if not isinstance(repo_model.bucket, BucketResponse):
                raise ValueError("The bucket on the catalog object must be a BucketResponse for Icechunk V2 repos!")
            credentials = await self._maybe_get_credentials_for_icechunk(bucket=repo_model.bucket, org=org, repo_name=repo_name)
            return await icechunk_store_from_repo_model(repo_model=repo_model, prefix=prefix, credentials=credentials, mode="w-")
...
class Client:
    ...
    def get_repo(self, name: str, *, checkout: Optional[bool] = None, read_only: bool = False) -> repo_v1.Repo | IcechunkStore:  # noqa
        """Get a repo by name
        ...
        try:
            ...
            if isinstance(arepo, IcechunkStore):
                return arepo
            else:
                raise ValueError("Output repo is not an IcechunkStore, but icechunk is in the environment!")
        except ImportError:
    ...
    def get_or_create_repo(
        ...
        *,
        ...
        kind: Optional[RepoKind] = None,
        prefix: Optional[str] = None,
    ) -> repo_v1.Repo | IcechunkStore:  # noqa
        """Get a repo by name. Create the repo if it doesn't already exist.
    ...
    def list_repos_for_bucket_config(self, *, org: str, nickname: str) -> list[repo_v1.Repo | IcechunkStore]:  # noqa
