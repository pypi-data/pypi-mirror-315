    ...
    [
        (
            ...
            [
                ...
                ("SELECT * WHERE sample_info(images)['shape'][0] > 200", 118078, None),
                ("SELECT * WHERE sample_info(images)['shape'][1] > 200", 118219, None),
                ("SELECT * WHERE sample_info(images)['shape'][2] == 3", 118060, None),
                ("SELECT * WHERE sample_info(images)['shape'][2] == 2", 0, None),
                ("SELECT * where sample_info(images)['license'] == 3", 0, None), # no license in sample_info
                ("SELECT * where sample_info(images)['filename'] == './coco/train2017/000000001792.jpg'", 1, None), # no license in sample_info
        ...
        (
            ...
            [
                ...
                ("select * where labels / 3 <  1", 18623, None),
...
def test_dataset_query_results(ds_name, queries):
    ...
    for query, result_size, result_indices in queries:
        ...
        if (len(result) != result_size):
            ...
            print("\tExpected size: ", result_size)
            ds.summary()
            print(result)
...
def test_false_where_statement_bug():
    view = api.tql.query(f"""select * from "{root + "/coco_small"}" where false""", token='')
    assert len(view) == 0
    assert view.images.shape == ()
...
def test_aliased_tensor_metadata():
    ds = deeplake.load("hub://activeloop/coco-train")
    alias = deeplake.query('select super_categories[NONZERO("categories" == 45)] as super from (select * from "hub://activeloop/coco-train" limit 1)')
    assert dict(ds['super_categories'].info) == dict(alias['super'].info)