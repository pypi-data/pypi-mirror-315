...
def test_bbox(get_pytest_local_path):
    ...
    with ds:
        ...
        assert ds.bboxes2.info.coords == ds.bboxes2.info["coords"] == None
...
def test_class_label_data():
    root = os.environ["VIZ_TEST_DATASETS_PATH"]
    ds = deeplake.query(f'SELECT * FROM "{root}/imagenet-labels" WHERE labels == \'goldfish\' UNION (SELECT * FROM "{root}/imagenet-labels" WHERE labels == \'carton\')')
    assert len(ds) == 2600
    assert ds.labels[0].data() == {'text': ['goldfish'], 'value': [1]}
    assert ds.labels[2599].data() == {'text': ['carton'], 'value': [478]}
