...
class EdfFile(object):
    def __init__(self, fn):
        ...
        signal_text_labels = [
            efile.signal_label(chn).strip() for chn in range(nsignals)
        ]
...
def edf2hdf5(fn):
    ...
    h5 = tables.openFile(
        "tstint16.h5", mode="w", title="test int16 file", filters=compfilter
    )
...
if __name__ == "__main__":
    ...
    for chn in range(f.nsignals):
        print(
            "%d\t%s\t%d" % (chn, f.signal_text_labels[chn], f.signal_sample_freqs[chn])
        )
    ...
    slab = h5.createArray(
        h5.root, "signal_text_labels", f.signal_text_labels, "signal text labels"
    )
