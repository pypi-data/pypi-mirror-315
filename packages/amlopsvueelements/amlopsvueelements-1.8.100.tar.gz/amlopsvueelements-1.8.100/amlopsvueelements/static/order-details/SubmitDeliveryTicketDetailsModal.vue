...
const fromDateTime = ref([getUpliftDateTime(fuelUplifts.value?.[0], orderStore?.order)]);
...
const populateDateTime = (index = 0) => {
  fromDateTime.value[index] = getUpliftDateTime(fuelUplifts.value?.[index], order?.value);
