...
copy_table，is_clear=true会提前drop table，如果表存在外键关联第一次可能删不掉，多执行几次copy_table即可
copy_table，rowid类型会转为varchar(18)，而不是oid，转为oid数据迁移不过去
copy_data，is_clear=true会提前清空表
copy_data，oracle中的chr(0)会被强行替换为chr(32)
copy_data，oracle中date和timestamp类型中的0000年会被强行替换为0001年，00月替换为01月，00日替换为01日
copy_data，收集元数据时，会count表，is_count_full=true则强制走全表，避免索引与表不同步的情况，因此大表可能比较慢
copy_index，is_clear决定是否会先drop已存在索引
copy_index，如果索引名已被占用（被其他对象占用），且is_clear=true会自动重命名
...
is_auto_count = conf_map['is_auto_count']
is_count_full = conf_map['is_count_full']
smallest_object = conf_map['smallest_object']
is_only_insert = conf_map['is_only_insert']
is_clear = conf_map['is_clear']
is_only_scn = conf_map['is_only_scn']
is_optimize_clob = conf_map['is_optimize_clob']
...
def copy_data():
    ...
    # 获取scn，如果需要
    if is_only_scn == 'true' and len(input_global_scn) == 0:
        global scn_time
    ...
    elif is_only_scn == 'true' and len(input_global_scn) > 0:
        ...
        try:
            ...
            for table_map in table_map_list:
                try:
                    ...
                    log.error('获取元数据', table_map[0] + '.' + table_map[1], str(_))
                    continue
    ...
    def callback(task):
        if task is not None:
            ...
            pp.run(get_data_from_oracle, task, name=task[1], callback=callback)
...
def get_data_from_oracle(table_map, mess, table_scn, col_desc, table_metadata=None, page=None, page_rn=None):
    ...
    def ready():
        try:
            ...
            # 获取连接
            src_db = DBConn(src_db_type, src_ip, src_port, src_database, src_read_username, src_read_password, poolsize=max_page_num + 1)
            ...
            for col in cols_for_copy:
                ...
                elif 'CLOB' in str(col[1]) and is_optimize_clob == 'true':
                    is_exists_clob = True
            ...
            return src_db, dst_db, once_num, rs, is_exists_clob, is_exists_blob, merge_col, cols_for_copy
        except Exception as _:
            log.error('到Oracle获取数据失败，准备阶段出错', my_mess, str(_))
    
    try:
        tp = ThreadPool(1)
        tid = tp.run(ready)
        src_db, dst_db, once_num, rs, is_exists_clob, is_exists_blob, merge_col, cols_for_copy = tp.get_result(tid, timeout=60)
    except Exception as _:
        ...
        # log.error('到Oracle获取数据失败', mess, '获取Oracle连接超时60s，请使用后面SQL，使用copy_from_sql补数', select_sql, str(_))
        log.warning('到Oracle获取数据失败', mess, '获取Oracle连接超时60s，自动重试')
        return task
    ...
    try: 
        ...
        while True:
            ...
            if is_exists_clob and is_optimize_clob == 'true':
                for row in rows:
                    idx = 0
                    row_new = []
            ...
            if len(rows_new) == 0:
                ...
                tp.run(save_to_pg, (dst_db, table_map, rows_new, cols_for_copy, my_mess, select_time, table_scn, is_exists_blob))
                # save_to_pg(dst_db, table_map, rows_new, cols_for_copy, my_mess, select_time, table_scn, is_exists_blob)
            i = i + 1
        tp.wait()
        return None
    except Exception as _:
        log.error('到Oracle获取数据失败，抽数阶段出错', mess, str(_))
        return None
    finally:
        src_db.close()
        dst_db.close()
...
def get_data_from_sql(table_map): 
    ...
    try:
        ...
        while True:
            ...
            if len(rows) == 0:
                break
            tp.run(save_to_pg, (dst_db, table_map, rows, cols, mess, select_time, '', is_exists_blob))
            i = i + 1
        tp.wait()
    except Exception as _:
...
def save_to_pg(dst_db, table_map, rows, cols, mess, select_time, table_scn, is_exists_blob):
    ...
    if is_only_insert == 'true' or is_exists_blob:
        try:
        ...
        for sql in sqls:
            if sql.startswith('create table '):
                ...
                try:
                    ...
                    if 'already exists' in str(_) and is_clear == 'true':
...
def copy_constraint(run_mode):
    ...
    if is_clear == 'true' or run_mode == 'drop_fk':
        for table_map in table_map_list:
...
def compare_data_number():
    ...
    if scn_time != 0 and is_only_scn == 'true' and run_mode == 'copy_data':
