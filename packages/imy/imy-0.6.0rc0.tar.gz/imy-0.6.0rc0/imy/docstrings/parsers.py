...
def parse_module(
    ...
    *,
    ...
    for name in public_member_names:
        ...
        module_.add_member(obj, name=name)
...
def parse_class(
    ...
    *,
    ...
    def add_fields(cls: type) -> None:
        ...
        for prop_name, prop in vars(cls).items():
            ...
            if isinstance(prop, property):
                class_.members[prop_name] = data_models.PropertyDocs.from_property(
                    prop, owner=class_
                )
    ...
    for func_ in class_.functions.values():
        if func_.name != "__init__":
            continue

        for param in func_.parameters.values():
