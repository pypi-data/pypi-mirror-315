...
async def upload_for_oss(file: bytes):
    token = await get_next_token_for_polling(FEISHU_URL_OSS, from_redis=True)
    data = await upload(file, token, vip=False)

    return data["data"]["ossPath"]
...
async def upload(file: bytes, token: Optional[str] = None, vip: Optional[bool] = True):
    ...
    data = await get_request_policy(token, vip)  # todo：修复 过期时间
...
async def create_task(request: VideoRequest, token: Optional[str] = None, vip: Optional[bool] = True):
    ...
    if request.first_frame_image:
        ...
        data = await upload(file, token=token, vip=vip)
        file_data = {
            "id": data['data']['fileID'],
            "name": "_.png",
            "type": "png"
...
if __name__ == '__main__':  # 304752356930580482
    ...
    # arun(upload(Path(p).read_bytes(), vip=vip))
