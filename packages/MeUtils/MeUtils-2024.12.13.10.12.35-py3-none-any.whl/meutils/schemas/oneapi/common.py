...
USER_USABLE_GROUPS = {
    "default": "1.0￥=1$默认分组-全模型",

    "vip3": "3.0￥=1$直连",
    "vip8": "8.0￥=1$直连❤️不计成本保证稳定性",
...
if _ := {k: f"888￥=1$🈲🈲🈲误用不退" for k, v in GROUP_RATIO.items() if k.endswith(("代理",))}:
    USER_USABLE_GROUPS = {**USER_USABLE_GROUPS, **_}
