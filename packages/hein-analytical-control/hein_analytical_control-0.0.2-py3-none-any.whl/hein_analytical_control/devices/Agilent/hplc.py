...
from hein_analytical_control.devices.Agilent.hplc_param_types import HPLCAvailStatus, HPLCRunningStatus, HPLCErrorStatus
...
class HPLCController:
    ...
    def _set_status(self):
        ...
        self._most_recent_hplc_status = HPLCAvailStatus[status] if HPLCAvailStatus.has_member_key(status) else \
            HPLCRunningStatus[status] if HPLCRunningStatus.has_member_key(status) else HPLCErrorStatus[
                status] if HPLCErrorStatus.has_member_key(status) else ""
    ...
    def update_method(self, method_dir=DEFAULT_METHOD_DIR):
        """
        Allows the user to update the current set method with new HPLC parameters. This
        assumes that
        1. the Excel sheet exists AND
        2. the Excel sheet follows this format OR (TODO) you provide you own

        Raises:
            IndexError: Response did not have expected format. Try again.
        """
        self.send(
            UPDATE_METHOD_CMD.format(method_dir=method_dir)
        )

        # check that method updated !! TODO
        for _ in range(10):
            try:
