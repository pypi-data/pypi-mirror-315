...
class ProxyUI:
    ...
    def _get_stats_text(self):
        ...
        return [
            ...
            ('class:stats', f'║ Interface: {interface_info:<30} ║\n'),
            ('class:stats', f'║ Active Connections: {stats.active_connections:<20} ║\n'),
            ('class:stats', f'║ Current Bandwidth: {self._format_bytes(bandwidth)}/s    ║\n'),
            ('class:stats', f'║ Total Data Sent: {self._format_bytes(stats.total_bytes_sent)}      ║\n'),
            ('class:stats', f'║ Total Data Received: {self._format_bytes(stats.total_bytes_received)}  ║\n'),
            ('class:stats', f'║ Uptime: {str(uptime).split(".")[0]}              ║\n'),
            ('class:stats', f'║ CPU Usage: {cpu_percent}%                        ║\n'),
            ('class:stats', f'║ Memory Usage: {self._format_bytes(memory.rss)}        ║\n'),
            ('class:title', '╚═════════════════════════════════════════╝\n'),
            ...
            try:
                if cmd == b'\x01':  # CONNECT
                    ...
                    if actual_ip != wifi_ip:
                        ...
                        # Force close and retry with explicit routing
                        remote.close()
                        return
