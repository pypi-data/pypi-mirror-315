...
class Setting(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield ScrollableContainer(
            Label(
                self.message("作品数据 / 文件保存根路径"),
                classes="params",
            ),
            Input(
                self.data["work_path"],
                placeholder=self.message("程序根路径"),
                valid_empty=True,
                id="work_path",
            ),
            Label(
                self.message("作品文件储存文件夹名称"),
                classes="params",
            ),
            Input(
            ...
            Label(
                self.message("作品文件名称格式"),
                classes="params",
            ),
            Input(
                self.data["name_format"],
                placeholder=self.message("发布时间 作者昵称 作品标题"),
                valid_empty=True,
                id="name_format",
            ),
            ...
            Label(
                self.message("User-Agent"),
                classes="params",
            ),
            Input(
                self.data["user_agent"],
                placeholder=self.message("内置 Chrome User Agent"),
                valid_empty=True,
                id="user_agent",
            ),
            Label(
                self.message("小红书网页版 Cookie"),
                classes="params",
            ),
            Input(
            ...
            Input(
                self.data["proxy"],
                placeholder=self.message("不使用代理"),
                valid_empty=True,
                id="proxy",
            ),
            Label(
                self.message("请求数据超时限制，单位：秒"),
                classes="params",
            ),
            Input(
            ...
            Label(
                self.message("下载文件时，每次从服务器获取的数据块大小，单位：字节"),
                classes="params",
            ),
            Input(
                str(self.data["chunk"]),
            ...
            Container(
                Checkbox(
                    self.message("记录作品详细数据"),
                    id="record_data",
                    value=self.data["record_data"],
                ),
                Checkbox(
                    self.message("作品文件夹归档模式"),
                    id="folder_mode",
                    value=self.data["folder_mode"],
                ),
                Checkbox(
                    self.message("视频作品下载开关"),
                    id="video_download",
                    value=self.data["video_download"],
                ),
                Checkbox(
                    self.message("图文作品下载开关"),
                    id="image_download",
            ...
            Container(
                Checkbox(
                    self.message("动图文件下载开关"),
                    id="live_download",
                    value=self.data["live_download"],
                ),
                Checkbox(
                    self.message("作品下载记录开关"),
                    id="download_record",
