...
class Generate_Transcation:
            ...
            if "sql_model" in sql_model and "columns" in sql_model['sql_model']:
                for column in sql_model['sql_model']['columns']:
                    ...
                    if str(sql_str).startswith("SELECT ") and not self.is_valid_json(sql_str):
                        ...
                        after_parenthesis = after_parenthesis[:-1].strip()
    ...
    def replace_validation(self, sql_models, obj_name, connection, Owner):
        for sql_model in sql_models:
            if "sql_model" in sql_model and "columns" in sql_model['sql_model']:
                for column in sql_model['sql_model']['columns']:
                                            ...
                                            if pattern in cleaned_str:
                                                ...
                                                for i in cleaned_str.split(pattern):
                                                    if ")" in i:
                                                        new_str = i.split(")")[0]
                                                        if '.' in new_str:
                                                            new_res_list.append(f":{int(new_str.split('.')[0])-1}.{new_str.split('.')[1]}")
    ...
    def get_model_from_schema_name(self):
        ...
        if "dbDetails" in jsondata and jsondata["dbDetails"] != None:
            ...
            logger.log(f"\nInside dbDetails value:::\t{dbDetails} \t{type(dbDetails)}","0")
        else:
    ...
    def enhancement_in_model(self, transaction_model, object_name, connection, owner):
        form_one_actions = []
        ...
        sql_models = self.replace_validation(sql_models, object_name, connection, owner)
        ...
        for sql_model in sql_models:
            ...
            if "sql_model" in sql_model and "actions_links" in sql_model['sql_model']:
                ...
                if (form_no == '1' or form_no == 1):
                    ...
                    check_lst = [item.split(':')[0] for item in actions_lst]
                    if len(form_one_actions) > 0:
                        for action in form_one_actions:
    ...
    def genearate_transaction(self, transaction_model, object_name, connection):
        ...
        if "attach_docs" in transaction_model:
            ...
            obj_attach_config.process_data(connection, attach_docs)
