...
class GenerateEditMetadataXML:
    ...
    def get_Table_Column(self, column_detail ,joins_details):
        ...
        dropdown_tags = (
            ...
            column_detail["edit_mask"].get("mask", "") == "ddlb"
            else ""
        )
        
        table_column_tag = f'''<table_column>
                            <type{' precision="3"' if column_detail['col_type'].lower() == 'number' else (
                                f' size="{"0" if column_detail["db_size"] == "" else round(float(column_detail["db_size"]))}"' 
        ...
        logger.log(f"table_column_tag:: {table_column_tag}")
    ...
    def get_Text_Object(self, column_detail, x, y):
        ...
        text_object_tag = f'''<TextObject>
                                    ...
                                    <visible>{"1" if not column_detail['hidden'] else "0" if str(column_detail['hidden']) == "1" else "1" if str(column_detail['hidden']) == "0" else "1"}</visible>
                                    <font>
    ...
    def get_Column_Object(self, column_detail, x, y, tabsequence):
        ...
        if 'edit_mask' in column_detail and 'mask' in column_detail['edit_mask']:
            if column_detail['edit_mask']['mask'] == 'ddlb':
        ...
        column_object_tag = f'''<ColumnObject>
                    ...
                    <height>15</height>
                    <width>{column_detail['width'] if column_detail['width'] else '200'}</width>
                    <format>{'dd/mm/yy' if column_detail['col_type'].upper() == 'DATE' else '[general]'}</format>
                    <html>
                        <valueishtml>0</valueishtml>
                    </html>
                    <name>{column_detail['db_name'].lower()}</name>
    ...
    def retrival_query(self, sqlModel, column_detail_lst, argument_list):
        ...
        logger.log(f"GenerateEditMetadataXML sql query ::: {sql_query}")
        return sql_query

    def get_argument_list(self, column_detail_lst, joins_details):
    ...
    def get_join_object(self, sqlModel, column_detail):
        ...
        if "joins" in sqlModel and "join_predicates" in sqlModel['joins'] and "joins" in sqlModel['joins']['join_predicates']:
            ...
            if len(join_Data_list) == 1:
                ...
                logger.log(f"Outside in edit")  
                for each_join_detail in join_Data_list :
                    if each_join_detail['main_table'] == True:
                        logger.log(f"each_join_detail['main_table']  in edit::: {each_join_detail}")  
                        if "table" in each_join_detail:
                            logger.log(f"Inside2 in edit")
                            update_tag = f'''<updatewhere>0</updatewhere>
                                             <updatekeyinplace>no</updatekeyinplace>
    ...
    def build_xml_str(self, object_name):
        ...
        for sqlmodels_list in self.jsonData["transaction"]["sql_models"]:
            ...
            for index, column_detail in enumerate(sqlmodels_list['sql_model']['columns']) :# [0]['column']):
                tableColumn     = self.get_Table_Column(column_detail['column'], sqlmodels_list['sql_model']['joins'])
                ...
                logger.log(f"current_group_name ::: {current_group_name}")    
                logger.log(f"previous_group_name ::: {previous_group_name}")   
                logger.log(f"index ::: {index}")   
                logger.log(f"len(column_detail['column']) - 1 ::: {len(sqlmodels_list['sql_model']['columns']) - 1}")                             
                condition_1 = (current_group_name != "" and previous_group_name != current_group_name)
                # condition_2 = (len(sqlmodels_list['sql_model']['columns']) - 1 == index)

                logger.log(f"1st condition ::: {condition_1}")
                # logger.log(f"2nd condition ::: {condition_2}")
                if condition_1 or index == 0:
                    logger.log(f"Inside")
            ...
            argument_list         = self.get_argument_list(sqlmodels_list['sql_model']['columns'],sqlmodels_list['sql_model']['joins'])
