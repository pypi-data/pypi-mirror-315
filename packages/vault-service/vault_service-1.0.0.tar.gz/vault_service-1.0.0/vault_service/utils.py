...
def store_secret(root_path: str, sub_path: str, secret_name: str, secret_data: SecretData, credentials: dict):
    try:
...
def get_secret(root_path: str, sub_path: str, secret_name: str, credentials: dict):
    try:
        vault_controller = VaultController(credentials)
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
...
def update_secret(root_path: str, sub_path: str, secret_name: str, secret_data: SecretData, credentials: dict):
    try:
        vault_controller = VaultController(credentials)
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
...
def delete_secret(root_path: str, sub_path: str, secret_name: str, credentials: dict):
    try:
        vault_controller = VaultController(credentials)
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
...
def get_all_secrets_for_tenant(root_path: str, sub_path: str, credentials: dict):
    try:
        vault_controller = VaultController(credentials)
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
        secrets = vault_controller.get_all_secrets_for_tenant(root_path, sub_path)
...
def delete_all_secrets_for_tenant(root_path: str, sub_path: str, credentials: dict):
    try:
        vault_controller = VaultController(credentials)
        if vault_controller is None:
            return Response(status="error", message="VaultController not initialized").dict()
        vault_controller.delete_all_secrets_for_tenant(root_path, sub_path)
