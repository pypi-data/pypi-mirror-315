...
class DateHelper:
    ...
    def ms_to_date(self, ms: int, override_timezone: str = None) -> datetime:
        ...
        try:
            ...
            raise ValueError(f"Error converting timestamp {ms}: {e}")
    ...
    def custom_range(self, start_date: datetime, end_date: datetime) -> (int, int):
        if not isinstance(start_date, datetime) or not isinstance(end_date, datetime):
            raise ValueError("start_date and end_date must be datetime objects")
        return self._to_milliseconds(start_date), self._to_milliseconds(end_date)
    ...
    def group_by_period(
        ...
        for item in data:
            try:
                created_time = self.ms_to_date(item[date_key], override_timezone)

                if group_by == "day":
                    key = created_time.strftime("%Y-%m-%d")
                elif group_by == "week":
                    key = created_time.strftime("%Y-W%U")
