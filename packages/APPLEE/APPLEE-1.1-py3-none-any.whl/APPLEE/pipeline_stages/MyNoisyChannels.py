...
class NoisyChannels:
    ...
    def __init__(self, raw, EEGFiltered= None, do_detrend=True, random_state=None, matlab_strict=False):
    ...
    def _get_filtered_data(self):
        ...
        bandpass_filter = _filter_design(
            ...
            #freq = np.array([0, 1 / self.sample_rate, 50 / self.sample_rate, 1]),
        )
    ...
    def find_all_bads(self, ransac=True, channel_wise=False, max_chunk_size=None,correlation_secs=1.0, correlation_threshold=0.4, frac_bad=0.01):
        ...
        self.find_bad_by_correlation(correlation_secs=correlation_secs, correlation_threshold=correlation_threshold, frac_bad=frac_bad)
    ...
    def find_bad_by_hfnoise(self, HF_zscore_threshold=5.0):
        ...
        if self.sample_rate > 100:
            # la razón entre la amplitud de alta frecuencia y la amplitud total estima los componentes superiores a 50 hz
            noisiness = np.divide(
                _mad(self.EEGData - self.EEGFiltered, axis=1), # Amplitude high frecuency
                _mad(self.EEGFiltered, axis=1), # Total amplitude
            )
