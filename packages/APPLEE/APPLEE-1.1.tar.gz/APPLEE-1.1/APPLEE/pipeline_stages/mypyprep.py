...
from .myrefence import Reference
...
class PrepPipeline:
    ...
    def fit(self):
        ...
        # Llamar a un nuevo método solo para interpolar
        reference.perform_interpolation()
...
def run_prep(raw,line_freqs,montage,show=False):
    raw = raw.copy()
    # Setting up channel prep parameters
    s_rate = raw.info.get("sfreq")
    # maybe use srate to make the harmonics and the phantoms of the base line freq
    all_channels_labels = raw.info.get('ch_names') # [FP1,FPZ,...]
    
    # prep_params = {'ref_chs': all_channels_labels,
    #         'reref_chs': all_channels_labels,
    #         'line_freqs': line_freqs}
    prep_params = {
    'ref_chs': [],  # Lista vacía para evitar la referenciación al promedio
    'reref_chs': [],  # Lista vacía para evitar la referenciación al promedio
    'line_freqs': line_freqs,  # Ajustar la frecuencia de línea si es necesario
