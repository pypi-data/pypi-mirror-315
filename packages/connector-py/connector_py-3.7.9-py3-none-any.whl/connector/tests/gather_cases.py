...
def gather_cases(
    ...
    capabilities_to_test: list[CapabilityName] | None = None,
    ...
    for capability in capabilities_to_test:
        pattern = f"test_{capability.value}_cases.py"
...
def calculate_coverage(integration: Integration, cases: list[str]) -> tuple[bool, set[str]]:
    ...
    existing_capabilities = set(
        ...
        for file in cases
        for capability in capabilities_to_test
        if f"test_{capability.value}_cases" in file
    )
    required_capabilities = set(capability.value for capability in capabilities_to_test)
