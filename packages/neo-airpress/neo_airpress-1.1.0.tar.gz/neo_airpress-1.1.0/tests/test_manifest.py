...
def test_should_raise_assertion_error_calling_manifest_without_mandatory_assets(
    pkpass,
):
...
def test_should_raise_assertion_error_calling_manifest_with_incomplete_pkpass(
    pkpass,
):
...
def test_should_create_manifest_dictionary(pkpass):
    pkpass.add_to_pass_package(
        ("icon.png", b"00001111"), ("pass.json", b"11110000")
    )
