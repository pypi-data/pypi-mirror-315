...
def cal_correct(
    ...
    return (acc_predict(logits, threshold).ravel() == y.cpu().numpy().ravel()).sum()
...
def is_early_stopping(best_score, score, min_loss, loss, eps):
    return score > best_score or (score == best_score and min_loss - loss > eps)

def is_early_stopping_loss(best_score, score, min_loss, loss, eps):
