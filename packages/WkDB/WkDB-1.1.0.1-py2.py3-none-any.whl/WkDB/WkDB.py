...
""" 
项目地址：https://gitee.com/purify_wang/wk-db.git
"""
...
class WkDB:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if db_type not in self.DB_TYPES:
            raise ValueError(f"db_type必须在{self.DB_TYPES}中！")
        ...
        if self.db_type == self.MYSQL:
            self.wildcard = "%s"
        if self.db_type == self.SQLITE3:
            self.wildcard = "?"
            self.kwargs.update({"check_same_thread": False})

        self.conn: sqlite3.Connection | pymysql.Connection = self.connect_db()
        atexit.register(self.close)

        self.new_thread(self.keep_connect, self.time_interval)

    def connect_db(self) -> sqlite3.Connection | pymysql.Connection:
        try:
            if self.db_type == self.MYSQL:
                conn = self.get_mysql_conn()
            elif self.db_type == self.SQLITE3:
                conn = self.get_sqlite3_conn()
            else:
                raise ValueError(f"db_type必须在{self.DB_TYPES}中！")
    ...
    def get_mysql_conn(self) -> pymysql.Connection:
        return pymysql.connect(**self.kwargs)

    def get_sqlite3_conn(self) -> sqlite3.Connection:
        return sqlite3.connect(**self.kwargs)
    ...
    def __get_query_params(self, obj: dict | list):
        if isinstance(obj, dict):
            return " AND ".join([f"`{column_name}` {'=' if obj[column_name] is not None else 'is'} {self.wildcard}" for column_name in obj.keys()])
        elif isinstance(obj, list):
            return " AND ".join([f"`{column_name}` {'=' if column_name is not None else 'is'} {self.wildcard}" for column_name in obj])

    def __get_set_params(self, obj: dict):
        return ", ".join([f"`{column_name}` {'=' if obj[column_name] is not None else 'is'} {self.wildcard}" for column_name in obj.keys()])
    ...
    def __get_placeholders(self, length):
        return ", ".join([self.wildcard] * length)
    ...
    def __package_data(self, data: list | tuple | None, cursor: sqlite3.Cursor):
        """用来封装数据: 把数据封装为json数据"""
        json_data = []
        try:
            if data is None:
    ...
    def __print_info(self, func_name, sql=None, values=None, success=True, error_msg=None, cursor: sqlite3.Cursor = None):
        if success:
    ...
    def get_column_names(self):
        ...
        """
        if self.db_type == self.SQLITE3:
            return self.__get_column_names__sqlite3()
        elif self.db_type == self.MYSQL:
            return self.__get_column_names_mysql()

    def __get_column_names__sqlite3(self):
    ...
    def __get_column_names_mysql(self):
