...
def create_node_selector_window():
    ...
    with dpg.child_window(pos=[5, 80], width=node_selector_holder_width, height=-1, parent='main_window',
                          no_scrollbar=True, always_auto_resize=True) as node_selector_holder:
        node_tree_title_id = dpg.add_text(default_value='Node Tree', indent=75)
        dpg.add_separator()

        with dpg.child_window(label='Node Selector', height=-1, parent=node_selector_holder, always_auto_resize=True) \
                as node_selector:
