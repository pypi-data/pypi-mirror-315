...
class SequenceTransformation(Transformation):
    ...
    def from_spec(cls, spec: Any) -> "SequenceTransformation":
        if not isinstance(spec, dict):
            raise ValueError(f"SequenceTransformation accepts dictionary, but got {spec}")

        steps = spec.get("steps", spec.get("transformations", spec.get("transform", [])))

        assert steps is not None, "Expected a list of transformations"

        return cls(
            [
                cls._parse_transformation(transformation_spec)
                for transformation_spec in steps
            ]
        )
