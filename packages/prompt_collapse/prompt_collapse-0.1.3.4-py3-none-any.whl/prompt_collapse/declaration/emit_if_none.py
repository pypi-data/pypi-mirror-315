...
class EmitIfNoneDeclaration(Declaration):
    def __init__(
        self,
        name: str,
        value: Any,
    ...
    def apply(self, local_state: State, global_state: State) -> Any:
        if global_state.exists(self._name):
            return global_state.get(self._name)
    ...
    @classmethod
    def from_spec(cls, spec: Any) -> "EmitIfNoneDeclaration":
        assert isinstance(spec, dict), f"Invalid spec for EmitIfNoneDeclaration: {spec}"
