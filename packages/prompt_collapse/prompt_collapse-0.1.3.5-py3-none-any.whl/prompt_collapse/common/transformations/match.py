...
class MatchTransformation(Transformation):
    ...
    def apply(self, value: Any, local_state: State, global_state: State) -> Any:
        ...
        """
        result = self._match_cases.get(value, self._default)

        if isinstance(result, Transformation):
            return result.apply(value, local_state, global_state)
    ...
    def _parse_transformation(transformation_spec: Any) -> Tuple[Optional[Any], Any]:
        ...
        if isinstance(new_value, dict):
            new_value = TRANSFORMATION_REGISTRY.parse(new_value)
