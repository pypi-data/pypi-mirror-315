...
class ShortIdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            "CRITICAL: You MUST end your response with one of these markers:\n\n"
            "#### DONE: *** - D*** I**\n"
            "- REQUIRED when BOTH new dependencies AND new eligible images (.png/.jpg/.jpeg/.ico) are needed\n"
            "- NO EXCEPTIONS - if plan mentions both, this marker MUST be used\n\n"
            "#### DONE: *** - D***\n" 
            "- REQUIRED when ONLY new dependencies are needed\n"
            "- MUST be used if ANY dependencies mentioned, but NO new images\n\n"
            "#### DONE: *** - I**\n"
            "- REQUIRED when ONLY new eligible images (.png/.jpg/.jpeg/.ico) are needed\n"
            "- MUST be used if ANY new images mentioned, but NO dependencies\n\n"
            "NO marker if neither dependencies nor images are needed\n"
            "Failure to use correct marker will result in INVALID response\n"
            "Double-check your response against these requirements\n"
        )
