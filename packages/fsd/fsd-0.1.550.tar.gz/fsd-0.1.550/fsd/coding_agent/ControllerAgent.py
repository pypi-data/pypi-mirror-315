...
from fsd.coding_agent.InitTaskAgent import InitTaskAgent
...
class ControllerAgent:
    def __init__(self, repo, explainer_controller):
        ...
        self.initTemplate = InitTaskAgent(repo)
    ...
    async def explainer_code_task_pipeline(self, tier, solution, role, original_prompt_language, file_attachments, assets_link, snow_mode):
        ...
        result = await self.initTemplate.get_init_plan(solution)
        if result.get('result') == 1 or result.get('result') == "1":
            success = await self.clone_template_repo(self.repo.get_repo_path())
            if not success:
                logger.error(" #### Failed to initialize template repository")
    ...
    async def codingProgress(self, tier, finalPrompt, role, original_prompt_language, file_attachments, focused_files, snow_mode, auto_mode, imageModel, codingModel):
        ...
        result = await self.initTemplate.get_init_plan(finalPrompt)
        if result.get('result') == 1 or result.get('result') == "1":
            success = await self.clone_template_repo(self.repo.get_repo_path())
            if not success:
                logger.error(" #### Failed to initialize template repository")
    ...
    async def clone_template_repo(self, destination_path):
        """
        Safely clone the template repository into the specified destination.
        
        Args:
            destination_path (str): The path where the repository should be cloned
        ...
        try:
            ...
            Repo.clone_from(
                "https://github.com/Zinley-dev/template-zl",
                destination_path
            )
            
            logger.info(f" #### Successfully cloned template repository to {destination_path}")
            
            # Add commit for template initialization
            self.repo.add_all_files("Zinley - Initialize template repository")
