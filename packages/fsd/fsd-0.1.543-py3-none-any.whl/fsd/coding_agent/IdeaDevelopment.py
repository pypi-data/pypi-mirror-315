...
class IdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            "   Format Guidelines:\n"
            "   SVG (Preferred for all icons and vector graphics):\n"
            ...
            "Verify unique paths\n\n"
            f"#### Build Commands\n"
            f"If required, provide {platform.system()}-specific commands:\n"
            f"```bash\n"
            f"# Dependencies/build/run commands\n"
            f"```\n\n"
            ...
            "End with #### DONE: *** - D*** I** if both dependencies and eligible images (.png, .jpg, .jpeg, .ico) needed\n"
            "End with #### DONE: *** - D*** if only dependencies needed\n"
            "End with #### DONE: *** - I** if only eligible images needed\n"
