...
@triton.jit
def _nexfort_fuse_layer_norm_broadcast_add_mul_residual_kernel(Y, X, W, B, A, R, shift, y_stride_r, y_stride_c, x_stride_r, x_stride_c, N, eps, HAS_WEIGHT: tl.constexpr, HAS_BIAS: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    tl.static_assert(False, 'placeholder')
...
@triton.jit
def _nexfort_apply_rotary_emb(xq_ptr, xk_ptr, cos_ptr, sin_ptr, xq_out_ptr, xk_out_ptr, B, M, N, stride_qa, stride_qb, stride_qc, stride_qd, stride_qe, stride_ka, stride_kb, stride_kc, stride_kd, stride_ke, stride_ca, stride_cb, stride_cc, stride_cd, stride_ce, stride_sa, stride_sb, stride_sc, stride_sd, stride_se, BLOCK_SIZE: tl.constexpr):
    tl.static_assert(False, 'placeholder')
...
@cached_heuristics({'EVEN_K': (lambda args: ((args['K'] % args['BLOCK_K']) == 0)), 'ALLOW_TF32': (lambda args: matmul_allow_tf32())})
@triton.jit
def _nexfort_fuse_quant_fp8_linear_mul_residual_kernel(X, X_S, W, W_S, B, L, S, R, Y, M_for_tune, M, N, K, stride_xsb, stride_sb, stride_sn, stride_ab, stride_am, stride_ak, stride_bk, stride_bn, stride_lb, stride_lm, stride_ln, stride_rb, stride_rm, stride_rn, stride_cb, stride_cm, stride_cn, HAS_BIAS: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr, GROUP_SIZE_M: tl.constexpr, EVEN_K: tl.constexpr, ACT_PER_TENSOR: tl.constexpr, WEIGHT_PER_TENSOR: tl.constexpr, ALLOW_TF32: tl.constexpr):
    tl.static_assert(False, 'placeholder')
...
@cached_heuristics({'EVEN_K': (lambda args: ((args['K'] % args['BLOCK_K']) == 0)), 'ALLOW_TF32': (lambda args: matmul_allow_tf32())})
@triton.jit
def _nexfort_quant_fp8_act_and_matmul_and_act_kernel(X, X_S, W, W_S, B, L, Y, M_for_tune, M, N, K, stride_xsb, stride_ab, stride_am, stride_ak, stride_bk, stride_bn, stride_lb, stride_lm, stride_ln, stride_cb, stride_cm, stride_cn, HAS_BIAS: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr, GROUP_SIZE_M: tl.constexpr, EVEN_K: tl.constexpr, PRE_ACTIVATION_METHOD: tl.constexpr, POST_ACTIVATION_METHOD: tl.constexpr, ACT_PER_TENSOR: tl.constexpr, WEIGHT_PER_TENSOR: tl.constexpr, ALLOW_TF32: tl.constexpr):
    tl.static_assert(False, 'placeholder')
