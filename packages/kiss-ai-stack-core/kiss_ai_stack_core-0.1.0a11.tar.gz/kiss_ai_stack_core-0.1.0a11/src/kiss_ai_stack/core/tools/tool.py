...
class Tool:
    ...
    async def store_docs(self, documents: List[str], metadata_list: List[Dict]) -> List[str]:
        ...
        """
        if self.__vector_db:
            LOG.info(f'Storing {len(documents)} documents in the vector database.')
            try:
                ids = await self.__vector_db.push(documents=documents, metadata_list=metadata_list)
                LOG.info(f'{len(ids)} documents successfully stored with generated IDs.')
                return ids
            except Exception as e:
                LOG.error(f'Failed to store documents: {str(e)}')
                raise IOError(f'Error storing documents: {str(e)}')
        else:
            error_message = 'Vector database has not been initialized.'
    ...
    async def process_query(self, query: str) -> Optional[ToolResponse]:
        ...
        try:
            if self.__tool_kind == ToolKind.RAG:
                LOG.info('Performing retrieval-augmented generation (RAG) for query processing.')
                chunk_result = await self.__vector_db.retrieve(query)
                chunks = chunk_result['documents'][0]  # Retrieving the first document chunk
                LOG.debug('Chunk metadata retrieved but content not logged for security reasons.')
                answer = await self.__ai_client.generate_answer(query, chunks)
                LOG.info('Answer generated by AI client.')
                ...
                LOG.info('Using direct prompt mode for query processing.')
                answer = await self.__ai_client.generate_answer(query=query)
                LOG.info('Answer generated by AI client (no documents).')

                return ToolResponse(answer=answer)

        except Exception as e:
            LOG.error(f'Failed to process query: {str(e)}')
            raise e
    ...
    async def destroy(self, cleanup: bool = False):
        ...
        if cleanup and self.__vector_db is not None:
            LOG.info('Cleaning up vector database collection.')
            await self.__vector_db.destroy()

        LOG.info('Destroying AI client connection.')
