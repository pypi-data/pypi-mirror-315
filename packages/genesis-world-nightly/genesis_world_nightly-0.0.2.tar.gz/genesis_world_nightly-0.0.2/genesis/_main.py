...
class JointControlGUI:
    ...
    def create_widgets(self):
        for i, name in enumerate(self.motor_names):
            ...
            slider = ttk.Scale(
                frame, from_=float(min_limit), to=float(max_limit), orient=tk.HORIZONTAL, length=300,
        ...
        tk.Button(
            self.master, text="Reset", font=("Arial", 12), command=self.reset_joint_position
        ).pack(pady=20)
...
def view(filename, collision, rotate, scale=1.0):
    ...
    dof_pos_limits = torch.stack(entity.get_dofs_limit(motor_dofs), dim=1)

    # Shared positions list between GUI and simulation
    gui_positions = [0.0] * len(motor_dofs)
    ...
    def start_gui():
        root = tk.Tk()
        app = JointControlGUI(root, motor_names, dof_pos_limits)
        app.joint_positions = gui_positions  # Share joint positions
    ...
    while scene.viewer.is_alive() and not is_gui_closed[0]:
        ...
        t += dt
        if rotate:
            entity.set_quat(gs.utils.geom.euler_to_quat(np.array([0, 0, t *50])))
...
def main():
    ...
    parser_view.add_argument('-c', '--collision', action='store_true', default=False, help='Whether to visualize collision geometry')
