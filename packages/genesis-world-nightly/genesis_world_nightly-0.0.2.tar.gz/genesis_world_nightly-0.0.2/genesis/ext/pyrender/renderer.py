...
class Renderer(object):
    ...
    def render_texts(self, texts, x, y, font_name='UbuntuMono-Regular',
                    font_pt=40, color=None, scale=1.0):
        x *= self.dpscale
        y *= self.dpscale
        font_pt *= self.dpscale
    ...
    def _floor_pass(self, scene, flags, seg_node_map=None):
        ...
        self.jit.forward_pass(self, V, P, cam_pos, flags | RenderFlags.SKIP_FLOOR, ProgramFlags.USE_MATERIAL, screen_size,
    ...
    def _forward_pass(self, scene, flags, seg_node_map=None):
        ...
        if flags & RenderFlags.SEG:
            ...
            self.jit.forward_pass(self, V, P, cam_pos, flags, ProgramFlags.USE_MATERIAL, screen_size, floor_tex=floor_tex)
    ...
    def _point_shadow_mapping_pass_legacy(self, scene, light_node, flags):
        ...
        for node in scene.sorted_mesh_nodes():
            ...
            for primitive in mesh.primitives:
                ...
                # First, get and bind the appropriate program
                program = self._get_primitive_program(
                    primitive, flags, ProgramFlags.POINT_SHADOW
                )
                program._bind()
    ...
    def _configure_floor_framebuffer(self):
        if self._floor_texture_depth is None:
            self._floor_texture_depth = Texture(width=self.viewport_width, height=self.viewport_height,
                                                source_channels='D', data_format=GL_FLOAT)
            self._floor_texture_depth._add_to_context()
        if self._floor_texture_color is None:
            self._floor_texture_color = Texture(width=self.viewport_width, height=self.viewport_height,
                                                source_channels='RGB', data_format=GL_FLOAT)
