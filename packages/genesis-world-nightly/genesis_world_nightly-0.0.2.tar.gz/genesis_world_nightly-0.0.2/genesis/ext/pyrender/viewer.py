...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, context, viewport_size=None,
                 render_flags=None, viewer_flags=None,
                 registered_keys=None, run_in_thread=False, auto_start=True, shadow=False, plane_reflection=False, **kwargs):
        ...
        self._renderer = Renderer(
            self._viewport_size[0], self._viewport_size[1], context.jit,
    ...
    def save_video(self, filename=None):
        ...
        # We need a separate thread to save the video so that viewer is not blocked.
        # The alive status of this thread is also used to inform user that video saving is in progress.
        self._video_saver = Thread(
            target = gs.tools.animate,
            args   = (self._saved_frames,),
            kwargs = {
                'filename' : filename,
            ...
            class CustomShaderCache():
                ...
                def get_program(self, vertex_shader, fragment_shader, geometry_shader=None, defines=None):
                    if self.program is None:
                        ...
                        print(absolute_path)
                        self.program = ShaderProgram(os.path.join(absolute_path.replace("viewer.py",""), "shaders/mesh_normal.vert"), os.path.join(absolute_path.replace("viewer.py",""), "shaders/mesh_normal.frag"), defines=defines)
                    return self.program
