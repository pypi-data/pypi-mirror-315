...
class JITRenderer:
    ...
    def set_primitive(self, scene, node_list, primitive_list):
        ...
        self.render_flags = np.zeros((n, 6), np.int8) # (blend, wireframe, double sided, pbr texture, reflective floor, transparent)
    ...
    def gen_func_ptr(self):
        ...
        @njit(none(
            int32[:], int32[:], float32[:, :, :], int32[:, :], float32[:, :], float32[:, :],
            int8[:, :], int32[:], int32[:], int32[:],
            float32[:, :], int32[:], float32[:, :, :, :], float32[:],
            float32[:, :], float32[:, :], float32[:], int32, float32[:, :], float32[:, :], int32, float32[:],
            self.gl.wrapper_type
        ), cache=True)
        def forward_pass(
            vao_id, program_id, pose, textures, pbr_mat, spec_mat,
            render_flags, mode, n_instances, n_indices,
            light, shadow_map, light_matrix, ambient_light,
            mat_V, mat_P, cam_pos, flags, color_list, reflection_mat, floor_tex, screen_size,
            gl
        ):
            ...
            for id in idx:
                ...
                if not (flags & RenderFlags_DEPTH_ONLY or flags & RenderFlags_SEG or flags & RenderFlags_FLAT):
                    tf = textures[id, 0]
                    texture_list = [
                        'normal_texture', 'occlusion_texture', 'emissive_texture', 'base_color_texture',
                        'metallic_roughness_texture', 'diffuse_texture', 'specular_glossiness_texture'
                    ]
                    for i in range(7):
                        if tf & (1<<i):
                            gl.glActiveTexture(GL_TEXTURE0 + active_texture)
                ...
                if n_indices[id] > 0:
                    gl.glDrawElementsInstanced(
                        mode[id], n_indices[id], GL_UNSIGNED_INT,
                        address_to_ptr(0), n_instances[id]
                    )
                else:
                    gl.glDrawArraysInstanced(
                        mode[id], 0, -n_indices[id], n_instances[id]
                    )
        ...
        def shadow_mapping_pass(
            vao_id, program_id, pose,
            mode, n_instances, n_indices,
            mat_V, mat_P, render_flags,
            gl
        ):
            last_pid = -1
        ...
        def point_shadow_mapping_pass(
            vao_id, program_id, pose,
            mode, n_instances, n_indices,
            light_matrix, light_pos, render_flags,
            gl
        ):
            last_pid = -1
            for id in range(len(vao_id)):
    ...
    def forward_pass(self, renderer, V, P, cam_pos, flags, program_flags, screen_size,
                     color_list=None, reflection_mat=np.identity(4, np.float32), floor_tex=0):
        self.load_programs(renderer, flags, program_flags)
        func = self._forward_pass
        # timer = time()
        if flags & RenderFlags.SEG:
            func(self.vao_id, self.program_id[(flags, program_flags)], self.pose, self.textures,
                self.pbr_mat, self.spec_mat, self.render_flags, self.mode,
                self.n_instances, self.n_indices, self.light, self.shadow_map, self.light_matrix,
                self.ambient_light, V.astype(np.float32), P.astype(np.float32),
                cam_pos.astype(np.float32), flags, color_list, reflection_mat, floor_tex, screen_size,
            ...
            func(self.vao_id, self.program_id[(flags, program_flags)], self.pose, self.textures,
                ...
                self.n_instances, self.n_indices, self.light, self.shadow_map, self.light_matrix,
                self.ambient_light, V.astype(np.float32), P.astype(np.float32),
                cam_pos.astype(np.float32), flags, self.pbr_mat, reflection_mat, floor_tex, screen_size,
                self.gl.wrapper_instance)
    ...
    def shadow_mapping_pass(self, renderer, V, P, flags, program_flags):
        ...
        func(
            self.vao_id, self.program_id[(flags, program_flags)], self.pose,
            self.mode, self.n_instances, self.n_indices, V.astype(np.float32), P.astype(np.float32), self.render_flags,
            self.gl.wrapper_instance
        )
    ...
    def point_shadow_mapping_pass(self, renderer, light_matrix, light_pos, flags, program_flags):
        ...
        func(
            self.vao_id, self.program_id[(flags, program_flags)], self.pose,
            self.mode, self.n_instances, self.n_indices, light_matrix.astype(np.float32), light_pos.astype(np.float32), self.render_flags,
            self.gl.wrapper_instance
        )
