...
class Surface(Options):
    ...
    double_sided      : Optional[bool]        = None # whether to render both sides of the surface. Defaults to True for Cloth material and False for others.
    ...
    def shortcut_info(name, map_name):
        return f'`{name}` is a shortcut for texture. '\
               f'When {name} is set, {map_name} setting is not allowed.'
    ...
    def requires_uv(self):
        return isinstance(self.get_texture(), ImageTexture) or \
               isinstance(self.opacity_texture, ImageTexture) or \
               isinstance(self.roughness_texture, ImageTexture) or \
               isinstance(self.metallic_texture, ImageTexture) or \
               isinstance(self.normal_texture, ImageTexture) or \
               isinstance(self.emissive_texture, ImageTexture)
