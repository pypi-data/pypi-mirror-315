    ...
    def __init__(self, **data):
        ...
        if self.euler is not None:
            if self.quat is None:
                self.quat = tuple(gs.utils.geom.euler_to_quat(self.euler))
...
class Mesh(FileMorph):
    ...
    def __init__(self, **data):
        ...
        if self.decimate and self.decimate_face_num < 100:
            gs.raise_exception('`decimate_face_num` should be greater than 100 to ensure sufficient geometry details are preserved.')
    ...
    def __init__(self, **data):
        ...
        if isinstance(self.subterrain_types, str):
            ...
            if np.array(self.subterrain_types).shape != (self.n_subterrains[0], self.n_subterrains[1]):
                gs.raise_exception('`subterrain_types` should be either a string or a 2D list of strings with the same shape as `n_subterrains`.')

        for row in self.subterrain_types:
            for subterrain_type in row:
                if subterrain_type not in supported_subterrain_types:
                    gs.raise_exception(f'Unsupported subterrain type: {subterrain_type}, should be one of {supported_subterrain_types}')

        if not mu.is_approx_multiple(self.subterrain_size[0], self.horizontal_scale) or not mu.is_approx_multiple(self.subterrain_size[1], self.horizontal_scale):
