...
class Timer:
    ...
    def _stamp(self, msg='', _ratio=1.0):
        ...
        print(f'{prefix}[{msg.ljust(self.msg_width)}] step: {step_time:5.3f}ms | accu: {accu_time:5.3f}ms | step_avg: {self.accu_log[msg][1]/self.accu_log[msg][0]:5.3f}ms | accu_avg: {self.accu_log[msg][2]/self.accu_log[msg][0]:5.3f}ms')
...
class FPSTracker:
    ...
    def step(self):
        ...
        if self.n_envs > 0:
            ...
            gs.logger.info(f'Running at ~<{self.total_fps:,.2f}>~ FPS (~<{fps:.2f}>~ FPS per env, ~<{self.n_envs}>~ envs).')
        else:
            self.total_fps = fps
