...
def reduce_graph(G, straight_thresh=10):
    ...
    for node in G.nodes():
        ...
        G_reduced.nodes[node].update(dict(
    ...
    for node in G_reduced.nodes():
        if node == ref_node:
            continue
        path = nx.shortest_path(G, source=ref_node, target=node) # NOTE: if there is loop, we pick shortest path to the reference node
        node_curr = ref_node
...
def plot_nxgraph(G, pos=None, plot_arrow=True, use_tick_labels=False,
                 show=True, figax=None, node_color=None, node_size=100,
                 plot_node_num=True):
    if pos is None:
    ...
    for vizedge in edge_xyz:
        ...
        if plot_arrow:
            a = Arrow3D(
                ...
                mutation_scale=20, lw=1.5, arrowstyle="-|>", color="tab:gray"
            )
            ax.add_artist(a)
