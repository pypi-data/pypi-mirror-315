...
def get_gsd_path(verts, faces, sdf_cell_size, sdf_min_res, sdf_max_res):
    hashkey = get_hashkey(verts.tobytes(), faces.tobytes(), str(sdf_cell_size).encode(), str(sdf_min_res).encode(), str(sdf_max_res).encode())
...
def get_remesh_path(verts, faces, edge_len_abs, edge_len_ratio, fix):
    hashkey = get_hashkey(verts.tobytes(), faces.tobytes(), str(edge_len_abs).encode(), str(edge_len_ratio).encode(), str(fix).encode())
    ...
    if isinstance(texture, gs.textures.ImageTexture):
        if uvs is not None:
            ...
            visual = trimesh.visual.ColorVisuals(
                vertex_colors=np.tile(texture.mean_color(), [n_verts, 1])
            )
        ...
        visual = trimesh.visual.ColorVisuals(
            vertex_colors=np.tile(np.array(texture.color), [n_verts, 1])
        )
...
def create_frame(origin_radius=0.012, axis_radius=0.005, axis_length=1.0, head_radius=0.01, head_length=0.03, sections=12):
    origin = create_sphere(radius=origin_radius, subdivisions=2)

    x = create_arrow(length=axis_length, radius=axis_radius, l_ratio=head_length/axis_length, r_ratio=head_radius/axis_radius, body_color=(0.7, 0.0, 0.0, 1.0), head_color=(0.7, 0.7, 0.7, 1.0), sections=sections)
    y = create_arrow(length=axis_length, radius=axis_radius, l_ratio=head_length/axis_length, r_ratio=head_radius/axis_radius, body_color=(0.0, 0.7, 0.0, 1.0), head_color=(0.7, 0.7, 0.7, 1.0), sections=sections)
    z = create_arrow(length=axis_length, radius=axis_radius, l_ratio=head_length/axis_length, r_ratio=head_radius/axis_radius, body_color=(0.0, 0.0, 0.7, 1.0), head_color=(0.7, 0.7, 0.7, 1.0), sections=sections)
...
def create_arrow(length=1.0, radius=0.02, l_ratio=0.25, r_ratio=1.5, body_color=(1.0, 1.0, 1.0, 1.0), head_color=(1.0, 1.0, 1.0, 1.0), sections=12):
    ...
    body.visual = trimesh.visual.ColorVisuals(
        vertex_colors=np.tile(body_color, [len(body.vertices), 1])
    )
    ...
    head.visual = trimesh.visual.ColorVisuals(
        vertex_colors=np.tile(head_color, [len(head.vertices), 1]).astype(float)
    )
...
def create_line(start, end, radius=0.002, color=(1.0, 1.0, 1.0, 1.0), sections=12):
    ...
    mesh.vertices = gu.transform_by_T(
        mesh.vertices,
        gu.trans_R_to_T(
            start,
            gu.z_to_R(end - start)
    ...
    mesh.visual = trimesh.visual.ColorVisuals(
        vertex_colors=np.tile(color, [len(mesh.vertices), 1]).astype(float)
    )
    return mesh
...
def create_tets_mesh(n_tets=1, halfsize=1.0, quats=None, randomize_halfsize=True):
    ...
    if randomize_halfsize:
        halfsize = np.tile(np.random.uniform(0.3, 1.9, size=(n_tets, 1)), [1, vert_per_tet * 3]).reshape(-1, 3) * halfsize
        halfsize = np.tile(np.random.uniform(0.3, 1.9, size=(n_tets * 4, 1)), [1, vert_per_tet // 4 * 3]).reshape(-1, 3) * halfsize
    ...
    faces_offset = np.tile(np.arange(0, n_tets).reshape(-1, 1) * vert_per_tet, [1, face_per_tet * 3]).reshape(n_tets * face_per_tet, 3)
    faces += faces_offset
...
def tetrahedralize_mesh(mesh, tet_cfg):
    pv_obj = pv.PolyData(
        mesh.vertices,
        np.concatenate([np.full((mesh.faces.shape[0], 1), mesh.faces.shape[1]), 
                        mesh.faces], axis=1)
    )
...
def visualize_tet(tet, pv_data, show_surface=True, plot_cell_qual=False):
    ...
    if show_surface:
        ...
        if plot_cell_qual:
            ...
            subgrid.plot(scalars=cell_qual, stitle='Quality',
                         cmap='bwr', clim=[0, 1], flip_scalars=True, show_edges=True)
        else:
            plotter = pv.Plotter()
