        ...
        for geom in link.collisions + link.visuals:
            ...
            if isinstance(geom.geometry.geometry, urdfpy.Mesh):
                ...
                if isinstance(geom.geometry.geometry, urdfpy.Box):
                    ...
                    tmesh      = trimesh.creation.cylinder(radius=geom.geometry.geometry.radius, height=geom.geometry.geometry.length)
    ...
    for joint in robot.joints:
        ...
        if joint.limit is not None:
            if joint.limit.effort is not None:
                j_info['dofs_force_range'] = j_info['dofs_force_range'] / np.abs(j_info['dofs_force_range']) * joint.limit.effort
