    ...
    if not os.path.exists(ptc_file_path):
        with gs.logger.timer(f'Sampling particles with ~<{sampler}>~ sampler and generating `.ptc` file:'):
            ...
            if os.path.exists(output_dir):
                process = subprocess.Popen(f'rm -rf {output_dir}', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                process.communicate()
...
def cylinder_to_particles(
    p_size=0.01, pos=(0, 0, 0), radius=0.5, height=1.0, sampler='random'
):
    if 'pbs' in sampler:
...
def filter_surface(positions, radii, particle_radius, half_width = 8.0, radius_scale = 1.0):
    ...
    gs.logger.debug(f'[ParticleMesher]: {surface_indices.info_msg}\n'
