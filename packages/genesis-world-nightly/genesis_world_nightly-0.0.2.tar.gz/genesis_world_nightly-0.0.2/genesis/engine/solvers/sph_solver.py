...
class SPHSolver(Solver):
    ...
    def init_particle_fields(self):
        ...
        self.particles_ng   = struct_particle_state_ng.field(shape=(self._n_particles,), needs_grad=False, layout=ti.Layout.SOA)
        self.particles_info = struct_particle_info.field(shape=(self._n_particles,), needs_grad=False, layout=ti.Layout.SOA)

        self.particles_reordered      = struct_particle_state.field(shape=(self._n_particles,), needs_grad=False, layout=ti.Layout.SOA)
        self.particles_ng_reordered   = struct_particle_state_ng.field(shape=(self._n_particles,), needs_grad=False, layout=ti.Layout.SOA)
        self.particles_info_reordered = struct_particle_info.field(shape=(self._n_particles,), needs_grad=False, layout=ti.Layout.SOA)

        self.particles_render = struct_particle_state_render.field(shape=self._n_particles, needs_grad=False, layout=ti.Layout.SOA)
    ...
    def _kernel_reorder_particles(self, f: ti.i32):
        self.sh.compute_reordered_idx(self._n_particles, self.particles.pos, self.particles_ng.active, self.particles_ng.reordered_idx)
        ...
        if ti.static(self._coupler._rigid_sph):
            for i, i_g in ti.ndrange(self._n_particles, self._coupler.rigid_solver.n_geoms):
                if self.particles_ng[i].active:
                    self._coupler.sph_rigid_normal_reordered[self.particles_ng[i].reordered_idx, i_g] = self._coupler.sph_rigid_normal[i, i_g]
    ...
    def _kernel_copy_from_reordered(self, f: ti.i32):
        ...
        if ti.static(self._coupler._rigid_sph):
            for i, i_g in ti.ndrange(self._n_particles, self._coupler.rigid_solver.n_geoms):
                if self.particles_ng[i].active:
                    self._coupler.sph_rigid_normal[i, i_g] = self._coupler.sph_rigid_normal_reordered[self.particles_ng[i].reordered_idx, i_g]

    @ti.func
    def _task_compute_rho(self, i, j, ret: ti.template()):
        ret += self._p_vol * self.cubic_kernel((self.particles_reordered[i].pos - self.particles_reordered[j].pos).norm())
    ...
    def _task_compute_non_pressure_forces(self, i, j, ret: ti.template()):
        ...
        if d_ij.norm() > self._particle_size:
            ret -= self.particles_info_reordered[i].gamma / self.particles_info_reordered[i].mass * self.particles_info_reordered[j].mass * d_ij * self.cubic_kernel(d_ij.norm())
        else:
            ret -= self.particles_info_reordered[i].gamma / self.particles_info_reordered[i].mass * self.particles_info_reordered[j].mass * d_ij * self.cubic_kernel(self._particle_size)
        ...
        d = 2 * (3 + 2)
        f_v = d * self.particles_info_reordered[i].mu * (self.particles_info_reordered[j].mass / self.particles_reordered[j].rho) * v_ij / (
            d_ij.norm()**2 + 0.01 * self._support_radius**2) * self.cubic_kernel_derivative(d_ij)
        ret += f_v
    ...
    def _task_compute_pressure_forces(self, i, j, ret: ti.template()):
        ...
        rho_j = self.particles_reordered[j].rho * self.particles_info_reordered[j].rho / self.particles_info_reordered[j].rho
        dp_j = self.particles_reordered[j].p / rho_j**2

        # Compute the pressure force contribution, Symmetric Formula
        ret += -self.particles_info_reordered[j].rho * self._p_vol * (dp_i + dp_j) \
    ...
    def _kernel_compute_pressure_forces(self, f: ti.i32):
        for i in range(self._n_particles):
            if self.particles_ng_reordered[i].active:
                self.particles_reordered[i].rho = ti.max(self.particles_reordered[i].rho, self.particles_info_reordered[i].rho)
                self.particles_reordered[i].p = self.particles_info_reordered[i].stiffness * (ti.pow(self.particles_reordered[i].rho / self.particles_info_reordered[i].rho, self.particles_info_reordered[i].exponent) - 1.0)
    ...
    def _kernel_advect_velocity(self, f: ti.i32):
        for i in range(self._n_particles):
            if self.particles_ng_reordered[i].active:
                self.particles_reordered[i].vel = self.particles_reordered[i].vel + self._substep_dt * self.particles_reordered[i].acc
    ...
    def _task_compute_DFSPH_factor(self, i, j, ret: ti.template()):
        # Fluid neighbors
        grad_j = -self._p_vol * self.cubic_kernel_derivative(self.particles_reordered[i].pos - self.particles_reordered[j].pos)
        ...
        for i in range(self._n_particles):
            if self.particles_ng_reordered[i].active:
                ...
                self.sh.for_all_neighbors(
                    ...
                    self._support_radius,
                    ret,
                    self._task_compute_DFSPH_factor)
    ...
    def _task_divergence_solver_iteration(self, i, j, ret: ti.template()):
        ...
        k_sum = self._density0 / self._density0 * ret.k_i  + k_j   # TODO: make the neighbor density different for multiphase fluid
        if ti.abs(k_sum) > self._df_eps:
            grad_p_j = -self._p_vol * self.cubic_kernel_derivative(self.particles_reordered.pos[i] - self.particles_reordered.pos[j])
            ret.dv -= k_sum * grad_p_j # ki, kj already contain inverse density, i.e., density canceled if not mutiphase flow
    ...
    def _kernel_compute_density_star(self):
        for i in range(self._n_particles):
            if self.particles_ng_reordered[i].active:
                ...
                drho = (
                    self.particles_reordered[i].rho / self._density0
                    + self._substep_dt * delta
                )
    ...
    def density_solve_iteration_task(self, i, j, ret: ti.template()):
        ...
        k_j = b_j * self.particles_reordered[j].dfsph_factor
        k_sum = self._density0 / self._density0 * ret.k_i + k_j # TODO: make the neighbor density0 different for multiphase fluid
        if ti.abs(k_sum) > self._df_eps:
            grad_p_j = -self._p_vol * self.cubic_kernel_derivative(self.particles_reordered[i].pos - self.particles_reordered[j].pos)
            # Directly update velocities instead of storing pressure accelerations
            ret.dv -= self._substep_dt * k_sum * grad_p_j  # ki, kj already contain inverse density, i.e., density canceled if not mutiphase flow
        ...
        for i in range(self._n_particles):
            if self.particles_ng_reordered[i].active:
                ...
                self.sh.for_all_neighbors(
                    ...
                    self._support_radius,
                    ret,
                    self.density_solve_iteration_task)
