...
class SFSolver(Solver):
    ...
    def colorize(self, s: ti.i32):
        for i, j, k in ti.ndrange(*self.res):
            ...
            if self.lower_y_vis < j < self.higher_y_vis:
                color = self.cold_color * (1 - self.grid.q[s+1, i, j, k][0]) + self.hot_color * self.grid.q[s+1, i, j, k][0]
    ...
    @ti.kernel
    def get_frame(self, s: ti.i32, v_np: ti.types.ndarray(), v_tmp_np: ti.types.ndarray(), div_np: ti.types.ndarray(), p_np: ti.types.ndarray(), q_np: ti.types.ndarray()):
        for i, j, k in ti.ndrange(*self.res):
    ...
    @ti.kernel
    def set_frame(self, s: ti.i32, v_np: ti.types.ndarray(), v_tmp_np: ti.types.ndarray(), div_np: ti.types.ndarray(), p_np: ti.types.ndarray(), q_np: ti.types.ndarray()):
        for i, j, k in ti.ndrange(*self.res):
            for n in ti.static(range(3)):
