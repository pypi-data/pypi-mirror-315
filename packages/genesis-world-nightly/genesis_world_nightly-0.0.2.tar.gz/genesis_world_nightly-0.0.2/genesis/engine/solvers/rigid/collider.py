...
class Collider:
    ...
    def _init_collision_fields(self):
        ...
        struct_sort_buffer = ti.types.struct(
            value  = gs.ti_float,
            i_g    = gs.ti_int,
            is_max = gs.ti_int
        )
        ...
        self.broad_collision_pairs = ti.Vector.field(2, dtype=gs.ti_int, shape=self._solver._batch_shape(max(1, self._max_collision_pairs)))
        ...
        self.n_contacts = ti.field(gs.ti_int, shape=self._solver._B) # total number of contacts, including hibernated contacts
    ...
    def _func_point_in_geom_aabb(self, point, i_g, i_b):
        return (point < self._solver.geoms_state[i_g, i_b].aabb_max).all() and (point > self._solver.geoms_state[i_g, i_b].aabb_min).all()

    @ti.func
    def _func_is_geom_aabbs_overlap(self, i_ga, i_gb, i_b):
        return not ((self._solver.geoms_state[i_ga, i_b].aabb_max <= self._solver.geoms_state[i_gb, i_b].aabb_min).any() or \
                    (self._solver.geoms_state[i_ga, i_b].aabb_min >= self._solver.geoms_state[i_gb, i_b].aabb_max).any())
    ...
    def _func_contact_vertex_sdf(self, i_ga, i_gb, i_b, ga_quat_rot):
        ...
        for i_v in range(ga_info.vert_start, ga_info.vert_end):
            p = gu.ti_transform_by_trans_quat(
                self._solver.verts_info[i_v].init_pos,
                ga_pos, ga_quat
            )
        ...
        for i_e in range(ga_info.edge_start, ga_info.edge_end):
            ...
            if cur_length > ga_sdf_cell_size:
                ...
                p_0 = gu.ti_transform_by_trans_quat(
                    self._solver.verts_info[i_v0].init_pos,
                    ga_state.pos, ga_state.quat
                )
                p_1 = gu.ti_transform_by_trans_quat(
                    self._solver.verts_info[i_v1].init_pos,
                    ga_state.pos, ga_state.quat
                )
    ...
    def _func_contact_convex_convex(self, i_ga, i_gb, i_b, ga_quat_rot, i_va_ws):
        ...
        if i_va == -1:
            ...
            pos_vb = gu.ti_transform_by_trans_quat(
                self._solver.verts_info[gb_vert_start].init_pos,
                gb_pos, gb_quat
            )
            i_va = self._solver.sdf._func_find_closest_vert(pos_vb, i_ga, i_b)
        i_v_closest = i_va
        pos_v_closest = gu.ti_transform_by_trans_quat(
            self._solver.verts_info[i_v_closest].init_pos,
            ga_pos, ga_quat
        )
    ...
    def _func_check_collision_valid(self, i_ga, i_gb, i_b):
        ...
        # self collision
        if ti.static(not self._solver._enable_self_collision) and \
            self._solver.links_info[i_la].root_idx == self._solver.links_info[i_lb].root_idx:
            is_valid = False
                ...
                if ti.static(not self._solver._use_hibernation):
                    for i in range(self._solver.n_geoms * 2):
                        if self.sort_buffer[i, i_b].is_max:
                            self.sort_buffer[i, i_b].value = self._solver.geoms_state[self.sort_buffer[i, i_b].i_g, i_b].aabb_max[axis]
                        else:
                            self.sort_buffer[i, i_b].value = self._solver.geoms_state[self.sort_buffer[i, i_b].i_g, i_b].aabb_min[axis]
    ...
    def _func_narrow_phase(self):
        ...
        for i_b in range(self._solver._B):
            for i_pair in range(self.n_broad_pairs[i_b]):
                ...
                if self._solver.geoms_info[i_ga].type == gs.GEOM_TYPE.SPHERE or self._solver.geoms_info[i_gb].type == gs.GEOM_TYPE.SPHERE:
                ...
                elif self._solver.geoms_info[i_ga].type == gs.GEOM_TYPE.TERRAIN or self._solver.geoms_info[i_gb].type == gs.GEOM_TYPE.TERRAIN:
                    if ti.static(self._has_terrain):
                        ...
                        if self._solver.geoms_info[i_ga].is_convex:
                            # initial point
                            is_col_0, normal_0, penetration_0, contact_pos_0, i_va0 = self._func_contact_convex_terrain(i_ga, i_gb, i_b, gu.ti_identity_quat(), self.contact_cache[i_ga, i_gb, i_b].i_va_0)
                            ...
                            if is_col_0:
                                ...
                                for i_rot in range(4):
                                    ...
                                    is_col, normal, penetration, contact_pos, i_va = self._func_contact_convex_terrain(i_ga, i_gb, i_b, gu.ti_rotvec_to_quat(axis * self._mc_perturbation), i_va0)
                                    
                                    if is_col:
                                        i_col = self.n_contacts[i_b]
                                        valid = True
                                        for j in range(n_valid):
                                            if (contact_pos - self.contact_data[i_col - j - 1, i_b].pos).norm() < self._mc_tolerance:
                                                valid = False
                                                break
                    ...
                    for i in range(2):
                        if i == 1:
                            i_ga, i_gb = i_gb, i_ga
                        # initial point
                        is_col_0, normal_0, penetration_0, contact_pos_0, i_va0 = self._func_contact_convex_convex(i_ga, i_gb, i_b, gu.ti_identity_quat(), self.contact_cache[i_ga, i_gb, i_b].i_va_0)
                        ...
                        if is_col_0:
                            ...
                            for i_rot in range(4):
                                ...
                                is_col, normal, penetration, contact_pos, i_va = self._func_contact_convex_convex(i_ga, i_gb, i_b, gu.ti_rotvec_to_quat(axis * self._mc_perturbation), i_va0)
                    ...
                    if ti.static(self._has_nonconvex_nonterrain):
                        for i in range(2):
                            if i == 1:
                                i_ga, i_gb = i_gb, i_ga
                            # initial point
                            is_col_0, normal_0, penetration_0, contact_pos_0 = self._func_contact_vertex_sdf(i_ga, i_gb, i_b, gu.ti_identity_quat())
                            ...
                            if is_col_0:
                                ...
                                for i_rot in range(4):
                                    ...
                                    is_col, normal, penetration, contact_pos = self._func_contact_vertex_sdf(i_ga, i_gb, i_b, gu.ti_rotvec_to_quat(axis * self._mc_perturbation))
