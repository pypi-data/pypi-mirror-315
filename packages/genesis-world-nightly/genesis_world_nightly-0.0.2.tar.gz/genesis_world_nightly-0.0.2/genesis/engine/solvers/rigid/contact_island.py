...
class ContactIsland:
    def __init__(self, collider):
        ...
        self.ci_edges = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape((
            self.collider._max_contact_pairs, 2)))

        self.edge_id = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape((
            self.collider._max_contact_pairs*2)))

        self.constraint_list = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape((
            self.collider._max_contact_pairs)))

        self.constraint_id = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape((
            self.solver.n_entities)))

        self.entity_edge = struct_agg_list.field(shape=self.solver._batch_shape(
            self.solver.n_entities), needs_grad=False, layout=ti.Layout.SOA)

        self.island_col = struct_agg_list.field(shape=self.solver._batch_shape(
            self.solver.n_entities), needs_grad=False, layout=ti.Layout.SOA)

        self.island_hibernated = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape((
            self.solver.n_entities)))

        self.island_entity = struct_agg_list.field(shape=self.solver._batch_shape(
            self.solver.n_entities), needs_grad=False, layout=ti.Layout.SOA)

        self.entity_id = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape((
            self.solver.n_entities)))
        ...
        self.entity_island = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape(
            self.solver.n_entities))
        self.stack = ti.field(dtype=gs.ti_int, shape=self.solver._batch_shape(
            self.solver.n_entities))
        ...
        for i_b in range(self.solver._B):
            ...
            for i in range(self.solver.n_entities):
                if self.entity_island[i, i_b] >= 0:
                    self.island_entity[self.entity_island[i, i_b], i_b].n = self.island_entity[self.entity_island[i, i_b], i_b].n + 1
