...
class RigidSolver(Solver):
    ...
    def __init__(self, scene, sim, options):
        ...
        if options.use_hibernation and not options.use_contact_island:
            gs.logger.warning('`use_hibernation` is set to False because `use_contact_island=False`. Please turn on `use_contact_island` if you want to use hibernation')
        ...
        self._sol_timeconst = max(options.timeconst, 2 * self._substep_dt)
    ...
    def _kernel_init_invweight(
        ...
        for i, i_b in ti.ndrange(self.n_links, self._B):
            if self.links_info[i].invweight < 0:
                self.links_info[i].invweight = invweight[i]
    ...
    def _init_dof_fields(self):
        ...
        self.dofs_info = struct_dof_info.field(shape=self.n_dofs_, needs_grad=False, layout=ti.Layout.SOA)
        self.dofs_state = struct_dof_state.field(shape=self._batch_shape(self.n_dofs_), needs_grad=False, layout=ti.Layout.SOA)
    ...
    def _init_link_fields(self):
        ...
        self.links_info = struct_link_info.field(shape=self.n_links, needs_grad=False, layout=ti.Layout.SOA)
        self.links_state = struct_link_state.field(shape=self._batch_shape(self.n_links), needs_grad=False, layout=ti.Layout.SOA)
    ...
    def _init_vert_fields(self):
        ...
        self.verts_state = struct_vert_state.field(shape=self._batch_shape(self.n_verts_), needs_grad=False, layout=ti.Layout.SOA)
    ...
    def _init_vvert_fields(self):
        ...
        if self.n_vverts > 0:
            ...
            self._kernel_init_vvert_fields(
                ...
                vverts_vgeom_idx = np.concatenate([np.full(vgeom.n_vverts, vgeom.idx) for vgeom in vgeoms], dtype=gs.np_int),
            )
    ...
    def _kernel_init_vert_fields(
        ...
        for i in range(self.n_edges):
            ...
            self.edges_info[i].length = (self.verts_info[edges[i, 0]].init_pos - self.verts_info[edges[i, 1]].init_pos).norm()
    ...
    def _init_geom_fields(self):
        ...
        self.geoms_init_AABB = ti.Vector.field(3, dtype=gs.ti_float, shape=(self.n_geoms_, 8)) # stores 8 corners of AABB
        self.geoms_state = struct_geom_state.field(shape=self._batch_shape(self.n_geoms_), needs_grad=False, layout=ti.Layout.SOA)
    ...
    def _init_vgeom_fields(self):
        ...
        self.vgeoms_state = struct_vgeom_state.field(shape=self._batch_shape(self.n_vgeoms_), needs_grad=False, layout=ti.Layout.SOA)
    ...
    def _init_entity_fields(self):
        ...
        self.entities_state = struct_entity_state.field(shape=self._batch_shape(self.n_entities), needs_grad=False, layout=ti.Layout.SOA)
        ...
        self._kernel_init_entity_fields(
            ...
            entities_gravity_compensation   = np.array([entity.gravity_compensation for entity in entities], dtype=gs.np_float),
        )
            ...
            for i_b in range(self._B):
                for i_e_ in range(self.n_awake_entities[i_b]):
                    ...
                    for i in range(self.entities_info[i_e].n_links):
                        ...
                        if i_p != -1:
                            self.links_state[i_p, i_b].crb_inertial = self.links_state[i_p, i_b].crb_inertial + self.links_state[i_l, i_b].crb_inertial
                            self.links_state[i_p, i_b].crb_mass = self.links_state[i_p, i_b].crb_mass + self.links_state[i_l, i_b].crb_mass

                            self.links_state[i_p, i_b].crb_pos = self.links_state[i_p, i_b].crb_pos + self.links_state[i_l, i_b].crb_pos
                            self.links_state[i_p, i_b].crb_quat = self.links_state[i_p, i_b].crb_quat + self.links_state[i_l, i_b].crb_quat
                ...
                for i_e_ in range(self.n_awake_entities[i_b]):
                    ...
                    for i_d in range(e_info.dof_start, e_info.dof_end):
                        for j_d in range(e_info.dof_start, e_info.dof_end):
                            self.mass_mat[i_d, j_d, i_b] = (self.dofs_state[i_d, i_b].f_ang.dot(self.dofs_state[j_d, i_b].cdof_ang) \
                            + self.dofs_state[i_d, i_b].f_vel.dot(self.dofs_state[j_d, i_b].cdof_vel)) * self.mass_parent_mask[i_d, j_d]

                    for i_d in range(e_info.dof_start, e_info.dof_end):
                        self.mass_mat[i_d, i_d, i_b] = self.mass_mat[i_d, i_d, i_b] + self.dofs_info[i_d].armature + self.dofs_info[i_d].damping * self._substep_dt
    ...
    def substep(self):
        from genesis.utils.tools import create_timer; timer = create_timer('rigid', level=1, ti_sync=True, skip_first_call=True)
        self._kernel_step_1()
    ...
    def _func_constraint_force(self):
        from genesis.utils.tools import create_timer; timer = create_timer(name='constraint_force', level=2, ti_sync=True, skip_first_call=True)
    ...
    def _process_dim(self, tensor, envs_idx=None):
        if self.n_envs == 0:
            if tensor.ndim == 1:
                ...
                gs.raise_exception(f'Invalid input shape: {tensor.shape}. Expecting a 1D tensor for non-parallelized scene.')
        else:
            if tensor.ndim == 2:
                if envs_idx is not None:
                    if tensor.shape[0] != len(envs_idx):
                        gs.raise_exception(f'Invalid input shape: {tensor.shape}. 1st dimension of input does not match `envs_idx`.')
                else:
                    if tensor.shape[0] != self.n_envs:
                        gs.raise_exception(f'Invalid input shape: {tensor.shape}. 1st dimension of input does not match `scene.n_envs`.')
            else:
                gs.raise_exception(f'Invalid input shape: {tensor.shape}. Expecting a 2D tensor for scene with parallelized envs.')
        return tensor
        ...
        if ti.static(self._use_hibernation):
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    self.links_state[i_l, i_b].mass_sum = 0.
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    self.links_state[i_l, i_b].i_pos, self.links_state[i_l, i_b].i_quat = gu.ti_transform_pos_quat_by_trans_quat(
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    i_r = self.links_info[i_l].root_idx
                    if i_l == i_r:
                        self.links_state[i_l, i_b].root_COM = self.links_state[i_l, i_b].root_COM / self.links_state[i_l, i_b].mass_sum
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    self.links_state[i_l, i_b].cinr_inertial, self.links_state[i_l, i_b].cinr_pos, \
                        self.links_state[i_l, i_b].cinr_quat, self.links_state[i_l, i_b].cinr_mass = \
                            gu.ti_transform_inertia_by_trans_quat(i_inertial, i_mass, self.links_state[i_l, i_b].i_pos, self.links_state[i_l, i_b].i_quat)
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    if l_info.joint_type == gs.JOINT_TYPE.FREE or (l_info.is_fixed and i_p == -1):
                        ...
                        self.links_state[i_l, i_b].j_pos, self.links_state[i_l, i_b].j_quat = \
                            gu.ti_transform_pos_quat_by_trans_quat(
                                l_info.pos, l_info.quat, p_pos, p_quat)

                        self.links_state[i_l, i_b].j_pos, self.links_state[i_l, i_b].j_quat = \
                            gu.ti_transform_pos_quat_by_trans_quat(
                                l_info.joint_pos, l_info.joint_quat, self.links_state[i_l, i_b].j_pos, self.links_state[i_l, i_b].j_quat)
                ...
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    if l_info.joint_type == gs.JOINT_TYPE.FREE:
                        for i_d in range(l_info.dof_start, l_info.dof_end):
                            ...
                            self.dofs_state[i_d, i_b].cdof_ang, self.dofs_state[i_d, i_b].cdof_vel = \
                                gu.ti_transform_motion_by_trans_quat(
                                    self.dofs_state[i_d, i_b].cdof_ang,
                                    self.dofs_state[i_d, i_b].cdof_vel,
                                    offset_pos, gu.ti_identity_quat()
                                )

                            self.dofs_state[i_d, i_b].cdofvel_ang = self.dofs_state[i_d, i_b].cdof_ang * self.dofs_state[i_d, i_b].vel
                            self.dofs_state[i_d, i_b].cdofvel_vel = self.dofs_state[i_d, i_b].cdof_vel * self.dofs_state[i_d, i_b].vel
    ...
    def _func_forward_kinematics_entity(self, i_e, i_b):
        ...
        for i_l in range(self.entities_info[i_e].link_start, self.entities_info[i_e].link_end):
            ...
            if l_info.joint_type == gs.JOINT_TYPE.FREE:
                ...
                dof_info = self.dofs_info[l_info.dof_start]
                self.links_state[i_l, i_b].j_pos = dof_info.motion_vel * self.qpos[l_info.q_start, i_b]
                self.links_state[i_l, i_b].j_quat = gu.ti_rotvec_to_quat(dof_info.motion_ang * self.qpos[l_info.q_start, i_b])
                ...
                for i_d in range(l_info.dof_start + 1, l_info.dof_end):
                    ...
                    self.links_state[i_l, i_b].j_ang = self.links_state[i_l, i_b].j_ang + \
                        gu.ti_transform_by_quat(ji_ang, ji_quat)

                    self.links_state[i_l, i_b].j_vel = self.links_state[i_l, i_b].j_vel + \
                        gu.ti_transform_by_quat(
                            ji_vel + ji_pos.cross(ji_ang),
                            ji_quat
                        )
            ...
            if l_info.joint_type != gs.JOINT_TYPE.FREE:
                anchor_pos, anchor_quat = gu.ti_transform_pos_quat_by_trans_quat(
                    l_info.joint_pos, l_info.joint_quat, ti.Vector.zero(gs.ti_float, 3), self.links_state[i_l, i_b].j_quat)

                self.links_state[i_l, i_b].j_pos = self.links_state[i_l, i_b].j_pos + l_info.joint_pos - anchor_pos
                self.links_state[i_l, i_b].j_pos, self.links_state[i_l, i_b].j_quat = gu.ti_transform_pos_quat_by_trans_quat(
        ...
        for i_l in range(self.entities_info[i_e].link_start, self.entities_info[i_e].link_end):
            ...
            if i_p == -1: # root link
                ...
                self.links_state[i_l, i_b].ang = p.ang + gu.ti_transform_by_quat(self.links_state[i_l, i_b].j_ang, self.links_state[i_l, i_b].quat)
                self.links_state[i_l, i_b].vel = p.vel + p.ang.cross(self.links_state[i_l, i_b].pos - p.pos) \
            ...
            for i_b in range(self._B):
                for i_e_ in range(self.n_awake_entities[i_b]):
                    ...
                    for i_g in range(e_info.geom_start, e_info.geom_end):
                        ...
                        self.geoms_state[i_g, i_b].pos, self.geoms_state[i_g, i_b].quat = \
    ...
    def _func_update_verts_for_geom(self, i_g, i_b):
        ...
        if not g_state.verts_updated:
            ...
            for i_v in range(g_info.vert_start, g_info.vert_end):
                self.verts_state[i_v, i_b].pos = gu.ti_transform_by_trans_quat(self.verts_info[i_v].init_pos, g_state.pos, g_state.quat)
    ...
    def _func_update_geom_aabbs(self):
        ...
        for i_g, i_b in ti.ndrange(self.n_geoms, self._B):
            ...
            for i_corner in range(8):
                corner_pos = gu.ti_transform_by_trans_quat(self.geoms_init_AABB[i_g, i_corner], g_state.pos, g_state.quat)
    ...
    def _kernel_update_vgeoms(self):
        ...
        for i_g, i_b in ti.ndrange(self.n_vgeoms, self._B):
            ...
            self.vgeoms_state[i_g, i_b].pos, self.vgeoms_state[i_g, i_b].quat = \
                gu.ti_transform_pos_quat_by_trans_quat(g_info.pos, g_info.quat, l.pos, l.quat)

    @ti.func
    def _func_hibernate(self):
        ti.loop_config(serialize = self._para_level < gs.PARA_LEVEL.PARTIAL)
        for i_e, i_b in ti.ndrange(self.n_entities, self._B):
            if not self.entities_state[i_e, i_b].hibernated and self.entities_info[i_e].n_dofs > 0: # We do not hibernate fixed entity
                hibernate = True
                for i_d in range(self.entities_info[i_e].dof_start, self.entities_info[i_e].dof_end):
                    if ti.abs(self.dofs_state[i_d, i_b].acc) > self._hibernation_thresh_acc or ti.abs(self.dofs_state[i_d, i_b].vel) > self._hibernation_thresh_vel:
                        hibernate = False
                        break
                ...
                if hibernate:
                    ...
                    for i_g in range(self.entities_info[i_e].geom_start, self.entities_info[i_e].geom_end):
                        self.collider.sort_buffer[self.geoms_state[i_g, i_b].min_buffer_idx, i_b].value = self.geoms_state[i_g, i_b].aabb_min[0]
                        self.collider.sort_buffer[self.geoms_state[i_g, i_b].max_buffer_idx, i_b].value = self.geoms_state[i_g, i_b].aabb_max[0]
    ...
    def _func_apply_external_force_link_frame(self, pos, force, link_idx, batch_idx):
        pos = gu.ti_transform_by_trans_quat(pos, self.links_state[link_idx, batch_idx].pos, self.links_state[link_idx, batch_idx].quat)
    ...
    def _func_apply_external_force_link_inertial_frame(self, pos, force, link_idx, batch_idx):
        pos = gu.ti_transform_by_trans_quat(pos, self.links_info[link_idx].inertial_pos, self.links_info[link_idx].inertial_quat)
        force = gu.ti_transform_by_quat(force, self.links_info[link_idx].inertial_quat)
    ...
    def _func_apply_external_torque_link_inertial_frame(self, torque, link_idx, batch_idx):
        torque = gu.ti_transform_by_quat(torque, self.links_info[link_idx].inertial_quat)
        ...
        for i_e, i_b in ti.ndrange(self.n_entities, self._B):
            ...
            for i_l in range(self.entities_info[i_e].link_start, self.entities_info[i_e].link_end):
                ...
                for i_d in range(l_info.dof_start, l_info.dof_end):
                    ...
                    elif self.dofs_state[i_d, i_b].ctrl_mode == gs.CTRL_MODE.POSITION and\
                        not (l_info.joint_type == gs.JOINT_TYPE.FREE and i_d >= l_info.dof_start + 3):
                        force = (
                ...
                if l_info.joint_type == gs.JOINT_TYPE.FREE and \
                    (self.dofs_state[ds+3, i_b].ctrl_mode == gs.CTRL_MODE.POSITION or \
                        self.dofs_state[ds+4, i_b].ctrl_mode == gs.CTRL_MODE.POSITION or \
                        self.dofs_state[ds+5, i_b].ctrl_mode == gs.CTRL_MODE.POSITION):
                        ...
                        for i_d in range(l_info.dof_start+3, l_info.dof_end):
                            force = (self.dofs_info[i_d].kp * rotvec[i_d-l_info.dof_start-3]
                                - self.dofs_info[i_d].kv * self.dofs_state[i_d, i_b].vel)
        ...
        if ti.static(self._use_hibernation):
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    i_l = self.awake_links[i_l_, i_b]
                    l_info = self.links_info[i_l]
                    if l_info.joint_type == gs.JOINT_TYPE.REVOLUTE or \
                    l_info.joint_type == gs.JOINT_TYPE.PRISMATIC or \
                    l_info.joint_type == gs.JOINT_TYPE.PLANAR or \
                    l_info.joint_type == gs.JOINT_TYPE.SPHERICAL:
                        dof_start = l_info.dof_start
                        q_start = l_info.q_start
                        q_end = l_info.q_end

                        for j_d in range(q_end - q_start):
                            self.dofs_state[dof_start+j_d, i_b].qf_passive = - self.qpos[q_start+j_d, i_b] * self.dofs_info[dof_start+j_d].stiffness
    ...
    def _func_system_update_force(self):
        if ti.static(self._use_hibernation):
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    i_l = self.awake_links[i_l_, i_b]
                    f1_ang, f1_vel = gu.inertial_mul(self.links_state[i_l, i_b].cinr_pos, self.links_state[i_l, i_b].cinr_inertial, self.links_state[i_l, i_b].cinr_mass, self.links_state[i_l, i_b].cdd_vel, self.links_state[i_l, i_b].cdd_ang)
                    ...
                    f2_ang, f2_vel = gu.motion_cross_force(self.links_state[i_l, i_b].cd_ang, self.links_state[i_l, i_b].cd_vel, f2_ang, f2_vel)
            ...
            for i_b in range(self._B):
                for i_e_ in range(self.n_awake_entities[i_b]):
                    ...
                    for i in range(e_info.n_links):
                        ...
                        i_p = self.links_info[i_l].parent_idx
                        if i_p != -1:
                            self.links_state[i_p, i_b].cfrc_flat_vel = self.links_state[i_p, i_b].cfrc_flat_vel + self.links_state[i_l, i_b].cfrc_flat_vel
                            self.links_state[i_p, i_b].cfrc_flat_ang = self.links_state[i_p, i_b].cfrc_flat_ang + self.links_state[i_l, i_b].cfrc_flat_ang
        ...
        if ti.static(self._use_hibernation):
            ...
            for i_l, i_b in ti.ndrange(self.n_links, self._B):
                ...
                if joint_type == gs.JOINT_TYPE.REVOLUTE or joint_type == gs.JOINT_TYPE.PRISMATIC:
                    ...
                    for i_d in range(self.links_info[i_l].dof_start, self.links_info[i_l].dof_end):
                        self.dofs_state[i_d, i_b].act_length = 0.
                        self.dofs_state[i_d, i_b].qf_actuator = self.dofs_state[i_d, i_b].act_length
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    for i_d in range(dof_start, dof_end):
                        ...
                        self.dofs_state[i_d, i_b].cdof_ang.dot(self.links_state[i_l, i_b].cfrc_flat_ang) + \
                        self.dofs_state[i_d, i_b].cdof_vel.dot(self.links_state[i_l, i_b].cfrc_flat_vel)

                        self.dofs_state[i_d, i_b].force = self.dofs_state[i_d, i_b].qf_passive \
                            - self.dofs_state[i_d, i_b].qf_bias \
            ...
            for i_b in range(self._B):
                for i_l_ in range(self.n_awake_links[i_b]):
                    ...
                    if joint_type == gs.JOINT_TYPE.FREE:
                        rot = ti.Vector([self.qpos[q_start+3, i_b], self.qpos[q_start+4, i_b], self.qpos[q_start+5, i_b], self.qpos[q_start+6, i_b]])
                        ang = ti.Vector([self.dofs_state[dof_start+3, i_b].vel, self.dofs_state[dof_start+4, i_b].vel, self.dofs_state[dof_start+5, i_b].vel]) * self._substep_dt
                        ...
                        vel = ti.Vector([self.dofs_state[dof_start, i_b].vel, self.dofs_state[dof_start+1, i_b].vel, self.dofs_state[dof_start+2, i_b].vel])
                        ...
                        for j in range(q_end - q_start):
                            self.qpos[q_start+j, i_b] = self.qpos[q_start+j, i_b] + self.dofs_state[dof_start+j, i_b].vel * self._substep_dt
    ...
    def _func_integrate_dq_entity(self, dq, i_e, i_b, respect_joint_limit):
        ...
        for i_l in range(e_info.link_start, e_info.link_end):
            ...
            if l_info.joint_type == gs.JOINT_TYPE.FREE:
                ...
                quat = ti.Vector([self.qpos[q_start+3, i_b], self.qpos[q_start+4, i_b], self.qpos[q_start+5, i_b], self.qpos[q_start+6, i_b]])
                dquat = gu.ti_rotvec_to_quat(ti.Vector([dq[dq_start+3, i_b], dq[dq_start+4, i_b], dq[dq_start+5, i_b]]))
                quat = gu.ti_transform_quat_by_quat(quat, dquat) # Note that this order is different from integrateing vel. Here dq is w.r.t to world.
                ...
                for i_d_ in range(l_info.n_dofs):
                    ...
                    if respect_joint_limit:
                        self.qpos[q_start+i_d_, i_b] = ti.math.clamp(self.qpos[q_start+i_d_, i_b], self.dofs_info[dof_start+i_d_].limit[0], self.dofs_info[dof_start+i_d_].limit[1])
    ...
    @ti.kernel
    def _kernel_get_state(self, qpos: ti.types.ndarray(), vel: ti.types.ndarray(), links_pos: ti.types.ndarray(), links_quat: ti.types.ndarray(), i_pos_shift: ti.types.ndarray(), mass_shift: ti.types.ndarray(), friction_ratio: ti.types.ndarray()):
        ti.loop_config(serialize = self._para_level < gs.PARA_LEVEL.ALL)
    ...
    @ti.kernel
    def _kernel_set_state(self, qpos: ti.types.ndarray(), dofs_vel: ti.types.ndarray(), links_pos: ti.types.ndarray(), links_quat: ti.types.ndarray(), i_pos_shift: ti.types.ndarray(), mass_shift: ti.types.ndarray(), friction_ratio: ti.types.ndarray()):
        ti.loop_config(serialize = self._para_level < gs.PARA_LEVEL.ALL)
        for i_q, i_b in ti.ndrange(self.n_qs, self._B):
    ...
    def get_state(self, f):
        if self.is_active():
            ...
            self._kernel_get_state(state.qpos, state.dofs_vel, state.links_pos, state.links_quat, state.i_pos_shift, state.mass_shift, state.friction_ratio)
        else:
            state = None
        return state
    ...
    def set_state(self, f, state):
        if self.is_active():
            self._kernel_set_state(state.qpos, state.dofs_vel, state.links_pos, state.links_quat, state.i_pos_shift, state.mass_shift, state.friction_ratio)
            self._kernel_forward_kinematics_links_geoms()
            self.collider.reset()
    ...
    def _validate_1D_io_variables(self, tensor, inputs_idx, envs_idx=None, batched=True, idx_name='dofs_idx'):
        ...
        if batched:
            ...
            if self.n_envs == 0:
                ...
                if tensor.ndim == 2:
                    if tensor.shape[0] != len(envs_idx):
                        gs.raise_exception(f'Invalid input shape: {tensor.shape}. First dimension of the input tensor does not match length of `envs_idx` (or `scene.n_envs` if `envs_idx` is None).')
                else:
    ...
    def _validate_2D_io_variables(self, tensor, inputs_idx, vec_size, envs_idx=None, batched=True, idx_name='links_idx'):
        inputs_idx = torch.as_tensor(inputs_idx, dtype=gs.tc_int, device=gs.device).contiguous()
        if inputs_idx.ndim != 1:
            gs.raise_exception(f'Expecting 1D tensor for `{idx_name}`.')
        ...
        if tensor is not None:
            ...
            if batched and self.n_envs > 0:
                if envs_idx is not None:
                    ...
                    B = self.n_envs
                tensor = torch.empty(self._batch_shape((len(inputs_idx), vec_size), True, B=B), dtype=gs.tc_float, device=gs.device)
            else:
        ...
        if batched:
            ...
            if self.n_envs == 0:
                if tensor.ndim == 2:
                    ...
                    gs.raise_exception(f'Invalid input shape: {tensor.shape}. Expecting a 2D tensor for non-parallelized scene.')
                ...
                if tensor.ndim == 3:
                    ...
                    gs.raise_exception(f'Invalid input shape: {tensor.shape}. Expecting a 3D tensor for scene with parallelized envs.')
    ...
    def set_geoms_friction_ratio(self, friction_ratio, geoms_idx, envs_idx=None):
        friction_ratio, geoms_idx, envs_idx = self._validate_1D_io_variables(friction_ratio, geoms_idx, envs_idx, idx_name='geoms_idx')
        ...
        for b in range(self._B):
            ...
            for i in range(n_propellers):
                ...
                self._func_apply_external_force_link_inertial_frame(
                    ...
                    ti.Vector([0.0, 0.0, force_i]),
                    propellers_link_idxs[i],
                    b
                )
            ...
            self._func_apply_external_torque_link_inertial_frame(
                ti.Vector([0.0, 0.0, torque]),
                COM_link_idx,
                b
            )
