...
class SDF:
    def __init__(self, rigid_solver):
        ...
        if self.solver.n_geoms > 0:
            ...
            self._kernel_init_geom_fields(
                ...
                geoms_sdf_closest_vert = np.concatenate([geom.sdf_closest_vert_flattened for geom in geoms], dtype=gs.np_int),
            )
        ...
        if self.solver.geoms_info[geom_idx].type == gs.GEOM_TYPE.TERRAIN: # Terrain uses finite difference
            if ti.static(self.solver.collider._has_terrain): # for speed up compilation
                ...
                for i in ti.static(range(3)):
                    ...
                    inc[i] += delta
                    dec[i] -= delta
                    sdf_grad_sdf[i] = (self.true_sdf_sdf(inc, geom_idx) - self.true_sdf_sdf(dec, geom_idx)) / (2 * delta)
            ...
            for offset in ti.grouped(ti.ndrange(2, 2, 2)):
                ...
                sdf_grad_sdf = (
                    ...
                    + w
                    * self.geoms_sdf_grad[
                        self.ravel_cell_idx(pos_cell, geom_sdf_res, geom_idx)
                    ]
                )
    ...
    def sdf_world(self, pos_world, geom_idx, batch_idx):
        ...
        if self.solver.geoms_info[geom_idx].type == gs.GEOM_TYPE.SPHERE:
            ...
            pos_mesh = gu.ti_inv_transform_by_trans_quat(
                pos_world, g_state.pos, g_state.quat
            )
            ...
            pos_sdf = gu.ti_transform_by_T(
                pos_mesh, self.geoms_info[geom_idx].T_mesh_to_sdf
            )
    ...
    def _func_find_closest_vert(self, pos_world, geom_idx, i_b):
        ...
        nearest_cell = ti.cast(
            ti.min(ti.max(pos_sdf, 0), geom_sdf_res - 1), gs.ti_int
        )
        return (
            self.geoms_sdf_closest_vert[
                self.ravel_cell_idx(nearest_cell, geom_sdf_res, geom_idx)
            ]
