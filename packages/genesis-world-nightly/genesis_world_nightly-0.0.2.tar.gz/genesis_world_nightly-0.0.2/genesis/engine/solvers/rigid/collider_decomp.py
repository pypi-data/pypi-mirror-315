        ...
        if not is_return:
            self._solver.geoms_state[i_ga, i_b].pos, self._solver.geoms_state[i_ga, i_b].quat = \
                gu.ti_transform_pos_quat_by_trans_quat(
                    ga_pos - self._solver.geoms_state[i_gb, i_b].pos, ga_quat,
                    ti.Vector.zero(gs.ti_float, 3), gu.ti_inv_quat(self._solver.geoms_state[i_gb, i_b].quat)
                )
            ...
            for i in range(3):
                ...
                if self._solver.terrain_xyz_maxmin[i] < self.xyz_max_min[i+3, i_b] - margin or \
                    self._solver.terrain_xyz_maxmin[i+3] > self.xyz_max_min[i, i_b] + margin:
                    is_return = True
                        ...
                        for i in range(2):
                            if cnt < self._n_contacts_per_pair:
                                ...
                                self.add_prism_vert(
                                    ...
                                    self._solver.terrain_hf[r+i, c]+margin, i_b)
                                if nvert > 2 and \
                                    (self.prism[3, i_b][2] >= self.xyz_max_min[5, i_b] or \
                                    self.prism[4, i_b][2] >= self.xyz_max_min[5, i_b] or \
                                    self.prism[5, i_b][2] >= self.xyz_max_min[5, i_b]):
                                    ...
                                    if is_col:
                                        ...
                                        valid = True
                                        for j in range(cnt):
                                            if (contact_pos - self.contact_data[i_col - j - 1, i_b].pos).norm() < tolerance:
                                                valid = False
                                                break
    ...
    def _func_narrow_phase_terrain(self):
        ...
        for i_b in range(self._solver._B):
            for i_pair in range(self.n_broad_pairs[i_b]):
                ...
                if self._solver.geoms_info[i_ga].type == gs.GEOM_TYPE.TERRAIN or self._solver.geoms_info[i_gb].type == gs.GEOM_TYPE.TERRAIN:
                        ...
                        for i in range(2):
                            ...
                            if is_col_0:
                                ...
                                for i_rot in range(4):
                                    ...
                                    is_col, normal, penetration, contact_pos = self._func_contact_vertex_sdf(i_ga, i_gb, i_b, gu.ti_identity_quat())
        ...
        if is_col:
            ...
            if multi_contact:
                ...
                if self._solver.geoms_info[i_gb].type != gs.GEOM_TYPE.BOX:
                    neighbor_start, neighbor_end = self.vert_neighbor_start[i_v], self.vert_neighbor_start[i_v] + self.vert_n_neighbors[i_v]
