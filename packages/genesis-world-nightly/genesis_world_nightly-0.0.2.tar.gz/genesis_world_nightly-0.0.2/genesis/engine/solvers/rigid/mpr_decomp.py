...
class MPR:
    ...
    def init_support(self):
        ...
        self.simplex_support = struct_support.field(
            shape=self._solver._batch_shape(
                (self._solver.n_geoms_, self._solver.n_geoms_, 4)
            ),
            layout=ti.Layout.SOA,
        )
        self.simplex_size = ti.field(gs.ti_int, 
            shape=self._solver._batch_shape(
                (self._solver.n_geoms_, self._solver.n_geoms_)
            )
        )
    ...
    def mpr_swap(self, i, j, i_ga, i_gb, i_b):
        self.simplex_support[i_ga, i_gb, i, i_b].v1, self.simplex_support[i_ga, i_gb, j, i_b].v1 = \
            self.simplex_support[i_ga, i_gb, j, i_b].v1, self.simplex_support[i_ga, i_gb, i, i_b].v1
        self.simplex_support[i_ga, i_gb, i, i_b].v2, self.simplex_support[i_ga, i_gb, j, i_b].v2 = \
            self.simplex_support[i_ga, i_gb, j, i_b].v2, self.simplex_support[i_ga, i_gb, i, i_b].v2
        self.simplex_support[i_ga, i_gb, i, i_b].v, self.simplex_support[i_ga, i_gb, j, i_b].v = \
            self.simplex_support[i_ga, i_gb, j, i_b].v, self.simplex_support[i_ga, i_gb, i, i_b].v
    ...
    def mpr_point_tri_depth(self, P, x0, B, C):
        ...
        d = w * v - r * r
        dist = s = t = gs.ti_float(0.)
        pdir = gs.ti_vec3([0., 0., 0.])
        if ti.abs(d) < gs.EPS:
            s = t = -1.0
        else:
            s = (q * r - w * p) / d
        ...
        if  (ti.abs(s) < gs.EPS or s > 0.) and \
            (ti.abs(s - 1.0) < gs.EPS or s < 1.0) and \
            (ti.abs(t) < gs.EPS or t > 0.0) and \
            (ti.abs(t - 1.0) < gs.EPS or t < 1.0) and \
            (ti.abs(t + s - 1.0) < gs.EPS or t + s < 1.0):
            pdir = x0 + d1 * s + d2 * t
            dist = (P - pdir).norm() ** 2
        else:
    ...
    def support_box(self, direction, i_g, i_b):
        ...
        d_box = gu.ti_transform_by_quat(
            direction, gu.ti_inv_quat(g_state.quat))
    ...
    def func_geom_support(self, direction, i_g, i_b):
        g_state = self._solver.geoms_state[i_g, i_b]
        direction_in_init_frame = gu.ti_transform_by_quat(
            direction, gu.ti_inv_quat(g_state.quat))
        ...
        for i_v in range(g_info.vert_start, g_info.vert_end):
            ...
            if dot > dot_max:
                ...
                dot_max = dot
                vid = i_v
        v_ = gu.ti_transform_by_trans_quat(
            v, g_state.pos, g_state.quat)
    ...
    def mpr_refine_portal(self, i_ga, i_gb, i_b):
        ...
        while True:
            ...
            if not self.mpr_portal_can_encapsule_origin(v, direction) or \
                self.mpr_portal_reach_tolerance(v, direction, i_ga, i_gb, i_b):
                ret = -1
                break
    ...
    def mpr_find_pos(self, i_ga, i_gb, i_b):
        b = ti.Vector([0., 0., 0., 0.], dt=gs.ti_float)
    ...
    def mpr_find_penetr_touch(self, i_ga, i_gb, i_b):
        ...
        normal = gs.ti_vec3([1., 0., 0.]) 
        pos = (self.simplex_support[i_ga, i_gb, 1, i_b].v1 + 
                self.simplex_support[i_ga, i_gb, 1, i_b].v2) * 0.5
    ...
    def mpr_find_penetr_segment(self, i_ga, i_gb, i_b):
        ...
        normal = self.simplex_support[i_ga, i_gb, 1, i_b].v * -1.
        pos = (self.simplex_support[i_ga, i_gb, 1, i_b].v1 + 
                self.simplex_support[i_ga, i_gb, 1, i_b].v2) * 0.5
    ...
    def mpr_find_penetration(self, i_ga, i_gb, i_b):
        ...
        while True:
            direction = self.mpr_portal_dir(i_ga, i_gb, i_b)
            v, v1, v2 = self.compute_support(direction, i_ga, i_gb, i_b)
            if self.mpr_portal_reach_tolerance(v, direction, i_ga, i_gb, i_b) or \
    ...
    def mpr_discover_portal(self, i_ga, i_gb, i_b):
        ...
        center_a = \
            gu.ti_transform_by_trans_quat(
                g_info.center, g_state.pos, g_state.quat)
        ...
        if self.simplex_support[i_ga, i_gb, 0, i_b].v.norm() < CCD_EPS:
            self.simplex_support[i_ga, i_gb, 0, i_b].v += gs.ti_vec3([10. * CCD_EPS, 0., 0.])
        ...
        if dot < CCD_EPS:
            ...
            direction = self.simplex_support[i_ga, i_gb, 0, i_b].v.cross(
                self.simplex_support[i_ga, i_gb, 1, i_b].v)
