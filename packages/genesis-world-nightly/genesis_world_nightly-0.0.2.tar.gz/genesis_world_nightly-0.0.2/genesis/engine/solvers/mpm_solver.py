...
class MPMSolver(Solver):
    ...
    def init_grid_fields(self):
        ...
        if self._use_sparse_grid:
            ...
            self.grid = grid_cell_state.field(shape=(self._sim.substeps_local+1, *self._grid_res), needs_grad=True, layout=ti.Layout.SOA)
    ...
    def build(self):
        ...
        if self.is_active():
            if self._enable_CPIC:
                gs.logger.warning('Kernel compilation takes longer when running MPM solver in CPIC mode. Please be patient.')
                if self._sim.requires_grad:
                    gs.raise_exception('CPIC is not supported in differentiable mode yet. Submit a feature request if you need it.')
            ...
            if self.substep_dt > suggested_dt:
                gs.logger.warning(f'Current `substep_dt` ({self.substep_dt:.6g}) is greater than suggested_dt ({suggested_dt:.6g}, calculated based on `grid_density`). Simulation might be unstable.')
    ...
    def compute_F_tmp(self, f: ti.i32):
        for i in range(self._n_particles):
            if self.particles_ng[f, i].active:
                self.particles[f, i].F_tmp = (ti.Matrix.identity(gs.ti_float, 3) + self.substep_dt * self.particles[f, i].C) @ self.particles[f, i].F

    @ti.kernel
    def svd(self, f: ti.i32):
        for i in range(self._n_particles):
            if self.particles_ng[f, i].active:
                self.particles[f, i].U, self.particles[f, i].S, self.particles[f, i].V = ti.svd(self.particles[f, i].F_tmp, gs.ti_float)

    @ti.kernel
    def svd_grad(self, f: ti.i32):
        for i in range(self._n_particles):
            if self.particles_ng[f, i].active:
                self.particles.grad[f, i].F_tmp += self.backward_svd(self.particles.grad[f, i].U, self.particles.grad[f, i].S, self.particles.grad[f, i].V, self.particles[f, i].U, self.particles[f, i].S, self.particles[f, i].V)
            ...
            if self.particles_ng[f, i].active:
                ...
                for offset in ti.static(ti.grouped(self.stencil_range())):
                    ...
                    if ti.static(self._enable_CPIC):
                        ...
                        if sep_geom_idx == -1:
                            self.grid[f, base - self._grid_offset + offset].vel_in += weight * (self.particles_info[i].mass * self.particles[f, i].vel + affine @ dpos)
        ...
        for i in range(self._n_particles):
            if self.particles_ng[f, i].active:
                ...
                for offset in ti.static(ti.grouped(self.stencil_range())):
                    ...
                    if ti.static(self._enable_CPIC):
                        sep_geom_idx = self._coupler.cpic_flag[
    ...
    def _kernel_add_particles(
        ...
        for i in range(n_particles):
            ...
            self.particles_info[i_global].muscle_direction = ti.Vector([0., 0., 1.], dt=gs.ti_float)
    ...
    def _kernel_update_render_fields(self, f: ti.i32):
        ...
        for i in range(self._n_vverts):
            ...
            for j in range(self._n_vvert_supports):
                vvert_pos += self.particles[f, self.vverts_info.support_idxs[i][j]].pos * self.vverts_info.support_weights[i][j]
