...
class RigidLink(RBC):
        ...
        if self._inertial_i is None:
            if self._init_mesh is None: # use sphere inertia with radius 0.1
                ...
                if inertia_mesh.is_watertight and self._init_mesh.mass > 0:
                    ...
                    self._inertial_i = self._init_mesh.moment_inertia_frame(T_inertia) / self._init_mesh.mass * self._inertial_mass

                else: # approximate with a sphere
                    self._inertial_i = 0.4 * self._inertial_mass * (max(self._init_mesh.bounds[1] - self._init_mesh.bounds[0]) / 2.0)**2 * np.eye(3)
    ...
    def _add_geom(self, mesh, init_pos, init_quat, type, friction, sol_params, center_init=None, needs_coup=False, data=None):
        geom = RigidGeom(
            link        = self,
    ...
    def _kernel_get_vverts(self, tensor: ti.types.ndarray()):
        for i_vg_, i_b in ti.ndrange(self.n_vgeoms, self._solver._B):
            ...
            for i_v in range(g_info.vvert_start, g_info.vvert_end):
                vvert_pos = gu.ti_transform_by_trans_quat(self._solver.vverts_info[i_v].init_pos, g_state.pos, g_state.quat)
                for j in range(3):
