...
class ToolEntity(Entity):
    ...
    @ti.kernel
    def save_ckpt_kernel(self, pos: ti.types.ndarray(), quat: ti.types.ndarray(), vel: ti.types.ndarray(), ang: ti.types.ndarray()):
    ...
    @ti.kernel
    def load_ckpt_kernel(self, pos: ti.types.ndarray(), quat: ti.types.ndarray(), vel: ti.types.ndarray(), ang: ti.types.ndarray()):
        for i in ti.static(range(3)):
            self.pos[0][i] = pos[i]
            self.vel[0][i] = vel[i]
            self.ang[0][i] = ang[i]
    ...
    def advect(self, f: ti.i32):
        ...
        self.quat[f+1] = ti_transform_quat_by_quat(self.quat[f], ti_rotvec_to_quat(self.ang[f] * self._solver.substep_dt))
