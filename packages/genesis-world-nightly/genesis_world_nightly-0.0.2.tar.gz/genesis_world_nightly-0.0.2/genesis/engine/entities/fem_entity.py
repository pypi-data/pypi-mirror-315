...
class FEMEntity(Entity):
    ...
    def set_muscle(self, muscle_group=None, muscle_direction=None):
        ...
        if muscle_direction is not None:
            ...
            assert torch.allclose(muscle_direction.norm(dim=-1), torch.Tensor([1.]).to(muscle_direction))
    ...
    def _add_to_solver(self, in_backward=False):
        if not in_backward:
            ...
            gs.logger.info(f'Entity {self.uid} added. class: {self.__class__.__name__}, morph: {self.morph.__class__.__name__}, size: ({self.n_elements}, {self.n_vertices}), material: {self.material}.')

        el2tri = np.array([ # follow the order with correct normal
            [[v[0], v[2], v[1]],
             [v[1], v[2], v[3]],
             [v[0], v[1], v[3]],
             [v[0], v[3], v[2]]
