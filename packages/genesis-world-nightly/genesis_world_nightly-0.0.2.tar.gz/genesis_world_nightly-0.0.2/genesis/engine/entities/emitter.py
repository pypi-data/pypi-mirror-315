...
class Emitter(RBC):
    def __init__(self, max_particles):
        ...
        gs.logger.info(f"Creating ~<{self._repr_type()}>~. id: ~~~<{self._uid}>~~~, max_particles: ~<{max_particles}>~.")
    ...
    def emit(self, droplet_shape, droplet_size, droplet_length=None, pos=(0.5, 0.5, 1.0), direction=(0, 0, -1), theta=0.0, speed=1.0, p_size=None):
        ...
        if droplet_length > 0.0:
            ...
            if n_particles > self._entity.n_particles:
                gs.logger.warning(f'Number of particles to emit ({n_particles}) at the current step is larger than the maximum number of particles ({self._entity.n_particles}).')
