...
class Camera(RBC):
        ...
        if self._raytracer is not None:
            ...
            if depth or segmentation or normal:
                if self._rasterizer is not None:
                    ...
                    _, depth_arr, seg_idxc_arr, normal_arr = self._rasterizer.render_camera(self, False, depth, segmentation, normal=normal)
                else:
                    gs.raise_exception("Cannot render depth or segmentation image.")

        elif self._rasterizer is not None:
            self._rasterizer.update_scene()
            rgb_arr, depth_arr, seg_idxc_arr, normal_arr = self._rasterizer.render_camera(self, rgb, depth, segmentation, normal=normal)
    ...
    def stop_recording(self, save_to_filename=None, fps=60):
        ...
        if save_to_filename is None:
            ...
            save_to_filename = os.path.splitext(os.path.basename(caller_file))[0] + f'_cam_{self.idx}_{time.strftime("%Y%m%d_%H%M%S")}.mp4'
