...
class Tensor(torch.Tensor):
            ...
            if isinstance(arg, cls):
                ...
                if arg.scene is not None:
                    if scene is None:
                        ...
                        gs.raise_exception(f'Tensors not derived from the same Scene object: Scene {scene.uid} vs Scene {arg.scene.uid}. Consider calling `Tensor.sceneless()` to detach a tensor from its scene if gradient flow back to the scene is not needed.')
    ...
    def assert_sceneless(self):
        if self.scene is not None:
            gs.raise_exception('Tensor not sceneless. If you are using a tensor derived from a scene to set scene configurations back, call Tensor.sceneless() to detach it from the scene.')

    def __repr__(self):
        return super().__repr__().replace('Tensor', 'gs.tensor')[:-1] + \
