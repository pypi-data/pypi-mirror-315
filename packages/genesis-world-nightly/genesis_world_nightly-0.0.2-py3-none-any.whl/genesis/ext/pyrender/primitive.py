    ...
    def _add_to_context(self):
        ...
        if self.stream_positions:
            ...
            glVertexAttribPointer(
                0, 3, GL_FLOAT, GL_FALSE, FLOAT_SZ * 3,
                ctypes.c_void_p(0)
            )
            ...
            if self.normals is not None:
                ...
                normal_data = np.ascontiguousarray(
                    normal_data.flatten().astype(np.float32)
                )
                glBufferData(
                    GL_ARRAY_BUFFER, FLOAT_SZ * len(normal_data),
                    normal_data, GL_STREAM_DRAW
                )

                glVertexAttribPointer(
                    1, 3, GL_FLOAT, GL_FALSE, FLOAT_SZ * 3,
                    ctypes.c_void_p(0)
                )
        ...
        glBufferData(
            ...
            pose_data, GL_STREAM_DRAW if self.stream_poses else GL_STATIC_DRAW
        )
