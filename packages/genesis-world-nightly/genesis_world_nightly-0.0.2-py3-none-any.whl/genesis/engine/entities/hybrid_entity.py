...
class HybridEntity(Entity):
    def __init__(
        ...
        if not material.use_default_coupling:
            ...
            if isinstance(mat_soft, gs.materials.MPM.Base):
                ...
                if self._update_soft_part_at_pre_coupling:
                    part_soft.solver.substep_pre_coupling = wrap_func(part_soft.solver.substep_pre_coupling, before=True)
                else:
                    part_soft.solver.substep_post_coupling = wrap_func(part_soft.solver.substep_post_coupling, before=False)
            else:
                raise ValueError(f"Cannot handle soft material {mat_soft}")
        ...
        assert self._solver_rigid.dt == self._solver_soft.dt, \
            "Rigid and soft solver should have the same dt for now"
    ...
    def _kernel_update_soft_part_mpm(self, f: ti.i32):
        for i in range(self._part_soft.n_particles):
            if self._solver_soft.particles_ng[f, i].active:
                ...
                dt_scale = self._solver_soft.substep_dt / self._solver_rigid.dt # NOTE: move soft part incrementally at soft solver's substeps
                ...
                dt_for_rigid_acc = self._solver_rigid.dt # NOTE: use rigid dt here as we are sorta doing integration within soft solver substep
...
def default_func_instantiate_rigid_from_soft(
    ...
    # add rigid entity
    mesh_center = (mesh.vertices.max(0) + mesh.vertices.min(0)) / 2.
...
def default_func_instantiate_rigid_soft_association_from_soft(
    ...
    for i, link in enumerate(part_rigid.links):
        geom = link.geoms[0]
        link_end_x_pos = gu.transform_by_trans_quat(
            np.zeros((3,)),
            trans=geom.init_pos * 2,
            quat=geom.init_quat,
        ) + link.init_x_pos # TODO: check if this is correct
        ...
        dist_to_link = dist_to_p0 * (positions_proj_on_line_t < 0).astype(float) + \
