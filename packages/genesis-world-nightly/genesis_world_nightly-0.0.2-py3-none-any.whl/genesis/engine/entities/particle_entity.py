...
class ParticleEntity(Entity):
    ...
    '''
    def __init__(self, scene, solver, material, morph, surface, particle_size, idx, particle_start, vvert_start=None, vface_start=None, need_skinning=True):
    ...
    def init_sampler(self):
        ...
        if not valid:
            gs.raise_exception(f'Only one of the following samplers is supported: [`regular`, `random`, `pbs`, `pbs-sdf_res`]. Got: {self.sampler}.')
    ...
    def _add_vverts_to_solver(self):
        ...
        Ps = np.stack([all_ps[:, i, :] for i in range(self.solver._n_vvert_supports - 1)], axis=2) - np.expand_dims(all_ps[:, -1, :], axis=2)
    ...
    def sample(self):
        ...
        if isinstance(self._morph, gs.options.morphs.Nowhere):
            ...
            self.mesh_set_group_ids = np.concatenate([np.ones((v.shape[0],), dtype=gs.np_int) * i for i, v in enumerate(particles)])
            particles = np.concatenate(particles)
            if not self._solver.boundary.is_inside(particles): # HACK no check
                gs.raise_exception(f'Entity has particles outside solver boundary. Note that for MPMSolver, boundary is slightly tighter than the specified domain due to safety padding.\n\nCurrent boundary:\n{self._solver.boundary}\n\nEntity to be added:\nmin: {particles.min(0)}\nmax: {particles.max(0)}\n')
    ...
    def set_velocity(self, vel):
        ...
        if self.sim.requires_grad:
            gs.logger.warning('Manually setting particle velocities. This is not recommended and could break gradient flow.')
    ...
    def set_position(self, pos):
        ...
        self._assert_active()
        if self.sim.requires_grad:
            gs.logger.warning('Manually setting particle positions. This is not recommended and could break gradient flow. This also resets particle stress and velocities.')
