...
class RigidEntity(Entity):
    ...
    def _init_jac_and_IK(self):
        ...
        self._IK_mat           = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._IK_error_dim, self._IK_error_dim)))
        self._IK_inv           = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._IK_error_dim, self._IK_error_dim)))
        self._IK_L             = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._IK_error_dim, self._IK_error_dim)))
        self._IK_U             = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._IK_error_dim, self._IK_error_dim)))
        self._IK_y             = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._IK_error_dim, self._IK_error_dim)))
        ...
        self._IK_jacobian      = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._IK_error_dim, self.n_dofs)))
        self._IK_jacobian_T    = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self.n_dofs, self._IK_error_dim)))
    ...
    def get_jacobian(self, link):
        ...
        '''
        if not self._requires_jac_and_IK:
            gs.raise_exception('Inverse kinematics and jacobian are disabled for this entity. Set `morph.requires_jac_and_IK` to True if you need them.')
    ...
    def inverse_kinematics_multilink(
        ...
        if init_qpos is not None:
            ...
            if init_qpos.shape[-1] != self.n_qs:
                gs.raise_exception(f'Size of last dimension `init_qpos` does not match entity\'s `n_qs`: {init_qpos.shape[-1]} vs {self.n_qs}.')
        ...
        poss = torch.stack([self._solver._process_dim(torch.as_tensor(pos, dtype=gs.tc_float, device=gs.device)) for pos in poss])
        quats = torch.stack([self._solver._process_dim(torch.as_tensor(quat, dtype=gs.tc_float, device=gs.device)) for quat in quats])
        ...
        if return_error:
            error_pose = self._IK_err_pose_best.to_torch(gs.device).reshape([self._IK_n_tgts, 6, -1])[:n_links].permute([2, 0, 1])
            if self._solver.n_envs == 0:
            ...
            for i_sample in range(max_samples):
                for _ in range(max_solver_iters):
                    ...
                    for i_ee in range(n_links):
                        ...
                        tgt_quat_i = ti.Vector([quats[i_ee, i_b, 0], quats[i_ee, i_b, 1], quats[i_ee, i_b, 2], quats[i_ee, i_b, 3]])
                        err_rot_i = gu.ti_quat_to_rotvec(gu.ti_transform_quat_by_quat(gu.ti_inv_quat(self._solver.links_state[i_l_ee, i_b].quat), tgt_quat_i))
                        for k in range(3):
                    ...
                    lu.mat_mul(self._IK_jacobian, self._IK_jacobian_T, self._IK_mat, n_error_dims, self.n_dofs, n_error_dims, i_b)
                    ...
                    lu.mat_mul_vec(self._IK_jacobian_T, self._IK_vec, self._IK_delta_qpos, self.n_dofs, n_error_dims, i_b)
                    for i in range(self.n_dofs):
                        self._IK_delta_qpos[i, i_b] = ti.math.clamp(self._IK_delta_qpos[i, i_b], -max_step_size, max_step_size)

                    # update q
                    self._solver._func_integrate_dq_entity(self._IK_delta_qpos, self._idx_in_solver, i_b, respect_joint_limit)
                ...
                if solved:
                    ...
                    for i_ee in range(n_links):
                        ...
                        error_pos_best = ti.Vector([self._IK_err_pose_best[i_ee * 6 + i_error, i_b] for i_error in range(3)])
                        error_rot_best = ti.Vector([self._IK_err_pose_best[i_ee * 6 + i_error, i_b] for i_error in range(3, 6)])
                        ...
                        for i_l in range(self.link_start, self.link_end):
                            ...
                            if l_info.joint_type == gs.JOINT_TYPE.FREE:
                                ...
                                if ti.math.isinf(dof_info.limit[0]) or ti.math.isinf(dof_info.limit[1]):
                                    ...
                                    self._solver.qpos[q_start, i_b] = dof_info.limit[0] + ti.random() * (dof_info.limit[1] - dof_info.limit[0])
                    else:
    ...
    def plan_path(
        ...
        if (qpos_start < q_limit_lower).any() or (qpos_start > q_limit_upper).any():
            gs.logger.warning('`qpos_start` exceeds joint limit. Relaxing joint limit to contain `qpos_start` for planning.')
        ...
        if (qpos_goal < q_limit_lower).any() or (qpos_goal > q_limit_upper).any():
            gs.logger.warning('`qpos_goal` exceeds joint limit. Relaxing joint limit to contain `qpos_goal` for planning.')
    ...
    def zero_all_dofs_velocity(self, envs_idx=None):
        ...
        if self._solver.n_envs == 0:
            ...
            qvel = torch.zeros(self._solver._batch_shape(self.n_dofs, True, B=len(envs_idx)), dtype=gs.tc_float, device=gs.device)
    ...
    def get_contacts(self, with_entity=None):
        ...
        valid_mask = np.logical_and(
            valid_mask,
            np.arange(valid_mask.shape[0])[:, None] < n_contacts
        )
    ...
    def get_links_net_contact_force(self):
        ...
        '''
        scene_links_force = self._solver.links_state.contact_force.to_torch(gs.device).clone().detach().permute([1, 0, 2])
    ...
    def set_friction_ratio(self, friction_ratio, link_indices, envs_idx=None):
        ...
        self._solver.set_geoms_friction_ratio(
            torch.cat([ratio.unsqueeze(-1).repeat(1, self._links[j].n_geoms) for j, ratio in zip(link_indices, friction_ratio.unbind(-1))], dim=-1),
            torch.tensor([[self._links[j]._geom_start + i for i in range(self._links[j].n_geoms)] for j in link_indices]).view(-1),
            envs_idx
        )
