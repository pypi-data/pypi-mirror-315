...
class RigidGeom(RBC):
    ...
    def __init__(
        ...
        if center_init is None:
            self._init_center_pos = np.repeat(self._init_verts.mean(0, keepdims=True), repeats=self._init_verts.shape[0], axis=0)
        else:
    ...
    def _preprocess(self):
        ...
        self._gsd_path = mu.get_gsd_path(self._init_verts, self._init_faces, self._material.sdf_cell_size, self._material.sdf_min_res, self._material.sdf_max_res)
        ...
        if not os.path.exists(self._gsd_path):
            with gs.logger.timer(f'Preprocessing geom idx ~~<{self._idx}>~~.'):
                ...
                ######## adjacency graph ########
                all_vert_neighbors_list = trimesh.Trimesh(vertices=self._init_verts, faces=self._init_faces, process=False).vertex_neighbors
    ...
    def _compute_closest_verts(self, query_points):
        ...
        verts_ids = verts_ids[np.arange(len(query_points)).astype(int), np.argmin(np.linalg.norm(self._init_verts[verts_ids] - query_points[:, None, :], axis=-1), axis=-1)]
        return verts_ids

    def _compute_sd_grad(self, query_points, delta=5e-4):
        ######## sdf gradient via finite differencing ########
        sd_val_xpos = self._compute_sd(query_points + np.array([delta, 0., 0.]))
        sd_val_xneg = self._compute_sd(query_points - np.array([delta, 0., 0.]))
        sd_val_ypos = self._compute_sd(query_points + np.array([0., delta, 0.]))
        sd_val_yneg = self._compute_sd(query_points - np.array([0., delta, 0.]))
        sd_val_zpos = self._compute_sd(query_points + np.array([0., 0., delta]))
        sd_val_zneg = self._compute_sd(query_points - np.array([0., 0., delta]))
    ...
    def visualize_sdf(
        ...
        if T is None:
            if pos is None:
                T = gu.trans_quat_to_T(
                    tensor_to_array(self.get_pos()),
                    tensor_to_array(self.get_quat())
                )
            else:
        ...
        if show_axis:
            ...
            axis_mesh.visual = trimesh.visual.ColorVisuals(vertex_colors=np.tile(np.array([axis_color]), [len(axis_mesh.vertices), 1]))
            self._solver.scene.draw_debug_mesh(axis_mesh, T=T)

        if show_boundary:
            boundary_mesh = trimesh.creation.box(extents=[1,1,1])
            boundary_mesh.vertices = gu.inv_transform_by_T((boundary_mesh.vertices + 0.5) * (self.sdf_res - 1), self.T_mesh_to_sdf)
            boundary_mesh.visual = trimesh.visual.ColorVisuals(vertex_colors=np.tile(np.array([boundary_color]), [len(boundary_mesh.vertices), 1]))
