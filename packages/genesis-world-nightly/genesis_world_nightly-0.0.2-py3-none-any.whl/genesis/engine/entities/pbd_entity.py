...
class PBDTetEntity(ParticleEntity):
    ...
    '''
    def __init__(self, scene, solver, material, morph, surface, particle_size, idx, particle_start, edge_start, vvert_start, vface_start):

        super().__init__(scene, solver, material, morph, surface, particle_size, idx, particle_start, vvert_start, vface_start)
...
class PBD2DEntity(PBDTetEntity):
    ...
    2D mesh entity.
    '''
    def __init__(self, scene, solver, material, morph, surface, particle_size, idx, particle_start, edge_start, inner_edge_start, vvert_start, vface_start):
        super().__init__(scene, solver, material, morph, surface, particle_size, idx, particle_start, edge_start, vvert_start, vface_start)
    ...
    def sample(self):
        ...
        self._edges_len_rest = np.linalg.norm(self._particles[self._edges[:, 0]] - self._particles[self._edges[:, 1]], axis=1)
        self._inner_edges_len_rest = np.linalg.norm(self._particles[self._inner_edges[:, 2]] - self._particles[self._inner_edges[:, 3]], axis=1)
...
class PBD3DEntity(PBDTetEntity):
    ...
    3D mesh entity.
    '''
    def __init__(self, scene, solver, material, morph, surface, particle_size, idx, particle_start, edge_start, elem_start, vvert_start, vface_start):
        super().__init__(scene, solver, material, morph, surface, particle_size, idx, particle_start, edge_start, vvert_start, vface_start)
    ...
    def sample(self):
        ...
        self._edges_len_rest = np.linalg.norm(self._particles[self._edges[:, 0]] - self._particles[self._edges[:, 1]], axis=1)
        self._elems_vol_rest = np.linalg.det(self._particles[self._elems[:, 1:]] - self._particles[self._elems[:, 0:1]]) / 6.0
        self._n_particles = len(self._particles)
