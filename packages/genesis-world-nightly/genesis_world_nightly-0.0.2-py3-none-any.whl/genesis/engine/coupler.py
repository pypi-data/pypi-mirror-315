...
class Coupler(RBC):
    ...
    def build(self):
        ...
        if self._rigid_mpm and self.mpm_solver.enable_CPIC:
            ...
            self.mpm_rigid_normal = ti.Vector.field(3, dtype=gs.ti_float, shape=(self.mpm_solver.n_particles, self.rigid_solver.n_geoms_))
        
        if self._rigid_sph:
            self.sph_rigid_normal = ti.Vector.field(3, dtype=gs.ti_float, shape=(self.sph_solver.n_particles, self.rigid_solver.n_geoms_))
            self.sph_rigid_normal_reordered = ti.Vector.field(3, dtype=gs.ti_float, shape=(self.sph_solver.n_particles, self.rigid_solver.n_geoms_))

        if self._rigid_pbd:
            self.pbd_rigid_normal = ti.Vector.field(3, dtype=gs.ti_float, shape=(self.pbd_solver.n_particles, self.rigid_solver.n_geoms))
            self.pbd_rigid_normal_reordered = ti.Vector.field(3, dtype=gs.ti_float, shape=(self.pbd_solver.n_particles, self.rigid_solver.n_geoms))
    ...
    def _func_collide_in_rigid_geom(self, pos_world, vel, mass, normal_rigid, influence, geom_idx, batch_idx):
        ...
        if rvel_normal_magnitude < 0: # colliding
            ...
            rvel_tan = rvel_tan / rvel_tan_norm * ti.max(0, rvel_tan_norm + rvel_normal_magnitude * g_info.coup_friction)
    ...
    def mpm_grid_op(self, f: ti.i32, t: ti.f32):
        ...
        for I in ti.grouped(ti.ndrange(*self.mpm_solver.grid_res)):
            if self.mpm_solver.grid[f, I].mass > gs.EPS:
                ...
                vel_mpm = (1 / self.mpm_solver.grid[f, I].mass) * self.mpm_solver.grid[
                    f, I
                ].vel_in

                # gravity
                vel_mpm += (
                    self.mpm_solver.substep_dt * self.mpm_solver._gravity[None]
                )
                ...
                if ti.static(self._mpm_sph):
                    ...
                    for offset in ti.grouped(ti.ndrange(self.mpm_sph_stencil_size, self.mpm_sph_stencil_size, self.mpm_sph_stencil_size)):
                        slot_idx = self.sph_solver.sh.grid_to_slot(base + offset)
                        for i in range(
                            self.sph_solver.sh.slot_start[slot_idx],
                            self.sph_solver.sh.slot_start[slot_idx]
                            + self.sph_solver.sh.slot_size[slot_idx],
                        ):
                    ...
                    if colliding_particles > 0:
                        ...
                        for offset in ti.grouped(ti.ndrange(self.mpm_sph_stencil_size, self.mpm_sph_stencil_size, self.mpm_sph_stencil_size)):
                            ...
                            for i in range(
                                ...
                                if ti.abs(pos - self.sph_solver.particles_reordered.pos[i]).max() < self.mpm_solver.dx * 0.5:
                                    self.sph_solver.particles_reordered[i].vel = self.sph_solver.particles_reordered[i].vel - delta_mv / self.sph_solver.particles_info_reordered[i].mass
                ...
                if ti.static(self._mpm_pbd):
                    ...
                    colliding_particles = 0
                    for offset in ti.grouped(ti.ndrange(self.mpm_pbd_stencil_size, self.mpm_pbd_stencil_size, self.mpm_pbd_stencil_size)):
                        slot_idx = self.pbd_solver.sh.grid_to_slot(base + offset)
                        for i in range(
                            self.pbd_solver.sh.slot_start[slot_idx],
                            self.pbd_solver.sh.slot_start[slot_idx]
                            + self.pbd_solver.sh.slot_size[slot_idx],
                        ):
                            ...
                            for i in range(
                                ...
                                if ti.abs(pos - self.pbd_solver.particles_reordered.pos[i]).max() < self.mpm_solver.dx * 0.5:
                                    if self.pbd_solver.particles_reordered[i].free:
                                        self.pbd_solver.particles_reordered[i].vel = self.pbd_solver.particles_reordered[i].vel - delta_mv / self.pbd_solver.particles_info_reordered[i].mass

                #################### MPM boundary ####################
                _, self.mpm_solver.grid[f, I].vel_out = (
                    self.mpm_solver.boundary.impose_pos_vel(pos, vel_mpm)
                )
                ...
                for i_g in range(self.rigid_solver.n_geoms):
                    if self.rigid_solver.geoms_info[i_g].needs_coup:
                        sdf_normal = self.rigid_solver.sdf.sdf_normal_world(
                            self.mpm_solver.particles[f, i].pos, i_g, 0
                        )
            ...
            if self.fem_solver.surface[i].active:
                ...
                if ti.static(self._rigid_fem):
                    ...
                    for j in ti.static(range(3)):
                        ...
                        vel_fem_sv = self._func_collide_with_rigid(
                            ...
                            mass / 3., # assume element mass uniformly distributed to vertices
                        )
                ...
                if ti.static(self._fem_mpm):
                    for j in ti.static(range(3)):
                        ...
                        mass_fem_sv = mass / 4. # assume element mass uniformly distributed
                        ...
                        for mpm_offset in ti.static(ti.grouped(self.mpm_solver.stencil_range())):
                            ...
                            if signed_dist <= self.mpm_solver.dx: # NOTE: use dx as minimal unit for collision
                                ...
                                if mass_mpm_at_cell > gs.EPS:
                                    delta_mpm_vel_at_cell_unmul = vel_fem_sv * mpm_weight - self.mpm_solver.grid[f, mpm_grid_I].vel_out
                                    mass_mul_at_cell = mpm_grid_mass / mass_fem_sv # NOTE: use un-reweighted mass instead of mass_mpm_at_cell
                ...
                if ti.static(self._fem_sph):
                    for j in ti.static(range(3)):
                        ...
                        if colliding_particles > 0:
                            ...
                            vel_fem_sv = vel_fem_sv_unprojected.dot(surface_normal) * surface_normal # exclude tangential velocity
                            ...
                            for offset in ti.grouped(ti.ndrange(stencil_size, stencil_size, stencil_size)):
                                ...
                                for k in range(
                                    ...
                                    if ti.abs(pos - self.sph_solver.particles_reordered.pos[k]).max() < dx * 0.5:
                                        self.sph_solver.particles_reordered[k].vel = self.sph_solver.particles_reordered[k].vel - delta_mv / self.sph_solver.particles_info_reordered[k].mass
        ...
        for i in range(self.sph_solver._n_particles):
            if self.sph_solver.particles_ng_reordered[i].active:
                ...
                for i_g in range(self.rigid_solver.n_geoms):
                    if self.rigid_solver.geoms_info[i_g].needs_coup:
                        i_b = 0
                        self.sph_solver.particles_reordered[i].vel, self.sph_rigid_normal_reordered[i, i_g] = self._func_collide_with_rigid_geom_robust(
        ...
        for i in range(self.pbd_solver._n_particles):
            if self.pbd_solver.particles_ng_reordered[i].active:
                ...
                for i_g in range(self.rigid_solver.n_geoms):
                    if self.rigid_solver.geoms_info[i_g].needs_coup:
                        i_b = 0
                        self.pbd_solver.particles_reordered[i].pos, self.pbd_solver.particles_reordered[i].vel, self.pbd_rigid_normal_reordered[i, i_g] = \
