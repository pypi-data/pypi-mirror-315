...
class Simulator(RBC):
    ...
    def _add_entity(self, morph, material, surface, visualize_contact=False):
        if isinstance(material, gs.materials.Tool):
            ...
            entity = HybridEntity(self.n_entities, self.scene, material, morph, surface) # adding to solver is handled in the hybrid entity
    ...
    def save_ckpt(self):
        ...
        if self._requires_grad:
            gs.logger.debug(f'Forward: Saved checkpoint for global substep {ckpt_start_substep} to {ckpt_end_step}. Now starts from substep {self._cur_substep_global}.')
    ...
    def load_ckpt(self):
        ...
        gs.logger.debug(f'Backward: Loaded checkpoint for global substep {ckpt_start_substep} to {ckpt_end_step}. Now starts from substep {ckpt_start_substep}.')
