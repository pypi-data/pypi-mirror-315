...
class PBDSolver(Solver):
        ...
        for i in range(self._n_particles):
            if self.particles[i].free:
                ...
                if self.particles_info[i].mat_type == self.MATS.CLOTH: 
                    f_air_resistance = self.particles_info[i].air_resistance * self.particles[i].vel.norm() * self.particles[i].vel
                    self.particles[i].vel = self.particles[i].vel - f_air_resistance / self.particles_info[i].mass * self._substep_dt
        ...
        for _ in ti.static(range(self._max_bending_solver_iterations)):
            for i_ie in range(self._n_inner_edges): # 140 - 142
                ...
                if w1 + w2 + w3 + w4 > 0.0:
                    ...
                    # XPBD
                    alpha = self.inner_edges_info[i_ie].bending_compliance / (self._substep_dt ** 2)
                    constraint = -ti.sqrt(1 - d ** 2) * constraint / (sum_wq + alpha) * self.inner_edges_info[i_ie].bending_relaxation
    ...
    def _func_solve_collision(self, i, j):
        ...
        if cur_dist < target_dist and rest_dist > target_dist:
            ...
            # equation (23)
            dv = (self.particles_reordered[i].pos - self.particles_reordered[i].ipos) - (self.particles_reordered[j].pos - self.particles_reordered[j].ipos)
            dpos = -(dv - n * n.dot(dv))
            # equation (24)
            ...
            if self.particles_info_reordered[i].mat_type != self.MATS.PARTICLE:
                ...
                for offset in ti.grouped(ti.ndrange((-1, 2), (-1, 2), (-1, 2))):
                    ...
                    for j in range(self.sh.slot_start[slot_idx], self.sh.slot_size[slot_idx] + self.sh.slot_start[slot_idx]):
                        if i != j and not (self.particles_info_reordered[i].mat_type == self.MATS.LIQUID and self.particles_info_reordered[j].mat_type == self.MATS.LIQUID):
                    ...
                    for offset in ti.grouped(ti.ndrange((-1, 2), (-1, 2), (-1, 2))):
                        ...
                        for j in range(self.sh.slot_start[slot_idx], self.sh.slot_size[slot_idx] + self.sh.slot_start[slot_idx]):
                            if i != j:
                                ...
                                self.particles_reordered[i].dpos = self.particles_reordered[i].dpos + left * right / self.particles_info_reordered[i].rho_rest * self.dist_scale * self.particles_info_reordered[i].density_relaxation
                ...
                if self.particles_info_reordered[i].mat_type == self.MATS.LIQUID:
                    ...
                    for offset in ti.grouped(ti.ndrange((-1, 2), (-1, 2), (-1, 2))):
                        ...
                        for j in range(self.sh.slot_start[slot_idx], self.sh.slot_size[slot_idx] + self.sh.slot_start[slot_idx]):
                            ...
                            pos_j = self.particles_reordered[j].pos
                            v_ij = (self.particles_reordered[j].pos - self.particles_reordered[j].ipos) - (self.particles_reordered[i].pos - self.particles_reordered[i].ipos)
                    ...
                    # ---Viscosity---
                    self.particles_reordered[i].dpos = self.particles_reordered[i].dpos + xsph_sum * self.particles_info_reordered[i].viscosity_relaxation
    ...
    def _kernel_compute_velocity(self, f: ti.i32):
        for i in range(self._n_particles):
            self.particles_reordered[i].vel = (self.particles_reordered[i].pos - self.particles_reordered[i].ipos) / self._substep_dt
    ...
    def _kernel_update_render_fields(self, f: ti.i32):
        ...
        for i in range(self._n_vverts):
            ...
            for j in range(self._n_vvert_supports):
                vvert_pos += self.particles[self.vverts_info.support_idxs[i][j]].pos * self.vverts_info.support_weights[i][j]
    ...
    def n_fluid_particles(self):
        if self.is_built:
            ...
            return sum([entity.n_fluid_particles if hasattr(entity, 'n_fluid_particles') else 0 for entity in self._entities])
