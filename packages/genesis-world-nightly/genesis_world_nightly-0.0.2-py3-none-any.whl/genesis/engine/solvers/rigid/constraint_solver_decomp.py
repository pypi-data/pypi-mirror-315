...
class ConstraintSolver:
    def __init__(self, rigid_solver):
        ...
        self.len_constraints = 5 * self._collider._max_contact_pairs + np.logical_not(np.isinf(self._solver.dofs_info.limit.to_numpy()[:,0])).sum()
        self.len_constraints_ = max(1, self.len_constraints)

        self.jac = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self.len_constraints_, self._solver.n_dofs_)))
        ...
        if self.sparse_solve:
            self.jac_relevant_dofs = ti.field(gs.ti_int, shape=self._solver._batch_shape((self.len_constraints_, self._solver.n_dofs_)))
        ...
        if self._solver_type == gs.constraint_solver.Newton:
            self.nt_H = ti.field(dtype=gs.ti_float, shape=self._solver._batch_shape((self._solver.n_dofs_, self._solver.n_dofs_)))
        ...
        for i_b in range(self._B):
            for i_col in range(self._collider.n_contacts[i_b]):
                ...
                for i in range(4):
                    ...
                    imp, aref = gu.imp_aref(self._solver._sol_timeconst, impact.sol_params, -impact.penetration, jac_qvel)
                ...
                if ti.static(self.sparse_solve):
                    if flag_update != -1:
                        ...
                        for k_ in range(self.jac_n_relevant_dofs[i_c, i_b]):
                            ...
                            tmp = Lkk * Lkk + self.nt_vec[k, i_b] * self.nt_vec[k, i_b] * (
                                flag_update * 2 - 1
                            )
                            if tmp < gs.EPS:
                                tmp = gs.EPS
                            ...
                            for i_ in range(k_):
                                ...
                                self.nt_H[i, k, i_b] = (
                                    self.nt_H[i, k, i_b]
                                    + s * self.nt_vec[i, i_b] * (flag_update * 2 - 1)
                                ) * cinv

                            for i_ in range(k_):
                                i = self.jac_relevant_dofs[i_c, i_, i_b] # i is strictly > k
                                self.nt_vec[i, i_b] = (
                                    self.nt_vec[i, i_b] * c - s * self.nt_H[i, k, i_b]
                                )
                ...
                for i_d1_ in range(jac_n_relevant_dofs):
                    ...
                    if ti.abs(self.jac[i_c, i_d1, i_b]) > gs.EPS:
                        for i_d2_ in range(i_d1_, jac_n_relevant_dofs):
                            ...
                            self.nt_H[i_d1, i_d2, i_b] = self.nt_H[i_d1, i_d2, i_b] + \
                                self.jac[i_c, i_d2, i_b] * self.jac[i_c, i_d1, i_b] * self.efc_D[i_c, i_b] * self.active[i_c, i_b]
        else:
            for i_c in range(self.n_constraints[i_b]):
                for i_d1 in range(self._solver.n_dofs):
    ...
    def _func_nt_chol_solve(self, i_b):
        ...
        for i_d in range(self._solver.n_dofs):
            for j_d in range(i_d):
                self.Mgrad[i_d, i_b] = self.Mgrad[i_d, i_b] - \
                    (self.nt_H[i_d, j_d, i_b] * self.Mgrad[j_d, i_b])
        ...
        for i_d_ in range(self._solver.n_dofs):
            ...
            for j_d in range(i_d + 1, self._solver.n_dofs):
                self.Mgrad[i_d, i_b] = self.Mgrad[i_d, i_b] - \
                    self.nt_H[j_d, i_d, i_b] * self.Mgrad[j_d, i_b]
    ...
    def _func_update_contact_force(self):
        ...
        for i_b in range(self._B):
            for i_col in range(self._collider.n_contacts[i_b]):
                ...
                self._solver.links_state[impact.link_a, i_b].contact_force = self._solver.links_state[impact.link_a, i_b].contact_force - force
                self._solver.links_state[impact.link_b, i_b].contact_force = self._solver.links_state[impact.link_b, i_b].contact_force + force
    ...
    def _func_ls_init(self, i_b):
        ...
        for i_d in range(self._solver.n_dofs):
            quad_gauss_1 += self.search[i_d, i_b] * self.Ma[i_d, i_b] -\
        ...
        for _i0 in range(1):
            ...
            for i_c in range(self.n_constraints[i_b]):
                self.quad[i_c, _i0+0, i_b] = self.efc_D[i_c, i_b] * (0.5 * self.Jaref[i_c, i_b] * self.Jaref[i_c, i_b])
    ...
    def _func_ls_point_fn(self, i_b, alpha):
        ...
        cost = alpha * alpha * tmp_quad_total2 + \
                alpha * tmp_quad_total1 + tmp_quad_total0
        ...
        if snorm < 1e-8:
            ...
            if ti.abs(p1_deriv_0) < gtol:
                ...
                while p1_deriv_0 * direction <= - gtol and self.ls_its[i_b] < self.ls_iterations:
                    ...
                    p1_alpha, p1_cost, p1_deriv_0, p1_deriv_1 = self._func_ls_point_fn(i_b, p1_alpha - p1_deriv_0 / p1_deriv_1)
                    if ti.abs(p1_deriv_0) < gtol:
                        ls_slope = ti.abs(p1_deriv_0) * slopescl
                        res_alpha = p1_alpha
                        done = True
                ...
                if not done:
                    ...
                    if not done:
                        ...
                        p1_next_alpha, p1_next_cost, p1_next_deriv_0, p1_next_deriv_1 = \
                            self._func_ls_point_fn(i_b, p1_alpha - p1_deriv_0 / p1_deriv_1)

                        while self.ls_its[i_b] < self.ls_iterations:

                            pmid_alpha, pmid_cost, pmid_deriv_0, pmid_deriv_1 = \
                                self._func_ls_point_fn(i_b, (p1_alpha + p2_alpha) * 0.5)

                            i = 0
                            self.candidates[4*i+0,i_b], self.candidates[4*i+1,i_b], self.candidates[4*i+2,i_b], self.candidates[4*i+3,i_b] = \
                                p1_next_alpha, p1_next_cost, p1_next_deriv_0, p1_next_deriv_1
                            i = 1
                            self.candidates[4*i+0,i_b], self.candidates[4*i+1,i_b], self.candidates[4*i+2,i_b], self.candidates[4*i+3,i_b] = \
                                p2_next_alpha, p2_next_cost, p2_next_deriv_0, p2_next_deriv_1
                            ...
                            best_i = -1
                            best_cost = gs.ti_float(0.)
                            for ii in range(3):
                                if ti.abs(self.candidates[4*ii+2,i_b]) < gtol and (best_i < 0 or self.candidates[4*ii+1,i_b] < best_cost):
                            ...
                            if best_i >= 0:
                                ...
                                b1, p1_alpha, p1_cost, p1_deriv_0, p1_deriv_1, \
                                    p1_next_alpha, p1_next_cost, p1_next_deriv_0, p1_next_deriv_1 = \
                                    self.update_bracket(p1_alpha, p1_cost, p1_deriv_0, p1_deriv_1, i_b)
                                b2, p2_alpha, p2_cost, p2_deriv_0, p2_deriv_1, \
                                    p2_next_alpha, p2_next_cost, p2_next_deriv_0, p2_next_deriv_1 = \
    ...
    def update_bracket(self, p_alpha, p_cost, p_deriv_0, p_deriv_1, i_b):
        ...
        for i in range(3):
            if p_deriv_0 < 0 and self.candidates[4*i+2,i_b] < 0 and p_deriv_0 < self.candidates[4*i+2,i_b]:
                p_alpha, p_cost, p_deriv_0, p_deriv_1 = \
                    self.candidates[4*i+0,i_b], self.candidates[4*i+1,i_b], \
                    self.candidates[4*i+2,i_b], self.candidates[4*i+3,i_b]
        ...
        p_next_alpha, p_next_cost, p_next_deriv_0, p_next_deriv_1 = \
            p_alpha, p_cost, p_deriv_0, p_deriv_1

        if flag > 0:
            p_next_alpha, p_next_cost, p_next_deriv_0, p_next_deriv_1 = \
                self._func_ls_point_fn(i_b, p_alpha - p_deriv_0 / p_deriv_1)
    ...
    def _func_solve_body(self, i_b):
        ...
        if ti.abs(alpha) < gs.EPS:
            ...
            if ti.static(self._solver_type == gs.constraint_solver.CG):
                ...
                self.cg_beta[i_b] = ti.max(0., self.cg_beta[i_b])
                for i_d in range(self._solver.n_dofs):
    ...
    def _func_update_constraint(self, i_b, qacc, Ma, cost):
        ...
        if ti.static(self.sparse_solve):
            ...
            for i_c in range(self.n_constraints[i_b]):
                for i_d_ in range(self.jac_n_relevant_dofs[i_c, i_b]):
                    i_d = self.jac_relevant_dofs[i_c, i_d_, i_b]
                    self.qfrc_constraint[i_d, i_b] = self.qfrc_constraint[i_d, i_b] + self.jac[i_c, i_d, i_b] * self.efc_force[i_c, i_b]
        else:
            for i_d in range(self._solver.n_dofs):
        ...
        # (Mx - Mx') * (x - x')
        for i_d in range(self._solver.n_dofs):
            v = 0.5 * (Ma[i_d, i_b] - self._solver.dofs_state[i_d, i_b].force) * (qacc[i_d, i_b] - self._solver.dofs_state[i_d, i_b].acc_smooth)
            self.gauss[i_b] = self.gauss[i_b] + v
            cost[i_b] = cost[i_b] + v

        # D * (Jx - aref) ** 2
        for i_c in range(self.n_constraints[i_b]):
            cost[i_b] = cost[i_b] + 0.5 * (self.efc_D[i_c, i_b] * self.Jaref[i_c, i_b] * self.Jaref[i_c, i_b] * self.active[i_c, i_b])

    @ti.func
    def _func_update_gradient(self, i_b):
        for i_d in range(self._solver.n_dofs):
            self.grad[i_d, i_b] = self.Ma[i_d, i_b] - self._solver.dofs_state[i_d, i_b].force - self.qfrc_constraint[i_d, i_b]
