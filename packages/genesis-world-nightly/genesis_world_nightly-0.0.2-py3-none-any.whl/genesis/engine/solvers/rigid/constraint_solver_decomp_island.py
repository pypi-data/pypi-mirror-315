            ...
            for i_d1_ in range(jac_n_relevant_dofs):
                ...
                if ti.abs(self.jac[i_c, i_d1, i_b]) > gs.EPS:
                    for i_d2_ in range(i_d1_ + 1):
                        i_d2 = self.jac_relevant_dofs[i_c,  jac_n_relevant_dofs - 1 - i_d2_, i_b] # i_d2 is strictly <= i_d1
                        
                        d1 = ti.max(i_d1, i_d2)
                        d2 = ti.min(i_d1, i_d2)

                        self.nt_H[d1, d2, i_b] = self.nt_H[d1, d2, i_b] + \
                            self.jac[i_c, d2, i_b] * self.jac[i_c, d1, i_b] * self.efc_D[i_c, i_b] * self.active[i_c, i_b]
    ...
    def _func_update_constraint(self, island, i_b, qacc, Ma, cost):
        ...
        for i_island_entity in range(self.contact_island.island_entity[island, i_b].n):
            ...
            for i_d in range(e_info.dof_start, e_info.dof_end):
                ...
                v = 0.5 * (Ma[i_d, i_b] - self._solver.dofs_state[i_d, i_b].force) * (qacc[i_d, i_b] - self._solver.dofs_state[i_d, i_b].acc)
