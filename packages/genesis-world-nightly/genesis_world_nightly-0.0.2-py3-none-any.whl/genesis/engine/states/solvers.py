...
class RigidSolverState:
    ...
    def __init__(self, scene):
        ...
        self.qpos       = gs.zeros(scene.sim.rigid_solver._batch_shape(scene.sim.rigid_solver.n_qs, True), dtype=float, requires_grad=False, scene=self.scene)
        self.dofs_vel   = gs.zeros(scene.sim.rigid_solver._batch_shape(scene.sim.rigid_solver.n_dofs, True), dtype=float, requires_grad=False, scene=self.scene)
        self.links_pos  = gs.zeros(scene.sim.rigid_solver._batch_shape((scene.sim.rigid_solver.n_links, 3), True), dtype=float, requires_grad=False, scene=self.scene)
        self.links_quat = gs.zeros(scene.sim.rigid_solver._batch_shape((scene.sim.rigid_solver.n_links, 4), True), dtype=float, requires_grad=False, scene=self.scene)
        self.i_pos_shift  = gs.zeros(scene.sim.rigid_solver._batch_shape((scene.sim.rigid_solver.n_links, 3), True), dtype=float, requires_grad=False, scene=self.scene)
        self.mass_shift  = gs.zeros(scene.sim.rigid_solver._batch_shape(scene.sim.rigid_solver.n_links, True), dtype=float, requires_grad=False, scene=self.scene)
        self.friction_ratio  = gs.ones(scene.sim.rigid_solver._batch_shape(scene.sim.rigid_solver.n_geoms, True), dtype=float, requires_grad=False, scene=self.scene)
...
class AvatarSolverState:
    ...
    def __init__(self, scene):
        ...
        self.qpos       = gs.zeros(scene.sim.avatar_solver._batch_shape(scene.sim.avatar_solver.n_qs, True), dtype=float, requires_grad=False, scene=self.scene)
        self.dofs_vel   = gs.zeros(scene.sim.avatar_solver._batch_shape(scene.sim.avatar_solver.n_dofs, True), dtype=float, requires_grad=False, scene=self.scene)
        self.links_pos  = gs.zeros(scene.sim.avatar_solver._batch_shape((scene.sim.avatar_solver.n_links, 3), True), dtype=float, requires_grad=False, scene=self.scene)
        self.links_quat = gs.zeros(scene.sim.avatar_solver._batch_shape((scene.sim.avatar_solver.n_links, 4), True), dtype=float, requires_grad=False, scene=self.scene)
...
class MPMSolverState(RBC):
    ...
    def __init__(self, scene):
        self._scene = scene
        self._pos    = gs.zeros((scene.sim.mpm_solver.n_particles, 3), dtype=float, requires_grad=scene.requires_grad, scene=self._scene)
        self._vel    = gs.zeros((scene.sim.mpm_solver.n_particles, 3), dtype=float, requires_grad=scene.requires_grad, scene=self._scene)
        self._C      = gs.zeros((scene.sim.mpm_solver.n_particles, 3, 3), dtype=float, requires_grad=scene.requires_grad, scene=self._scene)
        self._F      = gs.zeros((scene.sim.mpm_solver.n_particles, 3, 3), dtype=float, requires_grad=scene.requires_grad, scene=self._scene)
        self._Jp     = gs.zeros((scene.sim.mpm_solver.n_particles,), dtype=float, requires_grad=scene.requires_grad, scene=self._scene)
