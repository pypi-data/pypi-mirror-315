...
class Mesh(RBC):
        ...
        if self._surface.requires_uv():     # check uvs here
            if self._uvs is None:
                if 'mesh_path' in metadata:
                    gs.logger.warning(f"Texture given but asset missing uv info (or failed to load): {metadata['mesh_path']}")
                else:
    ...
    def tetrahedralize(self, order, mindihedral, minratio, nobisect, quality, verbose):
        ...
        pv_obj = pv.PolyData(
            self.verts,
            np.concatenate([np.full((self.faces.shape[0], 1), self.faces.shape[1]), 
                            self.faces], axis=1)
        )
        tet = tetgen.TetGen(pv_obj)
        verts, elems = tet.tetrahedralize(order=order, mindihedral=mindihedral, minratio=minratio, nobisect=nobisect, quality=quality, verbose=verbose)
    ...
    @classmethod
    def from_trimesh(cls, mesh, scale=None, convexify=False, decimate=False, decimate_face_num=500, metadata=dict(), surface=None):
        '''
    ...
    def from_morph_surface(cls, morph, surface=None):
        ...
        if isinstance(morph, gs.options.morphs.Mesh):
            if morph.file.endswith(('obj', 'ply', 'stl')):
                ...
                gs.raise_exception(f'File type not supported (yet). Submit a feature request if you need this: {morph.file}.')
