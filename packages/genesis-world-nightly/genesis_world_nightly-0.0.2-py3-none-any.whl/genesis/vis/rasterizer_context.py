...
class RasterizerContext:
    ...
    def init_meshes(self):
        ...
        self.world_frame_mesh = mu.create_frame(origin_radius=0.012, axis_radius=0.005, axis_length=self.world_frame_size, head_radius=0.01, head_length=0.03)

        self.link_frame_mesh = trimesh.creation.axis(origin_size=0.03, axis_radius=0.025, axis_length=1.0)
        self.link_frame_mesh.visual.face_colors[:, :3] = (0.7 * self.link_frame_mesh.visual.face_colors[:, :3]).astype(int)
        ...
        if self.link_frame_shown:
            if self.sim.rigid_solver.is_active():
                ...
                for link in links:
                    ...
                    buffer_updates[self._scene.get_buffer_id(self.link_frame_nodes[link.uid], 'model')] = link_T.transpose([0, 2, 1])
        ...
        if self.sim.tool_solver.is_active():
            for tool_entity in self.sim.tool_solver.entities:
                ...
                if tool_entity.mesh is not None:
                    mesh = trimesh.Trimesh(tool_entity.mesh.init_vertices_np,
                                           tool_entity.mesh.faces_np.reshape([-1, 3]),
                                           tool_entity.mesh.init_vertex_normals_np, process=False)
                    ...
                    self.add_static_node(tool_entity.uid, pyrender.Mesh.from_trimesh(mesh, double_sided=double_sided), pose=pose)
                ...
                for geom in geoms:
                    ...
                    self.rigid_nodes[geom.uid] = self.add_node(
                        pyrender.Mesh.from_trimesh(
                            ...
                            double_sided=geom.surface.double_sided if 'collision' not in rigid_entity.surface.vis_mode else False,
    ...
    def update_rigid(self, buffer_updates):
        if self.sim.rigid_solver.is_active():
            for rigid_entity in self.sim.rigid_solver.entities:
                ...
                for geom in geoms:
                    geom_T = geoms_T[geom.idx][:self.n_rendered_envs]
                    buffer_updates[self._scene.get_buffer_id(self.rigid_nodes[geom.uid], 'model')] = geom_T.transpose([0, 2, 1])
                    if isinstance(rigid_entity._morph, gs.morphs.Plane):
                        self.set_reflection_mat(geom_T)
                ...
                for geom in geoms:
                    ...
                    self.rigid_nodes[geom.uid] = self.add_node(
                        pyrender.Mesh.from_trimesh(
                            ...
                            double_sided = geom.surface.double_sided if 'collision' not in avatar_entity.surface.vis_mode else False,
                        )
                    )
        ...
        if self.sim.mpm_solver.is_active():
            for mpm_entity in self.sim.mpm_solver.entities:
                if mpm_entity.surface.vis_mode == 'recon':
                    ...
                    mesh = mu.create_sphere(self.sim.mpm_solver.particle_radius * self.particle_size_scale, subdivisions=1)
                    ...
                    self.add_dynamic_node(pyrender.Mesh.from_trimesh(mpm_entity.vmesh.trimesh, smooth=mpm_entity.surface.smooth))
            ...
            for mpm_entity in self.sim.mpm_solver.entities:
                if mpm_entity.surface.vis_mode == 'recon':
                    mesh = pu.particles_to_mesh(
                        positions=particles_all[mpm_entity.particle_start:mpm_entity.particle_end][active_all[mpm_entity.particle_start:mpm_entity.particle_end]],
                    ...
                    buffer_updates[self._scene.get_buffer_id(self.static_nodes[mpm_entity.uid], 'model')] = tfs.transpose([0, 2, 1])
        ...
        if self.sim.sph_solver.is_active():
            for sph_entity in self.sim.sph_solver.entities:
                if sph_entity.surface.vis_mode == 'recon':
                    ...
                    mesh = mu.create_sphere(self.sim.sph_solver.particle_radius * self.particle_size_scale, subdivisions=1)
    ...
    def update_sph(self, buffer_updates):
        if self.sim.sph_solver.is_active():
            ...
            for sph_entity in self.sim.sph_solver.entities:
                if sph_entity.surface.vis_mode == 'recon':
                    mesh = pu.particles_to_mesh(
                        positions=particles_all[sph_entity.particle_start:sph_entity.particle_end][active_all[sph_entity.particle_start:sph_entity.particle_end]],
                    ...
                    buffer_updates[self._scene.get_buffer_id(self.static_nodes[sph_entity.uid], 'model')] = tfs.transpose([0, 2, 1])
            ...
            for pbd_entity in self.sim.pbd_solver.entities:
                if pbd_entity.surface.vis_mode == 'recon':
                    ...
                    if self.render_particle_as == 'sphere':
                        mesh = mu.create_sphere(self.sim.pbd_solver.particle_radius * self.particle_size_scale, subdivisions=1)
                        ...
                        mesh = mu.create_tets_mesh(pbd_entity.n_particles, self.sim.pbd_solver.particle_radius * self.particle_size_scale)
                        mesh.visual = mu.surface_uvs_to_trimesh_visual(pbd_entity.surface, n_verts=len(mesh.vertices))
                    ...
                    self.add_static_node(pbd_entity.uid, pyrender.Mesh.from_trimesh(pbd_entity.vmesh.trimesh, smooth=pbd_entity.surface.smooth, double_sided=pbd_entity._surface.double_sided))
    ...
    def update_pbd(self, buffer_updates):
        if self.sim.pbd_solver.is_active():
            ...
            for pbd_entity in self.sim.pbd_solver.entities:
                if pbd_entity.surface.vis_mode == 'recon':
                    mesh = pu.particles_to_mesh(
                        positions=particles_all[pbd_entity.particle_start:pbd_entity.particle_end][active_all[pbd_entity.particle_start:pbd_entity.particle_end]],
                        radius=self.sim.mpm_solver.particle_radius,
                    ...
                    if self.render_particle_as == 'sphere':
                        ...
                        buffer_updates[self._scene.get_buffer_id(self.static_nodes[pbd_entity.uid], 'model')] = tfs.transpose([0, 2, 1])

                    elif self.render_particle_as == 'tet':
                        new_verts = mu.transform_tets_mesh_verts(
                            pbd_entity._tets_mesh.vertices,
                            positions=particles_all[
                                pbd_entity.particle_start : pbd_entity.particle_end
                            ],
                            zs=particles_vel_all[
                                pbd_entity.particle_start : pbd_entity.particle_end
                            ],
                        )
            ...
            for fem_entity in self.sim.fem_solver.entities:
                if fem_entity.surface.vis_mode == 'visual':
                    ...
                    triangles = triangles_all[
                        fem_entity.s_start:
                        (fem_entity.s_start + fem_entity.n_surfaces)
                    ] - fem_entity.v_start
                    mesh = trimesh.Trimesh(vertices, triangles, process=False)
                    mesh.visual = mu.surface_uvs_to_trimesh_visual(fem_entity.surface, n_verts=fem_entity.n_surface_vertices)
                    self.add_static_node(fem_entity.uid, pyrender.Mesh.from_trimesh(mesh, double_sided=fem_entity.surface.double_sided))
    ...
    def draw_debug_arrow(
        self, pos, vec=(0, 0, 1), radius=0.006, color=(1.0, 0.0, 0.0, 0.5)
    ):
    ...
    def draw_contact_arrow(
        self, pos, radius=0.006, force=(0, 0, 1), color=(0.0, 0.9, 0.8, 1.0)
    ):
    ...
    def draw_debug_spheres(self, poss, radius=0.01, color=(1.0, 0.0, 0.0, 0.5)):
        ...
        self.add_external_node(
            pyrender.Mesh.from_trimesh(mesh, smooth=True, poses=poses)
        )

    def draw_debug_box(
        self, bounds, color=(1.0, 0.0, 0.0, 1.0), wireframe=True, wireframe_radius=0.002
    ):
    ...
    def seg_idxc_rgb_arr_to_idxc_arr(self, seg_idxc_rgb_arr):
        ...
        seg_idxc_arr = np.array(seg_idxc_rgb_arr[:, :, 0] * 256 * 256 + seg_idxc_rgb_arr[:, :, 1] * 256 + seg_idxc_rgb_arr[:, :, 2], dtype=int)
        return seg_idxc_arr
