...
class Viewer(RBC):
    ...
    def build(self, scene):
        ...
        if gs.platform == 'Linux':
            ...
            run_in_thread = False
            auto_start    = False
            gs.logger.warning('Non-linux system detected. In order to use the interactive viewer, you need to manually run simulation in a separate thread and then start viewer. See `examples/render_on_macos.py`.')
