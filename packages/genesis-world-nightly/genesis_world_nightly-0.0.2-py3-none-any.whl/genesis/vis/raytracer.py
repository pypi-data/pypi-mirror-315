...
class EnvironmentSphere:
    ...
    '''
    def __init__(self, radius, pos, quat, surface=None, name=None,
                 file_path='meshes/env_sphere/env_sphere.obj'):
        self.radius    = radius
        self.pos       = pos
        self.quat      = quat
...
class Raytracer:
    def __init__(self, options, vis_options):
        ...
        self.env_sphere = EnvironmentSphere(
            pos     = options.env_pos,
            quat    = options.env_quat,
            radius  = options.env_radius,
            surface = options.env_surface
        )
        ...
        for light in options.lights:
            ...
            self.lights.append(SphereLight(
                radius  = light['radius'],
                pos     = light['pos'],
        ...
        self.lights.append(MeshLight(
            ...
            surface   = gs.surfaces.Plastic(
                color = (
                    ...
                    color[1],
                    color[2],
                    color[3] if color.shape[0] == 4 else 1.0
                ),
                emissive = (
                    color[0] * intensity,
                    color[1] * intensity,
        ...
        if self.sim.mpm_solver.is_active():
            for mpm_entity in self.sim.mpm_solver.entities:
                if mpm_entity.surface.vis_mode == 'visual':
                    ...
                    self.add_particles(str(mpm_entity.uid), self.sim.mpm_solver.particle_radius, mpm_entity.material.rho)
    ...
    def get_texture(self, texture):
        ...
        if isinstance(texture, gs.textures.ColorTexture):
            return LuisaRenderPy.ColorTexture(
                color = texture.color
            )
    ...
    def add_surface(self, shape_name, surface):
        ...
        if isinstance(surface, gs.surfaces.Plastic):
            ...
            surface_luisa = LuisaRenderPy.DisneySurface(
                ...
                diffuse_trans  = None if surface.diffuse_trans == 0.0 else self.get_texture(gs.textures.ColorTexture(color=(surface.diffuse_trans,))),
            )
        ...
        # add clamp normal
        self.shape_clamp_normals[shape_name] = min(self.clamp_normal, surface.normal_diff_clamp) if surface.smooth else 0.0
    ...
    def update_particles(self, name, particles, radius=None, particles_vel=None, particles_radii=None):
        if self.shape_reconstructs[name] is not None:
            mesh = pu.particles_to_mesh(
                positions = particles,
                radius    = radius,
                backend   = self.shape_reconstructs[name][0]
            )
            self.update_deformable(
                name, mesh.vertices, mesh.faces, mesh.vertex_normals, np.array([]),
            )
        else:
            radii = np.array([radius]) if particles_radii is None else particles_radii
            self._shapes[name].update(
                centers = particles,
                radii   = radii
            )
    ...
    def update_scene(self):
        ...
        if self.sim.mpm_solver.is_active():
            ...
            for mpm_entity in self.sim.mpm_solver.entities:
                if mpm_entity.surface.vis_mode == 'visual':
                    ...
                    particles = particles_all[mpm_entity.particle_start:mpm_entity.particle_end][active_all[mpm_entity.particle_start:mpm_entity.particle_end]]
                    particles_vel = particles_vel_all[mpm_entity.particle_start:mpm_entity.particle_end][active_all[mpm_entity.particle_start:mpm_entity.particle_end]]
                    self.update_particles(str(mpm_entity.uid), particles, self.sim.mpm_solver.particle_radius, particles_vel)
        ...
        if self.sim.sph_solver.is_active():
            ...
            for sph_entity in self.sim.sph_solver.entities:
                particles = particles_all[sph_entity.particle_start:sph_entity.particle_end][active_all[sph_entity.particle_start:sph_entity.particle_end]]
                particles_vel = particles_vel_all[sph_entity.particle_start:sph_entity.particle_end][active_all[sph_entity.particle_start:sph_entity.particle_end]]
                self.update_particles(str(sph_entity.uid), particles, self.sim.sph_solver.particle_radius, particles_vel)
        ...
        if self.sim.pbd_solver.is_active():
            ...
            for pbd_entity in self.sim.pbd_solver.entities:
                if pbd_entity.surface.vis_mode == 'visual':
                    ...
                    self.update_deformable(
                        str(pbd_entity.uid), vverts,
                        pbd_entity.vmesh.trimesh.faces,
                        trimesh.Trimesh(vertices = vverts, faces=pbd_entity.vmesh.trimesh.faces, process=False).vertex_normals, # TODO: make this more principled
                        np.array([]) if pbd_entity.vmesh.uvs is None else pbd_entity.vmesh.uvs,
                    )
                else:
                    if self.render_particle_as == 'sphere':
                        particles = particles_all[pbd_entity.particle_start:pbd_entity.particle_end][active_all[pbd_entity.particle_start:pbd_entity.particle_end]]
    ...
    def render_camera(self, camera):
        ...
        img = np.frombuffer(
            b, dtype=np.uint8).reshape(camera.res[1], camera.res[0], 4)[:, :, :3]
        return img
