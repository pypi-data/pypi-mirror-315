...
class Visualizer(RBC):
    ...
    def add_camera(self, res, pos, lookat, up, model, fov, aperture, focus_dist, GUI, spp, denoise):
        ...
        camera = Camera(self, len(self._cameras), model, res, pos, lookat, up, fov, aperture, focus_dist, GUI, spp, denoise)
    ...
    def build(self):
        ...
        if len(self._cameras) > 0 and gs.platform == 'Linux': # Non-linux system uses main thread for viewer, which hasn't been started yet here.
