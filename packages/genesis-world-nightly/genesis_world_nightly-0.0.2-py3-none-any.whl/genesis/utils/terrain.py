...
def parse_terrain(morph: Terrain, surface):
    ...
    if morph.height_field is None:
        ...
        heightfield = np.zeros(np.array(morph.n_subterrains) * np.array([subterrain_rows, subterrain_cols]), dtype=np.int16)
        ...
        for i in range(morph.n_subterrains[0]):
            for j in range(morph.n_subterrains[1]):
                ...
                heightfield[i * subterrain_rows:(i + 1) * subterrain_rows, j * subterrain_cols:(j + 1) * subterrain_cols] = subterrain_height_field
...
def convert_heightfield_to_watertight_trimesh(height_field_raw, horizontal_scale, vertical_scale, slope_threshold=None):
    ...
    triangles = np.concatenate([triangles_top, triangles_bottom, triangles_side_0, triangles_side_1, triangles_side_2, triangles_side_3], axis=0)
