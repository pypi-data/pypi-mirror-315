...
# Class for axis aligned bounding box
    ...
    def add_box(self, idx, src):
        self.box_data[idx].umin = ti.min(
            self.box_data[idx].umin, 
            self.box_data[src].umin)
        self.box_data[idx].umax = ti.max(
            self.box_data[idx].umax, 
            self.box_data[src].umax)
...
class BVHTree:
    ...
    def __init__(self, rigid_bodies):
        ...
        self.candidate_data = struct_candidate.field(shape=(self.len_cd_candidate), needs_grad=False, layout=ti.Layout.SOA)
        ...
        self.narrow_candidate = struct_narrow_candidate.field(shape=(self.len_cd_candidate), needs_grad=False, layout=ti.Layout.SOA)
        ...
        self.solver_params_contact = [2.0e-02, 1.0e+00, 9.0e-01, 9.5e-01, 1.0e-03, 5.0e-01, 2.0e+00]
    ...
    def cd_check_penetration(self, i, ctype):
        ...
        if ctype == 0: # vertex face
            ...
            is_col = self.point_inside_tet(
                t0_t1, t1_t1, t2_t1, t_center_b, p_t1)

            if is_col == 1:
                penetration = ti.sqrt(
                    self.point_plane_distance(p_t1, t0_t1, t1_t1, t2_t1))
            ...
            is_col, t, u, v = self.segment_intersect_triangle(
                ea0_t1, ea1_t1, eb0_t1, eb1_t1, t_center_b)
        ...
        for i in range(self.cd_counter[0]):
            if self.narrow_candidate[i].is_col == 1:
                ...
                if self.contact_aggregate[ga, gb].ctype == -1:
                    ...
                    if self.narrow_candidate[i].ctype == 0 and\
                        self.contact_aggregate[ga, gb].penetration > self.narrow_candidate[i].penetration:
                        self.contact_aggregate[ga, gb].n = n
                        self.contact_aggregate[ga, gb].pos = pos
                        self.contact_aggregate[ga, gb].penetration = penetration

                else: # edge edge
                    if self.narrow_candidate[i].ctype == 0 or \
                        self.contact_aggregate[ga, gb].penetration > self.narrow_candidate[i].penetration:
                        self.contact_aggregate[ga, gb].ctype = ctype
                        self.contact_aggregate[ga, gb].n = n
                        self.contact_aggregate[ga, gb].pos = pos
    ...
    def collision_detection(self):
        from genesis.utils.tools import create_timer; timer = create_timer(name='solve_quadratic', level=4, ti_sync=True, skip_first_call=True)
        self.cd_init()
        timer.stamp('cd_init')
        self.refit_func()
    ...
    def compute_constraint_system(self, n_con : int, n_dof : int):
        ...
        for ic1 in range(n_con):
            for ic2 in range(n_con):
                self.constraint_A[ic1, ic2] = 0.

        for ic1 in range(n_con):
            self.constraint_b[ic1] = 0.

        for ic1 in range(n_con):
            for ic2 in range(n_con):
                for jd1 in range(n_dof):
                    for jd2 in range(n_dof):
                        self.constraint_A[ic1, ic2] += self.con_jac[ic1, jd1] * self.solver.mass_mat_inv[jd1, jd2] * self.con_jac[ic2, jd2] 

        for ic1 in range(n_con):
            for jd1 in range(n_dof):
                for jd2 in range(n_dof):
                    self.constraint_b[ic1] += self.con_jac[ic1, jd1] * self.solver.mass_mat_inv[jd1, jd2] * self.solver.dof_state.qf_smooth[jd2] 
    ...
    def solve_system(self, Gammak: float, n_con : int, n_dof : int):
        for it in ti.static(range(self.n_iters)):
            ...
            # projection
            for pi in range(n_con):
                self.x[pi] = ti.math.sign(self.x1[pi]) * ti.max(0., ti.abs(self.x1[pi]) - self.lamda)


    @ti.kernel
    def project_constraint_force(self, n_con : int, n_dof : int):
        for jd1 in range(n_dof):
            self.solver.dof_state.qf_constraint[jd1] = 0.
    ...
    def generate_collision_shape_pairs(self):
        ...
        # node a, node b
        self.cd_node_que = ti.field(dtype=gs.ti_int, shape=(self.solver.n_geoms_max, self.solver.n_geoms_max, self.len_cd_que, 2))
    ...
    def build_one_tree(self, link):
        ...
        while head < tail:
            ...
            if n == 1:
                ...
                self._box.copy_box(
                    onode + my_id - node_offset, 
                    otri + self.face_buffer[fstart] - face_offset)

            else:
                for j in range(n):
                    self._box.add_box(
                        onode + my_id - node_offset, 
                        otri + self.face_buffer[fstart+j] - face_offset)
                ...
                if n == 2:
                    ...
                    for j in range(n):
                        ...
                        if self._box.inside_left(onode+my_id-node_offset, self.tri_centers[face_id]):
                            ...
                            self.face_buffer[left_idx], self.face_buffer[right_idx] = self.face_buffer[right_idx], self.face_buffer[left_idx]
                            right_idx -= 1
