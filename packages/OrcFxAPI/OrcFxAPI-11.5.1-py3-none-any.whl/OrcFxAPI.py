...
def _supportsPixelsPerInch() -> bool:
    return _checkVersion("11.5a2")


def supportsDrawSupportCylinderAxes() -> bool:
    return _checkVersion("11.5a17")
...
@enum.unique
class ExportedMeshType(enum.IntEnum):
...
class ViewParameters(PackedStructureWithObjectHandles):
    ...
    if supportsDrawSupportCylinderAxes():
        _fields_.extend(
...
class Model(_modelBase):
    ...
    def SaveModelViewMetafileMem(self, viewParameters: Optional[ViewParameters] = None) -> bytes:
        status = ctypes.c_int()
        buffer = Handle()
        bufferLen = ctypes.c_int64()
...
class BitmapCanvas(object):
    ...
    def SaveMem(self, fileFormat: BitmapFileFormat, jpegCompressionQuality: Optional[int] = None) -> bytes:
        saveFormat = BitmapCanvasSaveFormat(fileFormat, jpegCompressionQuality)
        buffer = Handle()
        bufferLen = ctypes.c_int64()
        status = ctypes.c_int()
