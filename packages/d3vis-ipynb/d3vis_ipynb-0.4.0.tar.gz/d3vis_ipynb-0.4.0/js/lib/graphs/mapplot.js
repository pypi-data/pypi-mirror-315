...
export class MapPlot extends BasePlot {
    plot(
        ...
        function reset() {
            GG.transition().duration(750).call(
                zoom.transform,
                d3.zoomIdentity,
        ...
        function loadSchoolsForState(stateName) {
            ...
            schoolsInState.forEach(function (school) {
                addSchoolMarker(parseFloat(school.LATITUDE), parseFloat(school.LONGITUDE), school.NO_ENTIDADE);
                schools.push(school);
            });
    ...
    replot(data, onSelectionChange, width, height, margin) {
        this.clear();
        this.plot(
            data,
            onSelectionChange,
