...
class Agent(AgentGeneric):
    ...
    def get_last_ts(self) -> str:
        ...
        sql_last_ts = f"""SELECT to_char(coalesce(synced_until_ts, '1990-01-01 00:00:00'), 'YYYY-MM-DD HH24:MI:SS')
            FROM {registry} WHERE task_id = '{task_id}' """
        result_set = self.hub.run(self.target_alias, sql_last_ts)
    ...
    def do_action(self, task_id, action_def, task_dir): # FIXME -- 2 do_Action fn!!! which one is correct??
        ...
        if not result_set:
            logging.warning(f"No data from {self.route_alias}")
            return True
        ...
        while True:
            ...
            if destination == 'direct':
                ...
                for jarjekorranumber, this_row in enumerate(rs):
                    ...
                    if method == 'ts':
                        sql_upd_ts = f'''UPDATE bis.bis_registry SET synced_until = '{last_ts}' WHERE task_id = '{task_id}' '''
                        bm.run(idx_trg, sql_upd_ts, False, True) # teeme iga kirje järel, see aeglustab, aga võimaldab katkiminemisi paremini hallata
                    esimene = False
