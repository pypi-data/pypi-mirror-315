...
from dapu.perks import get_dirs_with_definition, load_task_id_list_from_file, convert_to_bool, is_file_inside_git_area, load_yaml
...
class DapuRegistrar(DapuProcess):
    ...
    def run(self):
        logging.info(f"Working directory is {self.context.work_dir}")
        counter = self.refresh_registry() # 
        logging.info(f"Count of added or changed tasks is {counter}")
    ...
    def check_deletions_and_delete(self):
        ...
        logging.debug(f'Is work_dir inside GIT area: {inside_git_area}')
        please_delete = not inside_git_area
        deletion_marker_file: str = self.context.full_name_from_conf(self.context.FILENAME_FOR_DELETION)
        task_list_to_delete: list[str] | None = load_task_id_list_from_file(deletion_marker_file, please_delete)
    ...
    def analyze_project_sub_dirs(self) -> list[tuple[str, str]]:
        ...
        task_dirs: list[str] | None = get_dirs_with_definition(self.context.work_dir, self.context.FILENAME_DEFINITION_FILE)
        if not task_dirs: # both None and [] must have same effect
            halt(81, "Folders and files with definitions are missing")
        
        logging.info(f"Found {len(task_dirs)} possible tasks")
    ...
    def refresh_registry(self) -> int:
        ...
        for task_id_with_hash in task_list_active:
            ...
            if tester is None:
                logging.error(f"Skiping non-valid task ID {task_id}")
                continue
            
            try:
                counter += self.refresh_registry_with_task(task_id, task_hash) # +0 or +1 or exception
            except Exception as e1:
                logging.error(f"Task {task_id} has problems, skiping - {e1}")
                print(e1)
    ...
    def pull_task_data(self, task_id: str) -> tuple[int|None, str, int]:
        registry = self.context.find_registry_table_full_name('registry')
        sql = f"""SELECT id, coalesce(def_hash, ''), coalesce(table_version, 0) 
            FROM {registry} r 
            WHERE r.task_id = '{task_id}'"""
        result_set = self.context.target(sql)
        if len(result_set) == 0:
            return (None, '', 0)
        return result_set[0] # listi esimese elemendi, tuple kolme elemendiga (id on vajalik lihtsalt olemasolu lipuna)


    @DapuProcess.task_id_eventlog(flag='REGISTER')
    def add_task_to_registry(self, task_id: str) -> int|None: 
        """
        ...
        """
        logging.info(f'lisamine : {task_id}')
        registry = self.context.find_registry_table_full_name('registry')
        ...
        try:
            ...
            logging.error(f"during registering {e1}")
            return None
    ...
    def check_for_route(self, route_code, create_if_missing: bool = True) -> bool:
        ...
        if route_dir is None:
            logging.error(f"Route {route_code} don't have own folder")
            return False
        ...
        result_set = self.context.target(sql)
        if len(result_set) == 0:
            logging.warning(f"Route '{route_code}' doesn't exists in table {route}")
    ...
    def add_route(self, route_code: str, route_dir: str) -> bool:
        ...
        if not route_def:
            logging.error(f"New route '{route_code}' doesn't have route.yaml file with data to add to table")
            return False
        ...
        logging.info(f"Route '{route_code}' is now registered")
        return True
        ...
        if route_alias: # if alias is present, then lets check if it is present as well in main conf (by type)
            if route_type == 'sql':
                if not self.context.hub.is_profile_exists(route_alias):
                    logging.error(f"SQL route alias {route_alias} is not amoungst known connection profiles")
                    return False
            if route_type == 'file':
                ...
            if route_type == 'http':
    ...
    def eliminate_idle_from_agenda(self, task_id): # same fucntion is in Manager, can we do some refactoring..
        ...
        return self.context.target(sql, True)
    ...
    def check_and_perform_versioning(self, task_id: str, start_from_version: int, up_to_version: int|None, safe_versions: list[int]) -> bool:
        ...
        if up_to_version > start_from_version:
            logging.info(f"{task_id} starts versioning {start_from_version} --> {up_to_version}")
            ...
            if not self.task_table_upgrade(task_id, start_from_version):
                logging.error(f"Versioning failed {task_id}: {start_from_version} --> {up_to_version}")
                return False
            ...
            if self.versioning_had_unsafe_steps(safe_versions, range(start_from_version + 1, up_to_version + 1)):
    ...
    def task_table_upgrade(self, task_id: str, start_table_version: int) -> bool:
        ...
        if schema_name is None:
            logging.error(f"Task ID {task_id} is not correct")
            return False

        logging.info(f'Upgrade {schema_name}.{table_name} from ver {start_table_version}')
        
        #if not self.check_for_schema(schema_name, create_if_missing=True):
        #    print(f"ai-vai, skeemi {schema_name} pole üldse mitte")
        ...
        for sub_name in os.listdir(dir_path):   # file on lühike nimi (st ilma pathita)
            logging.info(f'Found {sub_name} in {dir_path}')
        ...
        for sub_name in file_list:
            ...
            logging.info(f'{schema_name}.{table_name} version to {new_ver} from file {sql_file_name}')
            sql = ''
            with open(sql_file_name, 'r+', encoding="utf-8") as sf:   # SQL failid peavad meil olema UTF-8 kodeeringus!
                sql = sql + sf.read()
    ...
    def update_task_hash(self, task_id, task_hash):
        ...
        logging.info(f"Tasks {task_id} definitions hash in registry updated")
    ...
    def update_dependencies(self, task_id, depend_list: list[str]) -> None:
        ...
        for master in depend_list:
            ...
            if master_route is None: # on any problems just skip it (TODO: less strict - lets allow 2-part (schema+table) reference too?)
                continue
        ...
        logging.info(f"Dependecies updated for {task_id}")
    ...
    def delete_task_from_registry(self, task_id: str):
        ...
        if result_set:
            logging.info(f"Task {task_id} deleted from registry")
    ...
    def refresh_registry_with_task(self, task_id: str, task_hash: str) -> int:
        ...
        """
        logging.debug(f"Lets analyze {task_id}")
        ...
        if task_file is None: # cannot be, but for separation and fulfillness lets check once more
            logging.error(f"Cannot find file {self.context.FILENAME_DEFINITION_FILE} for task {task_id}")
            return 0

        task_def: dict = load_yaml(None, task_file, empty_as={})
        if not task_def:
            logging.error(f"Definition file {task_file} is empty")
            return 0
        
        status: str = task_def.get('status', 'active')
        if status != 'active': # not in ('draft', 'old'):
            logging.info(f"{task_id} by status {status} don't need to be examined")
            return 0
    ...
    def validate_new_task(self, task_id: str) -> bool:
        ...
        """
        logging.debug(f"Lets validate {task_id}")
        (route_code, target_schema, _) = split_task_id(task_id)
        if route_code is None:
            logging.error(f"Cannot add task {task_id} to registry due wrong task_id (must be 3-parted, dot separated)")
            return False

        # check for existence of schema and DON'T create it - creation may need some additional work (access privileges)
        if not self.check_for_schema(target_schema, create_if_missing=False):
            logging.error(f"Schema is missing: {target_schema}")
            return False
        
        if not self.check_for_route(route_code, create_if_missing=True):
            logging.error(f"Route {route_code} was missing and we were not able to register it")
            return False
