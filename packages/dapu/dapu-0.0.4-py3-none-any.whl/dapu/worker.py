...
from dapu.perks import reconf_logging
...
class DapuWorker(DapuProcess):
    ...
    def run(self) -> int:
        ...
        # returns: -1 at least on error occured, 0 no tasks, 1 all well done - implemented in helper function
        jobs_finished = 0
        jobs_failed = 0
        logging.info(f"Worker: Working directory is {self.context.work_dir}")
        ...
        while (datetime.now() - t1 < timedelta(minutes=self.context.WORKER_NO_NEW_HAUL_AFTER_MINUTES) ): # repeat until worker yet has time
            ...
            if job is None:
                logging.info(f"No{' more' if jobs_finished + jobs_failed > 0 else ''} jobs")
                break
            if job.run(): 
                jobs_finished += 1
            else:
    ...
    def next_job_from_agenda(self) -> DapuJob | None :
        ...
        sql = f"""SELECT a.id
            ...
            WHERE a.status in ({JobStatus.IDLE.value}, {JobStatus.SKIPPED.value}) -- two allowed statuses
    ...
    def mark_as_skipped(self, agenda_id: int) -> list | None:
        agenda = self.context.find_registry_table_full_name('agenda')
        sql = f"""UPDATE {agenda} 
    ...
    def mark_as_lost(self, agenda_id: int) -> list | None:
        agenda = self.context.find_registry_table_full_name('agenda')
        sql = f"""UPDATE {agenda} 
