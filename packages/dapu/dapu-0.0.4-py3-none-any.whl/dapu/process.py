...
from dapu.perks import load_yaml_list_of_dicts
...
class DapuProcess:
    """
    All Dapu processes behave somewho similar way 
    """
    ...
    def __init__(self, args: list | DapuContext | None):
        ...
        if isinstance(args, list):
            ...
            self.context.list_of_profiles = self.load_project_connections() # this function uses self.context
            if not self.context.list_of_profiles:
                halt(5, "No profiles to connections")
            self.context.hub = Hub(self.context.list_of_profiles)
    ...
    def load_project_connections(self) -> list[dict] | None:
        """
        Project needs at least one connection (target database)
        Uses self.context.CONF_SQL_FILE_NAME for yaml-file name
        and validates existence of needed profile name (self.context.TARGET_ALIAS)
        """

        conn_file_full = self.context.full_name_from_conf(self.context.CONF_SQL_FILE_NAME)
        if (list_of_profiles := load_yaml_list_of_dicts(conn_file_full, False)) is None:
            halt(9, f"Cannot work without any connections")

        if len([item for item in list_of_profiles if item['name'] == self.context.TARGET_ALIAS]) == 0:
            halt(10, f"Cannot work without connection named as '{self.context.TARGET_ALIAS}'")
        
        logging.debug(f"Project connections succesfully loaded")
        # replace %VAR% with actual values using getenv or some other way:
