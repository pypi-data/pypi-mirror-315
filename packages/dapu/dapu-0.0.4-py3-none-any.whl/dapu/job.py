...
class DapuJob():
    ...
    def load_agenda_details(self) -> bool: # runs at end of INIT
        ...
        if result_set:
            ...
            logging.error(f"No job with agenda_id {self.agenda_id} amoungst enabled routes for worker. Ignore if does not repeat.")
            return False
    ...
    def prepare_agent(self, action_def: dict) -> AgentGeneric | None:
        ...
        """
        command: str = action_def.get('do', None)
        agent_module: ModuleType = self.prepare_agent_module(command)
    ...
    def find_module_name(self, action_alias: str) -> str:
        ...
        if warning := trusted_agent.get('warning', ''): # eg. deprecation warning
            ...
            logging.warning(f"{action_alias} says {warning}")
