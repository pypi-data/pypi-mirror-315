...
class DapuManager(DapuProcess):
    ...
    def run(self) -> int:
        ...
        logging.info(f"Added {new_count} jobs into Agenda")
        ...
        logging.info(f"There are {total_count} jobs in Agenda{add_text}") # if busy
        return new_count


    @DapuProcess.task_id_eventlog(flag='OUTAGENDA') # returns int
    def eliminate_idle_from_agenda(self, task_id):  # same function is in Registrar, can we do some refactoring..
        """
    ...
    def clear_failed_from_agenda(self):
        """
    ...
    def clear_manager_fantoms(self):
        """
    ...
    def add_jobs_to_agenda(self, commander: str='Manager') -> int:
        ...
        FROM (
            SELECT r.id, r.task_id, r.table_version, r.priority_class
                , coalesce(r.last_end_ts, '2000-01-01 00:00:00'::timestamp) as viimane_lopp
        ...
        """
        result_set_tasks_to_add = self.context.target(sql_reg, True)
        new_count = 0
        ...
        for rec_new_task in result_set_tasks_to_add:  # add to Agenda one-by-one
            ...
            new_count += self.add_task_to_agenda(task_id, commander, False, 0, JobStatus.IDLE.value)
        #
        return new_count


    @DapuProcess.task_id_eventlog(flag='AGENDA') # returns INT
    def add_task_to_agenda(self, task_id: str, commander:str, run_all_checks:bool = True, task_priority_shift: int=0, task_status:int = JobStatus.IDLE.value) -> list[tuple]:
        """
        ...
        try:
            ...
            logging.error(f"Problem with adding task '{task_id}' into Agenda")
            logging.error(f"{e1}")
            return []


    @DapuProcess.task_id_eventlog(flag='DEPENDANT') # returns INT
    def add_dependent_tasks_to_agenda(self, task_id: str, task_priority: int) -> list[tuple]:
        """
    ...
    def count_jobs_in_agenda(self) -> tuple[int, int, int]:
        """
