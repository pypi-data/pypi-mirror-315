...
class BlobNotReadyError(BQApiError):
    ...
    http_code = 202  # expected by resourceview.js
...
class ImageServiceFuture(BQApiError):
    ...
    def __init__(self, timeout_range=None, location="", msg=None):
        self.timeout_range = timeout_range or (1, 10)
        self.location = location
    ...
    def retry(self, v):
        ...
        self.retry_after = min(
            2**self.retry * random.randint(self.timeout_range[0], self.timeout_range[1]), self.max_retry_time
        )
        f = furl.furl(self.location)
        f.query.params["retry"] = self._retry
        self.location = f.url
...
class FutureNotReadyError(BQApiError):
    ...
    def __init__(self, msg, future=None):
        super().__init__(msg)
        self.future = future
...
class TableServiceFuture(BQApiError):
    """Raised when any operation timeout or is already locked
