...
class VQCollection:
    ...
    def __init__(self, sess, from_query=None, from_tags=None):
        self._sess = sess
        self._from_query = (
            [from_query[0], from_query[1], "", from_query[2] if len(from_query) > 2 else {}] if from_query is not None else None
    ...
    def order_by(self, order: list[tuple]) -> "VQCollection":
        ...
        Examples:
            >>> res = sess.select_from_tags("image", tag_query="plate:1234 AND @ts:>=2023-01-01").order_by([("@ts", "asc"), ("@name", "desc")])
        """
    ...
    def __next__(self):
        ...
        if self.match_iter is None:
            if self._from_query is not None:
                query, doc_var, order_clause, kwargs = self._from_query
                self.match_iter = iter(run_sparql_query(
...
class VQBuild(VQModule):
    ...
    def run(
        ...
        **kwargs,
        ...
        iterables = {iterable_node.get_value_str(): iterable_node.get("type", "string") for iterable_node in iterable_nodes}
...
class VQSession(BQSession):
    ...
    def select_from_sparql(self, pattern: str, select_var: str) -> VQCollection:
        ...
        _ = len(res)  # force query to catch any query errors
        return res
