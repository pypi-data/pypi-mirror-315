...
from fujitsu_quantum.config import DEFAULT_RETRY_MAX_ATTEMPTS, Config
    ...
    try:
        ...
        if options.command == "login":
            ...
            try:
                ...
                device_name = exc.get_tokens()["ChallengeParameters"].get("FRIENDLY_DEVICE_NAME", None)
                if device_name is not None:
                    code_prompt = "Enter the 6-digit code generated by your TOTP device ({0}): ".format(device_name)
                else:
                    code_prompt = "Enter the 6-digit code generated by your TOTP device): "
                ...
                FjqAuthBoto3.store_credentials(
                    client_id=Config.auth_sdk_client_id,
                    username=user,
            ...
            FjqAuthBoto3().refresh_auth(force=True)
            ...
            try:
                auth = FjqAuthBoto3()
                expr_time = datetime.strptime(auth.refresh_token_expiration_time, '%Y-%m-%d %H:%M:%S')
                if expr_time < datetime.now():
                    print('Credentials are outdated.\nPlease run `fujitsu-quantum login` to update the credentials.')
                else:
                    print(f'You have valid credentials.\nThe credentials will expire at {expr_time}.')
            except FjqAuthError as e:
                print(e.args[0])

        elif options.command == 'test':
            try:
                # set the retry-max-attempts to a small value to get test results quickly
                ...
                Config.retry_max_attempts = DEFAULT_RETRY_MAX_ATTEMPTS
