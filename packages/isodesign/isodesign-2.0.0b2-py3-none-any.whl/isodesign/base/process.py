...
class Process:
    ...
    def __init__(self):
        ...
        self.arguments_filters = {"fluxes_names": None, 
                                  "kind": None, 
                                  "pathways": None}
    ...
    def clear_tmp_folder(self, output_folder_path):
        """
        Clear the temp folder containing all the files generated by IsoDesign.
        """
       
        shutil.rmtree(Path(f"{output_folder_path}/{self.model_name}_tmp"))
    ...
    def generate_linp_files(self):
        ...
        with open(os.path.join(str(self.output_folder_path), f'{self.model_name}_files_combinations.txt'), 'w', encoding="utf-8") as f:
    ...
    def export_data(self, score_name):
        ...
        res_folder_path = Path(f"{self.output_folder_path}/{score_name}_res")
...
#     command_list = ["--prefix", test.model_name, "--emu", "--ln", "--noopt", "--np=1"]
#     test.influx_simulation(influx_mode="influx_s", param_list=command_list)
    
#     test.generate_summary()
#     # test.save_process_to_file()
#     test.data_filter(pathways=["GLYCOLYSIS"], kind=["NET"])
#     print(test.arguments_filters)
