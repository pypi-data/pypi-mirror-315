...
def register_multiselect_filter(count, key, filter_key):
    """
    This function registers the filters in the session.
    """
    st.session_state[f"{key}_{count}"] = st.session_state[f"selected_{key}_{count}"]
    process_object.arguments_filters[f"{filter_key}"] = st.session_state[f"selected_{key}_{count}"]

def register_multiselect_criteria(count, key, filter_key):
    pass
...
def display_dataframe(count):
    ...
    :param count: number of the score block
    """
    ...
    with st.expander("**Apply a filter**"):
        ...
        if f"flux_{count}" not in st.session_state:
            st.session_state[f"flux_{count}"] = []
        ...
        fluxes_name.multiselect(label="Fluxes_name", 
                                ...
                                default=st.session_state[f"flux_{count}"] if not process_object.arguments_filters["fluxes_names"] else process_object.arguments_filters["fluxes_names"],
                                on_change=register_multiselect_filter,
                                args=(count,"flux", "fluxes_names"))
        
        if f"kind_{count}" not in st.session_state:
            st.session_state[f"kind_{count}"] = []
        ...
        kind.multiselect(label="kind", 
                        ...
                        default=st.session_state[f"kind_{count}"] if not process_object.arguments_filters["kind"] else process_object.arguments_filters["kind"], 
                        on_change=register_multiselect_filter,
                        args=(count,"kind", "kind")
                        )
        
        if f"pathway_{count}" not in st.session_state:
            st.session_state[f"pathway_{count}"] = []
        pathway.multiselect(label="Pathway", 
                            ...
                            on_change=register_multiselect_filter,
                            args=(count,"pathway", "pathways"),
                            default=st.session_state[f"pathway_{count}"] if not process_object.arguments_filters["pathways"] else process_object.arguments_filters["pathways"]) 

    
    # Dataframe filtering method    
    process_object.data_filter(fluxes_names = process_object.arguments_filters["fluxes_names"], 
                               kind = process_object.arguments_filters["kind"], 
                               pathways= process_object.arguments_filters["pathways"])
...
def criteria_block(count):
    ...
    with criteria:
        ...
        if not "method_choice" in st.session_state:
...
st.write(process_object.all_scores)