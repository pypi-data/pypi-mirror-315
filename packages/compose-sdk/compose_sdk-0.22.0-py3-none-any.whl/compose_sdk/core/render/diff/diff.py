...
from ...ui import ComponentReturn, is_interactive_component, INTERACTION_TYPE
...
def diff_static_layouts_recursive(
    ...
    if old_layout["type"] != new_layout["type"] or interactive_id_changed:
    ...
    if (
        ...
        if JSON.stringify(old_model) != JSON.stringify(new_model):
            compressed = Compress.ui_tree(new_layout)
            ...
            return {
                "delete": [],
                "add": {},
                "update": {old_layout["model"]["id"]: compressed_model},
    ...
    for i in range(iterator_count):
        ...
        if old_child is not None and new_child is not None:
            ...
            update_obj = {**update_obj, **child_diff["update"]}
    ...
    if JSON.stringify(old_model) != JSON.stringify(new_model) or children_did_change:
