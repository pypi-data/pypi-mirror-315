...
def tooltip_helper(desc):
    ...
    if desc.inputs:
        ...
        tooltip.append(_tr.m[439, "Inputs:<ul>{0}</ul>"].format(inputs))
    else:
        tooltip.append(_tr.m[440, "No inputs"])

    if desc.outputs:
        outputs = "".join(inputs_fmt.format(name=out.name,
                                            class_name=type_str(out.types))
                          for out in desc.outputs)
        tooltip.append(_tr.m[441, "Outputs:<ul>{0}</ul>"].format(outputs))
    else:
        tooltip.append(_tr.m[442, "No outputs"])
