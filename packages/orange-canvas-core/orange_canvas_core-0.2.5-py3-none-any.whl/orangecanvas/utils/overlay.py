...
class NotificationMessageWidget(QWidget):
    ...
    def __init__(self, parent=None, icon=QIcon(), title="", text="", wordWrap=False,
                 ...
                 rejectLabel=_tr.m[449, "No"], **kwargs):
        super().__init__(parent, **kwargs)
        self._title = title
        self._text = text
        self._icon = QIcon()
    ...
    def addButton(self, button, *rolearg):
        ...
        if isinstance(button, QAbstractButton):
            if len(rolearg) != 1:
                raise TypeError(("Wrong number of arguments for " + "addButton(QAbstractButton, role)"))
            role = rolearg[0]
        elif isinstance(button, StandardButton):
            if rolearg:
                raise TypeError(("Wrong number of arguments for " + "addButton(StandardButton)"))
            stdbutton = button
            ...
            if len(rolearg) != 1:
                raise TypeError(("Wrong number of arguments for " + "addButton(str, ButtonRole)"))
            role = rolearg[0]
...
class NotificationWidget(QWidget):
    ...
    def __init__(self, parent, title="", text="", textFormat=Qt.AutoText,
                 ...
                 standardButtons=NoButton, acceptLabel="Ok", rejectLabel=_tr.m[450, "No"],  **kwargs):
        super().__init__(parent, **kwargs)
