...
class NewLinkAction(UserInteraction):
    ...
    def mousePressEvent(self, event):
        ...
        if anchor_item is not None and event.button() == Qt.LeftButton:
            ...
            event.accept()
            self.postQuickTip(
                self.tr((_tr.m[340, '<h3>Create new link</h3>'] + (_tr.m[341, '<p>Drag a link to an existing node or release on '] + (_tr.m[342, 'an empty spot to create a new node.</p>'] + (_tr.m[343, '<p>Hold Shift when releasing the mouse button to '] + _tr.m[344, 'edit connections.</p>']))))
...
def edit_links(
    ...
    dlg = EditLinksDialog(parent, windowTitle=_tr.m[345, "Edit Links"])
...
class EditNodeLinksAction(UserInteraction):
    ...
    def edit_links(self, initial_links=None):
        ...
        dlg = EditLinksDialog(self.document, windowTitle=_tr.m[346, "Edit Links"])
        ...
        if rval == EditLinksDialog.Accepted:
            ...
            stack = self.document.undoStack()
            stack.beginMacro(_tr.m[347, "Edit Links"])
...
class NewArrowAnnotation(UserInteraction):
    ...
    def start(self):
        ...
        self.postQuickTip(
            self.tr((_tr.m[348, '<h3>New arrow annotation</h3>'] + _tr.m[349, '<p>Click and drag to create a new arrow annotation</p>'])
...
class NewTextAnnotation(UserInteraction):
    ...
    def start(self):
        ...
        self.document.view().setCursor(Qt.CrossCursor)
        self.postQuickTip(
            self.tr((_tr.m[350, '<h3>New text annotation</h3>'] + (_tr.m[351, '<p>Click (and drag to resize) on the canvas to create '] + (_tr.m[352, 'a new text annotation item.</p>'] + (_tr.m[353, '<p>Right click on the annotation to change how it is '] + _tr.m[354, 'rendered (Markdown, HTML, ...).</p>']))))
...
class ResizeTextAnnotation(UserInteraction):
    ...
    def start(self) -> None:
        super().start()
        self.postQuickTip(
            self.tr((_tr.m[355, '<h3>Edit Text Annotation</h3>'] + (_tr.m[356, '<p>Drag control points to resize the annotation.</p>'] + (_tr.m[357, '<p>Right click on the annotation to change how it is '] + _tr.m[358, 'rendered (Markdown, HTML, ...).</p>']))))
        )
    ...
    def entryPoints(self) -> Iterable[Tuple['EntryPoint', 'DropHandler']]:
        ...
        for ep, value in ep_iter:
            if not issubclass(value, DropHandler):
                log.error(
                    (f"{ep} yielded {type(value)}, expected a " + f"{DropHandler} subtype")
                )
                continue
            try:
                handler = value()
        ...
        if len(self.__accepts) == 1:
            ...
            class Title(QWidgetAction):
                def createWidget(self, parent):
                    return QLabel(_tr.m[359, "<b>Select a widget</b>"], parent, margin=2)
