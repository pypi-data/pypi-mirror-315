...
class UserSettingsDialog(QMainWindow):
    ...
    def __setupUi(self):
        ...
        # General Tab
        tab = QWidget()
        self.addTab(tab, self.tr(_tr.m[219, "General"]),
                    toolTip=self.tr(_tr.m[220, "General Options"]))
        ...
        if languages:
            langlay = QHBoxLayout()
            label = QLabel(
                _tr.m[221, "Changes will take effect on next application startup."])
            label.setHidden(True)

            cm_lang = QComboBox(
                objectName="combo-language",
                toolTip=self.tr(_tr.m[222, "Select the application language."])
            )
            ...
            form.addRow(self.tr(_tr.m[223, "Language"]), langlay)
        ...
        cb_anim = QCheckBox(
            self.tr(_tr.m[224, "Enable node animations"]),
            objectName="enable-node-animations",
            toolTip=self.tr((_tr.m[225, "Enable shadow and ping animations for nodes "] + _tr.m[226, "in the workflow."]))
        )
        cb_anchors = QCheckBox(
            self.tr(_tr.m[227, "Open anchors on hover"]),
            objectName="open-anchors-on-hover",
            toolTip=self.tr(
                (_tr.m[228, "Open/expand node anchors on mouse hover (if unchecked the "] + _tr.m[229, "anchors are expanded when Shift key is pressed)."])
            ),
        )
        ...
        form.addRow(self.tr(_tr.m[230, "Nodes"]), nodes)
        ...
        cb_show = QCheckBox(
            self.tr(_tr.m[231, "Show channel names between widgets"]),
            objectName="show-channel-names",
            toolTip=self.tr((_tr.m[232, "Show source and sink channel names "] + _tr.m[233, "over the links."]))
        )
        ...
        form.addRow(self.tr(_tr.m[234, "Links"]), links)
        ...
        cb1 = QCheckBox(self.tr(_tr.m[235, "Open on double click"]),
                        toolTip=self.tr((_tr.m[236, "Open quick menu on a double click "] + _tr.m[237, "on an empty spot in the canvas"])))

        cb2 = QCheckBox(self.tr(_tr.m[238, "Open on right click"]),
                        toolTip=self.tr((_tr.m[239, "Open quick menu on a right click "] + _tr.m[240, "on an empty spot in the canvas"])))

        cb3 = QCheckBox(self.tr(_tr.m[241, "Open on space key press"]),
                        toolTip=self.tr((_tr.m[242, "Open quick menu on Space key press "] + _tr.m[243, "while the mouse is hovering over the canvas."])))

        cb4 = QCheckBox(self.tr(_tr.m[244, "Open on any key press"]),
                        toolTip=self.tr((_tr.m[245, "Open quick menu on any key press "] + _tr.m[246, "while the mouse is hovering over the canvas."])))

        cb5 = QCheckBox(self.tr(_tr.m[247, "Show categories"]),
                        toolTip=self.tr((_tr.m[248, "In addition to searching, allow filtering "] + _tr.m[249, "by categories."])))
        ...
        form.addRow(self.tr(_tr.m[250, "Quick menu"]), quickmenu)

        startup = QWidget(self, objectName="startup-group")
        startup.setLayout(QVBoxLayout())
        startup.layout().setContentsMargins(0, 0, 0, 0)

        cb_splash = QCheckBox(self.tr(_tr.m[251, "Show splash screen"]), self,
                              objectName="show-splash-screen")

        cb_welcome = QCheckBox(self.tr(_tr.m[252, "Show welcome screen"]), self,
                               objectName="show-welcome-screen")

        cb_crash = QCheckBox(self.tr(_tr.m[253, "Load crashed scratch workflows"]), self,
        ...
        form.addRow(self.tr(_tr.m[254, "On startup"]), startup)

        toolbox = QWidget(self, objectName="toolbox-group")
        toolbox.setLayout(QVBoxLayout())
        toolbox.layout().setContentsMargins(0, 0, 0, 0)

        exclusive = QCheckBox(self.tr(_tr.m[255, "Only one tab can be open at a time"]))

        self.bind(exclusive, "checked", "mainwindow/toolbox-dock-exclusive")

        toolbox.layout().addWidget(exclusive)

        form.addRow(self.tr(_tr.m[256, "Tool box"]), toolbox)
        tab.setLayout(form)

        # Style tab
        tab = StyleConfigWidget()
        self.addTab(tab, self.tr(_tr.m[257, "&Style"]), toolTip=_tr.m[258, "Application style"])
        self.bind(tab, "selectedStyle_", "application-style/style-name")
        self.bind(tab, "selectedPalette_", "application-style/palette")

        # Output Tab
        tab = QWidget()
        self.addTab(tab, self.tr(_tr.m[259, "Output"]),
                    toolTip=_tr.m[260, "Output Redirection"])

        form = FormLayout()

        combo = QComboBox()
        combo.addItems([self.tr(_tr.m[261, "Critical"]),
                        self.tr(_tr.m[262, "Error"]),
                        self.tr(_tr.m[263, "Warn"]),
                        self.tr(_tr.m[264, "Info"]),
                        self.tr(_tr.m[265, "Debug"])])
        self.bind(combo, "currentIndex", "logging/level")
        form.addRow(self.tr(_tr.m[266, "Logging"]), combo)

        box = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        cb1 = QCheckBox(self.tr(_tr.m[267, "Open in external browser"]),
                        objectName="open-in-external-browser")
        self.bind(cb1, "checked", "help/open-in-external-browser")
        layout.addWidget(cb1)
        box.setLayout(layout)
        form.addRow(self.tr(_tr.m[268, "Help window"]), box)
        ...
        self.addTab(tab, _tr.m[269, "Categories"])

        # Add-ons Tab
        tab = QWidget()
        self.addTab(tab, self.tr(_tr.m[270, "Add-ons"]),
                    toolTip=_tr.m[271, "Settings related to add-on installation"])
        ...
        cb_conda_install = QCheckBox(self.tr(_tr.m[272, "Install add-ons with conda"]), self,
        ...
        form.addRow(self.tr("Pip"), QLabel(_tr.m[273, "Pip install arguments:"]))
        ...
        # Network Tab
        tab = QWidget()
        self.addTab(tab, self.tr(_tr.m[274, "Network"]),
                    toolTip=_tr.m[275, "Settings related to networking"])
...
class StyleConfigWidget(QWidget):
    DisplayNames = {
        "windowsvista": _tr.m[276, "Windows (default)"],
        "macintosh": _tr.m[277, "macOS (default)"],
    ...
    def __init__(self, *args, **kwargs):
        ...
        # on application star.
        styles = [(_tr.m[278, "Default"], "")] + styles
        ...
        colors_cb.addItem(_tr.m[279, "Default"], userData="")
        ...
        colors_cb.addItem(_tr.m[280, "Dark"], userData="dark")

        form.addRow(_tr.m[281, "Style"], style_cb)
        form.addRow(_tr.m[282, "Color theme"], colors_cb)
        label = QLabel(
            _tr.m[283, "<small>Changes will be applied on next application startup.</small>"],
            enabled=False,
        )
