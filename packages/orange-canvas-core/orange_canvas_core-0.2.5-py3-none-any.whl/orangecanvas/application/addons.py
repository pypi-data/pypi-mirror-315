...
class AddonManagerDialog(QDialog):
    ...
    def __init__(self, parent=None, acceptDrops=True, *,
        ...
        self.__addmore = QPushButton(
            self.tr(_tr.m[12, "Add more..."]),
            toolTip=self.tr(_tr.m[13, "Add an add-on not listed below"]),
            autoDefault=False
        )
    ...
    def start(self, config):
        ...
        progress.setLabelText(
            self.tr(_tr.m[14, "Retrieving package list"])
        )
    ...
    def __on_query_done(self, f):
        ...
        def network_warning(exc):
            ...
            log.error(
                _tr.m[15, "Error fetching package list"],
                exc_info=(etype, exc, tb)
            )
            message_warning(
                _tr.m[16, "There's an issue with the internet connection."],
                title=_tr.m[17, "Error"],
                informative_text=
                    (_tr.m[18, "Please check you are connected to the internet.\n\n"] + (_tr.m[19, "If you are behind a proxy, please set it in Preferences "] + _tr.m[20, "- Network."])),
                details=
    ...
    def runQueryAndAddResults(
        ...
        progress = self.progressDialog()
        progress.setLabelText(_tr.m[21, "Running query"])
    ...
    def __run_add_package_dialog(self):
        self.__add_package_by_name_dialog = dlg = QDialog(
            self, windowTitle=_tr.m[22, "Add add-on by name"],
        )
        ...
        nameentry = QLineEdit(
            placeholderText=_tr.m[23, "Package name"],
            toolTip=(_tr.m[24, "Enter a package name as displayed on "] + _tr.m[25, "PyPI (capitalization is not important)"]))
        nameentry.setMinimumWidth(250)
        form.addRow(_tr.m[26, "Name:"], nameentry)
    ...
    def __show_error_for_query(self, text, error_details):
        message_error(text, title=_tr.m[28, "Error"], details=error_details)
    ...
    def __on_add_query_finish(self, f):
        ...
        try:
            ...
            log.error(_tr.m[29, "Query error:"], exc_info=True)
            error_text = _tr.m[30, "Failed to query package index"]
            error_details = traceback.format_exc()
        else:
            not_found = [r.queryname for r in result if r.installable is None]
            if not_found:
                error_text = "".join([
                    _tr.m[31, "The following packages were not found:<ul>"],
    ...
    def progressDialog(self):
        ...
        if self.__progress is None:
            self.__progress = QProgressDialog(
                self,
                minimum=0, maximum=0,
                labelText=self.tr(_tr.m[32, "Retrieving package list"]),
                sizeGripEnabled=False,
                windowTitle=_tr.m[33, "Progress"]
            )
    ...
    def __accepted(self):
        ...
        if core_upgrade:
            ...
            title = _tr.m[34, "Warning"]
            text = _tr.m[35, "This action will upgrade some core packages:\n"]
            text += "\n".join(sorted(core_upgrade))
            msg_box = QMessageBox(icon, title, text, buttons, self)
            msg_box.setInformativeText(_tr.m[36, "Do you want to continue?"])
        ...
        if steps:
            ...
            progress.show()
            progress.setLabelText(_tr.m[37, "Installing"])
    ...
    def __on_installer_error(self, command, pkg, retcode, output):
        ...
        message_error(
            _tr.m[38, "An error occurred while running a subprocess"], title=_tr.m[39, "Error"],
            informative_text=_tr.m[40, "{} exited with non zero status."].format(command),
            details="".join(output),
            parent=self
        )
        self.reject()
    ...
    def __on_installer_finished(self):
        ...
        def message_restart(parent):
            ...
            buttons = QMessageBox.Ok | QMessageBox.Cancel
            title = _tr.m[41, 'Information']
            text = (_tr.m[42, '{} needs to be restarted for the changes to take effect.']
                    .format(name))
            msg_box = QMessageBox(icon, title, text, buttons, parent)
            msg_box.setDefaultButton(QMessageBox.Ok)
            msg_box.setInformativeText(_tr.m[43, 'Press OK to restart {} now.']
                                       .format(name))
            msg_box.button(QMessageBox.Cancel).setText(_tr.m[44, 'Close later'])
            return msg_box.exec()
        ...
        if QMessageBox.Ok == message_restart(self):
            ...
            def restart():
                ...
                if any(w.isVisible() for w in windows):  # if a window close was cancelled
                    ...
                    QMessageBox(
                        text=_tr.m[45, "Restart Cancelled"],
                        informativeText=_tr.m[46, "Changes will be applied on {}'s next restart"]
                                        .format(name),
                        icon=QMessageBox.Information
                    ).exec()
                else:
