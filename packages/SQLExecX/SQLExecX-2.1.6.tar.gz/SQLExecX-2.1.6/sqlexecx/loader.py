...
class Loader:
    ...
    def to_df(self, df_type: str = "auto"):
        """
        transform to DataFrame of pandas or polars.

        :param df_type: auto, pandas or polars
        :return DataFrame of pandas or polars. if df_type is 'auto', use polars by default. if polars not installed, use pandas.
        """
        assert df_type in ["auto", "pandas", "polars"], "df_type must be 'auto', 'pandas' or 'polars'"
        if df_type == "auto":
            use_polars = True
            try:
                import polars as pd
