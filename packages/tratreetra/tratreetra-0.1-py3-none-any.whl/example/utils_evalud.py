...
# Original: https://raw.githubusercontent.com/UniversalDependencies/tools/refs/heads/master/eval.py
...
def load_conllu(file, path, treebank_type):
    ...
    while True:
            ...
            def process_word(word):
                ...
                if word.parent is None:
                    ...
                    if head < 0 or head > len(ud.words) - sentence_start:
                        raise UDError("HEAD '{}' points outside of the sentence that ends at line {}".format(
                            _encode(word.columns[HEAD]), line_idx))
                    if head:
            ...
            for word in ud.words[sentence_start:]:
                ...
                for (head, steps) in word.columns[DEPS]:  # (3,['conj:en','obj:voor'])
                    ...
                    if '.' in head:
                        if treebank_type.get('no_empty_nodes', False):
                            raise UDError(
                                "The collapsed CoNLL-U file still contains references to empty nodes at line {}: {}".format(
                                    line_idx, _encode(line)))
                        else:
                            continue
                    hd = int(head)
                ...
                if treebank_type.get('no_shared_dependents_in_coordination', False):  # enhancement  3
                    ...
                    for (hd, steps) in enhanced_deps:
                        ...
                        for (hd2, steps2) in enhanced_deps:
                            if steps == steps2 and hd2 == word.columns[
                                HEAD] and hd != hd2:  # checking only for ud_hd here, check for ud_dep as well?
                                duplicate = 1
                        if not (duplicate):
                ...
                if treebank_type.get('no_external_arguments_of_relative_clauses', False):  # enhancement 5
                    ...
                    for (parent, steps) in enhanced_deps:
                        ...
                        elif (parent and parent.columns[DEPREL].startswith('acl') and int(
                                parent.columns[HEAD]) == position - sentence_start):
        ...
        columns = line.split("\t")
        if len(columns) != 10:
            raise UDError("The CoNLL-U line does not contain 10 tab-separated columns at line {}: '{}'".format(line_idx,
                                                                                                               _encode(
                                                                                                                   line)))
        ...
        if "." in columns[ID]:
            ...
            if treebank_type.get('no_empty_nodes', False):
                raise UDError("The collapsed CoNLL-U line still contains empty nodes at line {}: {}".format(line_idx,
                                                                                                            _encode(
                                                                                                                line)))
            else:
                continue
        ...
        if "-" in columns[ID]:
            ...
            while words_found < words_expected:
                ...
                if len(word_columns) != 10:
                    raise UDError(
                        "The CoNLL-U line does not contain 10 tab-separated columns at line {}: '{}'".format(line_idx,
                                                                                                             _encode(
                                                                                                                 word_line)))
                if "." in word_columns[ID]:
                    if treebank_type.get('no_empty_nodes', False):
                        raise UDError(
                            "The collapsed CoNLL-U line still contains empty nodes at line {}: {}".format(line_idx,
                                                                                                          _encode(
                                                                                                              line)))
                    else:
                        continue
...
def evaluate(gold_ud, system_ud):
    ...
    if gold_ud.characters != system_ud.characters:
        ...
        if gtindex > 0:
            ...
            nexttokens = ' '.join(
                [''.join(gold_ud.characters[t.start:t.end]) for t in gold_ud.tokens[gtindex:gtindex + nnext]])
            gtokenreport = "File '{}':\n".format(gold_ud.path)
            gtokenreport += "  Token no. {} on line no. {} is the last one with all characters reproduced in the other file.\n".format(
                gtindex, gold_ud.tokens[gtindex - 1].line)
        ...
        if stindex > 0:
            ...
            prevtokens = ' '.join(
                [''.join(system_ud.characters[t.start:t.end]) for t in system_ud.tokens[nfirst:stindex]])
            nexttokens = ' '.join(
                [''.join(system_ud.characters[t.start:t.end]) for t in system_ud.tokens[stindex:stindex + nnext]])
            stokenreport = "File '{}':\n".format(system_ud.path)
            stokenreport += "  Token no. {} on line no. {} is the last one with all characters reproduced in the other file.\n".format(
                stindex, system_ud.tokens[stindex - 1].line)
...
def build_evaluation_table(evaluation, verbose, counts, enhanced):
    ...
    if not verbose and not counts:
        ...
        text.append("-----------+-----------+-----------+-----------+-----------")
        metrics = ["Tokens", "Sentences", "Words", "UPOS", "XPOS", "UFeats", "AllTags", "Lemmas", "UAS", "LAS", "CLAS",
                   "MLAS", "BLEX"]
        if enhanced:
            metrics += ["ELAS", "EULAS"]
