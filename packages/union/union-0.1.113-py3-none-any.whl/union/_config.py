...
_VANITY_UNION_URL: str = "serverless.union.ai"
...
def _get_endpoint_for_login(*, host: Optional[str] = None, serverless: bool = False) -> str:
    ...
    if endpoint == _VANITY_UNION_URL:
        return _DEFAULT_GCP_SERVERLESS_ENDPOINT
    return endpoint
...
def _should_default_to_union_semantics() -> bool:
    if _UNION_CONFIG.is_direct_union_cli_call:
        return True
    for ev in _UNION_CONFIG_ENV_VARS + _UNION_API_KEY_ENV_VARS + [_UNION_SERVERLESS_ENDPOINT_ENV_VAR]:
        if os.getenv(ev):
            return True
    return False
