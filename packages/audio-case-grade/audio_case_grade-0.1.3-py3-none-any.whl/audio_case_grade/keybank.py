...
def _clean_keybank(keybank: DataFrame, abbrev_map: Mapping[str, str]) -> DataFrame:
    ...
    def clean_cell(cell):
        ...
        if isinstance(cell, str):
            ...
            cleaned_lemma = [
                " ".join(_lemmatize(word, lemma) for word in key.split())
...
def get_keybank(
    ...
    if keybank.shape[1] != EXPECTED_COLS:
        raise ValueError(
            "DataFrame has incorrect dimensions. Expected Number of Columns: "
            + f"({EXPECTED_COLS}, got {keybank.shape[1]})"
        )
    abbrev = (
        get_abbreviations_csv()
