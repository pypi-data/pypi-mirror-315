...
def _objectives(text: str, keybank: Series) -> Objectives:
    ...
    dl = count_keywords(text, keybank.iloc[6])
    di = count_keywords(text, keybank.iloc[7])
...
def _plans(text: str, keybank: Series) -> Plans:
    ...
    tx = count_keywords(text, keybank.iloc[8])
    consults = count_keywords(text, keybank.iloc[9])
    interventions = count_keywords(text, keybank.iloc[10])
...
def _assessments(text: str, keybank: Series) -> Assessments:
    ...
    dx = count_keywords(text, keybank.iloc[11])
    ddx = count_keywords(text, keybank.iloc[12])
...
def _foundations(text: str, keybank: Series) -> Foundations:
    ...
    root = count_keywords(text, keybank.iloc[13])
