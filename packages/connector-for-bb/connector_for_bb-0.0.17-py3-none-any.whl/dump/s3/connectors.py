...
ZIP_FORMATS = {"zip", "bz2", "zstd", "xz", "tar"}
...
class S3UploadDF(S3Upload):
    def __init__(
        ...
        # df_compression_format="zip",
        compresslevel: int = 4,
        **kwargs,
    ):
        super().__init__(**kwargs)
        self.df_compression = df_compression
        self.df_compression_format = "zip"
        self.compresslevel = compresslevel

        if self.df_compression:
            if self.df_compression_format not in ZIP_FORMATS:
                raise ValueError(
                    f"Unkowm df_compression_format: {self.df_compression_format}, avaliable formats: {ZIP_FORMATS}"
                )
            self._df_file_system = TemporaryFileSystem(
                file_format=self.df_compression_format
            )
        else:
    ...
    def __get_file_name_no_ext(self, file_name: str):
        ...
        if extension in ZIP_FORMATS:
            logging.info(
                "In filename already specified extension, that will be ignored."
            )
            self.df_compression_format = extension
    ...
    def upload_to_s3_df(self, df: PandasDataFrame, file_name: str):
        ...
        if not self.df_compression:
            ...
            if file_extension in ZIP_FORMATS:
                raise ValueError(
            ...
            compression_opts = dict(
                method=self.df_compression_format,
                archive_name=f"{file_name_no_ext}.csv",
