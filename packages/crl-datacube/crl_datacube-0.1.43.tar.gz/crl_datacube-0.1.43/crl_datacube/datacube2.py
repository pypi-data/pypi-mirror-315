...
class DataCube:
    ...
    def _process_single_tile(self, idx, da, var, group, boxbuff=0.1):
        """Helper function to process a single tile for intake_data"""
        print("First line of processing is running now")
        i = self.prep_single_tile(idx, da, boxbuff=boxbuff)
        if i:
            idx, da_clipped = i
    ...
    def prep_single_tile(self, idx: tuple[int, int], da: xr.DataArray, boxbuff: int = 0.1):
        tile = self.tiles[idx]
        bbox = tile.boundingbox
    ...
    def intake_data(
        ...
        if parallelism > 1:
            with multiprocessing.Pool(processes=parallelism, initializer=quiet_logging) as pool:
                # Use partial to create a function with bound arguments
                from functools import partial
                process_func = partial(self._process_single_tile, da=da, var=var, group=group, boxbuff=boxbuff)
                results = list(tqdm(pool.imap_unordered(process_func, idxs), total=len(idxs)))
        else:
            for idx in tqdm(idxs, total=len(idxs)):
                self._process_single_tile(idx, da, var, group)
