...
def _is_localhost(uri: str):
    uri_split = urllib.parse.urlsplit(uri)
    return uri_split.hostname in ("localhost", "127.0.0.1")
...
def _offline_query_inputs_to_parquet(
    ...
    for single_input, single_input_times in offline_query_inputs:
        ...
        if not TS_COL_NAME in input_table.column_names:
...
"""
    ...
    def _do_request_inner(
        ...
        *,
        ...
        if "X-Chalk-Branch-Id" in headers and metadata_request and _is_localhost(host):
            # Route directly to the local branch server
...
"""
    ...
    def query(
        ...
        encoded_value_metrics_tag_by_features = encode_outputs(value_metrics_tag_by_features)
        if branch is ...:
            branch = self._branch
        now_str = None
        if now is not None:
    ...
    def multi_query(
        ...
        for query in queries:
            ...
            outputs = encode_outputs(query.output)
            encoded_value_metrics_tag_by_features = encode_outputs(query.value_metrics_tag_by_features)
    ...
    def offline_query(
        ...
        if input is None:
            ...
            if upload_input_as_table or (len(multi_input) > 0 and len(multi_input[0]) > 100) or num_shards:
    ...
    def _validate_test_stream_resolver_inputs(
        ...
        message_bodies_str: List[str] | None
        message_bodies_bytes: List[str] | None
        if bodies_are_bytes:
            message_bodies_bytes = [
                base64.b64encode(cast(bytes, message)).decode("utf-8") for message in message_bodies
            ]
            message_bodies_str = None
        else:
            if isinstance(message_bodies[0], BaseModel):
                message_bodies_str = []
                for message in message_bodies:
                    assert isinstance(message, BaseModel), "all message_bodies must be of same type"
                    message_bodies_str.append(message.json())
            else:
                message_bodies_str = [str(message) for message in message_bodies]
            message_bodies_bytes = None
        payloads: List[StreamResolverTestMessagePayload] = [
            StreamResolverTestMessagePayload(
                key=message_keys[i],
                message_str=message_bodies_str[i] if message_bodies_str else None,
                message_bytes=message_bodies_bytes[i] if message_bodies_bytes else None,
    ...
    def plan_query(
        ...
        encoded_inputs = encode_outputs(input)
        outputs = encode_outputs(output)
        if branch is ...:
            branch = self._branch
