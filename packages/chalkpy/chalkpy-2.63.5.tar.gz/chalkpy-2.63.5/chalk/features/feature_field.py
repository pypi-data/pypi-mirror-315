...
class FeatureNotFoundException(ValueError):
    def __init__(self, fqn: str) -> None:
        super().__init__(f"Feature '{fqn}' was not found in the registry. Please ensure that '{fqn}' is defined on your @features class and imported into this file.")
        ...
        if self._join_type == "has_one":
            if self.is_has_many:
                ...
                self.lsp_error_builder.add_diagnostic(
                    message=(
                        f"The attribute '{self.attribute_name}' on the feature class '{self.features_cls.__name__}' "
                        f"is specified as 'has_one' but its type annotation is a 'DataFrame'. "
                        f"Please remove the 'DataFrame' type if you'd like to define a 'has_one' relationship."
                    ),
        ...
        if self._join_type == "has_many":
            if self.is_has_one:
                ...
                self.lsp_error_builder.add_diagnostic(
                    message=(
                        f"The attribute '{self.attribute_name}' on the feature class '{self.features_cls.__name__}' "
                        f"is specified as 'has_many' but its type annotation is a feature class. "
                        f"Please use the 'DataFrame' type around the feature class if you'd like to define a 'has_many' relationship."
                    ),
    ...
    def _validate_filter(self, filter: Filter) -> Filter:
        ...
        def _validate_filter_internal(filter: Filter) -> Filter:
            ...
            if isinstance(filter.lhs, Filter) and isinstance(filter.lhs, Filter) and filter.operation == "and": # pyright: ignore[reportUnnecessaryIsInstance]
                if not self.is_has_one:
                    assert self.features_cls is not None
                    self.lsp_error_builder.add_diagnostic(
                        message=(
                            f"The attribute '{self.attribute_name}' on the feature class '{self.features_cls.__name__}' "
                            f"has a join function that is incorrectly configured. "
                            f"Joins with multiple clauses, e.g. `(a.id1 == b.id1) & (a.id2 == b.id2)`, are not supported for has-many features. "
                            f"Ensure that the join is a lambda function that sets features from two feature classes as "
                            f"equal. For example, ' = has_one(lambda: "
                            f"{self.features_cls.__name__}.id == "
                            f"OtherClass.{to_snake_case(self.features_cls.__name__)}_id"
                ...
                return Filter(
                    _validate_filter_internal(filter.lhs),
                    "and",
                    _validate_filter_internal(filter.rhs)
                )
