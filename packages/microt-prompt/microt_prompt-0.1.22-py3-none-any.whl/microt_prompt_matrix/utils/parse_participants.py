...
def filterParticipantList(p_id_list):
    ...
    for p_id in p_id_list:
        if not p_id.endswith(suffix):
            ...
            print(
                "warning: participant ID  ( {} ) doesn't need suffix \"{}\". The program will automatically strip the suffix for each participant ID.".format(
                    p_id, suffix))
...
def parse_participants(participant_text_file_path):
    ...
    [print(" {}.  {}".format(i + 1, p_id_list_with_suffix[i])) for i in range(len(p_id_list_with_suffix))]
    print("\n")
    return p_id_list_with_suffix


if __name__ == "__main__":
    participant_text_file_path = r"C:\Users\jixin\Documents\GitHub\microt_preprocessing\resource\sample_file\participants_sample"
