...
if __name__ == "__main__":
    intermediate_participant_path = r"C:\Users\jixin\Desktop\testing\abc@timestudy_com"
    date_in_study = "2020-09-24"

    # validated_date_list = validate_dates_before_after(intermediate_participant_path, date_in_study)
    # print(validated_date_list)
    df_logs_combined = combine_intermediate_file(intermediate_participant_path, date_in_study)

    t = ["Sep 24 08:24:01 2020"]
    converter = lambda x: datetime.strptime(x, "%b %d %H:%M:%S %Y") if (type(x) != float) else "NF"
    ta = list(map(converter, t))
    prompt_date = ta[0].date()

    df_logs_combined = df_logs_combined[df_logs_combined['Date'] == prompt_date]
    print(df_logs_combined.shape[0])