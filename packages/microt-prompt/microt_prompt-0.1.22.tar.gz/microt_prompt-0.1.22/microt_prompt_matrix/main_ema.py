...
from microt_prompt_matrix import feature_engineering_ema


def compute_features_phone(intermediate_participant_path, output_dir_path, date_in_study, lsf_json_folder_path,
                           decryption_password):
    # Switch
    print_stats = False
    ...
    # check if skip
    csv_file_name = "ema_feature_mx_{}".format(date_in_study) + ".csv"
    csv_save_path = os.path.join(output_dir_path, participant_id, csv_file_name)
    if os.path.exists(csv_save_path):
        print("Output exists. Skip EMA for {}-{}".format(participant_id, date_in_study))
        return
    ...
    feature1 = feature_engineering_ema.day_of_the_week.create_column(df_prompt['Actual_Prompt_Local_DateTime'], print_stats)
    feature_df['DAY_OF_THE_WEEK'] = feature1
    # - feature 2: Time of the day
    feature2 = feature_engineering_ema.time_of_the_day.create_column(df_prompt['Actual_Prompt_Local_DateTime'], print_stats)
    ...
    feature5, feature6, match_time_battery = feature_engineering_ema.battery_level.create_column(
    ...
    feature7, match_time_location, feature7_1 = feature_engineering_ema.location.create_column(participant_id, df_prompt['Actual_Prompt_Local_DateTime'], info_dict,
    ...
    feature8 = feature_engineering_ema.location_label.create_column(lsf_json_folder_path, participant_id, feature7)
    feature_df['LOCATION_LABEL'] = feature8

    # - feature 7: Screen Status
    feature9, match_time_screen = feature_engineering_ema.screen_status.create_column(
        df_prompt['Actual_Prompt_Local_DateTime'], intermediate_participant_path, date_in_study)
    feature_df['SCREEN_STATUS'] = feature9
    feature_df['match_time_screen'] = match_time_screen

    # - feature 10&11: wake and sleep hour
    feature10, feature11 = feature_engineering_ema.waking_hours.create_column(df_prompt['Actual_Prompt_Local_DateTime'],
                                                                              intermediate_participant_path,
                                                                              date_in_study)
    ...
    feature12, feature13, match_time_lock = feature_engineering_ema.phone_lock.create_column(
        df_prompt['Actual_Prompt_Local_DateTime'], intermediate_participant_path, date_in_study)
    ...
    parts_num = 4
    feature14, feature15, feature16 = feature_engineering_ema.parts_of_waking_hour.create_column(feature_df, parts_num)
...
if __name__ == "__main__":
    ...
    compute_features_phone(intermediate_participant_path, output_dir_path, date_in_study, lsf_json_folder_path,
                           decryption_password)
    end = time.time()
    print("Finished\n")
