...
class FeatureEdits:
    def __init__(self, edits: dict[Feature, float]):
        self._edits = edits

    def __str__(self):
        output = "FeatureEdits("
        for feature, value in self._edits.items():
            output += f"\n   {feature.label}: {value}"
        return output + "\n})"
    ...
    def __setitem__(self, feature: Feature, value: float):
    ...
    def __neg__(self):
        return FeatureEdits({feature: -value for feature, value in self._edits.items()})
