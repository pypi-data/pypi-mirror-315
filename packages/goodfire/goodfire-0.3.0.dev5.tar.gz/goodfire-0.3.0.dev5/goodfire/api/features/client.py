...
class AsyncFeaturesAPI:
    ...
    async def steer_to_token(
        self,
        messages: list[ChatMessage],
        model: Union[SUPPORTED_MODELS, Variant],
        steer_to: str,
    ):
        if not steer_to.startswith(" "):
            steer_to = " " + steer_to
        ...
        feature_ids_to_add = [feat["id"] for feat in response.json()["to_add"]]
        feature_ids_to_remove = [feat["id"] for feat in response.json()["to_ablate"]]

        return await asyncio.gather(
            self._list(feature_ids_to_add), self._list(feature_ids_to_remove)
        )
...
class FeaturesAPI:
    ...
    def attribution(
        ...
        return run_async_safely(
            self._async_client.attribution(prefix, model, steer_to, steer_away)
        )
    ...
    def steer_to_token(
        ...
        return run_async_safely(
            self._async_client.steer_to_token(messages, model, steer_to)
        )
