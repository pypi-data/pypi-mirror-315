...
from ..features.features import FeatureEdits
from ..variants.variants import SUPPORTED_MODELS
...
async def AsyncAutoEdits(
    ...
    final_features = sorted(
        final_features, key=lambda x: x.max_activation_strength, reverse=True
    )
    ...
    edits = [
        (
            item[0],
            (softmax_scores[i] * -1) if max_acts[item[0]] < 1 else softmax_scores[i],
        )
        for i, item in enumerate(max_acts_items)
        if softmax_scores[i] > 0.1
    ]

    edits = sorted(edits, key=lambda x: abs(x[1]), reverse=True)

    return FeatureEdits(edits).rescale()
