...
from ..variants.variants import (
    ACTIVATION_NORMALIZATION_FACTOR,
    SUPPORTED_MODELS,
    Variant,
)
...
async def claude_prompt(
    ...
    system_prompt = LanguageModelPrompt(
        """You are tasked with created a contrastive dataset of chat messages meant to invoke a specification.

    In other words you will receive a specification and you should create a positive stimulus that evokes this specification and carefully construct a negative stimulus that does not evoke this specification.

    The stimuli should be pairwise in a way that makes the contrast clear.

    Attempt to express a distribution of ideas around the specification such that the average response evokes the specification.

    The goal is ultimately to train a classifier that can successfully distinguish the prescense of the specification in chat messages.
    ...
    Include changes to the user query as well. For instance if the specification is about "conciseness" include the words "Be concise." in the user query in addition to a concise response.
    """
    )
    ...
    messages = await model.chat(
        message_history=[
            {
                "role": "user",
                "content": 'Here is the specification: "A kind-hearted gesture."',
            },
            {
                "role": "assistant",
            ...
            {
                "role": "user",
                "content": "Here is the specification: 'Medical diagnosis'",
            },
            {
                "role": "assistant",
            ...
            {
                "role": "user",
                "content": f'Here is the specification: "{specification}"',
            },
        ],
        system_prompt=system_prompt,
...
async def AsyncAutoConditional(
    ...
    max_coeff = max([abs(score) for score in regressor.coef_[0]])
    final_features = [
        feature
        for feature in features_to_use
        if abs(regressor.coef_[0][features_to_use_indexes.index(feature.index_in_sae)])
        > max_coeff * 0.5
    ]
    ...
    conditional_group = ConditionalGroup(
        [
            Conditional(
                ...
                regressor.coef_[0][features_to_use_indexes.index(feature.index_in_sae)]
                / ACTIVATION_NORMALIZATION_FACTOR[model],
                operator=(
                    ">="
            ...
            for _, feature in enumerate(final_features)
        ],
        operator="AND",
    )
