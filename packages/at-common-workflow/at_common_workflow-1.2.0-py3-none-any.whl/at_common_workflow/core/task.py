...
class Task:
    ...
    def __init__(
        ...
        args: Union[Dict[str, Any], Arguments],
        requires: Union[Dict[str, type], Schema],
        provides: Union[Dict[str, type], Schema],
        ...
        Args:
            ...
            args: Optional fixed arguments to pass to the function
            requires: Schema of required context inputs {context_key: type}
            provides: Schema of provided context outputs {context_key: type}
        ...
        self.args = args if isinstance(args, Arguments) else Arguments(args)
        self.requires = requires if isinstance(requires, Schema) else Schema(requires)
        self.provides = provides if isinstance(provides, Schema) else Schema(provides)
        ...
        common_keys = set(requires.keys()) & set(provides.keys())
        if common_keys:
    ...
    def _validate_mappings(self) -> None:
        ...
        for mapping_key in self.input_mappings:
            if mapping_key not in self.requires:
                raise ValueError(f"Input mapping '{mapping_key}' not in requires schema")

        # Validate output mappings
        for context_key in self.provides.keys():
        ...
        for mapping_key in self.output_mappings:
            if mapping_key not in self.provides:
                raise ValueError(f"Output mapping '{mapping_key}' not in provides schema")
    ...
    def _prepare_inputs(self, context: Context) -> Dict[str, Any]:
        ...
        for context_key, expected_type in self.requires.items():
    ...
    def _store_outputs(self, context: Context, result: Any) -> None:
        ...
        Convention:
        - All returns are stored as "return_0", "return_1", etc.
        - Single return value (including dict) is stored as "return_0"
        - Tuple/List returns: stored as "return_0", "return_1", etc.
        ...
        if isinstance(result, (tuple, list)):
            result_dict = {f"return_{i}": value for i, value in enumerate(result)}
        else:
            # Handle single return value (including dicts)
            result_dict = {"return_0": result}

        # Map the standardized names to context using output_mappings
        for context_key, expected_type in self.provides.items():
