...
class Workflow:
    ...
    def _build_dependency_graph(self) -> Dict[Task, Set[Task]]:
        ...
        for task in self.tasks:
            for other_task in self.tasks:
                if task != other_task:
                    if any(key in other_task.provides for key in task.requires.keys()):
    ...
    def _validate_type_compatibility(self) -> None:
        ...
        for task in self.tasks:
            for req_key, req_type in task.requires.items():
                ...
                provider = None
                for other_task in self.tasks:
                    if req_key in other_task.provides:
                        provider = other_task
                        break
                
                # If provider found, validate type compatibility
                if provider:
                    provided_type = provider.provides[req_key]
    ...
    def _validate_task_connections(self) -> None:
        ...
        for task in self.tasks:
            for required_key in task.requires:
                ...
                for provider in self.tasks:
                    if required_key in provider.provides:
                        satisfied = True
                        break
