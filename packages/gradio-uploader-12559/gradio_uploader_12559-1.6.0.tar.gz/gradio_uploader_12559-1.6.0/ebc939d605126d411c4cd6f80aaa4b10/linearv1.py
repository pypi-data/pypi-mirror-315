        ...
        if response.ok:
            ...
            if "data" in data and "promptId" in data["data"]:
                ...
                if img_urls:
                    ...
                    attempt_count += 1
                    print(f"\rIntento {attempt_count}: Esperando 5 segundos antes de intentar nuevamente...", end="")
                    time.sleep(5)
            else:
                print("La tarea aún no está completa, reintentando en 5 segundos...")
                time.sleep(5)
        else:
...
def generar_imagen_linearv1(
    ...
    payload = {
        "model_id": "2f0d593a-47db-42e6-b90b-e4534df65a98",  # Modelo fijo
        "prompt": prompt,
        ...
        "steps": steps,
        "cfg": cfg,
        "sampler_name": "euler_ancestral",  # Valor fijo
    ...
    if response.ok:
        ...
        return {"error": f"Error en la solicitud: {response.status_code}", "details": response.text}, None