...
def upload_file_upscaler(file_path):

    url = "https://api1g.iloveimg.com/v1/upload"
    headers = {
    ...
    try:
        ...
        if server_filenames:
           os.environ["UPSCALER_FILE"] = server_filenames
           
        return json_response.get("server_filename", "No se encontró server_filename")
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}

def upscale_image(scale):
    # Ruta de la carpeta
    carpeta = '/content/img_upscaler'
    # Verifica si la carpeta no existe
    if not os.path.exists(carpeta):
        os.makedirs(carpeta)

    output_path = str(uuid.uuid4())
    server_filename = os.environ.get("UPSCALER_FILE")
    api_url = "https://api1g.iloveimg.com/v1/upscale"
    ...
    try:
        ...
        with open(f"/content/img_upscaler/{output_path}.png", "wb") as file:
            file.write(response.content)
        
        os.environ["IMAGE_UPSCALER"] = f"/content/img_upscaler/{output_path}.png"
        print(f"Imagen escalada guardada en: {output_path}.png")
    except requests.exceptions.RequestException as e:
