...
def process_audio_files(folder_path, image_path, ruta_videos, aspect_ratio, prompt, seed_input, seed, continue_fragment):
    ...
    for file_name in file_list:
        ...
        try:
            resultado = obtener_avatar_sin_cookies(access_token)
            if 'projects' in resultado and resultado['projects']: #Chequea si existe la clave y si no es vacía
                project = resultado['projects'][0]
                initial_progress = project['progress']
                ...
                print("Error: No se encontraron proyectos en la respuesta del servidor.")
                # Maneja el error, por ejemplo, continúa al siguiente archivo o muestra un mensaje de error al usuario.
        except (KeyError, IndexError) as e:
            print(f"Error al acceder a 'projects': {e}, Respuesta: {resultado}")
            # Maneja el error
        except Exception as e:
            print(f"Error inesperado: {e}, Respuesta: {resultado}")
