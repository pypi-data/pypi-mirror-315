...
class MetricInsights(InsightsConfiguration):
    ...
    def copy_to_playground(
        ...
        with_evaluation_datasets : bool, optional
        ...
        path = (
            cls._path.format(get_entity_id(source_playground))
            + f"{get_entity_id(target_playground)}/"
        )
        cls._client.put(path, json=payload)
