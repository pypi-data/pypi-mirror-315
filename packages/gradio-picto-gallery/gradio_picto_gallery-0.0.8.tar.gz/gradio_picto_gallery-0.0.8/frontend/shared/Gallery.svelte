<script lang="ts">
  ...
  import TiThumbsUp from 'svelte-icons/ti/TiThumbsUp.svelte'
  import TiThumbsDown from 'svelte-icons/ti/TiThumbsDown.svelte'
  ...
  import { default as LikeButton } from "./LikeButton.svelte";
	import { IconButton } from "@gradio/atoms";
	import type { I18nFormatter } from "@gradio/utils";

	type GalleryImage = { image: FileData; caption: string | null; favorite: boolean | null };
	type GalleryData = GalleryImage[];

	export let show_label = true;
  export let show_like_dislike = true;
  ...
  export let show_edit_button = false;
	...
	const dispatch = createEventDispatcher<{
		change: undefined;
    select: SelectData;
    toggle_favorite: GalleryImage;
    like: GalleryImage;
    dislike: GalleryImage;
    edit: GalleryImage;
	}>();
...
{:else}
		...
		{#if selected_image && allow_preview}
			...
			>
				<div class="icon-buttons">
					...
					{#if selected_image.favorite}
							<IconButton
								Icon={FaStar}
								label="Remove from Favorite"
								class="icon-button"
								on:click={() => {
									selected_image.favorite = false
									dispatch("toggle_favorite", selected_image)
								}}
							/>
					{/if}
					{#if !selected_image.favorite}
	...
	{#if show_like_dislike}
      	<div class="bottom-icon-buttons">
            ...
            label="Good Job!"
            action="like"
						on:click={() => { dispatch("like", selected_image) }}
					/>
        	<LikeButton
            Icon={TiThumbsDown}
            label="meh..." 
            action="dislike"
						on:click={() => { 
							dispatch("dislike", selected_image) 
							}}
					/>
				</div>
	{/if}
						...
						{#if entry.favorite}
							<IconButton
								...
								on:click={() => {
									...
									dispatch("toggle_favorite", entry)
								}}
							/>
						{/if}
						{#if !entry.favorite}
	...
	{#if show_like_dislike}
      	<div class="bottom-icon-buttons">
            ...
            label="Good Job!"
            action="like"
						on:click={() => { dispatch("like", entry) }}
          />
        	<LikeButton
            Icon={TiThumbsDown}
            label="meh..." 
						action="dislike"
						on:click={() => { dispatch("dislike", entry) }}
          />
      	</div>
	{/if}
	      </div>
