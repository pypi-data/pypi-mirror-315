...
def get_latency(packet: pyshark.packet.packet.Packet, parser) -> float | None:
    ...
    if "WEBSOCKET" in packet:
        try:
            ...
            ws_payload_text = ws_payload.get("websocket.payload.text")
...
def main():
    try:
        ...
        parser.add_argument(
            "-i",
            "--interface",
            help="The network interface to listen on. Default is any for Linux or en0 for macOS.",
            default=("any" if system == "Linux" else "en0"),
            type=str,
        )
