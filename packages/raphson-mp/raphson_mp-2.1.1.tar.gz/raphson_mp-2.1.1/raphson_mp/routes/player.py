...
from raphson_mp import music, scanner, settings, util
...
async def route_copy_track(request: web.Request, conn: Connection, user: User):
    ...
    path = Path(playlist.path, track.path.name)
    await scanner.scan_track(conn, user, playlist_name, path, music.to_relpath(path))
