...
async def _make_request(method: str, api_method: str, **extra_params: str) -> dict[str, Any]:
    ...
    async with aiohttp.ClientSession(
        "https://ws.audioscrobbler.com/2.0/", headers={"User-Agent": settings.user_agent}, raise_for_status=True
    ) as session:
        if method == "post":
            async with session.post(
                "", data=query_string, headers={"Content-Type": "application/x-www-form-urlencoded"}
            ) as response:
...
def get_user_key(user: StandardUser) -> str | None:
    ...
    """
    result = user.conn.execute("SELECT key FROM user_lastfm WHERE user=?", (user.user_id,)).fetchone()
...
async def update_now_playing(user_key: str, meta: Metadata):
    ...
    log.info('Sending now playing to last.fm: %s', meta.relpath)
    await _make_request("post", "track.updateNowPlaying", artist=meta.artists[0], track=meta.title, sk=user_key)


async def scrobble(user_key: str, meta: Metadata, start_timestamp: int):
