...
from raphson_mp import db, music, settings
...
async def _choose_track(conn: Connection, previous_playlist: str | None = None) -> Track:
    ...
    track = cast(Track, await playlist.choose_track(None))
    return track
        ...
        def thread():
            with db.connect() as writable_conn:
                writable_conn.execute(
                    "INSERT INTO radio_track (track, start_time) VALUES (?, ?)",
...
async def get_next_track(conn: Connection) -> RadioTrack:
    ...
    if next_track_info is None:
        ...
        track = await _choose_track(conn, previous_playlist=current_track_playlist)
        def thread():
            with db.connect() as writable_conn:
                writable_conn.execute(
                    "INSERT INTO radio_track (track, start_time) VALUES (?, ?)",
