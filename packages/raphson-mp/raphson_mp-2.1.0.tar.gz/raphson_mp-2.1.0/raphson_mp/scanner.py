...
async def scan_track(conn: Connection, user: User | None, playlist_name: str, track_path: Path, track_relpath: str) -> bool:
    """
    Scan single track.
        ...
        def thread():
            with db.connect() as writable_conn:
                ...
                writable_conn.execute(
                    ...
                    """,
                    {**params.main_data, "playlist": playlist_name, "mtime": file_mtime},
                )
    ...
    if file_mtime != db_mtime:
        ...
        def thread():
            with db.connect() as writable_conn:
                ...
                writable_conn.executemany("INSERT INTO track_artist (track, artist) VALUES (:track, :artist)", params.artist_data)
...
async def scan(conn: Connection, user: User | None) -> None:
    """
