...
from raphson_mp.i18n import gettext as _
...
def chart_singleaxis(
    title: str, ldata: list[str], axisdata: list[str] | list[int], series: list[dict[str, Any]], horizontal: bool
) -> ChartT:
    if horizontal:
        ydata = axisdata
        xdata = None
...
def bar(title: str, name: str, axisdata: list[str] | list[int], seriesdata: list[int], horizontal: bool = False) -> ChartT:
...
def multibar(
    ...
    seriesdata: dict[str, list[int]],
    horizontal: bool = False,
    stack: bool = True,
) -> ChartT:
    series = [{"name": name, "type": "bar", "data": data} for name, data in seriesdata.items()]
    if stack:
        for item in series:
            item["stack"] = "x"
...
def heatmap(
    title: str,
    name: str,
    xdata: list[str] | list[int],
    ydata: list[str] | list[int],
    seriesdata: list[tuple[int, int, float]],
) -> ChartT:
...
def rows_to_xy_multi(
    rows: list[tuple[str, str, int]], case_sensitive: bool = True, restore_case: bool = False
...
def chart_last_chosen(conn: Connection) -> ChartT:
    """
    Last chosen chart
    """
    cur = conn.cursor()
...
def charts_playlists(conn: Connection) -> list[ChartT]:
    """
    ...
    return [
        bar(_("Number of tracks in playlists"), _("Number of tracks"), *rows_to_xy(counts)),
...
def chart_track_year(conn: Connection) -> ChartT:
    """
    ...
    return multibar(_("Track release year distribution"), [str(year) for year in range(min_year, max_year + 1)], data)


def to_usernames(usernames: str, counts: dict[int, list[int]]) -> dict[str, list[int]]:
    return {usernames[i]: values for i, (_user_id, values) in enumerate(counts.items())}


def charts_history(conn: Connection, after_timestamp: int) -> Iterator[ChartT]:
    """
    ...
    yield multibar(
        _("Most played artists"), *rows_to_xy_multi(rows, case_sensitive=False, restore_case=True), horizontal=True
    )
    ...
    yield multibar(
        _("Most played albums"), *rows_to_xy_multi(rows, case_sensitive=False, restore_case=True), horizontal=True
    )
    ...
    for (username,) in conn.execute(
        "SELECT DISTINCT username FROM history JOIN user ON history.user = user.id WHERE timestamp > ?",
        (after_timestamp,),
    ):
        time_of_day[username] = [0] * 24
        day_of_week[username] = [0] * 7

    for timestamp, username in conn.execute(
        "SELECT timestamp, username FROM history JOIN user ON history.user = user.id WHERE timestamp > ?",
        (after_timestamp,),
    ):
...
def chart_unique_artists(conn: Connection) -> ChartT:
    rows = conn.execute(
        """
...
def chart_popular_artists_tags(conn: Connection) -> Iterator[ChartT]:
    for table, title in (("artist", _("Artists in playlists")), ("tag", _("Tags in playlists"))):
        ...
        yield multibar(
            title, *rows_to_xy_multi(rows, case_sensitive=False, restore_case=table == "artist"), horizontal=True
        )


def similarity_heatmap(conn: Connection) -> ChartT:
    playlists = [
        row[0] for row in conn.execute("SELECT playlist FROM track GROUP BY playlist ORDER BY COUNT(*) DESC LIMIT 15")
    ]
    result = conn.execute(
        """
...
def get_data(period: StatsPeriod) -> list[ChartT]:
    """
    Generate charts as json data for stats.jinja2
    """
    with db.connect(read_only=True) as conn:
        data = [
            *charts_history(conn, period.after_timestamp),
