...
async def route_audio(request: web.Request, conn: Connection, _user: User):
    ...
    audio = await track.transcoded_audio(audio_format)
    response = web.Response(body=audio, content_type=audio_format.content_type)
...
async def route_update_metadata(request: web.Request, conn: Connection, user: User):
    ...
    await scanner.scan_track(conn, user, track.playlist, track.path, track.relpath)
