...
from raphson_mp import db, metadata, offline_sync
from raphson_mp.auth import User
from raphson_mp.decorators import route
from raphson_mp.offline_sync import OfflineSync, WebResponseProgress
...
class SyncState:
    ...
    def start_new(self, conn: Connection):
        self._sync = OfflineSync(conn, WebResponseProgress(), 0)
...
async def route_settings(request: web.Request, _conn: Connection, _user: User):
    ...
    await offline_sync.change_settings(server, token)
...
async def route_start(_request: web.Request, conn: Connection, _user: User) -> web.Response:
    if _STATE.active:
        raise web.HTTPBadRequest(reason="sync task is already running")

    _STATE.start_new(conn)
