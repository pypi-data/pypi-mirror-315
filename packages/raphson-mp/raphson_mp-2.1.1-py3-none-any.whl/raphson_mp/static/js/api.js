...
class ControlChannel {
    ...
    #open() {
        ...
        const uri = "/control?id=" + encodeURIComponent(this.#id);
        this.#socket = new WebSocket(uri);
        this.#socket.addEventListener("message", event => this.#handleMessage(event.data));
        this.#socket.addEventListener("close", () => this.#socketWorking = false);
        this.#socket.addEventListener("error", () => this.#socketWorking = false);

        // Reissue previous subscriptions when socket is opened
        this.#socket.addEventListener("open", () => {
            this.#socketWorking = true;
            this.sendMessage(ControlCommand.CLIENT_TOKEN, {"csrf": getCsrfToken()});
    ...
    sendMessage(name, data) {
        if (this.#socketWorking) {
            console.debug('ws: send:', name, data)
     ...
     * every 60 seconds, more frequently is recommended.
     * @param {Track} track
     * @param {boolean} paused
     * @param {number} position
     * @param {number} duration
     */
    async nowPlaying(track, paused, position, duration) {
        const data = {
            track: track.path,
            paused: paused,
            position: position,
        ...
        this.sendMessage(ControlCommand.CLIENT_PLAYING, data);
    }
...
class Music {
    ...
    async getPlaylistCheckboxes() {
        ...
        mainDiv.classList.add('playlist-checkboxes');
        otherDiv.classList.add('playlist-checkboxes');
