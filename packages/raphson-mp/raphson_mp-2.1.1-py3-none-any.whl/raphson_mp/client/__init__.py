...
from aiohttp import ClientResponseError, ClientTimeout, ClientWebSocketResponse, StreamReader, WSMsgType
...
from raphson_mp.common.playlist import Playlist
...
class RaphsonMusicClient:
    ...
    async def control_start(self, handler: Callable[[ServerCommand], Awaitable[ClientCommand | None]], reconnect: bool = True):
        while reconnect:
    ...
    async def control_send(self, command: ClientCommand):
        if not self.control_ws:
            raise UnsupportedOperation("websocket is not active")
