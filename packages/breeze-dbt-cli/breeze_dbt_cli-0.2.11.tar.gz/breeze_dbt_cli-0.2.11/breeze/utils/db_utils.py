...
def get_columns_from_database(database: str, schema: str, identifier: str) -> List[Tuple[str, str]]:
    ...
    if db_type == "postgres":
        ...
        return get_columns_using_connection_databricks(conn, schema, identifier)
    else:
...
def get_columns_using_connection_databricks(conn, schema, identifier) -> List[Tuple[str, str]]:
    columns = []
    try:
        cursor = conn.cursor()
