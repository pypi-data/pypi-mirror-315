...
def get_columns_from_database(database: str, schema: str, identifier: str) -> List[Tuple[str, str]]:
    ...
    if db_type == "postgres":
        ...
        return get_columns_using_connection_databricks(conn, schema, identifier, catalog)
    else:
...
def get_columns_using_connection_databricks(conn, schema, identifier, catalog) -> List[Tuple[str, str]]:
    columns = []
    try:
        cursor = conn.cursor()
...
def get_all_sources_from_schema(schema_name: str) -> List[str]:
    """
