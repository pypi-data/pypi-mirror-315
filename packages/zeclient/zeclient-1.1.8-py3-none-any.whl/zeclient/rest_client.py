...
class RestClient:
    ...
    def find_curves(self, group_name, curve_name=None, properties=None, observations=None, adaptors=None):
        if group_name is None:
            group_name = 'NOGROUP'
        uri = "/curve/groups/{}/curves".format(group_name)
        if curve_name is not None:
            uri = uri + "/" + curve_name
        if any(arg is True for arg in (properties,observations,adaptors)):
            query_params = []
            if properties is True:
                query_params.append(f"properties=True")
            if observations is True:
                query_params.append(f"observations=True")
            if adaptors is True:
                query_params.append(f"adaptors=True")
