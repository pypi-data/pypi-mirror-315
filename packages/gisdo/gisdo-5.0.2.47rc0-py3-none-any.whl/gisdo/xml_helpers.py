...
from kinder.core.dict.models import DouType
from kinder.core.dict.models import GroupTypeEnumerate
from kinder.core.dict.models import UnitKind
from kinder.core.emie_unit.models import EmieUnitModel
from kinder.core.emie_unit.models import UnitDocFiles
from kinder.core.employer.models import EmployerPost
from kinder.core.group.models import Group
from kinder.core.group.models import GroupStatusEnum
from kinder.core.group.models import Pupil
from kinder.core.unit.models import FilialData
from kinder.core.unit.models import FilialType
from kinder.core.unit.models import StructureTypes
from kinder.core.unit.models import Unit
from kinder.core.unit.models import UnitStatus

from .utils import UnitHelper
from .utils import get_file_url
from .utils import try_date_in_string
...
NOT_CALCULATED_STATUSES = [
    k for k, v in list(FUNCTIONING_MAPPING.items()) if v == NOT_FUNCTIONING
]
...
def _has_working_filials(dou):
    ...
    filials = FilialData.objects.filter(
        head=dou
    ).filter(
        UnitHelper.ALLOWED_FILIAL_FILTER
    ...
    dou_groups = Group.objects.filter(
        status=GroupStatusEnum.FACT
    ).filter(
        Q(unit=dou) |
        Q(unit__in=filials)
    )
    ...
    for group in dou_groups:
        children_in_fact = Pupil.objects.filter(
            grup=group, temporary_deduct=False,
            grup__status=GroupStatusEnum.FACT
        )

        children_in_plan = Pupil.objects.filter(
            Q(grup__room_id__isnull=False, grup__room_id=group.room_id)
        ).filter(
            temporary_deduct=False, grup__status=GroupStatusEnum.PLAN
        ).exclude(
            children_id__in=Pupil.objects.filter(
                grup__unit=group.unit, temporary_deduct=False,
                grup__status=GroupStatusEnum.FACT
...
def get_groups_num(dou):
    ...
    filials = FilialData.objects.filter(
        ...
        UnitHelper.ALLOWED_FILIAL_FILTER
    ).values_list("filial_id", flat=True).iterator()

    dou_groups = Group.extended_objects.with_count_norm().select_related(
        'type', 'health_need'
    ).filter(
        status=GroupStatusEnum.FACT
    ).filter(
        Q(unit=dou) | Q(unit__in=filials)
    )
...
def get_dou_filials(dou):
    ...
    return FilialData.objects.filter(
        head=dou
...
def get_units_report(mo, units):
    ...
    subquery_banned_units = Subquery(
        FilialData.objects.filter(
            Q(head__in=units_id)
            & UnitHelper.ALLOWED_FILIAL_FILTER
            & Q(filial__gisdo__related_to_mo=mo)
    ...
    result_units = (
        Unit.objects.filter(
            ...
            filial__in=subquery_banned_units))
...
def get_dou_filials_and_building_ids(dou):
    ...
    valid_filials = Unit.objects.filter(
        id__in=filials,
        is_filial__in=[FilialType.FILIAL, FilialType.CORPUS])
...
def get_additional_info_by_dou(dou, model=UnitDocFiles):
    ...
    extensions = model.objects.filter(
        Q(unit=dou) | Q(unit__in=filials)
...
def get_max_doo(mo):
    ...
    return mo.get_descendants().filter(
        kind=UnitKind.DOU
    ).exclude(
...
def get_employers_in_dou_by_category(dou_ids: list[int], category: list[str], part_time: bool = False) -> int:
    ...
    query = EmployerPost.objects.filter(
        post__post_category__code__in=category,
        employer__unit_id__in=dou_ids,
        employer__discharged=False
    ).exclude(date_of_resignation__lt=datetime.today())

    if not part_time:
        return query.filter(
            Q(part_time=part_time) |
            Q(contingentemployeepost__is_internal_part_time=True)).count()

    return query.filter(contingentemployeepost__is_external_part_time=True
                        ).count()
