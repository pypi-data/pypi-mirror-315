    ...
    def _add_agent(self, agent):
        ...
        if type(agent).__name__ in self.agent_list:
            if agent.tuple_name[0] in self.agent_list[type(agent).__name__]:
                self.agent_list[type(agent).__name__][agent.tuple_name[0]].update({agent.tuple_name})
                self._agents[agent.tuple_name] = agent
            else:
                self.agent_list[type(agent).__name__].update({agent.tuple_name[0] : {agent.tuple_name}})
                self._agents[agent.tuple_name] = agent
        else:
            self.agent_list[type(agent).__name__] = {agent.tuple_name[0] : {agent.tuple_name}}
            self._agents[agent.tuple_name] = agent
        
        self.print(f'Connecting agent {type(agent).__name__}:{agent.tuple_name}') if self.full_log else ...
