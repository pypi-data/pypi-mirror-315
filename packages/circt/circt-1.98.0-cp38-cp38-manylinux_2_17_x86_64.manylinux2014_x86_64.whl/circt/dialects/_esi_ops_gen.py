...
def stage(output, clk, rst, input, *, loc=None, ip=None) -> _ods_ir.Value:
  return PipelineStageOp(output=output, clk=clk, rst=rst, input=input, loc=loc, ip=ip).result
...
class ServiceDeclPortOp(_ods_ir.OpView):
  ...
  def __init__(self, inner_sym, toClientType, *, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_455')) else
      _ods_ir.AttrBuilder.get('anonymous_455')(toClientType, context=_ods_context))
    _ods_successors = None
    super().__init__(self.build_generic(attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip))
...
class ServiceImplClientRecordOp(_ods_ir.OpView):
  ...
  def __init__(self, relAppIDPath, servicePort, typeID, *, channelAssignments=None, implDetails=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_455')) else
      _ods_ir.AttrBuilder.get('anonymous_455')(typeID, context=_ods_context))
...
def manifest_service_impl(app_id, service_impl_name, impl_details, *, service=None, std_service=None, loc=None, ip=None) -> _ods_ir.Operation:
  return ServiceImplRecordOp(appID=app_id, serviceImplName=service_impl_name, implDetails=impl_details, service=service, stdService=std_service, loc=loc, ip=ip)
