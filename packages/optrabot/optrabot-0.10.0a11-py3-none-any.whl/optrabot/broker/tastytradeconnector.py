...
from optrabot.broker.order import OptionRight, Order as GenericOrder, OrderAction as GenericOrderAction, Leg as GenericOrderLeg
...
class TastytradeConnector(BrokerConnector):
	...
	async def placeOrder(self, order: GenericOrder, template: Template) -> bool:
		...
		newOrder = NewOrder(
			...
			legs=order.brokerSpecific['comboLegs'],
			price=Decimal(abs(order.price + 5))
		)
		try:
			response = account.place_order(self._session, newOrder)
