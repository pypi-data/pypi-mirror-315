...
class DicomFile:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.get_manufacturer().startswith("SIEMENS"):
            acq_datetime = self.timestamp(dcm.get("SeriesDate"), dcm.get("SeriesTime"), timezone)
        else:
        ...
        if parse or de_identify:
            ...
            self.acquisition_uid = series_uid + (
                "_" + str(self.acq_no) if self.acq_no is not None and int(self.acq_no) > 1 else ""
            )
    ...
    def subject_code(self):
        ...
        warnings.warn("'code' attribute is deprecated now. Use 'label'", DeprecationWarning)
