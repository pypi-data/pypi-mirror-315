...
class Client(contacts.Person, BeIdCardHolder, UserAuthored, ClientBase,
             BiographyOwner, Referrable, Dupable, Lockable,
    ...
    def update_owned_instance(self, owned):
        owned.project = self
        super().update_owned_instance(owned)
    ...
    def full_clean(self, *args, **kw):
        ...
        if self.ref and self.ref.upper().startswith(prefix):
            ...
            if len(num_root) >= num_width:
                ...
                ref_num = num_root + next_ref.rjust(num_width - len(num_root),
                                                    "0")
    ...
    def get_dupable_words(self, s):
        ...
        return super().get_dupable_words(s)
    ...
    def find_similar_instances(self, limit=None, **kwargs):
        ...
        qs = super().find_similar_instances(None, **kwargs)
        if self.national_id:
...
# dd.update_field(Client, 'client_state', default='coached')
...
class ClientDetail(dd.DetailLayout):
    ...
    checkdata.MessagesByOwner:30 dupable.SimilarObjects:30
    """,
                      label=_("Database"))
...
class Clients(contacts.Persons):
    ...
    def get_request_queryset(self, ar, **filter):
        ...
        if ce:
            # raise Exception("20240324 {}".format(ce))
...
def getter(k):
    ...
    def func(actor, obj, ar):
        return getattr(obj, k)

    return func


class AllClients(Clients, dd.VentilatingTable):
    auto_fit_column_widths = False
    column_names_template = "client_state \
    ...
    user event_policy {vcolumns}"

    detail_layout = None
    required_roles = dd.login_required(Explorer)
    ventilated_column_suffix = ':10'

    if dd.is_installed("trends"):
        # For each trend stage having subject_column True, Lino adds a virtual
        # column showing the date of the last event with that trend stage.
        ...
        def get_request_queryset(self, ar, **filter):
            ...
            ann = dict()
            for ts in TrendStage.objects.filter(subject_column=True):
                k = 'event_date_' + str(ts.id)
                ann[k] = models.Max(
                    'trend_events__event_date',
                    filter=models.Q(trend_events__trend_stage=ts))
            qs = qs.annotate(**ann)
            return qs
        ...
        def get_ventilated_columns(cls):
            ...
            for ts in TrendStage.objects.filter(subject_column=True):
                k = 'event_date_' + str(ts.id)
                yield dd.VirtualField(dd.DateField(str(ts)),
                                      getter(k),
                                      sortable_by=[k])
                # print("20231005 adding subject column", name, ts)
