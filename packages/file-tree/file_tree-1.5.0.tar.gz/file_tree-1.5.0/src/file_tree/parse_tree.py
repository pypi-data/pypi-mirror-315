...
def read_file_tree_text(lines: List[str], top_level: Union[str, Template]) -> FileTree:
...
def add_line_to_tree(
    tree: FileTree, line: str, current: List[Tuple[int, Template]], to_link: List[List[str]]
) -> List[Tuple[int, Optional[Template]]]:
...
def get_parent(nspaces: int, current: List[Tuple[int, Template]]) -> Template:
