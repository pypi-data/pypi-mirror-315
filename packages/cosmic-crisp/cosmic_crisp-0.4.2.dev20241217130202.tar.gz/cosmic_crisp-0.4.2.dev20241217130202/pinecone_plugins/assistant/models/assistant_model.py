...
class AssistantModel:
    ...
    def chat(
        ...
        :param json_response: If this flag is turned on, then the return type is a json response.
        ...
        if stream and json_response:
            raise ValueError("stream and json_response cannot be set to True at the same time")
        ...
        if stream:
            ...
            return self._chat_single(messages=messages, model=model, filter=filter, json_response=json_response)
    ...
    def _chat_single(
        ...
        chat_request = (
            ChatRequest(messages=messages, filter=filter, model=model, json_response=json_response)
            if filter
            else ChatRequest(messages=messages, model=model, json_response=json_response)
        )
