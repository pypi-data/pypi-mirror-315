...
class Evaluation:
    def __init__(self, client_builder):
        ...
        metrics_api = client_builder(MetricsApiClient, MetricsApi, 'unstable', host=self.host)
...
class Assistant(PineconePlugin):
    ...
    def create_assistant(
        ...
        >>> model = (...).create_assistant(assistant_name="example_assistant", instructions="Always answer politely", metadata=metadata, timeout=30)
        >>> print(model)
        ...
        """
        inline_object = CreateModelRequest(name=assistant_name, instructions=instructions, metadata=metadata)
    ...
    def describe_assistant(
        ...
        assistant = self._assistant_control_api.describe_assistant(assistant_name=assistant_name)
        return AssistantModel(assistant=assistant, client_builder=self._client_builder, config=self.config)
