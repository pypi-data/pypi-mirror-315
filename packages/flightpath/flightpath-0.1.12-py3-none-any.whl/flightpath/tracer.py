...
class TaskInstance:
    ...
    is_external_task_sensor: bool = False
    ...
    def ready_seconds(self) -> float:
        ...
        if self.is_external_task_sensor:
...
class AirflowClient(Client):
    ...
    def get_task_instance(self, dag_id: str, task_id: str, dag_run_id: str) -> TaskInstance:
        ...
        is_external_task_sensor = data["operator"] == "ExternalTaskSensor"
        ...
        return TaskInstance(
            ...
            is_external_task_sensor=is_external_task_sensor,
        )
    
    def _get_external_task_sensor_dependency(self, dag_id: str, task_id: str, dag_run_id: str) -> TaskInstance:
        logging.info(f"  Retrieving upstream information for ExternalTaskSensor task {task_id} in dag {dag_id}")
        task_instance_endpoint = f"{self.base_url}/api/v1/dags/{dag_id}/dagRuns/{dag_run_id}/taskInstances/{task_id}"

        logging.debug(
            f"Extracting upstream information for ExternalTaskSensor task {task_id} from endpoint: {task_instance_endpoint}"
        )
        ...
        if (
            ...
            logging.warning(
                f"Could not find upstream information for ExternalTaskSensor task {task_id} from endpoint: {task_instance_endpoint} for dag_run_id {dag_run_id}"
            )
    ...
    def _get_all_task_dependencies_for_dag(self, dag_id: str, dag_run_id: str) -> Mapping[Task, Collection[Task]]:
        ...
        logging.info(f"  Extracting all dependencies for dag {dag_id} from endpoint: {endpoint}")
        ...
        for task in tasks:
            ...
            #         self._get_external_task_sensor_dependency(dag_id=dag_id, task_id=task['task_id'], dag_run_id=dag_run_id)
            #     )
    ...
    def get_upstream_task_instances(self, ti: TaskInstance) -> list[TaskInstance]:
        if ti.is_external_task_sensor:
            return [self._get_external_task_sensor_dependency(dag_id=ti.dag_id, task_id=ti.task_id, dag_run_id=ti.dag_run_id)]
