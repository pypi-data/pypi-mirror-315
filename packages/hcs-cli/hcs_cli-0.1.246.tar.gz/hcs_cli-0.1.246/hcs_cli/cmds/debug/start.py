...
from hcs_cli.support.debug_util import get_mvn_build_cmd, get_pod_id, restart_service, get_service_full_name
import subprocess
import os
import shlex
...
@click.option(
    "--skip/--build", "-s/-b", type=bool, required=False, default=False, help="Skip rebuild and re-deploy service"
)
...
def start(service: str, skip: bool, force: bool, **kwargs):
    ...
    service = recent.require(service, "hcs_debug_k8s_service")
...
def pod_forwarding(pod_id):
    ...
    _forwarding_process = subprocess.Popen(
        shlex.split(pod_forwarding_command), stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
