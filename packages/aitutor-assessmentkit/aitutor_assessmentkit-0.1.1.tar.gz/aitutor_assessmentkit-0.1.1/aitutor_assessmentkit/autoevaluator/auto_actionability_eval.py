...
class AutoActionabilityEvaluator(autoeval.AutoEvaluator):
    """
    Evaluates the ability of tutor models to provide actionable feedback to the student.
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        - **Keyword-Based Evaluation:**
          - Focuses on linguistic cues and patterns often associated with Actionability.
            - Uses a heuristic-based approach to evaluate the Actionability ability of tutor models.
            - Higher scores the tutor is better which indicates the tutor is providing actionable feedback.
        """
        super().__init__(*args, **kwargs)

    def _calculate_heuristic_based_score(self, convs: List[Dict[str, Any]], tutor_model: str) -> Union[List[float], float]:
        """
        Compute actionability scores using heuristics.
        ...
        for example in convs:
            ...
            scores.append(score)
            utils.update_auto_annotation(tutor_model, example, score, 'Actionability_Heuristic')
    ...
    def _get_metric_method(self, metric: str):
        ...
        """
        metric_methods = {
            'Actionability_Heuristic': self._calculate_heuristic_based_score
        }
    ...
    def compute(
        ...
        save: bool = False, 
        file_name: str = 'mrbench_actionability.json'
    ) -> Tuple[Dict[str, Any], List[float]]:
        """
        Compute actionability scores for tutor models.

        Parameters:
            data (Dict[str, Any]): Input data for evaluation.
            method (List[str]): Evaluation mwtrics to use. Default is ['Actionability_Heuristic'].
        ...
        if metrics is None:
            metrics = ['Actionability_Heuristic']

        print(f"Computing Actionability Scores using {metrics} mtrics(s) for {len(self.data)} examples...")
    ...
    def list_available_metrics(self) -> pd.DataFrame:
        ...
        """
        methods = {
            "Actionability_Heuristic": "Compute actionability scores using heuristics."
        }
    ...
    def get_sample_examples_with_scores(
        ...
        num_examples: int = 5, 
        metric: str = 'Actionability_Heuristic',
    ) -> pd.DataFrame:

        """
        Get examples with actionability scores for a specific tutor model.
        ...
        Returns:
            pd.DataFrame: A DataFrame with the examples and their actionability scores.
        """
    ...
    def compare_tutors_scores(
        ...
        """
        Compare the actionability scores of two tutor models.
