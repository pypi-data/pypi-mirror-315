...
class AutoMistakeLocationEvaluator(autoeval.AutoEvaluator):
    """
    Evaluates the ability of tutor models to accurately locate mistakes in student responses.
       
    """

    def __init__(self, *args, **kwargs) -> None:
        """
        Initialize the AutoMistakeLocationEvaluator.

        Notes:

        - **Keyword-Based Evaluation:**
          - Focuses on linguistic cues and patterns often associated with Mistake Location.
            - Uses a heuristic-based approach to evaluate the Mistake Location ability of tutor models.
            - Higher scores indicate better tutor as it points to Mistake Location accurately.
        """
        super().__init__(*args, **kwargs)

    def _calculate_heuristic_based_score(self, convs: List[Dict[str, Any]], tutor_model: str) -> Union[List[float], float]:
        """
        Compute mistake location scores using heuristics.
        ...
        for example in convs:
            ...
            scores.append(score)
            utils.update_auto_annotation(tutor_model, example, score, 'Mistake_Location_Heuristic')
    ...
    def _get_metric_method(self, metric: str):
        ...
        """
        metric_methods = {
            'Mistake_Location_Heuristic': self._calculate_heuristic_based_score
        }
    ...
    def compute(
        ...
        save: bool = False, 
        file_name: str = 'mrbench_mistake_location.json'
    ) -> Tuple[Dict[str, Any], List[float]]:
        """
        Compute mistake location scores for tutor models.
        ...
        if metrics is None:
            metrics = ['Mistake_Location_Heuristic']

        print(f"Computing Mistake Location Scores using {metrics} mtrics(s) for {len(self.data)} examples...")
    ...
    def list_available_metrics(self) -> pd.DataFrame:
        ...
        """
        methods = {
            "Mistake_Location_Heuristic": "Compute mistake location scores using heuristics."
        }
    ...
    def get_sample_examples_with_scores(
        ...
        num_examples: int = 5, 
        metric: str = 'Mistake_Location_Heuristic',
    ) -> pd.DataFrame:

        """
        Get examples with mistake location scores for a specific tutor model.
