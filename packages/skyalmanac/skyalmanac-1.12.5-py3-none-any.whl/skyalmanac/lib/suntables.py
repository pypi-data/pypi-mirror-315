...
from skyalmanac.lib import alma_skyfield
...
def sunalmanac(first_day, dtp, dfra):
    ...
    if config.FANCYhd:
        return makeSUNnew(first_day, dtp, dfra) # use the 'fancyhdr' package
    else:
        return makeSUNold(first_day, dtp, dfra) # use old formatting
...
def hdrSUNnew(first_day, dtp, dfra):
...
def makeSUNnew(first_day, dtp, dfra):
...
\begin{document}'''
    ...
    if not config.DPonly:
        tex += hdrSUNnew(first_day,dtp,dfra)
...
def hdrSUNold(first_day, dtp, dfra):
    # build the front page
...
def makeSUNold(first_day, dtp, dfra):
...
\begin{document}'''
    ...
    if not config.DPonly:
        tex += hdrSUNold(first_day,dtp,dfra)
