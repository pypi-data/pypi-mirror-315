...
##    import concurrent.futures
    ...
    from skyalmanac.lib.alma_skyfield import planetstransit, moonGHA, equation_of_time, getDUT1, find_new_moon
    # ... following is required for MULTI-PROCESSING:
    from skyalmanac.lib.mp_eventtables import mp_twilight, mp_moonrise_set, mp_planetstransit
else:
    # ... following is required for SINGLE-PROCESSING:
    from skyalmanac.lib.alma_skyfield import twilight, moonrise_set2, planetstransit, moonGHA, equation_of_time, getDUT1, find_new_moon
...
def mp_twilight_worker(Date, ts, spad, lat):
    ...
    twi = mp_twilight(Date, lat, ts, spad, True) # ===>>> mp_eventtables.py
...
def mp_moonlight_worker(Date, ts, spad, lat):
    ...
    hemisph = 'N' if lat >= 0 else 'S'
    ml = mp_moonrise_set(Date, lat, ts, spad)    # ===>>> mp_eventtables.py
