...
def mp_planetGHA(d, ts, spad, obj):             # used in nautical.planetstab
...
def mp_planetstransit(d, ts, spad, obj, with_seconds = False):
...
def hor_parallax(d, ts, spad):      # used in nautical.starstab
...
# > > > > > > > > > > MULTIPROCESSING ENTRY POINT < < < < < < < < < <
def mp_sunmoon(date, d_valNA, ts, spad, n):
...
# > > > > > > > > > > MULTIPROCESSING ENTRY POINT < < < < < < < < < <
def mp_stellar_info(d, ts, spad, df, n):        # used in starstab
...
# > > > > > > > > > > MULTIPROCESSING ENTRY POINT < < < < < < < < < <
def mp_twilight(d, lat, ts, spad, with_seconds = False):    # used in nautical.twilighttab (section 1)
...
def mp_moonrise_set(d, lat, mstate0, ts, spad):   # used in nautical.twilighttab (section 2)
