...
if config.MULTIpr:  # in multi-processing mode ...
    ...
    from skyalmanac.lib.alma_skyfield import ariesGHA, venusGHA, marsGHA, jupiterGHA, saturnGHA, sunGHA, moonGHA, moonVD, sunSD, moonSD, vdm_Venus, vdm_Mars, vdm_Jupiter, vdm_Saturn, ariestransit, stellar_info, planetstransit, moonage, moonphase, equation_of_time, getDUT1, find_new_moon
    # ... following is required for MULTI-PROCESSING:
    from skyalmanac.lib.mp_nautical import mp_twilight, mp_moonrise_set, mp_planetstransit, hor_parallax, mp_planetGHA, mp_sunmoon
else:
    # ... following is required for SINGLE-PROCESSING:
    from skyalmanac.lib.alma_skyfield import ariesGHA, venusGHA, marsGHA, jupiterGHA, saturnGHA, sunGHA, moonGHA, moonVD, sunSD, moonSD, vdm_Venus, vdm_Mars, vdm_Jupiter, vdm_Saturn, ariestransit, stellar_info, planetstransit, twilight, moonrise_set, moonage, moonphase, equation_of_time, getDUT1, find_new_moon
...
# >>>>>>>>>>>>>>>>>>>>>>>>
def mp_sunmoon_worker(Date, d_valNA, ts, spad, n):
    # split the work by date into 3 separate days
    sunmoondata = mp_sunmoon(Date, d_valNA, ts, spad, n)      # ===>>> mp_nautical.py
    return sunmoondata
...
def sunmoontab(Date, ts, spad):
    ...
    if config.MULTIpr and config.WINpf:
        ...
        partial_func = partial(mp_sunmoon_worker, Date, config.d_valNA, ts, spad)
...
def mp_twilight_worker(Date, ts, spad, lat):
    ...
    twi = mp_twilight(Date, lat, ts, spad)          # ===>>> mp_nautical.py
...
def mp_moonlight_worker(Date, ts, spad, lat, mstate):
    ...
    hemisph = 'N' if lat >= 0 else 'S'
    ml = mp_moonrise_set(Date, lat, mstate, ts, spad)   # ===>>> mp_nautical.py
...
\setcounter{page}{1}    % otherwise it's 2
    ...
    Pyephem was then in 'maintenance mode'. Clearly Pyalmanac needed adaptation to use Skyfield, and thus Skyalmanac was born. However its performance was poor regarding the calculation of 'events' such as: sunrise, sunset, moonrise, moonset, civil twilight start/end and nautical twilight start/end. An interim (faster) solution was required.\\[12pt]
    A hybrid application, originally named Skyalmanac, was developed using Ephem to calculate 'events' and Skyfield for the rest. This was indeed much faster at the cost of poorer 'event time' data. It took a while to find a better solution: multiprocessing, which was built into Skyalmanac. This now could compare to the execution times in Pyalmanac but with improved data.\\[12pt]
    New functionality was added to Skyalmanac, e.g. lunar phase as a graphic; Lunar Distance tables and charts. The original Skyalmanac is deprecated and has now been replaced with the latest Skyalmanac code, so Skyalmanac and Skyalmanac are now identical apart from the name.
