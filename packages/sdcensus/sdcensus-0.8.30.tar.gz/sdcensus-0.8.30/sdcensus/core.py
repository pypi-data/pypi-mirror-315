...
__version__ = pkg_resources.require("sdcensus")[0].version
...
class ACS5Client(ACSClient):
    ...
    # Must be updated with each new ACS release
...
class Census(object):
    ...
    def __init__(self, key, year=None, session=None):
        ...
        self.session.headers.update({
            'User-Agent': ('python-sdcensus/{} '.format(__version__) +
                           'github.com/SociallyDetermined/sdcensus')
        })
