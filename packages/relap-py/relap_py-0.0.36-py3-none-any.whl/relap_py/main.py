...
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def message(message: str, typ='WARNING'):
    """ writing messages on screen
    :message: the message to be printed
    :typ: the type ERROR or WARNING
    """
    print(30*'-'+typ+30*'-')
    print(message)
    print(67*'-')
...
def read_variables(input_variables_file):
    ...
    general_options = ['no', 'top_left', 'top_right', 'mid_left', 'mid_right', 'bottom_left', 'bottom_right']
    for item in variables['general']:
        if item not in general_options:
            message(item + ' is not a possible option for the general column in variables', 'ERROR')
    return variables
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def make_strip(input_variables_file, strip_filename: str, form="fmtout"):
    ...
    if form != 'csv' and form != 'fmtout' :
        message('argument 1 should be either csv or fmtout', 'WARNING')
        if relap_scdap():
            form = 'csv'
            message('Argument 1 is set to csv')
        else:
            form = 'fmtout'
            message('Argument 1 is set to fmtout')
...
def plot_var(variable: str, variables_df, cases_df, case_folder = '',
    ...
    if type(dataExp) == str and dataExp != '' :
        message('you first need to load the experimental data to an Ndarray with genfromtxt method','ERROR')
    if type(dataExp) != str :
...
def general_fig(variables, dataCalc0, dataExp='' , annotations=[], labels=[]):
    ...
    if type(dataExp) == str and dataExp != '' :
        message('For the experimental data, you first need to load the experimental \n data to an Ndarray with genfromtxt method','ERROR')
    ...
    message('annotation location coordinates are the following', 'INFO')
    for i in range(0,3):
        l = 0
...
def plot_variables(variable: list, variables_df, cases_df, ylabel=''):
...
def read_exp(expdata_loc: str):
    ...
    try : 
        ...
        message('Experimental data not used')
        dataExp = np.zeros(shape=(10, 10))
        exp_file_bool = False
    return dataExp
def print_ss(case, dataExp='', llista= False, end_ss = 0, length = 60):
    ...
    :dataExp: (optional) If '', no experimental data is to be printed
    ...
    def avg_ss(cas, var):
        ...
        if relap_scdap():
            ...
            key= 'time'
        i = 0
        while cas[key][i] < (end_ss - length):
             index_start = i
             i += 1
        while cas[key][i] < end_ss:
            index_end = i
            i += 1
    ...
    if type(dataExp) != str :
        for var in llista: 
            if var in dataExp.dtype.names :
               print('{:<20s}{:>8E}{:>18E}'.format(var, avg_ss(case, var), avg_ss(dataExp, var)))
            else :
