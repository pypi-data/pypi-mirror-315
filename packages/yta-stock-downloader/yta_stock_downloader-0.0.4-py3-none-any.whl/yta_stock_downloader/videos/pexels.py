from yta_stock_downloader.images.pexels import PexelsLocale
...
PEXELS_GET_VIDEO_BY_ID_URL = 'https://api.pexels.com/videos/videos/'
...
def search_pexels_videos(query: str, locale: PexelsLocale = PexelsLocale.ES_ES, ignore_ids: list[str] = [], per_page: int = 25):
    ...
    # TODO: Do I have any library to do this (?)
    r = requests.get(PEXELS_SEARCH_VIDEOS_URL, params = params, headers = HEADERS)

    # TODO: Apply 'ignore_ids'

    # TODO: How is this response (?)
    return r.json()['videos']

def get_first_pexels_video(query: str, locale: PexelsLocale = PexelsLocale.ES_ES, ignore_ids = []):
    """
    ...
    'ignore_ids' ids list.
    """
    # TODO: Obtain the first one not ignored
    videos = search_pexels_videos(query, locale, ignore_ids)

    # TODO: What if no videos available, use next page (?)
    return videos

def get_pexels_video_by_id(id: str):
    """
    Obtain the pexels video with the provided 'id'
    (if existing).
    """
    if not PythonValidator.is_string(id):
        raise Exception('No "id" provided.')
    
    r = requests.get(f'{PEXELS_GET_VIDEO_BY_ID_URL}{str(id)}', headers = HEADERS)

    # TODO: Throw exception if not found
    # TODO: What does this send as response (?)
    return r.json()

def download_first_pexels_video(query: str, locale: PexelsLocale = PexelsLocale.ES_ES, ignore_ids: list['str'] = [], output_filename: Union[str, None] = None):
    if not PythonValidator.is_string(query):
        raise Exception('No "query" provided.')

    locale = PexelsLocale.to_enum(locale)

    # TODO: Do I need to check anything with 'ignore_ids' (?)

    # TODO: Obtain the first one to download
    first_video = get_first_pexels_video(query, locale, ignore_ids)

    if not first_video:
        raise Exception('No "video" found with the provided "query" and "locale" parameters.')

    if not output_filename:
        output_filename = create_temp_filename('tmp_pexels_video.mp4')

    # TODO: Use actual video 'url' (how is it provided here?)
    Downloader.download_video(first_video['url'], output_filename)
