...
def search_pexels_images(query: str, locale: PexelsLocale = PexelsLocale.ES_ES, ignore_ids: list[str] = []) -> list[PexelsImage]:
    """
    ...
    page_results = __search_pexels_images(query, locale, RESULTS_PER_PAGE, 1)
...
def get_first_pexels_image(query: str, locale: PexelsLocale = PexelsLocale.ES_ES, ignore_ids: list[str] = []) -> PexelsImage:
    ...
    results = search_pexels_images(query, locale, ignore_ids, per_page = 1)

    if not results:
        return None
        #raise Exception('No available images for the provided "query" and "locale", and also considering the provided "ignore_ids".')
