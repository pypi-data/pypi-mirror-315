...
from yta_stock_downloader.videos.pixabay import get_first_pixabay_video, download_first_pixabay_video, download_random_pixabay_video
...
class Pixabay:
    ...
    @staticmethod
    def download_random_video(query: str, ignore_ids: list[int] = [], output_filename: Union[str, None] = None):
        """
        Download a random video from the Pixabay
        provided and the given 'query' (if existing)
        avoiding the ones in the 'ignore_ids' list.
        ...
        not possible.
        """
        return download_random_pixabay_video(query, ignore_ids, output_filename)