...
class KalaTorch:
    ...
    def __init__(self, model_type="basic", input_size=None, output_size=None, hidden_layers=None):
        ...
        if self.model_type == "basic":
            ...
            self.model = self._build_rnn_model(input_size, output_size)
    ...
    def _build_rnn_model(self, input_size, output_size):
        """Build a simple recurrent neural network."""
        class RNNModel(nn.Module):
            def __init__(self):
                super(RNNModel, self).__init__()
                self.rnn = nn.RNN(input_size=input_size, hidden_size=20, num_layers=2, batch_first=True)
                self.fc = nn.Linear(20, output_size)

            def forward(self, x):
                out, _ = self.rnn(x)  # Only take the output tensor
    ...
    def train_step(self, x, y):
        ...
        print(colored(f"Train Loss: {loss.item():.4f}", "green"))
        return loss.item()
    ...
    def evaluate(self, x, y):
        ...
        with torch.no_grad():
            predictions = self.model(x)
            loss = self.loss_fn(predictions, y)
        print(colored(f"Evaluation Loss: {loss.item():.4f}", "blue"))
        return loss.item()
...
print(colored("KalaTorch module loaded successfully!", "yellow"))
