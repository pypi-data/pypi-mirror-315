...
class Mqttop:
    def __init__(self, host: str, port: int, clientid: str = '',
                 user: str = '', passwd: str = '',
                 data_length: int = 100,
                 transport: Literal["tcp", "websockets", "unix"] = "tcp", protocol: int = 3,
                 logger = None):
        """
        ...
        if logger is None:
            self.__logger = set_log("hzgt.mqtt", os.path.join("logs", "smtp.log"), level=2)
        else:
            self.__logger = logger

        if len(self.clientid) == 0 or self.clientid is None:
            self.__client = mqtt.Client(client_id="", transport=transport, protocol=self.protocol)
        else:
    ...
    def set_will(self, will_topic: str, will_msg: str):
        ...
        self.__logger.info(f"遗嘱信息: will_topic[`{will_topic}`] will_msg[`{will_msg}`] 已设置")
    ...
    def __on_connect_v3(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            # 连接失败并显示错误代码
            self.__logger.error(f'连接出错 rc={rc} 错误代码: {connection_status.get(rc, f"未知返回码{rc}")}')
            self.bool_con_success = False
