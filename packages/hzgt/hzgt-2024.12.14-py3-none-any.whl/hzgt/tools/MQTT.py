...
logger = set_log("hzgt.mqtt", "mqtt.log", level=1)


class Mqttop:
    def __init__(self, host: str, port: int, clientid: str = '',
                 user: str = '', passwd: str = '',
                 data_length: int = 100, protocol=3):
        """
        ...
        if len(self.clientid) == 0 or self.clientid is None:
            self.__client = mqtt.Client(client_id="", protocol=self.protocol)  # 创建对象, 强制clean_session=True
        else:
            self.__client = mqtt.Client(client_id=self.clientid, protocol=self.protocol)  # 创建对象
        logger.info(f"MQTT服务器[协议版本({self.protocol})]连接信息: host[`{self.host}`] port[`{self.port}`]")
    ...
    def set_will(self, will_topic: str, will_msg: str):
        ...
        logger.info(f"遗嘱信息: will_topic[`{will_topic}`] will_msg[`{will_msg}`] 已设置")
    ...
    def __on_disconnect(self, client, userdata, rc):
        ...
        """
        if self.bool_con_success:
            logger.info(f"MQTT连接已断开 返回码rc={rc}")
        self.bool_con_success = False
    ...
    def __on_connect_v3(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            # 连接成功
            logger.info(f'MQTT服务器 连接成功!')
            self.bool_con_success = True
        else:
            # 连接失败并显示错误代码
            logger.error(f'连接出错 rc={rc} 错误代码: {connection_status.get(rc, f"未知返回码{rc}")}')
            self.bool_con_success = False
    ...
    def __on_connect_v5(self, client, userdata, flags, rc, properties):
        ...
        if rc == 0:
            logger.info(f'MQTT服务器 连接成功!')
            self.bool_con_success = True
            logger.info(f"MQTT服务器 返回信息: {properties}")
        else:
            logger.error(f'连接出错 rc={rc} 错误代码: {connection_status.get(rc, f"未知返回码{rc}")}')
            self.bool_con_success = False
            logger.error(f"MQTT服务器 返回信息: {properties}")
    ...
    #     logger.info(f"订阅主题: {userdata, mid, properties, rc}")

    # 取消订阅主题事件
    # def __on_unsubscribe(client, userdata, mid, properties):
    #     logger.info(f"取消订阅主题: {properties}")

    # 发布信息事件
    # def __on_publish(client, userdata, mid, rc):
    #     logger.info(f"发布信息: {rc}")
    ...
    def publish(self, topic: str, msg: str, qos: int = 0,
        ...
        if status == 0 and bool_log:
            logger.info(f"发送成功 TOPIC[`{topic}`]  MSG[`{repr(msg)}`]")
        elif bool_log:
            logger.error(f"发送失败 TOPIC[`{topic}`]  MSG[`{repr(msg)}`]")
    ...
    def getdata(self, topic=None, index: int = 0, bool_del_data: bool = True, bool_all: bool = False):
        """
        获取接收到的数据
