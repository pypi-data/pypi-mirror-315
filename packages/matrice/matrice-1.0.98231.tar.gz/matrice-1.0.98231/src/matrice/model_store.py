...
def list_public_model_families(
    session, project_type="classification", page_size=10, page_num=0
):
    """
...
def list_private_model_families(
    session, project_id=None, project_name=None, page_size=10, page_num=0
):
    """
...
def get_all_models(
    session, project_id=None, project_name=None, project_type="classification"
):
    """
...
def get_all_model_families(
    session, project_id, project_name=None, project_type="classification"
):
    """
...
def get_automl_config(
    ...
    for model_data in data:
        ...
        model_config = {
            item["keyName"]: item["selectedValues"] for item in action_config_list
        }
        ...
        model_arch = ModelArch(
            session=session, model_family_name=model_family_name, model_key=model_key
        )
...
class ModelArch:
    ...
    def __init__(
        ...
        # Assert that either both model_family_name and model_key are provided, or model_arch_id is provided
        assert (
            model_family_name is not None and model_key is not None
        ) or model_arch_id is not None, (
            "Either both model_family_name and model_key must be provided, "
            "or model_arch_id must be provided."
        )
    ...
    def get_export_formats(self):
        ...
        if self.model_arch_id is not None:
            path = (
                f"/v1/model_store/get_export_formats?modelArchId={self.model_arch_id}"
            )
        else:
    ...
    def get_train_config(
        self, experiment_id, tuning_type="default", model_checkpoint="auto"
    ):
        """
        ...
        Example
        -------
        >>> train_config = model_arch.get_train_config("experiment123")
    ...
    def get_export_config(self, export_format):
        ...
        """
        model_export_config, error, message = self.get_export_action_config(
            export_format
        )
        if error:
            return None
...
class ModelFamily:
    ...
    def get_model_archs(self, model_name=None, model_key=None):
        ...
        if self.model_family_id:
            ...
            path = f"/v1/model_store/get_models_by_modelfamily?modelFamilyName={self.model_family_name}"  # TODO: not working
            ...
            model_arch_list = [
                {
                    ...
                    "model_arch_instance": ModelArch(
                        self.session, self.model_family_name, item["modelKey"]
                    ),
                }
...
class BYOM:
    ...
    def _load_config(self, config):
        if isinstance(config, str) and os.path.isfile(config):
            ...
            raise ValueError(
                "Invalid config. Must be a dictionary or a valid file path."
            )
    ...
    def _get_model_action_config_id(
        self, model_family_name, model_key, action_type, export_format=None
    ):
        model_arch = ModelArch(self.session, model_family_name, model_key)
    ...
    def add_model_family(self, model_family_info):
        ...
        headers = {"Content-Type": "application/json"}
        resp = self.rpc.post(path=path, headers=headers, payload=model_family_info)
    ...
    def update_model_family(
        self, model_family_name, model_family_info
    ):
        """
    ...
    def delete_model_action_config(self, model_family_name, model_key, action_type, action_config, export_format=None):
        model_action_config_id = self._get_model_action_config_id(model_family_name, model_key, action_type, action_config, export_format)
    ...
    #     # Add model info
    #     path = "/v1/model_store/add_model_arch"
    #     resp = self.rpc.post(
    #         path=path,
    #         headers={"Content-Type": "application/json"},
    ...
    #     return handle_response(
    #         resp,
    #         "New model info created",
    #         "An error occurred while creating model info",
    #     )
    ...
    #         model_family_name, model_key, action_type, action_config, export_format
    #     )
    #     path = f"/v1/model_action_config/{model_action_config_id}"

    #     payload = {
    #         "_idModelInfo": model_action_config_id,
    #         "actionType": action_type,
    #         "actionConfig": action_config,
    #         "dockerContainer": action_config.get("dockerContainer", ""),
    #         "dockerCredentials": action_config.get("dockerCredentials", {}),
    #         "privateDocker": action_config.get("privateDocker", False),
    #         "modelCheckpoint": action_config.get("modelCheckpoint", ""),
    #         "actionCallSignature": action_config.get("actionCallSignature", "")
    #     }
    ...
    def use_docker_image_from_public_model_family(
        self, model_family_name, docker_image_model_family_name, project_type
    ):
        payload = {
            "sourceModelFamilyId": self._get_model_faimly_id(
        ...
        docker_repos_info, error, message = self.get_public_model_families_docker(
            project_type
        )
        if error:
            return None, error, message
        ...
        if not payload.get("dockerRepo"):
            return None, "Docker repo not found", "Docker repo not found"
    ...
    def upload_model_family_codebase(
        ...
        payload = {
            ...
            "pytorch_version": cuda_version,  # TODO: check why CUDA version is passed as pytorch version, and make sure imageName is correct
            "accountNumber": self.account_number,
            "_idModelFamily": model_family_id,
        ...
        return handle_response(
            resp, "Model code base added", "Error adding model code base"
        )
    ...
    def get_model_family_codebase_details(self, model_family_name):
        ...
        resp = self.rpc.get(path=path)
        return handle_response(
            resp, "Code details retrieved", "Error getting code details"
        )
    ...
    def get_test_cases_by_type(self, model_family_name, test_cases_type="default"):
        ...
        if not error:
            return self._parse_test_cases(data), error, message
        else:
            return None, error, message

    def start_test_cases(self, model_family_name, model_key, project_type, test_cases):
        ...
        for test_case in test_cases:
            ...
            payload["modelActions"].append(model_aciton)

        path = "/v1/model_store/add_model_family_testcases" # TODO get the url and test
        headers = {"Content-Type": "application/json"}
        resp = self.rpc.post(path=path, headers=headers, payload=payload)
        return handle_response(
            resp,
    ...
    def get_started_test_cases(
        self, model_family_name, model_key=None
    ):
    ...
    def get_model_family_actions(self, model_family_name):
        def _format_actions(actions):
            ...
            for action_dict in actions:
                for model_key, model_actions in action_dict.items():
                    if model_key not in formated_actions:
                        formated_actions[model_key] = []
                    for action_type, action_configs in model_actions.items():
        ...
        if not error:
            return _format_actions(data), error, message
        else:
            return None, error, message

    def integrate_model_actions(
        self, model_family_name, model_key, model_actions
    ):
