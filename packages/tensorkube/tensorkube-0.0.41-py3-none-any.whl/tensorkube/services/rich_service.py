...
def list_tensorkube_deployments(env_name: Optional[str] = None, all: bool = False, old: bool = False):
    ...
    virtual_services = get_all_virtualservices(namespace=namespaces)
    for service in services:
        ...
        if old:
            ...
            domains = find_domains_for_service(virtualservices=virtual_services,service_name=service_name)
            custom_domains = list(filter(is_custom_domain, domains.domains))
            if custom_domains:
...
def display_dns_records_table(domain: str, validation_records: dict):
    ...
    console = Console()
    console.print(table)
    console.print("\n[yellow]Please configure these records in Cloudflare[/yellow]")
