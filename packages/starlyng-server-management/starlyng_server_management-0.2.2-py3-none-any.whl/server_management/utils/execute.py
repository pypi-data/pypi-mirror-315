...
from server_management.utils.bcm import (
...
from server_management.utils.file import set_last_reboot, create_bcm_sel_list_logs
from server_management.utils.connections import get_vast_listed_bcm_server_hostnames
...
def execute_smart_reboot_for_offline_hostnames(offline_ssh_servers: List[Server], bcm_servers: List[BCMServer], server_mgmt_dir: str) -> None:
    ...
    ssh_hostnames = {server.hostname for server in offline_ssh_servers}
    bcm_hostnames = {server.hostname for server in bcm_servers}
    if not ssh_hostnames.issubset(bcm_hostnames):
    ...
    offline_bcm_servers = [
        bcm_server for bcm_server, ssh_server in zip(bcm_servers, offline_ssh_servers)
        if ssh_server.hostname in [s.hostname for s in offline_ssh_servers]
    ]
    ...
    power_on_bcm_servers(offline_ssh_servers)

    # Log errors and clear logs
    logs_for_bcm_servers = sel_list_bcm_servers(offline_ssh_servers)
    create_bcm_sel_list_logs(logs_for_bcm_servers, server_mgmt_dir)
    sel_clear_bcm_servers(offline_ssh_servers)
