...
import dart_physics.server as server 
# import warnings
...
def run_server():
    server.app.run(host='0.0.0.0', port=5012)
...
class MJTeleop: 
    ...
    def post_physics_control(self): 
        ...
        if control_message is not None: 
            ...
            if "switch_task" in control_message:

                sim_dict = self.prepare_dict_to_send_avp() 
                # move the y value of each body by -10.0
                for key, value in sim_dict.items():
                    value[1, -1] -= 10.0

                self.streamer.set_sim_states(sim_dict) 

                task, robot = control_message.split(":")[1:]
                self.switch_task(robot, task) 
                self.qtarget = self.solve_ik()
            ...
            if control_message == "stop_recording":
                ...
                dexhub.save(success = False, local_directory = "./logs") 

                self.trajectory = [] 
                print(f"[RECORDING CTRL] Saved failure trajectory to logs/{len(files):03d}.npy")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--robot", default = "dual_panda")
