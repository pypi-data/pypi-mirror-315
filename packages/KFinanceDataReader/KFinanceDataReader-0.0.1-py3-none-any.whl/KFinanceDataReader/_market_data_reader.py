import pandas as pd
from . import FinanceDataReader as finance_data_reader
    ...
    def get_kospi_trader_df(self, start_date, end_date):
        ...
        buy_trade_df = pd.concat(
            [
                ...
                .sum(axis=1)
                .rename('기관'),
                kospi_buy_trade_df.loc[:, ['외국인', '기타외국인']].sum(axis=1).rename('기관'),
        ...
        sell_trade_df = pd.concat(
            [
                ...
                .sum(axis=1)
                .rename('기관'),
                kospi_sell_trade_df.loc[:, ['외국인', '기타외국인']].sum(axis=1).rename('기관'),
