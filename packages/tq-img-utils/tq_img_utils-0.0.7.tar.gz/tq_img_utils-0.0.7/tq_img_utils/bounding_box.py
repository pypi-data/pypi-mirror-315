...
class BoundingBox:
    ...
    The area of the bounding box couldn't be zero(left==right or top==bottom).
    """
    x: int
    y: int
    width: int
    ...
    def __init__(self, x: int, y: int, width: int, height: int,
        ...
        self.width = width
        self.height = height
        if constraint is not None:  # constraint 包含最低值为 0 的限制
    ...
    def union(self, e: 'BoundingBox') -> 'BoundingBox':
        ...
        union bounding format: (left_min, right_max, top_min, bottom_max). (not the traditional union of two bounding boxes)
        """
