...
from api_project import CASE_CONFIG_PATH, CASE_SCRIPT_DIR_PATH, CASE_DATA_DIR_PATH, FEED, CONFIG_DIR_PATH, FAVICON_PATH
from api_project import TAB_SPACE
from saf.utils.Curl2Object import Curl, Template
from api_project.__gui_version__ import __version__
...
class Controller:
    ...
    def run_convert(self,evt):
        ...
        if  file_path not in ['',  '请选择待转换的cURL文件...（必填项）']:
            ...
            for request in curl.group:
                ...
                API_NAME = _API_NAME_.upper().replace('.', '_') if _API_NAME_ != '' else f"API_{os.environ.get('API_COUNT')}"
                ...
                try:
                    ...
                    self.ui.tk_text_log.insert('end', f"{os.path.split(CASE_CONFIG_PATH)[1]} 写入{newline}失败：{e}\n")
                    self.logger.error(f"{os.path.split(CASE_CONFIG_PATH)[1]} 写入{newline}失败：{e}")
                ...
                try:
                    with open(CASE_SCRIPT, 'w', encoding='utf-8') as fw:
                        if self.ui.radio_value.get() == 1:
                            ...
                            fw.write(Template.requests_pytest_allure_template_at(request=request))
                        fw.close()
                    self.ui.tk_text_log.insert('end', f"test_{API_NAME.lower()}.py 写入成功！\n")
                    self.logger.info(f"test_{API_NAME.lower()}.py 写入成功！")
                except Exception as e:
                    self.ui.tk_text_log.insert('end', f"test_{API_NAME.lower()}.py 写入失败：{e}\n")
                    self.logger.error(f"test_{API_NAME.lower()}.py 写入失败：{e}")
                ...
                try:
                    ...
                    self.ui.tk_text_log.insert('end', f"{API_NAME}.csv 写入失败：{e}\n")
    ...
    def find_allure_path(self):
        ...
        if system == "Windows":
            try:
                result = subprocess.run(["where", "allure"], capture_output=True, text=True, check=True)
            ...
            pass #raise NotImplementedError(f"Unsupported operating system: {system}")
