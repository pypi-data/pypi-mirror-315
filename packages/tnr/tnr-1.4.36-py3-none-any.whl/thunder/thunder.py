...
@click.option('--template', type=click.Choice(['base', 'comfy-ui']), default='base', help='Template to create client instance with')
...
def connect(tunnel, instance_id=None):
    ...
    for curr_instance_id, metadata in instances.items():
        if curr_instance_id == instance_id:
            ...
            port = OPEN_PORTS.get(metadata.get('template', 'base'))
            host_alias = f"tnr-{instance_id}"
            exists, prev_ip = utils.get_ssh_config_entry(host_alias)
            if not exists:
                utils.add_instance_to_ssh_config(ip, keyfile, host_alias, port=port)
            else:
                if ip != prev_ip:
            ...
            if platform.system() == "Windows":
                ...
                tunnel_command = ""
                for port in tunnel:
                    tunnel_command += f"-L {port}:localhost:{port} "
                subprocess.run(
                    [
                        f"ssh \
                        {tunnel_command}\
